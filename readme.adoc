...
The metadata binds the secrets to a specific cryptographic algorithm and defines usage and access policies, thereby preventing the secrets from being used for other algorithms and, optionally, by unauthorized processes, security states, or other entities.

Cryptographic Contexts reside in one of two forms:

. Within protected architectural containers called *Cryptographic Registers*. These conceal their contents once the secrets and their metadata are written into them.
...
ACE can only utilize keys when they reside in a Cryptographic Register.
The architecture protects these registers by providing no means to export their secrets as plaintext, and it ensures keys are used strictly according to the policies in their metadata.
