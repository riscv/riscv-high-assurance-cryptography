...
- **m4 — CSR naming drift.** Priv CSR table says `mcrstatus`/`scrstatus`/`vscrstatus`; the defining section says `mlcrstatus`/`slcrstatus`/`vslcrstatus`; prose also uses `*lcrstate` and `_X_crstatus`. → One name everywhere.
