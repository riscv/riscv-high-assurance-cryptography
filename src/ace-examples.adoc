...
To address this, the `*crstatus` CSRs can be used to enable lazy context reloading.
When this requires freeing one or more CRs first, system software decides which contexts to evict.

==== Key Management and Provisioning Applications

Software environments that implement key management operations for other domains require their own CSK. A system may contain multiple such software environments.
...
[arabic, start=5]
. Set the CSK to the KMA’s CSK.
. Re-import the SCC into a CR
. Change the CSK to match the Requester domain's CSK.
