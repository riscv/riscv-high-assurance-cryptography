...
|   [8:1] |   8  | Algorithm              | Algorithm (such as primitive or mode).
|  [10:9] |   2  | SC_Protection          | Level of Side Channel Protection provided. +
The field's encoding is described in <<ACE-SC-protection-levels>>.
...
See also <<ACE-what-are-locales>> below for more details.
| [20:19] |   2  | Completion             | Defined in <<ACE-CC-complete>>. +
Used to determine whether operations must be restarted.
...
Algorithm defined.
| [28:25] |   4  | AC_Policy_1            | Forbid USE of this CC in User, Supervisor or Virtual Supervisor, Hypervisor, resp., Machine Mode if bit 25, 26, 27, resp. 28, is set.
...
See also <<stage-no-H-no-VMID>> below.
|    [31] |   1  | Reserved               | Undefined.
| [47:32] |  16  | AC_Policy_3            | Zero if bit 29 unset, else `ASID` to enforce.
...
[[ACE-what-are-locales,Remark {counter:remark}]]
*Remark {counter:remark}.* _Locales are used to restrict CCs to specific classes of devices or to a boot-session.
They are implemented as secret values that are combined with the CTK to compute the effective key to be used in export and import operations.
Such values are stored in _Locale Secrets Table_ (cf <<ACE-Locales>>), which consists of fixed system-wide values and of MM-programmable ones.
In the metadata, the Locale is expressed as an index into this table._
...
* `content_serialize`: Serialize a CC's content to a sequence of 128-bit blocks to be encrypted for export.
* `content_deserialize`: Takes a sequence of decrypted SCC blocks, each 128 bits in size, that come after the the metadata, nonce and authentication tag, and deserializes it into a CC's content.
...
We first define the RFC8452 Key Derivation and the Locale CTK Derivation functions.
...
content : array of bits(128);
M : array of bits(128);
let N : bits(96) = random(96 bits);
Kenc : bits(256);
Kauth : bits(128);
{Kenc, Kauth} = RFC8452_KeyDeriv(CTK);
let SIV : bits(96) = Poly1305(Kauth, N, metadata @ content)[95:0];

M[0][127:0] = N[63:0] @ metadata[63:0];
M[1][127:0] = SIV[95:0] @ N[95:64];
if (last_block_fractional) then last = num_blocks(content)-1 else last = num_blocks(content)-1
foreach(i from 0 to last) {
    M[i+2][127:0] = content[i] xor AESE(Kenc, SIV @ (binary(i)[31:0]));
}
if (last_block_fractional) then {
    M[last+2][63:0] = content[last] xor AESE(Kenc, SIV @ (binary(last+1)[31:0]))[63:0];
}
----

Due to the computational cost of the derivation functions, implementations may cache recently used derived keys for both the base CTK and locale-derived keys.
...
Certain format details are fixed across algorithms. All pure ciphers use a single key, regardless of whether they employ a normal or threshold implementation; threshold implementations are always reduced to a single key. Some modes follow the same convention, while others require algorithm-specific state formatting.
