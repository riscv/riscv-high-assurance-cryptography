...
bytes[7:0] contain contain the 64-bit metadata section +
bytes[19:8] contain the nonce (96 bits) +
bytes[31:20] contain the SIV (96 bits) +
bytes[n+32,32] contain `content_serialize(CC.content)`, where n is the number of bytes in `content_serialize(CC.content)`.

Only the serialization of the content is encrypted, while both metadata and content are authenticated, as described in <<ACE-AEAD>>.
