...
| [19:15] |   5  | Locale                 | Field which is either -1 or an index into the Locale Secrets Table. | Public
| [21:20] |   2  | Complete               | Whether the CC is complete (00) or interrupted while being configured (01), imported (10) or exported (11). Necessary to know whether to restart operations. | Public
| [27:22] |   6  | Stage/Direction        | Which internal stage the algorithm is, as defined by ace_stage or implicitly. Must be ZERO on configuration. This is algorithm defined. | Private
| [31:28] |   4  | AC Policy 1            | Can a User (if bit 28 set)/Supervisor or Virtual Supervisor (bit 29 set)/Hypervisor (bit 30 set)/Machine (bit 31 set) USE this key? | Private
| [33:32] |   2  | AC Policy 2            | If bit 32, resp., 33, is set, restrict USE by `ASID`, resp., `VMID`. | Private
| [49:34] |  16  | AC Policy 3            | Zero if bit 32 unset, else `ASID` to enforce.  | Private
| [63:50] |  14  | AC Policy 4            | Zero if bit 33 unset, else `VMID` to enforce.  | Private
|===

*Remark {counter:remark}*:: The short metadata format contains 22 bits of public metadata and 42 bits of private metadata.
...
|===
|     Field | Bits | Name                   | Description | Type
|       [0] |   1  | Format                 | 1 for short metadata format (64-bit) | Public
|     [8:1] |   8  | Algorithm              | Which primitive or mode (msb = 0 for architected algorithms, msb = 1 for custom ones). | Public
|    [10:9] |   2  | Side Channel Protected | Set if we require the implementation of the algorithm to provide Side Channel Protection. | Public
|   [12:11] |   2  | Direction Policy       | Use for encryption/decryption/both (only for enc/dec primitives or modes). | Public
|   [14:13] |   2  | Key Type               | By explicit value / Immutable (index) / Internally generated. | Public
...
|   [49:34] |  16  | AC Policy 3            | Zero if bit 32 unset, else `ASID` to enforce.  | Private
|   [63:50] |  14  | AC Policy 4            | Zero if bit 33 unset, else `VMID` to enforce.  | Private
|   [67:63] |   4  | Version                | Metadata format version | Public
