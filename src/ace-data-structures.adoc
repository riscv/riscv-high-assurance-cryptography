...
===== Using the Extended Metadata Format
...
We first define the RFC8452 Key Derivation

.RFC8452 Key Derivation
[source,sail,subs="+quotes,verbatim"]
----
typedef derived_keys = {
  enc : bits(256),
  auth : bits(128),
}
...
function RFC8452_KeyDeriv(K,L) = {
  deriv : derived_keys = {
    AESE256(K, L ^ (0[125:0] @ 0b101))[63:0] @ AESE256(K, L ^ (0[125:0] @ 0b100))[63:0] @
...
[source,sail]
----
let P : bits(content_length) P = content;
let N : bits(96) N = random(96 bits);
let Kenc : bits(256)
let Kauth : bits(128)
{ Kenc, Kauth } = RFC8452_KeyDeriv(CTK,0)
let SIV : bits(96) = Poly1305(Kauth, N, metadata @ P)[95:0];
M[0][127:0] = N[63:0] @ ((metadata[63:21] xor AESE(Kenc, SIV @ 0x0000 0000)[63:21])
                      @ metadata[20:0];
...
{ Kenc, Kauth } = RFC8452_KeyDeriv(CTK,0)
let SIV : bits(96) = Poly1305(Kauth, N, metadata @ P)[95:0];
let tmp : bits(128) = AESE256(Kenc, SIV @ 0x0000 0000);
M[0][127:0] = (metadata[127:84] xor tmp[127:84]) @ metadata[83:64]
              @ (metadata[63:21] xor tmp[63:21]) @ metadata[20:0];
M[1][127:0] = 0x00000000 @ N;
M[2][127:0] = SIV @ 0x00000000;
foreach(i from 0 to num_blocks(P)-1) {
