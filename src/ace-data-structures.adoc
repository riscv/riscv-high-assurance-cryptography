...
|     [0] |   1  | Format                 | 0 for this version of the metadata format. Future versions of the metadata format will have a 1 in this position and may also be longer than 64 bits. | Public
...
| [24:21] |   4  | Stage/Direction        | Internal current state of the state machine of the algorithm. Must be zero on configuration. Algorithm defined. | Private
| [28:25] |   4  | AC Policy 1            | Forbid USE of this CC in User, Supervisor or Virtual Supervisor, Hypervisor, resp., Machine Mode if bit 25, 26, 27, resp. 28, is set. | Private
| [30:29] |   2  | AC Policy 2            | If bit 29, resp., 30, is set, then restrict USE by `ASID`, resp., `VMID`. +
...
| [63:48] |  16  | AC Policy 4            | Zero if bit 33 unset, else `VMID` to enforce. Only 14 bits used, two most significant bits currently zero. | Private
|===
...
Custom algorithms may offer stronger side channel protection guarantees than those specified in the above encoding.
...
The "AC Policy 4" field is 16-bits long for compatibility with architectures employing 16-bit `VMID` fields (for instance, in heterogeneous systems).
...
Filtering use by Supervisor Domain or World ID is not supported.
Reconfiguring the CTK is the recommended approach for such isolation requirements.
