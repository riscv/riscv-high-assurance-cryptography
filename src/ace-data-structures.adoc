...
Encoding described in <<ACE-SC-protection-levels>>.
...
| [24:21] |   4  | Stage (also Direction) | Internal current state of the state machine of the algorithm. Must be zero upon configuration, otherwise algorithm specific.
...
The size of a SCC is also always a multiple of 8 bytes.

To describe how it is generated, we first define the RFC8452 Key Derivation and the Locale CTK Derivation functions.
...
|  7   | ace_err_incomplete_CC    | Software tried to use an incomplete CC as a source in a `ace.exec`, `ace.stage`, `ace.input`, `ace.output`, `ace.clone`, `ace.derive`, `ace.harden`, `ace.restrict` operations. +
This can be used to trigger an explicit lazy resumption of a `ace.init` or `ace.import` operation as an alternative to trapping.
|===
