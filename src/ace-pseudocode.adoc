...
octet at the lowest address in its least significant bits, and `@` places its left
operand in the more significant bits.
...
// The next operations set the nonce
ace.setst K0, #ace_state_set_aux_value
V0 ← nonce
ace.exec K0, V0
...
// Compute AD contribution to Auth Data
V0 ← SIV
ace.setst K0, #ace_state_hash_absorb, V0
foreach(i from 0 to len_in_bytes(AD)/16-1) {
   V0 ← AD[16*i .. 16*i+15]
   ace.exec K0, V0                       // Absorb it into hash
}
...
ace.setst K0, #ace_state_hash_finalize, X1
if (X1 != 0) then {
...
ace.setst K0, #ace_state_last_block, X1
if (X1 != 0) then {
...
ace.setst K0, #ace_state_hash_finalize
