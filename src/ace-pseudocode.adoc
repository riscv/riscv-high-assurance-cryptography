...
ace.setstate K0, #ace_state_encrypt         // Configure CR for encryption
ace.getstate X1, K0
if (X1 >= 24) then { handle error }
...
V1 ← nonce
ace.setstate K0, #ace_state_operate, V1
...
V0 ← IV
ace.setstate K0, #ace_state_encrypt, V1     // Configure CR for encryption
...
// The next operations set the nonce
ace.setstate K0, #ace_state_set_aux_value
V0 ← nonce
ace.exec K0, V0

// Compute AD contribution to Auth Data
ace.setstate K0, #ace_state_hash_absorb
...
ace.setstate K0, #ace_state_encrypt
foreach(i from 0 to len_in_bytes(PT)/16-1) {
   V0 ← PT[16*i .. 16*i+15]
   ace.exec V1, K0, V0
   CT[16*i .. 16*i+15] ← V1
...
ace.setstate K0, #ace_state_enc_tag_finalize, V1
...
ace.setstate K0, #ace_state_dec_tag_finalize
V1 ← bin(len_in_bits(AD), 64) @ bin(len_in_bits(CT), 64)
ace.exec V2, K0, V1                      // V2 contains the finalized hash

V1 ← tag_from_payload
ace.setstate K0, #ace_state_hash_verify, V1
ace.getstate X1, K0
if (X1 != ace_state_success) then { handle error }       // Do not use the plaintext if error occurred
----
...
ace.setstate K0, #ace_state_enc_tag_finalize
ace.exec V2,K0
tag ← V2
...
V0 ← nonce
ace.setstate K0, #ace_state_set_aux_value, V0

// Compute AD contribution to Auth Data
V0 ← SIV
ace.setstate K0, #ace_state_hash_absorb, V0
foreach(i from 0 to len_in_bytes(AD)/16-1) {
   V0 ← AD[16*i .. 16*i+15]
   ace.exec K0, V0                       // Absorb it into hash
}
...
// The next operations set the nonce
X1 ← N_len
ace.setstate K0, #ace_state_set_aux_value, X1
V0 ← N
ace.exec K0, V0

// The next state change also sets the tag length
X1 ← tag_len
ace.setstate K0, #ace_state_hash_absorb, X1
...
ace.setstate K0, #ace_state_hash_finalize, X1
...
ace.setstate K0, #ace_state_last_block, X1
if (X1 mod 128 != 0) then {
   V0 ← zeros(128)
...
----
V1 ← tag
ace.setstate K0, #ace_hash_verify, V1
ace.getstate X1, K0
if (X1 !== ace_state_success) then { handle error }       // do not use the plaintext if error
----
...
ace.setstate K0, #ace_hash_absorb
...
X1 ← remainder
ace.setstate K0, #ace_hash_last_block, X1
V0 ← M[16*blocks .. 16*blocks+15]
ace.exec K0, V0

ace.setstate K0, #ace_hash_hash_finalize
