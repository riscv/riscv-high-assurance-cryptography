...
In order to derive the derived encryption and authentication keys, we modify the AES-GCM-SIV key derivation function by adding a constant value to the input blocks, as described in <<ACE-formats>>.
...
The MM has the responsibility to "virtualize" the seventh locale secret.
...
Consequently, while `ace.export` remains functional on the same device to support context switching, an `ace.import` of the resulting blob on a device from a different OEM will trigger an authentication failure (provided that the OEM Secrets are, in fact, different)._
