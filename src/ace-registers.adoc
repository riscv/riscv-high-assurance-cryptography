...
==== ACECTK field in `mconfigptr`

[.red]#We will use the data structure pointer by `mconfigptr` in order to track optional features of ACE including whether the `acectk` CSR block is implemented, and if not whether whether the value is ephemeral or fixed.#
