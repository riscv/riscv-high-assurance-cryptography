...
==== ACECTKMODE field in mconfigptr
...
The fine-grained tracking of activeness and dirtiness of individual CCs is done using the `aceccactive` and `aceccdirty` CSRs, cf. <<ACE-tracking-cc-dirtiness>>,
...
If there is a write to `aceenable`, the following happens to `aceccvalid` and `aceccdirty`:

* If 0 is written to `aceenable` and the parent's domain `*status.ACES`=Enabled, then all CCs are cleared; both `aceccvalid` and `aceccdirty` are set to 0, and `*status.ACES` is set to Disabled-Clean.
* If 1 is written to `aceenable` and
 ** if the parent's domain `*status.ACES`=Disabled-Clean, then all CCs are cleared and both `aceccvalid` and `aceccdirty` are set to 0.
 ** if the parent's domain `*status.ACES`=Disabled-Clobbered, then the hart traps into the parent process to allow the latter to finish saving the state and enseure that both `aceccvalid` and `aceccdirty` are set to 0, and finally set `*status.ACES`=Disabled-Clean.
