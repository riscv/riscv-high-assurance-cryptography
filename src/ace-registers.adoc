...
If CTK can be programmed by Machine Mode, then eight 32-bit CSRs are available, called `acectk0`, `acectk1` .. `acectk7`, to allow Machine Mode to program the CTK.
...
The `acectk` block reads as zero in all cases and in all modes.
There is need to raise exceptions if SW running in any other mode tries to write to it.

NOTE: Discoverability of this feature is not critical, since this should be information known by the Machine Mode firmware.
...
* If software changes the value of `aceenable` to 0, then all CCs are cleared, both `aceccvalid` and `aceccdirty` are set to 0, and `*status.ACES` is set to Disabled-Clean.
* If software changes the value of `aceenable` to 1, and
 ** if the parent's domain `*status.ACES`=Disabled-Clean, then we simply assume that the parent process has cleared the CC and set both `aceccvalid` and `aceccdirty` to 0 before transferring the control flow to the current process;
 ** if the parent's domain `*status.ACES`=Disabled-Clobbered, then the hart traps to allow the parent domain responsible for handling context-switched to finish saving the state and ensure that both `aceccvalid` and `aceccdirty` are set to 0, and finally set `*status.ACES`=Disabled-Clean.
