 ...
 ** if the parent's domain `*status.ACES`=Disabled-Clean, all CCs are cleared and both `aceccvalid` and `aceccdirty` are set to 0.
 ** if the parent's domain `*status.ACES`=Disabled-Clobbered, the hart traps into the parent process to allow the latter to finish saving the state and enseure that both `aceccvalid` and `aceccdirty` are set to 0, and finally set `*status.ACES`=Disabled-Clean.
