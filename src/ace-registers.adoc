...
* `_ace.export_` _involves two passes, but the first pass is entirely internal to the ACE unit. Hence, the first pass cannot cause a page fault, and is easily restarted: specifically, it compute the SIV by applying POLYVAL to the serialized CC content.  Subsequently, the serialized metadata, nonce, and SIV are written out, followed by the CC content, which it serialized, encrypted and written out block by block. This part may cause page faults, but it can be easily restarted._
