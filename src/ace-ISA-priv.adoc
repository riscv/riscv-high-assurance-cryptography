...
To support lazy context switching of CR values, `scrstatus` and `vscrstatus` are extended to XLEN=64 bits.
_X_``crstatus``[__i__+32] encodes two states called Normal (0) and Lazy (1) for ``K(``__i__``)``.
On RV32, 32-bit CSRs `scrstatush` and `vscrstatush` shadow, respectively, bits [63:32] of `scrstatus` and `vscrstatus`.

Using this extension to _X_``crstatus``, when a context switching handler does not intend to restore ``K(``__i__``)`` at context restore, it sets instead _X_``crstatus``[__i__+32] to Lazy.
When _X_``crstatus``[__i__+32] is set, accessing ``K(``__i__``)`` causes the hardware to trap to allow the authoritative value of ``K(``__i__``)`` to be restored.
...
When an exception handler in Mode A saves CR ``K(``__i__``)`` for a process running in Mode B (less privileged than A), and then lazy restores it, it sets the _X_``crstatus``[__i__+32] for all modes from __X__ = A down to B's parent.
It suffices for A to set this value only at B's parent since the values propagate.
In a second moment, the handler at Mode A unconfigures ``K(``__i__``)``, which causes the hardware to implicitly clear _X_``crstatus``[__i__+32] and set _X_``crstatus``[i]= Dirty for all modes __X__ above A.
When the process in Mode B is resumed and accesses ``K(``__i__``)``, the authoritative SCC is found in the first mode that does not mark its ``K(``__i__``)`` as Lazy in its `crstatus`.
...
Any instruction that sets bit #__i__ of `vscrstatus` (resp. `scrstatus`) to Dirty, for 0 {le} __i__ < 32, also sets the corresponding bit of `scrstatus` (resp. `mcrstatus`) to Dirty. The ACES fields of `*status` are also set accordingly.

If the Lazy bits are defined,
any operation that sets the state of a CR to Clear or Dirty will also clear the Lazy bit.
Cleared Lazy values also propagate.
