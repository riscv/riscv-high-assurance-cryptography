...
A CSK can only become active if all either or four CSRs have been written to.
...
* Once all flags have been set, the values written by M-mode firmware are activated and replace the old values atomically. The new CSK can now be used for cryptographic operations.
* Any mode change to M-mode will reset the flags.
* If any of the flags are set, the ACE unit is not available and any attempt to execute any ACE instruction will raise an illegal instruction exception.
