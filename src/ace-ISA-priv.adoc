...
[[ACE-exception-codes]]
.Exception causes and their meanings, in decreasing priority order
...
| TBD  | `ace_exc_CR_other`      | Use of a CR as a source in any instruction (including `ace.size`), when the status of the CR is “Other/Lazy”.
...
If a configuration instruction cannot be performed because of insufficient residual CRF memory, exception `ace_exc_out_of_mem` is raised.
System software is responsible for freeing internal ACE unit space and retrying the instruction.
Alternatively, the software may set the CR's Metadata State to `ace_state_out_of_mem` and return from the exception handler.
...
{empty}
(((CSR, `scrstatus`)))
The Reserved for future uset register `scrstatus` tracks per-CR status, including dirtiness.

`scrstatus`(__i__) = `scrstatus`[2{interpunct}__i__+1:2{interpunct}__i__] is a two-bit field that encodes four possible statuses for ``K(``__i__``)``.:
...
|===
|Value |Name |Meaning
|   0   | Off         | The CR is unconfigured. Attempts to use it will raise exception `ace_exc_CR_unconf`. It can be configured.
|   1   | Other/Lazy  | The CR is may be configured or unconfigured. Attempts to export or use it will raise exception `ace_exc_CR_other`.
This can be used for lazy context switching at per-CR granularity.
...
demand. This mechanism can also be used asn an optimization or to emulate
a larger CRF, and is described here.
 ...
 * An “`ace.close`” instruction to suspend or finalize the procedure.
   If executed before all bytes are transferred, the CR state matches that of an interrupted monolithic instruction, including any special value of the _State_ Metadata field and the value of `acestart`. If executed after all bytes are transferred, it also performs any final cryptographic operations, such as authenticating the CR.
