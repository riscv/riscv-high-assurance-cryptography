...
Masks to determine which ACE exceptions vertically trap from the immediately less-privileged mode.
Bit __i__ set makes ACE error __i__+1 cause a trap.
These traps are handled using the architectural `*vec`, `*epc`, `*cause`, and `*tval` CSRs.
