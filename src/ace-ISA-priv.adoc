...
* The implementation shall maintain a per-hart set of flags indicating which CSRs in the group have been written to.
* These flags are initially unset.
* When M-Mode firmware writes to a CSR in the `macecsk` group, the implementation shall set the corresponding flag, and keep a copy of the value written so far. Reading the register will still return the old value.
