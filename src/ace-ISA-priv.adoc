...
| `Smaceemu`       | Defines trap-and-emulate behavior (cf. <<ACE-trap-and-emulate>>).                               | <<ACE-trap-and-emulate>>     | v0.0.0 | --
|===
...
[[ACE-trap-and-emulate]]
=== `Smaceemu`: Trap-and-emulate ACE memory instructions

When `Smaceemu` is implemented all ACE memory instructions cause an
...
[NOTE]
====
After much discussion, the task group has decided to encode the support for
trap-and-emulate behavior in a single extension. Unlike other virtualization
primitives in the privileged architecture specification, the support for
trap-and-emulate in ACE is statically defined through an ISA extension. The
main motivation behind this design choice is because the group did not see a
meaningful usecase in having dynamic trap-and-emulate support.
====
...
The `haceVirtBootScrt` group consists of HS- and VS-mode WARZ XLEN-bit CSRs, accessed via Indirect CSR Access: four in RV32, two in RV64.
