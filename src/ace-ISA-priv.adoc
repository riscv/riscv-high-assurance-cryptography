...
| TBD  | `ace_exc_invalid`              | Invalid parameters in metadata, including expired CR state. There is deliberately no separate `ace_exc_expired`: expiry is reported through this code, and the handler distinguishes it by reading the _State_ field of the CR, which the hardware has set to `ace_state_expired`.
|===
...
The alternative is to give the behavior its own enable — for instance a bit in an
`*envcfg` CSR — which leaves `TSR` untouched and lets the two controls be set
independently. Which of the two the architecture should adopt is not for this document to
decide alone.
====
...
S-mode should gate it in VS-mode by the same means.
====
