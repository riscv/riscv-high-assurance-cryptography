...
[WARNING]
====
This reuse of `TSR`, and the corresponding reuse of `VTSR` below, must be discussed with
...
* The implementation must maintain a per-hart set of flags indicating which CSRs in the group have been written to.
* These flags are initially unset.
* When M-Mode firmware writes to a CSR in the `macecsk` group, the implementation must set the corresponding flag, and keep a copy of the value written so far. Reading the register will still return the old value.
