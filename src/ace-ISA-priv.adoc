...
* `Smacepbootscrt` {implies} Defines the ACE OS Secret CSRs (`macePhysBootSecret{0..3}`).
* `Smacevbootscrt` {implies} Defines the ACE Boot Secret CSRs (`haceVirtBootSecret0{0..3}`).
* `Smacestatus` {implies} Defines the ACES per CC CSRs (`scrstatus`, `vscrstatus`, `mcrstatus`).
...
| 0xXXX       | SRW        | `sacetrapmask`              | [.red]#{warn} Discuss:# Mask to determine which ACE exceptions vertically trap
| (Indirect)  | MRW (RZ)   | `macecsk{0..7}`             | ACE Context Sealing Key
| (Indirect)  | MRW (RZ)   | `macePhysBootSecret{0..3}`  | Locality: Physical (host) Boot Session Secret
| (Indirect)  | HRW (RZ)   | `haceVirtBootSecret{0..3}`  | Locality: Virtual (guest) Boot Session Secret
...
`scrstatus[__i__]` encodes Clean (0) or Dirty (1) for CR #__i__.

Bits [63:32] are WPRI.

When the privilege mode is S, VS, or U:

* Any instruction that modifies CR #__i__ sets `scrstatus[__i__]` to Dirty.
...
`vscrstatus[__i__]` encodes Clean (0) or Dirty (1) for CR #__i__.
...
* Any instruction that modifies CR #__i__ sets `vscrstatus[__i__]` to Dirty.
...
[[ACE-lazy-loading]]
==== Extension of `*crstatus` for Lazy Loading

To support lazy loading of CR values, `scrstatus` and `vscrstatus` are extended to 64 bits.
On RV32, 32-bit CSRs `scrstatush` and `vscrstatush` shadow, respetively, bits [63:32] of `scrstatus` and `vscrstatus`.

_X_``crstatus[__i__+32]`` encodes Standard (0) or Lazy (1) for CR #__i__.

When X``crstatus[__i__+32]`` is Lazy, the semantic of but X``crstatus[__i__]`` changes as follows:
X``crstatus[__i__]`` is 0 to signal that the authoritative value of the CR has been saved in VS, or in S if V=0.
X``crstatus[__i__]`` is 1 to signal that the authoritative value of the CR has been saved in S and V=1.

These extensions to X``crstatus`` address the use case where context switching code does not restore a CR upon state restore, but sets the X``crstatus[__i__+32]`` bit instead to cause a trap on access ``K(``__i__``)`` and this trap serves to import the value.  The optimization can be extremely efficient if all the trap handler has to do is to check whether the associated SCC can be imported and, if not, to free another CR first. The latter can use a random selection or an implementation-defined HW assisted mechanism that marks the longest untouched configured CR.

Therefore, it is important to know _which_ environment has been the last one to export the authoritative value of ``K(``__i__``)``.

This feature would be necessary also to support VM migration, where it may happen that a VM is migrated to a target device where the CRF capacity is insufficient to import all the SCCs saved by the hypervisor.
...
Per-CR status propagates from less-privileged to more-privileged modes, analogously to the FS, VS, and ACES fields in `*status`, but with only two states: Clean and Dirty. Any instruction that sets bit #__i__ of `vscrstatus` (resp. `scrstatus`) to Dirty also sets the corresponding bit of `scrstatus` and `mcrstatus` (resp. `mcrstatus`). The ACES fields of `*status` are also set accordingly.
...
A write to the __i__-th CSR sets bits [(_i_+1)XLEN−1 : _i_·XLEN] of the Locality Secret PhysBootSecret (<<ACE-locality-indexes>>).
...
A write to the __i__-th CSR sets bits [(_i_+1)XLEN−1 : _i_·XLEN] of the Locality Secret VirtBootSecret (<<ACE-locality-indexes>>).
