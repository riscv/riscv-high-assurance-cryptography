...
Lazy loading defers the restoration of a CR from its saved SCC at context restore.
Instead, the CR is marked as “lazy”.
Any subsequent access to that CR—including export, but not operations that would overwrite it (such as clear, provisioning, or import)—causes a trap to the environment that holds the authoritative SCC.
...
Since processes can be interrupted by any higher-privileged mode, U-mode processes may be interrupted by H or VS.
Therefore, upon resuming a VM, it is important to know _which_ environment has been the last one to export the authoritative value of a given CR.
...
Let us associate numbers to the modes, 0 for U up to 3 for M, but the numbers may be larger in case we have one or more levels of nested virtualization.

When a mode A saves CR ``K(``__i__``)`` for a process in mode B < A, and then lazy restores it, it sets the `crstatus` for ``K(``__i__``)`` for all modes below A down to B to Lazy, but since this is done in SW, these values do not propagate. In other word, the `crstatus` for mode A and above becomes Dirty since the handler at mode A clears ``K(``__i__``)``.
The important thing is that when the process in mode B is resumed and accesses ``K(``__i__``)``, the value is found in the first mode that has the `crstatus` for the ``K(``__i__``)`` different from Lazy. Whether it is lest at Clear or Dirty, the value is authoritative (if it is Dirty, it means just that it is more recent than the value actually stored in some mode above A).

NOTE: This means that the software must support a call to the parent to get the SCC for the __i__-th CR, and it is a good idea to standardise the trap, even make it into an instruction.
