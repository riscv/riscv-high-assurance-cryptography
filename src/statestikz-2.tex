...
\begin{tikzpicture}
  ...
  \node[state, thick] at (9,-1) (algorithm defined) {\begin{tabular}{@{}c@{}}Algorithm\\Defined\end{tabular}};
  \node[state, thick] at (9,1)  (success failure)   {\begin{tabular}{@{}c@{}}Success/\\Failure\end{tabular}};

  \draw[->,densely dotted]  (initial)            to[out=90,in=0,looseness=0.6] (configuration hub);
  \draw[->,densely dotted]  (algorithm defined)  to[out=135,in=0] (configuration hub);
  ...
  \draw[<->,densely dotted] (configuration hub)  to[out=180,in=10] (various errors);
  ...
  \draw[->] (algorithm defined) to[out=-15,in=270] ($(algorithm defined)+(1.5,0)$) to[out=90,in=15] (algorithm defined);
  \draw[->, cross line] (algorithm defined) -- (invalid);
  \draw[->,cross line] (algorithm defined) to[out=150,in=0] (various errors);
