...
`Derived_CTK = Uniquify(CTK,LTST[index] || DUV_U || DUV_S })` or +
`Derived_CTK = Uniquify(CTK,LTST[index] || DUV_VU || DUV_VS || DUV_S})` +
and the `Derived_CTK` is value used to encrypt and authenticate the SCC.

[CAUTION]
====
This function must be as fast as possible. We cannot use a cryptographic hash or too many chained AES invocations. Roberto: what to do?
====
