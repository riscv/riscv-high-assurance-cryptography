...
The _Cryptographic Context_ (CC) is the fundamental abstraction in ACE.
...
The metadata specifies the cryptographic algorithm that is used in conjunction with the CC and optional usage-control policies.
...
A CR is _configured_ by writing metadata and content into it, which are thusly bound to form a CC.
...
ACE defines 32 Cryptographic Registers (CRs) for CC storage, rather than a traditional register file.
...
For software portability and interoperability, ACE defines two external formats for cryptographic contexts:
a configuration input format, loaded by `ace.init` into a CR to create a CC,
and an encrypted, authenticated SCC format produced by `ace.export` and that can be read by `ace.import`.
...
ACE architects support for several algorithms, and also define a minimal required subset [.red]#(current suggestion)#:
...
Description::
`ace.init` configures a CR with metadata and content to create a CC.
...
NOTE: The operation follows the two-phases processing model defined in <<ACE-formats>>.
...
Description::
Exports the CRs `Ks` or `K[Xs]` to memory at address `Xd`, using the procedure described in <<ACE-formats>>.
