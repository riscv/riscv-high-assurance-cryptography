...
| _BootSecret_              | Unique per the boot session of the host hardware and its software stack. Configured at boot of the physical device (see <<ACE-CSR-maceBootSecret>>). | Device | M | 6
| _VBootSecret_             | Unique per the boot session of the virtualized hardware and its software stack.
Regenerated at each virtual boot of a VM (see <<ACE-CSR-haceVBootSecret>>).                                | VM | H/M | 7
