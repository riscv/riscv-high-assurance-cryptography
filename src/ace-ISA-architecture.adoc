...
* `finalize()`: process the `state` into the digest value. We assume this re-uses `block` (the digest or a unit of XOF output should not be be bigger).
 ...
 ** In state _hash_absorb_, we can only issue ``ace.exec`` instructions of Form B, i.e., +
 “```ace.exec Kn|K[Xn], Vs```”, to absorb the data block `Vs` into the hash as follows.
+
--
  . `input_base <- 0` // or `acestart` upon resumption
  . while `(input_base < VL*SEW)` do { // as long as we have input to process
  .. `amount <- min(VL*SEW-input_base, b-block_base)`
  .. ``block[block_base + amount - 1 : block_base] <-`` +
      ``{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp} <- Vs[input_base + amount - 1 : input_base]``
 ...
 ** In state _hash_output_ we have two possible situations: we are in a hash function or in a XOF.
  *** Upon entering the state, the following operations are first performed:
  ...
  . `block_base <- 0`
 *** For each Form C `ace.exec` instruction “`ace.exec Vd, Kn|K[Xn]`”, the following operations are performed:
+
--
    * `amount : int`, `{nbsp}` amount of data to copy from `block` to `Vd`
    * `#define output_base input_base`

[]
   . `output_base <- 0` // or `acestart` upon resumption
    . while (`output_base` < `VL*SEW`) do {
     .. `amount` <- `min(VL*SEW - output_base, t - block_base)`
     .. ``Vd[output_base + amount - 1 : output_base] <- `` +
