...
// the rest of the block is to be executed uninterruptibly once the memory read is done
UNINTERRUPTIBLE_BLOCK_START
    SIV = M[20 .. 31]
    ...
    - if there is need for some internal operations to make the content available, such as
    - reconstructing threshold keys/values into a single share, these must be computed now
    - and read as part of the content as architected for non-threshold implementations
UNINTERRUPTIBLE_BLOCK_END
...
UNINTERRUPTIBLE_BLOCK_START
    ...
    - This includes computing shares for threshold implementations
UNINTERRUPTIBLE_BLOCK_END
----
