...
Let `m` be a metadata header, and
...
. `MH_size(x)` returns the byte length of the metadata header. Currently, is is always 8.
. `PI_content_size(x)` returns the byte length of the content section of a PI.
   It does not include the length of the optional Metadata Extension block.
   It does not depend on `m`._FilterMode_.
. `SCC_content_size(x)` returns the byte length of the fixed part of the content section of an SCC.
   ...
   It does not depend on `m`._FilterMode_.
.  `ME_size(x)` returns the byte length of the metadata extension.
...
`{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}MH_size(x)` + `ME_size(x)` + `PI_content_size(x)`,

and the total size in bytes of an SCC is

`{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}MH_size(x)` + 16 + `ME_size(x)` + `SCC_content_size(x)` + _ExtraContent_ * 8.
