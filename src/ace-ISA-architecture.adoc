...
From Smcsrind/Sscsrind, we reuse the Indirect CSR access mechanism.
...
| [12:11] |   2   | _DirectionPolicy_        | For encryption/decryption primitives or modes, determines whether the CC may be used for encryption (bit 11 set), decryption (bit 12 set), or both (bits 11 and 12 set). The field can be reused to expand the algorithm space for other types of operations.
| [14:13] |   2   | _SCProtection_           | Level of Side Channel Protection, encoded as per <<ACE-SC-protection-levels>>.
...
| 0xXXX          | SRW       | `sacecrstatus`          | Per-CR analog of `sstatus.ACES` field
| 0xXXX          | HRW       | `vsacecrstatus`         | Per-CR analog of `vsstatus.ACES` field
| 0xXXX          | MRW       | `macecrstatus`          | Per-CR analog of `mstatus.ACES` field
...
|   3   | Dirty              | ACE enabled. At least one CR or the ACEIOBUF may have been modified since the last context swap. This includes unconfigured states since switching code needs to act on them, by erasing any stale saved state.
|===
...
Attempts to execute any ACE instruction that accesses CR #__i__ raise ACE error _ace_err_unconfigured_ when ``sacecrstatus[``2__i__+1,2__i__``]`` is set to Off.
We define the semantics of the `acecrstatus` CSR.
“Configure” refers to the management operations that can configure a CR, namely `ace.provision`, `ace.import` and being the destination of an `ace.clone` or `ace.derive` operation. “Unconfigure” refers to the use of `ace.clear` on a CR.
...
When ``sacecrstatus[``2__i__+1,2__i__``]`` is set to Initial, attempts to _use_ or export the CR raise ACE error _ace_err_unconfigured_.
...
* `vsacecrstatus` substitutes for `sacecrstatus`, so instructions that normally read or modify `sacecrstatus` actually access `vsacecrstatus` instead.
If ``vsacecrstatus[``2__i__+1,2__i__``]`` is set to Initial or Clean, executing any instruction that changes CR #__i__ will change ``vsacecrstatus[``2__i__+1,2__i__``]`` to Dirty.
* Attempts to execute any ACE instruction that accesses CR #__i__ raise ACE error _ace_err_unconfigured_ when ``vsacecrstatus[``2__i__+1,2__i__``]`` is set to Off.
...
* `Xs`[10:0] and `Xs`[24:15] must be 0, else ACE error _ace_err_invalid_value_ is raised.
...
If `Xs`[12:33] = 0, then ACE error _ace_err_invalid_value_ is raised.
** If `m`[12:11] != `0b11`, then it must be `Xs`[12:11] = `m`[12:11] or `Xs`[12:11] = 0, in which case no change is made, else ACE error _ace_err_invalid_value_ is raised

* If the algorithm in the CR does not support encryption and decryption, then `Xs`[12:11] must be 0, else ACE error _ace_err_invalid_value_ is raised
* `Xs`[14:13] is used to turn on side-channel protection or increase its level, if implemented. The level is encoded according to <<ACE-SC-protection-levels>>.
Downgrading protection level is not permitted. Attempting to do so raises ACE error _ace_err_invalid_value_ (<<ACE-error-codes>>).
...
Not properly following the instruction ordering raises an illegal instruction exception and invalidates the CR.
