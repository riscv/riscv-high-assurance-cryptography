...
The fundamental concept of ACE is the _Cryptographic Context_ (CC), a data structure comprising a *metadata section* and a *content section* that form an undivisible unit.
...
| `ace.init`      | `Cd\|C[Xd],%offset(Xs)`          |  Sets a CRs's metadata and content.
...
so that privileged context switching code needs to save only the dirty CRs and restore only the configured ones:
...
//** At context save in privileged code: the CR's state is saved.
...
This can be used in low CRF memory situations to trigger a lazy re-load.
|===
...
// note: 128bits(metadata) = 0[63:0] * metadata
 ...
 ** `ace.init`:
  ...
  *** The corresponding exported SCC will include an 8-byte block in clear after the metadata with the `acestart` value, followed by the nonce and the tag, and the content will only contain the payload imported and read so far, zero padded up to a multiple of 8 bytes. This content is of course encrypted.
  The AEAD algorith will use `(length_of_payload)[64] @ metadata` in place of `0[64] @ metadata`.
...
NOTE: The advantage of `ace.reset` with respect to a loop that clears all CRs is that the entire CRF memory is cleared without the need for compaction/management between the separate `ace.clear` operations and this opertation cannot be interrupted, reducing potential wasteful context saves.
...
`Cd` is the source CR to be configured (encoded as a 5-bit immed).
...
Description::
`Cd` is the source CR to be cleared (encoded as a 5-bit immed). +
...
Description::
`Cd` is the destination CR into which the SCC is imported, as an immed (5-bit).
...
Description::
Clones source CRs `Cs` (or the CR indexed by `Xs`) into destination CRs `Cd` (or
...
In the "non-X" modes, i.e., in the CTR mode and its analague that replaces the arithmetic counter by a CRFSR, the sum of the lengths of IV and counter/LSFR equals the block size of the underlying keyed PRP or PRF. IV and counter/CRFSR are concatenated to form the input to the keyed PRP or PRF.
...
--
    . `absorb_into_hash(Vs)`.
    . `tmp <- enc_block(enc_key, 1'b0 @ (auth_tag xor N)[b-2:0])`
...
. If `ctr = 0`, then the CR is *invalidated*.
   . If the `SIV` is `b`-bits long, then +
     *** ``Vd <- Vs xor enc_block(key, 1'b1 @ SIV[b-2:b/4] @ (SIV[b/4-1:0] + ctr % 2^b/4^)),``
   . else (the `SIV` is `3b/4`-bits long) +
      *** ``Vd <- Vs xor enc_block(key, 1'b1 @ SIV[3b/4-2:0] @ ctr``.
   . `ctr <- ctr + 1`.
--
+
