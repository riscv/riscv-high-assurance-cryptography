...
|   [1:1] |   1   | _LengthExt_              | Value is 1 is there is an additional block of data at the end of content.
...
If any of these bits is set, the metadata is extended by the 128-bit block as described in <<ACE-metadata-extension>> (see <<ACE-combined-filters>>).
| [56:33] |  24   | _AdditionalLength_       | Additional content section in length
| [63:57] |   7   | Reserved                 | Value is 0 for the current metadata formats.
|===

[[ACE-metadata-extension]]
.Format of the optional metadata section extension for filtering
[width="100%",cols="^9%,^7%,16%,68%",options="header"]
|===
|     Field | Width | Name                    | Description
...
The optional metadata section extension for filtering is also confidential and will be encrypted upon export as an SCC.
