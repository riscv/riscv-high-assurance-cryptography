...
If `aceiobuflen` is zero, the ACEIOBUF is _unconfigured_ (it does not hold any content, similarly to unconfigured CRs, i.e., CRs in state Initial), and `ace.input` and `ace.output` raises ACE error _ace_err_unconfigured_.
...
Using the ACEIOBUF while `aceiobuftop`-`acestart` is zero or an invalid value raises ACE error _ace_err_invalid_value_.
...
If the value is not 32, the CR in all single-CR instructions, and the source CR in `ace.derive` or `ace.clone` can only have this value, otherwise an illegal instruction exception is raised.
 ...
 ** To transition to state _hash_absorb_:
 ...
 Other forms of  ``ace.state`` raise ACE error ace_err_not_permitted and the CR is *invalidated*. Then:
...
Other forms of ``ace.state`` raise ACE error _ace_err_not_permitted_ and the CR is *invalidated*.
