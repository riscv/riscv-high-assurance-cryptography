 ...
 ** StateNumber is non-zero for `ace.prov`.
...
`acestart` is an XLEN-bit RW CSR, to track progress of long-running interruptible instructions such as `ace.prov`, `ace.import`, `ace.export`, `ace.input`, and `ace.output`, as well as `ace.exec` on long inputs.
(((CSR, acestart)))
...
“Configure” refers to the management operations that can configure a CR, namely `ace.prov`, `ace.import` and being the destination of an `ace.clone` or `ace.derive` operation. “Unconfigure” refers to the use of `ace.clear` on a CR.
...
NOTE: `ace.prov`, as well as `ace.import`, `ace.export`, `ace.input`, and `ace.output` as described later, perform direct memory accesses.
This does not violate the RISC-V load/store architecture principles,
since `ace.prov`, `ace.import` and `ace.input` can be seen as special kinds of load instructions and
...
From the point of view of the system outside the ACE unit, `ace.prov` and `ace.import` appear as a series of sequential memory reads, and `ace.export`, appears as a series of sequential memory writes.
 ...
 ** `ace.prov`, `ace.clear` (thus also `ace.reset`), `ace.import`, `ace.export`, `ace.size` and `ace.available` work normally;
 ...
 ** `ace.error` always returns error _ace_err_invalid_CR_ after any operation involving the CR except for `ace.import` and `ace.export`, `ace.clear`, `ace.prov`, and `ace.size`, which all work normally, and using `ace.state` to return to state _initial_.
...
. *Provisioning Input*:
The data following the metadata block in the PI to `ace.prov`, such as keys and other required secrets.
