...
All operations perform *access* to CRs.
...
Accordingly, optional _Usage Control_ policies restrict only certain CC/CR usage operations.1
 ...
 ** *ace.clear*: sets status to *INITIAL*.
 ** *Configuration*: Works as expected; sets status to *DIRTY*.
 ** *ace.export*: Trigger a trap to the immediately higher mode for lazy loading.
...
* Neither S nor M/H implement lazy switching. Then each privileged mode must implement the basic rule:
. save only the dirty CRs, and,
. upon restore, restore the configured ones and explicitly clear the unconfigured ones.
...
... does not save the context (it is unconfigured), and1
..... it marks it as *CLEAN*
... at context restore, it just clears the CR and sets its to *LAZY* again returning control to the U process.
... Suppose however that S itself is now interrupted by H/M before S returns control to U
.... H/M sees a *CLEAN* CR and does nothing, except calling `ace.clear` again before returning control to S.
.... then S sees the sane state as before and there are no consistency or security issues.

..  Case 2: H/M interrupts the U process.
... It sees that the CR is *LAZY* and therefore
... does nothing to save the context
... at context restore, it just clears the CR and sets its  to *LAZY* again returning control to the U process.
