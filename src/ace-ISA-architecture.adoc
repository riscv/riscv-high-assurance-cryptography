...
RV64, called `acerctk0`, `acerctk1`, `acerctk2`, `acerctk3`, `acerctk0h`,
`acerctk1h`, `acerctk2h`, and `acerctk3h`, where register `acerctk[h]{i}` contains
bits [stem:[2XLEN-1\,i:XLEN\,i]] of the RCTK. The `*h` CSR variants are only
available in RV32. No exception is raised if software running in any other mode
tries to write to the `acerctk` block, such writes are simply ignored. The
`acerctk` block always reads as zero.
