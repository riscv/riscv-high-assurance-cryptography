...
[.red]#Idea: if the instruction uses public state, such as the metadata, this is addressed using RV architectural mechanisms, otherwise it uses an `aceerror`.
Exceptions to this must be well motivated.#

If the instruction itself is not allowed, not implemented, or is invoked on improperly configured state, then a normal illegal-instruction exception is raised.
Examples include:

* The ACEIOBUF range has an invalid length `aceiobuftop`-`acestart` (see <<ACE-CSR-aceiobuftop>> and (<<ACE-CSR-acestart>>);
...
* Software tried to use an unconfigured or partially configured CC in a CR as a source in an `ace.exec`, `ace.export`, `ace.state`, `ace.input`, `ace.output`, `ace.clone`, `ace.derive`, or `ace.restrict` operation.
