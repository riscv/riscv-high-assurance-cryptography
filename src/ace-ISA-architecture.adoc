...
If an algorithm defines 16 or fewer states (including the four architected ones), the current state number may be stored in the State_Number field.
...
State_Number must always be set to _initial_, _invalid_, and _completed_ if the state of the algorithm corresponds to these states.

The state values for the symmetric algorithms defined in this specification are listed in <<ACE-state-constants-symmetric>>.

During import and init, the State_Number field is set to _invalid_, and only at the end of the operation is it set to _initial_.
...
<| _ace_state_hash_absorb_         | _hash_absorb_       |  Start and initialize a hash function of the tag/MAC part of an algorithm and start absorbing blocks of data (in the case of an AEAD algorithm, this usually starts absorbing the associated data).
