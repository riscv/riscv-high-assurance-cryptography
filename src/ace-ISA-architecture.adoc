...
For each algorithm as specified in <<ACE-algorithms>>, the part of the content that normally contains the key(s) is replaced by its index into the SKT-for both PI and SCCs
...
* The first read is the read of the metadata header block itself:
...
In this proof-of-concept, we define scalar multiplication with known and unknown base point and ECDSA/EdDSA sign/verify.
