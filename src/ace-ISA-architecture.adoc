...
//would require defining arbitrary ranges with base addresses and using `acestart` relative to those bases for both input and output.
...
* There must be a 4-bit internal temporary state containing the original value of `metadata.State_Number`.
...
* The first read is the read of the metadata block itself. If fewer than 4 bytes are read before the instruction is interrupted, `acestart` = 0, so the instruction will be restarted.
