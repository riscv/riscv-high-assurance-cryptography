...
* `Smacestatus` => Defines the ACES per CC CSRs (`sacecrstatus`, `vsacecrstatus`).
...
| 0xXXX          | SRW       | `sacecrstatus`           | Array of two-bit fields (similar to ACES field, but per CR)
| 0xXXX          | HRW       | `vsacecrstatus`          | Array of two-bit fields (similar to ACES field, but per CR)
