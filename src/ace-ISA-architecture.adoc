...
| 0xXXX   | MRW (RZ)  | mace_OS_Secret{0..4}     | ACE OS Secret
| 0xXXX   | MRW (RZ)  | mace_Boot_Session0{0..4} | ACE Boot Session
|===
...
[.darkred]#Semantics TBD: In this case bits [63:2] are used as an identifier/address for the parent environment to solve the issue, content is empty, and SW will trap when trying to use this CC in any operation.#
...
| Chip_Unique_Secret        | Unique for each SoC. Permanent. | SoC | N | 2
...
| Product_Secret            | Identifies device/system type/model (product). Not necessarily permanent. Reconfigurable like OEM_Secret. | Device | N | 4
...
| Boot_Session_Secret       | Regenerated at each (virtual) boot. | OS | Allowed | 7
...
* `b` = block size in bits (8|`b`). [.darkred]#`VLEN` must be a multiple of `b`.#
 ...
 ** In state _encrypt_:
  ...
  *** [.darkred]#If `VLEN` is a multiple of `b`, then `ace.exec Vd,Cn|C[Xn],Vs` encrypts block-by-block.#
 ...
 ** In state _decrypt_:
  ...
  *** [.darkred]#If `VLEN` is a multiple of `b`, then `ace.exec Vd,Cn|C[Xn],Vs` decrypts block-by-block.#
  *** If any other Form of `ace.exec` is issued, then the CC is *invalidated*.
 ...
 ** In state _set_public_value_, only ``ace.exec`` instructions of Form B are allowed. +
  ...
  *** [.darkred]#If `VLEN` is larger than `b`, the most significant bits of `Vs` (i.e., the "tail" of the vector) are ignored.#
...
--
+
[.darkred]#If `VLEN` is a multiple of `b`, the above three commands are applied to each of the `VLEN/b` `b`-bit blocks of `Vs`, producing each time the corresponding `b`-bit block of `Vd`.
...
--
+
[.darkred]#If `VLEN` is a multiple of `b`, the above operation is executed `VLEN/b` times, absorbing each `b`-bit block of `Vs` into the hash.
...
--
+
[.darkred]#If `VLEN` is a multiple of `b`, the same behavior applies as in state _encrypt_.#

** In state _hash_finalize_dec_, only a Form A ``ace.exec`` instruction are allowed. The value
 `{nbsp}{nbsp}{nbsp}Vs = IntTo{b/2}Bits(bit_length(AD)) @ IntTo{b/2}Bits(bit_length(plaintext))` +
 must be set by the software. Then
"```ace.exec Vd,Cn|C[Xn],Vs```"
 computes the authentication tag as follows:
+
--
  . `absorb_into_hash(Vs)`.
  . `auth_tag <- auth_tag xor enc_block(key,ctr[b-1,b/4] @ 0[b/4-1,0])`.
...
--
+
[.darkred]#If `VLEN` is larger than `b`, only the `b` least significant bits `Vs` are compared to `auth_tag`.#
...
--
+
[.darkred]#If `VLEN` is a multiple of `b`, `absorb_into_hash` is executed on each of the `VLEN/b` blocks in `Vs`.#
...
--
+
[.darkred]#If `VLEN` is a multiple of `b`, the above operations are applied to each of the `VLEN/b` blocks in `Vs`.#
...
--
+
[.darkred]#If `VLEN` is larger than `b`, the most significant bits of `Vs` beyojnd the `b`-th (i.e., the "tail" of the vector) are ignored.#
...
--
+
[.darkred]#If `VLEN` is a multiple of `b`, then the above operations are applied to each `b`-bit block of `Vs`.#
...
* `n` = size of the internal state that is kept by the function.
 ...
 ** `partial_block`: `b/8` bytes (a priori we cannot know whether `b > VLEN` or not, and changing the internal memory requirements of a CC depending on the current `VLEN` can lead to memory exhaustion).
 ** `used_VLEN`: `4` bytes -- Current vector length in bits.
 ** `partial_len`: `2` bytes -- Length of absorbed data so far for the current block.
 ** `last_block_len`: `2` bytes -- Only if this Hash/MAC has special internal treatment for a fractional last block that goes beyond padding and appending the length, which are handled by user software.

* *Content, Exported*:
[lowerroman]
 . `key`: `k/8` bytes -- Only if keyed, i.e., a MAC.
 . `hash_state`: algorithm specific length.
 . `last_block_len`: `4` bytes -- Only if this Hash/MAC has special internal treatment for a fractional last block that goes beyond padding and appending the length, which are handled by user software. Otherwise, this is zero.
 . `partial_len`: `4` bytes -- Length of absorbed data so far for the current block.
...
* *States:* _initial_ (0), _hash_absorb_ (3 or 11), _hash_finalize_ (4 or 12), and _completed_ (15).
The values 11 and 12 are used when there is a partial block.
* *Allowed State Transitions:*
 ** _initial_ -> _hash_absorb_.
 ** _hash_absorb_ -> _hash_finalize_ -> _completed_.
 ** From any state to state _initial_.
 ...
 ** In state _initial_:
  *** `last_block_len` is set to the full block length `b`.
  *** `partial_len` is set to zero.
  *** `partial_block` is set to zero.
  *** `hash` is set to an initial value as required by the algorithm, which can be fixed or depend on the `key`.
 ...
 *** If `VLEN` is a multiple of `b` (including the case where `VLEN` = `b`), then
    we can only issue a single instruction ``ace.exec`` of Form B, i.e., "```ace.exec Cn|C[Xn],Vs```", to absorb the least significant `b` bits of data block `Vs` and write the final hash value in `Vd`.

 *** If `b` is a multiple of `VLEN` and `b` > `VLEN`, then we can issue one or more instructions ``ace.exec`` of Form B, i.e., "```ace.exec Cn|C[Xn],Vs```", to absorb the data block `Vs` into the hash, followed by a single instruction ``ace.exec`` of Form A or D,
 i.e. "```ace.exec Vd,Cn|C[Xn],Vs```" or "```ace.exec Vd,Cn|C[Xn]```"
 to absorb the last `VLEN`-bit block and to write the final hash value in `Vd`.
  . if `partial_len+VLEN < b`, then
  .. The instruction must have been of Form B, else the CC is *invalidated*.
  .. `Vs` is written to `partial_block` starting at byte offset `partial_len`.
  .. `partial_len <- partial_len + VLEN`.
  . else if `partial_len+VLEN = b`, then
  .. if the instruction is of Form A
  ... `Vs` is written to `partial_block` starting at byte offset `partial_len`.
  ... (`partial_len <- partial_len + VLEN`.)
  ... `process_block(partial_block)`
  ... finalize the hash, transition the state to _completed_, and return the value.
  .. else if the instruction is of Form B
  ... `Vs` is written to `partial_block` starting at byte offset `partial_len`.
  ... `partial_len <- partial_len + VLEN`.
  ... `process_block(partial_block)`
  .. else, the CC is *invalidated*.
  . else if `partial_len = b`, then
  .. If the instruction is of Form D
  ... finalize the hash, transition the state to _completed_, and return the value.
  .. else, the CC is *invalidated*.
