...
| _NoHWBinding_             | Independent of Chip, OEM, Device                                              |  --    |   --   |  2:0   |   0   | 0
| _ChipDesignerSecret_      | Identifies the designer of the SoC. Permanent.                                |  SoC   |   No   |  2:0   |   1   | 1
| _ChipModelSecret_         | Identifies specific chip model/family. Permanent.                             |  SoC   |   No   |  2:0   |   2   | 2
| _ChipUniqueSecret_        | Unique for each SoC. Permanent.                                               |  SoC   |   No   |  2:0   |   3   | 3
| _OEMSecret_               | Provisioned by OEM to differentiate their products from the competition.
Permanent, or reconfigurable via an implementation-defined HW authenticated mechanism.                      | Device |   No   |  2:0   |   4   | 4
| _ProductSecret_           | Identifies device/system type/model (product). +
Permanent or reconfigurable like OEMSecret.                                                                 | Device |   No   |  2:0   |   5   | 5
...
Reconfigurable like OEMSecret.                                                                              | Device |   No   |  2:0   |   6   | 6
| _BootSecret_              | Unique per the boot session of the host hardware and its firmware stack. +
Configured by M-mode during the boot process of the host (see <<ACE-CSR-maceBootSecret>>).                  | Device |   M    |   3    |  0/1  | 7
| _VBootSecret_             | Unique per the boot session of the virtual hardware and its software stack. +
Regenerated by H or higher at each virtual boot of a VM (see <<ACE-CSR-haceVBootecret>>).                   |   VM   |  H/M   |   4    |  0/1  | 8
| _SSecret_                 | Can be configured by Supervisor (see <<ACE-CSR-saceSSecret>>).                | Process |  S    |   5    |  0/1  | 9
