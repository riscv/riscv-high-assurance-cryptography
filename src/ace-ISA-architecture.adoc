...
Bit 3: `HasHash` +
Bit 4: `HasSignature` +
Bits 5-15: Padding (zero).
|  3   | 2               | `Progress`/`input_base`
|  4   | 2               | `block_base`
|  5   | 2               | Padding
|  6   | `2b/8`          | `Generator`, X coordinate
|  7   | `2b/8`          | `Generator`, Y coordinate
|  8   | `b/8`           | `Scalar`: `b`-bit string. Can be the private key.
|===
...
The `acemarchid` in this case is placed at Position 13.

[cols="^5%,<30%,<62",width="100%",options="header"]
|===
| Pos. | Size in Bytes                    | Field
|  9   | If `HasPublicKey`: `b/8`, else: 0 | `PublicKey`, X coordinate.
| 10   | If `HasPublicKey`: `b/8`, else: 0 | `PublicKey`, Y coordinate.
| 11   | If `HasSignature`: `b/8`, else: 0 | `Signature`, R value.
| 12   | If `HasSignature`: `b/8`, else: 0 | `Signature`, S value.
| 13   | If `HasHash`: `b/8`, else: 0      | `Hash`: a `b`-bit string representing the hash.
| 14   | If `mh`._FilterMode_ != 0: `b/8`, else: 0 |  `acemarchid`
| 15   | If `mh`._FilterMode_ != 0: `b/8`, else: 0 | Additional data (as in the *Internal State*). During signature generation this includes a `RandomNumber`, a `b`-bit string, erased upon completion, exported only if the signature operation is interrupted.
|===
