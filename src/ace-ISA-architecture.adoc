...
The sharing mechanism is implementation-defined. Reconfigurable like _OEMSecret_.                                      | Device |   HW   | [2:0]  |   6
|   6   | _PhysBootSecret_          | Unique per the boot session of the host hardware and its firmware stack.  +
...
|  --   | (No Binding at Modes)     | Independent of M-, S-, or VS-mode filters.                                       |  --    |   --   | [6:5]  |   0
|   8   | _SLocality_               | Configurable by modes higher than S to mutually isolate S-mode processes.        |  S     |   M    | [6:5]  |   1
|   9   | _VSLocality_              | Configurable by modes higher than VS to mutually isolate VS-mode processes.      |  VS    |   S    | [6:5]  |   2
|  10   | _ULocality_               | Configurable by modes higher than U to mutually isolate U-mode processes.        |  U     |   VS   | [6:5]  |   3
|===
...
_VSLocality_ is only active if the H extension is enabled and V=1.
When V=0, then the value of _VSLocality_ is replaced with _SLocality_.
...
ACE extends `*status` and `misa`, and adds up to 10 new CSRs and (up to) 28 indirect CSRs, which are listed in <<ACE-CSRs-table>>.
...
| (Indirect)  | MRW (RZ)  | `SLocality{0..3}`           | Locality: Mutually isolate S-mode processes
| (Indirect)  | SRW (RZ)  | `VSLocality{0..3}`          | Locality: Mutually isolate VS-mode processes
| (Indirect)  | HRW (RZ)  | `ULocality{0..3}`           | Locality: Mutually isolate U-mode processes
|===
...
Any access to `macePhysBootSecret` outside of M-mode causes an illegal instruction exception.
...
[[ACE-CSR-maceSLocality]]
===== `maceSLocality` Group

This is a group of M-Mode WARZ XLEN-bit CSRs, used to configure `maceSLocality`.
(((CSR, `maceSLocality`)))
In RV32 there are four CSRs.
In RV64 there are two CSRs.

A write to the __i__-th CSR sets Bits [(_i_+1) XLEN - 1 : _i_ XLEN] of `maceSLocality`.

Any access to `maceSLocality` outside of M-mode raises an illegal instruction exception.

//

[[ACE-CSR-haceVSLocality]]
===== `haceVSLocality` Group

This is a group of S-Mode WARZ XLEN-bit CSRs, used to configure `haceVSLocality`.
(((CSR, `haceVSLocality`)))
In RV32 there are four CSRs.
In RV64 there are two CSRs.

A write to the __i__-th CSR sets Bits [(_i_+1) XLEN - 1 : _i_ XLEN] of `haceVSLocality`.

Any access to `haceVSLocality` outside of S-mode raises an illegal instruction exception.

//

[[ACE-CSR-saceULocality]]
===== `saceULocality` Group

This is a group of (V)S-Mode WARZ XLEN-bit CSRs, used to configure `saceULocality`.
(((CSR, `maceSLocality`)))
In RV32 there are four CSRs.
In RV64 there are two CSRs.

A write to the __i__-th CSR sets Bits [(_i_+1) XLEN - 1 : _i_ XLEN] of `saceULocality`.

Any access to `saceULocality` outside of (V)S-mode raises an illegal instruction exception.
...
NOTE: The internal ACEIOBUF is not part of any CR.
In fact, `ace.output` can be used to save the contents of the buffer, and `ace.input` can be used to restore it freely as long as ``aceactivecr``'s value is 32.
...
// NOTE: This mechanism is one of the reasons, besides keeping the state machines simple, that in a given state of an algorithm, only one Form of `ace.exec` or `ace.state` is allowed.
// Some exceptions may be possible.
// For instance, both a Form D and a Form B could be allowed, with different semantics, and when using replacement sequences, these would be distinguished according to whether `aceiobuftop` - `acestart` is 0 or not.
