...
ACE implements the concept of _opaque keys_ via _Cryptographic Contexts_ (CCs), architectural containers that protect sensitive information.
Each CC contains an opaque *metadata section*, which does not require confidentiality, and a *content section*, which does.
...
Confidentiality and integrity of the content section are enforced by restricting access and modifications to architectural mechanisms that implement the rules of the configured algorithm. The concept of _opaqueness_ extends to all data within the content section.
