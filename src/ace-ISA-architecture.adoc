...
[WARNING]
--
If VS/HS/M cannot import a SCC accessed by some process at U/VS/S
because of a capacity issue, then the instruction at the requesting mode must fail and return _ace_err_memory_.
This means that, a least in theory, a piece of software should check for such an error after all operations, which can affect performance.
This can be an issue with in-order processors with few execution ports.
While this may be solved by delegating addressing this error to a special in-process trapping handler to free other CRs, we would have to  define this mechanism and expect software to handle it.

A different option is to allow the parent mode to implement lazy switching, i.e., when they cannot import a SCC required by the interrupted instruction, the parent chooses another CR used by the child, exports it and makes it *LAZY*.
This also means that in place of raising an _ace_err_memory_ error, out of CRF capacity situations should trap, which can be architected, and there could be a special address for the purpose.
(This would be the alternative to a caching mechanism as well.)
--
