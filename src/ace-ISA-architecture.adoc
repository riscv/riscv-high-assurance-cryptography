...
However, if any value of the state (such as a counter, or a partial hash value) is altered between the interruption and resumption of an export operation, the resumed export may use an incorrect SIV for the remaining SCC data, causing a subsequent import to fail authentication.
Therefor any operation that uses the same CR shall set a flag in it to indicate that the interrupted export operation must be restarted.
...
[WARNING]
--
Up to three Locality Secrets can be combined: A HW Binding one, one of either _PhysBootSecret_ or _VBootSecret_, and one from the SW Filter Group.
