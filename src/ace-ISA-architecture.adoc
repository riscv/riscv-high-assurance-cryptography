...
A value of 0 in the KeyType field indicates an explicit value.
A value of 1 in the KeyType field indicates a system key.
...
For each Locality we provide the following information: its index in the LST; its _domain_, i.e., whether the value is SoC-wide, device-wide, VM-wide, or per-process; its configurability; the bit field it is encoded into and its value in that field.
...
|   2   | _ChipSecret_              | Unique for each SoC. Permanent.                                               |  SoC   |   No   | [2:0]  |   3
...
//| {ge} 9 | _Reserved_               | Reserved for future use.                                                     |   --   |   --   | [7:6]  |  --
|===
