...
any cryptographic operation performed by ACE is executed within the constraints set by a CC.
A CC is an indivisible data structure comprising a *metadata section* and a *content section*.
The metadata defines the cryptographic algorithm that can be used in conjunction with the CC, and also includes optional usage policies.
The content holds sensitive material such as keys, secrets, and internal algorithm state.
The metadata has a fixed format, while the content format depends on the algorithm defined in the metadata.
Metadata does not require confidentiality, but the content must remain confidential. Both sections require integrity.
These security requirements are guaranteed by enforcing that all interactions with a CC follow the rules defined by the ACE architecture and the algorithm programmed in the CC itself.

Cryptographic Register:::
(((Cryptographic Register)))
The primary mechanism for realizing cryptographic contexts is the _Cryptographic Register_ (CR).
CRs are architectural-i.e., per-hart, containers safeguarding sensitive information.
A CR is _configured_ by writing metadata and clear content (e.g., key material) into it, binding them to create a CC.
A CC can only be used for cryptographic operations while resident in a CR.
Only the ACE unit may access or modify a CC within a CR, adhering to the rules of the architecture and of the algorithm.
...
To support context switching while at the same time protecting CCs outside architectural state, a CC can be exported as a _Sealed Cryptographic Context_ (SCC), an encrypted and authenticated package.
SCCs can be reimported into CRs when needed.
These operations use _Context Sealing Keys_ (CSKs), programmable by M-mode and dependent on _Localities_ (see below).
A CC sealed with a CSK cannot be reimported using a different CSK.
This enables flexible key separation by spatial and temporal domains.

Object-Orientation:::
CCs act as _objects_, exposing a uniform interface to different algorithms.
A CC encapsulates its behavior, allowing interchangeable use of ciphers like AES-128, AES-256, Калина-128, Калина-256, and SM4, and so on, in the same modes (ECB, GCM-SIV, XEX/XTS) without requiring any change to the compiled code. Side-channel-resistant variants are also supported transparently.
...
ACE distinguishes between CR management (import/export) and usage (cryptographic operations).
Exporting/reimporting SCCs and clearing/reinitializing CRs must always be possible to prevent denial of service by a process using all CRs.
Therefore, optional _Usage Control policies_ in the metadata only restrict cryptographic operations and state modifications, but not the management of CRs and SCCs.
This is also the reason why the term _Usage Control_ is used rather than _Access Control_.
...
ACE provides a number of unprivileged instructions, some implemented as CSR R/W operations.
The notation `C[Xd]`, for a GPR `Xd`, refers to the CR whose number is contained in `Xd`,
and `Cn|C[Xn]` means that there are two different encodings for when the CR number is passed as an immediate (`Cn`) or in `Xn`.
...
| `ace.init`      | `Cd\|C[Xd],%offset(Xs)`          |  Sets a CR's metadata and content.
| `ace.clear`     | `Cd\|C[Xd]`                      |  Clears a CR and marks it as no longer in use and clean.
| `ace.export`    | `%offset(Xn),Cs\|C[Xs]`          |  Securely exports the content of a CR to memory.
| `ace.import{.S}` | `Cd\|C[Xd],%offset(Xn)`         |  Authenticates and imports an SCC from memory. +
 `ace.import.S` provides optional support for system-defined formats.
| `ace.exec`
| A. {nbsp}{nbsp} `Vd,Cn\|C[Xn],Vs` +
  B. {nbsp}{nbsp} `{nbsp}{nbsp}{nbsp}Cn\|C[Xn],Vs` +
  C. {nbsp}{nbsp} `Vd,Cn\|C[Xn]` +
  D. {nbsp}{nbsp} `{nbsp}{nbsp}{nbsp}Cn\|C[Xn]`
...
| `ace.size`
| `Xd,Cs\|C[Xs]` +
`Xd,Xs` .2+|  Returns the SCC size in bytes for CR `Cs`/`C[Xs]` or for an algorithm with metadata `Xs`.
Returns zero if the algorithm is unsupported.
`ace.available` is an alias for `ace.size`.
| `ace.available` | `Xd,Xs`
| `ace.clone`     | `Cd\|C[Xd],Cs\|C[Xs]`            |  Clones a source CR to a destination CR.
...
`(ace.dir)`
| `Cd\|C[Xd],#immed` +
`Cd\|C[Xd],#immed,Xs`  |  Modifies algorithm state (e.g., phase transitions). `ace.dir` is an alias for `ace.state`.
| `ace.restrict` +
  `(ace.harden)`  | `Cd\|C[Xd],Xs`                   |  Narrows Usage Policies and enables/increases side-channel protection.
   Weakening Usage Policies and SCA protection is not permitted. `ace.harden` is an alias for `ace.restrict`.
| `ace.input`     | `Cd\|C[Xd],%offset(Xs)`          |  Optional. Provides input to the algorithm (for long inputs).
| `ace.output`    | `%offset(Xd),Cs\|C[Xs]`          |  Optional. Writes output from the algorithm (for long outputs).
...
ACE defines 32 Cryptographic Registers (CRs) for keys and metadata, rather than a traditional register file.
Each CR is identified by a number from 0 to 31.
(((Cryptographic Context)))

CRs reside in a dedicated _CR file_ (CRF), accessible only by the ACE implementation.
Each CR uses a variable amount of the capacity of the CRF, depending on the configured algorithm.
ACE returns _ace_err_memory_ if insufficient memory prevents successful execution of `ace.init`, `ace.import`, `ace.clone`, or `ace.derive`.
If this happens, software needs to free memory by choosing another CR and clearing it.
CRF capacity is implementation-defined, but implementations must provide sufficient CRF memory to prevent infinite `ace.export`/ `ace.import` loops,
and the CRF must hold at least an instance of the largest possible CC or the largest CC pair for `ace.derive`.
...
ACE extends `*status` and `misa` and adds 20 new CSRs (of which 16 are optional and MM-exclusive) to manage its execution environment.
...
===== ACES field in `*status` and `aceccstatus` CSR

The ACES field (bits 26:25) is added to `mstatus`, shadowed in `sstatus` and `vsstatus` (if the hypervisor extension is present).
...
|   1   | Initial            | ACE enabled. No CR is configured. Enabling via `ace.enable` clears contexts.
...
Setting ACES to Off, Initial, or Clean does not destroy/clear state.
...
(including configuration, e.g., `ace.init`, `ace.import`, `ace.clone`, or `ace.derive`)
...
CSR `aceccstatus`, is used to track the configuration and dirtiness of CRs on a per-CR basis,
so that privileged context switching code needs to save only the dirty CRs, and restore only the configured ones, unless the latter are lazy-loaded:
...
*INITIAL* (1)::
*Description:* CR unconfigured; no saved value in the context switching state.
...
*OFF* (0; also called *LAZY*)::
*Description:* CR unconfigured, but value saved for lazy loading.
...
*CLEAN* (2)::
*Description:* CR configured and loaded; unchanged since switch.
* *Usage:* Unrestricted; modification sets status to *DIRTY*.
* *Configuration:* Works as expected; sets status to *DIRTY*.

*DIRTY* (3)::
*Description:* CR configured and loaded; potentially changed since switch.
* *Usage:* Unrestricted.
...
Error codes 0-127 reserved for RVI, 128-255 for custom algorithms.
...
Unlike the vector extension’s `vstart`, which tracks element indices, ACE has no notion of an element; `acestart` therefore records a byte offset.
...
Typically, hardware sets this register.
...
The exact semantics of `acestart` are defined in <<ACE-formats>>

===== Reuse of ASID and VMID (to be verified)

ACE reuses `satp`.ASID and `hgatp`.VMID to optionally restrict the usage of CCs to a specific process or virtual machine.
...
(((CSR, `macercsk`)))
For this purpose, eight 32-bit WARL CSRs are used in RV32 and four 64-bit WARL CSRs in RV64.
The CSRs are: `macercsk0`, `macercsk1`, `macercsk2`, and `macercsk3` in RV64, to which RV32 adds
`macercsk4`, `macercsk5`, `macercsk6`, and `macercsk7`.
...
NOTE: Currently, RCSK is not persisted across reboots, mechanisms for RCSK persistence and cross-device transfer to assist VM mmigration are system-specific and belong to M-mode firmware. Cross-device RCSK migration must use a public-key key wrapping mechanism.
...
For software portability and interoperability, ACE defines two formats for cryptographic contexts: the configuration input format, loaded by `ace.init` into a CR to create a CC, and the encrypted, authenticated SCC format produced by `ace.export` and that can be read by `ace.import`.
...
NOTE: The information in the Completion field is necessary to make sure that any cryptographic operation attempted on a CR containing an incomplete CC raises an error or is trapped.
...
Filtering usage by Supervisor Domain or World ID is not supported, and not only because of a lack of space in a 64-bit metadata section:
...
In <<ACE-locality-indexes>>, we list the mandatory entries and whether M-mode can configure them.
We also specify the domain of each Locality Secret, i.e., whether the value is chip-wide (should be the same across all chiplets in the same SoC, two distinct SoCs the same decide may have different values),
...
| DeviceSecret              | Unique device (entire system) secret (_not_ the device ID such as a serial number). Shared across all harts in a SoC and shareable across multiple SoCs in a single device. Sharing mechanism impdef, i.e., not architected. Reconfigurable like OEMSecret. | Device | N | 5
...
A CC may reference a key via an index into the system-defined System Key Table (SKT).
A device-specific SKT entry may correspond to a different key on another device, leading to unspecified behavior.
Implementations of the SKT *must* provide a suitable Locality to the ACE unit during CR configuration,
and set usage policies that are at least as strict as the key's own.

[[ACE-formats]]
===== Formats of Configuration Inputs and Sealed Configuration Contexts

The formats of the configuration input, of the content section, and of the SCCs are dependent on the algorithm being used.
The sizes of the configuration input and of the SCC must be multiples of 8 bytes.
...
The first 8 bytes contain the 64-bit metadata section as specified in <<ACE-metadata>>,
followed by the content, i.e., the key(s) and possibly algorithm-specific information.

Plaintext Format of SCC:::
(((Cryptographic context, SCC plaintext)))
`bytes 0 .. 7  {nbsp}{nbsp}{nbsp}{nbsp}{nbsp}` contain the 64-bit metadata section, +
`bytes 8 .. 19  {nbsp}{nbsp}{nbsp}{nbsp}` contain the nonce (96 bits), +
`bytes 20 .. 31  {nbsp}{nbsp}{nbsp}` contain the tag (96 bits), and +
`bytes 32 .. u+31  {nbsp}` contain the serialization of the content of the `content`, with a byte length of `u`.
+
...
function DCSK_KeyDeriv(K,L) = {
  AESE256(K, L ^ (0[125:0] @ 0b010))[127:0] @ AESE256(K, L ^ (0[125:0] @ 0b000))[127:0]
}
----
...
z = size(content);   // content is serialized
num_blocks = ceil(z / 16);
last_block_fractional = z % 16 != 0;
content : array[z] of bytes;
M : array[32 + z] of bytes;
...
{Kenc : bits(256), Kauth : bits(128)} = RFC8452_KeyDeriv(CSK);

let T : bits(96) = POLYVAL(Kauth, 64'b0 @ metadata @ content)[95:0];

let SIV : bits(96) = AESE(Kenc, 0 1'b0 @ ((T xor N)[126:0])) >> 32;
// not preemptible until this point

// preemptible here (for instance because of page fault), with `acestart` from 0 to 15
...
foreach(i from 0 to num_blocks - 1) {
    if ((i == num_blocks - 1) and last_block_fractional) {
        // next instruction preemptible, with `acestart` from 32+i*16 to 39+i*16
        M[32+i*16 .. 39+i*16] = content[i*16 .. 7+i*16] xor
                                AESE(Kenc, 1'b1 @ SIV[94:0] @ (binary(i)[31:0]))[63:0];
    } else {
        // next instruction preemptible, with `acestart` from 32+i*16 to 47+i*16
...
- check if sufficient memory is available for algorithm encoded by metadata
...
let SIV: bits(96) = M[20 .. 31]

T : bits(128) = 64'b0 @ metadata;
T = T * Kauth;  // * = Montgomery multiplication
foreach(i from 0 to num_blocks - 1) {
    if ((i == num_blocks - 1) and last_block_fractional) {
        // next instruction preemptible, with `acestart` from 32+i*16 to 39+i*16
        content[i*16 .. 7+i*16] = M[32+i*16 .. 39+i*16] xor
                                  AESE(Kenc, 1'b1 @ SIV[94:0] @ (binary(i)[31:0]))[63:0];
        T = T xor content[i*16 .. 7+i*16];
    } else {
        // next instruction preemptible, with `acestart` from 32+i*16 to 47+i*16
        content[i*16 .. 15+i*16] = M[32+i*16 .. 47+i*16] xor
                                   AESE(Kenc, 1'b1 @ SIV[94:0] @ (binary(i)[31:0]));
        T = T xor content[i*16 .. 15+i*16];
    }
    T = T * Kauth;  // * = Montgomery multiplication
}

// now T POLYVAL(Kauth, 64'b0 @ metadata @ content)
let tag : bits(96) = AESE(Kenc, 0 1'b0 @ ((T xor N)[126:0]))[127:32] >> 32;
if (tag != SIV) then { error; }
...
[.red]#*TBD* Specify the exact semantics of# ```[.darkred]#acestart#```[.darkred]#, define the points at which operations can be interrupted:#
...
The envisioned usage model employs ASID/VMID filtering at the User level, and potentially at the Supervisor level in high-performance systems where migration is not required (e.g., automotive applications). Cryptographic domain separation via the RCSK is intended for Virtual Machines, Supervisor Domains, Worlds, and TEEs within those contexts, as these demand stronger security guarantees.

NOTE: Although the architecture does not prevent assigning unique RCSKs to U-mode processes, this would place a substantial burden on the system software just in term of the per-context-switch `ecall` overhead.
...
In all cases where a GPR is used to index a CR, we exclude the use of the zero-hardwired GPR `X0` as the index register—that is, the encoding of the register cannot be zero. This is reserved to encode some special operation, and otherwise raises an illegal instruction exception.
...
`Cd` is the source CR to be configured (encoded as a 5-bit immediate).
...
Description::
`Cd` is the source CR to be cleared (encoded as a 5-bit immediate). +
...
Description::
Exports the CRs `Cs` or `C[Xs]` to memory, starting at address `Xd`, using
...
Description::
`Cd` is the destination CR into which the SCC is imported, as an immediate (5-bit).
...
In other words, it is the responsibility of the system to augment all the system defined formats with a unique identifiers to distinguish the formats from each other.
...
Description::
Clones source CR `Cs` (or the CR indexed by `Xs`) into destination CR `Cd` (or
...
In order to support interrupts of the emulation libraries, such instructions need to respect the value of `acestart` and update it in accordance with the expected architected behavior of the trapped instructions.  We provide here non-normative definition for such a set of instructions in <<ACE-trappable-insns-support>>.
...
Such supporting instructions must be uninterruptible.

The initial cryptographic phase of `ace.export` and the authentication phase of `ace.import` must be uninterruptible as well.
Even if these parts are executed concurrently with other operations, the trap handler may need to wait for these steps to complete before proceeding with any memory accesses.

The support instructions are implementation-specific and not explicitly defined in the architecture.
However, interrupting the firmware implementation of `ace.init`, `ace.export`, `ace.import`, `ace.input`, or `ace.output` must leave the architectural state indistinguishable from the state expected from interrupting a monolithic implementation of the instructions.
...
If an invalid input is provided to any ACE instruction, or a non-admitted instruction is executed, while processing the algorithm in CR, say, `Cn`,
...
* For instructions that refer to `Cn`:
 ...
 ** In state _encrypt_:
  *** If{nbsp} `ace.exec Vd,Cn|C[Xn],Vs` {nbsp}is issued, then {nbsp} `Vd = enc_blk(Vs)`.
...
ace.import C0,(X5)                        // Import the SCC into CR C0
...
ace.state C0,#ace_state_encrypt           // Configure CR for encryption
ace.error X1                              // Read error from a CSR
...
** In state _hash_verify_, only an ``ace.exec`` instruction of Form B, i.e., "```ace.exec Cn|C[Xn],Vs```" is allowed.
...
vsetvli zero,4,e32,m1,ta,ma                      // Use 128-bit vectors
ace.import C0,(X5)                               // Imports SCC into CR C0
V5[127:32] ← IV
V5[31:0] ← 1
 ...
 ** In state _hash_absorb_, we can only issue ``ace.exec`` instructions of Form B, i.e., "```ace.exec Cn|C[Xn],Vs```", to absorb the data block `Vs` into the hash. This is performed as follows:
+
--
  . `tmp <- hash xor Vs`.
 ...
 ** Upon issuing `ace.state Cn|C[Xn],#ace_state_last_block, Xs` where `Xs` contains the length of the last block in bits, the following happens:
   . if `partial_len` <> 0, then the CR is *invalidated*. (The previous block is not complete.)
   . if `Xs > b` then the CR is *invalidated*.
   . `last_block_len <- Xs`.

 ** In state _hash_last_block_,  we can only issue a single ``ace.exec`` instruction of Form B, i.e., "```ace.exec Cn|C[Xn],Vs```", to absorb the data block `Vs` and write the final hash value in `Vd`. After this, the state transitions to _completed_:
+
--
  ...
  ** In state _hash_finalize_,
    then we can issue one ``ace.exec`` instruction of Form C, i.e., "```ace.exec Vd,Cn|C[Xn]```", to output the hash.
 ...
 ** In state _initial_:
  ...
  ** In state _hash_absorb_, we can only issue ``ace.exec`` instructions of Form B, i.e., "```ace.exec Cn|C[Xn],Vs```", to absorb the data block `Vs` into the hash:
 ...
 ** Upon issuing `ace.state Cn|C[Xn],#ace_last_block, Xs` where `Xs` contains the length of the last block in bits, the following happens:
+
--
 . if `partial_len` <> 0, then the CR is *invalidated*. (The previous block is not complete.)
 . if `Xs > b`, then the CR is *invalidated*.
 ...
 ** In state _hash_last_block_:
  ...
  *** If `VLEN` is a multiple of `b` (including the case where `VLEN` = `b`), then
      we only issue a single ``ace.exec`` instruction of Form B, i.e., "```ace.exec Cn|C[Xn],Vs```", to absorb the least significant `b` bits of data block `Vs`.
  *** If `b` is a multiple of `VLEN` and `b` > `VLEN`, then we can issue one or more ``ace.exec`` instructions of Form B, i.e., "```ace.exec Cn|C[Xn],Vs```", to absorb the data block `Vs` into the hash, i.e. "```ace.exec Cn|C[Xn],Vs```":
   . if `partial_len < b`, then
  ...
  ** In state _hash_finalize_:
   ...
   *** If `b` is a multiple of `VLEN` and `b` > `VLEN`, then we can issue one or more ``ace.exec`` instructions of Form C, i.e., "```ace.exec Vd,Cn|C[Xn]```", to output the hash:
