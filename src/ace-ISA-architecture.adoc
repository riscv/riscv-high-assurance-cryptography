...
* `F` = `00`, `r` = 0 {implies} Form A: Context type with the context given as `Ks`.
* `F` = `00`, `r` = 1 {implies} Form A: Context type with the context given indirectly as `K[Xs]`.
* `F` = `01`, `r` = 0 {implies} Form B: Metadata type.
* `F` = `10`, `r` = 0 {implies} Form C: Internal capacity type.
* `F` = `11`, `r` = 0 {implies} Form D: ACEIOBUF.
...
saved_SN_exists : bool;     // this is an ACE unit state
enc_key : bits(256);
auth_key : bits(128);
SIV : bits(128);
tag : bits(128);
