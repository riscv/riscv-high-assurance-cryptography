...
Two further CSRs: `aceconfigured` and `acedirty` are used to track the configuration and dirtiness of lockers in a more fine-grained manner:

. `aceconfigured` has a size of XLEN bits, of which only the 32 least significant bits are currently used.
``aceconfigured[``__i__``]`` is 1 if Locker #_i_ is configured, otherwise it is 0.

. The `acedirty` has a size of XLEN bits, of which only the 32 least significant bits are currently used.
``acedirty[``__i__``]``  is 1 if Locker #_i_ is (potentially) dirty, otherwise it is 0.
If Locker #_i_ is unconfigured, then ``acedirty[``__i__``]`` must read 0.

Using these two CSRs, privileged context switching code needs to save only the dirty lockers and restore only the configured ones.

Since we need a mechanism to potentially implement and track lazy-loading after a migration to a device with a smaller LF, we see in the next section how to use it and also implement general lazy-loading.
...
We now proceed to list the possible states a locker may have in the case we where we implement lazy loading.
Each state is represented by a 3-bit value, also given (and explained later).
...
** Preempted from this state to context save in privileged code: Nothing to save.
** *Usage* or *export* cause a trap to allow the OS to lazy load a CC's value in the locker for the current process first, then set `dirty <- 0`, `lazy <- 0`.
    The instruction is restarted unrestricted and operates from the state *CLEAN*.
    The instruction may cause the locker to become *DIRTY*.
...
*ALIEN* (`0b011`)::
`[.darkred]#*Description:*#{nbsp}`[.darkred]#_Locker is configured for an alien process; no value to be lazy-loaded for current process._# +
...
*ALIEN_LAZY* (`0b111`)::
`[.darkred]#*Description:*#{nbsp}`[.darkred]#_Locker is alien-configured; there is a value to be lazy loaded for the current process._# +
...
While restoring the state for a process, for a given locker, the privileged code can observe following situations, where we also record what it has to do in case every locker access is implemented as lazily as possible:
...
NOTE: The information in the Completion field is necessary to make sure that any cryptographic operation attempted on a locker containing an incomplete CC raises an error or is trapper.
