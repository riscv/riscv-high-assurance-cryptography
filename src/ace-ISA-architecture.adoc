...
For instance, SVE does not guarantee precise resume semantics like vstart does; the architects chose a simpler model where the instruction is either completed or restarted depending on implementation choice.
...
In particular, these issues arise even if modes are not implemented as programmer-facing features, since an AEAD mode of operation must be used to protect the confidentiality and integrity of any exported data.  And this leads to a simple observation: while architecting and implementing `vstart` is easy (no matter how brilliant the concept is), a similar mechanism for resuming cryptographic operations is way more complex.  The security of a mode of operation can break completely if any form of manipulation or observation of the state is possible.
...
and ask that all operations, including those that require memory accesses, be either completed before any interrupt or completely resumed afterwards.
However, this model would have represented a major departure from the current RISC-V approach.

Therefore, we have opted for an (unavoidable) _minor_ departure from a verbatim copy of the V extension approach.
We introduced a more fine-grained approach to handle partial operations, and performed a security analysis of difficult cases.
A challenging case follows a context switch occurring during an import operation, where the partial imported context must be exported, and later it must be resumed:
Resumption involves properly decrypting and authenticating the saved partial context, and then retrieving the old nonce, and SIV, reconstructing the partially computed hash from the now re-imported partial state using the original nonce and SIV, and continuing the operation from where it left off.  The chosen AEAD mode of operation lends it to a mostly linear sequence of memory reads, which is crucial for efficient resumption, and the fact that the partially saved information is not revealed eliminates the risk of successfully forging a different state.
