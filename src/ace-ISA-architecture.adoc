...
The Incomplete CR Metadata Header (ICREM) must distinguish between incomplete CRs following interrupted
...
|   [5:2] |   4  | Locality               | A field which is either -1 or an index into the Locality Secrets Table.
|   [6:6] |   1  | Variant                | The value is 0 if the configuration was interrupted at initialization, and
                                            the value is 1 if the configuration was interrupted while importing.
|   [7:7] |   1  | Reserved               | Reserved for future use.
|  [31:8] |  24  | Payload_Length         | Length of the payload in bytes (excluding the first 32 bytes)
| [63:32] |  16  | Reserved               | Zero for now.
|===

Fields Algorithm, SC_Protection, State Number, Direction_Policy and, perhaps even more notably, the Usage Control policies UC_Policy_{empty}_i_ (_i_ = 0 .. 3) are omitted, the reason being that usage of incomplete CRs is forbidden by the architecture anyway, and therefore the space can be used for other purposes.
...
Description::
`Kd` or `K[Xd]` denotes the CR to be cleared.
...
The new state, if allowed, is written into the State_Number field of the metadata, unless the algorithm requires more than 16 states, in which case an ad-hoc field must be defined in the context (for both internal representation and SCCs).
