 ...
 ** In state _hash_last_block_: TBD
...
//   .. `Vs` is written to `block` starting at byte offset `partial_block_len`.
//   .. `partial_block_len <- partial_block_len + VL*SEW`.
//   . else if `partial_block_len+VL*SEW == b`, then the CR is *invalidated*.

  ** In state _hash_finalize_: TBD
