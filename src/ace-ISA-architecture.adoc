...
For instance, the CRF must hold at least an instance of the largest possible CR or the largest CR pair for CR cloning or the derivation of a new CC from an existing one. An optional, possibly approximated, and implementation dependent mechanism to estimate the required capacity is defined in <<ACE-instruction-size-internal>>.
====
...
This is followed by the _content_, i.e., the key(s) and possibly other information, in a format that is algorithm-specific and that is defined in <<ACE-algorithms>>
for the currently architected algorithms.
...
The metadata section of a SCC has a fixed architectural format (in fact it is the same format as in the Initialization Inputs), while the content format is algorithm-dependent. cf.{nbsp}<<ACE-algorithms>> for the latter.
...
or to the _completed_ or _invalidated_ states (cf.{nbsp}<<ACE-state-constants>> and <<ACE-algorithms>>).
...
On RV64, instruction works as follows:
...
In particular, only a single form of `ace.exec` is allowed (cf.{nbsp}<<ACE-instruction-exec>>) and whenever we write “only Form X `ace.exec` instructions are allowed”, we also allow a replacement sequence using the `ace.input`, Form D `ace.exec`, and `ace.output` instructions as described in <<ACE-replacement-instructions>>.
...
We now describe the state machines of the various Ascon-based algorithms standardized by the NIST.
(((ACE, algorithm, Ascon)))
...
[WARNING]
In the TG there seems to be a consensus that we can resume init/import after a page fault or a similar event, but on context switches we do not export partial CRs. The CRs are cleared, and the instruction is restarted.

Our security analysis considered various scenarios, including a challenging case where a context switch interrupts an import operation, requiring the partially imported context to be exported and later resumed. We concluded that although privileged code can always replace an SCC in a saved process's context or memory, it cannot exploit the resumption mechanism to compromise the confidentiality or integrity of an SCC.
