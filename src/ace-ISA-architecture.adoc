...
|  3   | ace_state_hash_start          | Start (and initialize) a hash function or the sub-function of an algorithm.
|  4   | ace_state_hash_absorb         | Update the internal state of a hash function with new data.
|  5   | ace_state_hash_finalize       | Finalize the internal state of a hash function and prepare it for outputting the result.
 ...
 ** In state _set_public_value_, we can only issue an `ace.exec` of the form{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`,{nbsp}{nbsp} which sets the IV value to `Vs`.
  *** If{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`,{nbsp}{nbsp} is issued more times, it will just update the IV value again.
  *** Any other form of `ace.exec` will produce a null output and *invalidate* the CC.
 ** In state _operate_, we can only issue an `ace.exec` of the form `ace.exec Vd, Cn|C[Xn]`, which performs
  . If in a "not-X" mode, `tmp <- keystream_block(IV @ counter)`, +
    else `tmp <- keystream_block(IV xor counter)`
  . `tick_counter()`,
  . `Vd <- tmp`.
 ** In state _operate_, any other form of `ace.exec` will will produce a null output and *invalidate* the CC.
 ...
 ** In state _initial_:
  ...
  *** Any `ace.exec` operation will produce null outputs and the CC is *invalidated*.
 ** In state _set_public_value_, we can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp}{nbsp} to set the `IV <- Vs`.
    *** If no such instruction is issued, the value of the IV will remain zero.
    *** If{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`,{nbsp}{nbsp} is repeated, it will just re-write the IV.
 ...
 ** `auth_tag`: `b` bits.
+
Note that there are no fields for the lengths of the AD and of the plaintext, hence these values must be maintained by the calling software.
 ...
 * *Behavior:*
 ** In state _initial_ :
  *** If any `ace.exec` operations are issued, these will produce null outputs and the CC is *invalidated*.
 ...
 ** In state _hash_absorb_, we can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp}{nbsp} to accumulate a value in the auth_tag, namely:
 ...
 ** If the _encrypt_ state, we can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp}{nbsp} to
   encrypt `Vs` into `Vd`, and absorb the ciphertext `Vd` into the auth_tag, namely
    ...
    . `auth_tag <- auth_tag * hash_key`.
 ** In state _decrypt_, we can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp}{nbsp} to
   ...
   . `counter[b/4-1:0] <- counter[b/4-1:0] + 1`.
       If `counter[b/4-1:0]` rolls back to zero, the CC is *invalidated*.
 ** In state _hash_finalize_, we can only issue `ace.exec` form{nbsp}{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp}{nbsp} where `Vs = len_A @ len_C` (two fields of `b/2` bits, to compute the authentication tag and complete the algorithm as follows:
    . `auth_tag <- auth_tag xor Vs`,
    . `auth_tag <- auth_tag * hash_key`, and
    . `auth_tag <- auth_tag xor encrypt_block(key,counter[b-1,b/4] @ 0[b/4-1,0])`.
    . Finally, the auth_tag is returned in `Vd`, and the state is changed to _completed_.
