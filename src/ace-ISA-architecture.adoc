 ...
 ** `ace.init`:
  *** Once the first 8 bytes are read, the metadata must be completely processed and imported in the locker.
  *** Whenever 8+16__i__ bytes have been read, also _i_ blocks of content must be processed and imported in the locker.
...
NOTE: This instruction shares its encoding with `ace.clear` with `3b'111` as instruction code (use a GPR to select the locker).
