...
* `SmaceCSK` => Defines the Root Context Sealing Key CSRs (`macecsk{0..7}`), which are all MRO.
...
For instance, if a less expensive key derivation method is allowed (<<ACE-SCC-AEAD>>), we could rely entirely on the cryptographic approach based on localities, and eliminate the filtering according to the IDs of processes VMs and Hypervisors/SDs. In this case we would not need a 128-bit additional block for filtering and a 64-bit additional block would suffice and leave also enough space for additional policies.

{empty}
(((Cryptographic Context,metadata,format)))
The metadata section is 64 or 192 bits long.
...
|   [0:0] |   1   | Format                   | Value is 0 for the current version of the metadata format.
|  [11:1] |  11   | _Algorithm_              | Cryptographic algorithm (such as primitive, mode, or protocol).
| [13:12] |   2   | _DirectionPolicy_        | Selects the allowed direction for the cryptographic operation.
| [15:14] |   2   | _SCProtection_           | Side-Channel Protected Implementation (0 if not protected, 1 if protected).
| [16:16] |   1   | _KeyType_                | Key given by explicit value / System Key (index).
...
If any of these bits is set, the 128-bit block described in <<ACE-metadata-extension>> (see <<ACE-combined-filters>>) is appended to the metadata section.
| [47:36] |  12   | _ExtraContent_           | Length of additional content section (as 64-bit blocks). If non-zero, there is an additional block of data at the end of the _content_.
| [63:48] |  16   | Reserved                 | Value is 0 for the current metadata formats.
|===

[[ACE-metadata-extension]]
.Format of the optional metadata filtering extension
[width="100%",cols="^9%,^7%,14%,70%",options="header"]
|===
|     Field | Width | Name                    | Description
|    [31:0] |  32   | _SFilter_               | Value to match against `scontext`.
|   [63:32] |  32   | _MFilter_               | Value to match against `mcontext`.
|   [95:64] |  32   | _HFilter_               | Value to match against `hcontext`.
|  [127:96] |  32   | _TFilter_               | Value to match against an expiration date.
|===
...
Bits [63:33] and the optional metadata filtering extension
are confidential and will be encrypted upon export as an SCC.
...
For encryption/decryption primitives or modes, the DirectionPolicy field determines whether the CC may be used for encryption (lower bit), decryption (higher bit), or both.

For certain asymmetric primitives, the DirectionPolicy field determines whether they can be used for signature generation (lower bit), verification (higher bit), or both.

The DirectionPolicy field can be used by other primitives to encode additional algorithm identifiers, thereby expanding the available encoding space of the Algorithms field.
...
If the Locality Field of a CC is all zeros, then the CSK is not tweaked used.
Otherwise, this value minus one is used as an index _i_ in the LST, and the encryption depends on both the CSK and LST[_i_].
...
| (No Locality binding)     | No CSK derivation. CC bound to Root CSK only.                                                              | —  | — | 0x00
| _ChipDesignerSecret_      | Identifies the designer of the SoC. Permanent.                                                             | SoC | No | 1
| _ChipModelSecret_         | Identifies specific chip model/family. Permanent.                                                          | SoC | No | 2
| _ChipUniqueSecret_        | Unique for each SoC. Permanent.                                                                            | SoC | No | 3
| _OEMSecret_               | Provisioned by OEM to differentiate their products from the competition.
                              Permanent, or reconfigurable via an implementation-defined HW authenticated mechanism.                     | Device | No | 4
| _ProductSecret_           | Identifies device/system type/model (product). Permanent or reconfigurable like OEMSecret.                 | Device | No | 5
| _DeviceSecret_            | Unique device (entire system) secret (_not_ the device ID such as a serial number). Shared across all harts in a SoC and shareable across multiple SoKs in a single device. Sharing mechanism implementation-defined, i.e., not architected. Reconfigurable like OEMSecret.                                  | Device | No | 6
| _BootSecret_              | Unique per the boot session of the host hardware and its software stack. Configured by M-mode during the boot process of the physical device (see <<ACE-CSR-maceBootSecret>>). | Device | M | 7
| _VBootSecret_             | Unique per the boot session of the virtualized hardware and its software stack.
Regenerated by H or higher at each virtual boot of a VM (see <<ACE-CSR-haceVBootSecret>>).                                               | VM | H/M | 8
| _SSecret_                 | Can be configured by Supervisor (see <<ACE-CSR-saceSSecret>>).                                             | Process  | S | 9
| _Reserved_                | Reserved for future use.                                                                                   | Impdef | Impdef | 10-255
|===

The only restriction on the values of the LSs is that they must be randomly generated and be non-zero.

NOTE: Example: If a key's metadata specifies a Locality field value of 2, the import/export key is derived from the CSK and the OEM Secret.
...
The proposal for combination would be to encode 1-7 in three bits, then having a secret per SD/hypervisor, _VBootSecret_ and _SSecret_, selectable each with one bit, so a total of 6 bits. Note that the secret for M-mode (_BootSecret_), is in the first group.
An alternative is to just multiply the non-zero 125-bit strings in the binary Galois field defined by stem:[P(x)=x^{125}+x^{15}+x^{13}+x^{3}+1].
(for degrees 124 and 126 we can use stem:[P(x)=x^{124}+x^{7}+x^{4}+x^{3}+1] and stem:[P(x)=x^{126}+x^{8}+x^{7}+x^{2}+1]).

// P(x)=x^125^+x^15^+x^13^+x^3^+1.
...
If the FilterMode field is non-zero, an additional 128-bit block is added to the metadata to implement filtering according to secrets set by S/VS, HS, and M, and expiration time, depending on the bits set in FilterMode.
...
[WARNING]
====
* To configure the `macecsk`, `maceBootSecret`, `haceVBootSecret`, and `saceSSecret` CSR groups we use the Indirect CSR Access defined in cite:[RISCV-ISA-Priv] instead.
...
NOTE: We assume that  M-mode firmware is aware of whether the CSK is configurable, ephemeral, or fixed.
If not configurable, only a fixed CSK is guaranteed to persist across power cycles.
For reconfigurable CSKs, system-specific mechanisms for persistence and cross-device transfer (e.g., to support VM migration) are the responsibility of M-mode firmware.
CSK migration to any other CSK-receiving endpoint,
...
NOTE: Alternative approaches to re-configuring the CSK are allowed.
For instance, M-mode could have access, through an implementation-defined mechanism, to a secure IP block like Caliptra cite:[caliptra], which can be used to configure the CSK, and also to provide the mechanism for CSK export.
An advantage of this method is that it also shields the values of the CSKs from M-mode, which only would request a reconfiguration using a token or a handle.
...
....
+
The Form field and the 'r' bit assume these values:

* Form field = `00`, `r` bit = 0: Context type with the context given as `Ks`.
* Form field = `00`, `r` bit = 1: Context type with the context given indirectly as `K[Xs]`.
* Form field = `01`, `r` bit = 0: Metadata type.
* Form field = `10`, `r` bit = 0: Internal capacity type.
* Form field = `11`, `r` bit = 0: ACEOIOBUF.
...
* `Xs` inherits the format of the first 64-bit block of the metadata section.
Only the fields DirectionPolicy, SCProtection, Locality, UCPolicy, and FilterMode may be defined.
All other fields must be 0, else ACE error _ace_err_invalid_value_ is raised (<<ACE-error-codes>>).

* `Xs`.DirectionPolicy is used to set the new DirectionPolicy of the CR.
No operation may be enabled, any attempt to do so raises ACE error _ace_err_invalid_value_.
In some cases, a zero value is not permitted, such as disabling both encryption and decryption.

* `Xs`.SCProtection is used to turn on side-channel protection or raise its level, if such protection is implemented.
Attempting to remove or downgrade side channel protection, or to set a non-implemented level raises ACE error _ace_err_invalid_value_.

* If `Xs`.Locality is non-zero, then it can be used to restrict the Locality. Then, when the CR is exported as a CR, it will use the new tweak in the AEAD mode. Generalizing the Locality is not permitted. Attempting to do so raises ACE error _ace_err_invalid_value_.

* A zero in any of the three bits `Xs`.UCPolicy means that the corresponding policy is _not_ changed. +
A one in any of the three bits `Xs`.UCPolicy means that usage by the corresponding mode is not allowed, and the corresponding bit in the metadata of the destination CC is set to 1.

* A zero bit in `Xs`.FilterMode means that the corresponding policy set in `m`.FilterMode is _not_ changed. +
A set bit in `Xs`.FilterMode will add the corresponding policy to `m`.FilterMode only if none was programmed before, otherwise error ACE error _ace_err_invalid_value_ is raised. If `Xs`.FilterMode is non-zero, then `X[s2+1] @ Xs2` must be provided and it must have the same format as the metadata filtering extension in <<ACE-metadata-extension>>, where only the fields corresponding to the actual filters added are considered and the remaining bits are ignored.
* When FilterMode is set on a CC where it was previously unset, the SCC will contain the additional metadata filtering extension, where previously it would not have been included. Thus, the size of the SCC may be larger than a previous version.
--
+
On RV32:
...
(or 126 with <<ACE-SCC-alt-derivation>>). An alternative key derivation <<ACE-SCC-alt-derivation>> method halves the number of AES256 calls by using, essentially, the CMAC based random generation, which is secure for up to 2^64^ key generations with the same CSK, and then the security gracefully degrades (after 2^96^ keys there is still an overall security level of 128 bits).
...
[[ACE-CSK-vs-ID]]
=== Programmable CSKs vs. ID-based Usage Control
...
Since filtering by `(v)sacefilter` is inactive when the hart is in S mode or higher, context switching at U can be performed without modifying the CSK.
. At higher privilege levels, not only the usage, but even the import of CCs can be restricted by a programmable CSK.
...
Cryptographic domain separation via the CSK is intended for Virtual Machines, Supervisor Domains, Worlds, and TEEs, as these demand stronger security guarantees.
...
Hardware support—such as automatic CSK switching based on `*acefilter`, could alleviate this burden.
