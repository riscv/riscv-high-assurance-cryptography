...
--
+
*System Keys* are given by an index into an internal, system specific System Keys Table (SKT). For each algorithm as specified in <<ACE-algorithms>>, the part of the content that normally contains the key(s) is replaced by its index into the SKT-for both Initialization Input and the SCCs. The actual value of the key is never included in exported content. For threshold implementations, ACE may copy the key material from the SKT into the CR and expand it into multiple shares.
+
*Internally generated keys* are also system-specific. In the Initialization Input, the key material is replaced by a system-specific “recipe”, which is not defined by the ISA and may be non-portable. After internal key generation, the actual key value—not the recipe—is stored in the SCC content, consistent with other keys for the same algorithm. The Key_Type field in the metadata remains 2 across all CC representations.

//
State_Number field::: {empty}
+
--
The State_Number field must be zero in the Initialization Input.
...
Both metadata and the serialization of the content are authenticated, but only the latter is encrypted.
...
where z : int ← size(content);    // content as serialized "view", size in bytes derived from metadata
ASSERT(8 divides z);
...
NOTE: The returned value can significantly differ from the internal resource usage to store the CC's data in a CR—it can be larger (for simple primitives, because of the nonce/SIV overhead) but also smaller (for threshold implementations, where keys are exported in reconstructed form).
...
. For algorithms that require more than one key, such as XEX/XTS, ALL the keys are referred to by a single index, i.e., a single 64-bit value in the initialization input and the SCC. If the SKT does not provide entries with multiple keys for such algorithms, these algorithms cannot be used with system keys.
 ...
 ** To transition to state _hash_absorb_:
 the ``ace.state`` instruction must be used _without any additional inputs_, since the IV is already included in the Initialization Input.
...
For instance, for GCM-SIV (cf.{nbsp}<<ACE-GCM-SIV-mode>>), `nonce` would be included in the Initialization Input exactly as `IV` is included in the Initialization Input for GCM, and the transition to _hash_absorb_ would also use a Form A ``ace.state``.
...
In this case the state _set_aux_value_ is skipped, and the initialization input has the following form
