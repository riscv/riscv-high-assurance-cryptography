...
Management operations include *configuration* (e.g., CR initialization, import of an SCC, cloning or key derivation from one CR to another) and *export* of a CR as an SCC.
...
From the Sdtrig extension we reuse the `scontext`, `mcontext`, and `hcontext` CSRs. Sdtrig is not required per se.

===== Smcsrind/Sscsrind

From the Smcsrind/Sscsrind extensions we reuse the Indirect CSR access mechanism.
...
The support for the vector register file in ACE instructions and the implementation of the ACEIOBUF are not mutually exclusive.
...
. The _Initialization Input_ (II) format, which is loaded into a CR to create a C; and
...
The II and SCC formats are described in <<ACE-init-export-import>> and <<ACE-format-SCC>>, respectively.
...
System Keys are given by an index into an internal, system-specific System Keys Table (SKT). For each algorithm as specified in <<ACE-algorithms>>, the part of the content that normally contains the key(s) is replaced by its index into the SKT-for both II and SCCs. The actual value of the key is never included in exported content. For threshold implementations, ACE may copy the key material from the SKT into the CR and expand it into multiple shares.

System Generated Keys are also system-specific. In place of the II, the initialization operation reads a system-specific “recipe”, which is not portable. After internal key generation, the actual key value—not the recipe—is stored in the CR and SCC. The KeyType field in the metadata is 0.

//

===== StateNumber Field

The StateNumber field must be zero in an II.
...
|  14   | _ace_state_invalid_             | _invalid_           | The algorithm has abnormally terminated or initialization/import have been interrupted.
...
During init and import, the StateNumber field is set to _invalid_, and only at the end of the operation is it set to proper value, i.e., _initial_ or the value in the SCC (see <<ACE-init-export-import>>)
This means, by the rules in <<ACE-invalidation>>, that a partially initialized/imported CR cannot be exported. In particular, if a context switch occurs, that the more privileged code must unconfigure the CR, and upon resumption, the instruction will be restarted.  Only an invalid CR with an `acestart` different from zero is recognized as an incomplete CR allowing an init or import operation to resume.
...
|  [14:1] |  14   | _Filter2_                | Filter to match against a `mcontext` field.
| [16:15] |   2   | Reserved                 | Reserved for future use.
| [30:17] |  14   | _Filter3_                | Filter to match against a `hcontext` field.
| [32:31] |   2   | Reserved                 | Reserved for future use.
| [63:33] |  31   | _Filter4_                | Value to match against an expiration date.
|===
...
The subsequent bytes form the *content*—the key material and any algorithm-specific data—whose format and length are specified in <<ACE-algorithms>> for all architected algorithms. The II content's structure and size are entirely determined by the Algorithm and KeyType fields within the metadata.
...
* Invalid metadata section in `ace.init` or `ace.import`, other than ace_err_unimplemented, for instance:
 ** non-zeros in zero fields,
 ** invalid FilterMode for a 64-bit metadata block,
 ** non-implemented Locality value,
 ** StateNumber is non-zero for `ace.init`.
...
The II must be stored at an 8-byte aligned address, and its length must be a multiple of 8 bytes.
+
Similarly to vector store instructions, `ace.init` appears to other harts as a sequence of individual reads.
...
The operation is described in detail in <<ACE-init-export-import>>.
...
From the point of view of the system outside the ACE unit, `ace.init` and `ace.import` appear as a series of sequential memory reads, and `ace.export`, appears as a series of sequential memory writes.
...
. `II_content_size(x)` returns the size in bytes of the content section of an II. It depends only on the 11 bits `KeyType @ Algorithm`.
 ...
 ** `ace.error` always returns error _ace_err_invalid_CR_ after any operation involving the CR except for `ace.import` and `ace.export`, `ace.clear`, `ace.init`, and `ace.size`, which all work normally, and using `ace.state` to return to state _initial_.
...
Any CC with its algorithm implemented as a threshold will always initialize, import and export _all_ its keys and secrets as _single_, _reconstructed_ _shares_.
The II and SCC formats will be the same as for the non-threshold implementation variants of the same algorithm, with the only difference being the value of the Side Channel Protection bits in the metadata.
...
. In an II and in an SCC, the key field will contain the index of the key in place of the value.
...
The value of 1 may indicate both states _operate_ and _hash_absorb_ for the symmetric algorithms defined here.
