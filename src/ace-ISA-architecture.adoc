...
* Form C `ace.size` returns the amount of internal CRF capacity used by a CR with metadata section matching `Xn` on the Algorithm, SCProtection, and KeyType metadata fields. In this case, all other bits must be zero.
...
All other fields must be 0, else ACE error _ace_err_invalid_value_ is raised.
...
Let `m` be a metadata header.
// `{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}x` <- `m`._FilterMode_ @ `m`._KeyType_ @ `m`._DirectionPolicy_ @ `m`._Algorithm_ @ `m`._Format_.
The implementation provides the following functions and constants or an equivalent mechanism:

. `MH_size(m)` returns the byte length of the metadata header. In the current version of the specification, it is always 8.
. `PI_content_size(m)` returns the byte length of the content section of a PI.
   It does not include the length of the optional Metadata Extension block.
   It depends only on  `m`._Algorithm_, `m`._DirectionPolicy_, and `m`._KeyType_.
   It does not depend on `m`._FilterMode_.
. `SCC_content_size(m)` returns the byte length of the fixed part of the content section of an SCC.
   It depends only on  `m`._Algorithm_, `m`._DirectionPolicy_, and `m`._KeyType_.
   It does not depend on `m`._FilterMode_.
   It does not include the length of the variable part of the content of length _ExtraContent_ * 8 bytes, nor the length of the optional Metadata Extension block.
.  `ME_size(m)` returns the byte length of the metadata extension.
    In the current version of the specification, it is 0 when `m`._FilterMode_ is 0, and it is 16 otherwise.

The total size in bytes of a PI is therefore

`{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp} MH_size(m)` + `ME_size(m)` + `PI_content_size(m)`,

and the total size in bytes of an SCC is

`{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp} MH_size(m)` + 16 + `ME_size(m)` + `SCC_content_size(m)` + _ExtraContent_ * 8.
...
[]
----
m : bits(64) = metadata;               // from the CR
SIV : bits(128);
tmp : bits(128);
enc_key : bits(256);
auth_key : bits(128);
...
// note: content[] here already includes the optional metadata extension
// at the beginning and the optional extra content at the end
content : array[z] of bytes;

// External memory: metadata header, SIV, content
M : array[MH_size(metadata) + 16 + z] of bytes;
