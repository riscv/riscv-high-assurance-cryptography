...
** Upon entering states _encrypt_ or _decrypt_, following operation are performed:
 ...
 . `Ktop <- enc_blk(K, Nonce[1..122] || zeros(6))`
...
** To move to _hash_finalize_enc_, the `ace.state` operation must have the additional argument, which sets the length of the final block of the plaintext in `final_blk_len`.

** In state _hash_finalize_enc_, only ``ace.exec`` instructions of Form A are allowed. The value +
 `{nbsp}{nbsp}{nbsp}Vs = IntTo{b/2}Bits(bit_length(AD)) @ IntTo{b/2}Bits(bit_length(plaintext))` +
 must be set by the software, then,
  "```ace.exec Vd,Kn|K[Xn],Vs```"
  computes the authentication tag and completes the algorithm as follows: [.red]#TBD#.
+
If `VLEN` is larger than `b`, the most significant bits of `Vs` (i.e., the "tail" of the vector) are ignored.
...
** To move to _hash_finalize_dec_, the `ace.state` operation must have the additional argument, which sets the length of the final block of the ciphertext in `final_blk_len`.
