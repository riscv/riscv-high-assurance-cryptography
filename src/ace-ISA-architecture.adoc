...
* One can load clear data into a CR via `ace.init` (cf.<<ACE-instruction-init>>), but
...
* A CR can be *exported* in an authenticated, encrypted format via `ace.export` (cf.<<ACE-instruction-export>>) creating a Sealed Cryptographic Context (SCC).
A SCC can be later re-*imported*. into a CR, but the process, requiring authentication, may fail if the data in memory was altered.
Conventional registers do not-and do not need to-provide such functionality.

* The external and internal representations of a CR may have different sizes, unlike conventional registers, which essentially act as very fast pieces of memory,
...
* The amount of the capacity of the CRF used by a CR depends on the configured algorithm.
  ...
  In this case, ACE returns _ace_err_memory_ (cf.{nbsp}<<ACE-error-codes>>), and software must free capacity by unconfiguring one or more CRs.
