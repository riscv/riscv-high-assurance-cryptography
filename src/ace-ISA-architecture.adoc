...
| [33:32] |   2  | Filter-Mode            | Mode at which to apply the filter: 0 = none; 1 = U; 2 = (V)S; 3 = H.
| [55:34] |  22  | Filter                 | Value to match against the `*acefilter` CSRs: See <<ACE-CSR-filters>> for details.
| [63:56] |   8  | Reserved               | Zero.
|===
...
UNINTERRUPTIBLE_BLOCK_START
|   M[0 .. 15] = N[63:0]
|              @ (metadata[63:32] xor AESE(Kenc, 1'b1 @ SIV[94:0] @ zeros(32))[31:0])
|              @  metadata[31:0];
UNINTERRUPTIBLE_BLOCK_END
...
foreach(i from 0 to num_blocks - 1) {
    ...
    |                       AESE(Kenc, 1'b1 @ SIV[94:0] @ (binary(i+1)[31:0]))[63:0];
    |   else
    |       M[32+i*16 .. 47+i*16] = content[i*16 .. 15+i*16] xor
    |                       AESE(Kenc, 1'b1 @ SIV[94:0] @ (binary(i+1)[31:0]));
    UNINTERRUPTIBLE_BLOCK_END
}
----
...
UNINTERRUPTIBLE_BLOCK_START
|   SIV = M[20 .. 31];
|   metadata[63:32] = metadata[63:32] xor AESE(Kenc, 1'b1 @ SIV[94:0] @ zeros(32))[31:0];
|   tmp = 64'b0 @ metadata;
