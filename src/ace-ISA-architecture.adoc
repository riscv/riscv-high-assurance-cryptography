...
[WARNING]
Discussions are still ongoing on what subset will be required, and if a dependency on just the GPRs can be designed as well.
...
In any algorithm and in any state, only certain instructions are permitted.
Any other operation-including any non allowed Form of `ace.exec`-will result in the CR being *invalidated*.
...
ace.state K0, #ace_state_hash_finalize_enc
...
ace.state K0, #ace_state_hash_finalize_dec
...
** In state _hash_finalize_enc_, only an {nbsp}``ace.exec``{nbsp} of Form A is allowed, i.e.,
...
for i = 0 to floor(AD_len/128)-1 do
    - load V0 from address AD+16*i
    ace.exec K0, V0                                // Absorb it into hash
...
// Encrypt plaintext and continue computation of auth_tag
ace.state K0, #ace_state_encrypt
for i = 0 to floor(PT_len/128)-1 do
    - load V0 from address PT+16*i
    ace.exec V1, K0, V0
    - store V1 at address CT+16*i

X1 ← PT_len mod 128                                // Can be zero
ace.state K0, #ace_state_hash_finalize_enc,X1
if (X1 mod 128 != 0) then
    - load V0 from address AD+floor(AD_len/128)*i  // may be a shorter load
