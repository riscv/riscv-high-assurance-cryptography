...
* `absorb_into_hash(data)` absorbs data into the hash function by performing a step of (a variant of) GHASH or POLYVAL as specified by the algorithm
  ...
  . `auth_tag <- auth_tag xor Vs`.
  . `auth_tag <- auth_tag * hash_key` where `*` denotes either Galois Multiplication in an algorithm-specific Galois Field for GHASH, or
      Montgomery multiplication in an algorithm-specific Montgomery domain.
State Machine:::
