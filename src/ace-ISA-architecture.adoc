...
The “*_transient_” states are used during long operations that require additional information.
 ...
 ** _set_scalar_ can be skipped if HasScalar == true, and it can be repeated.
 ** * -> _point_mul_ (only if HasPoint == HasScalar == true) -> _output_ -> _initial_
 ** * -> {_set_hash_, _set_public_key_} -> _sign_generate_ (only if HasGenerator & HasPoint) -> _output_ -> _completed_
 ** * -> {_set_hash_, _set_public_key_} -> _set_signature_ (only if HasGenerator & HasPoint & HasScalar) -> _sign_verify_ -> _completed_
