...
It the ARC decides for the simpler approach, then some simplifications can be made: we do not need `acestart`, `aceiobuftop`, and the formats for partially saved contexts. The overall changes to the architecture would be relatively small, but responsiveness of the system for real-time applications, especialyl on smaller cores, may be adversely affected.
