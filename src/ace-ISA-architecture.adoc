...
* *States:* _initial_ (0), _set_public_value_ (7), _hash_absorb_ (3), _hash_finalize_ (4), _hash_finalize_dec_ (5), _hash_verify_ (6), _encrypt_ (1), and _decrypt_ (2).
* *Allowed State Transitions:*
 ** _initial_ -> _set_public_value_.
 ...
 ** _encrypt_ -> _hash_finalize_enc_, _initial_.
 ** _decrypt_ -> _hash_finalize_dec_, _initial_.
 ** _hash_absorb_ -> _initial_.
 ** _hash_finalize_enc_ -> _initial_.
 ** _hash_finalize_dec_ -> _hash_verify_, _initial_.
 * *Behavior:*
 ** In state _initial_ :
 ...
 ** If the _encrypt_ state, we can only issue an {nbsp}``ace.exec``{nbsp} of the form{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp} to
   encrypt `Vs` into `Vd`, and absorb the ciphertext `Vd` into the auth_tag, namely
    ...
    . `auth_tag <- auth_tag * hash_key`.
 ** In state _hash_finalize_enc_, we can only issue an {nbsp}``ace.exec``{nbsp} of the form{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp} where
 ...
 ** In state _hash_finalize_dec_, we can only issue an {nbsp}``ace.exec``{nbsp} of the form{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp} where
    `Vs = len_A @ len_C` (two fields of `b/2` bits), to compute the authentication tag and complete the algorithm as in
   . `auth_tag <- auth_tag xor Vs`.
   . `auth_tag <- auth_tag * hash_key`.
   . `auth_tag <- auth_tag xor encrypt_block(key,counter[b-1,b/4] @ 0[b/4-1,0])`.
