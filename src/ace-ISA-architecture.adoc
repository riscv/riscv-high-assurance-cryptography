...
Its possible values are described in <<ACE-aces-encoding>>, we report also <<fsxsencoding>> from the privileged spec.
...
[[fsxsencoding]]
.Recall the encoding of FS[1:0], VS[1:0], and XS[1:0] status fields
[%autowidth,float="center",align="center"width="60%",cols=">10%,<16%,<74%",options="header",]
|===
...
See <<ACE-CSR-enable>> and <<ACE-ACES-field-in-status>> for more details.
