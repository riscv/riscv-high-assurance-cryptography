 ...
 ** When transitioning from _set_public_value_ to either _encrypt_ or _decrypt_, the following operations are performed:
  . First, `mask <- encrypt_block(key2,mask)`. (The initial mask value is the encryption of the tweak.)
 ...
 ** `counter`: `b/4` bits.
 ** `auth_tag`: `b` bits. +
 If the `SIV` is `b` bits long, its value is the encryption last value of `auth_tag` with the most significant bit set to zero. +
 Some variants have a `3b/4`-bit `SIV`. A full `b`-bit `auth_tag` value is kept while absorbing data.
 The public `SIV` value is obtained from the encryption of the last value of `auth_tag` with the most significant bit set to zero
 by removing the `b/4` least significant bits before outputting it.
 In this case, in the algorithm description the `b/4` least significant bits of the content field `auth_tag` are just zeroed to keep compatibility with the modular addition of the counter value to these bits (i.e., without carry to the most significant `3b/4` bits).
 ** `enc_key`: `k` bits.
 ** `auth_key`: `b` bits.
 ...
 ** In state _initial_ :
  ...
  ** In state _set_public_value_:
    *** We can only issue  ``ace.exec`` instructions of the form{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp} to set the `nonce`.
  ...
  ** In state _hash_absorb_:
   *** We only issue  ``ace.exec`` instructions of the form{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp} to accumulate a value in the `auth_tag`:
   i.e., performing `absorb_into_hash(Vs)`
   *** Other forms of `ace.exe` are not allowed, they will produce null outputs and the CC is *invalidated*.

  ** In state _hash_finalize_enc_,
  we can only issue an {nbsp}``ace.exec``{nbsp} of Form A{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp} where it is the responsibility of the user to set +
  `Vs = IntTo{b/2}Bits(bit_length(AD)) @ IntTo{b/2}Bits(bit_length(plaintext))`, +
  to finalize the computation of the SIV/tag, by performing:
    . `absorb_into_hash(Vs)` (which modifies `auth_tag`).
    . `tmp <- encrypt_block(enc_key, 1b0 @ ((auth_tag xor N)[b-2:0]))`
  ...
  ** In state _encrypt_, we can only issue  ``ace.exec`` instructions of the form{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp} to encrypt `Vs` into `Vd`.
   . If `counter = 0`, the CC is *invalidated*.
   . ``Vd <- Vs xor encrypt_block(key, 1b1 @ `` +
    {nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}
    ``@ auth_tag[b-2:b/4] @ (auth_tag[b/4-1:0] + counter % 2^b/4^) )``.

 ** In state _decrypt_,  [.red]#To be completed.# we can only issue  ``ace.exec`` instructions of the form{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp} to
   decrypt `Vs` into `Vd`, namely and then absorb the plaintext `Vs` into the `auth_tag`, namely
     . If `counter = 0`, the CC is *invalidated*.
     . `absorb_into_hash(Vs)`.
     . if (`SIV` is `b` bits long) then +
       {nbsp}{nbsp}``Vd  <- Vs xor encrypt_block(key, 1b1 @`` +
       {nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}
       ``@ auth_tag[b-2:b/4] @ (auth_tag[b/4-1:0] + counter % 2^b/4^))`` +
       else (i.e., `SIV` is `3b/4` bits long) +
       {nbsp}{nbsp}``Vd  <- Vs xor encrypt_block(key,SIV @ counter)``
     . `counter <- counter + 1`.

 ** In state _hash_finalize_dec_, we can only issue  ``ace.exec`` instructions of the form{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp} where
