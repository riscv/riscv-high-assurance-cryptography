...
Implementations *must* provide a suitable _Locality_ field value to the ACE unit during CR provisioning from the SKT, set usage policies that are at least as strict as the key's own.

NOTE: Bit 7 can be used to define additional Localities. These could be used by revocation mechanisms.
...
input to the tag computation function, since it requires the same number of additional Galois Multiplications.
// This approach also leverages the existing , since it is an addition to the total payload, and thus not forgeable, while keeping the values secret, and thus
