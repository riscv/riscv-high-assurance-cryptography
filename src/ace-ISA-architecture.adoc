...
ACE provides different architected layers of usage control:

. CC usage can optionally be constrained by filters configured at various modes and that must be matched at lower-privileged modes against values embedded in the metadata extension. The filters could correspond to `scontext`, `hcontext`, `mcontext` (respectively 31, 14, and 14 bits long) but longer CSRs could be be defined in their place to allow matching against longer string.
+
