...
If a non-allowed state transition is requested in `ace.state`, including requesting encryption or decryption from a CC not allowing it, an ACE error _ace_err_unimplemented_ is raised.

If a non-allowed `ace.exec`, `ace.state` instruction or form thereof is issued, an illegal instruction exception is raised.
