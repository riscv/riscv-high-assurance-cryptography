...
| [51:36] |  16   | _ExtraContent_           | If non-zero, there is an additional segment of data at the end of the _content_.
The length of additional segment is _ExtraContent_ 64-bit blocks.
| [63:52] |  12   | Reserved                 | Value is 0 for the current metadata formats.
|===
...
|   [63:32] |  32   | _MFilter_               | Value to match against `mcontext` (least significant 14 bits).
|   [95:64] |  32   | _HFilter_               | Value to match against `hcontext` (least significant 14 bits).
...
// `{fournbsp}{fournbsp}x` <- `mh`._FilterMode_ @ `mh`._KeyType_ @ `mh`._DirectionPolicy_ @ `mh`._Algorithm_ @ `mh`._Format_.
...
. `PI_content_size(mh)` returns the byte length of the content section of a PI.
   ...
   It does not depend on `mh`._FilterMode_.
...
.  `ME_size(mh)` returns the byte length of the metadata extension.
    In the specification, it is 0 when `mh`._FilterMode_ is 0, and it is 16 otherwise.
...
|  1   | `ME_size(mh)`   | Metadata Extension
|  2   | `k/8`m or 8     | Key
|===
--
+
...
| Pos. | Size in Bytes   | Field
|  1   | `ME_size(mh)`   | Metadata Extension
|  2   | `k/8` or 8      | Key or System key index
|===
--
+
...
| Pos. | Size in Bytes   | Field
|  1   | `ME_size(mh)`   | Metadata Extension
|  2   | `k/8` or 8      | `key1` or System key index
|  3   | `k/8` or 0      | `key2` or System key index for `key1`
|===
--
+
...
| Pos. | Size in Bytes            | Name
|  1   | `ME_size(mh)`            | Metadata Extension
|  2   | If keyed: `k/8` or 8     | `key` or System key index
|===
--
+
...
+
--
[cols="^6%,^22%,<20%,<52%",options="header"]
|===
| Pos. | Size in Bytes            | Name                        | Notes
|  1   | `ME_size(mh)`            | Metadata Extension          |
|  2   | If keyed: `k/8` or 8     | `key` or System key index   |§
|  3   | `n/8`                    | `state`                     |
|  4   | `2`                      | `last_block_len`            | Only if this Hash/MAC has special internal treatment for a fractional last block that goes beyond padding and appending the length, which are handled by user software. Otherwise, this is zero.
|  5   | `2`                      | `block_base`                | Amount of data stored so far into `block`.  Only if used by the algorithm, else zero padding.
|  6   | `4`                      | `cumul_len`                 | Cumulative length of all data absorbed so far. Only if the algorithm must process and incorporate it in the hash on its own, else zero padding.
|  7   | `b/8`                    | `block`                     | It is included it the SCC only if there is a partial block, and the block must be stored separately from the `state`, and exclude it when not.
...
| Pos. | Size in Bytes   | Field
|  1   | `ME_size(mh)`   | Metadata Extension
|  2   | 16 or 8         | `key` or System key index
|===
--
+
...
| Pos. | Size in Bytes   | Field
|  1   | `ME_size(mh)`   | Metadata Extension
|  2   | 16 or 8         | `K1` or System key index
|  3   | 16 or 0         | `K2` or System key index in `K1`
|===
--
+
...
|  5   | 8               | `state`[3]
|  6   | 8               | `state`[4]
|  7   | 1               | `countdown` (from 3 down to 0)
|  8   | 7               | Padding
|===
--
+
...
NOTE: In the metadata, _only_ the curve type is embedded in Algorithm. For now, this is just 5 values.
...
| Pos. | Size in Bytes   | Field
|  1   | `ME_size(mh)`   | Metadata Extension
|  2   | 2               |  Bit 0: `HasGenerator` +
Bit 1: `HasScalar` +
Bit 2: `HasPublicKey` +
Bit 3: `HasHash` +
Bit 4: `HasSignature` +
Padding: 11 bits.
|  3   | 2               | `Progress`/`input_base`
|  4   | 2               | `Block_base`
|  5   | 2               | Padding
|  6   | `2b/8`          | `Generator` (X coordinate, Y coordinate): a `2b`-bit string representing Y @ X.
|  7   | `b/8`           | `Scalar`: `b`-bit string. Can be the private key.
|===
...
The `acemarchid` in this case is placed at Position 11.

[%autowidth,cols="^,^,<",options="header"]
|===
| Pos. | Size in Bytes                    | Field
|  8   | If `HasPublicKey`: +`2b/8`,
else: 0 | `PublicKey` (X coordinate, Y coordinate): a `2b`-bit string representing Y @ X.
|  9   | If `HasSignature`: `2b/8`,
else: 0 | `Signature` (R,S): a `2b`-bit string representing S @ R.
| 10   | If `HasHash`: `b/8`, +
else: 0      | `Hash`: a `b`-bit string representing the hash.
| 11   | If `mh`._FilterMode_ != 0: `b/8`, else: 0 |  `acemarchid`
| 12   | If `mh`._FilterMode_ != 0: `b/8`, else: 0 | Additional data (as in the *Internal State*). During signature generation this includes a `RandomNumber`, a `b`-bit string, erased upon completion, exported only if the signature operation is interrupted.
|===
...
`PublicKey`,
`Signature`, and
`Hash`—if they exist—, then this data, including `RandomNumber`, is discarded, and the operation is restarted.
--
+
