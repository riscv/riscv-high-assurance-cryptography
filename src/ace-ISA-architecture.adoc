...
* `Smacestatus` => Defines the ACES per CC CSRs (`scrstatus`, `vscrstatus`, `mcrstatus`).
...
| 0xXXX          | SRW       | `scrstatus`             | Per-CR dirtiness tracking
| 0xXXX          | HRW       | `vscrstatus`            | Per-CR dirtiness tracking
| 0xXXX          | MRW       | `mcrstatus`             | Per-CR dirtiness tracking
...
An ACE context status field, ACES, is added to `mstatus`[26:25] and shadowed in `sstatus`[26:25] (but, see <<ACE-CSR-mcrstatus>>).
...
The XLEN-bit register `scrstatus` tracks the dirtiness of CRs on a per-CR basis.

NOTE: If we implement lazy-loading in the architecture at some point, `scrstatus` may be made into a 64-bit register to track the laziness status of individual CRs in the upper 32 bits.
When XLEN=32, `scrstatush` would be a 32-bit register that aliases bits [63:32] of `scrstatus`, and `scrstatush` would not exist when XLEN=64.

``scrstatus[__i__]`` can take the values Clean (0), and Dirty (1), for CR #__i__.
Lazy-loading will require an additional CSR to track additional CR status (cf.{nbsp}<<ACE-LAZY>>).

The state of an individual CR in `scrstatus` after `ace.clear` or `ace.reset` (<<ACE-instruction-clear>>) is Clear.
...
When ``scrstatus[__i__]`` is set to Clean, executing any instruction that may change the CR #__i__ will change ``scrstatus[__i__]`` to Dirty.

NOTE: Implementations may also change ``scrstatus[__i__]`` from Initial or Clean to Dirty at any time, even when there is no change in CR #__i__ state.

NOTE: Context switching code will only see Clean or Dirty states. To check whether a CR is actually configured, `ace.size` (<<ACE-instruction-size>>) can be used, and if the instruction returns zero, thus means that CR is not configured, and any stale state for it shall be erased and released.

NOTE: `sstatus.ACES` is also in effect.

NOTE: T1he `scrstatus` CSR does not depend on the S extension.  In an M-U system, M-mode can use the CSR to optimize U-mode context switches.

[[ACE-CSR-vscrstatus]]
===== `vscrstatus`

The `vscrstatus` register exists only when the H extension is implemented.

The XLEN-bit register `vscrstatus` tracks the dirtiness of CRs on a per-CR basis.

NOTE: Similarly to `scrstatus`, if we implement lazy-loading in the architecture at some point, `vscrstatus` may be made into a 64-bit register to track the laziness status of individual CRs in the upper 32 bits.
When XLEN=32, `vscrstatush` would be a 32-bit register that aliases bits [63:32] of `vscrstatus`, and `vscrstatush` would not exist when XLEN=64.

When V=1, the following holds:

* ``vscrstatus[__i__]`` can take the values Clean (0) or Dirty (1), as defined for `scrstatus`, for CR #__i__.
* `vscrstatus` substitutes for `scrstatus`, so instructions that normally read or modify `scrstatus` actually access `vscrstatus` instead.

NOTE: `sstatus.ACES`, `vsstatus.ACES`, and `vscrstatus` are also in effect when V=1.
...
====
(((CSR, mcrstatus)))
A separate `mcrstatus` can be defined, _mutatis mutandis_, like `scrstatus` and `vscrstatus`, to allow M-mode to optimize context switches of Supervisor domains.
In this case, the `ACE` and `ACEIOBUF` fields in `sstatus` shall be unshadowed.

(A `ucrstatus` might become useful if the HFI extension gets approved, together with the fields in an additional `ustatus` CSR.)
====

//
[#discrete]
===== `*crstatus` Propagation.

NOTE: CR status propagates from the least privileged modes to the higher privileged ones.
Any instruction that changes bit \#__i__ of, say `vscrstatus` to Dirty, will also set the corresponding bit of `scrstatus` and `mcrstatus`.
Any instruction that changes bit \#__i__ of, say `scrstatus` to Dirty, will also set the corresponding bit of `mcrstatus`.
...
+
`ace.clear` unconfigures the state of the relevant CR by releasing its internal resources in the CRF and setting the corresponding entry in `*crstatus` to Dirty unless the CR was already unconfigured, in which case it does nothing.
...
Let us use value 0 in the entries of `crstatus` to support lazy loading, so we call this state *LAZY* in place of *OFF*.
...
The use of the **OFF** encoding in the per-CR fields of `crstatus` is a natural choice to denote an unavailable state that requires lazy loading to be made available. If all CRs were to be lazy-loaded, the ACES field would be set to **CLEAN** and not to **OFF**.
...
... The fields of `crstatus` must be expanded to three bits to accommodate a fifth status, *LAZY/DIRTY*;
... Another 32 bits worth of CSRs, say `crstatusext`, is used for this purpose; or
... The *INITIAL* state in `crstatus` is repurposed to denote this state, and to check whether a CR is unconfigured we check whether `ace.size` returns 0.
