...
If the Locality Field of a CC is zero, then the CSK is not used with a tweak.
Otherwise, this value of the Locality minus one is used as an index _i_ in the LST, and the encryption depends on both the CSK and LST[_i_].
...
[width="100%",cols="18%,43%,^9%,^9%,^7%,^7%,^7%",options="header"]
|===
| Name                      | Description                                                                   | Domain | Config |  Bits  | Value |Index
| _NoHWBinding_             | Independent of Chip, OEM, Device                                              |  --    |   --   |  2:0   |   0   |  --
| _ChipDesignerSecret_      | Identifies the designer of the SoC. Permanent.                                |  SoC   |   No   |  2:0   |   1   |  0
| _ChipModelSecret_         | Identifies specific chip model/family. Permanent.                             |  SoC   |   No   |  2:0   |   2   |  1
| _ChipUniqueSecret_        | Unique for each SoC. Permanent.                                               |  SoC   |   No   |  2:0   |   3   |  2
| _OEMSecret_               | Provisioned by OEM to differentiate their products from the competition.
Permanent, or reconfigurable via an implementation-defined HW authenticated mechanism.                      | Device |   No   |  2:0   |   4   |  3
| _ProductSecret_           | Identifies device/system type/model (product). +
Permanent or reconfigurable like OEMSecret.                                                                 | Device |   No   |  2:0   |   5   |  4
| _DeviceSecret_            | Unique device (entire system) secret.
Shared across all harts in a SoC and shareable across multiple SoCs in a single device.
The sharing mechanism is implementation-defined. +
Reconfigurable like OEMSecret.                                                                              | Device |   No   |  2:0   |   6   |  5
| _BootSecret_              | Unique per the boot session of the host hardware and its firmware stack. +
Configured by M-mode during the boot process of the host (see <<ACE-CSR-maceBootSecret>>).                  | Device |   M    |   3    |  0/1  |  6
| _VBootSecret_             | Unique per the boot session of the virtual hardware and its software stack. +
Regenerated by H or higher at each virtual boot of a VM (see <<ACE-CSR-haceVBootecret>>).                   |   VM   |  H/M   |   4    |  0/1  |  7
| _SSecret_                 | Can be configured by Supervisor (see <<ACE-CSR-saceSSecret>>).                | Process |  S    |   5    |  0/1  |  8
| _Reserved_                | Reserved for future use.                                                      | Impdef | Impdef |  7:6   | {ge} 64 | --
|===
...
. Multiply the 125-bit strings in the binary field defined by stem:[P(x)=x^{125}+x^{15}+x^{13}+x^{3}+1].
//(for degrees 124 and 126 we can use the polynomials stem:[P(x)=x^{124}+x^{7}+x^{4}+x^{3}+1] and stem:[P(x)=x^{126}+x^{8}+x^{7}+x^{2}+1]).
Note that the Locality Secrets are non-zero.
...
Only the fields _DirectionPolicy_, _SCProtection_, _Locality_, _UCPolicy_, and _FilterMode_ may be defined.
...
* If `Xs`._Locality_ is non-zero, then it can be used to restrict the Locality. Then, when the CR is exported as a CR, it will use the new tweak in the AEAD mode. Generalizing the Locality is not permitted. Attempting to do so raises ACE error _ace_err_invalid_value_.
...
* A zero bit in `Xs`._FilterMode_ means that the corresponding policy in `M`._FilterMode_ is _not_ changed. +
A bit set to 1 in `Xs`._FilterMode_ will add the corresponding policy to `M`._FilterMode_, but only if that policy was not already programmed, otherwise error ACE error _ace_err_invalid_value_ is raised.

* If `Xs`._FilterMode_ is non-zero, then
** The value `s2` is even and both `Xs2` and `X[s2+1]` are read as the 128-bit value `X[s2+1] @ Xs2`.
** `X[s2+1] @ Xs2` has the metadata filtering extension format described in <<ACE-metadata-extension>>, where only the fields corresponding to the filters actually added are considered.
//and the remaining bits are ignored.

* If `Xs`._FilterMode_ is non-zero when `M`._FilterMode_ is zero, if the `ace.restrict` operation is successful, an exported SCC will contain the additional metadata filtering extension, where previously it would not have been included. Thus, the size of the SCC may be larger than a previous version.
--
+
On RV32:
