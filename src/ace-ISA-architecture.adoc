...
[width="100%",cols="18%,50%,^9%,^9%,^7%,^7%",options="header"]
|===
| Name                      | Description                                                                               | Domain | Conf. | Value | Bits
| _NoHWBinding_             | Independent of Chip, OEM, Device                                                                |  —     | —  | 0  | 2:0
| _ChipDesignerSecret_      | Identifies the designer of the SoC. Permanent.                                                  |  SoC   | No | 1  | 2:0
| _ChipModelSecret_         | Identifies specific chip model/family. Permanent.                                               |  SoC   | No | 2  | 2:0
| _ChipUniqueSecret_        | Unique for each SoC. Permanent.                                                                 |  SoC   | No | 3  | 2:0
| _OEMSecret_               | Provisioned by OEM to differentiate their products from the competition.
                              Permanent, or reconfigurable via an implementation-defined HW authenticated mechanism.          | Device | No | 4  | 2:0
| _ProductSecret_           | Identifies device/system type/model (product). Permanent or reconfigurable like OEMSecret.      | Device | No | 5  | 2:0
| _DeviceSecret_            | Unique device (entire system) secret (_not_ the device ID such as a serial number).
Shared across all harts in a SoC and shareable across multiple SoCs in a single device.
Sharing mechanism implementation-defined, i.e., not architected. Reconfigurable like OEMSecret.                               | Device | No | 6  | 2:0
| _BootSecret_              | Unique per the boot session of the host hardware and its firmware stack.
Configured by M-mode during the boot process of the host (see <<ACE-CSR-maceBootSecret>>).                                    | Device | M  |    |  3
| _VBootSecret_             | Unique per the boot session of the virtual hardware and its software stack.
Regenerated by H or higher at each virtual boot of a VM (see <<ACE-CSR-haceVBootSecret>>).                                    | VM     | H/M | – |  4
| _SSecret_                 | Can be configured by Supervisor (see <<ACE-CSR-saceSSecret>>).                                  | Process  | S | – |  5
| _Reserved_                | Reserved for future use.                                                                        | Impdef | Impdef | {ge} 64 | -
|===
...
[WARNING]
--
Locality secrets may be combined.
The proposal is to have a 3-bit field to encode one of Localities 1–6 (0 means no binding to any of these), and 3 bits to encode respectively 7, 8, and 9.
We shall discuss two methods to combine locality secrets for the purpose of creating the tweaks to be used in the key derivation process:

. XOR them, allowing up to four to be added together.
. Multiply the non-zero 125-bit strings in the binary Galois field defined by stem:[P(x)=x^{125}+x^{15}+x^{13}+x^{3}+1].
(for degrees 124 and 126 we can use stem:[P(x)=x^{124}+x^{7}+x^{4}+x^{3}+1] and stem:[P(x)=x^{126}+x^{8}+x^{7}+x^{2}+1]).

Both methods theoretically reduce the distinguishability of a sequence or random numbers by just two bits (from 125 to 123 bits, say).
--
