...
To support context switching while protecting CCs outside architectural state, a CC may be *exported* from a CR as a *Sealed Cryptographic Context* (SCC), an encrypted and authenticated representation of a CC that can later be *imported* into a CR.
...
To prevent denial-of-service scenarios, management operations are always permitted, except when the CR is in an invalid state.
_Usage Control_ policies restrict only certain CC/CR usage operations.

Localities:::
(((Cryptographic Context, Locality)))
Finally, ACE can bind CCs to a *Localities*, i.e., restrict usage to specific devices or device classes, hardware configurations, physical or virtual boot cycles,
...
* `SmaceCSK` {implies} Defines the Context Sealing Key CSRs (`macecsk{0..7}`), which are all MRO.
* `SmaceCSKw` {implies} ACE Context Sealing Key CSRs are MRW. Depends on `SmaceCSK`.
...
ACE uses the Indirect CSR Access mechanism defined in Smcsrind/Sscsrind.
...
* Vector registers wide enough to accommodate the block-wise operations of the implemented algorithms;
...
CRs differ from conventional registers in several respects:

* Clear Content is loaded into a CR via `ace.prov` (<<ACE-instruction-init>>); there is no corresponding instruction to extract Content as cleartext.

* A CR may be *exported* in encrypted and authenticated form via `ace.export` (<<ACE-instruction-export>>), producing an SCC that may later be re-*imported* into a CR via `ace.import` (<<ACE-instruction-import>>); import fails if the SCC has been modified.

* The internal and external representations of a CR may differ in size.
...
* Unlike conventional registers, a CR may be _unconfigured_, which means that they hold no content.

* CRF capacity consumed by a CR depends on the configured algorithm; two CRs may consume different amounts. Insufficient residual CRF capacity may prevent execution of a configuration instruction (`ace.prov`, `ace.import`, `ace.clone` (<<ACE-instruction-clone>>), or `ace.derive` (<<ACE-instruction-derive>>)); software must then free capacity by unconfiguring other CRs.

* CRs may be addressed directly, as a 5-bit immediate, or indirectly, via a GPR value.

* The principal instructions that use and may modify CRs are:
  ** `ace.exec`, which performs cryptographic operations on data blocks according to the algorithm configured in a CR; and
  ** `ace.state`, which transitions between states of a mode of operation or hash function, optionally performing state initialization.

Accessing a CR with an index outside [0..31] raises an illegal instruction exception.

[NOTE]
====
Management operations comprise the configuration instructions (`ace.prov`, `ace.import`, `ace.clone`, and `ace.derive`) and the export operation (`ace.export`).
====
...
. Adding CR renaming would significantly increase microarchitectural complexity. However, CRs are redefined far less frequently than integer or floating-point registers (internal implied changes aside), so the absence of renaming is not expected to cause performance issues.
...
|  [11:1] |  11   | _Algorithm_        | Cryptographic algorithm (primitive, mode, or protocol).
| [13:12] |   2   | _DirectionPolicy_  | Permitted operation direction(s). See <<ACE-Algorithm-field>>.
...
| [31:29] |   3   | _UCPolicy_         | Usage Control Policy. Bits 29, 30, and 31, when set, disallow CC usage in U-mode, (V)S-mode, and H-mode, respectively.
| [45:32] |  14   | _ExtraContent_     | If non-zero, indicates an additional data segment appended to Content. Length is _ExtraContent_ × 8 bytes (up to 128 KiB).
...
For encryption/decryption primitives and modes, the _DirectionPolicy_ field indicates whether the CC may be used for encryption (lower bit set), decryption (upper bit set), or both.
...
For other primitives, the _DirectionPolicy_ field may encode additional algorithm identifiers to expand the effective encoding space of the _Algorithm_ field.
...
The architected  _StateNumber_ values which are common to all algorithms are defined in <<ACE-state-constants-fixed>>.
...
|  14   | ace_state_invalid       | _invalid_    | Algorithm terminated abnormally, or provisioning/import was interrupted.
|  15   | ace_state_completed     | _completed_  | Algorithm has completed (not necessarily without error).
|===

All states other than _invalid_ are termed _valid_. Unless an algorithm's specification states otherwise, a transition from any valid state to _initial_ is always permitted.

If an algorithm has 16 or fewer states (including the architected ones), the current state may be stored in the _StateNumber_ field. For algorithms with more than 16 states, an algorithm-specific representation is stored in Content; in this case, _StateNumber_ shall be set to _initial_ (0), _invalid_ (14), or _completed_ (15) when the algorithm is in the corresponding state, and to a non-reserved value (e.g., 1) otherwise.
...
This state change to _invalid_ does not apply during `ace.export`.
...
ACE maintains a table of 128-bit Locality Secrets (LS), the Locality Secrets Table (LST). Some entries are fixed or configurable only via an implementation-specific authenticated hardware procedure; these are global and shared across all ACE units in the same SoC. Other entries are per-hart architectural state programmed by privileged modes. Eleven LST entries are currently architected: the first seven constitute the _HW Binding Group_, binding SCCs to the hardware; entries 9–11 form the _SW Filter Group_.

<<ACE-locality-indexes>> lists the architected Localities. For each Locality we provide: index in the LST; domain (SoC-wide, device-wide, VM-wide, or per-process); configurability; bit-field encoding and value. Seven of the eight _Locality_ field bits are defined; bit 7 is reserved for future use.
...
|   3   | SubsystemSecret         | Unique subsystem secret.
Used to separate different subsystems within a SoC.
Permanent.
May not be the same (or synchronized) across related subsystems in a multi-SoC system.                               | Device |   HW   | [2:0]  |   4
|   4   | OEMSecret               | Provisioned by OEM to differentiate their products from the competition.
Permanent, or reconfigurable via a custom hardware authenticated mechanism.                                          | Device |   HW   | [2:0]  |   5
|   5   | ProductSecret           | Identifies device/system type/model (product).
Permanent or reconfigurable like OEMSecret.                                                                          | Device |   HW   | [2:0]  |   6
|   6   | DeviceSecret            | Unique device (entire system) secret.
...
Reconfigurable like OEMSecret.                                                                                       | Device |   HW   | [2:0]  |   7
|   7   | PhysBootSecret          | Unique per the boot session of the host hardware and its firmware stack.
Configured by M-mode during the boot process of the host (see <<ACE-CSR-macePhysBootSecret>>).                       | Device |   M    |   3    |  0/1
|   8   | VBootSecret             | Unique per the boot session of the virtual hardware and its software stack.
...
|   9   | MLocality               | Configurable by M to mutually isolate processes in lower-privileged modes.       |  S/HS  |   M    | [6:5]  |   1
|  10   | HLocality               | Configurable by H to mutually isolate processes in lower-privileged modes.       |  VS    |   H    | [6:5]  |   2
|  11   | SLocality               | Configurable by (V)S to mutually isolate processes in U-mode.                    |  U     |  (V)S  | [6:5]  |   3
|===

Implementations are not required to populate every HW Binding Group entry.
Only DeviceSecret and DeviceSecret are mandated.
An unconfigured entry is transparently replaced by the next more restrictive Locality which is defined, as in:
ChipDesignerSecret {rightarrow} ChipModelSecret {rightarrow} ChipSecret, and OEMSecret {rightarrow} ProductSecret {rightarrow} DeviceSecret.
...
Multiple Localities may be active concurrently, subject to architectural restrictions.
Currently, up to three Localities can be active at once: one from the HW Binding Group, one of either PhysBootSecret or VBootSecret, and one from the SW Filter Group. During SCC export and import, the corresponding Locality Secrets are prepended, in ascending index order, to the input of the AEAD tag computation. If the _Locality_ field is zero, no Locality Secrets are prepended.
...
The first eight bytes of the additional data segment hold a copy of the `acemarchid` CSR (<<ACE-CSR-acemarchid>>) of the originating machine, and are counted within the length recorded by the _ExtraContent_ field. The importing implementation uses this value to determine whether the additional data can be used.

NOTE: Typical extra content includes precomputed point multiples (whose computation is generally far more expensive than their encryption) and auxiliary data for side-channel-protected implementations, such as key shares or data to accelerate share reconstruction.
...
ACE extends `*status` and `misa`, and adds up to 13 new direct CSRs and up to 28 indirect CSRs.
...
| 0xXXX       | URW        | `acehtrapmask`              | (TBD) Mask that determines which ACE exceptions horizontally trap
| 0xXXX       | URW        | `acehtrapvector`            | (TBD) Address of the horizontal ACE trap handler
...
// [WARNING]
// ====
// * The last CSR groups use the Indirect CSR Access mechanism defined in cite:[RISCV-ISA-Priv]. Direct addresses may be allocated instead if desired.
// ====

//

==== Extensions to Existing CSRs
...
The 4-bit `aceerror` CSR holds the error code from the most recent ACE instruction.
A read of this CSR may stall until the preceding ACE operation completes.
The CSR is writable to support context switching.

The phrase “_raises ACE error X_” means that `aceerror` is set to value _X_. The same event may also raise an exception, triggering a vertical or horizontal trap; the handler may inspect `aceerror` to determine the error type.
...
* Invalid Metadata in `ace.prov` or `ace.import` (other than `ace_err_unimplemented`), such as:
 ** Nonzero bits in reserved fields,
 ** Unsupported _Locality_ value,
 ** Nonzero _StateNumber_ in a PI.
* An ACE operation issued with an invalid or zero ACEIOBUF transfer length (`aceiobuftop` − `acestart`); see <<ACE-CSR-aceiobuftop>> and <<ACE-CSR-acestart>>.
...
`acestart` is an XLEN-bit RW CSR that tracks the progress of long-running, interruptible ACE instructions: `ace.prov`, `ace.import`, `ace.export`, `ace.exec` (when iterating over multiple blocks), `ace.input`, and `ace.output`.
(((CSR, acestart)))

Only the instructions listed above honor a nonzero `acestart`; all other ACE instructions ignore it and are restarted.

Unlike `vstart`, which tracks element indices, `acestart` records a byte offset, reflecting the number of bytes processed so far. Hardware typically writes this register on interruption. Software may write zero to force a full restart and may save and restore the register across context switches. Writing arbitrary nonzero values is not supported and may raise an exception.

Provisioning, import, and export operations may be interrupted and resumed provided no context switch occurs; if a context switch occurs, they must be restarted.

[WARNING]
`acestart` may share the encoding of `vstart`, regardless of whether the V extension is present. When a vector instruction is interrupted, the CSR follows `vstart` semantics (count of processed elements); when an ACE instruction is interrupted, it follows `acestart` semantics (amount of processed bytes).
...
`acestart` also applies when `ace.exec` processes a multi-block vector operand (`VL*SEW` is a multiple of the algorithm's basic processing unit `b`): `ace.exec` operates iteratively and may be interrupted between blocks. Whether inputs and outputs use vector registers or ACEIOBUF, `acestart` always reflects the number of bytes processed.

Full `acestart` semantics are described in <<ACE-provisioning-export-import>>.

//The following two CSRs may be used to optimize ACEIOBUF save and restore.
...
When `aceiobuflen` is zero, the ACEIOBUF is _unconfigured_ (analogous to an unconfigured CR in state Initial); `ace.input` and `ace.output` raise `ace_err_unconfigured`.
...
Using ACEIOBUF when `aceiobuftop` − `acestart` is zero (with `aceiobuflen` nonzero) or yields an invalid transfer size raises `ace_err_invalid_value`.
...
`aceactivecr` is an XLEN-bit URW CSR; only the low 5 bits are defined. It binds `ace.input`, `ace.output`, `ace.exec`, `ace.state`, and `ace.derive` to a specific CR. Valid values are 0–31 (selecting a CR) or 32 (disabled).

`aceactivecr` may only be modified when `acesequence` is zero; otherwise, an illegal instruction exception is raised.

When `aceactivecr` is not 32, the CR operand in all single-CR instructions and the source CR in `ace.derive` and `ace.clone` must equal the selected value; any deviation raises an illegal instruction exception, ensuring correct instruction sequencing.
...
Bit __i__ of this CSR is set if ACE error __i__+1 causes a horizontal trap.
...
* _Usage-controlled_: whether the instruction is subject to usage control policies (_UCPolicy_);
...
. The code is written without considering the exceptions to be caught, usually only ace_err_invalid or ace_err_expired.
...
If a Form of `ace.exec`, `ace.state` or `ace.derive` that is not explicitly allowed in the current algorithm state is used, then an illegal instruction exception is raised.
...
// Forms A, B, and C of `ace.exec`, as well as Form C of both `ace.derive` and `ace.state`, can be replaced by using `ace.input` and `ace.output` instructions together with a Form D `ace.exec`, a Form A `ace.state`, or a Form A `ace.derive` instruction as follows:
...
Consequently, an implementation of GCM based on an ECB mode CC is possible, and we include it as an example below.
...
|   xv  | (variable)                                | Additional data from the *Internal State* (Content)..
|===
...
If _ExtraContent_ > `varlen`, Position xv contains the microarchitectural dependent part.
...
* If there is a `acemarchid` mismatch, both `RandomNumber` (Position xiv) and the additional data at Position xv are discarded, and the operation is restarted.
--
+
 ...
 ** Upon returning to State _initial_, if a Form B `ace.state` is used, the parameter `Xs` will determine whether the fields
 `Generator`, `Scalar`, and `PublicKey` are erased or kept for further operations. This is determined by Bits 0, 1, and 2 of `Xs` as follows: If Bit 0, 1, resp., 2 is set, then `Generator`, `Scalar`, resp., `PublicKey` is erased, and the corresponding Boolean flags `HasGenerator`, `HasScalar`, resp., `HasPublicKey` is unset as well.
...
- One option is to add an optional Metadata Extension block that contains values of `scontext` and `mcontext` (from Sdtrig) to match against, or alternative values based on other CSRs, perhaps defined ad hoc.
+
...
All these approaches are allowed by the architecture,
and the behavior of certain ACE instructions when interrupted at admissible points is fully architected.
