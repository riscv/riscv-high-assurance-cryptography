...
// include::ace-instructions-summary.adoc[]
...
In the column "MM-Configurable" we indicate whether the architecture may allow MM to configure the corresponding Locale Secret.
...
| Boot_Session              | Regenerated at each (virtual) boot. | Allowed | 6
| Reserved                  | Reserved for future use (RZ if not defined). | Impdef | 7-14
|===
...
NOTE: MM has the responsibility to "virtualize" the Boot_Session Locale Secret.
...
if (metadata.Locale == 15) then {
    ...
    CTK = DCTK(K,LST[metadata.Locale]);
}
...
// allow the unit to perform operations, access the general purpose and vector registers, as well as the Cryptographic Contexts.
