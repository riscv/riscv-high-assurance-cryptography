...
* `b` = block size (8|`b`). `VLEN` must be a multiple of `b`.
* `k` = key size (8|`k`).
...
Parameters:::
* `b` = block size (8|`b`). For this algorithm `VLEN` must be a multiple of `b`.
* `k` = key size (8|`k`).
* `j` = counter size (8|`j`).
* `n` = IV size (8|`n`).
...
* `b` = block size (8|`b`). For this algorithm we require `VLEN` to be an integer multiple of `b`.
* `k` = key size (8|`k`).
...
The state machine of this algorithm closely follows RFC 7253 except for the bit ordering and the use of zero-based indexing in place of one-based indexing as in the RFC. The algorithm and is of course identical to the RFC's for 128-bit-wide block ciphers, with the generalisation to other widths being straightforward and according the security proof of the original algorithm
and a lower bound on nonce length following https://eprint.iacr.org/2023/326.
...
+
--
* `b` = block size. `b` is a power of two, at least 64. For this algorithm we require `VLEN` to be an integer multiple of `b`.
* `k` = key size, `b`|`k`.
--
+
 ...
 ** _decrypt_ -> _last_block_dec_ -> _hash_verify_, later automatically transitioning to _completed_.
 ...
 ** To move to _set_public_value_, the `ace.state` operation must use the additional argument, which sets the nonce length `Nlen`.
 `Nlen` must be at least `g-1`. If not, an invalid instruction error is raised, and the CR is *invalidated*.
...
[]
 ...
 . `L <- double(L)`
 . `L <- double(L)`
 . `Nextended <- N[Nlen-1:0] @ 1'b1 @ zeros(b-g-1-Nlen) @ numtobits(taglen,g)`
 . `top <- str2num(Nextended[b-1:b-g+1])`
 . `Kbot <- enc_blk(K, zeros(g-1) @ Nextended[b-g:0])`
 ...
 ** `used_VLEN` -- Current vector length (up to MAXVLEN).
...
[lowerroman]
 ...
 . `hash_state`: `n` bits.
 . padding: 16 bits.
 . `used_VLEN`: 16 bits -- Current vector length (up to MAXVLEN).
