...
[.darkred]#Here we describe how a given bit in the CSRs
`aceccconfigured`, `aceccurrent`, `aceccdirty`, and `acecclazy` would be updated.#

First, the operating system must keep not only state for each process, but also keep track for each CC to which process it belongs.
If the CC belongs to a different process than the one that is currently executing, the bit in `acecclazy` must be set to `true`.

Concepts

* Current process, i.e., the one issuing the instruction triggering the behavior we describe), current CC (the one being enabled), and current state (the state of the current CC).
* Owning process, i.e., the process that owns the current configuration of the CC.

<<<

[[ACE-tracking-cc-dirtiness]]
===== Fine Grained CC Management: `aceccactive`, `aceccdirty`, and `acecclazy`

States we need:

*VOID* (`0b000`)::
[.darkred]#_CC is unconfigured, and there is no saved value for the current process._# +
`[darkred]#aceccconfigured[i] = 0, acecccurrent[i] = 0, acecclazy[i] = 0.#`
* In this case it must be `aceccdirty[i] = 0`. +
Using the CC gives an error, no trap. +
* Configuring the CC works as expected, sets: +
`aceccconfigured[i] = 1, acecccurrent[i] = 1, acecclazy[i] = 0,` and `aceccdirty[i] = 1`.

*LAZY* (`0b001`)::
[.darkred]#_CC is unconfigured, and parent has saved a value to be lazy-loaded for the current process._# +
`[darkred]#aceccconfigured[i] = 0, acecccurrent[i] = 0, acecclazy[i] = 1.#`
* In this case it must be `aceccdirty[i] = 0`. +
* Accessing the CC traps to allow the OS to lazy load it for the current process. +
* Can be used freely. If potentially modified, `aceccdirty[i]` becomes `1`. +
...
`[darkred]#aceccconfigured[i] = 1, acecccurrent[i] = 1, acecclazy[i] = 0, aceccdirty[i] = 0.#` +
...
`[darkred]#aceccconfigured[i] = 1, acecccurrent[i] = 1, acecclazy[i] = 0, aceccdirty[i] = 1.#` +
* If the process is interrupted now, the OS must save it and overwrite the previous value. +
...
`[darkred]#aceccconfigured[i] = 1, acecccurrent[i] = 0, acecclazy[i] = 0.#` +
* In this case _using_ it will given an error, whereas _configuring_ it will first trap to the operating system, to allow the latter to save the CC to the state of the owning process, clear it, update the flags to allow the current process to configure it, and return to the process to restart the operation allowing the configuration.
...
`[darkred]#aceccconfigured[i] = 1, acecccurrent[i] = 0, acecclazy[i] = 1.#` +
...
//+
//For single bits in `aceccactive`, `aceccdirty`, and `acecclazy`, the transitions should be similar, with some specific meanings
//+
//`aceactive[i] = 0` means that the CC does not contain a value for the current process, even though it might
//and will be lazily loaded when needed.
//The value of `aceccdirty[i]` in this case must be set to `0`.
//+
//`acecclazy[i] = 1` and `aceactive[i] = 1` means that the CC does contain a value, which belongs to a different process, and will be first saved in the right process' context and then, lazily loaded from the saved state of the current one when needed.
//The value of `aceccdirty[i]` in this case must be set to `0`.
//+
//`acecclazy[i] = 1` and `aceactive[i] = 1` means that the CC does contain a value, which belongs to a different process, and will be first saved in the right process' context and then, lazily loaded from the saved state of the current one when needed.
//The value of `aceccdirty[i]` in this case must be set to `0`.#

NOTE: `acecc*` can track the CC states for all higher-privileged modes.
