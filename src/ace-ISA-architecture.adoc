...
| BootSecret                  | Identifies Operating System (or System Stack).
                              Not permanent, must be configured at (virtual) boot.                                                      | OS | M | 6
| VBootSecret                | Regenerated at each (virtual) boot.                                                                       | OS | M | 7
...
====
(((CSR, macecrstatus)))
A separate `macecrstatus` can be defined, _mutatis mutandis_, like `sacecrstatus`/`vsacecrstatus`, to allow M-mode to optimize context switches of Supervisor domains.
