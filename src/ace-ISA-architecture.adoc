...
We define the Key Derivation and the DCSK Derivation functions
...
The instructions permitted in each algorithm state are typically `ace.exec` and `ace.state`, though this is not a strict requirement.
...
NOTE: The `ace.state` instruction used to transition to states _hash_absorb_, _encrypt_, or _decrypt_ normally sets the first (or only) public value.
...
_The `ace.input` and `ace.output` instructions may be used to replace the inputs and outputs to Forms A-C of `ace.exec`, the auxiliary inputs of Form C `ace.state` or of Form C of `ace.derive` in any algorithm, to the point that an implementation of ACE without the V extension or a subset thereof is possible._
...
* _In all algorithms, whenever one of the instructions in <<ACE-replacement-instructions>> occurs, it may be replaced by a replacement sequence. We do not write this explicitly in the algorithm descriptions for brevity._

NOTE: This mechanism is one of the reasons, besides keeping the state machines simple, that in a given state of an algorithm, in each state only one Form of `ace.exec` or `ace.state` is allowed.
...
foreach(i from 0 to ceil(len_in_bytes(PT)/16)-1) {
   ...
   V3 ← V3 xor V1                                // Horner scheme addition
   V3 ← Galoismul(V4, V3)                        // Horner multiply multiplication
   CT[16*i .. 16*i+15] ← V1
   counter ← counter + 1
}
...
** Upon issuing a Form C ```ace.state Kn|K[Xn], #ace_state_hash_absorb, INPUT```:
  ...
  *** NB: This version of the instruction is to be used during decryption.
      It can be invoked also during encryption, however, the `SIV` field will be overwritten anyway with the synthetic value.

** Upon issuing a Form A ```ace.state Kn|K[Xn], #ace_state_hash_absorb```:
  *** The `SIV` is not modified (it is assumed to be already zero).
  *** NB: This version of the instruction is to be used during encryption.
 ...
 ** In state _hash_absorb_, we can only issue ``ace.exec`` instructions of Form B, i.e., +
 “```ace.exec Kn|K[Xn], INPUT```,” to absorb the data block `INPUT` into the hash. This is performed as follows:
+
--
...
In this mode it is the responsibility of the user to apply proper padding to the AD and plaintext, and to remove the padding from the decryption of the ciphertext to recover the original plaintext.
...
The differences between the operation `ace.exec` computes in this state and the operation is computes in state _encrypt_ are the absence of the final `ASCON(8)` and the automatic transition to state _hash_output_.
