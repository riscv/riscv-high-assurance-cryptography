...
* `Zleaes128` => Support for AES-128 encryption and decryption (all modes)
* `Zleaes192` => Support for AES-128 encryption and decryption (all modes)
* `Zleaes256` => Support for AES-256 encryption and decryption (all modes)
* `Zlesha2` => Support for SHA-2 (all modes)
...
* `Zlace` => Must support `Zll128b`; must support `Zlr8`; must support for the following CSRs: `aceerror` and `acestart` (this is a dependency for Zicsr); must support the following instructions: `ace.init`, `ace.exec`, `ace.clear`, `ace.export`, `ace.import`, and `ace.size`; `ace.exec` must support `Zleaes128`.
