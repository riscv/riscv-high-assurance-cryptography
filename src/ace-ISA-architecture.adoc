...
The following table defines the state numbers used by the algorithms in this specification. The state name is the mnemonic for the corresponding state number, without the “ace_state_” prefix.

[[ACE-state-constants]]
.Constants defining CC states
[float="center",align="center",width="100%",cols="^6%,<26%,<17%,<51%",options="header"]
|===
...
|   4   | ace_state_hash_verify         | _hash_verify_       | Compare the internal state of a hash/tag function to the provided value.
...
|   8   | ace_state_enc_tag_finalize    | _enc_tag_finalize_  | Finalize the internal state of a tag computation function for the encryption algorithm of an authenticated encryption mode of operation.
|   9   | ace_state_dec_tag_finalize    | _dec_tag_finalize_  | Finalize the internal state of a tag computation function for the decryption algorithm of an authenticated decryption mode of operation.
...
|  14   | ace_state_invalidated         | _invalidated_       | The algorithm has abnormally terminated. `ace.error` will always return error code _ace_err_invalid_.
|  15   | ace_state_completed           | _completed_         | The algorithm has correctly completed.
|===
...
For algorithms requiring multiple public values or additional parameters (e.g., tag length), these can be set by transitioning to state _set_aux_value_ using a form of `ace.state` that accepts an additional argument.
...
Except if explicitly stated otherwise in an algorithm's specification, it is always possible to transition from any state to state _initial_.
 ...
 ** To transition to state _operate_, an ``ace.state`` instruction with a vector additional parameter must be issued, or its replacement sequence.
 ...
 ** The ``ace.state`` instruction to transition to either _encrypt_ or _decrypt_ must be of the form with a vector additional input, or its replacement sequence. +
...
// N.B. another ace.state K0, #const_set_aux_value would reset the internal state and allow to begin another decryption with a new IV.
 ...
 * *Behavior:*
  ...
  ** Upon issuing ```ace.state Kn|K[Xn], #ace_state_set_aux_value, INPUT``` or the corresponding replacement sequence, following is performed:
   . `tweak` <- `INPUT`, and
 ...
 ** To transition to state _hash_absorb_:
 the ``ace.state`` instruction must be used with a vector input that contains the IV, i.e., the fixed part of the counter.
...
** To transition to state _enc_tag_finalize_, the ``ace.state`` instruction must be used with a vector input that contains
the value +
 +
 `{nbsp}{nbsp}{nbsp}INPUT <- bin(bit_length(AD), b/2) @ bin(bit_length(plaintext), b/2)` +
 +
This must be set by the software.
...
** In state _enc_tag_finalize_, only ``ace.exec`` instructions of Form C or their replacement sequences are allowed.
must be set by the software, then,
  “```ace.exec OUTPUT, Kn|K[Xn]```”
  writes the `tag` to `OUTPUT` and the state transitions to _completed_.
...
** To transition to state _dec_tag_finalize_, the ``ace.state`` instruction must be used with a vector input that contains
the value +
 +
 `{nbsp}{nbsp}{nbsp}INPUT <- bin(bit_length(AD), b/2) @ bin(bit_length(plaintext), b/2)` +
 +
...
** To transition to state _hash_verify_, the ``ace.state`` instruction must be used with a vector input that contains
...
For instance, for GCM-SIV (cf.{nbsp}<<ACE-GCM-SIV-mode>>), `nonce` would be included in the Initialization Input exactly as `IV` is included in the Initialization Input for GCM, and the transition to _hash_absorb_ would also use an ``ace.state`` without additional inputs.
...
** Upon issuing ```ace.state Kn|K[Xn], #ace_state_set_aux_value, INPUT``` with a vector additional parameter, or its replacement sequence:
  *** The values of `nonce` is set to `INPUT`.

** Upon issuing ```ace.state Kn|K[Xn], #ace_state_set_aux_value``` with a GPR additional parameter or none:
...
** To move to _set_aux_value_, the `ace.state` operation must use the additional argument in a GPR, which sets the nonce length `N_len`.
...
** In state _set_aux_value_, only  ``ace.exec`` instructions of Form B, “```ace.exec Kn|K[Xn], INPUT```”, or their replacement sequences, are allowed, whereby `N` is set to `INPUT`, namely
...
** To move to _hash_absorb_, the `ace.state` operation must have the additional argument in a GPR `Xs`, which sets the tag length `tag_len`.
  *** If this argument (interpreted as an unsigned value) is larger than `b` or `0` (the latter occurs also if `X0` is passed as the additional argument), an invalid instruction error is raised, and the CR is *invalidated*.
...
** To complete the algorithm in the decryption branch, either an ``ace.state`` operation with a vector additional input must be used or its replacement sequence must be used to transition to state _hash_verify_.
 ...
 ** In state _initial_ :
  ...
  ** To transition to state _hash_absorb_, the ``ace.state`` instruction must have a vector additional argument, or be the corresponding replacement sequence.
     This performs:
    . `state[3] <- INPUT[63:0]`
