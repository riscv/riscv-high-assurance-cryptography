...
[WARNING]
====
* To configure the `macercsk`, `maceBootSecret`, `haceVBootSecret`, and `saceSSecret` CSR groups
...
|  6   | _ace_err_unimplemented_      | Not implemented algorithm or algorithm variant.
|  7   | _ace_err_invalid_value_      | Invalid metadata section in `ace.init` or `ace.import`, other than ace_err_unimplemented, for instance:
non-zeros in zero fields, invalid Filter_Mode for a 64-bit metadata block, non-implemented Locality value, State_Number is non-zero for `ace.init`. +
...
|  9   | _ace_err_state_error_        | Not allowed state transition in `ace.state`, including requesting encryption or decryption from a CC not allowing it.
| 10   | _ace_err_auth_               | Authentication failure in an algorithm.
|===
...
[WARNING]
ACE error code __i__ corresponds to bit \#__i__ in a mask CSR (or more than one) that determines whether the error traps instead of being handled directly by the software by checking `ace.error`.
This may be also a horizontal trap. Details [.red]#TBD#.
...
|===
| Value | Mnemonic for the Constant       | State Name          | Description
.2+| 1  | _ace_state_hash_absorb_         | _hash_absorb_       | Start and initialize a hash function of the tag/MAC part of an algorithm and start absorbing blocks of data (in the case of an AEAD algorithm, this usually starts absorbing the associated data).
