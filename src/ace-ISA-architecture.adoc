...
|   3   | Dirty              | ACE enabled. Some CRs or the ACEIOBUF may have been modified since the last context swap. This includes unconfigured states since switching code needs to act on them, by erasing any stale saved state.
|===
...
We say that some event _raises ACE error_ _some_number_ when it causes value _some_number_ to be written to `aceerror`.
...
|  1   | _ace_err_import_             | Authentication failure with `ace.import`.
...
ACE operation issued with invalid ACEIOBUF length `aceiobuftop`-`acestart` (see <<ACE-CSR-aceiobuftop>> and (<<ACE-CSR-acestart>>). +
Invalid input to `ace.restrict`.
|  8   | _ace_err_privilege_          | Violation of usage control policies in metadata (including Filters).
|  9   | _ace_err_state_error_        | Not allowed State_number in `ace.state`. +
Encryption, resp., decryption is requested from a CC not allowing it;
| 10   | _ace_err_not_permitted_      | A Form of `ace.exec`, `ace.state` or `ace.derive` that is not permitted in the current algorithm state is used;
| 11   | _ace_err_auth_               | Authentication failure in an algorithm.
|===

Using an incomplete CC in `ace.size` is allowed.

[WARNING]
Bit __i__ correspond also to bit __i__-1 in mask registers that determine whether the error traps instead of being handled directly by the software by checking `ace.error`.
...
Setting a larger value raises an illegal-instruction exception.

`aceiobuftop` must be {ge} `acestart`.
Setting a smaller value raises an illegal-instruction exception.
...
* Attempts to execute any ACE instruction that accesses CR #__i__ raise an illegal-instruction exception when ``vsacecrstatus[``2__i__+1,2__i__``]`` is set to Off.
...
The values written to the `haceVBootSecret` group must follow the update rules defined in <<ACE-CSR-group-update-policy>>.

[NOTE]
====
`haceVBootSecret` must be reconfigurable in order to support VM reboots which are independent of the boot cycles if the physical host.
====
...
In the first phase, the metadata block is read, which determines whether the encoded algorithm and its side-channel protection are implemented
zzzzzz
the length of the remaining information.
...
If at least 4 bytes have been read, the value of `metadata.State_Number` is known.
...
if (tag != SIV) then { aceerror ← ace_err_import; return }
...
** Upon issuing a Form A or B ```ace.state Kn|K[Xn], #ace_state_set_aux_value```:
  *** An ACE error _ace_err_not_permitted_ is raised and the CR is *invalidated*.
...
** To move to _set_aux_value_, a Form A `ace.state` must be used, where the auxiliary argument `Xs` sets the nonce length `N_len`.
 ...
 If a different form of `ace.state` is used, then ACE error _ace_err_not_permitted_ is raised, and the CR is *invalidated*.
...
** To move to _hash_absorb_, a Form A `ace.state` must be used, where the auxiliary argument `Xs` sets the tag length `tag_len`.
  ...
  *** If different Form of `ace.state` is used, then ACE error _ace_err_not_permitted_ is raised, and the CR is *invalidated*.
