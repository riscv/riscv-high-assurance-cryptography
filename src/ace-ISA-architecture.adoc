...
. The _Sealed Cryptographic Context_ (SCC) (<<ACE-format-SCC>>), an encrypted, authenticated format used when exporting a CR to memory for later re-import.
...
{empty}
(((Cryptographic Context,metadata)))
CCs contain 64 or 192 bits of metadata.
(((Cryptographic Context,metadata,header)))
The first 64 bits are the metadata header,
defined in <<ACE-metadata-header>>,
which serves as the header of the data formats used in CR provisioning and in SCC export and import.
...
| [47:36] |  12   | _ExtraContent_           | If non-zero, there is an additional segment of data at the end of the _content_.
The length of additional segment is _ExtraContent_ 64-bit blocks.
| [63:48] |  16   | Reserved                 | Value is 0 for the current metadata formats.
|===

If _FilterMode_ is not zero, then the provisioning and SCC data formats contain an
additional 128-bit block of metadata, called the _metadata extension_, defined in <<ACE-metadata-extension>>.
...
[[ACE-metadata-confidentiality]]
===== Confidentiality

The metadata header block is not confidential and will not be encrypted upon export as an SCC.
The optional metadata extension is confidential and will be encrypted upon export as an SCC.
...
System Keys are given by an index into an internal, system-specific System Keys Table (SKT).
For each algorithm as specified in <<ACE-algorithms>>, the part of the content that normally contains the key(s) is replaced by its index into the SKT-for both PIs and SCCs
The actual value of the key is never included in exported content.
For threshold implementations, ACE may copy the key material from the SKT into the CR and expand it into multiple shares.

System Generated Keys are also system-specific. In place of the PI, the provisioning operation reads a system-specific “recipe”, which is not portable.
After internal key generation, the actual key value—not the recipe—is stored in the CR and SCC. The KeyType field in the metadata is 0.
...
An implementation may not implement all the fields in the HW Binding group. In this case, an undefined value is transparently replaced with the value associated to the immediately more _general_ Locality, and so on, as follows:

* _ChipSecret_ -> _ChipModelSecret_ -> _ChipDesignerSecret_ -> No HW Binding,
* _DeviceSecret_ -> _ProductSecret_ -> _OEMSecret_ -> No HW Binding,
...
The first eight bytes of the additional information store a copy of the `marchid` value from the machine that created the data.
These eight bytes are counted within the 64‑bit blocks recorded by the _ExtraContent_ field.
This value is matched against the `marchid` of the device performing the import, to determine whether the additional data is usable.
This depends also on the algorithm.
...
However, several algorithms need to compute and cache additional data once provisioned, and their internal state will be larger than the size of the PI.
...
`bytes{nbsp}[{nbsp}24 .. u+23{nbsp}]{nbsp}{nbsp}` Encrypted {optional metadata extension and} serialized content of the CR.

Both metadata and the serialization of the content are authenticated, but the optional metadata extension and the content are encrypted.
...
|  0   | _ace_err_no_error_           | No error.
|  1   | _ace_err_import_auth_        | Authentication failure with `ace.import`.
|  2   | _ace_err_unconfigured_       | Use of an unconfigured, or partially configured CR as a source in any instruction except `ace.size`. +
Use of ACEIOBUF as an input or an output when it was unconfigured. +
Use of an out of range CR with indirect addressing.
|  3   | _ace_err_invalid_CR_         | Attempted to use an _invalid_, but not expired, CR.
|  4   | _ace_err_expired_CR_         | Attempted to use an expired CR.
 ...
 ** Invalid FilterMode for a 64-bit metadata header block,
...
NOTE: CR status propagates from the least privileged modes to the higher privileged ones, like the FS, VS, and ACES fields in `*status`, but there are only two states: Clean and Dirty.
...
* If `Xs`._FilterMode_ is non-zero when `M`._FilterMode_ is zero, if the `ace.restrict` operation is successful, an exported SCC will contain the additional metadata extension, where previously it would not have been included. Thus, the size of the SCC may be larger than a previous version.
--
+
On RV32:
...
. `MH_size(x)` returns the byte length of the metadata header. Currently, always 8.
...
. `SCC_content_size(x)` returns the size in bytes of the fixed part of the content section of an SCC. It depends only on the 13 bits `DirectionPolicy @ KeyType @ Algorithm`.

Let `ME` be the metadata extension size in bytes.
The total size in bytes of a PI is therefore

`{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}MH_size(x)` + (_FilterMode_ != 0) * `ME` + `PI_content_size(x)`,

and the total size in bytes of an SCC is

`{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}MH_size(x)` + 32 + (_FilterMode_ != 0) * `ME` + `SCC_content_size(x)` + _ExtraContent_ * 8.
...
The AEAD algorithm used to export and import CRs is an adaptation of AES-GCM-SIV cite:[DBLP-journals-iacr-GueronLL17,RFC8452].

[WARNING]
AES-GCM-SIV has been chosen because it only required single passes of data in main memory, reducing the number of external memory accesses. GCM uses also a single pass but requires separate nonce and tag values, increasing the size of the exported data by one block.
...
function KeyDeriv(key : bits(256), L : bits(125)) = {
  struct {
    AESE256(key, (L @ 0b101))[63:0] @ AESE256(key, (L @ 0b100))[63:0] @
    AESE256(key, (L @ 0b011))[63:0] @ AESE256(key, (L @ 0b010))[63:0],
    AESE256(key, (L @ 0b001))[63:0] @ AESE256(key, (L @ 0b000))[63:0]
  }
}
----
...
function KeyDeriv(key : bits(256), L : bits(126)) = {
  ...
  struct {
    AESE256(key, (L @ 0b01) xor t) @ AESE256(key, (L @ 0b10) xor t),
    AESE256(key, (L @ 0b11) xor t)
  }
}
----
...
z : int ← SCC_content_size(metadata[31:0]);
z ← z + metadata.ExtraContent * 8
if (metadata.FilterMode != 0) then
  z ← z + sizeof(metadata extension)

content : array[z] of bytes,         // {optional metadata extension) and content
M : array[24 + z] of bytes;          // this is the memory
...
* The first block read from memory by the operation is the metadata header block:
 ...
 ** If 4 bytes or more have been read, the value of `metadata.StateNumber` in the SCC is known. In this case, the instruction may be restarted with `acestart` {ge} 4 recording the amount of data read.
...
** The implementation must keep be a 4-bit internal temporary state containing the original value of `metadata.StateNumber` and a flag indicating whether the value has been saved.
** The flag is only set upon interruption of the import operation, and is otherwise always clear.
 *** The implementation will resume the import operation only if the flag is set, otherwise it will restart the operation.
 *** The flag is also used by the implementation to
 raise error _ace_err_unconfigured_ instead of _ace_err_invalid_CR_
 upon an attempt to use the partially imported CR.
...
. *Provisioning Input*:
The data following the metadata header block in the PI to `ace.prov`, such as keys and other required secrets.
...
. *Serialized Content*:
The data that follows the metadata header block in the plaintext portion of an SCC.
...
the value against which the internally computed tag must be compared:
   If the values match, then `aceerror` is set to _ace_err_no_error_,
...
* `RFC8452_KeyDeriv(key)` = `KeyDeriv(key,zeros(125))`, i.e., per RFC8452 (<<ACE-SCC-RFC8452-derivation>>).
...
--
  ...
  . else (tag length is `3b/4` bits)
    ...
    If `tag` and `SIV` match, `aceerror` is set to _ace_err_no_error_, else ACE error _ace_err_auth_ is raised.
  . The state transitions to _completed_.
--
+
If `IOLEN` > `b`, only the `b` least significant bits of `INPUT` are considered.
...
In this proof-of-concept, we only define scalar multiplication with known and unknown base point and ECDSA/EdDSA sign/verify for the NIST curves.
 ...
 ** Additional data, for instance precomputed multiples of the Generator, but also intermediate information, such as a random number, in operations such as signature generation.
...
The `SCC_content_size()` function returns only the length of the content so far. +
The total size of the following fields is stored in the _ExtraContent_ field of the metadata header.
...
[lowerroman, start=12]
  ...
  . Additional data (as in the *Internal State*).
...
* The format of the additional information is implementation defined and it is included in the _ExtraContent_ field of the metadata header.
* If there is a `marchid` mismatch, the additional data after
`PublicKey`,
`Signature`, and
`Hash`—if they exist—, including `RandomNumber`, is discarded, and the operation is restarted.
--
+
