...
| `ace.size`
| `Xd,Ls\|L[Xs]` +
`Xd,Xs` .2+|  `ace.size` returns the size in bytes of a SCC that would be exported from Locker `Ls` or `L[Xs]`, or from a locker that had `Xs` as its metadata block. +
...
For a given locker, the privileged code may encounter seven distinct scenarios.
...
[NOTE]
====
In practice, privileged code may combine both approaches. For example, lazy-loading could be employed only after migrating a virtual machine to a system with a smaller locker file, while otherwise following the simpler model. Over time, processes will adapt to the new hardware by offloading and reloading contexts as needed by following the recommendation to always check whether any configuration or import operation succeeds and manually managing the offloading.
...
NOTE: The fine-grained tracking could be extended to all higher-privilege modes. The S/VS modes can trust the state reported by U-mode, as U cannot write to the relevant CSRs. However, M-mode and HS-mode cannot rely on the values reported by S/VS, because those modes can write to these privileged registers. Consequently, when M-mode (or HS-mode) context-switches an S-mode (or VS-mode) domain, it cannot perform a fine-grained determination of which lockers have been modified, it must rely only on the `*status.ACES` registers and must therefore save and restore all lockers.
Adding further registers, such as S and VS versions of `aceconfigured`, `acealien`, and `acedirty` to track per-level modifications is possible and it would be beneficial for performance optimization.
...
* Any number of bytes can be initialized or imported in a locker, and exported from it, before the operation is preempted, depending on the alignment of the data.
 ...
 ** `ace.init` if the first 8 bytes are read, the metadata is also completely processed, then for each 16 byte block. For instance, if 28 bytes have been read in,
    the incomplete state has processed the metadata and the first content block, and knows 4 further bytes.
 ** `ace.export` [.red]#TBD#
 ** `ace.import` [.red]#TBD#
