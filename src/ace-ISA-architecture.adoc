...
* If `Zlacev` or `Zlaceio` are implemented: `misa` must implement the `L` bit, and `*status` must implement `ACES`.
...
ACE extends `*status` and `misa`, and adds up to 26 new CSRs, which are listed in <<ACE-CSRs-table>>.
...
{reg:[
    ...
    { bits:  5, name: 'Vs2', attr: ['use for auxiliary input','no auxiliary input (zero field)'] },
    { bits:  1, name: 0x1 },
    { bits:  2, name: 'R' },
...
|   9   | ace_state_dec_tag_finalize    | _dec_tag_finalize_  | Finalize internal tag computation for AE mode decryption.
...
_The `ace.input` and `ace.output` instructions may be used to replace the inputs and outputs to Forms A-C of `ace.exec`, the auxiliary input of Form C of `ace.state`, and the input of `ace.derive` in any algorithm, to the point that an implementation of ACE without the V extension or a subset thereof is possible._
...
[float="center",align="center",width="100%",cols="<18%,<41%,<41%",options="header"]
|===
| Instruction Type | Original Instruction          | Replacement Sequence
| Form A `ace.exec` | `ace.exec{nbsp}{nbsp}{nbsp}Vd, Kn\|K[Xn], Vs` | `ace.input{nbsp}{nbsp}Xl, Vs` +
`ace.exec{nbsp}{nbsp}{nbsp}Kn\|K[Xn]` +
`ace.output{nbsp}Vd,  Xl`  // Form D
| Form B `ace.exec` | `ace.exec{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}Kn\|K[Xn], Vs`     | `ace.input{nbsp}{nbsp}Xl, Vs` +
`ace.exec{nbsp}{nbsp}{nbsp}Kn\|K[Xn]`  // Form D
| Form C `ace.exec` | `ace.exec{nbsp}{nbsp}{nbsp}Vd, Kn\|K[Xn]`     | `ace.exec{nbsp}{nbsp}{nbsp}Kn\|K[Xn]` +
`ace.output{nbsp}Vd,  Xl`  // Form D
| | `ace.derive Kd\|K[Xd], Ks1\|K[Xs1], Vs2` | `ace.input{nbsp}{nbsp}Xl, Vs2` +
`ace.derive{nbsp}Kd\|K[Xd], Ks1\|K[Xs1]`
| Form C `ace.state` | `ace.state{nbsp}{nbsp}Kd\|K[Xd], #immed, Vs` | `ace.input{nbsp}{nbsp}Xl, Vs` +
...
* *States:* _initial_, _set_aux_value_, _encrypt_, _decrypt_.
 ...
 * *Behavior:*
  ...
  ** Upon issuing a Form C ```ace.state Kn|K[Xn], #ace_state_set_aux_value, INPUT```, following is performed:
   . `tweak` <- `INPUT`, and
   . *Instead of transitioning to state _set_aux_value_, the state remains unchanged (can be _initial_, _encrypt_, or _decrypt_).*
  ** In state _encrypt_, resp., _decrypt_, only ``ace.exec`` instructions of Form A are allowed.
...
* *States:* _initial_, _hash_absorb_, _encrypt_, _enc_tag_finalize_, _decrypt_, _dec_tag_finalize_, _hash_verify_, and _completed_.
...
State Machine:::
* *States:* _initial_, _set_aux_value_, _hash_absorb_, _enc_tag_finalize_, _dec_tag_finalize_, _encrypt_, _decrypt_, and _completed_.
...
** Upon issuing a Form B ```ace.state Kn|K[Xn], #ace_state_set_aux_value```:
...
* *States:* _initial_, _set_aux_value_, _hash_absorb_, _encrypt_, _enc_last_block_, _enc_tag_finalize_, _decrypt_, _dec_last_block_, _dec_tag_finalize_, _hash_verify_, and _completed_.
...
** To complete the algorithm in the decryption branch, a Form C ``ace.state`` operation must be used to transition to state _hash_verify_.
This operation also compares the `tag_len` least significant bits of `INPUT` with the `tag_len` least significant bits of `hash_P`:
+
--
  . If the values match, then the state is changed to _completed_, and `ace.error` returns no error,
  . else the state is changed to _completed_, and `ace.error` returns `ace_err_authentication`.
...
** In state _hash_finalize_, we can issue one ``ace.exec`` instruction of Form C, i.e., “```ace.exec OUTPUT, Kn|K[Xn]```”, to output the hash into `OUTPUT`.
...
* *States:* _initial_, _hash_absorb_, _encrypt_, _enc_last_block_, _enc_tag_finalize_,
 _decrypt_, _dec_last_block_, _dec_tag_finalize_, _hash_verify_, and _completed_.

* *Allowed State Transitions:*
 ** _initial_ -> _hash_absorb_ -> _hash_last_block_.
 **  _hash_absorb_, _hash_last_block_ -> _encrypt_ -- if encryption is allowed, else the CR is *invalidated*.
 **  _hash_absorb_, _hash_last_block_ -> _decrypt_ -- if decryption is allowed, else the CR is *invalidated*.
 ** _encrypt_ -> _enc_last_block_ -> _enc_tag_finalize_ -> _completed_.
...
** In state _enc_tag_finalize_, only one ``ace.exec`` instruction of Form C, to compute the tag, is allowed.
“```ace.exec OUTPUT, Kn|K[Xn]```”  performs
+
--
