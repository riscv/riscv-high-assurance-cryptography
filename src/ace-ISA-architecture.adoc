...
* During encryption, in state _hash_absorb_ we absorb the entire associated data, then the plaintext, and finally we move to state _hash_finalize_enc_ where we also absorb the lengths and produce the SIV/tag.
 ...
 ** In state _initial_ :
  ...
  ** In state _set_public_value_:
    *** We can only issue  ``ace.exec`` instructions of Form B, i.e. "```ace.exec Cn|C[Xn], Vs```," to set the value of `nonce`.
    *** If{nbsp} `ace.exec Cn|C[Xn], Vs`,{nbsp} is repeated, it will just re-write the value of `nonce`.
  ...
  ** In state _encrypt_, we can only issue  ``ace.exec`` instructions of the Form A, namely "```ace.exec Vd, Cn|C[Xn], Vs```," to encrypt `Vs` into `Vd` as follows:
   ...
   . ``Vd <- Vs xor encrypt_block(key,`` +
     ``{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}``
     ``1b1 @ auth_tag[b-2:b/4] @ (auth_tag[b/4-1:0] + counter % 2^b/4^))``.
   . `counter <- counter + 1`.
 ...
 ** In state _hash_finalize_dec_,
 ...
 +
    [.red]#Not sure this is necessary.# Finally, the state is automatically changed to _hash_verify_ (this is a very rare case of automatic transition that does not lead to state _completed_ or _invalid_) to prevent attacks where the adversary can extend the absorbtion/encryption process to try several values.

  ** In state _hash_verify_, we can only issue a Form B ``ace.exec`` instructions, i.e., "```ace.exec Cn|C[Xn], Vs`{nbsp}```", which
    compares `Vs` to `auth_tag`:
  *** If the values match, the state is changed to _completed_, and `ace.error` returns no error.
  *** If the values do not match, the state is changed to _completed_, and `ace.error` returns error `ace_err_authentication`.
