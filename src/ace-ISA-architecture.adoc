...
The most commonly used CTK is the _Base Context Transport Key_ (BCTK), which is stored in an MM-only CSR.
MM manages the BCTK unless it is hardwired or hardware-generated.
...
This binding uses both metadata and a _Derived CTK_ (DCTK) derived from the BCTK and a locale-specific secret via a key derivation function for the purpose of importing and exporting opaque keys.
...
Consequently, it _may_ maintain its own architectural state and have its own load-store unit.

// ///////////////////////////////////////////////////////////////////////////////////////////

=== Data Structures

An ACE implementation must support three distinct cryptographic data representations: the CC configuration format, the internal CC representation, and the encrypted, authenticated export/import SCC format. The configuration and SCC formats are architected for software portability and interoperability, enabling virtual-machine migration across heterogeneous hardware—for example, between cloud or consumer devices with CPUs from different vendors.

NOTE: Mechanisms for persisting Context Transport Keys (CTKs) across reboots or transferring them between devices are outside the scope of the present ACE architecture.
These are system-specific and belong to MM firmware.
...
The values should conform to <<ACE-stage_constants>>.
| [28:25] |   4  | UC_Policy_1            | "UC Policy" means "Use Control Policy". +
Disallow the _use_ of this CC in User, (Virtual) Supervisor, Hypervisor, resp., MM if bit 25, 26, 27, resp. 28, is set. Cf. <<stage-no-H-no-VMID>>.
...
Reconfiguring the BCTK is a better approach for such isolation requirements, whereas isolation by `ASID` and `VMID` is a weaker security feature._
...
bytes[u+31,32]{nbsp}{nbsp} contain `content_serialize(CC.content)` (u is clearly the byte length of the latter).
...
NOTE: Due to the computational cost of the derivation functions, implementations may cache recently used derived keys for both the base BCTK and locale-derived keys.
...
|  2   | ace_err_privilege         | `ace.exec`, `ace.state`, `ace.derive`, `ace.spawn` failed because the access control policies in the metadata prevent USE in the current architectural state.
...
A further difference between CCs and registers is that CCs can also be addressed indirectly, i.e., through the value of a GPR.
...
. We need to use one additional bit in the instruction encoding to determine whether a CC is given by an immed or by a GPR.
...
Disabling ACE is done by writing a zero to `aceenable`.
...
Note that when a process that has ACE enabled is preempted, the parent modes inherit the state to be able to export and
...
[.red]#However, we need a mechanism to make sure that if the system is not configured to insert `ASID`, resp., `VMID` into the GVAs/GPAs, we can still use them for the purpose of policing the use of CCs.#
...
If the BCTK can be programmed, it can only be programmed by MM.
For this purpose, we need eight 32-bit CSRs are available, called `acebctk0`, `acebctk1` .. `acebctk7`, where register `acebctk{i}` contains bits [stem:[31+32\,i:32\,i]] of the BCTK.
No exception is raised if software running in any other mode tries to write to the `acebctk` block, such writes are simply ignored.
The `acebctk` block always reads as zero.

NOTE: Discoverability of whether the BCTK can be configured or not is not critical, since this should be information known by the MM firmware.
[.red]#We can use the data structure pointed by `mconfigptr` in order to track optional features of ACE including whether the `acebctk` CSR block is implemented, and if not whether whether the value is ephemeral or fixed.# +
 +
User mode, and even operating systems and hypervisors, need not be aware of the `acebctk` block. Other mechanisms could have been chosen to program the CTK, such as an MM-exclusive dedicated hardware interface or memory-mapped interface.

===== Re-configurable Locales

We need two further groups of four consecutive 32-bit CSRs, programmable by MM, to define two reconfigurable locales
...
If the Locale Field of a context (CC/SCC) is -1 (all ones), then the BCTK is used.
Otherwise, this value is used as an index _i_ in the HST, and a derived key computed from BCTK and LST[_i_] is used for import/export in place of the BCTK.
...
NOTE: MM has the responsibility to "virtualize" the boot session locale secret, i.e., the sixth entry in the LST.
...
*Example*: _If a key's metadata specifies a Locale field value of "2", the key for import/export operations is derived from the BCTK and the "OEM Secret".
...
In what follows, the notation `Cn|C[Xn]` refers to a CC directly (i.e., the
...
the CCWK can be either the BCTK CSR or a Locale specific BCTK derived from the
CTK.
...
this specification. If the value cannot be transported to other devices or to
...
Description::
This instruction restricts the use of a CC to specific modes or to a specified `ASID` or `VMID`. +
...
* If `Xs`[29] is set and `m`[29] is NOT set, then Use_Policy_3, i.e., `m`[47:32], is set to `Xs`[47:32].
* If `Xs`[30] is set and `m`[30] is NOT set, then Use_Policy_4, i.e., `m`[63:48], is set to `Xs`[63:48].
...
. A list of *States* in which the internal state machine of the logic is executed, together with their representation, either as a number in the range [0..15] or by more complex structures. The values should conform to <<ACE-stage_constants>>.
...
Software environments that implement key management operations for other domains require their own BCTK. A system may contain multiple such software environments.
...
[arabic, start=5]
. Set the BCTK to the KMA’s BCTK.
. Re-import the SCC into a CC
. Change the BCTK to match the requester domain's BCTK.
...
`ace.state C1, #ace_const_hash_start` is used to start the hash function, as well as to reset it in case we want to compute a second hash using the same key.
`ace.state C1, #ace_const_hash_finalize` finalizes the hash function, and the hash value can be extracted with one or more `ace.exec V0, C1, ignore` instructions.
