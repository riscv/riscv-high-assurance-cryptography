...
For the 64-bit metadata block, only one bit can be set.
| [63:33] |  31  | _Filter_                 | Value to match against the `*acefilter` CSRs (see <<ACE-CSR-filters>> for details) or to compare to an internal secure clock.
|===
...
. The privileged code then saves the context, by exporting each multi-share field as a single share.
...
[WARNING]
A combination of more policies would be possible with an extended metadata format.
...
| 0xXXX          | SRW       | `sacefilter`            | Filter that applies to U-mode, or to VS-mode if H is active
...
the possible values are given in <<ACE-aces-encoding>>.
...
Attempts to execute any ACE instruction, or to access the ACE state, raise an illegal-instruction exception when `mstatus.ACES` is set to Off.

When `mstatus.ACES` is set to Initial or Clean, executing any instruction that changes ACE state will change `mstatus.ACES` to Dirty. Implementations may also change `mstatus.ACES` from Initial or Clean to Dirty at any time, even when there is no change in ACE state.
...
Writes to any ACES field do not affect CR contents.
...
[[ACE-ACEIOBS-field]]
===== ACE IO Buffer Status in `mstatus` and `sstatus`

An ACE IO Buffer status field, ACEIOBS, is added to `mstatus`[28:27] and shadowed in `sstatus`[28:27]  (but, see <<ACE-CSR-macecrstatus>>).
It is defined analogously to the floating-point context status field, FS, and to the vector context status field, VS.
If ACE is implemented, the ACEIOBS fields shall not be read-only zero, otherwise the field is read-only zero.
the possible values are given in <<ACE-aces-encoding>>.

[WARNING] If it is not possible to use four bits of `mstatus` for ACE, this field could be merged with , say, `aceerror`, `acesequence`, and  `aceactivecr` into a single CSR. This can be done in accordance with the ARC and does not need to be done now.

If the buffer is enabled (a non-zero value of `aceiobuflen`, see <<ACE-CSR-aceiobuflen>>), then it takes a non-zero value.
Its initial value is Initial, and upon any write to `aceiobuflen` its state returns to Initial.
In this case, it cannot be read or used in any operation, it must be written to first.

[[ACE-ACES-field-H]]
===== ACE Context Status in `vsstatus`

When the H extension is present, a vector context status field, ACES, is added to `vsstatus`[26:25].
It is defined analogously to the floating-point context status field, FS, and to the vector context status field, VS.

When V=1, both `vsstatus.ACES` and `mstatus.ACES` are in effect: attempts to execute any ACE instruction, or to
access the CRs or the input/output buffer, raises an illegal-instruction exception when either field is set to Off.

When V=1 and neither `vsstatus.ACES` nor `mstatus.ACES` is set to Off, executing any instruction that changes
ACE state, including the ACE CSs or the input/output buffer, will change both `mstatus.ACES` and `vsstatus.ACES` to Dirty.
// TODO: ace.reset should set Initial
...
[[ACE-ACEIOBS-field-H]]
===== ACE IO Buffer Status in `vsstatus`

When the H extension is present, an ACE IO Buffer status field, ACEIOBS, is added to `vsstatus`[28:27].
It is defined analogously to the floating-point context status field, FS, and to the vector context status field, VS.

When V=1, both `vsstatus.ACEIOBS` and `mstatus.ACEIOBS` are in effect: attempts to execute any ACE instruction that
accesses the input/output buffer raises an illegal-instruction exception when either field is set to Off.

When V=1 and neither `vsstatus.ACEIOBS` nor `mstatus.ACEIOBS` is set to Off, executing any instruction that changes
the state of the input/output buffer, will change both `mstatus.ACEIOBS` and `vsstatus.ACEIOBS` to Dirty.
Implementations may also change `mstatus.ACEIOBS` or `vsstatus.ACEIOBS` from Initial or Clean to Dirty at any time,
even when there is no change in the state of the input/output buffer.

If `vsstatus.ACEIOBS` is Dirty, `vsstatus.SD` is 1; otherwise, `vsstatus.SD` is set in accordance with existing specifications.

For implementations with a writable `misa.L` field, the `vsstatus.ACEIOBS` field may exist even if `misa.L` is clear.
...
Use of non-allowed instruction in the current algorithm state; or Attempt to use an invalid or partially configured CR.
...
Invalid metadata passed to `ace.init`;
...
Initial (value = 1) meant that the CR is unconfigured, and in particular it is the state of a CR after
`ace.clear` or `ace.reset` (<<ACE-instruction-clear>>).
...
//`ace.reset` and ace.clear should set Initial
...
The 64 bit register `vsacecrstatus` tracks the configuration and dirtiness of CRs on a per-CR basis.
...
When V=1, `vsacecrstatus` substitutes for the usual `sacecrstatus`, so
instructions that normally read or modify `sacecrstatus` actually access
`vsacecrstatus` instead.

When V=1, attempts to execute any ACE instruction that accesses CR #__i__ raise
an illegal-instruction exception when ``vsacecrstatus[``2__i__+1,2__i__``]`` is
set to Off.

When V=1 and ``vsacecrstatus[``2__i__+1,2__i__``]`` is set to Initial or Clean,
...
Implementations may also change ``vsacecrstatus[``2__i__+1,2__i__``]`` from
Initial or Clean to Dirty at any time, even when there is no change in CR #__i__ state.
...
Any access to the `maceBootSecret` block outside of M-mode will cause an illegal instruction exception.
...
Any access to the `maceVBootSecret` block outside of HS-mode will cause an illegal instruction exception.
...
Any access to the `saceSSecret` block outside of S-mode or higher will cause an illegal instruction exception.
...
`hcontext`/`mcontext` is only 14-bits long.
`Filter` can also be expanded using the currently reserved fields, up to 29 bits.
We believe that the final decision should be taken in accordance with the ARC.
Open is also the mattes of whether `scontext`/`hcontext`/`mcontext` can be composed.
This means that in theory we can have a filter that is 31+14+14=59 bits long.
