...
This is the format of the input to `ace.init`.
The first 8 bytes contain the 64-bit metadata section (cf.{nbsp}<<ACE-metadata>>),
...
`bytes 8 .. 19  {nbsp}{nbsp}{nbsp}{nbsp}` contain the nonce used in the encryption (96 bits), +
`bytes 20 .. 31  {nbsp}{nbsp}{nbsp}` contain the tag produced by the encryption method (96 bits), and +
`bytes 32 .. u+31  {nbsp}` contain the serialization of the content of the CR, with a byte length of `u`.
+
...
`Kd` is the source CR to be configured (encoded as a 5-bit immediate).
...
Description::
`Kd` is the source CR to be cleared (encoded as a 5-bit immediate). +
...
Description::
Exports the CRs `Ks` or `K[Xs]` to memory, starting at address `Xd`, using
 ...
 ** In state _encrypt_:
  *** If{nbsp} `ace.exec Vd,Kn|K[Xn],Vs` {nbsp}is issued, then {nbsp} `Vd = enc_blk(Vs)`.
  *** If `VLEN` is a multiple of `b`, then `ace.exec Vd,Kn|K[Xn],Vs` encrypts block-by-block.
  *** If any other Form of `ace.exec` is issued, then the CR is *invalidated*.
 ** In state _decrypt_:
  *** If{nbsp} `ace.exec Vd,Kn|K[Xn],Vs` {nbsp}is issued, then {nbsp} `Vd = dec_block(Vs)`.
  *** If `VLEN` is a multiple of `b`, then `ace.exec Vd,Kn|K[Xn],Vs` decrypts block-by-block.
  *** If any other Form of `ace.exec` is issued, then the CR is *invalidated*.
