...
NOTE: Context switching code sees only Clean or Dirty states. To establish whether a CR is actually configured, it can use `ace.size` (<<ACE-instruction-size>>) on a CR. If the instruction returns zero, that CR is not configured, and any stale state for it shall be erased and released.
