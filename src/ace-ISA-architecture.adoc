...
Let us use value 0 in the entries of `acecrstatus` to support lazy loading.
 ...
 ** *Unconfigure*: sets status to *INITIAL*.
 ** *Configure*: Works as expected; sets status to *DIRTY*.
 ** *Export* and *Usage*: The system traps to the higher-privileged mode that holds the authoritative SCC, which must import it into the CR. This may require freeing space by making another CR *LAZY*.
  Identifying the correct higher-privileged mode is typically straightforward.
  There is however one complex case, which occurs when U‑mode is preempted by the hypervisor (HV). Assume HV saved a CR and marked it *LAZY*.
  If the CR was *CLEAN*, the authoritative value is the one saved by S‑mode, so retrieving the value from the parent works well.
  If the CR was *DIRTY*, the authoritative value resides in HV, while S‑mode may hold a stale copy.
...
Architecting this feature requires careful consideration.
