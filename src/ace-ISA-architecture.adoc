...
The atomic cryptography extension adds some unprivileged CSRs to a base scalar
RISC-V ISA -- as well some privileged CSRs. It also requires a subset of the V
extension to be implemented, including the vector register file.
...
is done by writing a zero to `aceenable`, which also clears the CCs, as
...
If the RCTK can be programmed, it can only be programmed by MM. For this
