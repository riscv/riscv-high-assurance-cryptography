...
Fields Algorithm, SC_Protection, State Number, Direction_Policy and, perhaps even more notably, the Usage Control policies UC_Policy_{empty}__i__ (_i_ = 0 .. 3) are omitted, the reason being that usage of incomplete CRs is forbidden by the architecture anyway, and therefore the space can be used for other purposes.
