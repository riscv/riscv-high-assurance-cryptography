...
. `MH_size(m)` returns the byte length of the metadata header. Currently, is is always 8.
. `PI_content_size(m)` returns the byte length of the content section of a PI.
   ...
   It defends only on `m`._KeyType_, `m`._DirectionPolicy_ @ `m`._Algorithm_ @ `m`._Format_.
...
.  `ME_size(m)` returns the byte length of the metadata extension.
    In the present specification, it is 0 when `m`._FilterMode_ is 0 and it is 16 otherwise.
