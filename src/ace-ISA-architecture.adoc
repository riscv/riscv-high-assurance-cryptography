...
|===
|  Idx  | Name                      | Description                                                                   | Domain | Config?|  Bits  | Value
...
For the purpose of exporting, the optional metadata filtering extension is prepended to the content.
...
`bytes{nbsp}[{nbsp}24 .. u+23{nbsp}]{nbsp}{nbsp}` Encrypted {metadata filtering extension and} serialized content of the CR


Both metadata and the serialization of the content are authenticated, but only a part of the metadata and the content are encrypted.
...
* `RFC8452_KeyDeriv(key)` = `KeyDeriv(key,zeros(125))`, per RFC8452 (<<ACE-SCC-RFC8452-derivation>>).
 ...
 ** Upon completion of `ace.import`, the ACE unit recomputes `enc_key` and `auth_key`, i.e.:
  . (`enc_key`, `auth_key`) <- `KeyDeriv(key, zeros(125))`. // RFC8452
