...
[upperalpha]
. `ace.state Kd|K[Xd], #immed7{nbsp}{nbsp}{nbsp}{nbsp}`, with no auxiliary input.
. `ace.state Kd|K[Xd], #immed7, Xs`, with an auxiliary input in a GPR.
. `ace.state Kd|K[Xd], #immed7, Vs`, with an auxiliary input in a vector register.
...
`Xs` or `Vs` is a register that contains an auxiliary parameter for the state machine.
...
** In state _enc_tag_finalize_, only an ``ace.exec`` instruction of Form C, “```ace.exec OUTPUT,Kn|K[Xn]```,” , are allowed, which
