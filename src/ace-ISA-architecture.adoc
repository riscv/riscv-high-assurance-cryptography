...
If `aceiobuflen` is zero, the ACEIOBUF is _unconfigured_ (it does not hold any content, similarly to unconfigured CRs, i.e., CRs in state Initial), and `ace.input` and `ace.output` raises an invalid instruction exception.
...
`aceiobuftop` must be {le} `aceiobuflen`. Setting a larger value raises an invalid instruction exception.

`aceiobuftop` must be {ge} `acestart`. Setting a smaller value raises an invalid instruction exception.
...
// If `aceiobuftop` is zero, `ace.input` and `ace.output` raises an invalid instruction exception.
 ...
 ** To transition to state _hash_absorb_:
 ...
 Other forms of  ``ace.state`` raise an invalid instruction exception and the CR is *invalidated*. Then:
...
Other forms of ``ace.state`` raise an invalid instruction exception and the CR is *invalidated*.
