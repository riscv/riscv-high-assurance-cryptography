...
Off (value = 0) is currently an invalid value and unconditionally produces an error and currently it should not be allowed to set this value.
We reserve to use it for a future lazy-loading extension (under the name of _Lazy_).
...
+
`ace.clear` unconfigures the state of the relevant CR by releasing its internal resources in the CRF and setting the corresponding entry in `*acecrstatus` to Dirty
unless the CR was already unconfigured, in which case it does nothing.
+
...
The use of the **OFF** encoding in the per-CR fields of `acecrstatus` is a natural choice to denote an unavailable state that requires lazy loading to be made available. If all CRs were to be lazy-loaded, the ACES field would be set to **CLEAN** and not to **OFF**.
