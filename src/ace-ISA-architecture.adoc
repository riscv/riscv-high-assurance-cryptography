...
However, if capacity is insufficient even for lazy loading, the software responsible for loading an offloaded CR must also be able to offload other CRs in order to free space for the requested one.
 ...
 ** *Unconfigure*: sets status to *INITIAL*.
 ** *Configure*: Works as expected; sets status to *DIRTY*.
 ** *Export*: Trigger a trap to the immediately higher mode for lazy loading.
 If that mode does not have a saved value, it will trap to its parent to request the value, and will also keep it.
 This may have to be repeated a third time if H is implemented.
 The recovered SCC is imported into the CR, the status of the other is set to *CLEAN*, and the interrupted export operation is restarted.
 ** *Usage*: Trigger a trap to the immediately higher mode for lazy loading.
