...
| [25:24] |   2  | Using_IOBuf            | Zero if currently using registers, 1 if using `ace.input`/`ace.output` for the current `ace.exec`, in case the code is interrupted during between `ace.input` and `ace.exec` or between `ace.exec` and `ace.output`. 2 if the replacement sequence consists of three instructions. Cf. <<ACE-usage-input-output>>.
...
Field `Using_IOBuf` is normally zero. It is set to 1 as soon as we use a `ace.input` in place of a Form A or B `ace.exec` or a Form D `ace.exec` in place of a Form C `ace.exec`.
...
In what follows, “state _name_” refers to the state which has number _ace_state_name_ as described in <<ACE-state-constants>>.
