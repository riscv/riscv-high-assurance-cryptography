...
An implementation may not implement all the fields in the HW Binding group.
In this case, an undefined value is transparently replaced with the value associated to the immediately more _general_ Locality, and so on, as follows:

* _ChipSecret_ -> _ChipModelSecret_ -> _ChipDesignerSecret_, and
* _DeviceSecret_ -> _ProductSecret_ -> _OEMSecret_,
