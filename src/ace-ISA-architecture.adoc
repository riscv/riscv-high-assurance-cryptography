...
The value 1 is reserved for a format with a length of 128 bits.
...
| [21:17] |   4  | _State_Number_           | Encoding the current state of the state machine of the algorithm as a number.
| [22:21] |   2  | _Reserved_               | Zero.
| [27:23] |   5  | _Locality_               | A field which is either all ones (`0x1F`) or an index into the Locality Secrets Table.
...
If the Locality Field of a CC is all ones (`0x1F`), then the RCSK is used.
...
| (No Locality binding)     | No CSK derivation. CC bound to Root CSK only.                                                             | --  | -- | 0x1F
...
| Reserved                  | Reserved for future use (RZ if not defined).                                                              | Impdef | Impdef | 9-30
|===
...
`sacefilter` is used to filter CC usage in U-mode, or in VS-mode if the H extension is active,
`macefilter` is used to filter CC usage in S-mode,
and
