...
If `m`[14:13] = `0b11`, then at least one bit of `Xs`[14:13] must be set to 1 to select the allowed operation. +
...
** else `Xs`[14:13] must be 0.
* A 0 in any of the four bits `Xs`[31:28] means that the corresponding policy is _not_ changed.
...
If capacity is insufficient for lazy loading, the software reimporting an offloaded CR must be able to offload other CRs to free space. An alternative—requiring software to check for errors after each operation or to use an in-process trap handler—would impose significant burden on the software and possibly degrade performance.
