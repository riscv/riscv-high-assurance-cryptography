...
V0 ← nonce
ace.state K0, #ace_state_set_public_value, V0

// Compute AD contribution to Auth Data
V0 ← SIV
ace.state K0, #ace_state_hash_absorb, Vp
foreach(i from 0 to len_in_bytes(AD)/16-1) {
   V0 ← AD[16*i .. 16*i+15]
   ace.exec K0, V0                               // Absorb it into hash
}
...
* *States:* _initial_, _set_public_value_, _hash_absorb_, _encrypt_, _enc_last_block_, _enc_tag_finalize_, _decrypt_, _dec_last_block_, _dec_tag_finalize_, _hash_verify_, and _completed_.

* *Allowed State Transitions:*
 ** From all states to _initial_.
 ** _initial_ -> _set_public_value_ -> _hash_absorb_ -> _hash_finalize_
...
** To move to _set_public_value_, the `ace.state` operation must use the additional argument in a GPR, which sets the nonce length `N_len`.
