...
(((Locker)))
The primary mechanism for realizing cryptographic contexts is the _Cryptographic Locker_ (or _locker_).
Lockers are architectural-i.e., per-hart-containers that safeguard sensitive information.
A locker is _configured_ by writing the metadata and clear content (e.g., key material) into it.
This operation also binds the content to the metadata, creating a cryptographic context.
A CC can only be used in cryptographic operations while resident in a locker.
Only the ACE unit may access or modify a CC within a locker, according to the rules imposed by the ACE architecture and the configured algorithm.
...
Since a piece of software that uses a locker may be preempted, lockers need to be reconfigured at context switch.
...
A context sealed with a CKS cannot be reimported into a locker using a different CSK.
...
The non-standard notation `L[Xd]`, for a GPR Xd refers to the locker whose number is contained in `Xd`,
...
| `ace.init`      | `Ld\|L[Xd],%offset(Xs)`          |  Sets a lockers's metadata and content.
| `ace.clear`     | `Ld\|L[Xd]`                      |  Clears a locker and marks it as no longer in use and clean.
| `ace.export`    | `%offset(Xn),Ls\|L[Xs]`          |  Securely exports the content of a locker to memory.
| `ace.import{.S}` | `Ld\|L[Xd],%offset(Xn)`         |  Authenticates and imports a SCC from memory into a locker. +
...
| `ace.size`
| `Xd,Ls\|L[Xs]` +
`Xd,Xs` .2+|  `ace.size` returns the size in bytes of the SCC exported from Locker `Ls` or `L[Xs]`, or from a locker that had the metadata block `Xs`. +
Returns zero if the algorithm specified in `Xs` is not supported. +
`ace.available` is an alias for `ace.size`.
| `ace.available` | `Xd,Xs`
| `ace.clone`     | `Ld\|L[Xd],Ls\|L[Xs]`            |  Clones a source Locker into a destination Locker.
...
`(ace.dir)`
| `Ld\|L[Xd],#immed` +
`Ld\|L[Xd],#immed,Xs`  |  Triggers transitions in the algorithm's state (such as the different phases of hash functions or modes of operation) or provide additional information.
...
| `ace.input`     | `Ld\|L[Xd],%offset(Xs)`          |  Optional. Provides an input to the algorithm configured in Locker `Ld` or `L[Xd]`, in place of providing it through the `ace.exec` instruction. Needed only when the inputs are very long, such as the inputs to certain public key schemes.
| `ace.output`    | `%offset(Xd),Ls\|L[Xs]`          |  Optional. Writes the output of an algorithm configured in Locker `Ls` or `L[Xs]`, in place of providing it through the `ace.exec` instruction. Needed when the outputs are very long, such as the outputs of certain public key schemes.
...
ACE returns an error _ace_err_memory_ when insufficient memory prevents `ace.init` and `ace.import` operations, requiring the software to free memory by exporting or clearing a different locker. Implementations must provide sufficient LF memory to prevent infinite `ace.export`/ `ace.import` loops.
...
Writes to ACES do not affect the contents of the lockers:
Setting ACES <- Off does not destroy the state, nor does setting ACES <- Initial or Clean clear the lockers.
...
In order to know whether a locker is configured, we need an `aceconfigured` CSR, that has a bit per locker to denote whether it is configured or not.
In the simplest way, context switching privileged code can simply save and restore all configured lockers at each context switch,
where the contexts are saved if `*status.ACES` register is not Clean.

In general this is not very efficient.
Using an `acedirty` CSR, also with a bit per locker to denote whether the CC in the locker is guaranteed to be the same as the last saved state, or possibly changed, can help optimize context switching by saving only the dirty lockers.
//Fine-grained tracking activeness and dirtiness of individual lockers uses the `aceconfigured` and `acedirty` CSRs, cf.{nbsp}<<ACE-tracking-alien-lockers>>.

Since we need a mechanism to potentially implement and track lazy-loading after a migration to a device with a smaller LF, we see in the next section how to use it to have general lazy-loading.

[[ACE-tracking-alien-lockers]]
===== Fine Grained Locker Management with Lazy-Loading: `aceconfigured`, `acealien`, and `acedirty`

However, a more fine grained context switching and locker tracking mechanism will allow significant optimizations. We describe this now.

Let us define four properties for lockers:

[horizontal]
`configured`:: Indicates whether the locker is currently configured.
`alien`:: For a configured locker, indicates whether it is owned by the current process (value = 0) or by an alien one (value = 1).
`dirty`:: For a configured locker owned by the current process, indicates whether its state has been modified since the last state save.
...
It is the duty of the operating system to keep track:
...
** for each locker, whether it is:
 ...
 . configured and
  .. it contains a CC for the process (clean/dirty); or
  .. it contains a CC for an alien process (clean/dirty), together with the ID of latter.

* This is optimised by having a table of the IDs of the processes that own the CCs in each locker, or 0 if it is unallocated.
(This would be the PID of the scheduler in most operating systems and it is not used by normal processes.
A different magic value can be used.)

We list now the possible states a locker may have in case we _always_ implement lazy loading:

// ***

*VOID* (`0b000`)::
`[.darkred]#*Description:*#{nbsp}`[.darkred]#_Locker is unconfigured; there is no saved value for the current process._# +
`[.darkred]#*Properties:*{nbsp}{nbsp}configured = 0, lazy = 0.#` (Assumed: `dirty = 0` and `alien = 0`.)
** Preempted from this state to context save in privileged code: Nothing to save.
...
`configured <- 1, alien <- 0, lazy <- 0,` and `dirty <- 1` `{implies}` *DIRTY*.

*LAZY* (`0b100`)::
`[.darkred]#*Description:*#{nbsp}`[.darkred]#_Locker is unconfigured; parent has saved a value to be lazy-loaded for the current process._# +
`[.darkred]#*Properties:*{nbsp}{nbsp}configured = 0, lazy = 1.#` (Assumed: `dirty = 0` and `alien = 0`.)
** Preempted from this state to context save in privileged code: Nothing to save.
...
*CLEAN* (`0b001`)::
`[.darkred]#*Description:*#{nbsp}`[.darkred]#_Locker is configured for the current process and loaded. Unchanged since last switch._# +
`[.darkred]#*Properties:*{nbsp}{nbsp}configured = 1, alien = 0, lazy = 0, dirty = 0.#`
** Preempted from this state to context save in privileged code: Nothing to save, since the locker is already clean and the saved state also marks it as clean.
...
*DIRTY* (`0b101`)::
`[.darkred]#*Description:*#{nbsp}`[.darkred]#_Locker is configured for the current process and loaded. Potentially changed since last switch._# +
`[.darkred]#*Properties:*{nbsp}{nbsp}configured = 1, alien = 0, lazy = 0, dirty = 1.#`
** Preempted from this state to context save in privileged code: Nothing to save, the locker's state in the saved state must be marked as dirty.
** *Usage* is unrestricted.
** *Configuration*: Works as expected, and leave the properties unchanged: +
`configured <- 1, alien <- 0, lazy <- 0`, and `dirty <- 1` `{implies}` *DIRTY*.

*ALIEN* (`0b011`)::
`[.darkred]#*Description:*#{nbsp}`[.darkred]#_Locker is configured for alien process; blocked for current one; no value to be lazy-loaded._# +
`[.darkred]#*Properties:*{nbsp}{nbsp}configured = 1, alien = 1, lazy = 0.#`  (Assumed: `dirty = 0`.)
** Preempted from this state to context save in privileged code: nothing to do.
//  *** If next process has no CC to put in this locker  `{implies}` *ALIEN*.
//  *** If next process has a CC to put in this locker  `{implies}` *ALIEN_LAZY*.
...
*ALIEN_LAZY* (`0b111`)::
`[.darkred]#*Description:*#{nbsp}`[.darkred]#_Locker is configured for alien process; blocked for current one; there is a value to be lazy loaded for the current process._# +
`[.darkred]#*Properties:*{nbsp}{nbsp}configured = 1, alien = 1, lazy = 1.#`  (Assumed: `dirty = 0`.)
 ** Preempted from this state to context save in privileged code: nothing to do.
//  *** If next process has no CC to put in this locker  `{implies}` *ALIEN*.
//  *** If next process has a CC to put in this locker  `{implies}` *ALIEN_LAZY*.
 ** In this case accessing it (for usage, configuration, or export) will trap to the operating system.
 *** The operating system must save the current value to the state of its owner (if it is dirty, which is tracked by the supervisor in this case according to the dirtiness state of the locker when the owner relinquished control), and then load the value from the current state. +

While restoring the state for a process, for a given locker, the privileged code can observe following situations:

. The locker is unconfigured, and
 .. no CC for that locker is in the saved state of the process.
   * Nothing to do. Set state to *VOID*.
 .. a CC for that locker is in the saved state of the process.
   * Nothing to do. Set state to *LAZY*.
. The locker is already configured
...
The three-bit encoding in the listing of the states has been chosen to be represented by the bits in three CSRs `aceconfigured`, `acealien`, and `acedirty`, as follows:

* Bit 0 in `aceconfigured` distinguishes between unconfigured (value is 0) or configured (1);
* Bit 1 in `acealien` distinguishes between native (0) or alien (1), where an unconfigured locker is considered native in order to have a value equal to zero for the *VOID* state; and
* Bit 2 in `acedirty`, which has compressed semantics: it distinguishes between clean (0) and dirty (1) if the Locker is configured for the current process;
and between the absence of a value to lazy-load (0) or its presence (1) if the Locker is configured for an alien process.
Essentially 0 means "parent mode has nothing extra to do" and 1 means "parent mode has something dirty to do".

NOTE: If the `acealien` bit is always 0, then the bits in `aceconfigured` and `acedirty` have the same meaning as in the simpler model without lazy-loading.

NOTE: This 6-state property can in theory track the locker states for all higher-privileged modes.
...
This implies that M, resp., HS mode must always context-switch all lockers when context-switching S, resp., VS domains, since it doesn't know which lockers have changed, and therefore they can only use the `*status.ACES` fields.
While it would be possible to add additional registers to track changes at each level, ideally this should be done for F and V too (and in fact for all extensions that add state), in order to make the architecture consistent.

[[ACE-tracking-alien-hybrid]]
===== Support for Hybrid Laziness Tracking

In practice, the privileged code can also mix the two approaches, for instance considering lazy-loading only if in a VM that has been migrated to a machine with a reduced LF, choosing to behave like in the simpler model whenever possible, until the processes will adjust by themselves to the new hardware configuration by doing their own offload and reload of contexts (as they should do anyway).

It is ultimately a choice of the designer of the operating system to choose which approach they consider more appropriate for their specific use case.
The information provided by the hardware should be sufficient to allow adaptive behaviors for the purpose of optimizing performance.
...
NOTE: This instruction shares its encoding with `ace.clear` with `3'b111` as instruction code (use a GPR to select the locker).
However, the zero in the source field [11:7] for indirect locker indexing is defined as invalid/special, and this allows to define this instruction.#
--
+
...
* If the `immed12` = `0xa11`, then all lockers (that are not alien) are cleared and the new state of the ACE unit is Clear.
...
NOTE: The advantage of `ace.reset` with respect to a loop that clears all lockers is that the entire LF memory is cleared without the need for compaction/management between the separate `ace.clear` operations and this opertation cannot be interrupted, reducing potential wasteful context saves.
...
`Ld` is the source locker to be configured (encoded as a 5-bit immed).
...
Description::
`Ld` is the source locker to be cleared (encoded as a 5-bit immed). +
...
Description::
Exports the Locker `Ls` or `L[Xs]` to memory, starting at address `Xd`, using
...
Description::
`Ld` is the destination locker into which the SCC is imported, as an immed (5-bit).
...
Description::
Clones source Locker `Ls` (or the locker indexed by `Xs`) into destination Locker `Ld` (or
...
* Restrict the usage of a CC (as stored in a locker) to specific modes or to a specified `ASID` or `VMID`; and
* Activate the side-channel protection of the CC or increase it.

Let `m` be the metadata field of the CC in the destination locker.
...
If an invalid input is provided to any ACE instruction, or a non-admitted instruction is executed, while processing the algorithm in locker, say, `Ln`,
...
ace.import L0,(X5)                        // Import the SCC into Locker L0
...
ace.state L0,#ace_state_encrypt           // Configure locker for encryption
ace.error X1                              // Read error from a CSR
...
vsetvli zero,4,e32,m1,ta,ma                      // Use 128-bit vectors
ace.import L0,(X5)                               // Imports SCC into Locker L0
V5[127:32] ← IV
V5[31:0] ← 1
