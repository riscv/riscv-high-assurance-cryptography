...
It indicates the error condition on the last ACE instruction.
This implies that any read from the `aceerror` CSR may have to wait until the last operation in the ACE unit which has been issued by the current hart completes.
...
The format of the input to `ace.init` is simple: the first 8 bytes must contain the 64-bit metadata section, followed by any key and additional information in a format specified by the algorithm.
The total length of this input must be an integer multiple of 8 bytes.
...
[.red]#or we need to save and restore the state of a process in a consistent state in case we want to make cryptographic operations interruptible.#
There should be no other use cases for this instruction, as reading from (or writing to) a CC that is the target of another ACE instruction will cause the CPU to wait that the result is ready, exactly as with register reads.
--
...
[.red]#(Only in case we still want to make long cryptographic operations interruptible) Therefore, any piece of software that needs to interrupt a process in a consistent state must issue the `ace.wait` instruction before accessing any CC.#
...
[NOTE]
====
[.red]#Albeit simple, this algorithm already shows the difficulty of designing an architecture of when the vector length is smaller than the cipher's block size.
Of course, it is entirely plausible to use instruction sequences like the following#
[C]
----
...
----
[.red]#in place of a single "```ace.exec Vd, Cn|C[Xn], Vs```", but architecting all these variants would be cumbersome.
Therefore, we are going to assume that the architectures can work with either variable length vectors or with vector groups, and that
it is always possible to configure them so that the effective vector length at least the cipher's block size.#
====
