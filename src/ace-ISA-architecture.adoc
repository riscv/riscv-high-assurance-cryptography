 ...
 ** In state _set_public_value_, we can only issue ``ace.exec`` instructions of Form B, namely, "```ace.exec Cn|C[Xn], Vs```", which set the IV value to `Vs`.
  ...
  *** [.red]#If `VLEN` is larger than `n`, the most significant bits (i.e., the "tail" of the vector) are ignored.#
...
--
+
[.red]#If `VLEN` is a multiple of `b`, the above three commands are executed `VLEN/b` times, putting each time `b` bits of data into each `b`-bit block of `Vd`. This operation is clearly parallelizable.  If the number `x` of needed keystream blocks is not a multiple of `VLEN`/`b`, `VL` can be set to the remainder of the division of `x` by `VLEN`/`b` for the last instruction.#
...
--
+
[.red]#If `VLEN` is a multiple of `b`, the above two commands are executed `VLEN/b` times, encrypting/decr   each `b`-bit block of `Vs` into a `b`-bit block of `Vd`. This operation is clearly parallelizable. If the number `x` of blocks to encrypt/decrypt is not a multiple of `VLEN`/`b`, `VL` can be set to the remainder of the division of `x` by `VLEN`/`b` for the last instruction.#
