...
<<ACE-locality-indexes>> lists the architected Localities. For each Locality we provide: index in the LST; domain (SoC-wide, device-wide, VM-wide, or per-process); configurability; bit-field encoding and value. Seven of the eight _Locality_ field bits are defined; bit 7 is reserved for future use or custom Localities [.red]#(to be defined)#.
...
If a Form of `ace.exec`, `ace.state` or `ace.derive` not explicitly allowed in the current algorithm state is used, then an illegal instruction exception is raised.
...
The initial value of `acesequence` is 0. It remains zero while we are using the forms of `ace.exec`, `ace.derive`, and `ace.state` specified in the algorithm.
