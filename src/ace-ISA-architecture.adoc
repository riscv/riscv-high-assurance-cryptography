...
The preferred mechanism for supplying inputs to, and collecting outputs from, ACE operations is the vector register file of the V extension. ACE also defines an optional architectural input/output buffer (ACEIOBUF) that may be used in place of vector registers, as described in <<ACE-usage-input-output>>. Support for vector registers and for the ACEIOBUF is not mutually exclusive.

The ACEIOBUF is per-hart architectural state, separate from the CRs.
It may reside within CRF memory or in a separate memory region internal to the ACE state.

The total length and the range used in cryptographic instructions are controlled by the `aceiobuflen` (<<ACE-CSR-aceiobuflen>>) and `aceiobuftop` (<<ACE-CSR-aceiobuftop>>) CSRs.
...
An unpopulated entry is transparently replaced by the value of the immediately stricter Locality, propagating as follows:

* ChipSecret {leftarrow} ChipModelSecret {leftarrow} ChipDesignerSecret
* DeviceSecret {leftarrow} ProductSecret {leftarrow} OEMSecret

HLocality is active only when the H extension is enabled and V=1; otherwise, it mirrors MLocality.

NOTE: A system-key index may refer to keys with unrelated purposes on different devices, leading to unspecified behavior. Implementations shall assign usage policies at least as restrictive as the key's own policies, and shall supply an appropriate _Locality_ value during CR provisioning.

Multiple Localities may be simultaneously active, subject to restrictions.
At present, up to three may be active simultaneously: one from the HW Binding Group, one of PhysBootSecret or VBootSecret, and one from the SW Filter Group.
Locality Secrets are prepended to the input of the tag computation in the AEAD algorithm used for SCC export and import. If the _Locality_ field is zero, no value is prepended.
...
Bits 0, 1, and 2 of this field are ignored if U-mode, S-mode, and H-mode, respectively, are not supported or not enabled.
...
ACE extends `*status` and `misa`, and adds up to 10 new direct CSRs and up to 28 indirect CSRs.
...
ACES state transitions follow the same rules as the FS, VS, and XS transitions defined in cite:[RISCV-ISA-Priv].
...
The 8-bit `aceerror` CSR holds the error code from the most recent ACE instruction. A read may stall until the preceding ACE operation completes. The CSR is writable to support context switching.
...
Error codes 0–127 are reserved by RVI; codes 128–255 are available for custom algorithms. The standard error codes are:
