...
[.red]#This section is an early draft.#
...
[WARNING]
[.red]#Discussions are still ongoing on what subset will be required, and if a dependency on just the GPRs can be designed as well.#
...
* In place of load and store instructions, CRs can be *initialized* with clear data, but only *exported* using an AEAD algorithm, for later re-**import**.
  ...
  `ace.init` (<<ACE-instruction-init>>),
  `ace.export` (<<ACE-instruction-export>>), and
  `ace.import` (<<ACE-instruction-import>>). In brief:
** `ace.init` is a load-type instruction that reads clear text into a CR, but there is no corresponding "store" instruction;
...
* Unlike conventional registers, which always contain a value, CRs can be _unconfigured_, meaning they may hold no information.
  This is achieved by using the `ace.clear` instruction (<<ACE-instruction-clear>>).
...
|   [1:1] |   1  | Partial                | Normally, the value is 0. The value is 1 if any operation involving the configuration of the CC has been interrupted and may have to be restarted or resumed. If export is interrupted, it only needs `acestart` (<<ACE-CSR-acestart>>) for resumption.
|   [5:2] |   4  | Locality               | A field which is either -1 or an index into the Locality Secrets Table.
|  [14:6] |   9  | Algorithm              | Algorithm (such as primitive or mode). 0 means the CC is invalid.
| [16:15] |   2  | SC_Protection          | Level of Side Channel Protection, encoded as per <<ACE-SC-protection-levels>>.
| [18:17] |   2  | Direction_Policy       | Determines whether the CC may be used for encryption (bit 12 set), decryption (bit 13 set), or both (bits 12 and 13 set).
Valid only for encryption/decryption primitives or modes, and ignored, for instance, for hash functions.
| [20:19] |   2  | Key_Type               | By explicit value / Immutable (index) / Internally generated.
| [24:21] |   4  | State_Number (also Direction) | Encoding the current state of the state machine of the algorithm as a number.
| [25:25] |   1  | Using_IOBuf            | Zero if currently using registers, 1 if using `ace.input`/`ace.output` for the current `ace.exec`, in case the code is interrupted during between `ace.input` and `ace.exec` or between `ace.exec` and `ace.output`.
...
The "Partial" field signals whether any configuration operation on this cryptographic register has been initiated but not yet completed.
The state of such a CR is defined as `partial`.
This field applies only to `ace.init` and `ace.import` operations, because these access external memory, and not to `ace.clone` and `ace.derive` as they only operate internally.
`ace.export` is simply resumed using `acestart`.
Any cryptographic operation attempted on a CR containing an partial CC results in an error or is trapped.
Note also that there is a second metadata format for partial CRs, which is used when the CR is not fully initialized or imported, cf.{nbsp}<<ACE-format-configuration>>.
...
Algorithm numbers 448-511 are reserved for custom algorithms.
...
NOTE: The "State_Number" field accommodates modes of operation with a small number of states.
...
The "UC_Policy_4" field is 14 bits long, but the next two bits are reserved for compatibility with architectures employing 16-bit `VMID` fields (for instance, in heterogeneous systems).
...
Similarly to initialization inputs, the metadata has a fixed architectural format (in fact it is the same format as in initialization inputs), while the content format is algorithm-dependent.
...
ACE extends `*status` and `misa`, and adds 21 new CSRs (of which 18 are optional, 16 M-mode exclusive) to manage its execution environment.
...
NOTE: <<fsxsstates-copy>> is a copy of <<fsxsstates>>
of "The RISC-V Instruction Set Manual, Volume II - Privileged Architecture"
...
The new CSRs introduced by ACE are defined here.
...
We describe now the semantics of the `acecrstatus` bitfields:
...
** *Configuration:* does not trap, works as expected. Sets the status to *DIRTY*.
--
+
...
The semantics and use of `acestart` are described in <<ACE-format-configuration>> and [.red]#TBD#.
...
If the size is zero, the buffer is disabled and the instructions will fail.

[[ACE-CSR-aceiobuftop]]
===== `aceiobuftop`

`aceiobuftop` is an XLEN-bit RW CSR. +
`aceiobuftop` is used to program the top end of a transfer operation.
It works in conjunction with `acestart` to define the range of the buffer. +
...
CSRs `maceOSSecret0` and `maceOSSecret1`, are used to configure the "OSSecret" Locality (cf.{nbsp}<<ACE-Localities>>) in RV64, to which RV32 adds `maceOSSecret2` and `maceOSSecret3`.
(((CSR, `maceOSSecret*`)))

A write to `maceOSSecret{i}` set Bits [(_i_+1) XLEN - 1 : _i_ XLEN] of the "OSSecret".
...
CSRs `maceBootSecret0` and `maceBootSecret1`, are used to configure the "BootSecret" Locality (cf.{nbsp}<<ACE-Localities>>) in RV64, to which RV32 adds `maceBootSecret2` and `maceBootSecret3`.
(((CSR, `maceBootSecret*`)))

A write to `maceBootSecret{i}` set Bits [(_i_+1) XLEN - 1 : _i_ XLEN] of the "BootSecret".
...
`SERIALIZE()` and `DESERIALIZE()` map the content of a CR into a `content` byte array to be encrypted for export (as defined in <<ACE-algorithms>>) and map such a decrypted byte array back into a CR.
`content` is a virtual representation; its consecutive bytes do not imply physical contiguity within the CR, nor there is a 1-1 correspondence between its bytes and those in the CR.  For instance, keys may be split across shares in a threshold implementation.   However, any read or write to `content` must correctly map to the underlying representation. A write followed by a read must return the original data, and a read followed by a write must restore the identical microarchitectural state.
...
|   - This includes computing any derived values not given in the initialization
|   - input, as well as creating shares for threshold implementations.
UNINTERRUPTIBLE_BLOCK_END
----

==== Authenticated Encryption for Sealed Cryptographic Contexts
...
The generation of the SCC from the CC in a CR (<<ACE-SCC-export>>) and the import of a SCC into a CR (<<ACE-SCC-import>>) are defined next.
...
NOTE: It can happen that we export an partial CR, for instance, M-mode or HS-mode code is saving a CR that was only partially configured in U-mode when the U-mode process was interrupted. How this is handled in SCCs upon export and import (operation which could be interrupted as well) is <<ACE-incomplete-contexts>>.
(((Cryptographic Register,incomplete)))
...
==== Partial Cryptographic Contests

[WARNING]
We still need to evaluate all pro and cons of restarting vs. resuming for contexts (this is a separate matter from the input/output buffer).

An initialization or import operation may be interrupted by a context switch, preventing immediate return to the original control flow.

An initialization operation can be interrupted at any 8-byte multiple, and resumed from there. [.red]#TBD: we could actually always restart an initialization.#

As long as the operation can also be interrupted at a 8-byte multiple, but it will be resumed only at the 48th byte or beyond.
Otherwise, it will be restarted, and no state will be saved at context switch. [.red]#TBD: resuming could possibly be more expensive than saving no state and restarting. The third option, to wait and complete the operation, could instead cause delays.#

In case of context switch, the intermediate state of the CR must be saved and later restored to allow the operation to be resumed (by setting `acestart` similarly to the use of `vstart`).
(((Cryptographic Register,incomplete)))
...
NOTE: Fields Algorithm, SC_Protection, State Number, Direction_Policy and, perhaps even more notably, the Usage Control policies UC_Policy_{empty}__i__ (_i_ = 0 .. 3)
and Locality information are omitted, the reason being that usage of partial CRs is forbidden by the architecture anyway, and therefore the space can be used for other purposes.
Regarding the Locality, Locality violation errors should be handled where the complete CR is used, and therefore the RCSK is always used for partial CRs. This may also reduce the need to compute or cache DCSKs at higher privileged levels, overall improving performance.

The content format to export incomplete initializations and imports is the same as for the complete CRs,
only truncated to the actual imported length, rounded to a multiple of 8 bytes.
This length is stored in the Payload_Length field.

The content format to export incompletely imported CRs prepends the partial hash (`tmp` in <<ACE-SCC-import>>) to the content, which is thus extended by 16 bytes. These 16 bytes must be included in the Payload_Length field and encrypted as well.

After the partial CR has been decrypted and authenticated in privileged mode, it is marked as to be resumed.
Upon returning from the context switch handler,

. the old metadata, nonce and SIV are recovered from the memory of the less-privileged mode;

(These reads may be interrupted by page faults and immediately resumed upon mapping the target page.)

The following operations are then performed as an uninterruptible block.

[start=2]
. `z` and `num_blocks` are computed from the original content length, the starting value of `i` from `acestart`.
. If the interrupted operation was an import:
.. The `Kenc` and `Kauth` keys are derived.
.. The partial hash value is recomputed from the current content of the CR.

At this point the initialization or import operation can resume from the point of interruption.
...
ACE architects support for several algorithms, and also defines a minimal required subset [.red]#(Not finalized!)#:
...
//If the context `Ks` is clear, or `Xs` is out of range, the operation will report error ace_err_invalid.
...
NOTE: The returned value can significantly differ from the internal resource usage to store the CC's data in a CR —
...
Only the Algorithm and SC_Protection fields of the metadata section of a CC are considered. Only the bits corresponding to the
Algorithm, SP_Protection, Direction_Policy and Key_Type fields fields are considered and all other bits must be zero.
...
* Bits [17:16] and [63:26] of the input register `Xs` have the same format as the corresponding bits of the metadata.
* Bits [15:0] and [25:18] of the input register `Xs` are ignored.
* Bits [17:16] can be used to turn on or increase the level of the side-channel protection of the algorithm in the CC, if the microarchitecture supports the corresponding implementation. The levels are defined in <<ACE-SC-protection-levels>>. Downgrading side-channel protection is not permitted, and attempting to do so will result in error _ace_err_invalid_ (cf.{nbsp}<<ACE-error-codes>>).
...
First, the `ace.input` and `ace.output` instructions can replace the inputs and outputs to Forms A-C of `ace.exec` only if the size
in CSR `aceiobuflen` is non-zero and is a valid size (for instance `b` for an algorithm or a multiple thereof).

If `aceiobuflen` is non-zero, then forms A, B, and C of `ace.exec` can be replaced in the code as follows:

[[ACE-replacement-instructions]]
.Replacement for `ace.exec` instructions with input/output
[float="center",align="center",width="64%",cols="^8%,<31%,<25%",options="header"]
|===
| Form | Original Instruction          | Replacement
|   A  | `ace.exec{nbsp}Vd, Kn\|K[Xn], Vs` | `ace.input{nbsp}{nbsp}Xl, Vs` +
`ace.exec{nbsp}{nbsp}{nbsp}Kn\|K[Xn]` +
`ace.output{nbsp}Vd,  Xl`
|   B  | `ace.exec{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}Kn\|K[Xn], Vs`     | `ace.input{nbsp}{nbsp}Xl, Vs` +
`ace.exec{nbsp}{nbsp}{nbsp}Kn\|K[Xn]`
|   C  | `ace.exec{nbsp}Vd, Kn\|K[Xn]`     | `ace.exec{nbsp}{nbsp}{nbsp}Kn\|K[Xn]` +
`ace.output{nbsp}Vd,  Xl`
|===

If a Form A-C of `ace.exec` is used, only the allowed form can be used and the input and output vectors are used instead.
[.red]#To be discussed: We need to use a bit in the metadata to indicate whether we are using buffer or the registers. Y/N.#

This is reflected in the Using_IOBuf bit in the metadata.

Any Form A-C `ace.exec` instruction can be replaced by a Form `D` instruction in all algorithms.
The content of the internal buffers for inputs and outputs is zeroed by all algorithms in the _initial_ and _completed_ states.
`ace.input` is allowed whenever a Form A or B `ace.exec` is expected by the architecture, and
`ace.output` is allowed whenever a Form B or C `ace.exec` is expected.
Failure to issue these instructions can lead to undefined behavior, but it does not raise errors.
Issuing them when they are _not_ expected raises an invalid instruction exception.

NOTE: The internal input/output buffer, if enabled, it not part of a CR but is a separate structure.
In fact, `ace.output` can be used to save the contents of the buffer, and `ace.input` can be used to restore it.
...
[WARNING]
[.red]#This section is currently non-normative. There are ongoing discussions on whether to formalize it completely and make it normative.#
...
For each algorithm, in any state, only certain instructions are permitted.
...
We recall that operations performed by `ace.exec` or `ace.state`—as defined by the algorithm-specific rules—are uninterruptible. This includes processing of arbitrarily long vector inputs when all work is purely internal to ACE and in particular without access to external memory (e.g., encryption, decryption, hashing, or point multiplication). Because these operations may proceed independently of the CPU core, a hart implementation may require context-switching software to wait when accessing a CR that is being written. These instructions are therefore atomic and non-preemptible.
...
The GCM mode was defined by [.red]#to be completed#.
(((ACE, algorithm, GCM)))
...
We only support the OCB3 variant of OCB as specified in RFC 7253 and https://link.springer.com/article/10.1007/s00145-021-09399-8.
...
The state machine of this algorithm closely follows RFC 7253 except for the bit ordering and the use of zero-based indexing in place of one-based indexing as in the RFC. The algorithm and is of course identical to the RFC's for 128-bit-wide block ciphers, with the generalisation to other widths being straightforward and according the security proof of the original algorithm and a lower bound on nonce length following https://eprint.iacr.org/2023/326.
...
** In state _set_public_value_, only  ``ace.exec`` instructions of Form B, "```ace.exec Kn|K[Xn], Vs```", is allowed, whereby `N` is set to `Vs`, namely
  ...
  *** If no such `ace.exec` is issued, `N` remains zero and will cause an error later.
...
* `enc_blk(key, p) -> c` encrypts block `p` using the key, and returns the result `c`.
* `gen_subkeys(K) -> (K1,K2)` generates subkeys `K1` and `K2` from the master key `K` as follows: Let `C` be `0x1B` for `b` = 64, `0x87` for `b` = 128, and `0x425` for `b` = 256, then
...
//  ** In any state, issuing any Form of ``ace.exec`` that is not explicitly allowed will produce a null output and the CR will be *invalidated*.
...
* `k` = key size (8|`k`) -- only if keyed, i.e., a MAC.

Data Structures:::

* *Initialization Input*:
[lowerroman]
 . `key`: `k` bits -- Only if keyed, i.e., a MAC.

* *Internal State*:
 ** `key`: `k` bits -- Only if keyed, i.e., a MAC.
 ** `hash_state`: algorithm specific length.
 ** `last_block_len` -- Only if this Hash/MAC has special internal treatment for a fractional last block that goes beyond padding and appending the length, which are handled by user software  (field size at most 16 bits).
...
The usage of this type of algorithms is very similar to that of CMAC (<<ACE-CMAC-mode>>), with the exception that if the last block is not treated in a special way, there is no state _hash_last_block_ and after the last block is absorbed, the state is changed to _hash_finalize_.
...
While ACE can be implemented with the V extension as a requirement, ACE actually depends on a minimal subset of the RISC-V Vector Extension (RVV), which we call RVV-mini, for correct operation.
...
[[ACE-comparison-of-interruption]]
=== Interrupting Long Instructions on RISC-V vs Other Architectures

This document describes a secure mechanism analogous to RISC-V's `vstart` CSR, to enable hardware to resume a partially completed vector instruction after an interrupt or exception.

Other architectures, such as ARM or Intel ISAs, lack a direct equivalent.
For instance, ARM's SVE does not guarantee precise resume semantics like `vstart`.
Its architects opted for a simpler model where an instruction is either completed or restarted, depending on implementation.

Within the HAC TG, there has been significant debate on whether to adopt a similar model or to implement the more precise semantics of the V extension.
Two key distinctions between vector operations and cryptographic operations are:

. Cryptographic protocols often maintain additional state (e.g., a partially computed hash). This state must be stored externally for later resumption, leading to content expansion.
. Interrupting and resuming a cryptographic operation (such as a mode of operation) and storing partial results in external memory makes that state vulnerable to manipulation.  The security of a cryptographic operation can be completely compromised if the internal state can be manipulated or observed.

These issues persist even when modes are not programmer-facing, because an AEAD mode must be used to protect the confidentiality and integrity of any exported data. While `vstart` is a relatively simple feature, architecting a similar mechanism for cryptographic operations is far more complex.

[WARNING]
One could argue that these security concerns, coupled with the cost of encrypting and decrypting a partial SCC, outweigh the benefit of resuming an interrupted import versus restarting it. This reasoning could justify an "SVE-like" model for ACE: place the responsibility on software and the OS to ensure that all input and outputs  reside in physically mapped memory, and require that operations either complete before an interrupt handler runs or are restarted afterward.  However, this model departs from the current RISC-V approach.  Consequently, we have architected support for resumable operations, leaving it open whether to accept, reject, or make this feature optional.

We performed a security analysis of potential scenarios.
A particularly challenging scenario arises when a context switch occurs during an import: the partially imported context must be exported and later resumed.
Resumption requires properly decrypting and authenticating the saved partial context, retrieving the original nonce and SIV, reconstructing the partially computed hash from the re-imported state, and continuing from the point of interruption. The chosen AEAD mode supports a mostly linear sequence of memory reads, which is essential for efficient resumption, but a non-adjacent sequence of memory reads is required for the initial recovery of metadata, nonce, and SIV

We concluded that, while privileged code can always replace an SCC in a saved process's context or memory, it cannot violate the confidentiality or integrity of an SCC.

If the ARC opts for the simpler approach, several simplifications become possible: `acestart` and the formats for partially saved contexts would be unnecessary. Resumption of `ace.input` and `ace.output` could also be removed, as saving and restoring partial I/O buffers is typically more expensive than restarting.
These changes would be relatively minor architecturally, but could adversely affect system responsiveness for real-time applications, especially on smaller cores.

//

[comment]
--
ACE has mechanism similar to RISC-V’s `vstart` CSR that lets the hardware resume a partially completed vector instruction after an interrupt or exception.

Other architectures, such as the ARM or Intel ISAs do not provide a direct equivalent to this.
For instance, SVE does not guarantee precise resume semantics like `vstart` does;
the architects chose a simpler model where the instruction is either completed or restarted depending on implementation choice.

There has been significant debate within the HAC TG on whether to adopt a similar model or try to implement the more precise sematics of the V extension.
Two important differences between V's vectors and vector operation and handling cryptographic operations are:

. Many cryptographic protocols need to keep additional state, for instance a partially computed hash value. Such a state must be stored in external memory for later resumption, with leading to content expansion.
. Interrupting and resuming a cryptographic operation, such as a mode of operation, and storing the partial results in external memory, leaves the state amenable to manipulation.

In particular, these issues arise even if modes are not implemented as programmer-facing features, since an AEAD mode of operation must be used to protect the confidentiality and integrity of any exported data.  It turns out that while `vstart` is a simple feature (no matter how brilliant), architecting a similar mechanism for resuming cryptographic operations is way more complex.  The security of a mode of operation can break completely if any form of manipulation or observation of the state is possible.

We can argue that these security issues, and the costs of encrypting and decrypting a partial SCCs is higher than just re-decrypting the initial part of the interrupted import.
This is sufficient to justify a "SVE" model for ACE:
leave the responsibility to ensure that the entire buffer for any input and output is in physically mapped memory to the software and the OS,
and ask that all operations, including those that require memory accesses, be either completed before handing the control flow to an interrupt handler or completely resumed after the interrupt handler has completed.
However, this model represents a departure from the current RISC-V approach. +
Therefore, we decided to architect resumable operations, leaving it open whether to accept or reject this feature, or make it optional.

We performed a security analysis of all possible situations.
A challenging case occurs when a context switch occurs during an import operation: the partial imported context must be exported, and later it must be resumed.
Resumption involves properly decrypting and authenticating the saved partial context, and then retrieving the old nonce, and SIV, reconstructing the partially computed hash from the now re-imported partial state using the original nonce and SIV, and continuing the operation from where it left off.  The chosen AEAD mode of operation lends itself to a linear sequence of memory reads (except for the initial recovery or the original metadata, nonce and SIV), which is crucial for efficient resumption, and the fact that the partially saved information is not revealed eliminates the risk of successfully forging a different state.

As a result, privileged while code can always replace a SCC with a different one in a saved process's context, or even directly in a process's memory space, it cannot violate the confidentiality and integrity of a SCC.

It the ARC decides for the simpler approach, then some simplifications can be made: we do not need `acestart` and the formats for partially saved contexts.
Further simplifications can be made by removing resumption of `ace.input` and `ace.output` (after all, saving a partial input/output buffer and restoring it for resuming the operation is also more expensive than restart).
The overall changes to the architecture would be relatively small, but responsiveness of the system for real-time applications, especially on smaller cores, may be adversely affected.
--
