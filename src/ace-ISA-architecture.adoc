...
vector register file (see <<ACE-Dependencies>>). Discussions are still ongoing on what subset will be
...
The `aceerror` CSR is an 8-bit CSR.
...
In the next table, we define the mandatory error codes.
...
NOTE: User mode, and even operating systems and hypervisors, need not be aware of the `ace_OS_Secret*` and `ace_Boot_Session*` blocks. Other mechanisms could have been chosen to program the "OS Secret" and  "Boot Session" entries, such as an MM-exclusive dedicated hardware interface or memory-mapped interface.
