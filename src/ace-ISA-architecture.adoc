...
Here, “Configure” refers to the management operations that can configure a CR, namely `ace.init`, `ace.import` and being the destination of an `ace.clone` or `ace.derive` operation. “Unconfigure” refers to the use of `ace.clear` on a CR.
...
* The value 0 is currently not defined.
...
Either S‑mode must retain this dirtiness information for each of its child U‑mode processes, or the fields of `acecrstatus` must be expanded to three bits to accommodate a fifth status, *LAZY/DIRTY*.
