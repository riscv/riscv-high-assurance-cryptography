...
When ``scrstatus[__i__]`` is set to Clean, executing any instruction that changes CR #__i__ will change ``scrstatus[__i__]`` to Dirty.

// Implementations may also change ``scrstatus[__i__]`` from
// Initial or Clean to Dirty at any time, even when there is no change in CR #__i__ state.

NOTE: `sstatus.ACES` is also in effect.

NOTE: the `scrstatus` CSR does not depend on the S extension.  In an M-U system, M-mode can use the CSR to optimize U-mode context switches.
...
+
`ace.clear` unconfigures the state of the relevant CR by releasing its internal resources in the CRF and setting the corresponding entry in `*crstatus` to Dirty
