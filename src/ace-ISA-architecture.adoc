...
. `SCC_content_size(x)` returns the size in bytes of the fixed part content section of an SCC. It depends only on the 13 bits `DirectionPolicy @ KeyType @ Algorithm`.

The total size of a PI is therefore
+
`MD_size(x)` + (_FilterMode_ != 0) * sizeof(metadata extension) + `PI_content_size(x)`,
+
and the total size of an SCC is
+
`MD_size(x)` + (_FilterMode_ != 0) * sizeof(metadata extension) + `SCC_content_size(x)` + _ExtraContent_ * 8.
