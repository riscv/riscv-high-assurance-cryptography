...
. `SCC_content_size(x)` returns the size in bytes of the fixed part of the content section of an SCC.
   It does not include the length of the variable part of the content, _ExtraContent_ * 8 bytes, nor the length of the optional Metadata Extension block.
   It depends only on the 13 bits `DirectionPolicy @ KeyType @ Algorithm`.

Let `ME_size` be the metadata extension size in bytes.
The total size in bytes of a PI is therefore

`{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}MH_size(x)` + (_FilterMode_ != 0) * `ME_size` + `PI_content_size(x)`,

and the total size in bytes of an SCC is

`{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}MH_size(x)` + 32 + (_FilterMode_ != 0) * `ME_size` + `SCC_content_size(x)` + _ExtraContent_ * 8.
