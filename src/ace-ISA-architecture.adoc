...
* `Zlctrm` => Support for counter mode (individual details depend on the underlying primitive)-the `m` at the end denotes a mode
* `Zlxctrm` => Support for XCTR mode (individual details depend on the underlying primitive)
* `Zlgcmm` => Support for GCM mode (individual details depend on the underlying primitive)
* `Zlgcmsivm` => Support for GCM-SIV mode (individual details depend on the underlying primitive)
* `Zlxexm` => Support for XEX and XTS modes (individual details depend on the underlying primitive)
* `Zlocbm` => Support for OCB mode (individual details depend on the underlying primitive)
* `Zlcmacm` => Support for CMAC (individual details depend on the underlying primitive)
* `Zlesha2h` => Support for SHA-2 (all modes)-the `h` at the end denotes a hash function
* `Zlesha3h` => Support for SHA-3 (all modes)
* `Zlsm3h` => Support for SM-3 hash function (all applicable modes)
* `Zlhmacm` => Support for HMAC (individual details depend on the underlying primitive)
...
|   [1:1] |   1  | Incomplete             | Normally, the value is 0. The value is 1 if any operation involving the configuration of the CC has been interrupted and may have to be restarted or resumed. If export is interrupted, it only needs `acestart` (<<ACE-CSR-acestart>>) for resumption.
|   [5:2] |   4  | Locality               | A field which is either -1 or an index into the Locality Secrets Table.
|   [7:6] |   2  | Reserved               | Reserved for future use.
|  [15:8] |   8  | Algorithm              | Algorithm (such as primitive or mode). 0 means the CC is invalid.
...
The information in the "Incomplete" field is necessary to make sure that any cryptographic operation attempted on a CR containing an _incomplete_ CC results in an error or is trapped. Note also that there is a second metadata format for incomplete CRs, which is used when the CR is not fully initialized or imported, cf.{nbsp}<<ACE-format-configuration>>.
...
The semantics and use of `acestart` are described in <<ACE-format-configuration> and [.red]#TBD#. zzzz
...
==== Incomplete Cryptographic Contests
...
(((Cryptographic Register,incomplete)))
Then exporting the incomplete CR, the following metadata header format is used as the first 8 bytes of the SCC to store information about the incomplete CR.
After this header block, the SCC contains the nonce and SIV following <<ACE-format-SCC>>, and then the content section.

The Incomplete CR Metadata (ICREM) header must distinguish between incomplete CRs following interrupted
...
[[ACE-metadata-incomplete]]
.Format of the Incomplete CR Metadata
[width="100%",cols="^9%,^7%,17%,67%",options="header"]
|===
.>|   Field .>| Width +
(bits) .>| Name                  .>| Description
...
and Locality information are omitted, the reason being that usage of incomplete CRs is forbidden by the architecture anyway, and therefore the space can be used for other purposes.
Regarding the Locality, Locality violation errors should be handled where the complete CR is used, and therefore the RCSK is always used for incomplete CRs. This may also reduce the need to compute or cache DCSKs at higher privileged levels, overall improving performance.
...
After the incomplete CR has been decrypted and authenticated in privileged mode, it is marked as to be resumed, in which case, upon returning to the less-privileged mode, the following operations must be performed:
