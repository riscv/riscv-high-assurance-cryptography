...
*DIRTY* (value: 3), i.e. CR configured and loaded; potentially changed since switch.#::
* *Usage:* Unrestricted.
* *Configuration:* Works as expected; status unchanged.

NOTE: While the *LAZY* status reuses the FS/VS/ACES encoding for "OFF", this it does _not_ mean that the ISE is not active.
