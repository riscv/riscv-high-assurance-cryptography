 ...
 * *Behavior:*
  ...
  ** Upon issuing ```ace.state Kn|K[Xn], #ace_state_set_public_value, Vs```
   . `tweak` <- `Vs`, and
   . The state transitions to the previous state (can be _initial_, _encrypt_, or _decrypt_).
  ** In state _encrypt_, resp., _decrypt_, only ``ace.exec`` instructions of Form A are allowed.
     “```ace.exec Vd, Kn|K[Xn], Vs```” encrypts, resp., decrypts `Vs` into `Vd`.
