...
| [28:25] |   4  | _UC_Policy_              | “UC Policy” means “Usage Control Policy”. If bit 28, 29, 30, resp. 31 is set, it disallows the _usage_ of this CC in User, (V)S-mode, H-mode, resp. M-mode.
...
* `Xs`[10:0] and `Xs`[27:15] must be 0.
* `Xs`[14:11] and `Xs`[63:32] have the same format as the corresponding bits of the metadata.
...
** `Xs`[14:13] is used to deactivate either encryption or decryption in a CC that allows both.
** If `m`[14:13] = `0b11`, then at least one bit of `Xs`[14:13] must be set to 1 to select the allowed operation(s).
** If `m`[14:13] != `0b11`, then it must be `Xs`[14:13] = `m`[14:13] or `Xs`[14:13] = 0, in which case no change is made, else this will result in error _ace_err_invalid_.

* If the algorithm in the CR does not encryption and decryption, then `Xs`[14:13] must be 0.
...
* `Xs`[34:32] can be non-zero only if `m`[33:32] is 0, in which case  `m`[59:35] <- `Xs`[59:35], else `aceerror` is set to _ace_err_invalid_.
...
[.red]From the point of view of the ACE architecture, we need only to define two CSRs and their behaviors.
