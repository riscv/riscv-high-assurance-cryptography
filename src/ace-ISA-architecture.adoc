...
ACE keeps a table of 125-bit Locality Secrets (LS), the Locality Secrets Table (LST).
...
Currently, nine values in the LST are architected.
The first six Localities tie SCCs to the hardware.

The Locality Secrets are used to tweak the AEAD algorithm used to export and import CCs.
If the Locality Field of a CC is zero, then the CSK is used directly without tweaking the AEAD algorithm.
...
For each Locality we provide the following information: its index in the LST; its _Domain_, i.e., whether the value is chip-wide (i.e., the same across all chiplets in a SoC), device-wide, or OS-wide; the configurability; the bit field it is encoded into and its value in that field.

[[ACE-locality-indexes]]
.Architected Locality Values
[width="100%",cols="^7%,18%,43%,^9%,^9%,^7%,^7%",options="header"]
|===
| Index | Name                      | Description                                                                   | Domain | Config |  Bits  | Value
|   --  | (No HW Binding)           | Independent of Chip, OEM, Device                                              |  --    |   --   | [2:0]  |   0
|   0   | _ChipDesignerSecret_      | Identifies the designer of the SoC. Permanent.                                |  SoC   |   No   | [2:0]  |   1
|   1   | _ChipModelSecret_         | Identifies specific chip model/family. Permanent.                             |  SoC   |   No   | [2:0]  |   2
|   2   | _ChipUniqueSecret_        | Unique for each SoC. Permanent.                                               |  SoC   |   No   | [2:0]  |   3
|   3   | _OEMSecret_               | Provisioned by OEM   to differentiate their products from the competition.
Permanent, or reconfigurable via an implementation-defined HW authenticated mechanism.                              | Device |   HW   | [2:0]  |   4
|   4   | _ProductSecret_           | Identifies device/system type/model (product). +
Permanent or reconfigurable like _OEMSecret_.                                                                       | Device |   HW   | [2:0]  |   5
|   5   | _DeviceSecret_            | Unique device (entire system) secret.
...
Reconfigurable like _OEMSecret_.                                                                                    | Device |   HW   | [2:0]  |   6
|   6   | _BootSecret_              | Unique per the boot session of the host hardware and its firmware stack. +
Configured by M-mode during the boot process of the host (see <<ACE-CSR-maceBootSecret>>).                          | Device |   M    |   3    |  0/1
|   7   | _VBootSecret_             | Unique per the boot session of the virtual hardware and its software stack. +
Regenerated by H or higher at each virtual boot of a VM (see <<ACE-CSR-haceVBootSecret>>).                          |   VM   |  H/M   |   4    |  0/1
|   8   | _SSecret_                 | Configurable by S/VS-mode to mutually isolate +
processes in U-mode (see <<ACE-CSR-saceSSecret>>).                                                                  | Process | S/VS  |   5    |  0/1
|  --   | _Reserved_                | Reserved for future use.                                                      | Impdef | Impdef | [7:6]  |  --
|===
