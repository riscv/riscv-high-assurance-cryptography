...
1 or 2 if using `ace.input`/`ace.output` in place of vector registers.
...
[.red]#TBD: need to tie the `ace.input` and `ace.output` instructions to a context, otherwise the system does not know how to “attach” it to metadata.#
...
We use CSR `acesequence` in the metadata to indicate whether we are using buffer or the registers and to track the instruction sequence.
`acesequence` is normally zero.
As soon as an `ace.input` in place of a Form A or B `ace.exec` or `ace.derive`, or a Form D `ace.exec` in place of a Form C `ace.exec` is completed, it is set to 1.
In this case, `acesequence` remains 1 until the end of the second instruction of the replacement sequence, and then set to 2 until the end of the third instruction of the replacement sequence if there is a third instruction in the sequence.
At the end of the sequence, `acesequence` is reset to 0.
The ordering of these instructions is enforced by the architecture according to the value of `acesequence` and the state number of the CR configured in `aceactivecr`.
