...
let T : bits(96) = POLYVAL(Kauth, metadata @ content)[95:0];

let SIV : bits(96) = AESE(Kenc, 0 1'b0 @ ((T xor N)[126:0]))[127:32];
// not preemptible until this point

// preemptible here (for instance because of page fault), `acestart` = 0
M[15..0] = N[63:0] @ metadata[63:0];

// preemptible here, `acestart` = 16
M[31..16] = SIV[95:0] @ N[95:64];

foreach(i from 0 to num_blocks - 1) {
    // preemptible here, `acestart` = 32+i*16
    if ((i == num_blocks - 1) and last_block_fractional) {
        M[39+i*16..32+i*16] = content[7+i*16..i*16]
                              xor AESE(Kenc, 1'b1 @ SIV[94:0] @ (binary(i)[31:0]))[63:0];
    } else {
        M[47+i*16..32+i*16] = content[15+i*16..i*16]
                              xor AESE(Kenc, 1'b1 @ SIV[94:0] @ (binary(i)[31:0]));
    }
}
----
