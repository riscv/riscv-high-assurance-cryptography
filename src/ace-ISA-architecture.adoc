...
If the algorithm encoded in the CC has fewer states is less than 16, then this number is stored in this field, cf. <<stage-field>>,
otherwise an algorithm-specific representation is used in the content, and this field only admits the values 0 (initial), 15 (completed) or 1 (anything other operational state). The values must conform to <<ACE-stage_constants>>.
...
|  4   | ace_const_hash_absorb         | Update the internal state of a hash function with new data.
|  5   | ace_const_hash_finalize       | Finalize the internal state of a hash function and prepare it for outputting the result.
...
|  8   | ace_input_for_derive          | Tells the CC to expect additional information for feeding the `ace.derive` instruction.
| 15   | ace_completed                 | The algorithm has either completed or has abnormally terminated. The latter condition is determined by ace.error.
|===
...
An `ace.exec` instruction may modify the internal state of the CC, including automatically changing the stage field of the metadata section
making some `ace.stage` operations redundant.
...
NOTE: This instruction can be used multiple times, to generate multiple derived contexts.
...
* *Behavior:*
 ** `ace.stage` is in general used to move to the _encrypt_ or _decrypt_ state.
 ** If the metadata allows only one of encryption or decryption, the state changes automatically to _encrypt_ or _decrypt_ state, making a call to `ace.stage` redundant.
 ** In the _initial_ state, any `ace.exec` will cause an ace_error_invalid error.
 ** In the _encrypt_ state,
...
There is no need to architect the behavior of chaining modes,
such as
...
* *Behavior:*
 ** From the initial state the context automatically moves to _set_public_value_.
 ...
 ** In _set_public_value_ state, we can only issue an `ace.exec` of the form{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`,{nbsp}{nbsp} which sets the IV value to `Vs`.
    The state then changes to _operate_ (that is, there is no need to issue `ace.stage Cn|C[Xn], ace_const_operate`).
 ...
 ** In _operate_ state, we can only issue an `ace.exec` of the form `ace.exec Vd, Cn|C[Xn]`, which
  ...
  *** Other forms of `ace.exe` not allowed, will cause `error(ace_error_invalid)`.
  *** If 'ace.stage' is issued to return to _set_public_value_ (passing zero, i.e., _initial_, will also set to this state automatically), we can reuse the method with a new counter.
 ...
 ** _encrypt_, _decrypt_ -> _set_public_value_.
* *Behavior:*
 ** From the initial state the context automatically moves to _set_public_value_.
 ** In the _set_public_value_ state, we can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp}{nbsp} to set the `IV <- Vs`.
    If no such instruction is issued, IV is assumed to be zero.
 ...
 ** In _encrypt_ and _decrypt_ states, we can only issue `ace.exec Vd, Cn|C[Xn], Cs` which:
  ...
  *** If 'ace.stage' is issued to return to _set_public_value_ (passing zero, i.e., _initial_, will also set to this state automatically), we can reuse the method with a new IV.
...
NOTE: The lengths of the AD and of the plaintext must be maintained by the calling SW.
...
* `encrypt_block(key,p) -> c` encrypts block `p` using the key, and returns the result `c`.
* `decrypt_block(key,c) -> p` decrypts block `c` using the key, and returns the result `p`.
* `increment_counter()`: increments the value counter by one.

State Machine:::

* *States:* _initial_ (0), _set_public_value_ (6), _ace_const_hash_absorb_ (4), _ace_const_hash_finalize_ (5), _encrypt_ (1), and _decrypt_ (2).
* *Allowed State Transitions:*
 ** _initial_ -> _set_public_value_.
 ** _set_public_value_ -> _ace_const_hash_absorb_.
 ** _set_public_value_ -> _encrypt_, if metadata allows encryption, else `error(ace_error_invalid)`.
 ** _set_public_value_ -> _decrypt_, if metadata allows decryption, else `error(ace_error_invalid)`.
 ** _set_public_value_ -> _initial_, _set_public_value_.
 ** _encrypt_, _decrypt_ -> _ace_const_hash_absorb_, _initial_, _set_public_value_.
 ** _hash_absorb_ -> _ace_const_hash_finalize_, _initial_, _set_public_value_.
 ** _hash_finalize_ -> _initial_, _set_public_value_.
 * *Behavior:*
 ** From the _initial_ state, the context immediately transitions to state _set_public_value_.
 ** Once entered the _set_public_value_ state, the ACE unit performs:
    . `hash_key <- encrypt_block(key,0)`, and
    . `auth_tag <- 0`.
  ** In the _set_public_value_ state:
   *** We can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp}{nbsp} to set
       . `counter <- Vs << b/4`
    (there is no harm if this instruction is issued more than once).
   *** If no such {nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp}{nbsp} to set is issued, counter is assumed to be zero.
   *** Other forms of `ace.exe` not allowed, will cause `error(ace_error_invalid)`.
 ** Upon completion of `ace.import`, the ACE unit recomputes the hash_key:
  . `hash_key <- encrypt_block(key,0)`.
// ** In _encrypt_ and _decrypt_ states, we can only issue `ace.exec Vd, Cn|C[Xn], Cs` which:
  ** If the _encrypt_ state, we can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp}{nbsp} to
   *** Encrypt `Vs` into `Vd`, and accumulate ciphertext `Vd` into the auth_tag, namely
    . `Vd  <- Vd xor encrypt_block(key,counter)`,
    . `counter[b/4-1:0] <- counter[b/4-1:0] + 1`.
        If `counter[b/4-1:0]` rolls back to zero:
        .. the internal state will be invalidated (state set to _ace_completed_, all fields zeroed except the key) and
        .. ace.error will return ace_err_invalid.
    . `auth_tag <- auth_tag xor Vd`, and
    . `auth_tag <- auth_tag * hash_key`.
  ** In the _decrypt_  state, we can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp}{nbsp} to
   *** Accumulate ciphertext `Vs` into the auth_tag, then decrypt `Vs` into `Vd`, namely
   . `auth_tag <- auth_tag xor Vs`, and
   . `auth_tag <- auth_tag * hash_key`.
   . `Vd  <- Vd xor encrypt_block(key,counter)`,
   . `counter[b/4-1:0] <- counter[b/4-1:0] + 1`.
       If `counter[b/4-1:0]` rolls back to zero:
       .. the internal state will be invalidated (same as above) and
       .. ace.error will return ace_err_invalid.
 ** In the _hash_absorb_ state, we can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp}{nbsp} to accumulate a value in the auth_tag, namely:
    . `auth_tag <- auth_tag xor Vs` and
    . `auth_tag <- auth_tag * hash_key`.
 ** In the _hash_finalize_ state, we can only issue `ace.exec` form{nbsp}{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp}{nbsp} where `Vs = len_A @ len_C` (two fields of `b/2` bits, to compute the authentication tag and complete the algorithm as follows:
    . `auth_tag <- auth_tag xor Vs`,
    . `auth_tag <- auth_tag * hash_key`, and
    . `auth_tag <- auth_tag xor encrypt_block(key,counter[b-1,b/4] @ 0[b/4-1,0])`.
    . Finally, the auth_tag is returned in `Vd`, and the state is changed to _ace_completed_.
 ** In the _ace_completed_ state, no operation can be performed except `ace.init`, `ace.export`, `ace.import`, `ace.clear`, or `ace.error`.
       Any other operation, if it has an output, it returns zero, and will cause `ace.error` to return ace_err_invalid.
 ** [.red]#To be completed.#
