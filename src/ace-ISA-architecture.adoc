...
The subsequent contain the *Content*, key material and any algorithm-specific data used to initialize the CR.
The format and length of this information are specified in <<ACE-algorithms>> for all architected algorithms.
The PI Content's size is entirely determined by the _Algorithm_ and _KeyType_ fields within the Metadata.
...
Format of Provisioning Input::: {empty}
...
_initial_ (0),
_invalid_ (14), and
_completed_ (15), the ECC algorithms define the following states:
 ** _set_generator_ (1)
 ** _set_scalar_ (2)
 ** _gen_rnd_scalar_ (10)
 ** _point_mul_ (3)
...
`ace.state` to _set_generator_ or _set_publickey_ must use an additional parameter to allow setting compressed points.
