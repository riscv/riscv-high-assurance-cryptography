...
The formats of the II and the SCC are described in <<ACE-init-export-import>> and <<ACE-format-SCC>>, respectively.
...
* If the algorithm in the CR does not encryption and decryption, then `Xs`[12:11] must be 0, else ACE error _ace_err_invalid_value_ is raised
...
Downgrading protection level is not permitted. Attempting to do so raises ACE error _ace_err_invalid_ (<<ACE-error-codes>>).
...
* `Xs`[32:29] can be non-zero only if `m`[32:29] is 0, in which case  `m`[59:35] <- `Xs`[59:35], else ACE error _ace_err_invalid_value_ is raised
...
. `II_content_size(x)` returns the size in bytes of the content section of an II. It only depends on the 11 bits `Key_Type @ Algorithm`.
. `SCC_content_size(x)` returns the size in bytes of the content section of an SCC. It only depends on the 13 bits `Direction_Policy @ Key_Type @ Algorithm`.
 ...
 ** `ace.error` always returns error _ace_err_invalid_CR_ after any operation involving the CR except for `ace.import` and `ace.export`, `ace.clean`, `ace.init`, and `ace.size`, which all work normally, and using `ace.state` to return to state _initial_.
