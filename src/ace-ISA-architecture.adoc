 ...
 ** _operate_ -> _initial_.
* *Behavior:*
 ** In the _initial_ state, the counter field is set to 0 (or to 1 if LFSR).
 ** In _set_public_value_ state, we can only issue an `ace.exec` of the form{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`,{nbsp}{nbsp} which sets the IV value to `Vs`.
  *** If{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`,{nbsp}{nbsp} is repeated, it will just update the IV value again, with no adverse effect.
 ...
 ** In _operate_ state, we can only issue an `ace.exec` of the form `ace.exec Vd, Cn|C[Xn]`, which
  ...
  *** Other forms of `ace.exe` not allowed, will cause `error(ace_error_invalid)`.
  *** If 'ace.stage' is issued to return to _set_public_value_ (or even _initial_), we can reuse the method with a new counter.
...
* *Behavior:*
 ** In the _initial_ state, the IV is set to zero.
 ** If any `ace.exec` operations are issued while in the _initial_ state, these will produce null outputs and ace.error will return ace_err_invalid.
 ...
 ** In _encrypt_ and _decrypt_ states, we can only issue `ace.exec Vd, Cn|C[Xn], Cs` which:
  ...
  *** If 'ace.stage' is issued to return to _initial_ state, we can reuse the CC with a new IV.
 ...
 * *Behavior:*
 ** In the _initial_ state:
  *** If any `ace.exec` operations are issued, these will produce null outputs and ace.error will return ace_err_invalid.
 ...
 ** In the _set_public_value_ state:
  *** We can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp}{nbsp} to set
       . `counter <- Vs << b/4`  (i.e., the original b/4 most significant bits of Vs` are ignored, and the b/4 least significant bits of `counter` are set to zero.)
  *** If{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`,{nbsp}{nbsp} is repeated, it will just update the IV value again.
  *** If no such {nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp}{nbsp} to set is issued, counter remains zero.
  *** Other forms of `ace.exe` not allowed, will cause `error(ace_error_invalid)`.
