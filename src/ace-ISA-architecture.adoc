...
Writes to ACES do not affect the contents of the CCs:  Setting ACES=Off does not destroy the state, nor does setting ACES=Init or Clean clear the contents.
...
==== ace.enable/ace.disable/ace.clearall/ace.clear
...
* If the `immed12` = `0xa11`, then all CCs are cleared and the new state of the ACE unit is Clear. The alias for this instruction is `ace.clearall`.

`ace.enable` must be issued before using any ACE operation, i.e. `ace.config` passing the immediate 1.
--
+

NOTE: The advantage of `ace.clearall` with respect to a loop that clears all CCs is that the entire CF memory is cleared without the need for compaction/management between the separate `ace.clear` operations and this opertation cannot be interrupted, reducing potential wasteful context saves.
