...
These are, currently, `ace.exec` (<<ACE-instruction-exec>>) and `ace.derive` (<<ACE-instruction-derive>>).
...
|  [13:5] |   9  | Algorithm              | Algorithm (such as primitive or mode). 0 means the CC is invalid.
| [15:14] |   2  | SC_Protection          | Level of Side Channel Protection, encoded as per <<ACE-SC-protection-levels>>.
| [17:16] |   2  | Direction_Policy       | Determines whether the CC may be used for encryption (bit 12 set), decryption (bit 13 set), or both (bits 12 and 13 set). Valid only for encryption/decryption primitives or modes, and ignored, for instance, for hash functions.
| [19:18] |   2  | Key_Type               | Key given by explicit value / System Key (index) / Internally generated.
| [23:20] |   4  | State_Number (also Direction) | Encoding the current state of the state machine of the algorithm as a number. The value must be zero in the initialization data. If the algorithm encoded in the CC has at most 16 states (including _initial_, _completed_, and _invalidated_, cf. <<ACE-state-constants>>), then this number is stored in this field, otherwise an algorithm-specific representation is used in the content, and this field only admits the values 0 (initial), 14 (invalidated), 15 (completed) or 1 (any other operational state). The values must conform to <<ACE-state-constants>>.
| [25:24] |   2  | Using_IOBuf            | Zero if using vector registers for the current `ace.exec` or `ace.derive`. 1 or 2 if using `ace.input`/`ace.output` in place of vector registers. Details in <<ACE-usage-input-output>>.
| [29:26] |   4  | UC_Policy_1            | “UC Policy” means “Usage Control Policy”.  Disallow the _usage_ of this CC in User, (Virtual) Supervisor, Hypervisor, or M-mode if bit 26, 27, 28, or 29 is set.
...
This is followed by the _content_, i.e., the key(s) and possibly other information, in a format that is algorithm-specific.
See <<ACE-algorithms>> for the algorithm-specific formats of the content section of the Initialization Input.
...
The metadata section of a SCC has a fixed architectural format (in fact it is the same format as in the Initialization Inputs), while the content format is algorithm-dependent. See <<ACE-algorithms>> for the latter.
...
The fact that the ISE is not active is only encoded in ```*status```.
The encoding for **OFF** in the per-CR fields of `acecrstatus` is suitable though, since it would be the natural one to use in case state must be lazily loaded.
...
Initialization, import, and export operations can be interrupted and resumed if the interrupt does not result in a context switch, as indicated by the value in the `acestart` CSR.  If a context switch occurs, these operations have to be restarted.
...
For example, in AEAD algorithms, setting an IV or nonce _could_ implicitly transition to the state for absorbing associated data.
However, to ensure consistency, ACE requires explicit state transitions even when this incurs additional instructions.
This also helps the design of algorithms in a way that only one type of `ace.exec` instruction is used per distinct state of the algorithm.
...
* Bits [13:0] and [25:19] of the input register `Xs` are ignored.
* Bits [15:14], [17:16] and [63:32] of the input register `Xs` have the same format as the corresponding bits of the metadata.
* Bits [15:14] can be used to turn on or increase the level of the side-channel protection of the algorithm in the CC, if the microarchitecture supports the corresponding implementation. The levels are defined in <<ACE-SC-protection-levels>>. Downgrading side-channel protection is not permitted, and attempting to do so will result in error _ace_err_invalid_ (cf.{nbsp}<<ACE-error-codes>>).
* Bits [17:16] can be used to deactivate encryption or decryption in a CC that allows both. Writing to them cannot be used to deactivate both encryption and decryption simultaneously, nor to activate encryption, resp., decryption in a CC that did not allow it.
...
The above semantics are defined for RV64. For RV32, `s` must be even and register pair `X[s]` and `X[s+1]` is read.
In this case where `X[s]` corresponds to bits [31:0] and `X[s+1]` to bits [63:32] of the metadata.
...
They can be used only if the size in CSR `aceiobuflen` is non-zero and is a valid size (for instance `b` for an algorithm or a multiple thereof),
...
We use field `Using_IOBuf` in the metadata to indicate whether we are using buffer or the registers and to track the instruction sequence.
Field `Using_IOBuf` is normally zero.
It is set to 1 as soon as we use a `ace.input` in place of a Form A or B `ace.exec` or `ace.derive`, or a Form D `ace.exec` in place of a Form C `ace.exec`.
In this case, bit `Using_IOBuf` remains 1 until the end of the second instruction of the replacement sequence, in which case it is set to zero if the sequence is two instructions long, or set to 2 until the end of the third instruction of the replacement sequence if the sequence is three instructions long.
At the end of the sequence, `Using_IOBuf` is reset to 0.
The ordering of these instructions is enforced by the architecture by the state number and by the value of `Using_IOBuf`.
...
Regarding the allowed instructions, these are usually only `ace.exec` and `ace.state`.
In particular, only a single form of `ace.exec` is allowed (cf. <<ACE-instruction-exec>>) and whenever we write “only Form X `ace.exec` instructions are allowed”, we also allow a replacement sequence using the `ace.input`, Form D `ace.exec`, and `ace.output` instructions as described in <<ACE-replacement-instructions>>.
...
The Data Structures of the algorithms implicitly define the functions `SERIALIZE()` and `DESERIALIZE()` used in <<ACE-SCC-export>> and <<ACE-SCC-import>>.
...
`content[]` in the aforementioned pseudocodes, that satisfies the architected definition of the _Serialized Content_ for each algorithm, and viceversa, but of course implementations are free to do so.
`content[]` is a _view_ of (certain fields of the) the content of a CRF as if it were in the SCC plaintext format, such that any change in one (except for the absent fields) is reflected in the other, and converted on the fly by the microarchitecture:
For instance, consecutive bytes of `content[]` may not be adjacent in the internal memory of the CR, and one byte may correspond to multiple bytes.
...
For the operations performed by `ace.exec`, their actions on the basic processing blocks are uninterruptible.
If the operation admits inputs which are a multiple of this basic processing block, then each block is processed uninterruptibly but the instruction can be interrupted between blocks-as permitted by the implementation. In this case `acestart` is used to determine the point of resumption of the instruction.

An implementation may require context-switching software to wait when accessing a CR that is being written to.
--
+
...
NOTE: This ensures not only the compactness of a SCC, but also that the key can be ported across architectures.
...
While ACE can be implemented when the full V extension is present, ACE actually depends on a minimal subset of the RISC-V Vector Extension (RVV), which we call RVV-mini, for correct operation. Required features include:
(((RVV-mini)))
...
[.red]#This section describes a hypothetical mechanism for exporting partially initialized or imported cryptographic contexts.
This is a proof of concept only and not a current part of the ACE specification.#

Suppose we want to save and restore a _partial_ context, i.e. a context whose initialization or import operation has been interrupted for a context switch, preventing immediate return to the original control flow.
In this case, a “Partial” bit in the metadata signals whether any this has happened.
...
An initialization operation can be interrupted at any 8-byte multiple, and resumed from there.
An import operation can be interrupted at a 8-byte multiple, but it will be resumed only at the 48th byte or beyond.
Otherwise, it will be restarted, and no state will be saved at context switch.

NOTE: Resuming could possibly be more expensive than saving no state and restarting.
The third option, to wait and complete the operation, could instead cause delays.
...
A particularly challenging scenario arises when a context switch occurs during an import, where the partially imported context must be exported and later resumed.
...
We concluded that, while privileged code can always replace an SCC in a saved process's context or memory, it cannot violate the confidentiality or integrity of an SCC by abusing the resumption mechanism.
