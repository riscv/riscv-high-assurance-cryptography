...
// For CR-handling instructions such as `ace.init`, `ace.import` and `ace.export`, `acestart` indicates how many bytes of the input or the output  written. (e.g., , `ace.input`), this offset indicates how many bytes of input have already been read and processed.
// For a store-type instruction (e.g., `ace.export`, `ace.output`), it
// The value includes all sections of the input or of the output, i.e., metadata, SIV, and content, and accommodates piecewise execution via traps (cf.{nbsp}<<ACE-trappable-insns-support>>).
