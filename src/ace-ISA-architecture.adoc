...
Only the Algorithm and Key_Type fields of the metadata section of a CC are considered.
+
...
When defining an algorithm, it must be ensured the structure of the content section for the Initialization Input and its size
as well as The serialized content's structure and size can be entirely determined by the Algorithm and Key_Type fields within the metadata, and no other field.

Regarding the allowed instructions, these are usually only `ace.exec` and `ace.state`.
In particular, only a single form of `ace.exec` may be allowed (cf.{nbsp}<<ACE-instruction-exec>>), together with a suitable replacement sequence
as described in <<ACE-replacement-instructions>>.
