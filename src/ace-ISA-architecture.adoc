...
=== Annex: Interrupting Long Instructions

Initialization, import, and export operations can be interrupted and resumed if the interrupt does not result in a context switch.
The presence of this situation is indicated by a non-zero value in the `acestart` CSR.
Otherwise, these operations operations have to be restarted.

A different situation occurs if an `ace.exec` instruction is operating with long vectors that contains several elements to be processed.
In the algorithm description we explicitly mention that a block of operation is repeated for each block in the vector:
the execution of these iterative `ace.exec` instructions can be interrupted and resumed, and `acestart` reflects the number of bytes processed so far.

[[ACE-incomplete-contexts]]
==== Partial Cryptographic Contests

These may have to be restarted or resumed.
A "Partial" bit signals whether any configuration operation on this cryptographic register has been initiated but not yet completed.
...
//
[.red]#We seem to agree that we can resume init/import after a page fault or a similar thing, but on context switch we do not export partial CRs.
These are cleared and the instruction is restarted.#
