...
The reason is that operations that modify the internal state may be performed on a CR with an interrupted export operation.
...
If an export operation resumes with the flag cleared, it is restarted from the beginning instead.
