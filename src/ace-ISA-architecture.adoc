...
|   i  | `ME_size`       | Metadata Extension
|  ii  | `k`/8 or 8      | Key or System Key index
|===
--
+
...
|  Pos. | Size in Bytes   | Field
|    i  | `ME_size`      | Metadata Extension
|   ii  | `k`/8 or 8      | `key1` or System Key index
|  iii  | `k`/8 or 0      | `key2` or System Key index for `key1` given
|===
--
+
...
If block and tweak sizes are different, a reconfiguration of the vector length may be required between operations, unkless the tweak is shorter than the block size, in which case the instruction setting the tweak will only consider the necesssary bits.
...
|   i   | `ME_size`       | Metadata Extension
|  ii   | `k`/8 or 8      | `key` or System Key index
| iii   | 3``b``/32       | `IV`, i.e., bits [b-1:b/4] of the counter
|===
--
+
 ...
 ** To transition to State _hash_absorb_:
 the ``ace.state`` instruction must be of Forma A, i.e., _without any additional inputs_, since the IV is already included in the PI.
...
| Pos. | Size in Bytes            | Field
|  i   | `ME_size`                | Metadata Extension
|  ii  | If keyed: `k`/8 or 8     | `key` or System Key index
|===
--
+
...
|   ii  | If keyed: `k`/8 or 8     | `key` or System Key index   | --
|  iii  | `n`/8                    | `state`                     | --
|   iv  | `2`                      | `last_block_len`            | Only if this Hash/MAC has special internal treatment for a fractional last block that goes beyond padding and appending the length, which are handled by user software. Otherwise, this is zero.
|    v  | `2`                      | `block_base`                | Amount of data stored so far into `block`.  Only if used by the algorithm, else zero padding.
|   vi  | `4`                      | `cumul_len`                 | Cumulative length of all data absorbed so far. Only if the algorithm must process and incorporate it in the hash on its own, else zero padding.
|  vii  | `b`/8                    | `block`                     | It is included it the SCC only if there is a partial block, and the block must be stored separately from the `state`, and exclude it when not.
...
| Pos. | Size in Bytes   | Field
|  i   | `ME_size`       | Metadata Extension
| ii   | 16 or 8         | `key` or System Key index
|===
--
+
...
| Pos. | Size in Bytes   | Field
|   i  | `ME_size`      | Metadata Extension
|  ii  | 16 or 8         | `K1` or System Key index
| iii  | 16 or 0         | `K2` or System Key index in `K1`
|===
--
+
...
|    v  | 8               | `state`[3]
|   vi  | 8               | `state`[4]
|  vii  | 1               | `countdown` (from 3 down to 0)
| viii  | 7               | Padding
|===
--
+
 ...
 ** If signature generation or verification is allowed:
  ...
  *** `RandomNumber`: a `b`-bit string representing the random number.
...
|  Pos. | Size in Bytes   | Field
|  i    | `ME_size`       | Metadata Extension
|  ii   | 2               |  Bit 0: `HasGenerator` +
Bit 1: `HasScalar` +
Bit 2: `HasPublicKey` +
Bit 3: `HasHash` +
Bit 4: `HasSignature` +
Bit 5: `HasRandomNumber` +
Bits 6-15: Padding (zero).
|  iii  | 2               | `Progress`/`input_base`
|  iv   | 2               | `block_base`
|  v    | 2               | Padding
|  vi   | 2``b``/8        | `Generator`, X coordinate
|  vii  | 2``b``/8        | `Generator`, Y coordinate
|  viii | `b`/8           | `Scalar`: `b`-bit string. Can be the private key.
|===

The `SCC_content_size()` function returns only the length of the content in positions 1 to 8.
...
|===
|  Pos. | Size in Bytes                             | Field
|   ix  | If `mh`._ExtraContent_ != 0: 8, else: 0   |  `acemarchid`
|   x   | If `HasPublicKey`: `b`/8, else: 0         | `PublicKey`, X coordinate.
|   xi  | If `HasPublicKey`: `b`/8, else: 0         | `PublicKey`, Y coordinate.
|   xii | If `HasSignature`: `b`/8, else: 0         | `Signature`, R value.
|  xiii | If `HasSignature`: `b`/8, else: 0         | `Signature`, S value.
|  xiv  | If `HasRandomNumber`: `b`/8, else: 0      | `RandomNumber`.
|  xv   | If `HasHash`: `h`/8, else: 0              | `Hash`: a `h`-bit string representing the hash.
|  xvi  | (variable)                                | Additional data from the *Internal State*.  this includes a `RandomNumber`, a `b`-bit string, erased upon completion.
|===

The `HasRandomNumber` flag is set and the `RandomNumber` field is used only during signature generation.
`RandomNumber` is exported only if the signature operation is interrupted, otherwise it is upon completion of the signature generation.

The size in bytes of Fields 9 to 15 is:

`{fournbsp}{fournbsp}` varlen <- 8 + (`HasPublicKey` != 0) {interpunct} 2``b``/8 + (`HasSignature` != 0) {interpunct} 2``b``/8 + +
`{fournbsp}{fournbsp}{fournbsp}{fournbsp}{fournbsp}{fournbsp}{fournbsp}` + (`HasHash` != 0) {interpunct} ``h``/8 + (`HasRandomNumber` != 0) {interpunct} ``b``/8

If _ExtraContent_ != `varlen`, then the Additional Data is placed at Position 16. For this algorithm, the microarchitectural dependent part is this Additional Data.
