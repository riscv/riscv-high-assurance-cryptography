...
// `{fournbsp}{fournbsp}x` <- `m`._FilterMode_ @ `m`._KeyType_ @ `m`._DirectionPolicy_ @ `m`._Algorithm_ @ `m`._Format_.
The implementation provides the following functions and constants or an equivalent mechanism:

. `MH_size(m)` returns the byte length of the metadata header. In the current specification, it is always 8.
...
.  `ME_size(m)` returns the byte length of the metadata extension.
    In the specification, it is 0 when `m`._FilterMode_ is 0, and it is 16 otherwise.

The total size in bytes of a PI is therefore

`{fournbsp}{fournbsp} MH_size(m)` + `ME_size(m)` + `PI_content_size(m)`,

and the total size in bytes of an SCC is

`{fournbsp}{fournbsp} MH_size(m)` + 16 + `ME_size(m)` + `SCC_content_size(m)` + _ExtraContent_ * 8.
...
* The first block read from memory by the operation is the metadata header block `mh`:
...
* If an import operation is preempted, the hardware must save `mh.StateNumber` and set +
`mh.StateNumber <- ace_state_invalid` before relinquishing control.
** When control is returned, the previous value of `mh.StateNumber` is restored.
** The implementation must keep be a 4-bit internal temporary state containing the original value of `mh.StateNumber` and a flag indicating whether the value has been saved.
