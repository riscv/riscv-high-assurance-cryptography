...
// Any other operation that may change the state of the CR
// will set it to false again, to force a resumption of a restart
CR.exporting = true;

// Note: CR.content[] here includes the optional metadata extension
...
// where content is extended as described above

M : array[MH_size(mh) + 16 + z] of bytes;
let last_block_fractional : Boolean ← z % 16 != 0;
let num_blocks : int ← ceil(z / 16);

if (m.Locality == 15) then {
    {enc_key, auth_key} = KeyDeriv(CSK, zeros(125));
} else
    {enc_key, auth_key} = KeyDeriv(CSK, LST[mh.Locality]);
}

tmp ← POLYVAL(auth_key, zeros(64) @ mh @ CR.content[]);
 ...
 ** If 4 bytes or more have been read, the value of `m.StateNumber` in the SCC is known. In this case, the instruction may be restarted with `acestart` {ge} 4 recording the amount of data read.

* If an import operation is preempted, the hardware must save `m.StateNumber` and set +
`m.StateNumber <- ace_state_invalid` before relinquishing control.
** When control is returned, the previous value of `m.StateNumber` is restored.
** The implementation must keep be a 4-bit internal temporary state containing the original value of `m.StateNumber` and a flag indicating whether the value has been saved.
...
CR.importing : bool;        // this is a "hidden" per-CR ACE unit state
enc_key : bits(256);
auth_key : bits(128);
SIV : bits(128);
tmp : bits(128);
...
// During import, the original StateNumber is saved and temporarily
// replaced in the CR with ace_state_invalid to prevent other operations.

saved_SN : bits(4) ← tmp[63:0].StateNumber;
...
tmp ← AESE(enc_key, 0 @ tmp[126:0]); // tag
if (tmp != SIV) then { aceerror ← ace_err_import_auth; return }
