...
We use CSR `acesequence` to indicate whether we are using buffer or the registers and to track the instruction sequence.
...
The ordering of these instructions is enforced by the architecture according to the value of `acesequence` and the state number of the CR configured in `meta`.
