...
However, any read or write to `content` must reversibly map to the underlying representation. A write followed by a read must return the original data, and a read followed by a write must restore an equivalent microarchitectural state.
