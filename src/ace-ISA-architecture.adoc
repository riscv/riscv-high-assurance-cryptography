...
| _BootSecret_              | Unique per the boot session of the host hardware and its firmware stack. Configured by M-mode during the boot process of the physical device (see <<ACE-CSR-maceBootSecret>>). | Device | M | 7
| _MSecret_                 | Can be configured by M-mode (see <<ACE-CSR-saceSSecret>>).                                             | Process  | S | 9
...
* Invalid metadata section in `ace.provision` or `ace.import`, other than _ace_err_unimplemented_, for instance:
...
* Form field = `11`, `r` bit = 0: ACEIOBUF.
...
* Internal Capacity `ace.size` returns the amount of internal CRF capacity used by a CR with metadata section matching `Xn` on the Algorithm, SCProtection, and KeyType metadata fields. In this case, all other bits must be zero.
If the combination of Algorithm, SCProtection, and KeyType fields is not supported, `ace.size` returns 0.
...
* `Xs`._DirectionPolicy_ is used to set the new DirectionPolicy of the CR.
...
* If `Xs`._Locality_+_ is non-zero, then it can be used to restrict the Locality. Then, when the CR is exported as a CR, it will use the new tweak in the AEAD mode. Generalizing the Locality is not permitted. Attempting to do so raises ACE error _ace_err_invalid_value_.
...
* A zero bit in `Xs`._FilterMode_ means that the corresponding policy set in `m`._FilterMode_ is _not_ changed. +
A set bit in `Xs`._FilterMode_ will add the corresponding policy to `m`._FilterMode_ only if none was programmed before, otherwise error ACE error _ace_err_invalid_value_ is raised. If `Xs`._FilterMode_ is non-zero, then `X[s2+1] @ Xs2` must be provided and it must have the same format as the metadata filtering extension in <<ACE-metadata-extension>>, where only the fields corresponding to the actual filters added are considered and the remaining bits are ignored.
* When _FilterMode_ is set in a CC where it was previously unset, the SCC will contain the additional metadata filtering extension, where previously it would not have been included. Thus, the size of the SCC may be larger than a previous version.
--
+
On RV32:
...
function KeyDeriv(key : bits(256), L : bits(126)) = {
  precomputed: t = AESE256(key, zeros(128)) * 2; // using the field multiplication
  struct {
 ...
 * An “`ace.close`” instruction to suspend or finalize the procedure.
   If executed before all bytes are transferred, the CR state matches that of an interrupted monolithic instruction, including any special value of the _StateNumber_ metadata field and the value of `acestart`. If executed after all bytes are transferred, it also performs any final cryptographic operations, such as authenticating the CR, if needed.
...
. The code is written without considering the exceptions to be caught, usually only _ace_err_invalid_CR_ or _ace_err_expired_CR_.
. If the code runs without such errors, then the two CSRs are reset and the routine returns normally.
. If, say, an _ace_err_invalid_CR_ or _ace_err_expired_CR_ error is raised, the horizontal trap diverts the control flow to the catch code, which usually cleans up, and returns to the caller of the subroutine, not to the routine itself, with an error code.
...
the value against which the internally computed tag must be compared:
   ...
   else an ACE error _ace_err_auth_ is raised.
...
--
  ...
  . else (tag length is `3b/4` bits)
    ...
    If `tag` and `SIV` match, `aceerror` is set to _ace_err_ok_, else ACE error _ace_err_auth_ is raised.
  . The state transitions to _completed_.
--
+
If `IOLEN` > `b`, only the `b` least significant bits of `INPUT` are considered.
...
--
  . If the values match, then `aceerror` is set to _ace_err_ok_,
  . else `aceerror` is set to _ace_err_auth_.
  . The state transitions to _completed_.
--
+
...
. If the values match, then `aceerror` is set to _ace_err_ok_,
  else ACE error _ace_err_auth_ is raised. The state is changed to _completed_.
--
+
...
Localities programmable at M/H/S could be used in this case, however, we note that in order to allow proper context switching, they need to be readable as well by higher-execution environments, which opens up an attack surface. This said, also `*filter`'s can be written by higher-execution environments, and these are thus also manipulable by the latter—and are shorter.
