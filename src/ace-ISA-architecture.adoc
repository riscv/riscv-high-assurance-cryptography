...
NOTE: We introduce the concept of _Usage Control_, distinct from Access Control, to underline the fact that restricted CCs and SCCs can still be exported and reimported by otherwise unauthorized environments-to the extent that the architecture supports it. Only the _usage_ in cryptographic operations is restricted.
...
NOTE: The term "Locality" is borrowed from TPMs. In TPMs, localities are used to restrict object usage by binding it to the source of a command (e.g., application layer or driver layer), each represented by an integer. However, TPM localities are simply hints provided and enforced by the software communicating with the TPM, and are therefore not trusted inputs to the TPM itself; their semantics are platform-specific. In ACE, by contrast, localities are enforced within the ACE implementation itself. While software exploits (e.g., against MM or TEE applets) or hardware tampering with the Locality Secrets Table (LST) could potentially circumvent this mechanism, doing so is significantly more difficult than in the TPM case.


[comment]
--
The term "Locality" is borrowed from TPMs. In TPMs, localities restrict object authorization by indicating the command source (e.g., application or driver layer), each represented by an integer. However, TPM localities are hints enforced by the software that talks to teh TPM, and are not trusted inputs to the latter. In ACE, localities are enforced by hardware. Although software exploits (e.g., against the MM or TEE applets) or hardware tampering with the Locality Secrets Table (LST) could circumvent the feature, defeating this hardware-based mechanism is significantly more difficult than in the TPM case.

We borrow the term "Locality" from TPMs.
On TPMs localities are used to restrict TPM object authorization. A locality indicates the source of the command, for example it could be from the application layer or the driver layer, each would have it's own locality which is represented by an integer.
However, for TPMs localities are hints to the TPM and are enforced by the software communicating to the TPM.
Therefore, they are not trusted inputs on their own and are implemented in platform specific ways.
Localities in ACE, on the other hand, are enforced by the hardware layer, and while software exploit of the MM or of, say, TEE applets, or hardware tampering with the LST entries could lead to the suppression or circumvention of the feature, it is still a much harder mechanism to defeat than in the TPM case.
--
...
| `ace.input`     | `Cd\|C[Xd], %offset(Xs)`         |  Optional. Provides an input to the algorithm configured in the CC `Cd` or `CC[Xd]`, in place of providing it through the `ace.exec` instruction. Neede only when the inputs are very long, such as the inputs to certain public key schemes.
| `ace.output`    | `%offset(Xd), Cs\|C[Xs]`         |  Optional. Writes the output of an algorithm configured in the CC `Cs` or `CC[Xs]`, in place of providing it through the `ace.exec` instruction. Useful when the outputs are very long, such as the outputs of certain public key schemes.
| `ace.wait`      |                                  |  Waits until all operations in the ACE unit which have been issued by the current hart have completed.
...
While the MM can write to the `acerctk` block, the CSRs in it will read as zero.
The `acerctk` block is not accessible in any other mode. Accessing it will cause an illegal instruction exception.
...
// The first two Locality Secrets are chip-wide, and should be the same across all
// chiplets in the same SoC -- a separate chip in the same system may have different values.
// Three further Locality Secrets must be the same across the entire device, and the
// Locality Secrets at indexes 5 and 6 can be reprogrammed by MM.
...
// Without this comment, the next part is not properly formatted...
Encoding::
The encoding uses `ace.exec` Form B', with bit 8 (which is in the `rd` field) set.
+
[wavedrom, , svg]
....
{reg:[
...
Description::
Wait until all current operations in the ACE unit which are issued by the current hart complete.
This is necessary if we need to read the correct error status.
...
This is implemented as an `ace.wait` instruction
(cf. <<ACE-instruction-wait>>) followed by a read from CSR `aceerror`.
...
Example pseudocode is given in <<ACE-pseudocode-keystream-modes>>.
...
+
NOTE: There is no harm in moving to state _set_public_value_2_ and
setting the SIV while encrypting, since that field is not used in the encryption path until it is set (thus overwriting any previous content) by the state machine itself.
 ...
 ** In state _hash_finalize_dec_,
    ...
    `Vs = IntTo{b/2}Bits(bit_length(AD)) @ IntTo{b/2}Bits(bit_length(plaintext))`. +
//    The semantics are almost identical to issuing a Form A ``ace.exec`` in state _hash_finalize_enc_ except that the SIV/tag value is _not_ output, but is only kept in `auth_tag`.
...
We show in <<ACE-pseudocode-GCM-SIV-encryption>> how to perform AEAD encryption with a GCM-SIV CC.
...
Let `C` be the non-leading coefficients of the lexicographically first irreducible binary polynomial of degree `b` with the minimal number of ones: `0x1B` for `b` = 64, `0x87` for `b` = 128, and `0x425` for `b` = 256. Then
