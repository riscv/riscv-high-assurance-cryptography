...
--
* `b` = block size in bits (8|`b`). For this algorithm we require `VLEN` to be an integer multiple of `b`.
* `k` = key size in bits (8|`k`). If `k` <> `b`, the cipher must support a key size of `b` bits as well.
--
+
Also, define `g` = `log_2(b)` and `u` = `b - g - 1`.
...
[lowerroman]
 . `key`: `k/8` bytes, or 8 bytes if a System Key index.
 . `N`: `b/8` bytes, but only `u` bits defined. The top `b-u` bits are set to zero.
 . `L`: `b/8` bytes.
 . `offset`: `b/8` bytes.
 . `hash_A`: `b/8` bytes.
 . `hash_P`: `b/8` bytes.
 . `taglen`: 4 bytes
 . `final_blk_len`: 4 bytes

NOTE: `ktop`: is not serialized, and thus it is recomputed upon `ace.import`.
...
** In state _set_public_value_, only  ``ace.exec`` instructions of Form B, "```ace.exec Kn|K[Xn],Vs```", is allowed.
   whereby `N` is set to `Vs`.
   *** If "```ace.exec Kn|K[Xn],Vs```," is repeated, it will just re-write `N` and repeat the above operations.
   *** If no such `ace.exec` is issued, `N` remains zero, and the above operations are performed
...
** Upon entering state _hash_absorb_,  following operations are performed:
  ...
  . `offset <- zeros(128)`
  . `L <- enc_blk(key, zeros(128))   `  // L_* in RFC
  . `L <- double(L)                  `  // L_$ in RFC
  . `L <- double(L)                  `  // L_0 in RFC
...
** To move to _hash_finalize_, the `ace.state` operation must have the additional argument, which sets the length of the final block of the AD in `final_blk_len`.

** In state _hash_finalize_, only one `ace.exec` instructions of Form B is allowed if `final_blk_len` <> 0.
 "```ace.exec Kn|K[Xn],Vs```" which absorbs the last data block in `auth_tag`, i.e.:
+
--
  . `offset <- offset xor enc_blk(key, zeros(128))`  // L_* in RFC
  . `tmp <- (Vs[final_blk_len-1:0] || 1 || zeros((b-1)-final_blk_len) xor offset`
  . `hash_A <- hash_A xor enc_blk(K, Vs)`
--
+
If `final_blk_len` = 0, however, no `ace.exec` can be executed, and only an `ace.state` instruction is allowed to enter state _encrypt_ or _decrypt_.
...
** Upon entering states `_encrypt_` or `_decrypt_`, following operation are performed:
 ...
 . `L <- double(L)                  `  // L_$ in RFC
 . `L <- double(L)                  `  // L_0 in RFC
 . `Nonce <- num2str(taglen mod 128,7) || zeros(120-bitlen(N)) || 1 || N` [.red]#correct this#
 . `bottom <- str2num(Nonce[123..128])` [.red]#correct this#
 . `Ktop <- enc_blk(K, Nonce[1..122] || zeros(6))` [.red]#correct this#
 . `Stretch <- Ktop || (Ktop[1..64] xor Ktop[9..72])` [.red]#correct this#
 . `offset <- Stretch[1+bottom..128+bottom]` [.red]#correct this#
...
** To move to _last_block_enc_, the `ace.state` operation must have the additional argument, which sets the bit length of the final block of the plaintext in `final_blk_len`.

** In state _last_block_enc_, only an ``ace.exec`` instruction of Form A "```ace.exec Vd,Kn|K[Xn],Vs```," is allowed if `final_blk_len` is nonzero.
In this case, following operations are performed:
+
--
  . `aux <- enc_blk(key, zeros(128))`
  . `offset <- offset xor aux`                                             // L_* in RFC
  . `tmp : bits(final_blk_len) <- enc_blk(K, offset)[final_blk_len-1:0]` // pad
  . `tmp <- Vs[final_blk_len-1:0] xor tmp`                                 // C_*
  . `Vd <- tmp || zeros(b-final_blk_len)`
  . `tmp2 : bits(b) <- (tmp[final_blk_len-1:0] || 1 || zeros((b-1)-final_blk_len) xor offset`
  . `aux <- double(aux)`                                                   // L_$
  . `hash_B <- enc_blk(K, hash_B xor offset xor aux) xor hash_A`
--
+
On the other hand, if `final_blk_len` is zero, only an ``ace.exec`` instruction of Form D, "```ace.exec Kn|K[Xn]```," is allowed, which performs:
+
--
  . `aux <- enc_blk(key, zeros(128))`
  . `aux <- double(aux)`                                                   // L_$
...
only an ``ace.exec`` instruction of Form C, "```ace.exec Vd,Kn|K[Xn]```," is allowed, which puts `hash_B[taglen-1:0]` into `Vd`, zero padded.
...
--
  ...
  . `hash_B <- hash_B xor tmp`
  . `Vd <- tmp`
--
+
If `VLEN` is a multiple of `b`, the same behavior applies as in state _encrypt_.

** To move to _last_block_dec_, the `ace.state` operation must have the additional argument, which sets the bit length of the final block of the ciphertext in `final_blk_len`.

** In state _last_block_dec_, only an ``ace.exec`` instruction of Form A "```ace.exec Vd,Kn|K[Xn],Vs```," is allowed if `final_blk_len` is nonzero.
In this case, following operations are performed:
+
--
  . `aux <- enc_blk(key, zeros(128))`
  ...
  . `tmp <- Vs[final_blk_len-1:0] xor tmp`                                 // C_*
  . `Vd <- tmp || zeros(b-final_blk_len)`
  . `tmp2 : bits(b) <- (Vs[final_blk_len-1:0] || 1 || zeros((b-1)-final_blk_len) xor offset`
  . `aux <- double(aux)`                                                   // L_$
  . `hash_B <- enc_blk(K, hash_B xor offset xor aux) xor hash_A`
--
+
...
--
  ...
  . `hash_B <- enc_blk(K, hash_B xor offset xor aux) xor hash_A`
--
Then the state is changed to _hash_verify_.
+

** In state _hash_verify_, only an ``ace.exec`` instruction of Form B, i.e., "```ace.exec Kn|K[Xn],Vs```" is allowed.
    It compares `Vs` with `hash_B`:
+
--
  . If the values match, then the state is changed to _completed_, and `ace.error` returns no error,
  . else the state is changed to _completed_, and `ace.error` returns `ace_err_authentication`.
