...
* *States:* _initial_, _set_public_value_, _hash_absorb_, _encrypt_, _enc_hash_finalize_, _decrypt_, _dec_hash_finalize_, and _completed_.
 ...
 ** _hash_absorb_ -> _decrypt_ -- if metadata allows decryption, else the CR is *invalidated*.
 ** _encrypt_ -> _enc_hash_finalize_, later automatically transitioning to _completed_.
 ** _decrypt_ -> _dec_hash_finalize_, later automatically transitioning to _completed_.
...
** In state _enc_hash_finalize_, only one ``ace.exec`` instruction of Form C is allowed.
“```ace.exec Vd, Kn|K[Xn]```”  performs
+
--
...
. `state[3] <- state[3] xor key[63:0]`
. `state[4] <- state[4] xor key[127:64]`
. `Vs[127:0]` is compared to `state[3] @ state[4]` +
If the values match, then the state is changed to _completed_, and `ace.error` returns no error,
else the state is changed to _completed_, and `ace.error` returns `ace_err_authentication`.
--
+
