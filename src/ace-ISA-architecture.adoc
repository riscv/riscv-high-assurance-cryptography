...
|  2   | _ace_err_unconfigured_       | Attempted to use use an unconfigured, or partially configured CR as a source in any instruction except `ace.size`. +
Attempted to use ACEIOBUF as an input or an output when it was unconfigured. +
Attempted to use an out of range CR with indirect addressing.
...
|  6   | _ace_err_unimplemented_      | Not implemented algorithm or variant. State number not allowed in current algorithm in `ace.state`
