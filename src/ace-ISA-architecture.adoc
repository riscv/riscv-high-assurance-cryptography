...
* In place of load and store instructions, CRs can be *initialized* with clear data, but only *exported* using an AEAD algorithm, for later re-**import**.
  These actions correspond to three instructions:

  . `ace.init` (cf.{nbsp}<<ACE-instruction-init>>) is a load-type instruction that reads clear text into a CR, but there is no corresponding “store” instruction. In other words: _we can write clear data into a CR but not retrieve the clear data as such_.
  . `ace.export` (cf.{nbsp}<<ACE-instruction-export>>) exports a CR using an AEAD algorithm. The size of the exported data may be different from the data read by `ace.init` and the internal state of the CR.
  . `ace.import` (cf.{nbsp}<<ACE-instruction-import>>) may fail if the authentication tag cannot be verified.
...
Insufficient residual CRF capacity may prevent the execution of an instruction that configures a CR, such as `ace.init`, `ace.import`,
 `ace.clone`, (cf.{nbsp}<<ACE-instruction-clone>>) or `ace.derive` (cf.{nbsp}<<ACE-instruction-derive>>).
In this case, ACE returns _ace_err_memory_, and software must free capacity by unconfiguring one or more CRs.

Accessing a CR that is outside the range [.red]#[0..31] (currently)# raises a invalid instruction error.
...
`acestart` is an XLEN-bit RW CSR, similar to `vstart`, tracking progress of long-running interruptible instructions such as `ace.init`, `ace.import`, `ace.export`, `ace.input`, and `ace.output`, as well as `ace.exec` on long vectors.
(((CSR, acestart)))
...
{reg:[
    ...
    { bits:  3, name: 'ace.export', attr: [0x4,0x5] },
    { bits:  5, name: 'Xd' },
...
....
+
Bit 25 is 0 for `ace.exec` operations. If bit 25 is 1, then other operations are encoded. +
...
// Without this comment, the next part is not properly formatted...
Encoding::
The encoding shares `func3` with `ace.exec`, however with bit 25 (the lsb of `func7`) equal to 1 and `func2` equal to 1.
...
Encoding::
The encoding shares `func3` with `ace.exec`, however with bit 25 (the lsb of `func7`) equal to 1 and `func2` equal to 2.
...
Encoding::
The encoding shares `func3` with `ace.exec`, however with bit 25 (the lsb of `func7`) equal to 1 and `func2` equal to 3.
+
[wavedrom, , svg]
....
{reg:[
...
....
{reg:[
    { bits:  7, name: 0x7b, attr: ['custom-2'] },
    { bits:  5, name: 'immed[4:0]' },
...
{reg:[
    { bits:  7, name: 0x5b, attr: ['custom-2'] },
    { bits:  5, name: 'Xd' },
    { bits:  3, name: 0x5, attr: ['ace.input'] },
    { bits:  5, name: 'Xl' },
    { bits: 12, name: 'immed' },
]}
....
...
The Serialized Content section under Data Structures in an algorithm specification (cf.{nbsp}<<ACE-algorithms>>) defines the structure of the `content[]` array.
...
Adding a dedicated mode to the ACE unit is a minor effort, and it allows internal logic to optimize execution beyond the capabilities of, for instance, a compiler on an in-order CPU. It also means that the CPU does not have to implement the carry-less multiplication (cf.{nbsp}<<ACE-V-dependency>>), and only a shared ACE unit has to,
