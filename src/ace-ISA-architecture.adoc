...
* *States:* _initial_, _set_public_value_, _set_public_value_2_, _hash_absorb_,
_encrypt_, _last_block_enc_, _hash_finalize_enc_,
_decrypt_, _last_block_dec_, _hash_finalize_dec_,_hash_verify_, and _completed_.
...
--
+
If `VLEN` is a multiple of `b`, the above are executed `VLEN/b` times, encrypting each `b`-bit block of `Vs` into the corresponding `b`-bit block of `Vd`.
...
--
  ...
  . `tmp <- enc_blk(K, offset)     ` // pad
  . `tmp <- Vs xor tmp             ` // C_*
  . `Vd <- tmp[final_blk_len-1:0] || zeros(b-final_blk_len)`
  . `tmp <- (tmp[final_blk_len-1:0] || 1 || zeros((b-1)-final_blk_len) xor offset`
  . `aux <- double(aux)            ` // L_$
