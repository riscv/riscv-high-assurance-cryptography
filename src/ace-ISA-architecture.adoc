...
[.darkred]#We can use the data structure pointed by `mconfigptr` in order to track optional features of ACE including whether the `acerctk` CSR block is implemented, and if not whether whether the value is ephemeral or fixed.# +
 +
...
|   [1:1] |   1  | Is_Placeholder         | 0 for a normal configuration input and SCC.
...
|   [3:2] |   2  | Mode_Usage             | Minimal mode that can use an `ace.exec` on this CC. 00 for U, 01 for S, 10 for HS, 11 for M.
|  [63:4] |  60  | Reference              | This is a reference to the actual SCC, in a format that is specific to the parent environment.
                                            It can be a pointer to the actual SCC, with the four least significant bits removed.
...
In a placeholder SCCm the content is empty.
...
.Generation of the SCC from a non-Placeholder CC Using AES-GCM-SIV (RFC8452)
[source,sail]
----
metadata : bits(64);
last_block_fractional : boolean;
...
The generation of the SCC from a Placeholder CC uses the same algorithm,
but obviously only M[0] and M[1] are generated, and the Reference field is encrypted.
Furthermore, Localities do not apply to Placeholder CCs, and therefore the used CTK is always the current RCTK.

.Generation of the SCC from a Placeholder CC Using AES-GCM-SIV (RFC8452)
[source,sail]
----
metadata : bits(64);
last_block_fractional : boolean;
...
{Kenc, Kauth} = RFC8452_KeyDeriv(RCTK);
let T : bits(96) = POLYVAL(Kauth, metadata @ IntTo64Bits(64)
                                           @ IntTo64Bits(len_in_bits(content)))[95:0];
let SIV : bits(96) = AESE(Kenc, 0 1b0 @ ((T xor N)[126:0]))[127:32];

M[0][127:0] = N[63:0]
              @ ( metadata[63:4] xor AESE(Kenc, 1'b1 @ SIV[94:0] @ 31'b1 @ 1'b0 )
              @ metadata[3:0];
M[1][127:0] = SIV[95:0] @ N[95:64];
----
