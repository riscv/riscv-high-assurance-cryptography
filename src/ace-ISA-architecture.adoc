...
[WARNING]
In this section we assume that there are two status bits per CR, i.e., we can have two additional status values beside Clean and Dirty.
...
Since we have now four possible statuses per CR, we define an additional one and call it *LAZY*.
...
There is however one complex case, which occurs when:
  ...
  . If the CR was *CLEAN*, an SCC in VS-mode would be authoritative, and if there is no SCC in VS-mode, the value in HS is authoritative, so retrieving the value from the parent first, and grandparent otherwise, works well.
...
+
This is easily solved by using the fourth possible CR status, which we call *LAZY/DIRTY*—this is set by the HV in the expanded `vscrdirty` CSR when it restores the CR before returning control directly to U.
