...
|   1   | ace_state_hash_absorb +
ace_state_operate
| Start (and initialize) a hash function or the sub-function of an algorithm, preparing it to absorb blocks of data. +
...
|   8   | ace_state_encrypt             | Encryption is requested.
 ...
 ** Upon issuing “```ace.state Kn|K[Xn], #ace_state_last_block, Xs```” where `Xs` contains the bit length of the last block, the following happens:
   . if `partial_block_len` != 0, then the CR is *invalidated*. (The previous block is not complete.)
...
* `t` = tag len (8|`t`). This is the size of the digest.
 ...
 ** `partial_block_len`: int.  Length of absorbed data so far for the current block (field size at most 16 bits). Only if used by the algorithm.
 ** `block`: `b` bits.
...
[lowerroman]
 ...
 . `partial_block_len`: 16 bits. Length of filled data so far into `block`.  Only if used by the algorithm, else zero padding.
 . padding: 16 bits.
 . `block`: `b` bits.
...
* `finalize() -> result`: process the `state` into the digest value.
...
* *Behavior:*
 ** In state _initial_:
  *** `last_block_len`, `partial_block_len` and `block` are set to zero.
...
[]
  . `input_remaining <- VL*SEW`
  . `input_base <- 0`
  . while `(input_remaining > 0)` do {
  .. if `(input_remaining ≤ b-partial_block_len)` then {
  ... ``block[partial_block_len + input_remaining - 1 : partial_block_len] <- ``{empty} +
``{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}<- Vs[input_base + input_remaining - 1 : input_base]``
  ... `input_base <- input_base + input_remaining`
  ... `partial_block_len <- 0`
  ... `partial_block_len <- partial_block_len + input_remaining`
  .. } else {
  ... ``block[b - 1 : partial_block_len] <- ``{empty} +
``{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}<- Vs[input_base + b - 1 - partial_block_len : input_base]``
  ... `input_base <- input_base + (b - partial_block_len)`
  ... `input_remaining <- input_remaining - (b - partial_block_len)`
  ... `partial_block_len <- b`
  .. }
  .. if (`partial_block_len` == `b`) then {
  ... `absorb()`
  ... `partial_block_len <- 0`
  .. }
  . }
...
//
** Upon issuing “`ace.state Kn|K[Xn], #ace_state_hash_finalize, Xs`” where `Xs` contains the length of the last block, the following happens:
+
--
 . if `partial_block_len` != 0, then the CR is *invalidated*. (The previous block is not complete.)
 . if `Xs > b`, then the CR is *invalidated*.
...
--
+
Then, state transitions to _hash_finalize_ as expected.

 ** In state _hash_finalize_, we can have the following four situations:
 *** `partial_block_len` < `last_block_len` and `partial_block_len` != -1;
  we can only issue ``ace.exec`` instructions of Form B, i.e., “```ace.exec Kn|K[Xn], Vs```”, to absorb a single data block `Vs` into the hash.
  The operations are mostly the same as the operations for state _hash_absorb_ except that:
  . Only one block is absorbed.
  . The length of the block in bits is given by the length of the last block.
 *** `partial_block_len` > `last_block_len`  and `partial_block_len` != -1;: The CR is *invalidated*.
 *** `partial_block_len` = `last_block_len`.
  . To handle this case, the last three instructions must be changed to
[start=4]
  .. if (`partial_block_len` == `last_block_len`) then {
  ... `absorb_last_block()`
  ... `partial_block_len <- -1`
  . and when this happens [.red]#TBD#.
 ****  and `partial_block_len` == -1;
 ...
 ** `partial_block_len`.  Length of absorbed data so far for the current block (field size at most 16 bits). Only if used by the algorithm.
...
// . `used_VLxSEW`: 16 bits. Current vector length.
 . `partial_block_len`: 16 bits. Length of absorbed data so far for the current block.
