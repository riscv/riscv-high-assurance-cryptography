...
A CC encapsulates its behavior, allowing interchangeable use of ciphers like AES-128, AES-256, Калина-128, Калина-256, and SM4, and so on, in various modes (ECB, GCM-SIV, XEX/XTS) without changes to the compiled code. Side-channel-resistant variants are also supported transparently.
...
// NOTE: The term "Locality" is borrowed from TPM terminology. Unlike TPM localities (provided by sources such as an application, operating system, or driver layer), ACE allows MM firmware to reprogram Locality Secrets (e.g., "OSSecret") for fine-grained control, such as unique values per Trusted OS, instead of having a single locality for all Trusted OSes in system.
...
[NOTE]
====
_A single `acestart` value suffices due to the way the authenticated encryption in `ace.import`/`ace.export` works._

* `_ace.init_` _and_ `_ace.import_`_: Metadata is read first to determine length. Subsequent blocks are processed sequentially._
* `_ace.export_`_: Uses two passes. However, the first pass to compute the SIV is purely internal, and never accesses memory so page faults cannot occur. This first stage is assumed to be uninterruptible. The second pass writes metadata, nonce, SIV, then encrypted content to memory. Only the second pass can cause page faults and requires resumption._
...
[.darkred]#Verification needed: Ensure we can use ASID/VMID usage for restriction works if not used in address translation.#

[[ACE-providing-keys,Remark {counter:remark}]]
*Remark {counter:remark}.* _The software in each World/Supervisor Domain is normally responsible to generate or obtain their own keys, and if a process (applet) in one World/Supervisor Domain is responsible to provide keys to other Worlds/Supervisor Domains, it is the responsibility of the M-mode formware to perform an appropriate `ace.import`/RCSK switch/`ace.export` sequence._
...
[NOTE]
====
`maceBootSecret*` must be reconfigurable to support VM reboots independent of the physical host.
====
...
Implementations of the SKT must provide a suitable Locality to the ACE unit during CC configuration,
...
// Each algorithm must provide the following functions:
...
The formats of the configuration payload, of the content section, and of SCCs depend on the algorithm being used.
The size of a SCC, and of all byte arrays read or written by `ace.init`, `ace.import` and `ace.export`, must be a multiple of 8 bytes.
...
followed by the content, i.e., the key(s) and possibly other information, which then is deserialized.
...
`bytes 20 .. 31  {nbsp}{nbsp}{nbsp}{nbsp}{nbsp}` contain the tag (96 bits), and +
`bytes 32 .. u+31  {nbsp}{nbsp}` contains the serialization of the content of the `content`, which has a byte length of `u`.
+
