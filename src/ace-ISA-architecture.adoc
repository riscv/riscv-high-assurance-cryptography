...
* *Allowed State Transitions:*
 ** _initial_ -> _encrypt_, if metadata allows encryption, else the CR is *invalidated*.
 ** _initial_ -> _decrypt_, if metadata allows decryption, else the CR is *invalidated*.
 ...
 ** _set_aux_value_ -> _encrypt_ -- if metadata allows encryption, else the CR is *invalidated*.
 ** _set_aux_value_ -> _decrypt_ -- if metadata allows decryption, else the CR is *invalidated*.
...
foreach(i from 0 to ceil(len_in_bytes(AD)/16)-1) {
   ...
   V3 ← Galoismul(V4, V3)                      // Horner scheme mult
}
 ...
 ** In state _hash_absorb_, only ``ace.exec`` instructions of the Form “```ace.exec Kn|K[Xn], INPUT```” are allowed. +
...
// The next state change also sets the tag lenth
X1 ← tag_len
 ...
 **  _hash_absorb_, _hash_last_block_ -> _encrypt_ -- if metadata allows encryption, else the CR is *invalidated*.
 **  _hash_absorb_, _hash_last_block_ -> _decrypt_ -- if metadata allows decryption, else the CR is *invalidated*.
 ** _encrypt_ -> _enc_last_block_ -> _enc_tag_finalize_ -> _completed_.
...
In this case the state _set_aux_value_ is skipped, and the initialisation input has the followin form
