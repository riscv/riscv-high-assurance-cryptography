...
For secp256r1 the size of the main fields `b` is 256 bits.
...
* *Internal State*:
  ...
  ** Content:
   ...
   *** `Block_base`: integer.
   *** `Point` (X coordinate, Y coordinate): two `b`-bit strings.
   *** `Scalar`: `b`-bit string.
...
* *Allowed State Transitions:*
 * _initial_ -> { _set_point_, _set_scalar_ } -> *
 * _set_point_ -> _set_scalar_
 * _set_scalar_ -> _set_point_
 * _set_point_ can be skipped if HasPoint == true, and it can be repeated.
 ...
 * * -> _set_hash_ -> _sign_generate_ (only if HasPoint == true) -> _output_ -> _completed_
 * * -> _set_hash_ -> _set_signature_ (only if HasPoint == HasScalar == true) -> _sign_verify_ -> _completed_

In state _set_point_ we can either set a point for scalar multiplication or a public key.
