...
// (`ace.init`, `ace.import`, `ace.clone`, or `ace.derive`-the names should be self-explicative,
// and the authoritative definitions are given in <<ACE-instructions-detailed>>).
If this happens, software needs to free memory by choosing another CR and clearing it.

[.red]#Move this somewhere else?#
CRF capacity is implementation-defined, but implementations must provide sufficient CRF memory to prevent infinite
exporting/importing
//`ace.export`/ `ace.import`
loops,
and the CRF must hold at least an instance of the largest possible CC or the largest CC pair for CR cloning or derivation (i.e., the derivation of a new CC from an existing one, e.g., a key derivation function).
...
The internal representation of the metadata in a CR is implementation specific.
...
| Value | Description
|   0   | Explicit value.
|   1   | Internal table. System specific. For each algorithm as specified in <<ACE-algorithms>>, the part of the content that normally contains the key(s) is replaced by a 64-bit value which is an index into an internal System Keys Table (SKT)-for both Configuration Input and the SCCs. The actual value of the key is never included in exported content. For threshold implementations, ACE may copy the key material from the SKT into the CR and expand it into multiple shares.
...
The Configuration Input is used to configure a CR and thus create a Cryptographic Context (CC).
Its size must be a multiple of 8 bytes.

The first 8 bytes of the Configuration Input contain the metadata section as specified in <<ACE-metadata>>.
...
See <<ACE-algorithms>> for algorithm-specific details of the content part of the configuration input.
