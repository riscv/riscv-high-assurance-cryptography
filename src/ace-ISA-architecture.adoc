...
Each CC consists of an opaque *metadata section*, which does not require confidentiality, and of a *content section*, which requires confidentiality.
...
The confidentiality and integrity of CC content section is enforced by restricting modifications and access to architectural mechanisms that enforce rules defined by the configured algorithm. The concept of _opaqueness_ extends to the entirety of the data in the content section.
...
NOTE: An example of the usefulness of our approach is the ability to use the same binary interface for different algorithms and modes.
...
However, in the US and Europe the underlying block cipher would be the AES, in China SM4, in Russia Кузнечик and in Ukraine Калина.
The CC would encapsulate the cipher and the user code would need no changes to support multiple algorithms, neither at the source code level nor at the binary level, cf.{nbsp}<<ACE-GCM-mode>>.
...
NOTE: The distinction between CC/SCC management instructions and "use-of-CC-for-cryptography" operations is the reason we introduce the concept of _Usage Control_ and do nto re-use the more common term _Access Control_.
...
| 0xXXX   | URO       | `acecclazy`              | Bitmap of lazy-loaded Contexts ("placeholder Contexts")
| 0xXXX   | URW       | `acestart`               | ACE start byte index
| 0xXXX   | MRW (RZ)  | `macerctk{0..7}`         | ACE Root Context Transport Key
| 0xXXX   | MRW (RZ)  | `maceOSSecret{0..4}`     | ACE OS Secret
| 0xXXX   | MRW (RZ)  | `maceBootSecret0{0..4}`  | ACE Boot Secret
|===
...
Disabling ACE is done by writing a zero to `aceenable`, which also clears the CCs, as described in <<ACE-tracking-cc-dirtiness>>. When ACE is disabled, any ACE operation will cause an invalid instruction exception, except feature enablement.
...
|===
|Status |Name |Meaning
|   0   | Off / Disabled-Clobbered | ACE ISE not enabled in the current hart, software must not assume that the contexts are cleared and the may be configured from other environments. They are however, not accessible.
The hart will trap if ACE ISE is enabled by writing to the `aceenable` CSR. +
To be used when: +
...
|   1   | Init / Disabled-Clean     | ACE ISE not enabled, software assumes no context being configured for this hart. +
...
* If ``aceenable``'s value is changed to 1, and if the parent's ``*status.ACES``=Disabled-Clobbered, then the hart traps to allow the parent domain responsible for handling context-switched to finish saving the state and ensure that both `aceccvalid` and `aceccdirty` are set to 0, and finally set ``*status.ACES`` to Disabled-Clean.
// [.darkred]#This behavior should be restricted only to User mode.#

Regarding the `acecclazy` bits, most operations attempted on a CC with this bit set will trap (cf. <<ACE-rules-lazy-loader>> for the exact listing of which operations trap), to allow the parent software to load the context, possibly freeing other contexts in order to make room for it.
For this reason we call CCs marked in `acecclazy` "placeholder contexts".
While this feature is not strictly necessary in normal use, it is needed to support VM migration for the case where the ACE unit of the target device has a smaller CF. In turn, this allows system software to implement lazy loading for all context, if desired.
...
Like `vstart`, `acestart` is a standard read/write CSR; however, software must not write arbitrary non-zero values to it.
...
_A single `acestart` value suffices due to the authenticated encryption algorithm used. The operations behave as follows:_
...
NOTE: Although these operations are inherently complex, the ACE unit is architected to function like a separate machine executing simpler instructions sequentially. State machines receive commands from the CPU and break them into smaller steps.
Therefore, separating data transfer from cryptographic operations in import/export is unnecessary. Otherwise, the loaded payload would need to be stored within the ACE unit rather than processed inline with minimal overhead, and this may lead to higher internal resource requirements.
While resuming interrupted `ace.import` and `ace.export` operations is not necessarily significantly faster than restarting, because of the many cryptographic operations that are needed on the inputs and outputs, resuming avoids potential circular page-fault issues, and for this reason we support it.
...
[.darkred]#However, we need a mechanism to make sure that if the system is not configured to insert `ASID`, resp., `VMID` into the GVAs/GPAs, we can still usage them for the purpose of policing the usage of CCs.#
...
(including _initial_, _completed_, and _invalidated_, cf. <<ACE-state-constants>>), then this number is stored in this field, cf. <<state-field>>,
...
Disallow the _usage_ of this CC in User, (Virtual) Supervisor, Hypervisor, resp., MM if bit 26, 27, 28, resp. 29, is set. Cf. <<state-no-H-no-VMID>>.
| [31:30] |   2  | UC_Policy_2            | If bit 30, resp., 31, is set, then restrict the usage by `ASID`, resp., `VMID`. Restriction by `ASID` ignored if in S Mode or higher. Cf. <<state-no-H-no-VMID>>.
| [47:32] |  16  | UC_Policy_3            | Zero if bit 30 unset, else `ASID` to enforce.
| [63:48] |  16  | UC_Policy_4            | Zero if bit 31 unset, else `VMID` to enforce. Cf. <<state-no-H-no-VMID>>.
|===
...
[[state-field,Remark {counter:remark}]]
...
NOTE: In many algorithms, the successive state is often implied by the operation
just performed. For instance, in AEAD algorithms, after setting an IV or a nonce,
one could argue that there is no need to explicitly move to the state of absorbing
the associated data into the hash. However, to avoid inconsistency, we have chosen
to avoid implicit state transitions, even if this could save a handful of instructions,
...
_completed_ or _invalidated_ (see <<ACE-state-constants>> and <<ACE-algorithms>>).
...
`Cn` is the CC of which to set the state state number. +
`Xn` is a GPR containing a number from 0 to 31 to dynamically address the
context in which to change the internal state. +
...
--
+
If `VLEN` is a multiple of `b`, the above operation is executed `VLEN/b` times, absorbing each `b`-bit block of `Vs` into the hash.
