 ...
 ** _encrypt_, _decrypt_ -> _initial_, _ace_const_hash_absorb_.
 ** _hash_absorb_ -> _initial_, _ace_const_hash_finalize_.
 ...
 ** Once entered the _set_public_value_ state, the ACE unit performs:
  ...
  ** In the _set_public_value_ state:
   *** We can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp}{nbsp} to set `counter <- Vs << b/4`
    (there is no harm if this instruction is issued more than once).
    If no such instruction is issued, counter is assumed to be zero.
  *** Other forms of `ace.exe` not allowed, will cause `error(ace_error_invalid)`.
...
// ** In _encrypt_ and _decrypt_ states, we can only issue `ace.exec Vd, Cn|C[Xn], Cs` which:
  ** If the _encrypt_ state, we can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp}{nbsp} to
   *** Encrypt `Vs` into `Vd`, and accumulate `Vd` into the auth_tag, namely
  ...
  ** In the _decrypt_  state, we can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp}{nbsp} to
   *** [.red]#To be completed.#
 ** In the _hash_absorb_ state, we can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp}{nbsp} to accumulate a value in the auth_tag, namely `auth_tag <- auth_tag xor Vs` and then `auth_tag <- auth_tag * hash_key`.
 ** In the _hash_finalize_ state, we can only issue `ace.exec` form{nbsp}{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp}{nbsp} where `Vs = len_A @ len_C` (two fields of `b/2` bits, to compute the authentication tag as follows: `auth_tag <- auth_tag xor Vs` and then `auth_tag <- auth_tag * hash_key`,
    `auth_tag <- auth_tag xor encrypt_block(key,counter[b-1,b/4] @ 0[b/4-1,0])`. The auth_tag is returned in `Vd`, and the state is changed to _ace_completed_
