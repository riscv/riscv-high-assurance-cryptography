...
If `aceiobuflen` is zero, the input/output buffer is _unconfigured_ (it does not hold any content, similarly to unconfigured CRs, i.e., CRs in state Initial), and `ace.input` and `ace.output` will raise an invalid instruction exception.
