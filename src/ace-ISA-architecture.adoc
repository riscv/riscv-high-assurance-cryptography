...
All the fields are presented in the order they must be included in the configuration input with zero-based indexes, and with their lengths expressed in bytes.
...
. *Content, Exported*:
The data that must follow the metadata block in the plaintext form of the SCC, as serialized by the ACE unit.
All the fields are presented in the order they must be included in the SCC with zero-based indexes, and with their lengths expressed in bytes.
...
* *Content, Internal*:
 ** `key`: `k/8` bytes.
 ** `IV`: `n/8` bytes (also 8|`n`).
 ** `ctr`: `j/8` bytes (8|`j`). // The counter
