...
ACE does not add a register file, but it adds 32 Cryptographic Contexts and
...
. We use one additional bit in the instruction encoding to determine whether a CC is given by an immed or by a GPR.
...
store the SCC. +
 +
Exports the contents of CC `Cs` to memory, starting at address `Xd`, using
the procedure described in <<ACE-formats>>. +
// Depending
// on the type and policies of the key — which are stored in the metadata of `Cs`,
// the CTK can be either the RCTK CSR or a Locale specific RCTK derived from the
// CTK. +
 +
...
If the context `Cs` is clear,
//or does not exist in the microarchitecture
