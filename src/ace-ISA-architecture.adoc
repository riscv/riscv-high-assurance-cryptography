...
ACE extends `*status` and `misa`, and adds up to 25 new CSRs to manage its execution environment.
...
| 0xXXX          | URW       | `acestart`               | ACE start bytes index
...
| 0xXXX          | SRW       | `sacefilter`             | Filter for `ace.input`/`ace.output` that applies to U-mode
| 0xXXX          | MRW       | `macefilter`             | Filter for `ace.input`/`ace.output` that applies to (V)S/H-mode
...
If a vector instruction is interrupted, the register would follow the `vstart` semantics, whereas if an ACE instruction is interrupted, the register would follow the `acestart` semantics.
...
Regardnless of whether the operations are on vectors or on the input/output buffer, `acestart` reflects the number of bytes processed so far.
...
|   - implementations.
|
|   tmp ← POLYVAL(Kauth, zeros(64) @ metadata @ content);
|   SIV ← AESE(Kenc, 0 @ (tmp[126:0] xor (zeros(31) @ N)))) >> 32;
UNINTERRUPTIBLE_BLOCK_END

// preemptible here, with `acestart` from 0 to 15
UNINTERRUPTIBLE_BLOCK_START
|   M[0 .. 15] ← N[63:0]
|              @ (metadata[63:32] xor AESE(Kenc, 1 @ SIV[94:0] @ zeros(32))[31:0])
|              @  metadata[31:0];
UNINTERRUPTIBLE_BLOCK_END
...
foreach(i from 0 to num_blocks - 1) {
    ...
    |                       AESE(Kenc, 1 @ SIV[94:0] @ (binary(i+1)[31:0]))[63:0];
    |   else
    |       M[32+i*16 .. 47+i*16] ← content[i*16 .. 15+i*16] xor
    |                       AESE(Kenc, 1 @ SIV[94:0] @ (binary(i+1)[31:0]));
    UNINTERRUPTIBLE_BLOCK_END
}
----
...
|                     xor AESE(Kenc, 1 @ SIV[94:0] @ zeros(32))[31:0];
...
UNINTERRUPTIBLE_BLOCK_START
|   tag ← AESE(Kenc, 0 @ (tmp[126:0] xor (zeros(31) @ N)))) >> 32;
 ...
 * `aceoffset`, XLEN bits wide, holds the offset within the CR's virtual `content` or the input/output buffer [.red]#(maybe same CSR as `acestart` ?)#.
...
The `ace.input` and `ace.output` instructions can replace the inputs and outputs to Forms A-C of `ace.exec` and the input of `ace.derive` in any algorihm, to the point that an implementation of ACE without the V extension or a subset thereof is possible.
...
--
    . `absorb(Vs)`.
    . `tmp <- enc_blk(enc_key, 0 @ (auth_tag xor N)[b-2:0])`
...
. If `ctr == 0`, then the CR is *invalidated*.
   . If the `SIV` is `b`-bits long, then
     .. ``Vd <- Vs xor enc_blk(key, 1 @ SIV[b-2:b/4] @ (SIV[b/4-1:0] + ctr % 2^b/4^)),``
   . else (the `SIV` is `3b/4`-bits long)
      .. ``Vd <- Vs xor enc_blk(key, 1 @ SIV[3b/4-2:0] @ ctr``.
   . `ctr <- ctr + 1`.
--
+
