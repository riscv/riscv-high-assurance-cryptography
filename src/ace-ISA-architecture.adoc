...
* Invalid metadata section in `ace.prov` or `ace.import`, other than _ace_err_unimplemented_, for instance:
...
as an example.  This type of filtering may also be removed completely.
...
It also sets the corresponding entry in `*crdirty` to Dirty unless the CR was already unconfigured, in which case it does not change the state in `*crdirty`.
+
