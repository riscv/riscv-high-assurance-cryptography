...
By contrast, the specifications for the Ascon suite are specific to the Ascon permutation.
...
These two instructions can be used is implemented and the size in CSR `aceiobuflen` is non-zero and is a valid size (for instance the block size `b` or a multiple thereof), otherwise an invalid instruction error is raised.
...
+
--
. `Vd <- (state[0] xor Vs[63:0]) @ (state[1] xor Vs[127:64])`
. `state[0] <- Vs[63:0]`
. `state[1] <- Vs[127:64]`
. `ASCON(8)`
--
...
** In _dec_hash_finalize_, an `ace.state Kn|K[Xn], #ace_const_dec_hash_finalize, Xs` instruction may executed with a parameter, which sets
