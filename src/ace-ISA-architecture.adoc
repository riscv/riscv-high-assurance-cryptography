...
| `ace.wait`      |                                  |  Waits that all current operations in the ACE unit which are issued by the current hart completes. This is necessary if we need to read the correct error status.
| `ace.error`     |                                  |  The ACE unit may be in an error state due to a previous operation. The error value is written into GPR Xd.
|===
...
NOTE: The operation conceptually split into two phases.
...
Description::
Waits that all current operations in the ACE unit which are issued by the current hart completes. This is necessary if we need to read the correct error status.
...
. *Configuration Input*:
The data that follows the metadata block, such as key(s) and other secrets which are required to initialize a CC, in the initialization input passed to `ace.init`.
All the fields are listed in the order they must be included in the configuration input, and with their lengths expressed in bytes.

. *Content, Internal*:
Information that must be in the internal state of a CC.
The fields are not ordered and the given lengths in bytes can be different than the indicated ones, for instance for threshold or fault-tolerant implementations, where redundancy is required. In particular, we also list the full, single-share, key size also if a System Key is used, because this is the size of the key information used in the algorithms.

. *Content, Exported*:
The data that must follow the metadata block in the plaintext form of the SCC, as serialized by
`content_serialize` and generated by `content_serialize` for the algorithm.
All the fields are listed in the order they must be included in the SCC, and with their lengths expressed in bytes.
...
. Accordingly, the `content_serialize` and `content_deserialize` functions will support System Keys by producing and reading the shorter fields with the indexes in place of the longer fields with the key values
. Internally, the ACE unit can either directly use the value from the SKT, or copy it to the CC.  The latter is useful for threshold implementations.
 ...
 ** In state _initial_ :
  ...
  ** In states _set_public_value_, resp. _set_public_value_2_:
    ...
    *** If no such "```ace.exec Cn|C[Xn], Vs```" is issued, `nonce`, resp. `SIV`, remains zero.
...
* *States:* _initial_ (0), _hash_absorb_ (3), _hash_finalize_ (4),
* *Allowed State Transitions:*
 ** _initial_ -> _hash_absorb_.
 ** _hash_absorb_ -> _hash_finalize_.
 ...
 ** Upon issuing `ace.stage Cn|C[Xn], #ace_state_hash_finalize, Xs` where `Xs` contains the length of the last block in bits, the CC will transition to state _hash_finalize_, where any further {nbsp} `ace.exec Cn|C[Xn], Vs` will simply set (and possibly overwrite) the last partial block.
...
[.red]#Fill missing details and example#
