...
Bit 7 is currently undefined. It can be used to define additional Localities.
...
are: to XOR the values; and to concatenate them. Another possibility is to multiply them in the 128-bit binary Galois field, but
but this would not be faster than prepending the values to the input to the tag computation function, as it requires the same number of additional Galois Multiplications.
--
