...
|   [1:1] |   1   | _AdditionalSize_         | Whether there is an additional block of data at the end of content.
...
For the 64-bit metadata block, in general only one bit can be set, [.red]#we define exceptions in <<ACE-combined-filters>>#.
...
NOTE: Setting multiple bits of `Xs`[32:29] may change the metadata format from 64 bits to 128 bits.
...
// NOTE: The `ace.clone` instruction is an optimization for the sequence `ace.export` followed by `ace.import`. While re-using cryptographic state (e.g., encrypting with the same counter) can enable certain attacks, this risk cannot be eliminated by omitting `ace.clone`, as a state can be exported and re-imported multiple times.
...
the generation of an SCC from a CR (<<ACE-SCC-export>>) and the import of an SCC into a CR (<<ACE-SCC-import>>) next.
...
NOTE: Due to the computational cost of the derivation functions, implementations may cache recently used derived keys for both the Root RCSK and Locality-derived keys.
...
// If a different form of `ace.state` is used, then an illegal instruction exception is raised, and the CR is *invalidated*.
...
* *Provisioning Input*:
 ** In the metadata,
  _Just_ the curve description is embedded in Algorithm. This needs to encode just 5 values.
...
* *Internal State*:
  ** Content:
   ...
   *** `Scalar`: `b`-bit string.
   *** If either signature generation or verification is allowed:
...
[lowerroman]
  ...
  . `HasHash`: bool.
  . `HasSignature`: bool.
  . `Progress`/`input_base`: integer (byte).
  . `Block_base`: integer (byte).
  . Padding: 40 bits.
  . `HasGenerator`: `Generator` (X coordinate, Y coordinate): a `2b`-bit string representing Y @ X.
...
The total size of the serialized content after `Scalar`—i.e., starring with the `PublicKey` field—is encoded in the additional size block after the metadata.
 ...
 ** _output_ (9)
 ** _completed_ (15)
 ** The “*transient” states are used during long operations that require additional information.  If the operations are interrupted, they are in the transient state. When they are completed, they are in the non-transient state.
...
* *Allowed State Transitions:*
 ** _initial_ -> { _set_generator_, _set_scalar_ } -> *
 ** _set_generator_ -> _set_scalar_
 ** _set_scalar_ -> _set_generator_
 ** _set_generator_ can be skipped if HasPoint == true, and it can be repeated.
 ** _set_scalar_ can be skipped if HasNumber == true, and it can be repeated.
 ** * -> _point_mul_ (only if HasPoint == HasNumber == true) -> _output_ -> _initial_
 ** * -> {_set_hash_, _set_public_key_} -> _sign_generate_ (only if HasGenerator & HasPoint) -> _output_ -> _completed_
 ** * -> {_set_hash_, _set_public_key_} -> _set_signature_ (only if HasGenerator & HasPoint & HasNumber) -> _sign_verify_ -> _completed_
...
Let us use value 0 in the entries of `crdirty` to support lazy loading, so we call this state *LAZY* in place of *OFF*.
...
The use of the **OFF** encoding in the per-CR fields of `crdirty` is a natural choice to denote an unavailable state that requires lazy loading to be made available. If all CRs were to be lazy-loaded, the ACES field would be set to **CLEAN** and not to **OFF**.

If an *Export* or *Usage* operation is issued at U-mode, then
...
There is however one complex case, which occurs when:
  ...
  . If the CR was *CLEAN*, an SCC in S-mode would be authoritative, and if there is no SCC in S-mode, the value in H is authoritative, so
...
+
Possible solutions where the dirtiness information is maintained architecturally include:

... The fields of `crdirty` must be expanded to three bits to accommodate a fifth status, *LAZY/DIRTY*;
... Another 32 bits worth of CSRs, say `crdirtyext`, is used for this purpose; or
... The *INITIAL* state in `crdirty` is repurposed to denote this state, and to check whether a CR is unconfigured we check whether `ace.size` returns 0.
