...
| [15:15] |   1   | _Key_Type_               | Key given by explicit value / System Key (index).
| [19:16] |   4   | _State_Number_           | Encoding the current state of the state machine of the algorithm as a number.
...
| [28:26] |   3   | _UC_Policy_              | “UC Policy” means “Usage Control Policy”. If bit 25, 26, resp., 27 is set, it disallows the _usage_ of this CC in User, (V)S-mode, resp., H-mode.
...
The Direction_Policy field is defined only for algorithms that support both encryption and decryption. For algorithms that do not, such as hash functions or some public-key operations, these two bits are not used for selecting the primitive's direction and may instead encode additional algorithm identifiers, thereby expanding the available encoding space of the Algorithms field.
...
System Generated Keys are also system-specific. In place of the II, the initialization operation reads a system-specific “recipe”, which is not portable. After internal key generation, the actual key value—not the recipe—is stored in the CR and SCC. The Key_Type field in the metadata is 0.

//

===== State_Number Field

The State_Number field must be zero in an II.

Some values of State_Number are architected, and are defined in <<ACE-state-constants-fixed>>.
these include _initial_, _operate_, _invalid_, and _completed_, which correspond to values 0, 1, 14, and 15.
...
|   1   | _ace_state_operate_             | _operate_           | Generic operational state.
...
All states different from _invalid_ are _valid states_.

If an algorithm defines 16 or fewer states (including the four architected ones), the current state number is stored in the State_Number field.
These algorithms are allowed to redefine _ace_state_hash_absorb_, but not the states _initial_, _invalid_ and _completed_.

For algorithms with more than 16 states, an algorithm-specific representation is used in the CC content, and the State_Number field is restricted to the
architected values, where _operate_ must be used in any operational state—i.e., different from _initial_, _invalid_, and _completed_.
...
|===
|   Field | Width | Name                     | Description
| [46:33] |  14   | _Filter_1A_              | Value to match against `mcontext`.
| [48:47] |   2   | Zero                     | Zero.
| [62:49] |  14   | _Filter_1B_              | Value to match against `hcontext`.
| [63:63] |   1   | Zero                     | Zero.
|===

In this case, bits 1 and 2 of Filter_Mode can be set.

Other combinations of more than one bit set in Filter_Mode are only allowed if the metadata type is set for 128-bit metadata, i.e., if the Format bit in the metadata is set to 1.
In this case, if Filter_1 is used to match against `scontext` or an expiration date, and we have at least one other filter, then the format of the extension section is given in the following table.
...
The content format—and consequently its length—are algorithm-specific (<<ACE-algorithms>>) and are determined solely by the Algorithm, Key_Type, and Direction_Policy fields in the metadata.
...
* If neither `vsstatus.ACES` nor `mstatus.ACES` is set to Off, executing any instruction that changes ACE state, including the ACE CSs or the ACEIOBUF, will change both `mstatus.ACES` and `vsstatus.ACES` to Dirty.
// TODO: ace.reset should set Initial
...
|  5   | _ace_err_memory_             | Insufficient CRF memory for configuration instruction. This causes an exception that may lead to a horizontal trap.
|  6   | _ace_err_unimplemented_      | Not implemented algorithm or algorithm variant. Not allowed state transition in `ace.state`, including requesting encryption or decryption from a CC not allowing it.
 ...
 ** invalid Filter_Mode for a 64-bit metadata block,
 ** non-implemented Locality value,
 ** State_Number is non-zero for `ace.init`.
...
[WARNING]
[.red]#Optional (need more discussion): ACE error code __i__ corresponds to bit number __i__ in a mask CSR (or more than one) that determines whether the error traps instead of being handled directly by the software by checking# `[.red]#ace.error#`[.red]#. This may be also a horizontal trap. Details in <<TBD>>.#
...
//The algorithms may have basic processing units which could correspond to vector elements.
// However, in an attached architecture, depending on these units may slow down the implementation.
...
However, software must not write arbitrary nonzero values to it,
as the hardware is not required to support all possible values and this may result in an exception-examples are provided in the definitions of `aceiobuflen` (<<ACE-CSR-aceiobuflen>>) and `aceiobuftop` (<<ACE-CSR-aceiobuftop>>).
...
Another example is when providing IVs/nonces. Software can configure the buffer to the maximum size it intends to use and then adjust `aceiobuftop` for shorter operations. Reconfiguration would necessitate zeroing the buffer to prevent data leakage and could incur a performance penalty if internal resizing occurs.
...
Any access to the `haceVBootSecret` group outside of VS-mode and higher causes an illegal instruction exception.
...
* It is _usage-controlled_, i.e., whether the instruction is subject to usage control, which determines which software entities can use it (by _UC_Policy_ and non-time-based _Filter_);
...
* Internal Capacity `ace.size` returns the amount of internal CF capacity used by a CR with metadata section matching `Xn` on the Algorithm, SP_Protection, and Key_Type metadata fields. In this case, all other bits must be zero.
...
. `II_content_size(x)` returns the size in bytes of the content section of an II. It depends only on the 11 bits `Key_Type @ Algorithm`.
. `SCC_content_size(x)` returns the size in bytes of the content section of an SCC. It depends only on the 13 bits `Direction_Policy @ Key_Type @ Algorithm`.
...
* There must be a 4-bit internal temporary state containing the original value of `metadata.State_Number` and a flag indicating whether the value has been saved.
* If, upon returning to the interrupted process, the bit signaling that a value for `metadata.State_Number` has been saved is unset, the operation is restarted.
 ...
 ** If at least 4 bytes have been read, the original value of `metadata.State_Number` is known, so the instruction may be restarted with `acestart` {ge} 4.
...
|===
| Value | Mnemonic for the Constant       | State Name          | Description
|   1   | _ace_state_hash_absorb_         | _hash_absorb_       | Start and initialize a hash function of the tag/MAC part of an algorithm and start absorbing blocks of data (in the case of an AEAD algorithm, this usually starts absorbing the associated data).
...
The value of 1 means also _ace_state_hash_absorb_ for the symmetric algorithms defined here.
...
The state number is stored in the metadata or content section of the CC/SCC, as described in <<ACE-metadata>>, with the constraint that the State_Number field must always contain one of the values 0, 1, 14, or 15, where 1 stands for all other internal states.
...
** Upon issuing a Form A or B ```ace.state Kn|K[Xn], #ace_state_set_aux_value```:
  *** An illegal instruction exception is raised and the CR is *invalidated*.
