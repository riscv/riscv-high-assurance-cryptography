...
| 0xXXX          | MRW       | `macecrstatus`          | Per-CR analogue of `mstatus.ACES` field
...
NOTE: The `ace.export` instruction does not have an `ace.export.S` analogue to `ace.import.S`.
...
Forms A, B, and C of `ace.exec`, as well as Forms C of both `ace.derive` and `ace.state`, can be replaced
...
the value against which the internally computed tag must be compared:
   ...
   else and ACE error _ace_err_algo_auth_ is raised.
...
. At the User and Supervisor levels, CC usage can be optionally constrained by `(v)sacefilter` and `macefilter`.
