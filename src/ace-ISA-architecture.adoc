...
ACE extends the `*status` and `misa` CSRs and adds 22 new CSRs to manage its execution environment.
...
[NOTE]
====
While the standard floating-point and vector extensions do not support User Mode disable/enable instructions, the ACE unit can in general be enabled and disabled in User Mode, instead of using an OS call or trapping.
However, if the ACE unit contains state from a different process, and its interface is disabled for the current one, enabling it will trap to the Supervisor (as described in <<ACE-ACES-field-in-status>>) to allow the latter to first save the state in case it was not saved before and then clear it (lazy context switch).
...
|   1   | Init               | ACE is enabled. The state shall be that no CC is configured. +
...
Setting ACES=Off does not destroy the state, nor does setting ACES=Enabled clear the contents.
...
===== Fine Grained CC Management: `aceccactive`, `aceccdirty`, `acecclazy`
...
.Generation of the SCC from a CC Using AES-GCM
[source,sail]
----
metadata : bits(64);
...
let IV : bits(96) = random(96 bits);
H : bits(128);
let tag : bits(128) = 128'b0;
tmp : bits(128);
...
let : bits(128) = Kauth AESE(Kenc, 0)
let ctr : bits(128) = IV @ 31'b0 @ 1'b1;
...
foreach(i from 0 to last) {
    tmp = content[i] xor AESE(Kenc, 1 @ SIV[94:0] @ (binary(i+2)[31:0]));
    M[i + 2][127:0] = tmp;
    tag = tag xor tmp;
...
if (last_block_fractional) then {
    M[last + 1][63:0] = content[last + 1]
                        xor AESE(Kenc, 1 @ SIV[94:0] @ (binary(last + 2)[31:0]))[63:0];
}
...
In all cases where a GPR is used to index a CC, we cannot use `X0` as the index register, which is hardwired to zero.
The result will be an invalid-instruction exception, unless some special operation is defined using that encoding.

[[ACE-instruction-enable-enable]]
==== ace.enable/ace.enable (instruction?)

`ace.config #immed` +
`ace.enable` +
`ace.disable` +
`ace.clearall`
(((ACE, instruction, ace.config)))
(((ACE, instruction, ace.enable)))
(((ACE, instruction, ace.disable)))
(((ACE, instruction, ace.clearall)))

Encoding:::
[.darkred]#Proposal: use `ace.clear` with `3'b111` as instruction code (use a GPR to select the CC) and zero in the source field [11:7], so it looks like we are using the zero-fixed GPR to select the CC, which is defined as invalid or defined as special.#

Description::
Enable/Disable the feature in the current process domain.
...
The instruction will trap to allow the parent to enable the feature.

Passing a zero will disable the feature (by trapping).
...
The advantage is that the entire CF memory is cleared without the need for compaction/management between separate `ace.clear` operations and there will be no interrupts in between.#
