 ...
 ** `SIV`: `b` bits. +
 ...
 In this case, in the algorithm description the `b/4` least significant bits of the content field `auth_tag` are just zeroed to keep compatibility with the modular addition of the counter value to these bits (i.e., without carry to the most significant `3b/4` bits). The same representation is used for the `SIV` field. +
 Note that the `SIV` field is actually only needed for decryption, since it must be maintained separately from the `auth_tag` field, which is instead recomputed, whereas for encryption it is first computed completely and then used in the encryption process.
 ** `enc_key`: `k` bits.
 ** `auth_key`: `b` bits.
...
State Machine:::
* *States:* _initial_ (0), _set_public_value_ (7), _hash_absorb_ (3), _hash_finalize_enc_ (4), _hash_finalize_dec_ (5), _encrypt_ (1), and _decrypt_ (2).
...
Accepting this redundant behavior simplifies the implementation of the state machine, since the state path +
_initial_ -> _set_public_value_ {-> _set_public_value_2_} -> _hash_absorb +
is common to both encryption and decryption paths.
  ...
  ** In state _encrypt_, we can only issue  ``ace.exec`` instructions of the Form A, namely "```ace.exec Vd, Cn|C[Xn], Vs```," to encrypt `Vs` into `Vd` as follows:
   ...
   . ``Vd <- Vs xor encrypt_block(key,`` +
     ...
     ``1b1 @ SIV[b-2:b/4] @ (SIV[b/4-1:0] + counter % 2^b/4^))``.
   . `counter <- counter + 1`.
...
We now show how to use this a GCM-SIV CC to perform AEAD encryption:
...
ace.exec C0, V1		          	       // Absorb lengths into hash
...
The next pseudocode shows AEAD Decryption with a GCM-SIV CC.
...
// Finally, we absorb the lengths into auth_tag AND verify
ace.stage C0, #ace_const_hash_finalize_enc
V1 ← IntTo64Bits(len(AD)) @ IntTo64Bits(len(PT))
ace.exec C0, V1
