 ...
 ** `L`, `offset`, `hash_A`, `hash_P` : `b/8` bytes.
 ** `Nlen`,`taglen`, `last_blk_len`: int

* *Serialized Content*:
[lowerroman]
 . `key`: `k/8` bytes, or 8 bytes if a System Key index.
 . `N`: `b/8` bytes, but only `u+1` bits defined.
 . `L`: `b/8` bytes.
 . `offset`: `b/8` bytes.
 . `hash_A`: `b/8` bytes.
 . `hash_P`: `b/8` bytes.
 . `taglen`: 2 bytes
 . `last_blk_len`: 2 bytes
 . `Nlen` : 2 bytes
 . padding : 2 bytes
...
[]
  . `offset <- offset xor enc_blk(key, zeros(128))`
