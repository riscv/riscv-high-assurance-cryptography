...
An initialization or import operation may be interrupted by context switching code, i.e., not returning to the original control flow immediately after the handler is done. In this case the intermediate state of the CR must be saved and then restored in a way that allows for resumption of the operation (by setting also the value of `acestart`).
(((Cryptographic Register,incomplete)))
...
. `z` and `num_blocks` are computed from the original content length, and the starting value of `i` with the help of the `acestart`.
