...
The fundamental concept of ACE is the _Cryptographic Context_ (CC), a data structure comprising a *metadata section* and a *content section* that form an indivisible unit.
...
The metadata has a fixed format, and the format of the content depends on the algorithm defined in the metadata.
...
This operation also binds the content to the metadata, effectively creating a Cryptographic Context.
...
These operations use _Context Sealing Keys_ (CSKs), which may be programmed by M-mode and depend on _Localities_ (see below).
A context sealed with a CSK cannot be reimported into a CR using a different CSK.
...
Therefore, the optional _Usage Control policies_ in the metadata section may only be able to restrict the cryptographic operations that are allowed by the algorithms configured by a CC, including modifying its state, but not the operations that manage CRs and SCCs.
...
This binding uses both metadata and a _Derived Context Sealing Key_ (DCSK) derived from the _Root Context Sealing Key_ (RCSK) and a Locality-specific secret via a key derivation function for the purpose of importing and exporting opaque keys.
...
The non-standard notation `C[Xd]`, for a GPR Xd refers to the CR whose number is contained in `Xd`,
...
| `ace.import{.S}` | `Cd\|C[Xd],%offset(Xn)`         |  Authenticates and imports a SCC from memory into a CR. +
...
| `ace.size`
| `Xd,Cs\|C[Xs]` +
`Xd,Xs` .2+|  `ace.size` returns the size in bytes of a SCC that would be exported from CR `Cs` or `C[Xs]`, or from a CR that had `Xs` as its metadata block.
Returns zero if the algorithm specified in `Xs` is not supported.
`ace.available` is an alias for `ace.size`.
| `ace.available` | `Xd,Xs`
| `ace.clone`     | `Cd\|C[Xd],Cs\|C[Xs]`            |  Clones a source CR into a destination CR.
| `ace.derive`    | `Cd\|C[Xd],Cs\|C[Xs]` +
`Cd\|C[Xd],Cs\|C[Xs],Vs2` +
|  Optional. Uses the CC in the source CR to configure the destination CR.
For instance, the source CR could contain a KEM that configures the destination CR with a symmetric key.
| `ace.state` +
`(ace.dir)`
| `Cd\|C[Xd],#immed` +
`Cd\|C[Xd],#immed,Xs`  |  Modifies the algorithm's state, such as triggering transitions between the different phases of hash functions or modes of operation, or provide additional information.
...
| `ace.input`     | `Cd\|C[Xd],%offset(Xs)`          |  Optional. Provides an input to the algorithm configured in CR `Cd` or `C[Xd]`, in place of providing it through the `ace.exec` instruction. Needed only when the inputs are very long, such as the inputs to certain public key schemes.
| `ace.output`    | `%offset(Xd),Cs\|C[Xs]`          |  Optional. Writes the output of an algorithm configured in CR `Cs` or `C[Xs]`, in place of providing it through the `ace.exec` instruction. Needed when the outputs are very long, such as the outputs of certain public key schemes.
...
CRs reside in a dedicated internal _CR file_ (CRF) that is inaccessible to any entity outside the ACE implementation, including software, other hardware blocks, and invasive debug interfaces. The CRF's memory capacity is implementation-defined and may be as large as 32 times the size of the largest possible CR; however, a smaller amount of memory may be used in practice. At a minimum, the CRF must be capable of holding either a single instance of the largest possible CR, or the largest CR pair in which the CC in the first CR derives the second (via `ace.derive`).
...
ACE returns an error _ace_err_memory_ when insufficient memory prevents `ace.init` and `ace.import` operations, requiring the software to free memory by exporting or clearing a different CR. Implementations must provide sufficient CRF memory to prevent infinite `ace.export`/ `ace.import` loops.
...
If the ACE unit contains state from a different process, and the ISE is disabled for the current one, then S-mode is supposed to have set the state to Off.
...
Reading `aceerror` may cause the hart to stall until that operation finishes.
The CSR is writable, allowing context-switching software to restore the error state when a process is resumed.

The standard error codes are given in <<ACE-error-codes>> below.
...
the current internal state of the CC does not expect the used Form of `ace.exec`;
...
|  5   | ace_err_memory          | The free internal CRF memory is not sufficient to perform the `ace.init`, `ace.exec`, `ace.clone`, or `ace.derive`.
The implementation must make sure this cannot happen with any other operation.
If algorithms that support `ace.derive` are implemented, the implementation must make sure that both the input and output CCs of this operation can be allocated in the CRF.
...
This can be used in low CRF memory situations to trigger a lazy reload.
|===
...
While resuming interrupted `ace.import` and `ace.export` operations is not necessarily significantly faster than restarting, because of the many cryptographic operations that are needed on the inputs and outputs, resuming avoids potential circular page-fault issues, and for this reason we provide support for it.
====
...
to give only certain U-mode processes and S-mode processes access to certain key spaces.
...
[.darkred]#We need to make sure that
we can still use `ASID`, resp., `VMID` to restrict CC usage
even if the system is not configured to insert into the GVAs/GPAs.#
...
NOTE: We assume M-mode firmware knows whether the RCSK is configurable, or whether its value is ephemerally generated by HW or fixed.
...
[NOTE]
====
The reason `maceBootSecret*` must be reconfigurable is that it must change in a VM if the latter's virtual hardware is re-booted, and this can happen independently from the underlying host and other VMs. For instance, a VM may be frozen, then the host may be rebooted, and then the VM may be resumed, in which case the virtual machine was not rebooted.
====
...
For software portability and interoperability, ACE architects two formats for cryptographic contexts: the configuration input format, loaded by `ace.init` into a CR to create a CC, and the encrypted, authenticated SCC format produced by `ace.export` and that can be read by `ace.import`. The internal representation of a CC within a CR is implementation-specific.
