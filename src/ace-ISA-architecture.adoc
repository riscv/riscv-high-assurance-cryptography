...
depend on the algorithm being used.
...
foreach(i from 0 to num_blocks - 1) {
    if ((i == num_blocks - 1) and last_block_fractional) {
        // next instruction preemptible, with `acestart` from 32+i*16 to 39+i*16
        content[7+i*16..i*16] = M[39+i*16..32+i*16] xor
                                AESE(Kenc, 1'b1 @ SIV[94:0] @ (binary(i)[31:0]))[63:0];
        T = T xor content[7+i*16..i*16];
    } else {
        // next instruction preemptible, with `acestart` from 32+i*16 to 47+i*16
        content[15+i*16..i*16] = M[47+i*16..32+i*16] xor
                                 AESE(Kenc, 1'b1 @ SIV[94:0] @ (binary(i)[31:0]));
        T = T xor content[15+i*16..i*16];
    }
    T = T * Kauth;  // * = Montgomery multiplication
}
...
This could be accelerated by hardware support, for instance, by providing a mechanism to change automatically RCSKs depending on the ASID/VMID without requiring an `ecall` per context switch. We are not exploring such a solution for now because it would be very difficult to architect and inflexible.
--
