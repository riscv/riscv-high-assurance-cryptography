...
a data structure consisting of a *metadata section*, and a *content section*, which are indissolubly bound to each other.
...
* Bit 2 has a compressed semantic, since it distinguishes between clean (0) and dirty (1) if the Locker is configured for the current process;
and between the absence of a value to lazy-load (0) or its presence (1) if the Locker is configured for an alien process.
Essentially 0 means "parent mode has nothing extra to do" and 1 means "parent mode has something extra to do".
