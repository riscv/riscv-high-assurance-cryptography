...
. `MD_size(x)` returns the byte length of the metadata header. Currently, always 8.
...
`{nbsp}{nbsp}{nbsp}{nbsp}MD_size(x)` + (_FilterMode_ != 0) * `ME` + `PI_content_size(x)`,

and the total size in bytes of an SCC is

`{nbsp}{nbsp}{nbsp}{nbsp}MD_size(x)` + 32 + (_FilterMode_ != 0) * `ME` + `SCC_content_size(x)` + _ExtraContent_ * 8.
