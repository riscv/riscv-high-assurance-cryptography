...
`aceiobuflen` and `aceiobuftop` are both XLEN-bit RW CSRs. +
`aceiobuflen` is used to program the size of the input/output buffer for the `ace.input` and `ace.output` instructions. +
`aceiobuftop` is used to program the end of a transfer operation. It works in conjunction with `acestart` to define the range of the buffer. +
