...
are: to XOR the values; and to concatenate them.
If we consider them as 127-bit strings instead and multiply them
in the binary field defined by stem:[P(x)=x^{127}+x+1], this would work as well, but not be faster than prepending the values to the
input to POLYVAL, since it requires the same number of additional Galois Multiplications. The approach of prepending the values to the
input to POLYVAL is more provably secure, since it is an addition to the total payload, and thus not forgeable, while keeping the values secret, and thus
not requiring their encryption.
--
