...
A CC encapsulates algorithm-specific behavior, enabling interchangeable use of different primitives (e.g., AES-128, AES-256, Калина-128, Калина-256, Кузнечик, SM4) in the same mode of operation (e.g., ECB, GCM-SIV, XEX/XTS) without changes to compiled code.
...
To prevent denial of service, management operations must _always_ be permitted, unless the CR is in an invalid state.
...
Finally, ACE can bind CCs to a *Locality*, i.e., restrict usage to specific devices or device classes, hardware configurations, physical or virtual boot cycles,
and process domains at various modes.
This binding uses Metadata when a CC is in a CR, and augments the _Context Sealing Key_ (CSK) by a Locality-specific _tweak_.
...
|   1   | Initial | ACE enabled. No CR or ACEIOBUF is configured.
...
The SXLEN-bit register `scrdirty` tracks per-CR dirtiness. `scrdirty[__i__]` encodes Clean (0) or Dirty (1) for CR #__i__. Bits [SXLEN-1:32] are WPRI.
...
NOTE: `vscrdirty` exists only when the H extension is implemented.

The VSXLEN-bit register `vscrdirty` tracks per-CR dirtiness for virtualized contexts. `vscrdirty[__i__]` encodes Clean (0) or Dirty (1) for CR #__i__. Bits [VSXLEN-1:32] are WPRI.
