...
|   [1:1] |   1  | Incomplete             | Normally 0. The value is 1 if any operation involving the configuration of the CC has been interrupted and may have to be restarted or resumed. If export is interrupted, it only needs `acestart` (<<ACE-CSR-acestart>>) for resumption.
...
|   [1:1] |   1  | Incomplete             | 1.
|   [5:2] |   4  | Locality               | A field which is either -1 or an index into the Locality Secrets Table.
|   [6:6] |   1  | Variant                | 0 = configuration interrupted at initialization, and +
                                            1 = configuration interrupted while importing.
