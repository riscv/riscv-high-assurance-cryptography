...
The current specification doesn't say anything about transition to ACES=enabled, which would allow dirty state with ACES=disabled-clean.

The rules get a more complex in virtual (V=1) mode, where a single write to aceenable can modify both mstatus and vsstatus:
...
[.red]#*TBD* Specify the exact semantics of `ccstart` ato bend define the points at which various operations can be interrupted.#
