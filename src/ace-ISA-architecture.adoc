...
// ** The flag is also used by the implementation to raise error _ace_err_unconfigured_ instead of _ace_err_invalid_CR_ upon an attempt to use the partially imported CR.
...
** Exceptions that immediately return to the interrupted import operation, such as memory exception, may cause the hardware to resume the import operation instead, if the CR was not otherwise touched.
