...
Setting ACES=Off does not destroy the state, nor does setting ACES=Init or Clean clear the contents.
...
In all cases where a GPR is used to index a CC, we exclude the use of `X0` as the index register, which is hardwired to zero.
