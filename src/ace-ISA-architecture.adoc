...
|  6   | ace_err_unconfigured    | Usage of unconfigured CR in `ace.exec`, `ace.state`, `ace.input`, `ace.output`, `ace.clone`, `ace.derive`, or `ace.harden`/`ace.restrict`. May trigger lazy reload.
|===
...
(((ACE, algorithm, SHA-3)))
These primitives are designed around a special parameter choice of KECCAC cite:[DBLP-conf-eurocrypt-BertoniDPA13] standardized by NIST (NIST, 2015).
