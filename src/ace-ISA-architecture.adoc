...
| 0xXXX   | MRW (RZ)  | maceBootSecret0{0..4}  | ACE Boot Session
|===
...
[.darkred]#*TBD* Specify the exact semantics of `ccstart` ato bend define the points at which various operations can be interrupted.#
 ...
 ** Upon issuing `ace.state Cn|C[Xn],#ace_state_last_block, Xs` where `Xs` contains the length of the last block in bits, following happens:
   . if `partial_len` <> 0, then the CC is *invalidated*. (This means we have not completed the previous block.)
...
--
 . if `partial_len` <> 0, then the CC is *invalidated*. (This means we have not completed the previous block.)
 . if `last_block_len` <> 0, then the CC is *invalidated*. (This means we issued the instruction already.)
 . if `Xs > b`, then the CC is *invalidated*.
