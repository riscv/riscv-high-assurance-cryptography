...
| _No Locality binding_     | No CSK derivation. CC bound to Root CSK only.                                                             | —  | — | 0xF
...
| OEMSecret                 | Provisioned by OEM to differentiate their products from the competition.
                              Can be permanent, or optionally reconfigurable by an implementation-defined HW authenticated mechanism.  | Device | No | 3
...
| Device | M | 6
| VBootSecret               | Regenerated at each (virtual) boot. Not permanent, to be configured at the virtual boot of each VM, or also per SD (see <<ACE-CSR-haceVBootSecret>>). | VM/SD | H/M | 7
| SSecret                   | Can be configured by Supervisor (see <<ACE-CSR-saceSSecret>>).                                             | Process  | S | 8
| Reserved                  | Reserved for future use.                                                                                   | Impdef | Impdef | 9-14
|===
