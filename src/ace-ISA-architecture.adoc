...
let SIV : bits(96) = AESE(Kenc, 0 @ (T xor N) [126:0])[127:32];
...
of instructions, except for transitions to states such as _completed_ or _invalidated_
(see <<ACE-stage-constants>> and <<ACE-Algorithms>>).
...
* *States:* _initial_ (0), _set_public_value_ (7), _operate_ (1).
* *Allowed State Transitions:*
...
* *States:* _initial_ (0), _set_public_value_ (7), _hash_absorb_ (3), _encrypt_ (1), _hash_finalize_ (4), _decrypt_ (2), _hash_finalize_dec_ (5), and _hash_verify_ (6).
 ...
 ** In state _hash_absorb_, we can only issue an {nbsp}``ace.exec``{nbsp} of the form{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp} to accumulate a value in the auth_tag, by performing:
   . `absorb_into_hash(Vs)`.
 ** If the _encrypt_ state, we can only issue an {nbsp}``ace.exec``{nbsp} of the form{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp} to
   encrypt `Vs` into `Vd`, and absorb the ciphertext `Vd` into the `auth_tag`, namely
    ...
    . `tmp <- Vs xor encrypt_block(key,counter)`.
    . `absorb_into_hash(tmp)`.
    . `Vd <- tmp`.
    . `counter[b/4-1:0] <- counter[b/4-1:0] + 1`.

 ** In state _hash_finalize_enc_, we can only issue an {nbsp}``ace.exec``{nbsp} of the form{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp} where `Vs = len_A @ len_C` (two fields of `b/2` bits), to compute the authentication tag and complete the algorithm as follows:
    . `absorb_into_hash(Vd)`.
 ...
 ** In state _decrypt_, we can only issue an {nbsp}``ace.exec``{nbsp} of the form{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp} to
   ...
   . If `counter[b/4-1:0] = 0`, the CC is *invalidated*.
   . `absorb_into_hash(Vs)`.
   . `Vd  <- Vs xor encrypt_block(key,counter)`.
   . `counter[b/4-1:0] <- counter[b/4-1:0] + 1`.
 ** In state _hash_finalize_dec_, we can only issue an {nbsp}``ace.exec``{nbsp} of the form{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp} where
    `Vs = len_A @ len_C` (two fields of `b/2` bits), to compute the authentication tag as follows:
   . `absorb_into_hash(Vs)`.
   . `auth_tag <- auth_tag xor encrypt_block(key,counter[b-1,b/4] @ 0[b/4-1,0])`.
 ** In state _hash_verify_, we can only issue an {nbsp}``ace.exec``{nbsp} of the form{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp}.
    `Vs` is compared to `auth_tag`:
  *** If the values match, the state is changed to _completed_, and `ace.error` returns no error.
  *** If the values do not match, the state is changed to _completed_, and `ace.error` returns error `ace_err_authentication`.
 ...
 ** `nonce`: `3b/4` bits.
 ** `counter`: `b/4` bits.
 ** `auth_tag`: `b` bits. (In general, the `SIV` is the last value of `auth_tag` upon encryption. In some variants, the `SIV` is derived from the `auth_tag`, which is used only internally, by removing the `3b/4` least significant bits before outputting it.)
 ** `enc_key`: `k` bits.
 ** `auth_key`: `b` bits.
...
* `encrypt_block(key,p) -> c` encrypts block `p` using the key, and returns the result `c`.
* `polyval(data)` absorbs data into the hash function by performing a step of (a variant of) GHASH or POLYVAL as specified by the algorithm
   (for instance, AES-GCM-SIV uses POLYVAL):
  . `auth_tag <- auth_tag xor Vs`.
  . `auth_tag <- auth_tag * hash_key` in an algorithm-specific Galois Field, or +
  `auth_tag <- auth_tag $ hash_key`, where the `$` denotes the multiplication in the Montgomery domain of POLYVAL.

State Machine:::
* *States:* _initial_ (0), _set_public_value_ (7), _hash_absorb_ (3), _hash_finalize_enc_ (4), _hash_finalize_dec_ (5), _hash_verify_ (6), _encrypt_ (1), and _decrypt_ (2).

* *Allowed State Transitions:*
 ** From all states to _initial_.
 ** _initial_ -> _set_public_value_ -> _hash_absorb_ -> _hash_finalize_enc_ -> _encrypt_.
 ** _initial_ -> _set_public_value_ -> _hash_absorb_ -> _decrypt_ -> _hash_finalize_dec_ -> _hash_verify_.
 ...
 ** In state _initial_ :
  ...
  *** The following initialization operations are performed:
    ...
    . `auth_tag`, `nonce`, `auth_tag`, `counter <- 0`.

  ** In state _set_public_value_:
    *** We can only issue an {nbsp}``ace.exec``{nbsp} of the form{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp} to set the `nonce`.
    *** If{nbsp} `ace.exec Cn|C[Xn], Vs`,{nbsp} is repeated, it will just re-write the `nonce`.
    *** If no such {nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp} to set is issued, `nonce` remains zero.
    *** Other forms of `ace.exe` are not allowed, they will produce null outputs and the CC is *invalidated*.

  ** In state _hash_absorb_, we can only issue an {nbsp}``ace.exec``{nbsp} of the form{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp} to accumulate a value in the `auth_tag`.  [.red]#Steps to be added.#

  ** In state _hash_finalize_enc_, we can only issue an {nbsp}``ace.exec``{nbsp} of the form{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp} where `Vs = len_A @ len_C` (two fields of `b/2` bits), to finalize the computation of the SIV/tag and output it.
  ** If the _encrypt_ state, we can only issue an {nbsp}``ace.exec``{nbsp} of the form{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp} to encrypt `Vs` into `Vd`. [.red]#Steps to be added.# (This is a counter mode operation.)
 ...
 ** In state _decrypt_,  [.red]#To be completed.# we can only issue an {nbsp}``ace.exec``{nbsp} of the form{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp} to
   decrypt `Vs` into `Vd`, namely and then absorb the plaintext `Vs` into the `auth_tag`, namely
     . If `counter[b/4-1:0] = 0`, the CC is *invalidated*.
     . `absorb_into_hash(Vs)`.
     . `Vd  <- Vs xor encrypt_block(key,counter)`.
     . `counter[b/4-1:0] <- counter[b/4-1:0] + 1`.
