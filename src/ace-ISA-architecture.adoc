...
// CR.exporting is always false except during export. Any other operation that
// may change the state of the CR will set it to false again, to force a restart
// of the export operation in order to ensure that the CR is in a consistent state.
CR.exporting = true;
