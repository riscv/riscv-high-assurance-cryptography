...
* ACE instructions can refer to CRs directly, i.e., with a 5-bit immediate in the instruction encoding, but also indirectly, i.e., through a value in the range [0 .. 31] in a GPR. Indirectly accessing a CR that is outside the defined range ([.red]#currently [0 .. 31]#) raises a invalid instruction error.
