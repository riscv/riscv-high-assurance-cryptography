...
ACE defines an optional input/output buffer (ACEIOB) that can be used to store data for cryptographic operations.
...
However, the ACEIOB can also be used instead, as explained in <<ACE-replacement-instructions>>.
...
|===
|Status |Name |Meaning
|   0   | Off                | ACE disabled. CRs and the input/output buffer are inaccessible. The hart will trap if the user requests to enable the feature.
|   1   | Initial            | ACE enabled. All CRs and the input/output buffer are unconfigured.
|   2   | Clean              | ACE enabled. Some CRs or the input/output buffer are configured, and the state matches the last context swap.
|   3   | Dirty              | ACE enabled. Some CRs or the input/output buffer may have been modified since the last context swap. This includes cleared/unconfigured states since switching code needs to act on them, erasing any saved state.
|===
...
Writes to any ACES field do not affect the contents of the CRs or of the input/output buffer.
...
When V=1 and neither `vsstatus.ACES` nor `mstatus.ACES` is set to Off, executing any instruction that changes ACE state, including the ACE CSs or the input/output buffer, will change both `mstatus.ACES` and `vsstatus.ACES` to Dirty.
// TODO: ace.reset should set Initial
...
If `aceiobuflen` is zero, the input/output buffer is _unconfigured_ (it does not hold any content, similarly to unconfigured CRs, i.e., CRs in state Initial), and `ace.input` and `ace.output` raises an invalid instruction exception.
...
In this case, the `ACE` and `ACEIOB` fields in `sstatus` shall be un-shadowed.
...
If all CRs _and_ the input/output buffer are Clear, the state in `*status.ACES` is set to Initial.
