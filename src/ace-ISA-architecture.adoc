...
ACE implements _opaque keys_ through the _Cryptographic Context_ (CC), an architectural container that safeguards sensitive information.
Each CC consists of a *metadata section*, which does not require confidentiality, and a confidential *content section*.
The metadata defines the cryptographic algorithm and optional usage policies; the content holds sensitive material such as keys, other secrets, and internal algorithm state.
Integrity of the entire CC section and confidentiality of its content section are enforced by allowing access and modification of a CC only according to the rules of ACE and of the configured algorithm.
The principle of _opaqueness_ applies to _all_ data within the content section.
...
In order to allow efficient reconfiguration of CCs
and protect their information also outside the architectural state,
a CC can be exported as a _Sealed Cryptographic Context_ (SCC), i.e., an encrypted and authenticated package.
A SCC can be reimported into a CC when needed, and a CC's content is opaque also when exported as an SCC.
These operations use _Context Wrapping Keys_ (CWKs), which may be programmed by MM and depend on _Localities_ (see below).
//Because of its encryption, also the content section in an SCC is called _opaque_.

Usage Control Policy:::
(((Cryptographic context, Usage Control)))
An important distinction in ACE is between the _import_ and _export_ operations on CCs on one hand, and the _use_ of CCs in cryptographic operations on the other hand.
It must be always possible to import and export SCCs, as well as clear and reinitialize CCs with new data: If this were not possible, then a process may be able to deny all other processes access to ACE by simply configuring all the CCs.
Therefore, the optional _Usage Control policies_ in the metadata section may only be able to restrict the cryptographic operations that are allowed by the algorithms configured by a CC, including modifying its state, but not the operations that manage CCs and SCCs. [.darkred]#TBD: Add to each instruction whether it may be usage-controlled or not.#

NOTE: The distinction between CC/SCC management instructions and "use-of-CC-for-cryptography" operations is the reason we speak of _Usage Control_ in place of the term _Access Control_.
...
This binding uses both metadata and a _Derived CWK_ (DCWK) derived from the RCWK and a Locality-specific secret via a key derivation function for the purpose of importing and exporting opaque keys.
...
| `ace.init`      | `Cd\|C[Xd],%offset(Xs)`          |  Sets a CC's metadata and content.
...
(((Cryptographic context, context file)))
...
| 0xXXX          | URO       | `aceccactive`            | Bitmap of active contexts
| 0xXXX          | URO       | `aceccdirty`             | Bitmap of dirty contexts
| 0xXXX          | URO       | `acecclazy`              | Bitmap of lazy-loaded ("placeholder") contexts
| 0xXXX          | URW       | `acestart`               | ACE start byte index
| 0xXXX--0xXXX   | MRW (RZ)  | `macercwk{0..7}`         | ACE Root Context Wrapping Key
...
// [[ACE-CSR-enable]]
// ===== `aceenable` (deprecated)

// `aceenable` is a single-bit CSR. (deprecated)
//(((CSR,aceenable)))
...
// When ACE is disabled, any ACE operation will cause an invalid instruction exception, except feature enablement.
...
// re-import the CCs. Hence, `aceenable` is not cleared, and the `*status.ACES` fields (see <<ACE-ACES-field-in-status>>) of the parent domains will take the value Enabled.
...
// To be used when: +
// ∙{nbsp}{nbsp} M mode (or HS mode) boots the S mode stack since a priori it does not know whether S supports the feature, and therefore it must force trapping, or, +
// ∙{nbsp}{nbsp}When the S SW stack supports lazy context swtching.
|   1   | Initial            | ACE is enabled. The state shall be that no CC is configured. +
...
|   3   | Dirty              | ACE is enabled. there may have been changes to some CC since the last time the value "Clean" was written.
|===

Writes to ACES do not affect the contents of the CCs:  Setting ACES=Off does not destroy the state, nor does setting ACES=Initial or Clean clear the contents.
...
[width=75,align=center,float=center,cols="<,<,<,<,<"]
|===
5+^|Execute instruction to read state (N/A)
...
Let us define four properties for Cryptographic Contexts (CCs):

[horizontal]
`configured`:: Indicates whether the Cryptographic Context (CC) is currently configured.
`native`:: If the CC is configured, indicates whether the current process is its owner.
`dirty`:: If the CC is configured and owned by the current process, indicates whether its state has been modified.
`lazy`:: Indicates whether the CC's configuration is stored only in the parent's switching context, regardless of whether the CC is unconfigured or occupied by a context belonging to a different process.

We need to distinguish between the current process and the owning process of the configuration of a CC.

The operating system must keep not only state for each process, but also keep track for each CC to which process it belongs.
...
===== Fine Grained CC Management: `aceccconfigured`, `aceccnative`, and `aceccdirtylazy`

States we need:

*VOID* (`0b000`)::
[.darkred]#_CC is unconfigured, and there is no saved value for the current process._# +
`[.darkred]#configured = 0, native = 0, lazy = 0.#`
** In this case it must be `dirty = 0`. +
Using the CC gives an error, no trap.
** Configuring the CC works as expected, sets: +
`configured = 1, native = 1, lazy = 0,` and `dirty = 1` {implies} *DIRTY*.

//

*LAZY* (`0b100`)::
[.darkred]#_CC is unconfigured, and parent has saved a value to be lazy-loaded for the current process._# +
`[.darkred]#configured = 0, native = 0, lazy = 1.#`
** In this case it must be `dirty = 0`.
** Accessing the CC traps to allow the OS to lazy load it for the current process.
** Can be used freely. If potentially modified, `dirty` becomes `1`.
** Configuring the CC works as expected, and sets: +
`configured = 1, native = 1, lazy = 0`, and `dirty = 1` {implies} *DIRTY*.

//

*CLEAN* (`0b011`)::
[.darkred]#_CC is configured for the current process and loaded. Unchanged since last switch._# +
...
*DIRTY* (`0b111`)::
[.darkred]#_CC is configured for the current process and loaded. Potentially changed since last switch._# +
`[darkred]#configured = 1, native = 1, lazy = 0, dirty = 1.#`
** If the process is interrupted now, the OS must save the CC.
** Rest is as in the previous case.

//

*ALIEN* (`0b001`)::
[.darkred]#_CC is configured for a different process, blocked for the current one, no value to be lazy-loaded._# +
`[darkred]#configured = 1, native = 0, lazy = 0.#`
** _using_ the CC will result in an error.
** _configuring_ the CC will first trap to the operating system, to allow the latter to save the CC to the state of the owning process, clear it, update the flags to *VOID* to allow the current process to configure it, and return to the process to restart the operation allowing the configuration.

//

*ALIEN_LAZY* (`0b101`)::
[.darkred]#_CC is configured for a different process, blocked for the current one, and there is a value to be lazy loaded for the current process._# +
...
* Bit 0 distinguishes between unconfigured (0) or configured (1);
...
This implies that M, resp., HS mode must always context-switch all CCs when context-switching S, resp., VS domains, since it doesn't know which CCs have changed.
We *could* add additional registers to track changes at each level.  This should be done for F and V too, ideally, in order to make the architecture consistent.
...
This is done by using three CSRs, which track the use and dirtiness of the CCs individually, as well as their lazy loading status.

[arabic]
. `aceccactive`: XLEN bits, with a bit per CC to denote whether the register is in use or not (cleared contextually to CC erasure to dispose of the key).
(((CSR,aceccactive)))
...
(((CSR,aceccdirty)))
. `acecclazy`: XLEN bits, with a bit per CC to track whether the corresponding CC is to be lazily loaded or is actually present.
(((CSR,acecclazy)))

If the _i_-th bit of `aceccdirty` is set, then the _i_-th bit of `aceccactive` must also be set.

If the _i_-th bit of `acecclazy` is set, then the _i_-th bit of `aceccactive` is automatically cleared, and the CC does not consume any memory in the CF.
...
* If ``aceenable``'s value is changed to 1, and if the parent domain's ``*status.ACES``=[.red]#INIT?#Disabled-Clean, then we simply assume that the parent process has cleared the CC and set both `aceccvalid` and `aceccdirty` to 0 before transferring the control flow to the current process.
* If ``aceenable``'s value is changed to 1, and if the parent's ``*status.ACES``=[.red]#OFF?#/Disabled-Clobbered, then the hart traps to allow the parent domain responsible for handling context-switched to finish saving the state and ensure that both `aceccvalid` and `aceccdirty` are set to 0, and finally set ``*status.ACES`` to Disabled-Clean.
// [.darkred]#This behavior should be restricted only to User mode.#

When the `acecclazy` bit for a given CC is set, most operations targeting that CC will trap.
//(see <<ACE-rules-lazy-loader>> for the specific list).
This trap enables parent software to load the context, potentially by first freeing other contexts to create space.
Consequently, we call CCs with the `acecclazy` bit set _placeholder contexts_.
Although not required for typical operation, this mechanism is essential for supporting virtual machine migration when the target device's ACE unit has a smaller context file (CF), in which case, the OS in the VM may not be able to reload all contexts for a given process.
It also permits system software to implement lazy loading for all contexts.
____
...
*Remark {counter:remark}.* _The software in each World/Supervisor Domain is normally responsible to generate or obtain their own keys, and if a process (applet) in one World/Supervisor Domain is responsible to provide keys to other Worlds/Supervisor Domains, it is the responsibility of the MM to perform an appropriate sequence of import, switch the CWK, then export operations._

[[ACE-CSR-macercwk]]
===== `macercwk`

If the RCWK can be programmed, it can only be programmed by M-mode.
(((CSR, `macercwk`)))
For this purpose, we use eight 32-bit WARL CSRs in RV32 and four 64-bit WARL CSRs in RV64.
The CSRs are: `macercwk0`, `macercwk1`, `macercwk2`, and `macercwk3` in RV64, to which we add
`macercwk4`, `macercwk5`, `macercwk6`, and `macercwk7` in RV32.
Bits [(_i_+1) XLEN - 1 : _i_ XLEN] of the RCWK are written to `macercwk{i}`.

Attempting to access the `macercwk` block outside of MM will cause an illegal instruction exception.
...
[.darkred]#We can use the data structure pointed by `mconfigptr` in order to track optional features of ACE including whether the `macercwk` CSR block is implemented, and if not whether whether the value is ephemeral or fixed.#

//User mode, and even operating systems and hypervisors, need not be aware of the `macercwk` block. Other mechanisms could have been chosen to program the CWK, such as an MM-exclusive dedicated hardware interface or memory-mapped interface.

NOTE: Mechanisms for persisting Root Context Wrapping Keys (RCWKs) across reboots or transferring them between devices are outside the scope of the ACE architecture. Such mechanisms are system-specific and fall under the purview of MM firmware.
...
Reconfiguring the RCWK is a better approach for such isolation requirements, whereas isolation by `ASID` and `VMID` is a weaker security feature.
...
If the Locality Field of a context (CC/SCC) is -1 (all ones), then the RCWK is used.
Otherwise, this value is used as an index _i_ in the HST, and a derived key computed from RCWK and LST[_i_] is used for import/export in place of the RCWK.
The encryption and authentication keys are then derived from the DCWK.
...
(Index)
| (No Locality binding)     | No CWK derivation. CC bound to Root CWK only. | -- | -- | -1 (15)
...
*Example*: _If a key's metadata specifies a Locality field value of "2", the key for import/export operations is derived from the RCWK and the "OEM Secret".
...
To describe the authenticated encryption of a SCC, we first define the RFC8452 Key Derivation and the DCWK Derivation functions.
...
if (metadata.Locality == 15) then {
    ...
    CWK = DCWK_KeyDeriv(RCWK, LST[metadata.Locality]);
}

{Kenc, Kauth} = RFC8452_KeyDeriv(CWK);
...
cache recently used derived keys for both the Root RCWK and Locality-derived keys.

[[ACE-RCWK-vs-ID]]
=== Programmable RCWKs vs. ID-based Usage Control
...
Because filtering by ASID is inactive when the hart is in S mode or higher, context switching can be performed without modifying the RCWK.

At higher privilege levels, not only the usage, but even the import of CCs can be restricted by a programmable RCWK.
...
The envisioned usage model employs ASID/VMID filtering at the U level, and potentially at the S level in high-performance systems where migration is not required (e.g., automotive applications). Cryptographic domain separation via the RCWK is intended for Virtual Machines, Supervisor Domains, Worlds, and TEEs within those contexts, as these demand stronger security guarantees.

Although the architecture does not preclude assigning unique RCWKs to individual U processes, doing so would impose a significant burden on the system software stack. Hardware support could mitigate this—for example, by automatically switching RCWKs based on ASID/VMID without per-context-switch `ecall` overhead. Such a solution is not currently being explored due to its architectural complexity and inflexibility, and HW cost-for instance, it would require secure management of very large RCWK tables, including the potential need to save, restore, or migrate of entire blocks of RCWKs.
...
Since ASID filtering is not active while the hart is in S mode or higher, context switching can be performed without changing the RCWK.

At higher levels, even import of CCs can be constrained by a programmable RCWK.
This is a more secure solution, and prevents also `ace.import`, but it is also heavier, since it requires an `ecall`.

Indeed, we have envisioned a system where ASID/VMID filtering is used at the U level, and possibly at S as well in systems where migration is not supported but high performance is (such as automotive applications). The cryptographic domain separation by RCWK is for VMs, Supervisor Domains, Worlds, and also for TEEs in the previous contexts as well, since these require higher security guarantees.

While the architecture does not prevent individual RCWKs per U-mode process, this would add burden to the system stack.
This could be accelerated by hardware support, for instance, by providing a mechanism to change automatically RCWKs depending on the ASID/VMID without requiring an `ecall` per context switch. We are not exploring such a solution for now because it would be very difficult to architect and unflexible.
--
...
In all cases where a GPR is used to index a CC, we exclude the use of the zero-hardwired GPR `X0` as the index register-that is, the encoding of the register cannot be zero. This is reserved to encode some special operation, and otherwise raises an invalid-instruction exception.
...
* If the `immed12` = `0xa11`, then all CCs are cleared and the new state of the ACE unit is Clear. The alias for this instruction is `ace.reset`.

`ace.enable` must be issued before using any ACE operation, i.e. `ace.config` passing the immediate 1.
--
+

NOTE: The advantage of `ace.reset` with respect to a loop that clears all CCs is that the entire CF memory is cleared without the need for compaction/management between the separate `ace.clear` operations and this opertation cannot be interrupted, reducing potential wasteful context saves.

The instruction will trap in the follwong cases: [.red]#TBD#.

NOTE: This instruction shares its encoding with `ace.clear` with `3'b111` as instruction code (use a GPR to select the CC). However, the zero in the source field [11:7] for indirect CC indexing is defined as invalid/special, and this allows to define this instruction.#


// [.darkred]#Proposal: With the immediate `0xa11` (all), all CCs will be cleared, as if `ace.clear` had been called on each of them, but the feature is not disabled.
...
// specific DCWK). The instruction imports them like software keys and sets
...
NOTE: The value can significantly differ from the internal resource usage of the CC —
 ...
 * A “`close`” instruction to suspend or finalize the procedure.
   If executed before all bytes are transferred, the CC state matches that of an interrupted monolithic instruction, including the Completion metadata field and the `acestart` value. If executed after all bytes are transferred, it also performs any final cryptographic operations, such as authenticating the CC, if needed.
...
(((Cryptographic context, invalidated)))
...
// [[ACE-rules-lazy-loader]]
// ==== Rules for Lazy-Loaded CCs
//
// If a CC is marked in `acecclazy`, it is called a "placeholder" CC.
// Any operation involving that CC will trap, unless the operation overwrites the CC, that is,
// for `ace.init`, `ace.import`, as well as `ace.clone` and `ace.derive` if this CC is the destination.
// In these cases, the corresponding bit in `acecclazy` will be cleared.

// A placeholder CC is created like any other CC, by providing a Configuration Input.
// The latter consists only of the metadata block, which is partly encrypted.
//
// An `ace.exec` instruction of Form C, i.e., "```ace.exec Vd,Cn|C[Xn]```," where `Cn` or `C[Xn]` contains a placeholder CC, will copy the Reference field in `Vd`
// (the four least significant bits of `Vd` will be zero), but only if the Mode_Usage field (see <<ACE-metadata-placeholder>>) allows it, i.e., if the value of this field is 0, 1, 2, resp. 3, the mode must be at least User, Supervisor, Hypervisor, resp., Machine.
// If, on the other hand, the mode is less privileged than the value of the Mode_Usage field, the instruction will trap instead.
// This allows context switching software to create Placeholder CCs, which will trap when the software attempts to execute them to allow the parent mode to load the actual ones.

//An `ace.exec` instruction of Form B, i.e., "```ace.exec Cn|C[Xn],Vs```," where `Cn` or `C[Xn]` contains a placeholder CC, will overwrite the Reference field of the CC with `Vs` (the four least significant bits of `Vs` will be ignored).
 ...
 ** In state _set_public_value_, only ``ace.exec`` instructions of Form B are allowed. +
  ...
  *** Any other Form of ``ace.exec`` will produce a null output and *invalidate* the CC.
 ...
 ** In state _operate_, any other Form of ``ace.exec`` will produce a null output and *invalidate* the CC.
 ...
 ** In state _set_public_value_:
  ...
  *** If `VLEN` is larger than `b`, the most significant bits of `Vs` (i.e., the "tail" of the vector) are ignored.
  *** Any other Form of ``ace.exec`` will produce null outputs and causes the CC to be *invalidated*.
 ...
 ** Upon issuing `ace.state Cn|C[Xn],#ace_state_last_block, Xs` where `Xs` contains the length of the last block in bits, following happens:
   . if `partial_len` <> 0, then the CC is *invalidated*. (The previous block is not complete.)
   . if `Xs > b` then the CC is *invalidated*.
   . `last_block_len <- Xs`.
 ...
 ** In state _initial_:
  ...
  ** In each state except _initial_, before performing any operation except `ace.import`, `ace.export`, and `ace.clone`, `used_VLEN` is compared to `VLEN`.
  If there is a mismatch, the CC will be *invalidated* and any operation will result in an invalid instruction exception.
...
--
 . if `partial_len` <> 0, then the CC is *invalidated*. (The previous block is not complete.)
 . if `Xs > b`, then the CC is *invalidated*.
 . if `Xs = 0`, then the CC is *invalidated*.
 . `last_block_len <- Xs`.
--
+

 ** In state _hash_last_block_:
  *** if `last_block_len` = 0, then the CC is *invalidated*.
  ...
  *** If `b` is a multiple of `VLEN` and `b` > `VLEN`, then we can one or more instructions ``ace.exec`` of Form B, i.e., "```ace.exec Cn|C[Xn],Vs```", to absorb the data block `Vs` into the hash, i.e. "```ace.exec Cn|C[Xn],Vs```":
   . if `partial_len < b`, then
   .. The instruction must be of Form B, else the CC is *invalidated*.
   .. `Vs` is written to `partial_block` starting at byte offset `partial_len`.
   .. `partial_len <- partial_len + VLEN`.
   . else if `partial_len+VLEN = b`, then the CC is *invalidated*.
...
To address this, the `acecclazy` CSR to enables lazy context reloading by causing the hart to trap to the parent mode if a CC being accessed is marked as "lazy", i.e., the corresponding bit in `acecclazy` is set.  When this requires freeing one or more CCs first, system software decides which contexts to evict.
...
Software environments that implement key management operations for other domains require their own RCWK. A system may contain multiple such software environments.
...
The MM, which controls the CWK, can the

[arabic, start=5]
. Set the RCWK to the KMA’s RCWK.
. Re-import the SCC into a CC
. Change the RCWK to match the requester domain's RCWK.
