...
// These are, currently, `ace.exec` (cf.{nbsp}<<ACE-instruction-exec>>),
// `ace.state` (cf.{nbsp}<<ACE-instruction-state>>),
// and `ace.derive` (cf.{nbsp}<<ACE-instruction-derive>>).
...
** `ace.export` exports a CR using an AEAD algorithm. The size of the exported data may be different from the data read by `ace.init` and the internal state of the CR; and
...
* ACE instructions can refer to CRs directly, i.e., with a 5-bit immediate in the instruction encoding, but also indirectly, i.e., through a value in a GPR. Indirectly accessing a CR that is outside the defined range raises a invalid instruction error.
* Few instructions can use and possibly change the contents of the CRs.
...
The formats of the II and of the SCC are described in <<ACE-init-export-import-semantics>> and <<ACE-format-SCC>>, respectively.
...
Future formats with a 1 in this position may be 128 bits long (or longer).
|   [4:1] |   4  | Locality               | A field which is either all ones (15) or an index into the Locality Secrets Table.
|   [5:5] |   1  | Reserved               | Zero.
|  [15:6] |  10  | Algorithm              | Algorithm (such as primitive or mode).
| [17:16] |   2  | SC_Protection          | Level of Side Channel Protection, encoded as per <<ACE-SC-protection-levels>>.
| [19:18] |   2  | Direction_Policy       | Determines whether the CC may be used for encryption (bit 18 set), decryption (bit 19 set), or both (bits 18 and 19 set). Valid only for encryption/decryption primitives or modes, and ignored, for instance, for hash functions.
...
+
*Internally generated keys* are also system-specific. In the II, the key material is replaced by a system-specific “recipe”, which is not defined by the ISA and may be non-portable. After internal key generation, the actual key value—not the recipe—is stored in the SCC content, consistent with other keys for the same algorithm. The Key_Type field in the metadata remains 2 across all CC representations.
...
Three values are architected for all algorithms: 0, 14, and 15, representing the _initial_, _invalidated_, and _completed_ states, respectively. These denote a CC that is ready to operate, has been invalidated, or has finished its operations.

If an algorithm defines 16 or fewer states (including _initial_, _completed_, and _invalidated_), the current state number is stored directly in this field. For algorithms with more than 16 states, an algorithm-specific representation is used in the CC content, and the State_Number field is restricted to the values 0, 1, 14, and 15, where 1 serves as a placeholder for any operational state other than the three architected ones. The state values for algorithms defined in this specification are listed in <<ACE-state-constants>>.
--
+
...
| OEMSecret                 | Provisioned by OEM to differentiate their products from the competition.
                              Can be permanent, or optionally reconfigurable by an implementation-defined HW authenticated mechanisms. | Device | N | 3
...
The total length of the II must be a multiple of 8 bytes.
...
In the simple ECB schemes (<<ACE-ECB-mode>>) not hardened against side-channel attacks, a CR need not contain more than the metadata and the key, hence their  resource usage matches the size of the II (unless the key is not a multiple of 64 bits).
...
The content format—and consequently its length—is algorithm-specific (cf.{nbsp}<<ACE-algorithms>>) and is determined solely by the Algorithm and Key_Type fields in the metadata as well, but in general differs from the content section of the II.
...
ACE extends `*status` and `misa`, and adds up to 25 new CSRs, which are listed in <<ACE-CSRs-table>>.
...
|  5   | ace_err_memory          | Insufficient CRF memory for `ace.init`, `ace.exec`, `ace.clone`, or `ace.derive`.
...
The value includes all sections of the output, i.e., metadata, SIV, and content, and accommodates piecewise execution via traps (cf.{nbsp}<<ACE-trappable-insns-support>>).
...
The semantics and use of `acestart` are described in <<ACE-init-export-import-semantics>>.
...
This is a group of M-Mode WARZ XLEN-bit CSRs used to configure `macercsk`.
(((CSR, `macercsk`)))
...
NOTE: The operation follows the two-phases processing model defined in <<ACE-init-export-import-semantics>>.
...
+
The instruction must follow the interruptibility rules defined in <<ACE-CSR-acestart>> and <<ACE-init-export-import-semantics>>.
...
(`ace.import`).
+
`ace.import` must follow the interruptibility rules defined in <<ACE-CSR-acestart>> and <<ACE-init-export-import-semantics>>.
...
Bit `v` (i.e., bit 16) is 0 for Forms A and B and 1 for Form C. +
Bit `g` (i.e., bit 17) is used to distinguish Form A, with `g` = 0, from Form B, with `g` = 1. +
...
`ace.error` is implemented as a read from CSR `aceerror`, which may cause the hart to wait if the error code is not yet available (cf.{nbsp}<<ACE-CSR-ace-error>>).
...
....
+
The type field can assume four possible values:

* `001`: Context type with the context given as `Ks`.
* `010`: Context type with the context given indirectly as `K[Xs]`.
* `011`: Metadata type.
* `100`: Internal capacity type.
...
+
On RV32, `Xs` must contain only bits [31:0] of the metadata.  This suffices since they contain all fields required to determine the size of the SCC.
...
+
On RV32, `Xs` must contain only bits [31:0] of the metadata (as for the metadata type instruction).

Size Functions::: {empty}
+
--
...
. `SCC_content_size(x)` returns the size in bytes of the content section of an SCC. It only depends on the 12 bits `Key_Type @ Algorithm`.

The total size of an II is therefore `MD_size()` + `II_content_size()`, and the total size of an SCC is `MD_size()` + `SCC_content_size()`.
--
+
...
* `Xs`[19:16] and `Xs`[63:32] have the same format as the corresponding bits of the metadata.
* `Xs`[17:16] is used to turn on side-channel protection or increase its level, if implemented.
The level is encoded according to <<ACE-SC-protection-levels>>.
Downgrading protection level is not permitted. Attempting to do so will cause error _ace_err_invalid_ (cf.{nbsp}<<ACE-error-codes>>).
* `Xs`[19:18] is used to deactivate either encryption or decryption in a CC that allows both. +
If `m`[19:18] = `0b11`, then at least one bit of `Xs`[19:18] must be set to 1 to select which operation is kept. +
If `m`[19:18] != `0b11`, then it must be `Xs`[19:18] = `m`[19:18] or `Xs`[19:18] = 0, in which case no change is made, else this will result in error _ace_err_invalid_.
...
* `Xs`[33:32] can be non-zero only if `m`[33:32] is zero, in which case  `m`[55:34] <- `Xs`[55:34], else `aceerror` is set to _ace_err_invalid_.
...
{reg:[
    ...
    { bits:  3, name: 0x3, attr: ['ace.exec','(Form D\')'] },
    { bits:  5, name: 'Ks|K[Xs]' },
    { bits:  5, name: 0x5, attr: ['ace.clone'] },
    { bits:  1, name: 0x0 },
    { bits:  2, name: 'R' },
    { bits:  4, name: 0x0 },
]}
...
. `01`{nbsp}{nbsp} if the source CR is selected using a GPR and the output CR is encoded as an immediate value.
...
[[ACE-init-export-import-semantics]]
=== Semantics of Initialization, Export and Import of Cryptographic Contexts

From the point of view of the system outside the ACE unit
`ace.init` and `ace.import`
appear a series of sequential memory reads, and
`ace.export`,
appear a series of sequential memory writes.

The only points of interruption of these instructions are the memory read or write operations.
These may be interrupted because of a memory exception.

The instructions may resume from the point of interruption only if the control flow is returned to them immediately after the interrupt handler has completed.
The privileged interrupt handler must ensure that the CR is cleared and the instruction restarted only in this situation, and any other circumstance, such as any interruption of the handler itself, sleep, hibernation, transfer of the process to a different hart, all clear the CR and cause the instruction to be restarted. The implementation will also restart any operation if the internal state of the CR is not in a valid interrupted state.

If the instruction is interrupted to perform context switching, it must be restarted upon resuming the process, and
if the instruction is `ace.init` or `ace.import`, the involved CR must be cleared or marked as such.
...
[comment]
--
For very long SCCs, the requirement that the initial cryptographic phase of `ace.export` be uninterruptible may represent a performance bottleneck or a timing hurdle, for instance for real-time systems.
Since `acestart` must be a multiple of 4 or 8, depending on the implementation, we could consider writing the next (odd) integer to mark an interruption of the SIV computation. Since the latter is performed in blocks of 16 bytes using POLYVAL,
with reference to <<ACE-SCC-export>>
`acestart` would take 17 after the metadata has been multiplied by the hashing key,
then 33, 49, etc., after the first, second, third, etc., block of content has been added to the partial hash and the latter multiplied by the hashing key.
The instruction would not be interrupted between the last hashing key multiplication and the subsequent encryption. Hence, a value of
`SCC_content_size() + 17` would denote that the memory write of the metadata has been interrupted.
However, all these features require the ability of saving an "interrupted" state for a CR.
--

The Data Structures of the algorithms (as defined in <<ACE-algorithms>>) also define the `content[]` array used in the pseudocode.
Architecturally, the `content[]` array gives access to the serialized representation of the content of a CR.

[NOTE]
====
While `content[]` provides a view of (certain fields of the) the content of a CR  that matches the serialized SCC format, it need not be explicitly stored in the ACE unit's memory, and converted to and from the internal representation before exporting and after importing.
...
We start with the algorithm performed by the initialization instruction, which reads an II and uses it to initialize a CR.
...
- Read sequentially and parse the z-8 bytes starting at M+8.
----

[[ACE-SCC-AEAD]]
==== AEAD Algorithm for SCC Export and Import

The AEAD algorithm used to export CRs into SCCs is a slight modification of AES-GCM-SIV cite:[DBLP-journals-iacr-GueronLL17,RFC8452].
We define the Key Derivation and the DCSK Derivation functions first.
...
The generation of a SCC from a CR (cf.{nbsp}<<ACE-SCC-export>>) and the import of a SCC into a CR (cf.{nbsp}<<ACE-SCC-import>>) are defined next.
...
where z : int ← size(SCC) - 8;    // content as serialized "view", size in bytes derived from metadata
ASSERT(8 divides z);
...
[WARNING]
At the end we must add a list of algorithm numbers!
...
[WARNING]
When defining an algorithm, the structure and size of the content of the II and of the serialized content *must* be determinable solely from the Algorithm and Key_Type fields in the metadata.
...
|===
| Value | Mnemonic for the Constant     | State Name        | Examples
|   0   | ace_state_initial             | _initial_           | The context is initialized and ready for operation.
.2+| 1  | ace_state_hash_absorb         | _hash_absorb_       | Start and initialize a hash/tag function of an algorithm and start absorbing blocks of data.
<| ace_state_operate                    | _operate_           | Generic operational state.
...
|   6   | ace_state_enc_last_block      | _enc_last_block_    | For the encryption path of authenticated encryption modes of operation to process the last block separately.
|   7   | ace_state_dec_last_block      | _dec_last_block_    | For the decryption path of authenticated encryption modes of operation to process the last block separately.
|   8   | ace_state_enc_tag_finalize    | _enc_tag_finalize_  | Finalize the tag computation for AE mode encryption.
|   9   | ace_state_dec_tag_finalize    | _dec_tag_finalize_  | Finalize internal tag computation for AE mode decryption, verify the value.
|  10   | ace_state_encrypt             | _encrypt_           | In this state encryption is performed, and also absorption in a tag computation function if part of the algorithm.
|  11   | ace_state_decrypt             | _decrypt_           | In this state decryption is performed, and also absorption in a tag computation function if part of the algorithm.
|  12   | ace_state_set_aux_value       | _set_aux_value_     | Set a public/auxiliary value in an algorithm.
|  14   | ace_state_invalidated         | _invalidated_       | The algorithm has abnormally terminated: +
`ace.error` will always return error code _ace_err_invalid_.
...
Unless explicitly stated otherwise in an algorithm's specification, it is always possible to transition from any state to state _initial_.
...
One such example is automatic transitions from hash finalization to verification, or from hash verification to the _completed_ state (cf.{nbsp}<<ACE-state-constants>> and <<ACE-algorithms>>) to prevent that the user may test several values against an unknown tag and thus potentially gain information about a hashing key.
...
These two instructions can be used if enabled in the implementation and the range size `aceiobuftop-acestart` is non-zero and is a valid size for the algorithm, otherwise an invalid instruction error is raised.

If `aceiobuflen` is non-zero, Forms A, B, and C of `ace.exec`, as well as Forms C of `ace.derive` and `ace.state`, can be replaced as follows:
...
Furthermore, besides ensuring the compactness of a SCC, this rule also guarantees key portability across architectures,
such as between threshold implementations that use different numbers of shares while claiming the same side‑channel protection order.
...
. In the II and in the SCC, the key/main key field will contain the index of the key in the SKT in place of a key value.
...
. For algorithms that require more than one key, such as XEX/XTS, a single index is still used-and thus a single 64-bit value in the II and SCC.
  If the SKT does not contain entries with multiple keys for such algorithms, these algorithms cannot be used with system keys.
 ...
 ** From any state to state _encrypt_, if encryption is allowed, else the CR is *invalidated*.
 ** From any state to state _decrypt_, if decryption is allowed, else the CR is *invalidated*.
* *Behavior:*
...
NOTE: Another ```ace.state K0, #ace_state_initial``` would reset the internal state and allow to begin another decryption with a new IV.
...
does not have to implement the carry-less multiplication (cf.{nbsp}<<ACE-V-dependency>>),
...
For instance, for GCM-SIV (cf.{nbsp}<<ACE-GCM-SIV-mode>>), `nonce` would be included in the II exactly as `IV` is included in the II for GCM, and the transition to _hash_absorb_ would also use a Form A ``ace.state``.
 ...
 ** In state _decrypt_,  only  ``ace.exec`` instructions of the Form A are allowed, namely “```ace.exec OUTPUT, Kn|K[Xn], INPUT```,” to decrypt `INPUT` into `OUTPUT` and then absorb the plaintext `INPUT` into the `tag`:
+
--
     . If `ctr == 0`, then the CR is *invalidated*.
     . If the `SIV` is `b`-bits long, then +
...
transition to state _hash_verify_ and compares the `tag_len` least significant bits of `INPUT` with the `tag_len` least significant bits of `hash_P`:
+
--
  . If the values match, then the state is changed to _completed_, and `ace.error` is set to no error,
  . else the state is changed to _completed_, and `ace.error` returns `ace_err_authentication`.
--
+
...
* *Allowed State Transitions:*
 ** _initial_ -> _hash_absorb_.
 ** _hash_absorb_ -> _hash_last_block_ -> _hash_output_ -> _completed_.
