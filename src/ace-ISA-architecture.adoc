...
| BootSecret                | Identifies the boot session of the host hardware and its software stack. Not permanent, to be configured at physical boot. | OS | M | 6
| VBootSecret               | Regenerated at each (virtual) boot. Not permanent, to be configured at the virtual boot of each VM, or also per SD. | OS | H | 7
...
M-Mode and S-Mode reconfigure (virtualize) VBootSecret, BootSecret, and SSecret, by writing to groups of CSR which are defined in <<ACE-CSR-maceBootSecret>>, <<ACE-CSR-maceVBootSecret>>, and <<ACE-CSR-saceSSecret>>.
...
ACE extends `*status` and `misa`, and adds up to 11 new CSRs, which are listed in <<ACE-CSRs-table>>.
...
This would limit the number of allocated CSRs to at most 11.
...
* `Xs`[11:0] and `Xs`[27:16] must be 0.
