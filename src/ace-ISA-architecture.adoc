...
===== Tracking CC dirtiness: aceccactive and aceccdirty
...
* CSRs `ace_OS_Secret0` and `ace_OS_Secret1`, to configure the "OS_Secret" entry in RV64, to which RV32 adds `ace_OS_Secret2` and `ace_OS_Secret3`.
* CSRs `ace_Boot_Session0` and `ace_Boot_Session1`, to configure the "Boot_Session_Secret" entry in RV64, to which RV32 adds `ace_Boot_Session2` and `ace_Boot_Session3` (cf.{nbsp}<<ACE-Localities>>).
...
NOTE: User mode, and even operating systems and hypervisors, need not be aware of the `ace_OS_Secret*` and `ace_Boot_Session*` blocks. Other mechanisms could have been chosen to program the "OS Secret" and  "Boot Session Secret" entries, such as an MM-exclusive dedicated hardware interface or memory-mapped interface.
