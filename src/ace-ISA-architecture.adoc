...
The metadata section's format is fixed, and it specifies the cryptographic algorithm associated with the CC and optional usage control policies.
...
This binding uses metadata when a CC is in a CR, and augments _Root Context Sealing Key_ (RCSK) by a Locality-specific _tweak_.
...
* `Zlv` => Requires V (or a suitable subset thereof). Must support for the following CSRs: `aceerror` and `acestart` (this is a dependency for Zicsr); must support the following instructions: `ace.provision`, all four forms of `ace.exec`, `ace.clear`, `ace.export`, `ace.import`, and `ace.size`; must support `Zleaes128` and `Zleaes256` in ECB and GCM-SIV modes.
* `Zlio` => Must support for the following CSRs: `aceerror`, `acestart` (this is a dependency for Zicsr), `aceiobuflen` and `aceiobuftop`; must support the following instructions: `ace.provision`, Form D of `ace.exec`, `ace.input` and `ace.output`, `ace.clear`, `ace.export`, `ace.import`, and `ace.size`; must support `Zleaes128` and `Zleaes256` in ECB and GCM-SIV modes.
...
* `Smacebootscrt` => Defines the ACE Boot Secret CSRs (`haceVBootSecret0{0..3}`).
* `Smacestatus` => Defines the ACES per CC CSRs (`scrdirty`, `vscrdirty`, `mcrdirty`).
...
From Sdtrig we reuse the `scontext`, `mcontext`, and `hcontext` CSRs. Sdtrig is not required per se.
...
If ACE is implemented on vector registers, it does not require the full V extension; it depends on a minimal subset, which we call RVV-mini.
(((RVV-mini)))
...
* Clear data is loaded into CR using the `ace.provision` instruction (<<ACE-instruction-init>>), but
...
* The amount of the capacity of the CRF used by a CR depends on the configured algorithm.
  ...
  Insufficient residual CRF capacity may prevent the execution of a CR *configuration* instruction, i.e. `ace.provision`, `ace.import`,
...
[NOTE]
====
The management operations include the *configuration* instructions (`ace.provision`, `ace.import`, `ace.clone`, and `ace.derive`) and the export operation (`ace.export`).
====
...
. The Sealed Cryptographic Context (SCC) (<<ACE-format-SCC>>), an encrypted, authenticated format used when exporting a CR to memory for later re-import.

Both formats rely on the same metadata section format, which is described here.
The PI and SCC formats are described in <<ACE-provision-export-import>> and <<ACE-format-SCC>>, respectively.


==== Metadata Format

[WARNING]
The metadata section of a CC is used as the header of the data formats used in CR provisioning, and SCC export and import functions.
Its definition is in flux, as it is subject to change based on the ACR feedback.
For instance, if a less expensive key derivation method is allowed (<<ACE-SCC-AEAD>>), we could rely entirely on the cryptographic approach based on localities, and eliminate the filtering acording to the IDs of processes VMs and Hypervisors/SDs. In this case, bits [63:29] could be repurposed, and an expiration data field could be made smaller.

{empty}
(((Cryptographic Context,metadata,format)))
The metadata section is 64, 128, or 192 bits long.
The metadata section is used as the header of the data formats used in CR provisioning, and SCC export and import functions.
...
|   [0:0] |   1   | Format                   | Value is 1 if the optional metadata extension section (ACE-metadata-extension) is present, else 0.
|   [1:1] |   1   | _LengthExt_              | Value is 1 is there is an additional block of data at the end of content, in which case the medata is extended with a block
|  [11:2] |  10   | _Algorithm_              | Cryptographic algorithm (such as primitive, mode, or protocol).
| [13:12] |   2   | _DirectionPolicy_        | For encryption/decryption primitives or modes, determines whether the CC may be used for encryption (bit 11 set), decryption (bit 12 set), or both (bits 11 and 12 set). The field can be reused to expand the algorithm space for other types of operations.
| [14:14] |   1   | _SCProtection_           | Side-Channel Protected Implementation (0 if not protected, 1 if protected).
| [15:15] |   1   | _KeyType_                | Key given by explicit value / System Key (index).
| [19:16] |   4   | _StateNumber_            | Encoding the current state of the state machine of the algorithm as a number.
| [23:20] |   4   | _Locality_               | A field which is either all ones (`0xF`) or an index into the Locality Secrets Table.
| [25:24] |   2   | Reserved                 | Value is 0 for the current metadata formats. (Expansion of Locality?)
| [28:26] |   3   | _UCPolicy_               | “UC Policy” means “Usage Control Policy”. If bit 25, 26, resp., 27 is set, it disallows the _usage_ of this CC in User, (V)S-mode, resp., H-mode.
| [32:29] |   4   | _FilterMode_             | Filter enablement:
bits 0, 1, resp., 2 = enable the filter below S/VS-mode, H-mode, resp., M-mode.
bit 3 = enable expiration date (minutes since January 1^st^, 2027). +
For the 64-bit metadata block, in general only one bit can be set but, see <<ACE-combined-filters>>.
| [63:33] |  31   | _Filter1_                | Value to match filters against (see <<ACE-combined-filters>> for details), including an internal secure clock. +
An alternative format for these fields is described in <<ACE-metadata-split-filter1>>.
|===

[[ACE-metadata-split-filter1]]
.Format of the Filter1 field of the Metadata section for two short filters
[width="100%",cols="^9%,^7%,16%,68%",options="header"]
|===
|   Field | Width | Name                     | Description
| [46:33] |  14   | _Filter1A_               | Value to match against `mcontext`.
| [48:47] |   2   | Zero                     | Zero.
| [62:49] |  14   | _Filter1B_               | Value to match against `hcontext`.
| [63:63] |   1   | Zero                     | Zero.
|===
...
|===
|   Field | Width | Name                     | Description
|   [0:0] |   1   | _Format2_                | Value is 0 for the current 128-bit metadata format.
|  [31:1] |  31   | _Filter2_                | Value to match against an expiration date.
| [32:32] |   1   | Reserved                 | Reserved for future use.
| [46:33] |  14   | _Filter3A_               | Filter to match against a `mcontext` field.
| [48:47] |   2   | Reserved                 | Reserved for future use.
| [62:49] |  14   | _Filter3B_               | Filter to match against a `hcontext` field.
| [63:63] |   1   | Reserved                 | Reserved for future use.
|===

[[ACE-metadata-extension-data]]
.Format of the optional metadata additional data length section
[width="100%",cols="^9%,^7%,16%,68%",options="header"]
|===
|   Field | Width | Name                     | Description
| [31:0]  |  32   | _Length_                 | Length of the additional block at the end of the content section.
| [63:32] |  32   | Reserved                 | Reserved for future use.
|===
...
Bits [32:0] are not confidential and will not be encrypted upon export as an SCC. +
Bits [63:33] are confidential and will be encrypted upon export as an SCC.
...
The DirectionPolicy field is defined only for algorithms that support both encryption and decryption. For algorithms that do not, such as hash functions or some public-key operations, these two bits are not used for selecting the primitive's direction and may instead encode additional algorithm identifiers, thereby expanding the available encoding space of the Algorithms field.
...
// In such a case, the value of the SCProtection field must be set to 3.

===== KeyType Field

ACE supports keys provided by software, as well as system keys and keys computed with a system “recipe”, i.e., _system generated keys_.
...
System Keys are given by an index into an internal, system-specific System Keys Table (SKT). For each algorithm as specified in <<ACE-algorithms>>, the part of the content that normally contains the key(s) is replaced by its index into the SKT-for both PI and SCCs. The actual value of the key is never included in exported content. For threshold implementations, ACE may copy the key material from the SKT into the CR and expand it into multiple shares.

System Generated Keys are also system-specific. In place of the PI, the provisioning operation reads a system-specific “recipe”, which is not portable. After internal key generation, the actual key value—not the recipe—is stored in the CR and SCC. The KeyType field in the metadata is 0.

//

===== StateNumber Field

The StateNumber field must be zero in a PI.

Some values of StateNumber are architected, and are defined in <<ACE-state-constants-fixed>>.
...
|   0   | _ace_state_initial_             | _initial_           | The context has just been provisioned or reset, and ready for operation.
|  14   | _ace_state_invalid_             | _invalid_           | The algorithm has abnormally terminated or provisioning/import have been interrupted.
...
If an algorithm defines 16 or fewer states (including the four architected ones), the current state number may be stored in the StateNumber field.

For algorithms with more than 16 states, an algorithm-specific representation is used in the CR content and exported in the SCC.
How to use information is up to the algorithm designer, with the following condition:
StateNumber must always be set to _initial_, _invalid_, and _completed_ if the state of the algorithm corresponds to these states.

The state values for the symmetric algorithms defined in this specification are listed in <<ACE-state-constants-symmetric>>.

During init and import, the StateNumber field is set to _invalid_, and only at the end of the operation is it set to proper value, i.e., _initial_ or the value in the SCC (see <<ACE-provision-export-import>>)
This means, by the rules in <<ACE-invalidation>>, that a partially provisioned/imported CR cannot be exported. In particular, if a context switch occurs, that the more privileged code must unconfigure the CR, and upon resumption, the instruction will be restarted.  Only an invalid CR with an `acestart` different from zero is recognized as an incomplete CR allowing an init or import operation to resume.
...
However, if any value of the state (such as a counter, or a partial hash value) is altered between the interruption and resumption of an export operation, the resumed export may use an incorrect SIV for the remaining SCC data, causing a subsequent import to fail authentication. This is a general issue with CRs, since more privileged code can use them unless blocked by the UCPolicy. We do not consider this a security threat: it affects only SCC availability, not confidentiality or integrity,
...
ACE keeps a table of 128-bit [.red]#(maybe shorter)# Locality Secrets (LS), the Locality Secrets Table (LST).
...
Otherwise, this value is used as an index _i_ in the LST, and the encryption depends on both the RCSK and LST[_i_].
<<ACE-locality-indexes>> lists the architected Localities and which mode is allowed to configure them.
The domain of each Locality Secret is specified, i.e., whether the value is chip-wide (i.e., the same across all chiplets in a SoC), device-wide, or OS-wide.

[[ACE-locality-indexes]]
.Architected Locality Values
[width="100%",cols="20%,53%,^9%,^9%,^9%",options="header"]
|===
| Name                      | Description                                                                                       | Domain | Conf.. | Value
| (No Locality binding)     | No CSK derivation. CC bound to Root CSK only.                                                              | —  | — | 0xF
| _ChipDesignerSecret_      | Identifies the designer of the SoC. Permanent.                                                             | SoC | No | 0
| _ChipModelSecret_         | Identifies specific chip model/family. Permanent.                                                          | SoC | No | 1
| _ChipUniqueSecret_        | Unique for each SoC. Permanent.                                                                            | SoC | No | 2
| _OEMSecret_               | Provisioned by OEM to differentiate their products from the competition.
                              Permanent, or reconfigurable via an implementation-defined HW authenticated mechanism.                     | Device | No | 3
| _ProductSecret_           | Identifies device/system type/model (product). Permanent or reconfigurable like OEMSecret.                 | Device | No | 4
| _DeviceSecret_            | Unique device (entire system) secret (_not_ the device ID such as a serial number). Shared across all harts in a SoC and shareable across multiple SoKs in a single device. Sharing mechanism implementation-defined, i.e., not architected. Reconfigurable like OEMSecret.                                  | Device | No | 5
| _BootSecret_              | Unique per the boot session of the host hardware and its software stack. Configured by M-mode during the boot process of the physical device (see <<ACE-CSR-maceBootSecret>>). | Device | M | 6
| _VBootSecret_             | Unique per the boot session of the virtualized hardware and its software stack.
Regenerated by H or higher at each virtual boot of a VM (see <<ACE-CSR-haceVBootSecret>>).                                               | VM | H/M | 7
| _SSecret_                 | Can be configured by Supervisor (see <<ACE-CSR-saceSSecret>>).                                             | Process  | S | 8
| _Reserved_                | Reserved for future use.                                                                                   | Impdef | Impdef | 9-14
|===

The only restriction on the values of the LSs is that they must be randomly generated and be non-zero.

NOTE: Example: If a key's metadata specifies a Locality field value of 2, the import/export key is derived from the RCSK and the OEM Secret.
An SCC created on one device will fail authentication when imported on another device if their OEM Secrets differ.

NOTE: A CC may reference a key by an index into the System Key Table (SKT).
...
Implementations of the SKT *must* provide a suitable Locality to the ACE unit during CR provisioning, and set usage policies that are at least as strict as the key's own.

[WARNING]
Locality secrets may be combined. Suggestion: just add them for the purpose of creating the tweaks to be used in the key derivation process, as, for instance, allowing four to be added together reduces the distinguishability of a sequence or random numbers by just two bits (from 128 to 126)—and these values are never observed directly.
The proposal for combination would be to encode 0-6 in three bits (and an ignore value of 7), then having a secret per SD/hypervisor, _VBootSecret_ and _SSecret_, selectable each with one bit, so a total of 6 bits. Note that the secret for M-mode (_BootSecret_), is in the first group.

[WARNING]
An extension field for the Locality field may be provided in the 128-bit (or longer) metadata format, if defined.
...
===== FilterMode and Filter Fields

[WARNING]
The function of these fields must be finalized. In particular, the additional Filter fields and the 64-bit metadata expansion block are not yet agreed upon.

To filter by time is a simple comparison between the current time and the expiration time in the metadata.
The ACE Unit must have access to a secure clock.
When the current time is beyond the expiration time in the metadata, the ACE Unit invalidates the CR (see <<ACE-invalidation>>), and the `aceerror` CSR takes the value _ace_err_expired_CR_ (see <<ACE-CSR-ace-error>>).
...
If the Filter field is used to match against `mcontext`, `hcontext` or both, its 31 bits are formatted as follows:


In this case, bits 1 and 2 of FilterMode can be set.

Other combinations of more than one bit set in FilterMode are only allowed if the metadata type is set for 128-bit metadata, i.e., if the Format bit in the metadata is set to 1.
In this case, if Filter1 is used to match against `scontext` or an expiration date, and we have at least one other filter, then the format of the extension section is given in the following table.


The _Format2_ field is not encrypted.

If both filtering according to `scontext` and an expiration date are required, then `scontext` is encoded in the standard metadata block and the expiration date is encoded in the extension section.
...
The subsequent bytes form the *content*—the key material and any algorithm-specific data—whose format and length are specified in <<ACE-algorithms>> for all architected algorithms. The PI content's structure and size are entirely determined by the Algorithm and KeyType fields within the metadata.

NOTE: The actual resource usage of a CR may be larger than the size of a PI.
In the simple ECB schemes (<<ACE-ECB-mode>>) not hardened against SCA, a CR need not contain more than the metadata and the key, hence their resource usage matches the size of the PI (unless the key is not a multiple of 64 bits).
However, several algorithms need to compute and cache additional data once initialized, and their internal state will be larger than the size of the PI.
...
The metadata section of an SCC follows the same fixed architectural format as the metadata section of the PI.
The content format—and consequently its length—are algorithm-specific (<<ACE-algorithms>>) and are determined solely by the Algorithm, KeyType, and DirectionPolicy fields in the metadata.
...
| (Indirect)     | MRW (RZ)  | `maceBootSecret{0..3}`  | Locality: Physical Boot Session Secret
| (Indirect)     | HRW (RZ)  | `haceVBootSecret{0..3}` | Locality: Virtual Boot Session Secret
| (Indirect)     | SRW (RZ)  | `saceSSecret{0..3}`     | Locality: Supervisor Secret
|===

[WARNING]
====
* To configure the `macercsk`, `maceBootSecret`, `haceVBootSecret`, and `saceSSecret` CSR groups we use the Indirect CSR Access defined in cite:[RISCV-ISA-Priv] instead.
...
Writes to any ACES field do not affect the contents of the CRs, of the ACEIOBUF, or of any ACE-specific CSRs.
...
* If neither `vsstatus.ACES` nor `mstatus.ACES` is set to Off, executing any instruction that changes ACE state, including the CRs or the ACEIOBUF, changes both `mstatus.ACES` and `vsstatus.ACES` to Dirty.
...
|  0   | _ace_err_ok_                 | No error.
|  1   | _ace_err_import_auth_        | Authentication failure with `ace.import`.
|  2   | _ace_err_unconfigured_       | Software used an unconfigured or _invalid_, but not expired, or partially configured CR as a source in any instruction except `ace.size`. +
Software used an unconfigured ACEIOBUF as an input or an output.
|  3   | _ace_err_invalid_CR_         | Attempt to use an invalid CR.
|  4   | _ace_err_expired_CR_         | Attempt to use an expired CR.
|  5   | _ace_err_memory_             | Insufficient CRF memory for configuration instruction.
|  6   | _ace_err_unimplemented_      | Not implemented algorithm or variant.
Not allowed state number in `ace.state`, including requesting encryption or decryption from a CC not allowing it.
|  7   | _ace_err_invalid_value_      | Invalid value, except _ace_err_unimplemented_ (see below).
|  8   | _ace_err_privilege_          | Violation of usage control policies in metadata (including Filters).
|  9   | _ace_err_auth_               | Authentication failure in an algorithm.
|===

ACE error _ace_err_invalid_value_ collects various cases, for instance (but, there could be more):

* Invalid metadata section in `ace.provision` or `ace.import`, other than ace_err_unimplemented, for instance:
 ** Non-zeros in zero fields,
 ** Invalid FilterMode for a 64-bit metadata block,
 ** Non-implemented Locality value,
 ** StateNumber is non-zero for `ace.provision`.
...
If a Form of `ace.exec`, `ace.state` or `ace.derive` that is not permitted in the current algorithm state is used, then an illegal instruction exception is raised.
...
|           .>| 5, 4          .>| Invalid metadata section in `ace.provision` or `ace.import`, including specifying a not implemented algorithm or side-channel protected variant.
...
`acestart` is an XLEN-bit RW CSR, to track progress of long-running interruptible instructions such as `ace.provision`, `ace.import`, `ace.export`, `ace.input`, and `ace.output`, as well as `ace.exec` on long inputs.
(((CSR, acestart)))
...
* `_ace.provision_` _and_ `_ace.import_` _use a single pass: all blocks are processed sequentially._
...
The semantics and use of `acestart` are described in <<ACE-provision-export-import>>.
...
If `aceiobuflen` is zero, the ACEIOBUF is _unconfigured_ (it does not hold any content, similarly to unconfigured CRs, i.e., CRs in state Initial), and `ace.input` and `ace.output` raise ACE error _ace_err_unconfigured_.

Writing to `aceiobuflen` may fail if the value is larger than the maximum allowed by the architecture, and raises ACE error _ace_err_memory_.
...
ACE error _ace_err_memory_ is raised, but reading `aceiobuflen` will return the actual length, on which software can therefore act.
...
If the total length of the ACEIOBUF is not fixed, the same operation will also raise ACE error _ace_err_memory_, and cause `aceiobuflen` to read as zero.
...
Setting a larger value raises ACE error _ace_err_invalid_value_.

`aceiobuftop` must be {ge} `acestart`.
Setting a smaller value raises ACE error _ace_err_invalid_value_.
...
Using the ACEIOBUF while `aceiobuftop`-`acestart` is zero (`aceiobuflen` being non-zero) or an invalid value raises ACE error _ace_err_invalid_value_.
...
// Reconfiguration would necessitate zeroing the buffer to prevent data leakage and could incur a performance penalty if internal resizing occurs.
// The purpose of the ACEIOBUF is not to serve as a general scratchpad memory for cryptographic operations.
...
The XLEN-bit register `scrdirty` tracks the dirtiness of CRs on a per-CR basis.

Any access to `scrdirty` outside of M-mode causes an illegal instruction exception
...
The state of an individual CR in `scrdirty` after `ace.clear` or `ace.reset` (<<ACE-instruction-clear>>) is Clear.
However, `ace.clear` makes the `*status.ACES` field Dirty, to signal higher privileged code to erase any older saved state for unconfigured CRs during context switch.

When ``scrdirty[__i__]`` is set to Clean, executing any instruction that may change the CR #__i__ will change ``scrdirty[__i__]`` to Dirty.

NOTE: Implementations may also change ``scrdirty[__i__]`` from Initial or Clean to Dirty at any time, even when there is no change in CR #__i__ state.
...
NOTE: T1he `scrdirty` CSR does not depend on the S extension.  In an M-U system, M-mode can use the CSR to optimize U-mode context switches.

NOTE: If we implement lazy-loading in the architecture at some point, `scrdirty` may be made into a 64-bit register to track the laziness status of individual CRs in the upper 32 bits.
When XLEN=32, `scrdirtyh` would be a 32-bit register that aliases bits [63:32] of `scrdirty`, and `scrdirtyh` would not exist when XLEN=64.

[[ACE-CSR-vscrdirty]]
===== `vscrdirty`

The `vscrdirty` register exists only when the H extension is implemented.

The XLEN-bit register `vscrdirty` tracks the dirtiness of CRs on a per-CR basis.

Any access to `vscrdirty` outside of S-mode or higher causes an illegal instruction exception

NOTE: Similarly to `scrdirty`, if we implement lazy-loading in the architecture at some point, `vscrdirty` may be made into a 64-bit register to track the laziness status of individual CRs in the upper 32 bits.
When XLEN=32, `vscrdirtyh` would be a 32-bit register that aliases bits [63:32] of `vscrdirty`, and `vscrdirtyh` would not exist when XLEN=64.

When V=1, the following holds:

* ``vscrdirty[__i__]`` can take the values Clean (0) or Dirty (1), as defined for `scrdirty`, for CR #__i__.
...
NOTE: `sstatus.ACES`, `vsstatus.ACES`, and `vscrdirty` are also in effect when V=1.
...
In this case, the `ACE` and `ACEIOBUF` fields in `sstatus` shall be unshadowed.
...
NOTE: CR status propagates from the least privileged modes to the higher privileged ones.
Any instruction that changes bit \#__i__ of, say `vscrdirty` to Dirty, will also set the corresponding bit of `scrdirty` and `mcrdirty`.
Any instruction that changes bit \#__i__ of, say `scrdirty` to Dirty, will also set the corresponding bit of `mcrdirty`.
...
Any access to `macercsk` outside of M-mode causes an illegal instruction exception.
...
NOTE: Alternative approaches to re-configuring the RCSK are allowed.
...
An advantage of this method is that it also shields the values of the RCSKs from M-mode, which only would request a reconfiguration using a token or a handle.
A possible disadvantage could be reduced performance.
...
Any access to `maceBootSecret` outside of M-mode causes an illegal instruction exception.

//

[[ACE-CSR-haceVBootSecret]]
===== `haceVBootSecret` Group

This is a group of H- and VS-Mode WARZ XLEN-bit CSRs used to configure `haceVBootSecret`.
These are addressed using Indirect CSR Access.
(((CSR, `haceVBootSecret`)))
In RV32 there are four CSRs.
In RV64 there are two CSRs.

A write to the __i__-th CSR sets Bits [(_i_+1) XLEN - 1 : _i_ XLEN] of `haceVBootSecret`.

Any access to `haceVBootSecret` outside of VS-mode and higher causes an illegal instruction exception.

[NOTE]
====
`haceVBootSecret` must be reconfigurable in order to support VM reboots which are independent of the boot cycles of the physical host.
====
...
This is a group of S-Mode WARZ XLEN-bit CSRs, used to configure `saceSSecret`.
(((CSR, `saceSSecret`)))
In RV32 there are four CSRs.
In RV64 there are two CSRs.
...
Any access to `saceSSecret` outside of S-mode or higher raises an illegal instruction exception.
...
`sacefilter` is an XLEN-bit SRW CSR (actual length to be defined).

`vsacefilter` is an XLEN-bit HRW CSR (actual length to be defined).

`macefilter` is an XLEN-bit MRW CSR (actual length to be defined).
...
[WARNING]
These three registers are placeholders for a later decision on how to handle the CR filtering in different modes—taken in accordance with the ARC.
One option could be to use `scontext`, `mcontext`, and `hcontext` from the Sdtrig extension, and in <<ACE-combined-filters>> we use the sizes of these CSR
as an example.
...
We currently use `custom-0` for load-type instructions, `custom-1` for store-type instructions, and `custom-2` for operational instructions (with bits [1:0] always equal to 3-all instructions are 32 bits long), to align with store/load/operational instructions in the base and floating-point instruction sets.
...
When a GPR is used to index a CR, use of the zero register `X0` gved and results in an illegal instruction exception.

For each instruction, we specify whether:

* It _possibly modifies state_;
* It is _usage-controlled_, i.e., whether the instruction is subject to usage control, which determines which software entities can use it (by _UCPolicy_ and non-time-based _Filter*_);
...
`ace.provision Kd|K[Xd], %offset(Xs)`
(((ACE, instruction, ace.provision)))

Encoding::
`ace.provision` is an I-type instruction.
+
[wavedrom, , svg]
....
{reg:[
    ...
    { bits:  3, name: 'ace.provision', attr: [0x4,0x5] },
    { bits:  5, name: 'Xs' },
...
Description::
`ace.provision` provisions a CR with metadata and content to create a CC.
...
+
Similarly to vector store instructions, `ace.provision` appears to other harts as a sequence of individual reads.
...
NOTE: `ace.provision`, as well as `ace.import`, `ace.export`, `ace.input`, and `ace.output` as described later, perform direct memory accesses.
This does not violate the RISC-V load/store architecture principles,
since these instructions can be viewed as special kinds of load and store instructions
// as ACE is architected to behave like an _attached_ unit, which may have its own interfaces.

The operation is described in detail in <<ACE-provision-export-import>>.
...
+
`ace.clear` unconfigures the state of the relevant CR by releasing its internal resources in the CRF and setting the corresponding entry in `*crdirty` to Dirty unless the CR was already unconfigured, in which case it does nothing.
...
If the context is unconfigured, ACE error _ace_err_unconfigured_ is raised.
If `Xs` is out of range, ACE error _ace_err_invalid_value_ is raised.
+
The instruction must follow the interruptibility rules defined in <<ACE-CSR-acestart>> and <<ACE-provision-export-import>>.
...
(`ace.import`).
+
`ace.import` must follow the interruptibility rules defined in <<ACE-CSR-acestart>> and <<ACE-provision-export-import>>.
...
The new state number, if allowed, is written into the StateNumber field of the metadata, unless the algorithm requires more than 16 states, in which case an ad hoc field must be defined in the context (for both internal representation and SCCs).
...
Encoding::
The encoding shares `func3` with `ace.exec`, however with bit 25 (the lsb of `func7`) equal to 1 and the two msbs of `func7` equal to `00`.
+
[wavedrom, , svg]
....
{reg:[
...
* Metadata type `ace.size` returns the total size (in bytes) of an SCC having the metadata section given in `Xn` into GPR `Xd`, and 0 if the algorithm is not supported.
+
...
* Internal Capacity `ace.size` returns the amount of internal CF capacity used by a CR with metadata section matching `Xn` on the Algorithm, SP_Protection, and KeyType metadata fields. In this case, all other bits must be zero.
If the combination of Algorithm, SP_Protection, and KeyType fields is not supported, `ace.size` returns 0.
If, instead, only the StateNumber is non-zero and its value is 15, resp., 14, `ace.size` returns the total, resp., the remaining internal capacity of the CRF. +
...
* ACEIOBUF type. The source register contains the desired byte length of the buffer to be allocated, and the destination register contains the size of the buffer in the ACE unit.
...
* `Xs`[11:0] and `Xs`[24:15] must be 0, else ACE error _ace_err_invalid_value_ is raised.
* `Xs`[14:12] and `Xs`[63:26] have the same format as the corresponding bits of the metadata.

* If the algorithm in the CR supports encryption and decryption, then:
** `Xs`[13:12] is used to deactivate either encryption or decryption in a CC that allows both.
** If `m`[13:12] = `0b11`, then at least one bit of `Xs`[13:12] must be set to 1 to select the allowed operation(s).
If `Xs`[13:12] = 0, then ACE error _ace_err_invalid_value_ is raised.
** If `m`[13:12] != `0b11`, then it must be `Xs`[13:12] = `m`[13:12] or `Xs`[13:12] = 0, in which case no change is made, else ACE error _ace_err_invalid_value_ is raised

* If the algorithm in the CR does not support encryption and decryption, then `Xs`[13:12] must be 0, else ACE error _ace_err_invalid_value_ is raised
* `Xs`[14] is used to turn on side-channel protection, if implemented.
Removing the side channel protection level is not permitted. Attempting to do so raises ACE error _ace_err_invalid_value_ (<<ACE-error-codes>>).

* A zero in any of the three bits `Xs`[28:26] means that the corresponding policy is _not_ changed.
* A one in any of the three bits `Xs`[28:26] means that usage by the corresponding mode is forbidden, and the corresponding bit in the metadata of the destination CC is set to 1.
* `Xs`[32:29] can be non-zero only if `m`[32:29] is 0, in which case  `m`[32:29] <- `Xs`[32:29], else ACE error _ace_err_invalid_value_ is raised
* This instruction cannot set a Locality.
--
+
On RV32, `s` must be even, the register pair `X[s]` and `X[s+1]` are read, and `(X[s+1] @ X[s])` replaces `Xs` in the RV64 description.

NOTE: Setting multiple bits of `Xs`[32:29] may require a change of the metadata format from 64 bits to 128 bits.
...
If `Xl` is greater than `aceiobuftop` - `acestart`, ACE error _ace_err_invalid_value_ is raised.
+
...
[[ACE-provision-export-import]]
=== Provisioning, Export and Import of Cryptographic Contexts

From the point of view of the system outside the ACE unit, `ace.provision` and `ace.import` appear as a series of sequential memory reads, and `ace.export`, appears as a series of sequential memory writes.
...
The privileged interrupt handler must ensure that the instruction is restarted only in this situation, and in any other circumstance, including context switching, the instruction must be restarted, in which case if the instruction is `ace.provision` or `ace.import` it must also clear the CR.
...
. `PI_content_size(x)` returns the size in bytes of the content section of a PI. It depends only on the 11 bits `KeyType @ Algorithm`.
. `SCC_content_size(x)` returns the size in bytes of the content section of an SCC. It depends only on the 13 bits `DirectionPolicy @ KeyType @ Algorithm`.

The total size of a PI is therefore `MD_size(x)` + `PI_content_size(x)`, and the total size of an SCC is `MD_size(x)` + `SCC_content_size(x)`.

[[ACE-provision]]
==== Provisioning

We start with the algorithm performed by `ace.provision`, which reads a PI and uses it to provision a CR.

[[ACE-provision-listing]]
...
[WARNING]
The reason this algorithm has been chosen is that it only required single passes of data in main memory, reducing the number of external memory accesses.
...
(or 126 with <<ACE-SCC-alt-derivation>>). An alternative key derivation <<ACE-SCC-alt-derivation>> method halves the number of AES256 calls by using, essentially, the CMAC based random generation, which is secure for up to 2^64^ key generations with the same RCSK, and then the security gracefully degrades (after 2^96^ keys there is still an overall security level of 128 bits).
...
[]
----
function KeyDeriv(key : bits(256), L : bits(125)) = {
  struct {
    AESE256(key, (L @ 0b101))[63:0] @
    AESE256(key, (L @ 0b100))[63:0] @
    AESE256(key, (L @ 0b011))[63:0] @
    AESE256(key, (L @ 0b010))[63:0],
    AESE256(key, (L @ 0b001))[63:0] @
    AESE256(key, (L @ 0b000))[63:0]
  }
}
----
...
[]
----
function KeyDeriv(key : bits(256), L : bits(126)) = {
  precomputed: t = AESE256(key, zeros(128)) * 2; // using the field multiplicatoin
  struct {
...
if (metadata.Locality == 15) then
    {enc_key, auth_key} = KeyDeriv(CSK, zeros(125));
else
    {enc_key, auth_key} = KeyDeriv(CSK, LST[metadata.Locality]);

metadata.StateNumber ← ace_state_initial
tmp ← POLYVAL(auth_key, zeros(64) @ metadata @ content[]);
SIV ← AESE(enc_key, 0 @ tmp[126:0]));

metadata.StateNumber ← ace_state_invalid
...
* If an import operation is preempted, the hardware must save `metadata.StateNumber` and set +
`metadata.StateNumber <- ace_state_invalid` before relinquishing control.
* When control is returned, the previous value of `metadata.StateNumber` is restored.
* There must be a 4-bit internal temporary state containing the original value of `metadata.StateNumber` and a flag indicating whether the value has been saved.
* If, upon returning to the interrupted process, the bit signaling that a value for `metadata.StateNumber` has been saved is unset, the operation is restarted.
* The first read is the read of the metadata block itself:
 ** If fewer than 4 bytes are read before the instruction is interrupted, `acestart` = 0, so the instruction will be restarted.
 ** If at least 4 bytes have been read, the original value of `metadata.StateNumber` is known, so the instruction may be restarted with `acestart` {ge} 4.
...
saved_SN : bits(4) ← metadata.StateNumber
metadata.StateNumber ← ace_state_invalid
...
if (tag != SIV) then { aceerror ← ace_err_import_auth; return }
...
If an invalid input is provided to any ACE instruction, or an instruction that is not explicitly allowed in a given state is executed, while processing the algorithm in CR, say, `Kn`, then `Kn` is *invalidated*, namely, the following happens:

* `Kn` is put into state _invalid_;
* For instructions that refer to `Kn`:
 ** `ace.provision`, `ace.clear` (thus also `ace.reset`), `ace.import`, `ace.export`, `ace.size` and `ace.available` work normally;
 ** `ace.exec`, `ace.clone`, `ace.derive`, and `ace.restrict` raise an ACE error _ace_err_invalid_CR_;
 ** `ace.input` and `ace.output` raise an ACE error _ace_err_invalid_CR_ when `aceactivecr` is `Kn`;
 ** `ace.state` raises an ACE error _ace_err_invalid_CR_, unless it is used to return to state _initial_;
 ** `ace.error` always returns error _ace_err_invalid_CR_ after any operation involving the CR except for `ace.import` and `ace.export`, `ace.clear`, `ace.provision`, and `ace.size`, which all work normally, and using `ace.state` to return to state _initial_.
* The current instruction is terminated.

In State _completed_, only `ace.provision`, `ace.export`, `ace.import`, `ace.clear`, and `ace.error` are permitted.
...
Instructions `ace.provision`, `ace.export`, `ace.import`, `ace.input`, and `ace.output` support long inputs and outputs.
...
// Interrupting the firmware implementation of `ace.provision`, `ace.export`, `ace.import`, `ace.input`, or `ace.output` must leave the architectural state indistinguishable from the state expected from interrupting a monolithic implementation of the instructions.
...
. The code is written without considering the exceptions to be caught, usually only _ace_err_invalid_ or _ace_err_expired_.
. If the code runs without such errors, then the two CSRs are reset and the routine returns normally.
. If, say, an _ace_err_invalid_ or _ace_err_expired_ error is raised, the horizontal trap diverts the control flow to the catch code, which usually cleans up, and returns to the caller of the subroutine, not to the routine itself, with an error code.
...
. *Provisioning Input*:
The data following the metadata block in the PI to `ace.provision`, such as keys and other required secrets.
...
. *Serialized Content*:
The data that follows the metadata block in the plaintext portion of an SCC.
...
If a non-allowed state transition is requested in `ace.state`, including requesting encryption or decryption from a CC not allowing it, an ACE error _ace_err_unimplemented_ is raised.  This also invalidates the CR.

If a non explicitly allowed `ace.exec`, `ace.state` instruction or form thereof is issued, an illegal instruction exception is raised and the CR is invalidated.
...
`ace.input` and `ace.output` may be used if enabled and `aceiobuflen` is non-zero, otherwise an ACE error _ace_err_unconfigured_ is raised.
If `aceiobuflen` is non-zero, and the range size `aceiobuftop-acestart` is zero or an invalid size for the algorithm, the `ace.exec` operation is not performed and ACE error _ace_err_invalid_value_ is raised.
...
In all algorithms, whenever one of the instructions in <<ACE-replacement-instructions>> occurs, it may be substituted by a replacement sequence.
We do not write this explicitly in the algorithm descriptions for brevity.
...
. In a PI and in an SCC, the key field will contain the index of the key in place of the value.
...
. For algorithms that require more than one key, such as XEX/XTS, a single index is still used to an entry in the SKT that contains multiple keys—and thus a single 64-bit value in the PI and SCC.
...
|===
| Value | Mnemonic for the Constant       | State Name          | Description
.2+| 1  | _ace_state_operate_             |  _operate_          |Generic operational state.
<| _ace_state_hash_absorb_         | _hash_absorb_       |  Start and initialize a hash function of the tag/MAC part of an algorithm and start absorbing blocks of data (in the case of an AEAD algorithm, this usually is the associated data).
...
|   6   | _ace_state_encrypt_             | _encrypt_           | In this state encryption is performed, and an update to the tag computation if part of the algorithm.
|   7   | _ace_state_decrypt_             | _decrypt_           | In this state decryption is performed, and an update to the tag computation if part of the algorithm.
|   8   | _ace_state_enc_last_block_      | _enc_last_block_    | For the encryption path of authenticated encryption modes of operation to process the last block separately.
|   9   | _ace_state_dec_last_block_      | _dec_last_block_    | For the decryption path of authenticated encryption modes of operation to process the last block separately.
|  10   | _ace_state_enc_tag_finalize_    | _enc_tag_finalize_  | Finalize the tag computation for AE mode encryption.
|  11   | _ace_state_dec_tag_finalize_    | _dec_tag_finalize_  | Finalize internal tag computation for AE mode decryption, verify the value.
|  12   | _ace_state_set_aux_value_       | _set_aux_value_     | Set a public/auxiliary value in an algorithm.
|===

The value of 1 may be used to indicate both states _operate_ and _hash_absorb_ for the symmetric algorithms defined here.
...
* *Provisioning Input*:
[lowerroman]
 . `key`: `k` bits, or 64 bits if it is a System Key index.
...
* *Provisioning Input*:
[lowerroman]
 . `key1`: `k` bits, or 64 bits if it is a System Key index.
...
the value against which the internally computed tag must be compared:
   ...
   else an ACE error _ace_err_algo_auth_ is raised.
...
[lowerroman]
 . `key`: `k` bits, or 64 bits if it is a System Key index.
 . `IV`: `3b/4` bits.
 . Padding: `b/4` bits, unless 256 divides `b`, in which case the PI's length is already a multiple of 64 bits without this field.

* *State Machine Behavior:*

 ** To transition to state _hash_absorb_:
 the ``ace.state`` instruction must be used _without any additional inputs_, since the IV is already included in the PI.

 ** Once in state _completed_, the CC with this algorithm *cannot* be brought back to state _initial_.

These rules do not prevent cloning the CR at the beginning and re-using it, but, for decryption, it can only be used with the intended ciphertext.
...
--
  ...
  . else (tag length is `3b/4` bits)
    ...
    If `tag` and `SIV` match, `aceerror` is set to _ace_err_ok_, else ACE error _ace_err_algo_auth_ is raised.
  . The state transitions to _completed_.
--
+
If `IOLEN` > `b`, only the `b` least significant bits of `INPUT` are considered.
...
The algorithm is of course identical to the RFC's for 128-bit-wide block ciphers, with the generalization to other widths being straightforward and according the security proof of the original algorithm and a lower bound on nonce length following cite:[cryptoeprint-2023-326].
...
** To move to _set_aux_value_, a Form B `ace.state` must be used, where the auxiliary argument `Xs` sets the nonce length `N_len`.
 `N_len` must be at least `g-1`. If not, ACE error _ace_err_invalid_value_ is raised, and the CR is *invalidated*.
...
** To move to _hash_absorb_, a Form B `ace.state` must be used, where the auxiliary argument `Xs` sets the tag length `tag_len`.
  *** If this argument (interpreted as an unsigned value) is larger than `b` or is equal to `0` (the latter occurs also if `X0` is passed as the auxiliary argument), an _ace_err_invalid_value_ error is raised, and the CR is *invalidated*.
  *** If `N[u] == 1`, this means that `N` was not set in state _set_aux_value_. We do not allow code that skips nonce setting, as this mode is not secure under nonce reuse, therefore in this case ACE error _ace_err_invalid_value_ is raised, and the CR is *invalidated*.
...
** Upon entering states `_encrypt_` or `_decrypt_`, the _setup_ and _init_ stages are performed:
+
--
...
--
  . If the values match, then `aceerror` is set to _ace_err_ok_,
  . else `aceerror` is set to _ace_err_algo_auth_.
  . The state transitions to _completed_.
--
+
...
[lowerroman]
 ...
 . `block`: `b` bits.
    We include it in an SCC only if there is a partial block, and exclude it when not.
...
* *Provisioning Input*: the content is empty, i.e., there is only the metadata section.
...
--
 . `state[0] <- state[0] xor INPUT[63:0]`
 . `state[1] <- state[1] xor INPUT[127:64]`
 . `OUTPUT <- state[0] @ state[1]` // only the 128 lsbs of `OUTPUT` are written to.
...
--
 ...
 . `state[3] <- state[3] xor key[63:0]`
 . `state[4] <- state[4] xor key[127:64]`
 . `OUTPUT <- state[3] @ state[4]` // only the 128 lsbs of `OUTPUT` are written to.
...
If the value is out of bounds, the instruction raises ACE error _ace_err_invalid_value_ and the CR is *invalidated*.
...
. If the values match, then `aceerror` is set to _ace_err_ok_,
  else ACE error _ace_err_algo_auth_ is raised. The state is changed to _completed_.
--
+
...
[WARNING]
This section needs finalization. The best approach is to have different sizes depending on whether signature generation and/or verification is required, and then always export the max. size except, perhaps, the additional data which may contain precomputations done by the backend.

We need to define the elliptic curves that are actually used in practice, such as:
...
Algorithms: scalar multiplication with known and unknown base point and ECDSA/EdDSA sign/verify.

For secp256r1 the size `b` of the main fields (coordinates, signature R and S fields, scalars) is 256 bits.

For secp384r1, `b` is 384 bits.

For secp521r1, `b` is 576 bits.

Data Structures:::

* *Provisioning Input*:
 ** In the metadata, _only_ the curve type is embedded in Algorithm. For now, this is just 5 values.
  For signatures, use the DirectionPolicy field to determine whether signature generation is allowed (bit 12) or signature verification (bit 13).
  It is possible to have both or none of signature generation and verification.
 ...
 ** `Block_base`: integer.
 ** `Generator` (X coordinate, Y coordinate): a `2b`-bit string representing Y @ X.
 ** `Scalar`: `b`-bit string.
 ** If signature generation or verification allowed:
  *** `PublicKey` (X coordinate, Y coordinate): a `2b`-bit string representing Y @ X.
  *** `Signature` (R,S): a `2b`-bit string representing S @ R.
  *** `Hash`: a `b`-bit string representing the hash.
 ** During signature generation only:
  *** `RandomNumber`: `b`-bit string.
 ** Additional data (in certain states).

* *Serialized Content*:
+
--
The first five fields are serialized as bits 0, 1, 2, 3, and 4 of a byte.
...
[lowerroman]
  ...
  . `HasSignature`: Bool.
  . Padding: 8 bits.
  . `Progress`/`input_base`: integer (16 bits).
  . `Block_base`: integer (16 bits).
  . Padding: 16 bits.
  . `Generator` (X coordinate, Y coordinate): a `2b`-bit string representing Y @ X.
  . `Scalar`: `b`-bit string. Can be the private key.

The additional size block after the metadata encodes the length of all fields after `Scalar`—i.e., starting with the `PublicKey` field:
[lowerroman, start=11]
  . If `HasPublicKey`: `PublicKey` (X coordinate, Y coordinate):a `2b`-bit string representing Y @ X.
  . If `HasSignature`: `Signature` (R,S): a `2b`-bit string representing S @ R.
  . If `HasHash`: `Hash`: a `b`-bit string representing the hash.
  . During signature generation: `RandomNumber`: `b`-bit string (erased upon completion, exported only if the signature operation is interrupted).
  . Additional data (in certain states).

EdDSA formats are similar, but in the formats, the public key is represented as a `b`-bit string.
--
+
...
In this state, these states may use additional information and append it to the state.
* The format of the additional information is implementation defined and its length is given by an extra 64-bit block after the metadata block
in which the length, in bytes, is given as an unsigned integer. This block is included in the authentication of the SCC.
--
+
 ...
 *** -> _point_mul_ only if `HasPoint` & `HasScalar`
 ...
 ** All the “set” and “output” operations work like the input and output of blocks in the Hash functions, in other words, they use input_base for the parsing of the `INPUT` and block_base for the fill level of any information to be provisioned.
...
. At the User and Supervisor levels, CC usage can optionally be constrained by `(v)sacefilter` and `macefilter`.
This is a lightweight mechanism.
Since filtering by `(v)sacefilter` is inactive when the hart is in S mode or higher, context switching at U can be performed without modifying the RCSK.
...
Localities programmable at M/H/S could be used in this case, however, we note that in order to allow proper context switching, they need to be readable as well by higher-execution environments, which opens up an attack surface. This said, also `*filter`'s can be written by higher-execution environments, and these are this also manipulable by the latter—and are shorter.

The envisioned usage model employs `*acefilter` filtering at the User level, and also at the Supervisor level in high-performance systems where migration is not required (e.g., automotive applications).
Cryptographic domain separation via the RCSK is intended for Virtual Machines, Supervisor Domains, Worlds, and TEEs, as these demand stronger security guarantees.
...
Hardware support—such as automatic RCSK switching based on `*acefilter`, could alleviate this burden.
...
// * The selection method for choosing the CR to offload could be random, heuristic, or based on implementation-specific hardware-provided mechanisms to select a least-recently-used CR.

We assume that besides `scrdirty`, `vscrdirty` and `mcrdirty`, we have three more CSRs `scrlazy`, `vscrlazy` and `mcrlazy` that can be used to mark a CR as lazy, in particular CR number __i__ is *LAZY* is `*crlazy`[__i__] is set and `*crdirty`[__i__] is not set.
...
[WARNING]
Architecting this feature requires careful consideration—there may be further corner cases, so both architectural states and expected software behavior need to be analyzed in depth.
