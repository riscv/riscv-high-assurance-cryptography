...
`bytes 0 .. 3  {nbsp}{nbsp}{nbsp}{nbsp}{nbsp}` metadata section, non-encrypted part (includes Algorithm and Key_Type) +
`bytes 4 .. 7  {nbsp}{nbsp}{nbsp}{nbsp}{nbsp}` metadata section, encrypted part +
...
function DCSK_KeyDeriv(key,L) = {
    AESE256(key, L ^ (zeros(127) @ ones(1))) @ AESE256(key, L ^ zeros(128))
}
----
...
|   SIV ← AESE(Kenc, zeros(1) @ (tmp[126:0] xor (zeros(31) @ N)))) >> 32;
UNINTERRUPTIBLE_BLOCK_END

// preemptible here, with `acestart` from 0 to 15
UNINTERRUPTIBLE_BLOCK_START
|   M[0 .. 15] ← N[63:0]
|              @ (metadata[63:32] xor AESE(Kenc, ones(1) @ SIV[94:0] @ zeros(32))[31:0])
|              @  metadata[31:0];
UNINTERRUPTIBLE_BLOCK_END
...
foreach(i from 0 to num_blocks - 1) {
    ...
    |                       AESE(Kenc, ones(1) @ SIV[94:0] @ (binary(i+1)[31:0]))[63:0];
    |   else
    |       M[32+i*16 .. 47+i*16] ← content[i*16 .. 15+i*16] xor
    |                       AESE(Kenc, ones(1) @ SIV[94:0] @ (binary(i+1)[31:0]));
    UNINTERRUPTIBLE_BLOCK_END
}
----
...
UNINTERRUPTIBLE_BLOCK_START
| SIV ← M[20 .. 31];
| metadata[63:32] ← metadata[63:32] xor AESE(Kenc, ones(1) @ SIV[94:0] @ zeros(32))[31:0];
...
UNINTERRUPTIBLE_BLOCK_START
|   tag ← AESE(Kenc, zeros(1) @ (tmp[126:0] xor (zeros(31) @ N)))) >> 32;
 ...
 ** In state _set_public_value_:
  *** Only  ``ace.exec`` instructions of Form B are allowed.
  “```ace.exec Kn|K[Xn], Vs```” sets
   . `ctr <- Vs @ (b/4-1)b'0 @ ones(1)`, i.e., the `b/4` most significant bits of `Vs` are ignored, and the `b/4` least significant bits of `ctr` represent the integer 1.
...
--
    . `absorb(Vs)`.
    . `tmp <- enc_blk(enc_key, zeros(1) @ (auth_tag xor N)[b-2:0])`
...
. If `ctr == 0`, then the CR is *invalidated*.
   . If the `SIV` is `b`-bits long, then
     .. ``Vd <- Vs xor enc_blk(key, ones(1) @ SIV[b-2:b/4] @ (SIV[b/4-1:0] + ctr % 2^b/4^)),``
   . else (the `SIV` is `3b/4`-bits long)
      .. ``Vd <- Vs xor enc_blk(key, ones(1) @ SIV[3b/4-2:0] @ ctr``.
   . `ctr <- ctr + 1`.
--
+
...
** To move to _set_public_value_2_, the `ace.state` operation must have the additional argument, which sets the tag length `tag_len`.
  ...
  *** If `N[u] == ones(1)`, this means that `N` was not set in state _set_public_value_. We do not allow code that skips nonce setting, as this mode is not secure under nonce reuse, therefore in this case an invalid instruction error is raised, and the CR is *invalidated*.
...
[]
 ...
 . `L <- enc_blk(key, zeros(b))`
 . `L <- double(double(L))`
 . `N_ext <- N[N_len-1:0] @ ones(1) @ zeros(b-g-1-N_len) @ numtobits(tag_len,g)`
...
[]
 ...
 . `tmp2 <- (zeros(b-last_blk_len-1) @ ones(1) @ tmp) xor offset`
 . `aux <- double(aux)`
...
--
[start=6]
. `tmp2 <- (zeros(b-last_blk_len-1) @ ones(1) @ Vs[last_blk_len-1:0]) xor offset`
--
+
