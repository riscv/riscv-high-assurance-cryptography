...
[WARNING]
[.red]#If this CSR is used also to load/store the input/output buffer, we should call it# `[.red]#acestart#`[.red]#.#
...
The semantics and use of `crstart` are described in <<ACE-format-configuration>> and [.red]#TBD#.
...
`ace.harden{nbsp}{nbsp}{nbsp}{nbsp}Kd|K[Xd], Xs`
...
Forms A, B, and C of `ace.exec` are replaced in the code as follows:
 ...
 * `acelen`, XLEN bits wide, holds the max size of the current CR or the input/output buffer.
 * `acedest`, XLEN bits wide, holds what we are transferring: 0..31 for the CRs and, say, 255 for the input/output buffer.
 * `aceoffset`, XLEN bits wide, holds the offset within the CR virtual `content` or the input/output buffer (maybe the same as `crstart` ?).
