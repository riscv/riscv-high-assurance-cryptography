...
When `mstatus.ACES` is set to Initial or Clean, executing any instruction that changes ACE state will change `mstatus.ACES` to Dirty. Implementations may also change `mstatus.ACES` from Initial or Clean to Dirty at any time, even when there is no change in vector state.
...
The CSR `acecrstatus` tracks the configuration and dirtiness of CRs on a per-CR basis, so that context switching code needs to save only the dirty CRs, and, upon restore, restore only the configured ones and explicitly clear the unconfigured ones.

. For RV64, `acecrstatus` has a size of 64 bits,
``acecrstatus[``2__i__+1,2__i__``]`` can take the values Initial, Clean, Dirty as defined for `*status.ACES` for CR #__i__.
...
NOTE: Accurate setting of `acecrstatus` is an optimization.
Software will typically use `acecrstatus` to reduce context-swap overhead.
 ...
 ** *Unconfigure*: status remains *INITIAL*.
 ** *Configure*: Does not trap. Transitions to *DIRTY*.
 ** *Export*: Raises _ace_err_unconfigured_. No trap is generated.
 ...
 ** *Unconfigure*: sets status to *INITIAL*.
 ** *Configure*: Works as expected; sets status to *DIRTY*.
 ...
 ** *Unconfigure*: sets status to *INITIAL*.
 ** *Configure*: Works as expected; status remains *DIRTY*.
 ** *Export*: Unrestricted.
 ** *Usage*: Unrestricted.
--
