...
ACE defines an optional input/output buffer (ACEIOBUF) that can be used to store data for cryptographic operations.

The preferred way to provide inputs and ACE operations and collect their outputs is using the vector register file of the V extensions.
However, the ACEIOBUF can also be used instead, as explained in <<ACE-replacement-instructions>>.

Its size and the range of it used in cryptographic instructions are defined by the `aceiobuflen` and `aceiobuftop` CSRs,
...
| Device | M | 6
| VBootSecret               | Regenerated at each (virtual) boot. Not permanent, to be configured at the virtual boot of each VM, or also per SD (see <<ACE-CSR-maceVBootSecret>>). | HV | H | 7
| SSecret                   | Can be configured by Supervisor (see <<ACE-CSR-saceSSecret>>).                                             | OS | S | 8
...
ACE extends `*status` and `misa`, and adds up to 12 new CSRs, which are listed in <<ACE-CSRs-table>>.
...
A write to `maceBootSecret{i}` sets Bits [(_i_+1) XLEN - 1 : _i_ XLEN] of `BootSecret`.

Any access to the `maceBootSecret` block outside of M-mode causes an illegal instruction exception.
...
This is a group of HS-Mode WARZ XLEN-bit CSRs. It is used to configure `maceVBootSecret`.
(((CSR, `maceVBootSecret*`)))

* In RV32 there are four CSRs: `maceVBootSecret0`, `maceVBootSecret1`, `maceVBootSecret2`, `maceVBootSecret3`.
* In RV64 there are two CSRs: `maceVBootSecret0`, `maceVBootSecret1`.

A write to `maceVBootSecret{i}` sets Bits [(_i_+1) XLEN - 1 : _i_ XLEN] of `maceVBootSecret`.

Any access to the `maceVBootSecret` block outside of HS-mode causes an illegal instruction exception.
...
* In RV32 there are four CSRs: `saceSSecret0`, `saceSSecret1`, `saceSSecret2`, `saceSSecret3`.
* In RV64 there are two CSRs: `saceSSecret0`, `saceSSecret1`.

A write to `saceSSecret{i}` sets Bits [(_i_+1) XLEN - 1 : _i_ XLEN] of `saceSSecret`.

Any access to the `saceSSecret` block outside of S-mode or higher raises an illegal instruction exception.
