 ...
 ** Upon issuing `ace.stage Cn|C[Xn],#ace_state_last_block, Xs` where `Xs` contains the length of the last block in bits, following happens:
 ...
 ** Upon issuing `ace.stage Cn|C[Xn],#ace_last_block, Xs` where `Xs` contains the length of the last block in bits, following happens:
+
--
 . if `partial_len` <> 0, then the CC is *invalidated*. (This means we have not completed the previous block.)
 ...
 ** In state _hash_last_block_:
  ...
  ** In state _hash_finalize_:
   *** If `VLEN` is a multiple of `b` (including the case where `VLEN` = `b`), then
      ...
      The state is then changed to _completed_ with no errors.
   *** If `b` is a multiple of `VLEN` and `b` > `VLEN`, then we can issue one or more instructions ``ace.exec``of Form C, i.e., "```ace.exec Vd,Cn|C[Xn]```", to output the hash:
    . Upon entering state _hash_finalize_, `last_block_len` is reused to store the length of the output hash.
    . After each "```ace.exec Vd,Cn|C[Xn]```":
     .. We write a new block of the final value of the hash function to `Vd`, with the caveat that if `last_block_len` is non zero bust smaller than `VLEN`, the value in `Vd` is truncated and zero-padded.
     .. `last_block_len <- max(last_block_len - VLEN, 0)`
     .. if `last_block_len` = 0 then the state is changed to _completed_ with no errors.
