...
bits 0, 1, resp., 2 = enable the filter below S/VS-mode, HS-mode, resp., M-mode.
...
| (Indirect)     | MRW (RZ)  | `maceBootSecret{0..3}`  | ACE Physical Boot Session Secret
| (Indirect)     | HRW (RZ)  | `maceVBootSecret{0..3}` | ACE Virtual Boot Session Secret
...
|  2   | _ace_err_unconfigured_       | Software used an unconfigured or _invalid_, but not expired, or partially configured CR, ACEIOBUF as a source in any instruction except `ace.size`. Software used an unconfigured ACEIOBUF as an input or an output.
...
|  7   | _ace_err_invalid_value_      | Invalid metadata section in `ace.init` or `ace.import`, other than ace_err_unimplemented.
The ACEIOBUF range has an invalid length `aceiobuftop`-`acestart` (see <<ACE-CSR-aceiobuftop>> and (<<ACE-CSR-acestart>>) for the operation.
Invalid input to `ace.restrict`.
|  8   | _ace_err_privilege_          | Violation of usage control policies in metadata (including Filters).
|  9   | _ace_err_state_error_        | Not allowed State_number in `ace.state`; Encryption, resp., decryption is requested from a CC not allowing it;
