...
| VBootSecret               | Regenerated at each (virtual) boot. Not permanent, to be configured at the virtual boot of each VM or SD. | OS | M | 7
...
ACE extends `*status` and `misa`, and adds up to 31 new CSRs, which are listed in <<ACE-CSRs-table>>.
...
| 0xXXX--0xXXX   | MRW (RZ)  | `maceBootSecret{0..3}`  | ACE OS Secret
| 0xXXX--0xXXX   | HRW (RZ)  | `maceVBootSecret{0..3}` | ACE Boot Secret
| 0xXXX--0xXXX   | SRW (RZ)  | `saceSSecret{0..3}`     | ACE Supervisor Secret
|===

[WARNING]
Instead of the `macercsk`, `maceBootSecret` `maceVBootSecret`, and `saceSSecret` CSR groups
(as defined in chapter 5 of cite:[RISCV-ISA-Priv]])
we should use the mechanism decribed in “Smcsrind/Sscsrind” Indirect CSR Access.
This would limit the numer of allocated CSRs to at most 11.
...
An ACE IO Buffer status field, ACEIOBS, is added to `mstatus`[28:27] and shadowed in `sstatus`[28:27].
It is defined analogously to the floating-point context status field, FS, and to the vector context status field, VS.
