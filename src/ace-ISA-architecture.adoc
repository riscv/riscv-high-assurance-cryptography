...
* One can load clear data into a CR via `ace.init` (<<ACE-instruction-init>>), but
...
* A CR can be *exported* in an authenticated, encrypted format via `ace.export` (<<ACE-instruction-export>>) creating a Sealed Cryptographic Context (SCC).
...
* The amount of the capacity of the CRF used by a CR depends on the configured algorithm.
  ...
  In this case,
//, ACE returns _ace_err_memory_ (<<ACE-error-codes>>), and
software must free capacity by unconfiguring other CRs.
...
S-mode is responsible for virtualizing the SSecret Locality Secret.
 ...
 ** *Usage*: Trigger a trap to the operating system for lazy loading. Execution resumes from state *CLEAN*.
 ** *Export*: Trigger a trap to the operating system for lazy loading. Execution resumes from state *CLEAN*.
