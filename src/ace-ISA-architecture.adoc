...
|   [1:1] |   1  | Reserved               | Zero.
|  [11:2] |  10  | Algorithm              | Algorithm (such as primitive or mode).
| [13:12] |   2  | SC_Protection          | Level of Side Channel Protection, encoded as per <<ACE-SC-protection-levels>>.
| [15:14] |   2  | Direction_Policy       | Determines whether the CC may be used for encryption (bit 18 set), decryption (bit 19 set), or both (bits 18 and 19 set). Valid only for encryption/decryption primitives or modes, and ignored, for instance, for hash functions.
| [17:16] |   2  | Key_Type               | Key given by explicit value / System Key (index) / Internally generated.
| [21:18] |   4  | State_Number           | Encoding the current state of the state machine of the algorithm as a number.
| [25:22] |   4  | Locality               | A field which is either all ones (15) or an index into the Locality Secrets Table.
...
Export is unaffected by this, but any operation that could modify the internal state should not be performed while an interrupted export operation is in progress.
Since privileged code knows that the instruction has been interrupted, it must avoid using such an operation-it is supposed not to use CCs from child processes anyway.
...
However, if a value such as a counter is modified between the interruption and resumption of an export operation, the resumed export may encrypt the remaining SCC data with an incorrect SIV.  Consequently, a later import of that SCC will fail.  This is a general issue with CRs, because more privileged code can use them unless restricted by the corresponding UC_Policy bit.  We do not regard this as a security threat, as it only affects SCC availability—not confidentiality or integrity—and compromised privileged code already has the ability to tamper with the availability of user‑process resources.
--
...
Bits 26, 27, resp 28 are ignored if U-mode, S-mode, resp. H-mode is not supported/enabled.

===== Filter-Mode and Filter Fields
...
* `Xs`[11:0] and `Xs`[25:16] must be zero.
...
|  14   | ace_state_invalid             | _invalid_           | The algorithm has abnormally terminated or initialization/import have not completed:
 ...
 ** From any state to _encrypt_, if encryption is allowed, else the CR is *invalidated* (cf.{nbsp}<<ACE-invalidation>>).
 ** From any state to _decrypt_, if decryption is allowed, else the CR is *invalidated*.
