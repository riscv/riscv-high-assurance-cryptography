...
[WARNING] This section needs finalization. The best approach is to have different sizes depending on whether signature generation and/or verification is required, and then always export the max. size except, perhaps, the additional data which may contain precomputations done by the backend.
...
// . X coordinate of the Public Key, 256 bits
// . Y coordinate of the Public Key, 256 bits
// . Hash of message, 256 bits
// . SignatureR, 256 bits
// . SignatureS, 256 bits
...
The input for ECDSA-Sign on secp256r1/secp384r1/secp521r1 are just the message hash, `b` bits long.
 ...
 ** Metadata:
  ...
  *** For hashing, use the DirectionPolicy field to determine whether signature generation is allowed (bit 12) or signature verification (bit 13).
...
* *Internal State*:
  ...
  ** Content:
   ...
   *** `Block_base`: integer.
   *** `Generator` (X coordinate, Y coordinate): two `b`-bit strings.
   *** `Scalar`: `b`-bit string. // can be the private key
   *** `PublicKey` (X coordinate, Y coordinate): two `b`-bit strings.
   *** `Signature` (R,S): two `b`-bit strings.
...
[lowerroman]
  . Metadata.
  . `HasGenerator`: bool (byte).
  . `HasNumber`: bool (byte).
  . `HasPublicKey`: bool (byte).
  . `HasSignature`: bool (byte).
  . `Progress`/`input_base`: integer (byte).
  . `Block_base`: integer (byte).
  . Padding: 16 bits.
  . `HasGenerator`: `Generator` (X coordinate, Y coordinate): two `b`-bit strings.
  . `Scalar`: `b`-bit string. Can be the private key.
  . If `HasPublicKey`: `PublicKey` (X coordinate, Y coordinate): two `b`-bit strings.
  . If `HasSignature`: `Signature` (R,S): two `b`-bit strings.
  . If signing: `RandomNumber`: `b`-bit string.
  . Additional data (in certain states).
 ...
 * * -> {_set_hash_, _set_public_key_} -> _sign_generate_ (only if HasPoint == true) -> _output_ -> _completed_
 * * -> {_set_hash_, _set_public_key_} -> _set_signature_ (only if HasPoint == HasNumber == true) -> _sign_verify_ -> _completed_
