...
The fundamental concept of ACE is the _Cryptographic Context_ (CC).
This a data structure consisting of a *metadata section*, and a *content section*, which are indissolubly bound to each other.
The metadata defines the cryptographic algorithm and optional usage policies;
the content holds sensitive material such as keys, other secrets, and internal algorithm state.
The metadata section does not require confidentiality,
the content section requires confidentiality, and both require integrity.
Integrity of the entire CC and confidentiality of its content section are enforced by allowing access and modification of a CC only according to the rules of the ACE architecture and of the configured algorithm.

Cryptographic Locker:::
(((Locker)))
The first mechanism by which ACE implements CCs is the _Cryptographic Locker_, or locker for short.
A locker is an architectural container that safeguards sensitive information.
It can be seen as a one-way register: It is possible to _configure_ a locker with metadata and clear content (such as the actual values of keys);
however, the content cannot extracted in a way that would compromise its confidentiality.
Since CCs can only be used in cryptographic operations when stored in a locker,
this write operation effectively creates a CC, by making the two sections of the CC inseparable.
Of course, the ACE unit has access to the content section in order to perform the cryptographic operations allowed by the algorithm that is encoded in the metadata.
// We thus say that the content section of a CC in a locker is _opaque_, and in particular we can speak of an _opaque key_.
...
In order to allow efficient reconfiguration of lockers, and protect CCs also outside the architectural state,
a CC can be exported from a locker as a _Sealed Cryptographic Context_ (SCC), i.e., an encrypted and authenticated package.
A SCC can be reimported into a locker when needed.
...
An important distinction in ACE is between the _import_ and _export_ operations on lockers on one hand, and the _use_ of CCs therein in cryptographic operations on the other hand.
It must be always possible to export SCCs and reimport them into lockers, as well as clear and reinitialize lockers with new data:
If this were not possible, then a process may be able to deny all other processes access to ACE by simply configuring all the lockers.
Therefore, the optional _Usage Control policies_ in the metadata section may only be able to restrict the cryptographic operations that are allowed by the algorithms configured by a CC, including modifying its state, but not the operations that manage lockers and SCCs. [.darkred]#TBD: Add to each instruction whether it may be usage-controlled or not.#

NOTE: The distinction between locker/SCC management instructions and "use-of-CC-for-cryptography" operations is the reason we speak of _Usage Control_ in place of the term _Access Control_.
...
| `ace.init`      | `Cd\|C[Xd],%offset(Xs)`          |  Sets a lockers's metadata and content.
| `ace.clear`     | `Cd\|C[Xd]`                      |  Clears a locker and marks it as no longer in use and clean.
| `ace.export`    | `%offset(Xn),Cs\|C[Xs]`          |  Securely exports the content of a locker to memory.
| `ace.import{.S}` | `Cd\|C[Xd],%offset(Xn)`         |  Authenticates and imports a SCC from memory into a locker. +
...
| A. {nbsp}{nbsp} `Vd,Cn\|C[Xn],Vs` +
  ...
  D. {nbsp}{nbsp} `Cn\|C[Xn]`
| Performs a cryptographic operation. The instruction may also modify the internal state of the CC in the locker. This instruction admits four _forms_, depending on whether it accepts an input or not, and whether it produces an output or not.
| `ace.size`
| `Xd,Cs\|C[Xs]` +
`Xd,Xs` .2+|  `ace.size` returns the size in bytes of the SCC exported from locker `Cs` or `C[Xs]`, or from a locker that had the metadata block `Xs`. +
Returns zero if the algorithm specified in `Xs` is not supported. +
`ace.available` is an alias for `ace.size`.
| `ace.available` | `Xd,Xs`
| `ace.clone`     | `Cd\|C[Xd],Cs\|C[Xs]`            |  Clones a source locker into a destination locker.
| `ace.derive`    | `Cd\|C[Xd],Cs\|C[Xs]` +
`Cd\|C[Xd],Cs\|C[Xs],Vs2` +
|  Optional. Uses the CC in the source locker to configure the destination locker. +
For instance, the source locker could contain a KEM that configures the destination locker with a symmetric key.
| `ace.state` +
`(ace.dir)`
| `Cd\|C[Xd],#immed` +
`Cd\|C[Xd],#immed,Xs`  |  Triggers transitions in the algorithm's state (such as the different phases of hash functions or modes of operation) or provide additional information.
...
| `ace.input`     | `Cd\|C[Xd],%offset(Xs)`          |  Optional. Provides an input to the algorithm configured in locker `Cd` or `CC[Xd]`, in place of providing it through the `ace.exec` instruction. Needed only when the inputs are very long, such as the inputs to certain public key schemes.
| `ace.output`    | `%offset(Xd),Cs\|C[Xs]`          |  Optional. Writes the output of an algorithm configured in locker `Cs` or `CC[Xs]`, in place of providing it through the `ace.exec` instruction. Needed when the outputs are very long, such as the outputs of certain public key schemes.
...
Rather, it defines 32 Cryptographic Lockers for storing cryptographic keys and metadata.
Each lockers is identified by a number from 0 to 31, which the ACE unit uses to access its contents.
(((Cryptographic Context)))

Lockers reside in a dedicated internal _context file_ (CF) that is inaccessible to any entity outside the ACE implementation, including software, other hardware blocks, and invasive debug interfaces. The CF's memory capacity is implementation-defined and may be as large as 32 times the size of the largest possible CC; however, a smaller amount of memory may be used in practice. At a minimum, the CF must be capable of holding either (i) a single instance of the largest possible CC, or the largest CC pair in which the first CC derives the second (via `ace.derive`), as well as (ii) a metadata block for each other possible context.
(((Context file)))

The size of each locker is dynamic, determined by the cryptographic algorithm with which it is configured.
This is an important difference with respect to GPRs and vector registers, which always have the same size.
ACE returns an error _ace_err_memory_ when insufficient memory prevents `ace.init` and `ace.import` operations, requiring the software to free memory by exporting or clearing a different locker. Implementations must provide sufficient CF memory to prevent infinite `ace.export`/ `ace.import` loops.

A further difference between lockers and registers is that lockers are not only addressable via static 5-bit immediate addressing (as normal registers), but also indirectly, i.e., via a value from 0 to 31 in any general-purpose register.
This has two major implications:

. Adding renaming to lockers would add significant complexity to the microarchitecture.
  However lockers are much less often redefined than, say, integer or floating point registers, except for internal implied changes, and therefore we can assume that lockers are not renamed without performance issues.
. An additional bit in the instruction encoding determines whether a locker number is given by a 5-bit immediate or by the value in a GPR.

[[ACE-CSR]]
==== CSRs

ACE extends the `*status` and `misa` CSRs and adds 21 new CSRs to manage its execution environment, of which 16 are optional and exclusive to MM.
...
//| 0xXXX        | URW       | `aceenable`              | Enablement of ACE instructions
| 0xXXX          | URW       | `aceerror`               | Error code of the last ACE instruction
| 0xXXX          | URO       | `aceconfigured`          | Bitmap of configured lockers
| 0xXXX          | URO       | `aceccnative`            | Bitmap of which lockers contain a CC belonging to the current process
| 0xXXX          | URO       | `aceccstale`             | Bitmap of lazy-loaded or dirty lockers
...
[comment]
--
In the "standard" RISC-V way, higher privilege software should be able to look at just `sstatus.ACES` to know whether to proceed and look at `acedirty` and possibly save some context.
The following shows how `aceenable` accesses can be translated to `sstatus.ACES` accesses, avoiding the need to context switch `aceenable` CSR, because it can be interpreted as a virtual interface for `sstatus.ACES`.

csrrw one, aceenable, a0 does, depending on on state of sstatus.ACES

* ACES=disabled-clean -> ACES=enabled, a0=0
* ACES=disabled-dirty -> trap
* ACES=enabled -> a0=1

csrrw zero, aceenable, a0 does

* ACES=disabled-clean -> a0=0
* ACES=disabled-dirty -> trap
* ACES=enabled -> clear all CCs, ACES=disabled-clean, a0=1

(The current specification doesn't say anything about transition to ACES=enabled,
which would allow dirty state with ACES=disabled-clean.)

The rules get a more complex in virtual (V=1) mode,
where a single write to aceenable can modify both mstatus and vsstatus:

* mstatus.ACES=disabled-dirty -> CSR accesses trigger virtual-instruction trap
* vsstatus.ACES=disabled-dirty -> CSR accesses trigger illegal-instruction trap
* otherwise -> result based on previous value of vsstatus
** write 1: mstatus.ACES and vsstatus.ACES = enabled
** write 0: mstatus.ACES and vsstatus.ACES = disabled-clean, clear all CCs if any were enabled

(Maybe turn the interface to instructions? We do not need return values.)
--
...
|   1   | Initial            | ACE is enabled. The state shall be that no locker is configured. +
...
|   3   | Dirty              | ACE is enabled. there may have been changes to some locker since the last time the value "Clean" was written.
|===

Writes to ACES do not affect the contents of the lockers:  Setting ACES <- Off does not destroy the state, nor does setting ACES <- Initial or Clean clear the lockers.
...
Enabling ACE will trap to the Supervisor to allow the latter to first save the state in case it was not saved before and then clear it (lazy context switch).

Fine-grained tracking activeness and dirtiness of individual lockers uses the `aceactive` and `acedirty` CSRs, cf.{nbsp}<<ACE-tracking-locker-dirtiness>>.

//
//
[[fsxsstates-copy]]
.ACES state transitions (FS, VS, and XS state transitions -- to be cannibalised)
...
[[ACE-tracking-locker-dirtiness]]
===== Fine Grained Locker Management: `aceconfigured`, `acenative`, and `acestale`

Let us define four properties for lockers:

[horizontal]
`configured`:: Indicates whether the locker is currently configured.
`native`:: If the locker is configured, indicates whether the current process is its owner or an alien process is
`dirty`:: If the locker is configured and owned by the current process, indicates whether its state has been modified.
`lazy`:: Indicates whether a configuration for this locker is stored only in the parent's switching context,
regardless of whether the locker is unconfigured or occupied by a CC belonging to a different process.

Definition of *authoritative*: [.red]#TBD#.

It is the duty of the operating system must keep track:
* for each process, and
** for each locker, whether it is:
 . unconfigured and
  .. there is no saved authoritative value for the current process; or
  .. there is a saved authoritative value for the current proceszs
 . configured and
  .. it contains an authoritative CC for the process; or
  .. it contains an authoritative CC for an alien process, together with the ID of latter.

We list now the possible states a CC may have:

*VOID* (`0b000`)::
[.darkred]#_Locker is unconfigured; there is no saved value for the current process._# +
`[.darkred]#configured = 0, lazy = 0.#`
** We assume `dirty = 0` and `native = 0`.
** *Usage* gives error _ace_err_unconfigured_, does not trap.
** *Configuration* does not trap, works as expected-including errors with *export* or *clone* - and sets: +
`configured <- 1, native <- 1, lazy <- 0,` and `dirty <- 1` {implies} *DIRTY*.

//

*LAZY* (`0b100`)::
[.darkred]#_Locker is unconfigured; parent has saved a value to be lazy-loaded for the current process._# +
`[.darkred]#configured = 0, lazy = 1.#`
** We assume `dirty = 0` and `native = 0`.
** *Usage* or *export* cause a trap to allow the OS to lazy load a CC's value in the locker for the current process first, then set `dirty <- 0`, `lazy <- 0` and the instruction is restarted unrestricted and operates from the state *CLEAN*.
   If the CC in the locker has been potentially modified, then `dirty <- 1` {implies} *DIRTY*.
** *Configuration* of the locker works as expected, and sets: +
   `configured <- 1, native <- 1, lazy <- 0`, and `dirty <- 1` {implies} *DIRTY*.

//

*CLEAN* (`0b011`)::
[.darkred]#_Locker is configured for the current process and loaded. Unchanged since last switch._# +
`[darkred]#configured = 1, native = 1, lazy = 0, dirty = 0.#`
** If the process is interrupted now, the OS does not need to save it again.
** *Usage* is unrestricted; if the CC in the locker is potentially modified, `dirty <- 1` {implies} *DIRTY*.
** *Configuration* works as expected, and sets: +
`configured <- 1, native <- 1, lazy <- 0`, and `dirty <- 1` {implies} *DIRTY*.

//

*DIRTY* (`0b111`)::
[.darkred]#_Locker is configured for the current process and loaded. Potentially changed since last switch._# +
`[darkred]#configured = 1, native = 1, lazy = 0, dirty = 1.#`
** If the process is interrupted now, the OS must save the CC in the locker.
** *Usage* is unrestricted.
** *Configuration*: Works as expected, and leave the properties unchanged: +
`configured <- 1, native <- 1, lazy <- 0`, and `dirty <- 1` {implies} *DIRTY*.

//

*ALIEN* (`0b001`)::
[.darkred]#_Locker is configured for a different process; blocked for the current one; no value to be lazy-loaded._# +
`[darkred]#configured = 1, native = 0, lazy = 0.#`
** *Usage* results in error _ace_err_unconfigured_
** *Configuration* of the locker will first trap to the operating system, to allow the latter to:
export the CC in the locker to the state of the owning process;
clear the locker;
update the flags to *VOID* to allow the current process to configure it, and return to the process to restart the operation allowing the configuration.

//

*ALIEN_LAZY* (`0b101`)::
[.darkred]#_Locker is configured for a different process; blocked for the current one; there is a value to be lazy loaded for the current process._# +
`[darkred]#configured = 1, native = 0, lazy = 1.#`
** In this case accessing it will trap to the operating system.
** The operating system must save the current value to the state of its owner, load the value from the current state. +

//

The encoding can be interpreted as follows:

* Bit 0 distinguishes between unconfigured (value is 0) or configured (1);
* Bit 1 distinguishes between native (1) or alien (0), where unconfigured is considered alien in order to have a value equal to zero for the *VOID* state; and
* Bit 2 has a compressed semantic, since it distinguishes between present/clean (0) or lazy-loaded/dirty (1) -- essentially 0 means "parent mode has nothing extra to do" and 1 means "parent mode has something extra to do".

Therefore, we can collect the bits into three CSRs:
Bit 0 in `aceconfigured`, Bit 1 in `acenative`, and Bit 2 in `acestale`.
The reason for the use of "stale" here is that, in computer architecture, a state is said to be stale if it cannot be used as-is because it is displaced, like a CC that needs to be lazy-loaded first, or its value no longer reflects the correct state, like the CC in the locker saved by the context switching code in the parent mode. So, there is a bit of an abuse of language, since in the contexts of laziness and dirtiness it is not the same object that it stale.

NOTE: This 6-state property can track the locker states for all higher-privileged modes.
S/VS can trust the values coming from U, since U cannot write to these registers.
However, M and HS cannot trust the values from S and VS, since the latter can write to these registers.
This implies that M, resp., HS mode must always context-switch all lockers when context-switching S, resp., VS domains, since it doesn't know which lockers have changed. We *could* add additional registers to track changes at each level.  This should be done for F and V too, ideally, in order to make the architecture consistent.

// [.red]#Remember to add the handling of the fine-grained CC management CSRs.#
...
the change in usage policies in `ace.restrict` is not permitted because it would either downgrade or overwrite the current restrictions in the CC in the locker;
software tried to use an incomplete CC in a locker as a source in a `ace.exec`, `ace.state`, `ace.input`, `ace.output`, `ace.clone`, `ace.derive`, `ace.harden`, `ace.restrict` operations.
...
If algorithms that support `ace.derive` are implemented, the implementation must make sure that both the input and output CCs of this operation can be allocated in the CF.
|  6   | ace_err_unconfigured    | Software tried to use an unconfigured locker as a source in one of the following instructions: `ace.exec`, `ace.state`, `ace.input`, `ace.output`, `ace.clone`, `ace.derive`, or `ace.harden`/`ace.restrict`.. +
This can be used in low CF memory situations to trigger a lazy re-load.
|===
(((ACE, error codes)))
...
If sufficient memory is available, subsequent blocks are read sequentially and passed to a deserialization circuit to populate the locker with the CC._
* `_ace.import_` _operates similarly to `ace.init`: the metadata section is read first to determine the payload length. If sufficient memory is available, the remaining blocks are read, decrypted, and passed to a deserialization circuit to populate the locker._
* `_ace.export_` _involves two passes, but the first pass is entirely internal to the ACE unit. Hence, the first pass cannot cause a page fault, and is easily restarted. Specifically, it computes the SIV by applying POLYVAL to the serialized content of the CC in the locker. Subsequently, the serialized metadata, the nonce, and the SIV are written out, followed by the content serialized, encrypted and written out block by block. It is this last part that may cause page faults, and that may require resumption._
...
ACE defines two distinct representations of cryptographic information: the configuration format, which is loaded into a locker to create a CC, and the encrypted, authenticated SCC format. These two  formats are architected for software portability and interoperability.
The internal CC representation in a locker, on the other hand, is implementation-specific.
...
This format is used for locker initialization, and CC export and import purposes.
...
| [17:16] |   2  | Completion             | Defined in <<ACE-locker-complete>>.
...
Otherwise, this value is used as an index _i_ in the HST, and a derived key computed from RCSK and LST[_i_] is used for import/export in place of the RCSK.
...
| OEMSecret                 | Provisioned by OEM to differentiate their products from the competition.
                              Can be permanent in a device — optional reconfiguration using impdef HW authenticated mechanisms. | Device | N | 3
...
*Example*: _If a key's metadata specifies a Locality field value of "2", the key for import/export operations is derived from the RCSK and the "OEM Secret".
...
NOTE: _A CC may reference a key via an index into the system-defined System Key Table (SKT).loc
...
* `metadata_serialize(locker) -> byte array`: Serializes the internal metadata into a 64-bit block for export.
* `metadata_deserialize(CC,byte array) -> locker.metadata`: Takes a metadata block formatted as described in <<ACE-metadata>> and deserializes it to the internal representation of the metadata section.
* `initial_content_deserialize(CC,byte array) -> locker.content`:
...
* `content_serialize(locker) -> byte array`: Serializes a CC's content to a byte array to be encrypted for export.
* `content_deserialize(locker,byte array) -> CC.content`:
Takes the decrypted byte array, that comes after the metadata, nonce and authentication tag, and deserializes it into a locker's content.
...
`bytes 32..u+31  {nbsp}{nbsp}` contain `content_serialize(locker.content)`, which has a byte length of `u`.
+
...
To describe the authenticated encryption of a SCC, we first define the RFC8452 Key Derivation and the DCSK Derivation functions.
...
We can now define the generation of the SCC from the CC in a locker.
...
// We should not forget that ACE has been designed to provide a standardised accelerator interface to software, where the software could also set their own keys. I have worked on DRM schemes, like implementations of Widevine, that use an obfuscation mechanism for the keys, which are then briefly unwrapped to be used, and then erased — all in user space. This would optimise such mechanisms and increase security by reducing the time keys in clear are exposed in RAM.
// While "reprovisioning" the keys could in theory work in a scheme where the keys only come from registered trusted applets (but it would require a very complex SW infrastructure, where agents register themselves to some MM component), in an user space environment, adding reprovisioning would be a burden to SW, as it could happen anywhere in the code, so we would have to check for errors after each operation, or at least have in-process trapping, because the CC may disappear from under your feet at any time.
// Since all sorts of user space applications are a fundamental motivation for the feature. Restricting this to only keys provisioned by other environments would vastly reduce the likelihood of being accepted.
...
In all cases where a GPR is used to index a locker, we exclude the use of the zero-hardwired GPR `X0` as the index register-that is, the encoding of the register cannot be zero. This is reserved to encode some special operation, and otherwise raises an invalid-instruction exception.
...
* If the `immed12` = `0xa11`, then all lockers are cleared and the new state of the ACE unit is Clear. The alias for this instruction is `ace.reset`.

`ace.enable` must be issued before using any ACE operation, i.e. `ace.config` passing the immediate 1.
--
+

NOTE: The advantage of `ace.reset` with respect to a loop that clears all lockers is that the entire CF memory is cleared without the need for compaction/management between the separate `ace.clear` operations and this opertation cannot be interrupted, reducing potential wasteful context saves.

The instruction will trap in the follwong cases: [.red]#TBD#.

NOTE: This instruction shares its encoding with `ace.clear` with `3'b111` as instruction code (use a GPR to select the locker). However, the zero in the source field [11:7] for indirect locker indexing is defined as invalid/special, and this allows to define this instruction.#


// [.darkred]#Proposal: With the immediate `0xa11` (all), all lockers will be cleared, as if `ace.clear` had been called on each of them, but the feature is not disabled.
...
Description::
`ace.init` configures a locker with metadata and content.
`Cd` is the source locker to be configured (encoded as a 5-bit immed).
...
Description::
`Cd` is the source locker to be cleared (encoded as a 5-bit immed). +
...
Description::
Exports the locker `Cs` or `C[Xs]` to memory, starting at address `Xd`, using
...
Description::
`Cd` is the destination locker into which the SCC is imported, as an immed (5-bit).
Alternatively, the number of the locker into which import the SCC is stored in
...
. `ace.exec  {nbsp}{nbsp}{nbsp}Cn|C[Xn],Vs` {nbsp}{nbsp} takes an input vector but does not write to an output vector.
. `ace.exec  Vd,Cn|C[Xn]{nbsp}{nbsp}{nbsp}` {nbsp}{nbsp} does not take an input but writes to an output vector.
. `ace.exec  {nbsp}{nbsp}{nbsp}Cn|C[Xn]{nbsp}{nbsp}{nbsp}` {nbsp}{nbsp} does not take an input vector and does not write to an output vector.

Form D instructions perform internal processing only in the CC in the given locker.
...
....
+
Bit `r` (i.e., bit 27) is 0 if the locker is encoded as an immediate, and 1 if the locker's number is given in a GPR.

Description::
This instruction writes the size of the SCC corresponding to the source locker,
...
NOTE: The value can significantly differ from the internal resource usage to store the CC's data in a locker —
...
....
+
Bits [27:26], i.e., field `R`, take following values depending on how the lockers are given:
+
[upperalpha]
. `0b00` if the source and the output lockers are both encoded as immediates.
. `0b01` if the source locker is selected using a GPR and the output locker is encoded as an immediate.
. `0b10` if the source locker is encoded as immediate and the output locker is selected using a GPR.
. `0b11` if the source and the output lockers are both selected using the values of GPRs.

// Comment to ensure the proper indentation in th next Description

Description::
Clones source locker `Cs` (or the locker indexed by `Xs`) into destination locker `Cd` (or
into the locker indexed by `Xd`).
...
Description::
This instruction restricts the usage of a CC (as stored in a locker) to specific modes or to a specified `ASID` or `VMID`. +
 ...
 * A `mv` instruction that loads a specified number of bytes from a GPR or vector register into a the state of a locker within the ACE unit.
 ...
 * A corresponding `mv` instruction that exports a specified number of bytes from a locker to a GPR or vector register.
 ...
 * A “`close`” instruction to suspend or finalize the procedure.
   If executed before all bytes are transferred, the locker state matches that of an interrupted monolithic instruction, including the Completion metadata field and the `acestart` value. If executed after all bytes are transferred, it also performs any final cryptographic operations, such as authenticating the locker, if needed.
...
If an invalid input is provided to any ACE instruction, or a non-admitted instruction is executed, while processing the algorithm in locker, say, `Cn`,
...
The function that puts the locker in this state is called `invalidate()`.

In state _completed_, no operation can be performed except `ace.init`, `ace.export`, `ace.import`, `ace.clear`, or `ace.error`.
Any other operation, if it has an output, it returns zero, and will cause the locker to be *invalidated*.
...
Since such operations can operate independently of the CPU core, they may continue to execute also when the process that issued them is interrupted, so the hart implementation may cause the context switching software to wait when accessing a locker that is going to be written to.
We do not specify whether these instructions write their results to the locker atomically or piecewise.
[.darkred]#(Only in case we still want to make long cryptographic operations interruptible.)#
...
NOTE: Internally, the ACE unit can either directly use the value from the SKT, or copy it to the internal state of the locker.
 ...
 ** From any state to state _encrypt_, if metadata allows encryption, else the locker is *invalidated*.
 ** From any state to state _decrypt_, if metadata allows decryption, else the locker is *invalidated*.
* *Behavior:*
 ** In state _initial_, any `ace.exec` will result in the locker being *invalidated*.
 ** In state _encrypt_:
  *** If{nbsp} `ace.exec Vd,Cn|C[Xn],Vs` {nbsp}is issued, then {nbsp} `Vd = enc_block(Vs)`.
  *** If `VLEN` is a multiple of `b`, then `ace.exec Vd,Cn|C[Xn],Vs` encrypts block-by-block.
  *** If any other Form of `ace.exec` is issued, then the locker is *invalidated*.
 ** In state _decrypt_:
  *** If{nbsp} `ace.exec Vd,Cn|C[Xn],Vs` {nbsp}is issued, then {nbsp} `Vd = dec_block(Vs)`.
  *** If `VLEN` is a multiple of `b`, then `ace.exec Vd,Cn|C[Xn],Vs` decrypts block-by-block.
  *** If any other Form of `ace.exec` is issued, then the locker is *invalidated*.
...
----
[.darkred]#in place of a single `ace.exec Vd,Cn|C[Xn],Vs`.
...
ace.import C0,(X5)                        // Import the SCC into locker C0
...
ace.state C0,#ace_state_encrypt           // Configure locker for encryption
ace.error X1                              // Read error from a CSR
 ...
 ** In state _set_public_value_, only ``ace.exec`` instructions of Form B are allowed. +
     "```ace.exec Cn|C[Xn],Vs```" sets the IV value to `Vs`.
  ...
  *** Any other Form of ``ace.exec`` will produce a null output and *invalidate* the locker.
 ** In state _operate_, only ``ace.exec`` instructions of Form C are allowed. +
     "```ace.exec Vd,Cn|C[Xn]```" performs:
+
--
 ...
 ** In state _operate_, any other Form of ``ace.exec`` will produce a null output and *invalidate* the locker.
...
ace.state C0,#ace_state_set_public_value
V0 ← nonce
ace.exec C0,V0
 ...
 ** _set_public_value_ -> _encrypt_, if metadata allows encryption, else the locker is *invalidated*.
 ** _set_public_value_ -> _decrypt_, if metadata allows decryption, else the locker is *invalidated*.
 ** _encrypt_, _decrypt_ -> _initial_.

* *Behavior:*
 ** In state _initial_:
  *** The `mask` field is set to zero.
  *** Any {nbsp}``ace.exec``{nbsp} operation will produce null outputs and causes the locker to be *invalidated*.
 ** In state _set_public_value_, only ``ace.exec`` instructions of Form B are allowed. +
 "```ace.exec Cn|C[Xn],Vs```" sets the _tweak_. The `mask` field is used to store the tweak: `mask <- Vs`.
  ...
  *** Other forms of `ace.exe` not allowed, will produce null outputs and the locker is *invalidated*.
 ...
 ** In states _encrypt_ and _decrypt_, only ``ace.exec`` instructions of Form A are allowed. +
    "```ace.exec Vd,Cn|C[Xn],Vs```" performs the following operations:
+
--
...
ace.state C0, #const_set_public_value            // this can be an IV or a nonce.
V0 ← IV
ace.exec C0,V0
...
// N.B. another ace.state C0, #const_set_public_value would reset the internal state and allow to begin another decryption with a new IV.
 ...
 ** _hash_absorb_ -> _encrypt_ -- if metadata allows encryption, else the locker is *invalidated*.
 ** _hash_absorb_ -> _decrypt_ -- if metadata allows decryption, else the locker is *invalidated*.
 ...
 ** In state _set_public_value_:
  *** Only  ``ace.exec`` instructions of Form B are allowed.
  "```ace.exec Cn|C[Xn],Vs```" sets
   . `ctr <- Vs @ (b/4-1)'b0 @ 1'b1`, i.e., the `b/4` most significant bits of `Vs` are ignored, and the `b/4` least significant bits of `ctr` represent the integer 1.
  *** If "```ace.exec Cn|C[Xn],Vs```," is repeated, it will just re-write the IV.
  *** If no such "```ace.exec Cn|C[Xn],Vs```" to set is issued, `ctr` remains zero.
  *** If `VLEN` is larger than `b`, the most significant bits of `Vs` (i.e., the "tail" of the vector) are ignored.
  *** Any other Form of ``ace.exec`` will produce null outputs and causes the locker to be *invalidated*.

 ** In state _hash_absorb_, only  ``ace.exec`` instructions of Form B are allowed. +
 "```ace.exec Cn|C[Xn],Vs```" absorbs a data block in `auth_tag`, i.e.:
+
--
 . `absorb_into_hash(Vs)`.
--
...
This operation is parallelizable by having the implementation precompute the first four powers of `hash_key`, keeping it internally in the locker (and reconstructing these powers after `ace.import`). If the number `x` of blocks to process is not a multiple of `VLEN`/`b`, `VL` can be set to the remainder of the division of `x` by `VLEN`/`b` for the last instruction.

** If state _encrypt_, only ``ace.exec`` instructions of Form A are allowed. +
   "```ace.exec Vd,Cn|C[Xn],Vs```" encrypts `Vs` into `Vd`, and absorbs the ciphertext `Vd` into the `auth_tag`, namely:
+
--
  . If `ctr[b/4-1:0] = 0`, the locker is *invalidated*.
...
The value +
 `{nbsp}{nbsp}{nbsp}Vs = IntTo{b/2}Bits(bit_length(AD)) @ IntTo{b/2}Bits(bit_length(plaintext))` +
 must be set by the software, then "```ace.exec Vd,Cn|C[Xn],Vs```" computes the authentication tag as follows:
+
--
  . `absorb_into_hash(Vs)`.
  . `auth_tag <- auth_tag xor enc_block(key,ctr[b-1,b/4] @ 0[b/4-1,0])`.
...
** In state _hash_verify_, only an ``ace.exec`` instructions of Form B, i.e., "```ace.exec Cn|C[Xn],Vs```" are allowed.
...
// The next operations set the nonce (and the derived Authentication Key).
ace.state C0,#ace_const_set_public_value
V0 ← nonce
ace.exec C0,V0

// Compute AD contribution to Auth Data
ace.state C0,#ace_const_hash_absorb
...
ace.state C0,#ace_const_hash_finalize_enc
...
ace.state C0,#ace_const_hash_finalize_dec
V1 ← IntTo64Bits(len(AD)) @ IntTo64Bits(len(CT))
ace.exec V2,C0,V1                                // V2 contains the finalized hash

ace.state C0,#ace_const_hash_verify
V1 ← tag_from_payload
...
vsetvli zero,4,e32,m1,ta,ma                      // Use 128-bit vectors
ace.import C0,(X5)                               // Imports SCC into locker C0
V5[127:32] ← IV
V5[31:0] ← 1
 ...
 ** Transition to _encrypt_ possible only if metadata allows encryption, else the locker is *invalidated*.
 ** Transition to _decrypt_ possible only if metadata allows decryption, else the locker is *invalidated*.

* *Behavior:*
 ** In state _initial_ :
  *** Any {nbsp}``ace.exec``{nbsp} operation will produce a null output and the locker will be *invalidated*.
  *** The following initialization operations are performed:
  ...
  ** In states _set_public_value_, resp. _set_public_value_2_:
    ...
    *** If `VLEN` is larger than `b`, the most significant bits of `Vs` (i.e., the "tail" of the vector) are ignored.
    *** Other forms of `ace.exe` will produce null outputs and *invalidate* the locker.
+
...
+
--
   *** Only ``ace.exec`` instructions of the Form "```ace.exec Cn|C[Xn],Vs```" are allowed. Such an instruction accumulates a value in the `auth_tag`:
...
*** Other forms of `ace.exec` will produce null outputs and *invalidate* the locker.

** In state _hash_finalize_enc_, only an {nbsp}``ace.exec``{nbsp} of Form A is allowed. +
  "```ace.exec Vd,Cn|C[Xn],Vs```," where it is the responsibility of the user to set +
`{nbsp}{nbsp}{nbsp}Vs = IntTo{b/2}Bits(bit_length(AD)) @ IntTo{b/2}Bits(bit_length(plaintext))`, +
...
** In state _encrypt_, only  ``ace.exec`` instructions of the Form A are allowed, namely "```ace.exec Vd,Cn|C[Xn],Vs```," to encrypt `Vs` into `Vd` as follows:
+
--
. If `ctr = 0`, then the locker is *invalidated*.
...
** In state _decrypt_,  only  ``ace.exec`` instructions of the Form A are allowed, namely "```ace.exec Vd,Cn|C[Xn],Vs```," to decrypt `Vs` into `Vd` and then absorb the plaintext `Vs` into the `auth_tag`:
+
--
     . If `ctr = 0`, then the locker is *invalidated*.
     . If the `SIV` is `b`-bits long, then +
 ...
 ** In state _hash_finalize_dec_,
    ...
    namely "```ace.exec Cn|C[Xn],Vs```," where it is the responsibility of the user to set +
    `{nbsp}{nbsp}{nbsp}Vs = IntTo{b/2}Bits(bit_length(AD)) @ IntTo{b/2}Bits(bit_length(plaintext))`. +
...
V0 ← nonce
ace.exec C0,V0
ace.state C0,#ace_const_set_public_value_2
V0 ← SIV
ace.exec C0,V0
 ...
 ** In state _initial_:
  ...
  *** `hash` is set to zero.
  *** Any `ace.exec` will result in the locker being *invalidated*.
 ...
 ** Upon issuing `ace.state Cn|C[Xn],#ace_state_last_block, Xs` where `Xs` contains the length of the last block in bits, following happens:
   . if `partial_len` <> 0, then the locker is *invalidated*. (The previous block is not complete.)
   . if `Xs > b` then the locker is *invalidated*.
   . `last_block_len <- Xs`.

 ** In state _hash_last_block_,  we can only issue a single instruction ``ace.exec`` of Form B, i.e., "```ace.exec Cn|C[Xn],Vs```", to absorb the data block `Vs` and write the final hash value in `Vd`. After this, the state transitions to _completed_:
+
--
  ...
  ** In state _hash_finalize_,
    then we can issue one instructions ``ace.exec``of Form C, i.e., "```ace.exec Vd,Cn|C[Xn]```", to output the hash.
 ...
 ** In any state, issuing any Form of ``ace.exec`` that is not the allowed one will produce a null output and the locker will be *invalidated*.
 ...
 ** In state _initial_:
  ...
  ** In each state except _initial_, before performing any operation except `ace.import`, `ace.export`, and `ace.clone`, `used_VLEN` is compared to `VLEN`.
  If there is a mismatch, the locker will be *invalidated* and any operation will result in an invalid instruction exception.
 ...
 ** Upon issuing `ace.state Cn|C[Xn],#ace_last_block, Xs` where `Xs` contains the length of the last block in bits, following happens:
+
--
 . if `partial_len` <> 0, then the locker is *invalidated*. (The previous block is not complete.)
 . if `Xs > b`, then the locker is *invalidated*.
 . if `Xs = 0`, then the locker is *invalidated*.
 . `last_block_len <- Xs`.
--
+

 ** In state _hash_last_block_:
  *** if `last_block_len` = 0, then the locker is *invalidated*.
  *** If `VLEN` is a multiple of `b` (including the case where `VLEN` = `b`), then
      we only issue a single ``ace.exec`` of Form B, i.e., "```ace.exec Cn|C[Xn],Vs```", to absorb the least significant `b` bits of data block `Vs`.
  *** If `b` is a multiple of `VLEN` and `b` > `VLEN`, then we can one or more instructions ``ace.exec`` of Form B, i.e., "```ace.exec Cn|C[Xn],Vs```", to absorb the data block `Vs` into the hash, i.e. "```ace.exec Cn|C[Xn],Vs```":
   . if `partial_len < b`, then
   .. The instruction must be of Form B, else the locker is *invalidated*.
   .. `Vs` is written to `partial_block` starting at byte offset `partial_len`.
   .. `partial_len <- partial_len + VLEN`.
   . else if `partial_len+VLEN = b`, then the locker is *invalidated*.

  ** In state _hash_finalize_:
   *** If the function is a XOF, then
       we can issue an arbitrarily large number of ``ace.exec`` of Form C instructions, i.e., "```ace.exec Vd,Cn|C[Xn]```", to output the hash.
       This can be done with any of the two mechanisms below, except that the state never transitions to _completed_.
   *** If `VLEN` is a multiple of `b` (including the case where `VLEN` = `b`), then
      we only issue a single ``ace.exec`` of Form C, i.e., "```ace.exec Vd,Cn|C[Xn]```", to output the hash.
   ...
   *** If `b` is a multiple of `VLEN` and `b` > `VLEN`, then we can issue one or more instructions ``ace.exec``of Form C, i.e., "```ace.exec Vd,Cn|C[Xn]```", to output the hash:
    ...
    . After each "```ace.exec Vd,Cn|C[Xn]```":
     ...
     .. if `last_block_len` = 0 then the state is changed to _completed_ with no errors, unless the algorithm is a XOF, in which case the internal state is updated again, `last_block_len <- b`, and more "```ace.exec Vd,Cn|C[Xn]```" instructions can be issued.
...
Users may migrate their virtual machines (VMs) between client devices with CPUs from different silicon providers—a practice already common when moving interrupted VMs between Intel and AMD systems. In such a scenario, the ACE implementation on the destination device might have a smaller Context File (CF) capacity. System software could then be unable to restore all cryptographic contexts.

To address this, the `acelazy` CSR to enables lazy context reloading by causing the hart to trap to the parent mode if a locker being accessed is marked as "lazy", i.e., the corresponding bit in `acelazy` is set.  When this requires freeing one or more lockers first, system software decides which contexts to evict.
...
. Configure a locker with this key and its associated metadata.
. Export this locker to a SCC in memory.
...
. Re-import the SCC into a locker
...
This is the most generic approach and allows each piece of software to use a locker of choice to generate CCs and import SCCs.
...
// Various optimizations are possible, which can be interesting for embedded environments: Steps 3., 5., and 6. can be omitted if the locker used for these operations is fixed and no intermediate SW environments modify that locker.
