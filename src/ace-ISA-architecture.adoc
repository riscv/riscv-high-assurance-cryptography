...
Regarding the `acecclazy` bits, most operations attempted on a CC with this bit set will trap (cf. <<ACE-rules-placeholder>> for the exact listing of which operations trap), to allow the parent software to load the context, possibly freeing other contexts in order to make room for it.
...
In such cases, implementations may set `acestart` to a non-zero value, enabling the instruction to resume from the interruption point rather than restart from the beginning.
Since ACE does not have the concept of the length of an element,
`acestart` represents a byte offset rather than an element index as in `vstart`.
Specifically, a load-type instruction was interrupted after reading _and_ processing `acestart` bytes of input in the state, and a store-type instruction was interrupted after computing and writing `acestart` bytes of output.
...
[.darkred]#*TBD* Specify the exact semantics of `acestart` ato bend define the points at which various operations can be interrupted.#
...
// .RFC8452 Key Derivation, Simplified
// [source,sail]
// ----
// function RFC8452_KeyDeriv(K) = {
//   struct {
//     AESE256(K, (0[125:0] @ 0b101))[127:0] @
//     AESE256(K, (0[125:0] @ 0b011))[127:0],
//     AESE256(K, (0[125:0] @ 0b000))[127:0]
//   }
// }
// ----
...
[[ACE-rules-placeholder]]
==== Rules for Placeholder CCs
