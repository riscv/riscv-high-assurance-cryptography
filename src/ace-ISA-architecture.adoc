...
[WARNING]
If the H extension is not present, `hgatp` is also not present.
We propose to add this register also when ACE is provided in order to allow M-mode to be able to separate a main OS from a TEE, for instance, on devices that implement TEEs using Supervisor Domains or other mechanisms based on PMP/Smepmp, Sv and Smmpt-see cite:[RISC-V-SPMP] and cite:[RISC-V-SD].
