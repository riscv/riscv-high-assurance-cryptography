...
_HLocality_ is only active if the H extension is enabled and V=1. Otherwise, its value is a shadow of _MLocality_.
...
[WARNING]
--
Up to three Locality Secrets can be combined: A HW Binding one, one of _PhysBootSecret_ or _VBootSecret_, and one from the SW Filter Group.
We envision two methods to combine them for the purpose of creating the data to prepend to the SIV computation or verification in AES-GCM-SIV:

. XOR them.
. Concatenate them.

// Multiply them in the binary field defined by stem:[P(x)=x^{127}+x+1]. This works because the given polynomial is primitive and the Locality Secrets are non-zero.
...
// Both methods theoretically reduce the distinguishability of a sequence or random numbers by just two bits (from 127 to 125 bits, say).
--
...
* A non-zero value in `Xs`._ExpirationDate_ will be copied to `M`._ExpirationDate_, but only if that policy was not already programmed, otherwise error ACE error _ace_err_invalid_value_ is raised.
...
[]
----
function KeyDeriv(key : bits(256)) = {
  struct {
...
The export and import algorithms are the nonced version of the AES-GCM-SIV algorithm.
However, we use the Locality as the nonce, which completely changes the encryption and authentication values.
...
{enc_key, auth_key} ← KeyDeriv(CSK);
L : list ← {} if (mh.Locality == 0) else get_locality(mh.Locality)
