...
Unlike the vector extension’s `vstart`, which tracks element indices, ACE has no _intrinsic_ notion equivalent to that of vector elements-only the algorithms may have basic processing units, but in an attached architecture depending on these may slow down the implementation.
