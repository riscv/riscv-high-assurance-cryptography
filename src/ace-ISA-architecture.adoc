...
** The flag is only set in this circumstance.
 *** The implementation will resume the import operation only if the flag is set, otherwise it will restart the operation.
 *** The flag is also used by the implementation to determine whether any attempt to use the partially imported CR will raise error _ace_err_unconfigured_ instead of _ace_err_invalid_CR_.
