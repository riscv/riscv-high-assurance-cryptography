...
* *Internal State*:
 ** `key`: `k/8` bytes.
 ** `ctr`, `hash_key`, `auth_tag`: `b/8` bytes.
...
--
+
Also, define `g` = `ceil(log_2(b))` and `u` = `b - g - 1`.
 ...
 ** `key`: `k/8` bytes.
 ** `N`: `u` bits.
 ** `ktop`, `L`, `offset`, `hash_A`, `hash_P` : `b/8` bytes.
 ** `taglen`, `last_blk_len`: int
...
[]
  ...
  . `tmp <- (zeros(b-last_blk_len-1) @ 1b'1 @ Vs[last_blk_len-1:0]) xor offset` // CipherInput
...
[]
 ...
 . `tmp <- enc_blk(K, offset)[last_blk_len-1:0]` // pad
 . `tmp <- Vs[last_blk_len-1:0] xor tmp`         // C_*
 . `Vd <- zeros(b-last_blk_len) @ tmp`
 . `tmp2 <- (zeros(b-last_blk_len-1) @ 1'b1 @ tmp[last_blk_len-1:0]) xor offset`
 . `aux <- double(aux)`                         // L_$
...
//  . `offset <- offset xor aux`                                             // L_* in RFC
//  . `tmp <- enc_blk(K, offset)[last_blk_len-1:0]`                           // pad
//  . `tmp <- Vs[last_blk_len-1:0] xor tmp`                                   // P_*
//  . `Vd <- zeros(b-last_blk_len) @ tmp`
//  . `tmp2 <- (zeros(b-last_blk_len-1) @ 1'b1 @ Vs[last_blk_len-1:0]) xor offset`
//  . `aux <- double(aux)`                                                   // L_$
//  . `hash_P <- enc_blk(K, hash_P xor tmp2 xor offset xor aux) xor hash_A`
...
** In state _hash_verify_, only an ``ace.exec`` instruction of Form B, i.e., "```ace.exec Kn|K[Xn],Vs```" is allowed.
    It compares the `taglen` least significant bits of `Vs` with `hash_P`:
+
--
  . If the values match, then the state is changed to _completed_, and `ace.error` returns no error,
  . else the state is changed to _completed_, and `ace.error` returns `ace_err_authentication`.
