...
* `encrypt_block(key,p) -> c` encrypts block `p` using the key, and returns the result `c`.
* `polyval(data)` absorbs data into the hash function by performing a step of polyval.
  . `auth_tag <- auth_tag xor Vs`.
  . `auth_tag <- auth_tag *' hash_key`, where the `*'` is the multiplication in the Montgomery domain of Polyval.
