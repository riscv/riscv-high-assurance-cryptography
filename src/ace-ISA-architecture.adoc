...
===== Usage of `acecrstatus` and Software Behavior

In this non-normative section we describe how the various components of the system software stack are expected to use `acecrstatus`.

We need to consider various situations:

* Neither S nor M/H implement lazy switching. +
Then, each privileged mode must implement the _Basic Rule_:
...
* S implements lazy switching, whereas M/H does not, it only needs to maintain compatibility with it. Hence, a *LAZY* CR is only stored in S and not in H/M.
...
.... before returning control to the U process.
...
* M/H implements lazy switching for the purpose of migrating VMs, where one cannot a priori know the size of the CRF at the destination, but S does not.
Therefore, S implements the Basic Rule.

[.red]#TBD#

* Both S and M/H implement lazy switching.
