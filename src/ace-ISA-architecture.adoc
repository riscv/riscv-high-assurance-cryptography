...
ECDSA-Verify on secp256r1 needs following information:
...
For secp384r1, the fields are all 48 bytes long.

For secp521r1, the fields are all 72 bytes long (zero-padded).

The input for ECDSA-Sign on secp256r1/secp384r1/secp521r1 are just the message hash, respectively 32. 48, and 72 bytes long.
 ...
 ** Metadata:
  ...
  *** For hashing, use the DirectionPolicy field to determine whether signature generation is allowed (bit 11) or signature verification (bit 10).
 ...
 * _output_ (8)
 * _completed_ (15)
 * The “*transient” states are used during long operations that require additional information.
   The format of this additional information is implementation defined and its length is given by an extra block after the metadata block
   in which the length, in bytes, is given as a 64-bit unsigned integer. This block is included in the authentication of the SCC.

* *Allowed State Transitions:*
 * _initial_ -> { _set_point_, _set_scalar_ } -> *
 * _set_point_ -> _set_scalar_
 * _set_scalar_ -> _set_point_
 * _set_point_ can be skipped if HasPoint == true, but it can be repeated.
 * _set_scalar_ can be skipped if HasScalar == true, but it can be repeated.
 * * -> _point_mul_ (only if HasPoint == HasScalar == true) -> _output_ -> _initial_
 * * -> _set_hash_ -> _sign_generate_ -> _output_ -> _completed_
 * * -> _set_hash_ -> _set_signature_ -> _sign_verify_ -> _completed_
