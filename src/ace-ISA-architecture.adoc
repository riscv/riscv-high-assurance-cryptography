...
Lazy loading defers loading a value until it is actually needed, which can improve performance and help manage limited CRF capacity.
This may even be necessary to support migration to systems with smaller CRFs.

However, if capacity is insufficient even for lazy loading, the software responsible for reimporting an offloaded CR must also be able to offload other CRs in order to free space for the requested one.
An alternative approach would require software to check for errors after each operation and handle capacity exhaustion itself, or to use an in-process trap handler. This would impose significant burden on software and likely degrade performance.
...
* This also means that in place of raising an _ace_err_memory_ error, out of CRF capacity situations should trap.
This mechanism could be architected, and the handler's address could be separately specified.
...
Let us define also value 0 in the entries of `acecrstatus` to support lazy loading.

* *LAZY* (value: 0).
+
--
*Definition*: Unconfigured. A value is saved for lazy loading in one of the parent modes.

 ** *Unconfigure*: sets status to *INITIAL*.
 ** *Configure*: Works as expected; sets status to *DIRTY*.
 ** *Export*: Trap to parent mode for lazy loading, possibly repeating the process up to Machine.
 The recovered SCC is imported into the CR (space may need to be freed), its status is set to *CLEAN*, and the interrupted export operation is restarted.
 ** *Usage*: Trigger a trap as for *Export*.
 The execution of the interrupted instruction restarts from state *CLEAN*.
--
+
