...
We describe a more fine grained context switching and locker tracking mechanism, based on lazy loading, that will allow significant optimizations.
Let us define first four properties for lockers:

[horizontal]
`configured`:: Indicates whether the locker is currently configured (value = 1).
`alien`:: For a configured locker, indicates whether the CC in the locker is owned by the current process (value = 0) or by a different one (value = 1), in which case we call that CC _alien_.
...
`lazy`:: Indicates whether a configuration for this locker for the current process is stored only in the switching context,
regardless of whether the locker itself is unconfigured or occupied by an alien CC.

It is the duty of the privileged code to keep track:
...
NOTE: This is optimised by having a table of the IDs of the processes that own the CCs in each locker, or 0 if it is unallocated.
...
** At context save in privileged code: Nothing to save, since the locker is already clean and the saved state also marks it as clean.
...
** At context save in privileged code: Nothing to save, the locker's state in the saved state must be marked as dirty.
...
** At context save in privileged code: nothing to do.
