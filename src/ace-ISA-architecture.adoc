...
* `Zlace` => Must support for the following CSRs: `aceerror` and `crstart` (this is a dependency for Zicsr); must support the following instructions: `ace.init`, `ace.exec`, `ace.clear`, `ace.export`, `ace.import`, and `ace.size`; `ace.exec` must support `Zleaes128`.

Priv:

* If `Zlace` is implemented: `misa` must implement the `L` bit, and `*status` must implement `ACES`.
* `Smacectk` => Defines the ACE Root Context Sealing Key CSRs (`macercsk{0..7}`), where all CSRs are MRO.
...
* `Smacestatus` => Defines the ACES per CC CSRs (`maceccstatus`)
...
|   [1:1] |   1  | Partial                | Normally, the value is 0. The value is 1 if any operation involving the configuration of the CC has been interrupted and may have to be restarted or resumed. If export is interrupted, it only needs `crstart` (<<ACE-CSR-crstart>>) for resumption.
...
`ace.export` is simply resumed using `crstart`.
...
Algorithm numbers 0-959 are architecture-defined and the list of such encodings is maintained by RVI.
See <<ACE-algorithms>> for the currently encoded algorithms.
Algorithm numbers 960-1023 are reserved for custom algorithms.
...
| 0xXXX          | URW       | `crstart`               | ACE start byte index
...
//We define here the 20 new CSRs introduced by ACE.
...
`crstart` is an XLEN-bit RW CSR, similar to `vstart`, tracking progress of long-running interruptible instructions `ace.init`, `ace.import`, `ace.export`, `ace.input`, and `ace.output`.
(((CSR, crstart)))
...
`crstart` therefore records a byte offset.
...
Only the instructions explicitly mentioned above support non-zero `crstart` values; other ACE instructions ignore this register, and will be restarted.

[NOTE]
====
_A single `crstart` value suffices due to the way the init/import/export instructions operate:_
...
The semantics and use of `crstart` are described in <<ACE-format-configuration> and [.red]#TBD#. zzzz
...
Two groups of M-mode WARL CSRs are used for two reconfigurable Localities (cf.{nbsp}<<ACE-concepts>> and <<ACE-Localities>>).
These are:
...
// next instruction preemptible, with `crstart` from 0 to 7
...
foreach(i from 0 to num_blocks - 1) {
    // next instruction preemptible, with `crstart` from 32+i*16 to 39+i*16 or 47+i*16
    UNINTERRUPTIBLE_BLOCK_START
...
// if preempted here, `crstart` will be z
UNINTERRUPTIBLE_BLOCK_START
...
In the pseudocode listings, we mark the inunterruptible code blocks with `UNINTERRUPTIBLE_BLOCK_START` and `UNINTERRUPTIBLE_BLOCK_END`:
Interruption of the export or import operatrions is only possible before a `UNINTERRUPTIBLE_BLOCK_START` or after a `UNINTERRUPTIBLE_BLOCK_END`, and when an interrupted operation is resumed, it will resume at the point of interruption.

The part before the first `UNINTERRUPTIBLE_BLOCK_START` consists only of type declarations or of information that is trivial to reconstruct (for instance, once the metadata is known, the size of the content and some values derived from it).

As the names imply, `SERIALIZE()` and `DESERIALIZE()` serialize and deserialize the content of a CR as defined in <<ACE-algorithms>>.
The `content` array does not necessarily represent the actual data in the memory of the CR, it is instead a virtual representation of that data: values with consecutive indices may not be in fact contiguous and one byte may be split across multiple bytes, for instance if keys are split into multiple shares for threshold implementations are used. In any case, reading and writing to `context` must modify the internal representation of a context accordingly, writing some information and then reading it back must yield the same result, and reading some data and then writing it back must given the exact same microarchitectural state.
...
// preemptible here, with `crstart` from 0 to 15
UNINTERRUPTIBLE_BLOCK_START
|   M[0 .. 15] = N[63:0] @ metadata[63:0];
UNINTERRUPTIBLE_BLOCK_END

// preemptible here, with `crstart` from 16 to 31
UNINTERRUPTIBLE_BLOCK_START
...
|   {Kenc, Kauth} = RFC8452_KeyDeriv(CSK);
|
|   // next instruction preemptible, with `crstart` from 8 to 19
|   N = M[8 .. 19]
UNINTERRUPTIBLE_BLOCK_END

// next instruction preemptible, with `crstart` from 20 to 31
UNINTERRUPTIBLE_BLOCK_START
|   SIV = M[20 .. 31]
|   tmp = 64'b0 @ metadata;
...
// if preempted here, `crstart` will be z + 32
...
An initialization or import operation may be interrupted by context switching code, i.e., not returning to the original control flow immediately after the handler is done. In this case the intermediate state of the CR must be saved and then restored in a way that allows for resumption of the operation (by setting also the value of `crstart`).
(((Cryptographic Register,incomplete)))
Then exporting the partial CR, the following metadata header format is used as the first 8 bytes of the SCC to store information about the partial CR.
After this header block, the SCC contains the nonce and SIV following <<ACE-format-SCC>>, and then the content section.

The Partial CR Metadata (PCREM) header must distinguish between partial CRs following interrupted
initialization and import operations, and in the second case also the content section must be modified accordingly.
In particular, for the interrupted import case, we need to save the current partial tag value that will have to be verified at the end,
but the original metadata block, nonce and SIV can be recovered from the original SCC.
Note that the metadata of that SCC can also indicate a partial initialization or import operation.
...
The content format to export incomplete initializations is the same, only truncated to the actual imported length, rounded to a multiple of 8 bytes.

The content format to export incompletely imported CRs prepends the partial hash (`tmp` in <<ACE-SCC-import>>) to the content, which is thus extended by 16 bytes,
These 16 bytes must be included in the Payload_Length field and encrypted as well.

After the partial CR has been decrypted and authenticated in privileged mode, it is marked as to be resumed, in which case, upon returning to the less-privileged mode, the following operations must be performed:

. The old metadata, nonce and SIV are recovered from the memory of the less-privileged mode. These reads may be interrupted by page faults, but they are easy to resume.
. `z` and `num_blocks` are computed from the original content length, and the starting value of `i` with the help of the `crstart`.
. The `Kenc` and `Kauth` keys are derived.
. The old partial hash value is restored.

At this point the initialization or import operation can resume from the interrupted point.
...
* `crstart` counts all the bytes of the payload, including the metadata for all inputs and outputs, and the nonce and the tag as well for SCCs.
 ...
 ** `ace.init`:
  ...
  *** If the operation is preempted, `crstart` will be the number of bytes read.
  *** The corresponding exported SCC will include an 8-byte block in clear after the metadata with the `crstart` value, followed by the nonce and the tag, and the content will only contain the payload imported and read so far, zero padded up to a multiple of 8 bytes. This content is of course encrypted.
...
* it is _trappable_, i.e., can be implemented by privileged code using custom instructions, and whether
* it can be _interrupted_, for instance by context switches, page faults or other interrupts.

NOTE: Trappability is distinct from custom implementations of primitives within the ACE unit.
...
The instruction must follow the interruptibility rules defined in <<ACE-CSR-crstart>>.
...
|   2   | ace_state_hash_finalize       | Finalize the internal state of a hash function and possibly prepare it for outputting the result. In this state additional data may be absorbed, such as lengths.
|   3   | ace_state_hash_finalize_enc   | Finalize the internal state of a hash function for the encryption algorithm of an authenticated encryption mode of operation and possibly prepare it for outputting the result. In this state additional data may be absorbed, such as lengths.
|   4   | ace_state_hash_finalize_dec   | Finalize the internal state of a hash function for the decryption algorithm of an authenticated decryption mode of operation. In this state additional data may be absorbed, such as lengths.
|   5   | ace_state_hash_verify         | Finalize the internal state of a hash function and compare it to the provided value.
 ...
 * A `mv` instruction that loads a specified number of bytes from a GPR or vector register into the state of a CR within the ACE unit.
   ...
   The instruction also updates the `crstart` CSR to follow the internal progress.
 ...
 * A “`close`” instruction to suspend or finalize the procedure.
   If executed before all bytes are transferred, the CR state matches that of an interrupted monolithic instruction, including the Incomplete metadata field and the `crstart` value. If executed after all bytes are transferred, it also performs any final cryptographic operations, such as authenticating the CR, if needed.
...
in particular, they need to respect the value of `crstart` and update it in accordance with the expected architected behavior of the emulated instructions.
...
Fields are listed in the order they appear in the initialization input, each with its length in bits.

. *Internal State*:
Information that must be maintained in the internal state of a CC.
The fields are unordered, and their lengths in bits may differ from those listed—for example, in threshold or fault-tolerant implementations where redundancy is added. Notably, the full single-share key size is listed even when a System Key is used, as this reflects the size of the key material the algorithm operates on.

. *Serialized Content*:
The data that follows the metadata block in the plaintext portion of the SCC.
The fields are listed in the order they appear in the SCC, each with its length in bits.
...
--
+
If `VLEN` > `b`, the above operation is executed `VLEN/b` times.
...
We only support the OCB3 variant of OCB as specified in RFC 7253.
While this algorithm is asymptotically more efficient than GCM and GCM-SIV, it has a much heavier setup and
its state, both internal and exported requires significantly more memory than the other modes.
...
+
--
* `b` = block size in bits. `b` is a power of two, at least 64. For this algorithm we require `VLEN` to be an integer multiple of `b`.
* `k` = key size in bits, `b`|`k`.
--
+
...
* *States:* _initial_, _set_public_value_, _set_public_value_2_, _hash_absorb_, _encrypt_, _last_block_enc_, _hash_finalize_enc_, _decrypt_, _last_block_dec_, _hash_finalize_dec_, _hash_verify_, and _completed_.
 ...
 ** _encrypt_ -> _last_block_enc_ -> _hash_finalize_enc_ -> _hash_output_, later automatically transitioning to _completed_.
 ** _decrypt_ -> _last_block_enc_ -> _hash_finalize_dec_.
 ** _hash_finalize_dec_ -> _hash_verify_, later automatically transitioning to _completed_.
 ...
 ** To move to _set_public_value_, the `ace.state` operation must use the additional argument, which sets the nonce length `Nlen`.
...
** To move to _set_public_value_2_, the `ace.state` operation must have the additional argument, which sets the tag length `taglen`.
  ...
  *** If `N[u] == 1'b1`, this means that `N` was not set in state _set_public_value_. We do not allow code that skips nonce setting, as this mode is not secure under nonce reuse, therefore in this case invalid instruction error is raised, and the CR is *invalidated*.
  *** The following operation must be a `ace.state` to move to _hash_absorb_
...
** Upon entering states `_encrypt_` or `_decrypt_`, following operation are performed:
+
--
 * `Nonce : bits(b)`
 * `Stretch : bits(3b/2)`
...
[]
 ...
 . `L <- double(L)`
 . `L <- double(L)`
 . `Nonce <- N[Nlen-1:0] @ 1'b1 @ zeros(b-g-1-Nlen) @ numtobits(taglen,g)`
 . `top <- str2num(Nonce[b-1:b-g+1])`
 . `Kbot <- enc_blk(K, zeros(g-1) @ Nonce[b-g:0])`
 . `Stretch <- (Kbot[b/2-1:0] xor Kbot[b/2+7..8]) @ Kbot`
 . `offset <- Stretch[b+top:top]`
--
+
** In state _encrypt_, only ``ace.exec`` instructions of Form A are allowed. +
...
--
+
If `VLEN` is a multiple of `b`, the above operation sequence is executed `VLEN/b` times, encrypting each `b`-bit block of `Vs` into the corresponding `b`-bit block of `Vd`.

** To move to _last_block_enc_, the `ace.state` operation must have the additional argument, which sets the bit length of the final block of the plaintext in `last_blk_len`.

** In state _last_block_enc_, only an ``ace.exec`` instruction of Form A "```ace.exec Vd,Kn|K[Xn],Vs```," is allowed if `last_blk_len` is nonzero.
...
[]
  . `aux <- enc_blk(key, zeros(b))`
  . `aux <- double(aux)`
  . `hash_P <- enc_blk(K, hash_P xor offset xor aux) xor hash_A`
--
+
In both cases the state then transitions to _hash_finalize_enc_.

** In state _hash_finalize_enc_,
only an ``ace.exec`` instruction of Form C, "```ace.exec Vd,Kn|K[Xn]```," is allowed, which puts `hash_P[taglen-1:0]` into `Vd`, zero padded.
...
** To move to _last_block_dec_, the `ace.state` operation must have the additional argument, which sets the bit length of the final block of the ciphertext in `last_blk_len`.

** In state _last_block_dec_, only an ``ace.exec`` instruction of Form A "```ace.exec Vd,Kn|K[Xn],Vs```," is allowed if `last_blk_len` is nonzero.
In this case, the same operations as in state _last_block_enc_ are performed except for line 6., that reads instead
+
--
[start=6]
...
--
+
If `last_blk_len` is zero, only an ``ace.exec`` instruction of Form D, "```ace.exec Kn|K[Xn]```," is allowed, which performs the same operations as in state _last_block_enc_.
+
For any value of `last_blk_len`, the state is changed to _hash_verify_.

** In state _hash_verify_, only an ``ace.exec`` instruction of Form B, i.e., "```ace.exec Kn|K[Xn],Vs```" is allowed.
    It compares the `taglen` least significant bits of `Vs` with the `taglen` least significant bits of `hash_P`:
+
--
  . If the values match, then the state is changed to _completed_, and `ace.error` returns no error,
  . else the state is changed to _completed_, and `ace.error` returns `ace_err_authentication`.
...
[.red]#TBD: fix the endianness and the base of all the fields!!!#.
 ...
 ** Upon issuing `ace.state Kn|K[Xn],#ace_state_last_block, Xs` where `Xs` contains the bit length of the last block in bits, the following happens:
 ...
 ** `partial_len` -- Length of absorbed data so far for the current block (field size at most 16 bits).
 ** `partial_block`: `b` bits (a priori we cannot know whether `b > VLEN` or not, and changing the internal memory requirements of a CC depending on the `VLEN` can lead to memory exhaustion).
...
[lowerroman]
 ...
 . `hash_state`: `n` bits.
 . padding: 16 bits.
 . `used_VLEN`: 16 bits -- Current vector length in bits (up to MAXVLEN).
 ...
 . `partial_len`: 16 bits -- Length of absorbed data so far for the current block.
