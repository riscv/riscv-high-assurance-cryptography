...
* Bits [14:0] and [25:19] of the input register `Xs` are ignored.
* Bits [16:15], [18:17] and [63:32] of the input register `Xs` have the same format as the corresponding bits of the metadata.
* Bits [16:15] can be used to turn on or increase the level of the side-channel protection of the algorithm in the CC, if the microarchitecture supports the corresponding implementation. The levels are defined in <<ACE-SC-protection-levels>>. Downgrading side-channel protection is not permitted, and attempting to do so will result in error _ace_err_invalid_ (cf.{nbsp}<<ACE-error-codes>>).
* Bits [18:17] can be used to deactivate encryption or decryption in a CC that allows both. Writing to them cannot be used to deactivate both encryption and decryption simultaneously, nor to activate encryption, resp., decryption in a CC that did not allow it.
