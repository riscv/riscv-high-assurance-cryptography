...
Its definition is in flux, as it is subject to change based on the ARC feedback.
...
_Encoding_::
`ace.avail` is an alias to Form B of `ace.size`, as it returns 0 if the algorithm is not supported.
...
[.red]#Use only Localities instead?#
