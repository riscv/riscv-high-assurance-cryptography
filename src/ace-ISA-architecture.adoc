...
| [29:26] |   4  | UC_Policy_1            | “UC Policy” means “Usage Control Policy”. If bit 26, 27, 28, resp. 29 is set, it disallows the _usage_ of this CC in User, (V)S-mode, H-mode, resp. M-mode.
...
* Restrictions by `ASID` are not applied if in S-mode or higher; and restrictions by `VMID` are not applied if in H- or M-mode.

* If S-mode is not implemented, and bits 30 and [47:32] must be set to 0.

* If the H extension is not enabled, bits 31 and [63:48] must be set to 0.

* However, see <<ACE-CSR-reuse-ASID-VMID>> for possible extensions of this mechanism to the cases where S-mode or H-mode is not supported.
