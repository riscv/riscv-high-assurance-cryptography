...
[width="100%",cols="16%,27%,57%",options="header",subs="+macros"]
|===
...
`Cd\|C[Xd], Cs\|C[Xs], Vs2` +
|  Optional. Uses the source CC to configure the destination CC. Used if the source CC is, for instance, a KEM that configures the destination CC with a symmetric key. Then, the SW can clear the source CC to release resources.
| `ace.state` +
`(ace.dir)`
| `Cd\|C[Xd], #immed` +
