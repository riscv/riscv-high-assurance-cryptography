 ...
 * *Behavior:*
 ** While in the _initial_ state, immediately after configuration, the ACE unit performs:
 ...
 ** In the _initial_ state, any `ace.exec` will result in `ace.error` returning ace_error_invalid.
 ** In the _set_public_value_ state, we can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Cn|C[Xn], Vs`{nbsp}{nbsp} to set `counter <- Vs << b/4`
    (there is no harm if this instruction is issued more than once).
    If no such instruction is issued, counter is assumed to be zero.
  *** Other forms of `ace.exe` not allowed, will cause `error(ace_error_invalid)`.
 ** In _encrypt_ and _decrypt_ states, we can only issue `ace.exec Vd, Cn|C[Xn], Cs` which:
  *** If the state is _encrypt_, we can only issue the `ace.exec` form{nbsp}{nbsp} `ace.exec Vd, Cn|C[Xn], Vs`{nbsp}{nbsp} to
   **** Encrypt `Vs` into `Vd`.
   **** Accumulate `Vd` into the auth_tag, namely `auth_tag <- auth_tag xor Vd` and then `auth_tag <- auth_tag * hash_key`.
  *** If the state is _decrypt_,  [.red]#To be completed.#
  *** ... and updates the counter: `counter[b/4-1:0] <- counter[b/4-1:0] + 1`. If the lower b/4 bits of the counter roll back to zero,
      the internal state will be invalidated (state set to _ace_completed_, all fields zeroed except the key) and ace.error will return ace_err_invalid.
