...
|===
// | [31:30] |   2  | UC_Policy_2            | If bit 30, resp., 31, is set, then restrict the usage by `ASID`, resp., `VMID`.
...
Their values are compared against the Filter field of the metadata, to determine whether an `ace.exec`, `ace.state`, `ace.harden` or `ace.derive` instruction is allowed to execute:
...
[WARNING]
These two registers are placeholders for a later decision on how to handle the filtering of CC usage in different modes.
...
[[ACE-algorithms]]
=== Data Formats and Instruction Semantics for Architected Algorithms
...
.2+| 1  | ace_state_hash_absorb         | Start (and initialize) a hash function or the sub-function of an algorithm, start to absorb blocks of data.
<| ace_state_operate                     | Generic operational state.
|   2   | ace_state_hash_last_block     | In this state additional data may be absorbed, such as partial blocks and lengths, if the algorithm processes the last block of (a section of) its input in a different way from the other blocks.
|   3   | ace_state_hash_finalize       | Finalize the internal state of a hash function and possibly prepare it for outputting the result.
|   4   | ace_state_hash_verify         | Compare the internal state of a hash function to the provided value.
|   5   | ace_state_hash_output         | Enter the hash output phase of a hash_function.
|   6   | ace_state_enc_last_block      | For the encryption path of authenticated encryption modes of operation to process “last block” separately.
|   7   | ace_state_dec_last_block      | For the decryption path of authenticated encryption modes of operation to process “last block” separately.
|   8   | ace_state_enc_hash_finalize   | Finalize the internal state of a hash function for the encryption algorithm of an authenticated encryption mode of operation and possibly prepare it for outputting the result.
|   9   | ace_state_dec_hash_finalize   | Finalize the internal state of a hash function for the decryption algorithm of an authenticated decryption mode of operation.
|  10   | ace_state_encrypt             | Encryption is requested.
|  11   | ace_state_decrypt             | Decryption is requested.
...
<| ace_state_input_for_derive   | Tells the CC to expect additional information for feeding the `ace.derive` instruction.
...
===== Rules when using the `ace.input` and `ace.output` instructions
...
If `aceiobuflen` is non-zero, Forms A, B, and C of `ace.exec` and `ace.derive` can be replaced as follows:
...
The internal input/output buffer is not part of a CR but is a separate structure. It is also architectural, i.e., per-hart.
...
In all algorithms, whenever a Form A-C `ace.exec`, or `ace.derive` is replaced by a sequence of instructions, we need to make another adjustment, namely that
in place of `VL*SEW` we use `aceiobuflen`. In this case, `b` must divide `aceiobuflen` and `acestart`, instead of `VL*SEW` and `acestart`.
 ...
 ** In states _encrypt_ or _decrypt_, we can only issue Form A `ace.exec` instructions, i.e., of the Form “`ace.exec Vd, Kn|K[Xn], Vs`.”
  *** If the state is _encrypt_, then `Vd = enc_blk(key, Vs)`.
  *** If the state is _decrypt_, then `Vd = dec_blk(key, Vs)`.
 ...
 ** In state _set_public_value_:
  *** Only  ``ace.exec`` instructions of Form B are allowed.
  “```ace.exec Kn|K[Xn], Vs```” sets
   . `ctr <- Vs @ (b/4-1)b'0 @ 1`, i.e., the `b/4` most significant bits of `Vs` are ignored, and the `b/4` least significant bits of `ctr` represent the integer 1.
 ...
 ** In state _hash_absorb_, only  ``ace.exec`` instructions of Form B are allowed. +
 “```ace.exec Kn|K[Xn], Vs```” absorbs a data block in `tag`, i.e.:
+
--
 . `absorb(Vs)`.
--
...
** In state _encrypt_, only ``ace.exec`` instructions of Form A are allowed.
   “```ace.exec Vd, Kn|K[Xn], Vs```” encrypts `Vs` into `Vd`, and absorbs the ciphertext `Vd` into the `tag`, namely:
+
--
...
+
--
*** `Vs = bin(bit_length(AD), b/2) @ bin(bit_length(plaintext), b/2)`
--
+
...
--
  ...
  . Finally, `tag` is returned in `Vd`, and the state is changed to _completed_.
--
+
If `VL*SEW` > `b`, only the `b` least significant bits of `Vs` are considered.
...
** In state _decrypt_, only  ``ace.exec`` instructions of Form A are allowed.
 `ace.exec Vd,Kn|K[Xn],Vs`
   absorbs the ciphertext `Vs` into the `tag`, then decrypts `Vs` into `Vd`, namely
+
--
  . If `ctr[b/4-1:0] == 0`, then
  .. the CR is *invalidated*.
...
The value +
 `{nbsp}{nbsp}{nbsp}Vs = bin(bit_length(AD), b/2) @ bin(bit_length(plaintext), b/2)` +
...
--
+
If `VL*SEW` > `b`, only the `b` least significant bits of `Vs` are compared to `tag`.
...
This mode is useful in scenarios where the IV needs to be fixed.

The most important difference between GCM and GCM with a Predefined IV is in the structure of the Initialization Input:
...
This mode _cannot_ be emulated with an ECB CR because it would reveal the internal data.

For all the following IV- or nonce-based modes, versions with a set IV or nonce (such as for GCM-SIV) can also be defined.
For instance, for GCM-SIV (cf.{nbsp}<<ACE-GCM-SIV-mode??), `nonce` would be included in the Initialization Input exactly as `IV` is included in the Initialization Input for GCM, and state _set_public_value_ would be skipped, going directly to _hash_absorb_  in the encryption path, and to _set_public_value_2_ in the decryption path.
...
+
--
   *** Only ``ace.exec`` instructions of the Form “```ace.exec Kn|K[Xn], Vs```” are allowed. Such an instruction accumulates a value in the `tag`:
...
. If `ctr == 0`, then the CR is *invalidated*.
   . If the `SIV` is `b`-bits long, then
     .. ``Vd <- Vs xor enc_blk(enc_key, 1 @ SIV[b-2:b/4] @ (SIV[b/4-1:0] + ctr % 2^b/4^)),``
   . else (the `SIV` is `3b/4`-bits long)
      .. ``Vd <- Vs xor enc_blk(enc_key, 1 @ SIV[3b/4-2:0] @ ctr``.
   . `ctr <- ctr + 1`.
--
+
If `VL*SEW` > `b`, the above operations are applied to each of the `VL*SEW/b` blocks in `Vs`.

** In state _decrypt_,  only  ``ace.exec`` instructions of the Form A are allowed, namely “```ace.exec Vd, Kn|K[Xn], Vs```,” to decrypt `Vs` into `Vd` and then absorb the plaintext `Vs` into the `tag`:
+
--
     . If `ctr == 0`, then the CR is *invalidated*.
     . If the `SIV` is `b`-bits long, then +
...
** Upon completion of `ace.import`, the ACE unit recomputes `enc_key` and `auth_key`
  . (`enc_key`, `auth_key`) <- `RFC8452_KeyDeriv(key)`.
...
** In state _hash_finalize_, only one `ace.exec` instructions of Form B is allowed if `last_blk_len` != `0`.
 “```ace.exec Kn|K[Xn], Vs```” which absorbs the last data block in `tag`, i.e.:
+
--
  * `tmp : bits(b)`
...
[]
  ...
  . `tmp <- (zeros(b-last_blk_len-1) @ 1 @ Vs[last_blk_len-1:0]) xor offset`
...
+
** In state _encrypt_, only ``ace.exec`` instructions of Form A are allowed:
   “```ace.exec Vd, Kn|K[Xn], Vs```” absorbs the plaintext `Vs` into the `tag` an encrypts `Vs` into `Vd`, namely:
+
--
 . `L <- double(L)`
 . `offset <- offset xor L`
...
** In state _decrypt_, only ``ace.exec`` instructions of Form A are allowed.
    “```ace.exec Vd, Kn|K[Xn], Vs```” decrypts `Vs` into `Vd`, and absorbs the plaintext `Vd` into the `tag`, namely:
+
--
  * `tmp : bits(b)`
...
--
[start=6]
. `tmp2 <- (zeros(b-last_blk_len-1) @ 1 @ Vs[last_blk_len-1:0]) xor offset`
--
+
...
[WARNING]
====
[.red]#*TBD*: relax the condition that either the block size is an integer multiple of the vector length or the vector length is an integer multiple of the block size.#
====
...
* `t` = tag len (8|`t`). This is the size of the digest. The present description assumes `t` :le: `b`.
 ...
 ** `input_base`: int. offset within input to read from (field size at most 16 bits).
...
[lowerroman]
 ...
 . `cumul_len`: 32-bit unsigned int. Cumulative length of all data absorbed so far (if the algorithm must process and incorporate it in the hash on its own).
 . `block`: `b` bits.
...
* `absorb_last_block()`: process `block` into the hash or MAC, using a different function for the last block.
* `finalize() -> result`: process the `state` into the digest value. Assumed to re-use `block` (the digest or a unit of XOF output should not be be bigger).
 ...
 ** In state _initial_:
  ...
  ** In state _hash_absorb_, we can only issue ``ace.exec`` instructions of Form B, i.e., “```ace.exec Kn|K[Xn], Vs```”, to absorb the data block `Vs` into the hash as follows.
+
--
  * `input_remaining` : int
  * `input_base` : int {nbsp}{nbsp} // This is the value written into `acestart` if the operation is interrupted.

[]
  . `input_remaining <- VL*SEW {nbsp}{nbsp}`// or `VL*SEW-acestart` upon resumption
  . `input_base <- 0 {nbsp}{nbsp}` // or `acestart` upon resumption
  . while `(input_remaining > 0)` do {
  .. if `(input_remaining ≤ b-block_base)` then {
  ... ``block[block_base + input_remaining - 1 : block_base] <- ``{empty} +
``{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}<- Vs[input_base + input_remaining - 1 : input_base]``
  ... `input_base <- input_base + input_remaining`
  ... `block_base <- block_base + input_remaining`
  ... `input_remaining <- 0` // the input vector has been used up
  ... `input_base <- 0`
  .. } else {
  ... ``block[b - 1 : block_base] <- ``{empty} +
``{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}<- Vs[input_base + b - 1 - block_base : input_base]``
  ... `input_base <- input_base + (b - block_base)`
  ... `input_remaining <- input_remaining - (b - block_base)`
  ... `block_base <- b` // the block buffer has been filled
  .. }
  .. if (`block_base` == `b`) then {
  ... `absorb()`
  ... `block_base <- 0`
  ... if (`input_remaining` == 0) then we terminate the instruction.
...
// .. *** `block_base` != `ones(16)` and `block_base` < `last_block_len`;
// ..  we can only issue ``ace.exec`` instructions of Form B, i.e., “```ace.exec Kn|K[Xn], Vs```”, to absorb a single data vector `Vs` into the hash.
// ..  The operations are mostly the same as the operations for state _hash_absorb_ except that:
// ..  . Only one block is absorbed.
// ..  . The length of the block in bits is given by `block_base` and not by the parameter `b`.
// .. *** `block_base` != `ones(16)` and `block_base` > `last_block_len`: The CR is *invalidated*.
// .. *** `block_base` = `last_block_len`.
// ..  . To handle this case, the last three instructions must be changed to
// ..[start=4]
// ..  .. if (`block_base` == `last_block_len`) then {
// ..  ... `absorb_last_block()`
// ..  ... the state transitions to _hash_outout_ and the instruction terminates. }
// .. *** and `block_base` == `ones(16)`;
// ..   We can only issue ``ace.exec`` instructions of Form C, i.e., “```ace.exec Vd, Kn|K[Xn]```”, to output the hash value.
// ..   Then
// ..   **** Hash Case
// ..   **** XOF case
...
2. Let n = len(P)/r.
3. Let c = b - r.
4. Let P[0], … , P[n-1] be the unique sequence of strings of length r such that P = P[0] || … || P[n-1].
5. Let S = zeros(b)
.
6. For i from 0 to n-1, let S = f (S xor (Pi @ zeros(c))).
...
* `b` = 128. For this algorithm we require `VL*SEW` to be an integer multiple of `b`.
* `k` = 128.
...
* *States:* _initial_, _set_public_value_, _hash_absorb_, _encrypt_, _enc_last_block_, _enc_hash_finalize_,
 _decrypt_, _dec_last_block_, _dec_hash_finalize_, _hash_verify_, and _completed_.
 ...
 ** _hash_absorb_ -> _encrypt_ -- if metadata allows encryption, else the CR is *invalidated*.
 ** _hash_absorb_ -> _decrypt_ -- if metadata allows decryption, else the CR is *invalidated*.
 ** _encrypt_ -> _enc_last_block_, _enc_hash_finalize_, later automatically transitioning to _completed_.
 ** _decrypt_ -> _dec_last_block_, _dec_hash_finalize_, _hash_verify_, later automatically transitioning to _completed_.
 ...
 ** In state _set_public_value_:
  *** Only  ``ace.exec`` instructions of Form B are allowed.
  “```ace.exec Kn|K[Xn], Vs```” sets
   . `state[3] <- Vs[63:0]`
   . `state[4] <- Vs[127:64]`
  *** If “```ace.exec Kn|K[Xn], Vs```,” is repeated, it will just re-write the nonce.
  *** If no such `ace.exec` to set the nonce is issued, the nonce subfield of `state` remains zero.
  *** If `VL*SEW` > 128, only the 128 least significant bits of `Vs` are considered.
  ...
  ** In state _hash_absorb_, only  ``ace.exec`` instructions of Form B are allowed. +
 “```ace.exec Kn|K[Xn], Vs```” absorbs a data block into the state, i.e.:
+
--
 . `ASCON(8)`
 . `state[0] <- state[0] xor Vs[63:0]`
 . `state[1] <- state[1] xor Vs[127:64]`
--
+
If `VL*SEW` > 128, the above group of operations is executed `VL*SEW/128` times, once for each 128-bit block of `Vs`.
...
** In state _encrypt_, only ``ace.exec`` instructions of Form A are allowed. +
   “```ace.exec Vd, Kn|K[Xn], Vs```” encrypts `Vs` into `Vd`, namely:
+
--
. `state[0] <- state[0] xor Vs[63:0]`
. `state[1] <- state[1] xor Vs[127:64]`
...
--
+
If `VL*SEW` is a multiple of 128, the above group of operations is executed `VL*SEW/128` times, encrypting each 128-bit block of `Vs` into the corresponding 128-bit block of `Vd`.
...
“```ace.exec Vd, Kn|K[Xn], Vs```”  performs
+
--
. `state[0] <- state[0] xor Vs[63:0]`
. `state[1] <- state[1] xor Vs[127:64]`
. `Vd <- state[0] @ state[1]` // only the least significant 128 bits of `Vd` are written to.
--
+
In this state only one 128-bit block is processed. +
The state machine then transitions to state _enc_hash_finalize_.

** In state _enc_hash_finalize_, only one ``ace.exec`` instruction of Form C, to compute the tag, is allowed.
“```ace.exec Vd, Kn|K[Xn]```”  performs
+
--
...
. `state[3] <- state[3] xor key[63:0]`
. `state[4] <- state[4] xor key[127:64]`
. `Vd <- state[3] @ state[4]` // only the least significant 128 bits of `Vd` are written to.
--
+
In this state only one 128-bit block is processed. +
...
** In state _decrypt_, only  ``ace.exec`` instructions of Form A are allowed., i.e.,
 `ace.exec Vd,Kn|K[Xn],Vs`. This operation performs:
+
--
* `tmp : bits(128)`

[]
. `tmp <- (state[0] xor Vs[63:0]) @ (state[1] xor Vs[127:64])`
. `state[0] <- Vs[63:0]`
. `state[1] <- Vs[127:64]`
. `ASCON(8)`
. `Vd <- tmp`
--
+
If `VL*SEW` is a multiple of 128, the above group of operations is executed `VL*SEW/128` times, decrypting each 128-bit block of `Vs` into the corresponding 128-bit block of `Vd`.
...
--
+
The state machine then transitions to state _dec_hash_finalize_.

** In state _dec_hash_finalize_, an `ace.state Kn|K[Xn], #ace_const_dec_hash_finalize, Xs` instruction may be executed, setting
...
. `state[3] <- state[3] xor key[63:0]`
. `state[4] <- state[4] xor key[127:64]`
. `Vs[tag_len-1:0]` is compared to `(state[3] @ state[4])[tag_len-1:0]` +
If the values match, then the state is changed to _completed_, and `ace.error` returns no error,
else the state is changed to _completed_, and `ace.error` returns `ace_err_authentication`.
--
+
...
* `b` = 64. For this algorithm we require `VL*SEW` to be an integer multiple of `b`.
* `k` = 128.
...
+
--
 . `state[0] <- state[0] xor Vs`
 . `ASCON(12)`
--
+
If `VL*SEW` > `64`, the above group of operations is executed `VL*SEW/64` times, once for each `64`-bit block of `Vs`.

** Upon entering _hash_finalize_, `countdown` is set to 3.

** In state _hash_finalize_, at most four ``ace.exec`` instructions of Form C, to compute the tag, are allowed.
“```ace.exec Vd, Kn|K[Xn]```”  performs
+
--
. `ASCON(12)`
...
--
+
If `VL*SEW` > `64`, the above group of operations is executed `VL*SEW/64` times, writing to `Vd[63:0]` the first time,
`Vd[127:64]` the second time, `Vd[191:128]` the third time, `Vd[255:192]` the fourth time.
If more ``ace.exec`` instructions are required, writing restarts from `Vd[63:0]`.
...
this may occur also if If `VL*SEW` > `256`.
...
four, two, or one ``ace.exec`` instruction is required, respectively.
...
===== Ascon-CXOF128 extendable-output function with user-defined customization string
...
We describe here the algorithms for the primitives in the SHA-3 family.
...
[WARNING]
====
[.red]#In <<ACE-hash-functions>> there is a condition that either the block size is an integer multiple of the vector length or the vector length is an integer multiple of the block size. It must be relaxed to allow for more flexibility in the design of hash functions, and include SHA3-224 more easily.#
====


These are simple instantiations of the algorithm specified in <<ACE-hash-functions>>.
...
* `b` = block size (8|`b`). It can be 1600 - `c` with `c` = 448, 512, 768, or 1024.
* `n` = 1600
 ...
 ** `block_base`.  Length of absorbed data so far for the current block (field size at most 16 bits). Only if used by the algorithm.
 ** `block`: `b` bits (a priori we cannot know whether `b > VL*SEW` or not, and changing the internal memory requirements of a CC depending on the `VL*SEW` can lead to memory exhaustion).
...
// . `used_VLxSEW`: 16 bits. Current vector length.
 . `block_base`: 16 bits. Length of absorbed data so far for the current block.
 . `block`: `b` bits. We include it only if there is a partial block, and exclude it when not.
...
* *Behavior:*
 ** In state _initial_:
  *** `state`: [.red]#insert here the IV etc.#
