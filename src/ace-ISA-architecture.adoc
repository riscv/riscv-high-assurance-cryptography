 ...
 ** _encrypt_ -> _enc_hash_finalize_ -> _completed_.
 ** _decrypt_ -> _dec_hash_finalize_ -> _hash_verify_ -> _completed_.
 ...
 ** _encrypt_ -> _enc_last_block_ -> _enc_hash_finalize_ -> _hash_output_ -> _completed_.
...
--
  ...
  .. ``block[block_base + amount - 1 : block_base] <-`` +
      ``{nbsp}{nbsp}{nbsp}{nbsp} <- Vs[input_base + amount - 1 : input_base]`` +
        ...
        ``{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}{nbsp}+ Vs[input_base + amount - 1 : input_base]`` , +
...
These, and RawSHAKE128, RawSHAKE256 as well, are simple instantiations of the algorithm specified in <<ACE-hash-functions>>.
These algorithms are instantiations of a specific parameter choice of KECCAC cite:[DBLP-conf-eurocrypt-BertoniDPA13], and have been standardized by NIST as SHA-3 cite:[nist-fips-202].
...
* *Behavior:* (We describe only the differences with respect to the generic behavior described in <<ACE-hash-functions>>.)
 ...
 ** _hash_absorb_ -> _encrypt_ -- if metadata allows encryption, else the CR is *invalidated*.
 ** _hash_absorb_ -> _decrypt_ -- if metadata allows decryption, else the CR is *invalidated*.
 ** _encrypt_ -> _enc_last_block_ -> _enc_hash_finalize_ -> _completed_.
 ** _decrypt_ -> _dec_last_block_ -> _dec_hash_finalize_ -> _hash_verify_ -> _completed_.
