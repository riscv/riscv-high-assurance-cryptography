...
A CC is data structure comprising a *metadata section* and a *content section* that form an indivisible unit.
...
These requirements are guaranteed by enforcing that all access and modification of a CC follow the rules defined by the ACE architecture and the algorithm.
...
A CC encapsulates its behavior, allowing interchangeable use of ciphers like AES-128, AES-256, Калина-128, Калина-256, and SM4, and so on, in various modes (ECB, GCM-SIV, XEX/XTS) without code changes, provided the code supports variable SCC sizes. Side-channel-resistant variants are also supported transparently.
...
[.darkred]#Discussions are still ongoing on what subset will be required, and if a dependency on just the GPRs can be provided in addition.#
...
`(ace.dir)`
| `Cd\|C[Xd],#immed` +
`Cd\|C[Xd],#immed,Xs`  |  Modifies algorithm state (e.g., phase transitions). `ace.dir` is an alias.
| `ace.restrict` +
  `(ace.harden)`  | `Cd\|C[Xd],Xs`                   |  Narrows Usage Policies and enables/increases side-channel protection. Reversing these changes is not permitted. `ace.harden` is an alias.
| `ace.input`     | `Cd\|C[Xd],%offset(Xs)`          |  Optional. Provides input to the algorithm (for long inputs).
| `ace.output`    | `%offset(Xd),Cs\|C[Xs]`          |  Optional. Writes algorithm output (for long outputs).
...
CRF capacity is implementation-defined but must hold at least the largest possible CR or the largest CR pair for `ace.derive`.
(((Cryptographic Register File)))

The size of each CR, determined by the configured algorithm.
ACE returns _ace_err_memory_ if insufficient memory prevents `ace.init`, `ace.import`, `ace.clone`, or `ace.derive`, requiring software to free memory by choosing another CR and clearing it. Implementations must provide sufficient CRF memory to prevent infinite `ace.export`/ `ace.import` loops.
...
Bit `L` (11) in `misa` indicates ACE extension presence.
...
Any other operation that has an output returns zero and will cause the CR to be *invalidated*.
...
Since such operations can operate independently of the CPU core, they may continue to execute even when the process that issued them is interrupted, so the hart implementation may cause the context switching software to wait when accessing a CR that is going to be written to.
