...
The metadata specifies the cryptographic algorithm associated to the CC and optional usage ontrol policies.
...
An initialization or import operation may be interrupted by a context switch, preventing immediate return to the original control flow. In this case, the intermediate state of the CR must be saved and later restored to allow the operation to be resumed (by setting `acestart` similarly to the use of `vstart`).
(((Cryptographic Register,incomplete)))
...
//The PCREM header distinguishes between partial CRs resulting from interrupted initialization versus import operations. In the import case, the content section is also modified: it must preserve the current partial authentication tag value needed for final verification, while the original metadata, nonce, and SIV can be recovered from the source SCC. Note that the metadata within that source SCC may itself indicate a partial initialization or import.
...
Usage-controlled: *NO*. +
Trappable: `ace.reset` *YES*, it can be implemented as a series of `ace.clear` instructions. +
Interruptible: *NO*.
...
[WARNING]
[.red]#To be discussed: how to represent in a CC, how to save and resume the IO buffer upon context switch. I would not add state to the metadata, we could add the IO buffer as an extra architectural state that is simply saved as a variable-length register by privileged code and restored. This state does not require confidentiality nor integrity, and therefore it can be implemented easily. A sequence of# ``[.red]#ace.mv#``[.red]#'s like those mentioned in <<ACE-trappable-insns-support>> is sufficient. To denote partial load/store in this buffer we can probably re-use# `[.red]#acestart#`[.red]#.#
 ...
 * `acedest`, XLEN bits wide, holds what we are transferring: 0..31 for the CRs and, say, 255 for the input/output buffer.
 * `aceoffset`, XLEN bits wide, holds the offset within the CR virtual `content` or the input/output buffer (maybe the same as `acestart` ?).
...
We could have argued that these security issues are sufficient to justify a "SVE" model for ACE:
...
However, this model would have represented a major departure from the current RISC-V approach.

Therefore, we decide to allow resumable operations, with some restrictions.
For instance, if an export operation is interrupted before metadata, nonce and SIV ([.red]#and one further block?#) are written out, it is simply resumed.
...
Further simplifications can be made by removing resumption of `ace.input` and `ace.output`.
