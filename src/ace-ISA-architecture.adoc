...
* `absorb(data)` absorbs data into the hash function.
 ...
 ** In state _initial_ :
  ...
  *** The following initialization operations are performed:
   . [.red]#TBD#
 ** In states _set_public_value_ and _set_public_value_2_:
  *** Only  ``ace.exec`` instructions of Form B are allowed.
   **** In state _set_public_value_, the `IV` is set to `Vs`.
   **** In state _set_public_value_2_, `taglen` is set to `Vs`.
  *** If "```ace.exec Kn|K[Xn],Vs```," is repeated, it will just re-write `IV` or `taglen`.
  *** If no such `ace.exec` is issued, these values are set to zero and `b`, respectively.
  *** If `VLEN` is larger than `b`, the most significant bits of `Vs` (i.e., the "tail" of the vector) are ignored.
  *** Any other Form of `ace.exec` will produce null outputs and causes the CR to be *invalidated*.
