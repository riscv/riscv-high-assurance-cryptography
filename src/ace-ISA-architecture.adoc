...
| [49:36] |  14   | _ExtraContent_           | If non-zero, there is an additional segment of data at the end of the _content_.
The length of additional segment is _ExtraContent_ 64-bit blocks. (14 bits allow up to 128 kB of additional data.)
| [63:50] |  14   | Reserved                 | Value is 0 for the current metadata formats.
|===

If _FilterMode_ is not zero, then the provisioning and SCC data formats contain an additional 128-bit block of metadata, called the _metadata extension_, defined in <<ACE-metadata-extension>>.
...
A value of 0 in the _KeyType_ field indicates an explicit value.
A value of 1 in the _KeyType_ field indicates a system key.
...
After internal key generation, the actual key value—not the recipe—is stored in the CR and SCC. The _KeyType_ field in the metadata is 0.
...
During init and import, the _StateNumber_ field is set to _invalid_, and only at the end of the operation is it set to proper value, i.e., _initial_ or the value in the SCC (see <<ACE-provision-export-import>>)
...
If the _Locality_ Field of a CC is zero, then the CSK is used directly without tweaking the AEAD algorithm.
...
For each _Locality_ we provide the following information: its index in the LST; its _domain_, i.e., whether the value is SoC-wide, device-wide, VM-wide, or per-process; its configurability; the bit field it is encoded into and its value in that field.
...
|   8   | _MLocality_               | Configurable by M to mutually isolate processes in lower-privileged modes.       |  S/HS  |   M    | [6:5]  |   1
|   9   | _HLocality_               | Configurable by H to mutually isolate processes in lower-privileged modes.       |  VS    |   H    | [6:5]  |   2
|  10   | _SLocality_               | Configurable by (V)S to mutually isolate processes in U-mode.                    |  U     |  (V)S  | [6:5]  |   3
|===
...
_HLocality_ is only active if the H extension is enabled and V=1.
...
Implementations of the SKT *must* provide a suitable _Locality_ field value to the ACE unit during CR provisioning, and set usage policies that are at least as strict as the key's own.
...
If the _FilterMode_ field is non-zero, an additional 128-bit block is added to the metadata to implement filtering according to secrets set by S/VS, HS, and M, and expiration date.
...
The subsequent bytes form the *content*—the key material and any algorithm-specific data—whose format and length are specified in <<ACE-algorithms>> for all architected algorithms. The PI content's structure and size are entirely determined by the _Algorithm_ and _KeyType_ fields within the metadata.

NOTE: The resource usage of a CR may be larger than the size of a PI.
In ECB schemes (<<ACE-ECB-mode>>) not hardened against SCA, a CR need not contain more than the metadata and the key, hence their resource usage matches the size of the PI (unless the key is not a multiple of 64 bits).
However, for many algorithms, the internal state is larger than the size of the PI.
...
NOTE: The size of the SCC can differ significantly from the internal resource usage to store the CC's data in a CR.
 ...
 ** Invalid _FilterMode_ for a 64-bit metadata header block,
...
Any access to `maceLocality` outside of M-mode raises an illegal instruction exception.
...
Any access to `haceLocality` outside of H-mode raises an illegal instruction exception.
...
The register group is only accessible in S/HS-mode, VS-mode, and M-mode.
