...
|   2   | ace_state_hash_finalize       | Finalize the internal state of a hash function and possibly prepare it for outputting the result. +
In this state and in the following two states, additional data may be absorbed, such as partial blocks and lengths.
|   3   | ace_state_hash_finalize_enc   | Finalize the internal state of a hash function for the encryption algorithm of an authenticated encryption mode of operation and possibly prepare it for outputting the result.
|   4   | ace_state_hash_finalize_dec   | Finalize the internal state of a hash function for the decryption algorithm of an authenticated decryption mode of operation.
|   5   | ace_state_hash_verify         | Compare the internal state of a hash function to the provided value.
|   6   | ace_state_last_block_enc      | For the encryption path of authenticated encryption modes of operation if the length of the last block must be given.
|   7   | ace_state_last_block_dec      | For the decryption path of authenticated encryption modes of operation if the length of the last block must be given.
|   8   | ace_state_encrypt +
ace_const_operate
 ...
 ** In states _encrypt_ or _decrypt_, we can only issue a Form A `ace.exec`, i.e., `ace.exec Vd,Kn|K[Xn],Vs`.
  ...
  *** If `VL*SEW` is a multiple of `b`, then `ace.exec Vd,Kn|K[Xn],Vs` encrypts/decrypts block-by-block.
 ...
 ** In state _set_public_value_, only ``ace.exec`` instructions of Form B are allowed. +
  ...
  *** If `VL*SEW` is larger than `b`, the most significant bits of `Vs` (i.e., the "tail" of the vector) are ignored.
...
--
+
If `VL*SEW` is a multiple of `b`, the same behavior applies as in state _encrypt_.
...
--
+
If `VL*SEW` is larger than `b`, only the `b` least significant bits of `Vs` are compared to `auth_tag`.
...
--
+
If `VL*SEW` is larger than `b`, the most significant bits of `Vs` beyond the `b`-th (i.e., the "tail" of the vector) are ignored.
...
--
+
If `VL*SEW` is a multiple of `b`, the above operations are applied to each of the `VL*SEW/b` blocks in `Vs`.
 ...
 ** `N_len`, `tag_len`, `last_blk_len`: int
...
[]
 ...
 . `Kbot <- enc_blk(key, zeros(g-1) @ N_ext[b-g:0])`
...
--
+
If `VL*SEW` is a multiple of `b`, the above operation sequence is executed `VL*SEW/b` times, encrypting each `b`-bit block of `Vs` into the corresponding `b`-bit block of `Vd`.
...
** In state _hash_finalize_enc_, only an ``ace.exec`` instruction of Form C, "```ace.exec Vd,Kn|K[Xn]```," is allowed, which
...
[.red]#TBD: double check the endianness and the base of all the fields!!!#.

[.red]#TBD: Write missing example!#
...
--
+
If `VL*SEW` is a multiple of `b`, then the above operations are applied to each `b`-bit block of `Vs`.
 ...
 ** In state _initial_:
  ...
  *** If `VL*SEW` is a multiple of `b` (including the case where `VL*SEW` = `b`), then `process_block` is applied to each `b`-bit segment of `Vs`.
  ...
  ** In state _hash_finalize_:
   ...
   *** If `b` is a multiple of `VL*SEW` and `b` > `VL*SEW`, then we can issue one or more ``ace.exec`` instructions of Form C, i.e., "```ace.exec Vd,Kn|K[Xn]```", to output the hash:
    ...
    . After each "```ace.exec Vd,Kn|K[Xn]```":
     .. We write a new block of the final value of the hash function to `Vd`, with the caveat that if `last_block_len` is non-zero, but strictly smaller than `VL*SEW`, the value in `Vd` is accordingly truncated and zero-padded.
