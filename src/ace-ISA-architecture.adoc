...
The metadata sections's format is fixed, and it specifies the cryptographic algorithm associated to the CC and optional usage control policies.
...
The formats of the II and of the SCC are described in <<ACE-format-configuration>> and <<ACE-format-SCC>>, respectively.
...
Initialization, export and import instructions can only be interrupted in the places where reads from or writes to external memory may be interrupted, for instance because of memory access exceptions. In the pseudocode listings of this section, these memory read or write operations are explicitly marked, together with the corresponding range of values for `acestart`.
...
In case of a context switch, initialization, export and import of CCs must be restarted when control flow is returned to the interrupted process.
Furthermore, the CR being initialized or into which a CC is being imported must be cleared and not saved by the (more) privileged code.
...
We start with the algorithm that reads an II and uses it to initialize a CR.
...
z ← size of initialization input according to metadata;
z ← z - 8 // size of the metadata
...
foreach(i from 0 to num_blocks - 1) {
   if ((i == num_blocks - 1) and last_block_fractional)
      // FOLLOWING MEMORY READ PREEMPTIBLE, `ACESTART` FROM 8+i*16 TO 15+i*16
      read M[8+i*16 .. 15+i*16]
   else
      // FOLLOWING MEMORY READ PREEMPTIBLE, `ACESTART` FROM 8+i*16 TO 23+i*16
      read M[8+i*16 .. 23+i*16];
}
// Now the content[] must be completed according to the algorithm.
----
...
// before we describe the algorithms for
// SCC export (cf.{nbsp}<<ACE-SCC-export>>) and import (cf.{nbsp}<<ACE-SCC-import>>).
...
On RV32, `Xs` needs to contain only the lower 32 bits of the metadata section, since they contain the fields which are required to determine the size of the SCC.
* Internal Capacity `ace.size` returns the amount of internal CRF capacity a CC having the metadata section given in `Xn` would use.
This is expressed in terms of implementation-dependent units. Passing the value `ones(64)` into `Xs` results in the total amount of internal capacity.
Only the bits corresponding to the Algorithm, SP_Protection, and Key_Type metadata fields are considered and all other bits must be zero, except when passing `ones(64)`.
If the combination of Algorithm, SP_Protection, and Key_Type fields is not supported, the instruction writes zero into `Xd`.
On RV32 the GPR `Xs` needs to contain only the lower 32 bits of the metadata section, since they contain the fields which are required to determine the size of the SCC. In this case, the value of `Xs` to obtain the full CRF capacity is `ones(32)`.

We assume that the implementation provides the following functions (or an equivalent mechanism):

. `MD_size()` maps the first 32 bits of the metadata section to the total byte length of the entire metadata section.
  Currently, it just checks bit 0. If the value of this bit is 0, it returns 8, otherwise it fails.
. `II_content_size()` maps the 12 bit value `Key_Type @ Algorithm` to the size in bytes of the content section of an II.
. `SCC_content_size()` maps the 12 bit value `Key_Type @ Algorithm` to the size in bytes of the content section of an SCC.
. `CRsize()` maps the 12 bit value `SP_Protection @ Key_Type @ Algorithm` to the set of integers, returning capacity usage of a CR of the total capacity of the CRF.

The total size of an II is therefore `MD_size()` + `II_content_size()`, and the total size of an SCC is `MD_size()` + `SCC_content_size()`.
Since the `ace.size` instructions can read the entire metadata, they can provide this information.
...
* `Xs`[17:16] is used to turn on or increase the level of the side-channel protection, if the microarchitecture supports it.
The levels are defined in <<ACE-SC-protection-levels>>.
Downgrading side-channel protection is not permitted, and attempting to do so will result in error _ace_err_invalid_ (cf.{nbsp}<<ACE-error-codes>>).
...
** In state _set_aux_value_, only  ``ace.exec`` instructions of Form B, “```ace.exec Kn|K[Xn], INPUT```”, are allowed, whereby `N` is set to `INPUT`, namely
  ...
  *** If no such `ace.exec` is issued, `N` is not changed, which means that its msb is 1.
...
If the value is out of bounds, the instruction raises an invalid instruction error and the CR is *invalidated*.s
