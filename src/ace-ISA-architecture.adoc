 ...
 ** *Unconfigure*: sets status to *INITIAL*.
 ** *Configure*: Works as expected; sets status to *DIRTY*.
 ** *Export* and *Usage*: The system traps to the higher-privileged mode that holds the authoritative SCC, which must import it into the CR. This may require freeing space by marking another CR as *LAZY*.
  Identifying the correct higher-privileged mode is typically straightforward.
  The complex case occurs when U‑mode is preempted by the hypervisor (HV). Assume HV saved a CR and marked it *LAZY*.
  If the CR was *CLEAN*, the authoritative value is the one saved by S‑mode.
  ...
  S‑mode must retain this dirtiness information for each of its child U‑mode processes.
