...
While the *LAZY* status reuses the FS/VS/ACES encoding for “OFF”, this does not indicate that the ISE is inactive, nor that the ISE cannot use that CR.
...
//
[.red]#In the TG there seems to be a consensus that we can resume init/import after a page fault or a similar event, but on context switches we do not export partial CRs. These are cleared and the instruction is restarted.#
