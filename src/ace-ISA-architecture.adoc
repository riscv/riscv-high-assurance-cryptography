...
| [19:18] |   2  | Direction_Policy       | Determines whether the CC may be used for encryption (bit 12 set), decryption (bit 13 set), or both (bits 12 and 13 set).
Valid only for encryption/decryption primitives or modes, and ignored, for instance, for hash functions.
| [21:20] |   2  | Key_Type               | By explicit value / Immutable (index) / Internally generated.
...
+
[upperalpha]
. `2'b00`{nbsp}{nbsp} if the source and the output CRs are both encoded as immediates.
. `2'b01`{nbsp}{nbsp} if the source CR is selected using a GPR and the output CR is encoded as an immediate.
. `2'b10`{nbsp}{nbsp} if the source CR is encoded as immediate and the output CR is selected using a GPR.
. `2'b11`{nbsp}{nbsp} if the source and the output CRs are both selected using the values of GPRs.
