...
A different option is to allow the parent mode to implement lazy switching, i.e., when they cannot import a SCC required by the interrupted instruction, the parent chooses another CR used by the child, exports it and makes it *LAZY*. This would be the alternative to a caching mechanism as well.
--
