...
`src/ace-ISA-algorithms.adoc:2342`: "secp521r1 (NIST P-521): `b` = 576 (55 msbs set to zero), `h` = `j` = 512", where `j` is "the bit length of the random number used as nonce in signature generation". A 512-bit nonce for a 521-bit order means `k < 2^512` always — roughly a 9-bit bias, which enables practical lattice-based private-key recovery from a few hundred signatures. FIPS 186-5 §A.2–A.3 requires negligible bias (e.g. 64 extra bits reduced mod `n`, or rejection sampling).
*Resolution:* set `j ≥ 521` with an approved generation method.
...
**M1. The threat model is excluded from the built document, non-normative where it exists, and omits major attacker classes.** `src/ace.adoc:105` comments out the annex include; `src/ace-annexes.adoc:88-112` marks the model informative and describes it as "straightforward" in about twenty lines. It omits speculative and transient execution (the only related text is an informative aside at `src/ace-ISA-unpriv.adoc:169`, with no normative rule that register content may not influence transient state), fault injection (though `_SCProtection_`=2 promises "fault-tolerant"), DMA/IOMMU attackers, and Debug-mode adversaries. It never states that M-mode is fully trusted, even though M-mode owns the CSK and all Localities and — since trap-and-emulate is explicitly permitted (`src/ace-ISA-unpriv.adoc:69-76`) — may see all register content in the clear. That is in tension with the annex's own premise of cryptographic separation for Supervisor Domains and TEEs and with the introduction's unqualified claim of an interface "without giving software access to the keys". *Promote it into the normative document with a per-mode trust-boundary table, and qualify the introduction's claim as "software below M-mode".*

**M2. The CSK has no architectural requirements.** `src/ace-ISA-priv.adoc:300-317`. The confidentiality of every SCC rests on the Context Sealing Key, yet the specification never states its reset value when `SmaceCSK` is absent, whether it is per-hart or per-ACE-unit (harts share a unit per `src/ace-ISA-unpriv.adoc:67`, making the difference observable), that it must be unpredictable and device-unique, what the CSK is midway through the non-atomic multi-CSR write, that writing it must flush the derived-key cache the spec explicitly encourages (`src/ace-ISA-unpriv.adoc:2787-2788`), or what happens to registers in `_CfgStExporting_`/`_CfgStImporting_` when it changes underneath them. With no key identifier or epoch in the SCC, post-rotation import failures are indistinguishable from attack. *Add a normative CSK section covering entropy, scope, reset, commit semantics, cache flush, and an SCC-visible epoch.*

**M3. The VM-migration claim requires exporting the CSK — escrow of every sealed context — through an unspecified mechanism.** `src/ace-introduction.adoc:69` versus `src/ace-ISA-priv.adoc:313-315`. An SCC is bound to the CSK and, with hardware or `PhysBootScrt` Localities, to the source machine's immutable secrets, so migration requires either that no context use such binding or that the source CSK be transferred — collapsing the "cryptographic domain separation across spatial and temporal domains" claimed at `src/ace-ISA-unpriv.adoc:36`. The wrapping mechanism is left entirely implementation-specific. *State which Locality/CSK configurations are migratable and either architect the transfer or downgrade the claim.*
...
**M6. A usage-policy violation destroys the context, giving any lower-privileged mode a cross-domain kill primitive.** `src/ace-ISA-unpriv.adoc:487` ("will set _State_ to `ace_state_priv_viol`") composed with `:469` ("Upon transitioning to an Error State, the Content of a CR is invalidated and emptied") means a single `ace.exec` from a disallowed mode permanently destroys the target context. Since registers are shared across modes (C7), a U-mode process can enumerate all registers and wipe every context belonging to S-, HS- or M-mode. This directly contradicts the design rationale at `:51`, which says management operations are less restricted than usage operations precisely "to prevent denial-of-service scenarios". *Make usage-policy violations non-destructive faults.*

**M7. `ace.restrictv` is specified to rewrite the entire MDH with no monotonicity constraint.** `src/ace-ISA-unpriv.adoc:1770`. The field-restriction rules and the "all other fields must be 0" constraint are written only for `ace.restrictl`/`ace.restricth`. Read literally, the vector form can rewrite `_Algorithm_`, `_KeyType_`, `_SCProtection_`, `_State_` and `_ConfigStatus_` — and rebinding an `_Algorithm_` on a live context is direct key extraction (take a key restricted to a MAC and rebind it to raw ECB). *State that `ace.restrictv` is exactly the union of the two scalar forms, with all other MDH bits required to be zero.*
