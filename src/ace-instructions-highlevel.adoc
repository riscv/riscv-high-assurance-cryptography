...
| `ace.init`      | Cn\|Xn, %offset(Xs)       |  Sets the algorithm and either the key or info on how to fetch and use system key (keystore).
...
| `ace.import`    | Cd\|Xd, %offset(Xn), S    |  Authenticates and imports a SCC from memory into a CC. `ace import` can optionally support system defined blobs if the system flag S = true.
| `ace.exec`
