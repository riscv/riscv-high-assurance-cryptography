...
| `ace.config`    | #immed                    |  Enable/Disable the feature in the current process domain and determine how many contexts we need. Pseudo-operation for a write to a WARL CSR.
...
| `ace.restrict`  | Cd\|Xd, Xs                |  Optional. Can be used to prevent the use of a CC to specific modes, and to restrict the use of a CC to a specified `ASID` or `VMID` is not specified previously. Cannot modify current policy if already set.
