...
The Atomic Cryptography Extension adds some unprivileged CSRs to a base scalar RISC-V ISA -- besides some privileged CSRs.

An ACE context status field, ACES, is added to `mstatus`[26:25] and shadowed in `sstatus`[26:25],
and possibly `vsstatus`[26:25] if the hypervisor extension is present (this repurposes two of the WPRI bits in these CSRs).
The field is defined analogously to the floating-point context status field, FS, and to the vector context status field, VS.
It will be used in the same way as the FS and VS fields to optimize, for instance, context switching.

*`acecsr`* is not similar to `vcsr` and `vcsr` since it does not have to encode information such as rounding modes.
It contains the operating mode (literally, just whether the unit is active or not) and exception status of the ACE unit (i.e., the error code of the last ACE operation).
(*Definition of the fields TBD*)
...
* `ACE_CTK`, if writable, is writable exclusively by MM.
...
*TBD*: _How to deal with the discoverability of the fact whether `ACE_CTK` is writable ot not even by MM._ Low priority, but must be addressed before final release.
