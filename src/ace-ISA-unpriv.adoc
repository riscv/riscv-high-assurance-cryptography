...
|   [45:32] |  14  | _AuxDataLen_      | If non-zero, indicates that an additional, variable-length, implementation-dependent data section is appended to the Content. The length of this _Auxiliary Data Section_ (ADS) is _AuxDataLen_ * 128 bits.
...
If _AuxDataLen_ is zero, there is no ADS.
...
If _AuxDataLen_ is not zero or one, the first 128-bit block of this additional section contains
...
(cf. <<ACE-CSR-impl-ids>>). These 16 bytes are counted within the length recorded by the _AuxDataLen_ field.
The importing implementation uses this value to determine whether the ADS can be used.

Since an ADS contains at least an authentication tag (16 bytes) and the originating implementation qualifier (also 16 bytes), the minumum length is 32 bytes, corresponding to an _AuxDataLen_ value of 2.
...
If _AuxDataLen_ > 0 (hence, for import only), `ace.load` will _not_ load data beyond the maximal admissible size allowed for the Algorithm encoded in the MDH. In case an ADS is provided, an implementation may decide to stop the transfer once, upon transferring the IMPQUAL block. In these two cases, SIV2 and, if _AuxDataLen_ {ge} 2, the IMPQUAL are loaded in the hart-local ACE state.  This is necessary to allow the `ace.mgmt` instruction to properly process and finalize these conditions.
...
implementation-specific data, and _AuxDataLen_ must be zero in the MDH of a PI. Nor does
...
. Second, if _AuxDataLen_ {ne} 0 (export cannot produce a value of 1), then the implementation-dependent data is processed:
...
. Finally, output `saved_MDH`, then `SIV` and `Content1~CT~[]`, and, if AuxDataLen {ne} 0, `IMPQUAL`, `SIV2` and `Content2~CT~[]`.
--
