...
_Unsupported_,
_Out_of_Memory_, resp.,
_privilege_violation_
when the following three conditions are met: they cannot resolve exception
`ace_exc_unsupported`,`ace_exc_out_of_memory`, resp.,
...
This allows the emulation of an Algorithm.
--
...
[start=4]
 . An `ace.setst` or an `ace.exec` instruction is issued on an expired CR.
 . If, upon resumption of interrupted operations, the CR has expired.

The following conditions cause a transition to Error State _Expired_:

[start=6]
. Any usage instruction is issued with an expired CR.
. `ace.derive` instruction is issued with an expired source or target CR:
   the expiration check is performed on both the source and target CRs and
   whenever either CR has expired is is transitioned to Error State _Expired_.
--
...
Unlike ordinary RISC-V exceptions, an ACE exception _does_ commit the specified CR-state effects before the trap is taken; re-execution of the same instruction therefore observes the updated CR state.  This is reflected in the updated `ace.exec` semantics when issued on several blocks.  Therefore, a restart of `ace.exec` should not be forced.
...
An implementation always has the option to restart a
`ace.load`, `ace.store`, `ace.input`, `ace.output`, and `ace.exec`
operation instead of resuming it, for any reason, and may ignore and clear `acestart`, with following exception:
...
The ACE instructions are designed so that the same instruction sequences work correctly when this rule is respected and on every ACE implementation.
