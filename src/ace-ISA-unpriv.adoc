...
+
--
`ace.load` imports the Content of a PI or an SCC into the CR `Kd` or `K{Xd}`, not the MDH, SIV, SIV2 or IMPQUAL values.
The latter are loaded by software separately.
...
Architecturally, the CRF is represented in the following way:

. There is an array of 32 128-bit blocks, one for each CR, containing the MDH.
. There are three 128-bit registers, which hold the SIV, IMPQUAL, and SIV2 values of the CR being currently managed.
. There is an integer register, which holds the index of the CR being currently managed, or 32 if no CR is currently managed.
. There is some amount of memory that can be dynamically allocated to the various registers, corresponding to Content[] while provisioning and
