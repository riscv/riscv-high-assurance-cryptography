...
|   [25:21] |   5  | _State_           | Current state of the CR/Algorithm, encoded as an integer. For instance, whether the CR is _Unconfigured_, encrypting or decrypting data, absorbing data in a hash/mac/tag, computing elliptic curve point multiples, generating or verifying signatures, or in an error state.  See <<ACE-State-field>>.
...
<<ACE-length-rule>>>> is the single authoritative statement of which MDH fields determine the sizes of the inputs used for provisioning a CR, of the Sealed Cryptographic Contexts, and of the amount of internal memory resources maximally required by the Algorithm encoded in the CR.
...
The currently defined Error States are listed in decreasing severity order, with Error State 24 being the most severe and Error State 29 the least severe.
...
Instructions `ace.size`, `ace.getmd*`, `ace.getst`, `ace.setst` with `#immed7` equal to `ace_state_unconfigured` or an
Error State, `ace.clear`, and `ace.clearall`, which are normally considered usage instructions, are also allowed
when the _ConfigStatus_ is not `ace_cfgst_complete`, together with `ace.mv`.
`ace.load` is allowed in States _ace_cfgst_provisioning_ and _ace_cfgst_importing_,
`ace.store` is allowed in State _ace_cfgst_exporting_.
