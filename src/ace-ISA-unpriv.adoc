...
In certain situations, in addition to raising an ACE error, an ACE instruction may cause a CR to be *invalidated*. This means that the following happens:
 ...
 ** `ace.input` and `ace.output` raise ACE error ace_err_invalid_CR when `aceactivecr` points to an invalid CR;
...
The trap is precise, i.e., all instructions before the one that causes the trap are completed, and [.red]#{warn} the address of the instruction may be set in the User EPC# `[.red]#uepc#` [.red]#CSR. _If_# `[.red]#uepc#` [.red]#_is not defined, it could be defined anyway, however for this type of applications, a local variable can be used to keep track of the place where the error occurred_.# Since `aceerror` provides the cause, a CSR like `ucause` is not necessary as well.

NOTE: The main application of such code is when revocation or expiration cause a CR to become invalid.
In such contexts, it is not the duty of the cryptographic subroutine to resolve the issue, but of the code that makes use of the latter, as the keys may have to be renegotiated, and this may even need user intervention.
...
Some specifications define algorithm families that can be instantiated with any primitive meeting certain properties, typically constraints on the block size `b` and the key size `k`—such as GCM or GCM-SIV, which are well defined not only with AES but also with other block ciphers, such as SM4.
By contrast, the specifications for other algorithm suites are specific only to some primitives, for instance, Ascon.
...
. A list of *States* in which the internal state machine of the logic is executed, together with their representation according to the rules set in <<ACE-StateNumber-field>>.
. A complete list of the *allowed state transitions*, including the precise conditions under which each transition is allowed, the Form of `ace.state` used for each of these transitions and the meaning of any additional parameter;
...
When ACEIOBUF is implemented, in all algorithms the following applies:
any occurrence of the “Original Instructions” in <<ACE-replacement-instructions>> may be substituted by the corresponding “Replacement Sequence” where `ace.input` and `ace.output` instructions are used in place of the vector register inputs and outputs, even if this is not explicitly written each time.
...
The only operations performed by `ace.input` and `ace.output` are to provide the inputs to the cryptographic operations and read out their output.
No computation is performed on the data by `ace.input` and `ace.output`.
...
If `aceiobuflen` is non-zero, and the range size `aceiobuftop` - `acestart` is an invalid size for the algorithm, the `ace.exec` operation is not performed and ACE error ace_err_invalid_value is raised.
...
This is done by setting the `aceactivecr` CSR (<<ACE-CSR-aceactivecr>>) first. The user must not forget to set it back to 32 at the end of their code.

The architecture uses the `acesequence` CSR to track the progress of the replacement sequences.
The initial value of `acesequence` is 0. It remains zero while the forms of `ace.exec` and `ace.state` specified in the algorithm are used.
...
In particular, `acesequence` may be saved and restored by privileged code at context switch together WITH acestart[XLEN-9:0].
Abusing this register may also allow to arbitrarily repeat `ace.exec` or `ace.state` instructions.
However, if the algorithm and its state machine are correctly designed, this will not allow the user to compromise the internal state of the algorithm.
...
The PI and SCC Formats will be the same as for the non-threshold implementation variants of the same algorithm, with the only difference being the value of the _SCProtection_ in the Metadata.

For the purpose of architecture specification, all values are also always represented as a single share in this document.
Threshold implementations are simply implementation variants of the same algorithm.
...
If reconstructing the values is efficient but generating the shares is a slow process, the implementation may take advantage of the optional Additional Data in the Content to store information that can accelerate share generation, such as a copy of some (or all) shares.
...
The following table, together with <<ACE-state-constants-fixed>>, defines the states used by the symmetric algorithms in this specification.
