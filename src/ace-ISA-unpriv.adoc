...
software obligation (<<ACE-CC-formats>>). No confidentiality property depends on the check;
...
+
The architecturally observable behavior is described in <<ACE-CC-formats>>.
...
[[ACE-data-formats]]
==== Formats of Provisioning Input, Sealed Cryptographic Context, and Partially Configured Cryptographic Context
...
If the context is exported while in an Error State, Sections 3–5 are empty; that is, the SCC consists only of a plaintext `MDH` and `SIV`.

During import of an encrypted and authenticated SCC, `Content1[]` and `Content2[]` are kept encrypted in the CR until
the `ace.mgmt` operation to complete the import is issued. These instruction then decrypts and authenticates `Content1[]` and `Content2[]`.

If a partially imported SCC is exported, the data is exported as-is, i.e., encrypted.

If the context is exported while partially configured, i.e., provisioning or import has not completed, then no encryption and authentication of the data is performed. The data is exported as is: unencrypted if the original interrupted operation was a provisioning operation, and encrypted if it was an import operation.
In the latter case, `SIV`, `SIV2` and `IMPQUAL` are not included in the export, but only saved and restored with the process's state at context save and restore if a context switch occurs.
The format of the exported data is therefore the following:
...
// This is addressed in <<ACE-CC-formats>>.
...
The management operations at the base level (i.e., the outermost nesting level) are the only ones that may perform cryptographic operations:

* If the management operation at the base level is an import,
cryptographic verification is only performed at the management completion operation,
after MDH, `SIV`, \{ if _AuxDataLen_, also `SIV2` and `IMPQUAL`, \}, `Content1[]` \{ and `Content2[]` \} have been loaded.

* If the management operation is an export, then the `ace.mgmt` to start the operation prepares the CR by encrypting it and generating `SIV` and, if _AuxDataLen_, also `SIV2` and `IMPQUAL`.  The management operation to complete the export will either restore the clear contents from an internal copy or re-decrypt it and authenticate it.

The software, by saving metadata headers in its own state, maintains an implicit stack of the _State_ values, with the top value corresponding to the previous nesting level.
The `ace.mgmt` completion operation restores the previous nesting level from the stack. This way, cryptographic operations need to be performed only at the outermost level.  Concluding a base-level import or an export early will not authenticate, and therefore they cannot be used to leak plaintext at an outer nesting level. The only way plaintext is allowed to be exported is if the outermost operation itself was a provisioning operation, in which case the plaintext was already available to software.  In order to properly track `ace.load`, `ace.store`, and `ace.mv`, the CSR `acestart` must be saved and restored at interruption and before control return. This holds also for `SIV`, `SIV2` and `IMPQUAL`. Since software may load and set up these values before issuing `ace.mgmt`, these CSRs must be saved and restored unconditionally.
