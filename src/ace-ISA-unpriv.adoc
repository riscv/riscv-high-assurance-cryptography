...
More precisely: For each _j_ from `acestart` to `end` − 1, the byte at memory
address ``Xs1 + %offset + ``_j_ is loaded into the _j_-th byte of the
serialized `Content[]` of the CR.  `acestart` tracks the prefix-complete number of bytes loaded.
...
[[ACE-rules-formats]]
==== Rules for Provisioning and Import Formats
