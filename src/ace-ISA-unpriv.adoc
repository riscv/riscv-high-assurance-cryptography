...
. For `r` = {lceil}``IOLEN``/``b``{rceil}, we replace a Form A `ace.exec` instruction by `r` Form B `ace.exec` instruction
followed by `r` Form C `ace.exec` ones. These are tracked in `acesequence` using values 1 to 2``r``.
. The Form B `ace.exec` instructions read bits [`IOLEN`-1:0], then [2``IOLEN``-1,``IOLEN``], and so on,
discarding the `b`-`r`{interpunct} `IOLEN` significant bits of the last input if `IOLEN` does not divide `b` exactly.
. The `r`-th instruction also applies the primitive to this buffer.
. Then Form C `ace.exec` instructions write out bits [`IOLEN`-1:0], then [2``IOLEN``-1,``IOLEN``], and so on, of the result into the output vector register,
padding the last output with `b`-`r`{interpunct} `IOLEN` zeros in the most if `IOLEN` does not divide `b` exactly.
