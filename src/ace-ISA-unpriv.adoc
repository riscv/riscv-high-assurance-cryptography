...
Configured by M-mode during the boot process of the host (see <<ACE-CSR-macePhysBootSecret>>).                       | Device |   M    | [4:3]  |   1
|   7   | VirtBootSecret          | Unique per the boot session of the virtual hardware and its software stack.
Regenerated by H or higher at each virtual boot of a VM (see <<ACE-CSR-haceVirtBootSecret>>).                        |   VM   |  H/M   | [4:3]  |   2
 ...
 ** SiPSecret {rightarrow} ChipFamilySecret or ChipSecret;
 ** ChipFamilySecret {rightarrow} ChipSecret;
 ** OEMSecret {rightarrow} ProductSecret or DeviceSecret;
 ** ProductSecret {rightarrow} DeviceSecret;
