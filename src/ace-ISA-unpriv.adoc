...
|   61     | `ace_cfg_exporting_partial` | The CR, which was in an Incomplete State, has been placed in exporting mode.
|   62     | `ace_cfg_importing_partial` | A CR exported while in an Incomplete State is being reimported.
|   63     | (Reserved)               | Reserved.
|===

(((ACE, Algorithm, incomplete states)))
States 56–63, i.e., the Configuration States, are also called _Incomplete_:
...
when the CR is in an _Incomplete_ State.
...
.. `ace_cfg_importing_partial` if the data to import corresponds to a CR in an Incomplete State.
. After `ace.mgmt`, with #immed7 = `ace_cfg_exporting`, the CR's _State_ becomes
.. `ace_cfg_exporting` if the CR is in a Valid State,
.. `ace_cfg_exporting_partial` if the CR being exported is in an Incomplete State.
...
.. if it completed a management operation resulting in a CR in Incomplete State, `acemanagedcr` is left unchanged.
...
... If `ml`._State_ is an Incomplete State, then CR._State_ is set to `ace_cfg_importing_partial`;
...
.. If `ml`._State_ is in an Incomplete State (we are exporting a not fully configured CR or we are initiating a nested CR export):
+
...
// . If the MDH._State_ is an Incomplete State, then:
