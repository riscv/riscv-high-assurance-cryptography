...
first payload changes; an attacker cannot graft the Implementation VDS of one SCC onto the architecture-dependent part of another.
====
...
[NOTE]
====
This procedure ensures that if the SCC's optional implementation-specific VDS makes the SCC too large to fit in a CR, or if that VDS is incompatible, the implementation ignores the optional data, but keeps the architecture-dependent content: Only a failure of the first authentication (`SIV`) rejects the entire SCC.
====
