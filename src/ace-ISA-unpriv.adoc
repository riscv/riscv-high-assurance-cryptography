...
Seven of the eight _Locality_ field bits are defined; bit 7 is reserved for future use or custom Localities #({warn} To be defined.)#.
...
nonzero bits in reserved fields; unsupported _Locality_ value; or a nonzero _StateNumber_.
+
...
+
The instruction follows the interruptibility rules defined in <<ACE-CSR-acestart>> and <<ACE-provisioning-export-import>>.
+
...
(`ace.import`).
+
`ace.import` follows the interruptibility rules defined in <<ACE-CSR-acestart>> and <<ACE-provisioning-export-import>>.
...
It is conceivable to support vector lengths smaller than a cipher’s block size in encryption/encryption modes.
We define first how to replace a Form A `ace.exec` instruction:

. If `IOLEN` is smaller than `b`, we first add an internal `b`-bit buffer to the internal state of the CR.
. For __r__ = {lceil}``IOLEN``/``b``{rceil}, we replace a Form A `ace.exec` instruction by __r__ Form B `ace.exec` instruction
followed by __r__ Form C `ace.exec` ones. These are tracked in `acesequence` using values 1 to 2__r__-1 (after the 2__r__-th instruction, `acesequence` is set to zero and not to 2__r__).
. The __i__-th Form B `ace.exec` instructions copies its input into bits {nbsp}[min(__i__{interpunct}``IOLEN``,`b`)-1 : (__i__-1){interpunct}``IOLEN``]{nbsp}
of the `b`-bit buffer, for __i__ = 1,2,..,__r__.
The use of “min” ensures that the `b`-__r__{interpunct}``IOLEN`` significant bits of the last input are discarded if `IOLEN` does not divide `b` exactly.
. The __r__-th Form B `ace.exec` instruction also applies the primitive to this buffer.
. The __i__-th Form C `ace.exec` instructions can be issued to write out bits {nbsp}[min(__i__{interpunct}``IOLEN``,`b`)-1 : (__i__-1){interpunct}``IOLEN``]{nbsp}
of the `b`-bit buffer to its output vector register, for __i__ = 1,2,..,__r__.
...
It would require a `b`-bit buffer for each CR implementing such a cipher when `IOLEN` is smaller than `b`, whereas a single ACEIOBUF would require fewer resources.
