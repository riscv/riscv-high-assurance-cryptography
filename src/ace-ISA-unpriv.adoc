...
This category has the second highest priority, and comprises following conditions:
 ...
 . Any instruction with an ACE opcode may raise an illegal-instruction exception for the purpose of emulation, or if certain fields have invalid encodings in sub-fields (such as `v` = `h` = 1 in `ace.restrict`), or if the instruction belongs to an unimplemented (sub)extension.
