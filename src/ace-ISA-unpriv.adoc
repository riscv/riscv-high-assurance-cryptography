...
[NOTE]
====
The rules governing `ace.mgmt` are necessary because any management operation may be interrupted, possibly by multiple nested interrupts or exceptions, with or without context switches. Saving and restoring _ConfigStatus_ is the interrupted software's responsibility. This keeps the logic simple and supports arbitrarily many levels of nested virtualization.

These rules also allow an interrupted import to resume on a hart attached to a different ACE Unit implementation. The architecture ensures interoperability when the importing implementation supports the algorithm encoded in the SCC. However, if the SCC's optional _Implementation (Specific) Variable-Length Data Section_ (Implementation VDS) makes the SCC too large to fit in a CR, or if that VDS is incompatible, the implementation must ignore the optional data. The importing implementation may create different optional data. In both cases, it updates _ImpDataLen_ accordingly.
