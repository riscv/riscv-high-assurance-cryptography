...
GR6. A 128-bit MDH in a vector operand:::
...
authentication tag(s) and the additional variable-length data; for a PI it is the size of the entire
...
unsupported, or the MDH is otherwise malformed. A return of `32` is *not* an error indication: it is
