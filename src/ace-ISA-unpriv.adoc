...
`ace.derive` must be of Form B, where the optional parameter contains (the first 32-bits of) a metadata header, that defines either an encryption algorithm, or a MAC algorithm, or an authenticated encryption algorithm. The _KeyType_ field must be 0, as well as _Locality_ and _UsagePolicy_. The latter two will be copied from the source CR to the destination CR.
