...
operation: an instruction naming a CR in a Configuration State is rejected by Rule SGR15
...
Rule SGR17, resp. SGR18, of <<ACE-State-field>> raises an illegal-instruction exception
...
If the CR's _State_ is not one of the States admitted by Rule SGR17 (<<ACE-State-field>>) — which excludes every Complete State and the _Unconfigured_ State — or `acestart` is not a multiple of 16, an illegal-instruction exception is raised without state change.
...
Analogously to `ace.load` (<<ACE-instruction-load>>), a _State_ not admitted by Rule SGR18 (<<ACE-State-field>>), or an `acestart` that is not a multiple of 16, raises an illegal-instruction exception without state change.
--
...
Further usage of the same CR—for instance, a later `ace.exec` or `ace.setst` to a valid state—performs no operation and raises no exception, leaving the _State_ field unchanged (Rule SGR13 in <<ACE-State-field>>) until software detects the Error State.
...
`ace.restrictl/ace.restricth/ace.restrictv` may be used in any Complete State; in the _Unconfigured_ State it performs no operation, and in a Configuration State it raises ACE exception `ace_exc_privilege_violation` (Rule SGR15 in <<ACE-State-field>>). Care is required:
...
* If source and destination are the same CR the instruction performs no operation, even if the CR is _Unconfigured_ or in a Configuration State, which is excepted from Rule SGR15 (<<ACE-State-field>>).
...
(Rule SGR15 in <<ACE-State-field>>), with no state change to either CR.
