...
From the point of view of the architecture, the trap would be precise, i.e., all instructions before the one that causes the trap are completed, and the address of the instruction should be set in the User EPC (`uepc`) CSR (if it is not defined, we defined it). A CSR like `ucause` would not be used in this case, with `aceerror` providing the instruction.
