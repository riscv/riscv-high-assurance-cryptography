...
It is conceivable to support a vector length `IOLEN` smaller than a cipher’s block size `b` in encryption modes.
We add a `b`-bit field called `buffer` to the state of the CR.
Let __r__ <- {lceil}``IOLEN``/``b``{rceil}.
We define first how to replace a Form A `ace.exec` instruction:

.  The idea is to replace a Form A `ace.exec` instruction by __r__ Form B `ace.exec` instruction followed by __r__ Form C `ace.exec` ones.
This replacement sequence is tracked in `acesequence` using values 1 to 2__r__-1 (after the 2__r__-th instruction, `acesequence` is set to zero and not to 2__r__).
. The __i__-th Form B `ace.exec` instructions copies its input into bits {nbsp}[min(__i__{interpunct}``IOLEN``,`b`)-1 : (__i__-1){interpunct}``IOLEN``]{nbsp} of the `b`-bit `buffer`, for __i__ = 1,2,..,__r__. The use of “min” ensures that the `b`-__r__{interpunct}``IOLEN`` significant bits of the last input are discarded if `IOLEN` does not divide `b` exactly.
. The __r__-th Form B `ace.exec` instruction also applies the primitive to this `buffer`.
. The __i__-th Form C `ace.exec` instructions can be issued to write out bits {nbsp}[min(__i__{interpunct}``IOLEN``,`b`)-1 : (__i__-1){interpunct}``IOLEN``]{nbsp} of the `b`-bit `buffer` to its output vector register, for __i__ = 1,2,..,__r__.
. After the __r__-th Form C `ace.exec` instruction is completed, `acesequence` is reset to zero.

Form B and C `ace.exec` instructions can be replaced in a similar way.

Specifying this behavior would complicate both the architecture and its implementation.
It would to require a `b`-bit field tp each CR implementing such a cipher where `b` is larger than `IOLEN`, whereas a single ACEIOBUF would require fewer resources.
It would also burden the software using the feature by requiring to use more instructions, potentially in variable length loops for portability.
Using existing architectural features, such as reconfiguring the vector lengths, or using vector register groups, would be a simpler approach–beside the aforementioned use of ACEIOBUF.

We do define piece-wise loading of the state for functions with significantly longer inputs, such as hash functions, because these may use the state of the hash function to absorb partial inputs instead of requiring a separate buffer to be used only when full, and on some architectures the long inputs to hash functions could require unsupported vector lengths.
...
it must be possible to configure a GCM CC with both secret key and the IV.
...
* `k` = key size (64|`k`).
* `b` = block size, (64|`b`). For this algorithm, `IOLEN` must be an integer multiple of `b`.
...
+
--
* `b` = block size. `b` is a power of two, and {ge} 64. For OCB, `IOLEN` must be an integer multiple of `b`.
* `k` = key size, `b`|`k`.
--
+
...
** To move to _hash_absorb_, a Form B `ace.state` must be used, where the auxiliary argument `Xs` sets the tag length `tag_len`.
  ...
  *** If `N[u] == 1`, this means that `N` was not set in State _set_aux_value_. Nonce setting cannot be skipped, as this mode is not secure under nonce reuse, therefore in this case ACE error ace_err_invalid_value is raised, and the CR is *invalidated*.
...
If all three are not used by the hashing algorithm, then the 64-bit block is omitted, and `block` is clearly also
 ...
 ** In State _hash_output_, the cases where the current function is a hash function or a XOF behave differently.
...
. Scalar multiplication with known and unknown base point. This can be used to implement Diffie-Hellman key exchange.
...
The last point is very difficult to implement in full generality, since there are several options, with multiple choice of KDF, hash function, and input formats.
In most cases, covering all options would be impractical.
Only the Chinese SM2 standards fully specify an integrated encryption scheme cite:[GMT-0003-4-2012]–a later revision of the present specification may add direct support for the corresponding algorithm.
In general, point multiplication is the basic mechanism by which KEM and PKE schemes are implemented and this implies that the key values derived using these must be present in the memory of the environment deriving them, albeit shortly if these are then only used to provision a CR.
...
For a sender to perform public-key encryption, the `SecondPt` field must be set with the _recipient_'s public key, `HasSecondPt` must be True, and the `Scalar` field must be set with the _sender_'s private key.

For a recipient to decrypt a public-key encrypted message, the `SecondPt` field must be set with the _sender_'s public key, `HasSecondPt` must be True, and the `Scalar` field must be set with the _recipient_'s private key.

For ECIES: Before invoking `ace.derive`, it is allowed to transition to State _absorb_S1_ where the bit length of the optional S1 must be given as a parameter to the `ace.state` transition call through a GPR, and this value is set in `Progress`.
Then, one or more Form B `ace.exec` calls can be made to absorb consecutive parts of S1 into the state, and the amount of absorbed data so far is subtracted from `Progress` until it is zero. Excess data is ignored. S2 must be handled by the user using the MAC function (for NIST curves, an HMAC, with S2 appended to the ciphertext).
`ace.derive` must be of Form B, where the optional parameter contains (the first 32-bits of) a metadata header, that defines either an encryption algorithm, or a MAC algorithm, or an authenticated encryption algorithm. The _KeyType_ field must be 0, as well as _Locality_ and _UsagePolicy_. The latter two will be copied from the source CR to the destination CR. For KDF use cite:[NIST-SP-800-56Cr2].

The computation of a derived context starts either with the transition to _absorb_S1_ or with the issuing of the `ace.derive` instruction.
 ...
 * We assume that the KDF takes first the entire point (X and Y coordinates) or the only coordinate in case of Edwards curves.
 * The KDF can then absorb additional data into the state.
. Finalize and seed one or more keys.
. We choose to define auxiliary symmetric algorithms with hidden keys as outputs of `ace.derive`.
. For instance, AES/SM2-CTR followed by some MAC function that can absorb an additional string after the plaintext.
  These strings are different from the associated data because they are provided _after_ the ciphertext.
====

===== KDF

This family of algorithms will work in two modes: generating L bits of output, or generated two keys of size L1, L2, with prescribed metadata headers M1, M2.

We need to support: NIST SP 800-56C Rev. 2

Parameters:::

* `b` = block size (8|`b`). Similarly to Hash functions, this is the size of an absorbed block and may be different from the size of the internal state that is kept by the function.
* `t` = tag len (8|`t`). This is the size of the a block that is output each time by the function. The present description assumes `t` {le} `b`.
