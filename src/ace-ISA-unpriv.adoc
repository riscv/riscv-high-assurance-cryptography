...
| `Zlctrm`       | CTR mode.^4^                        | <<ACE-keystream-modes>>                        | v0.0.0          | --
| `Zlxctrm`      | XCTR mode.^4^                       | <<ACE-keystream-modes>>                        | v0.0.0          | --
| `Zlxexm`       | XEX.^5^                             | <<ACE-XEX-XTS-modes>>                          | v0.0.0          | --
.2+| `Zlgcmm`    | GCM mode.^4^                        | <<ACE-GCM-mode>>                               | v0.0.0          | --
|           GCM with set IV.^4^                        | <<ACE-GCM-with-IV-mode>>                       | v0.0.0          | --
| `Zlgcmsivm`    | GCM-SIV.^4^                         | <<ACE-GCM-SIV-mode>>                           | v0.0.0          | --
.2+| `Zlocbm`    | OCB3.^5^                            | <<ACE-OCB-mode>>                               | v0.0.0          | --
|          OCB3 with set IV.^5^                        | <<ACE-OCB-mode>>                               | v0.0.0          | --
...
| `Zlkmacm`      | KMAC function with hidden key. Depends on `Zlesha3h`. | TBD                          | v0.0.0          | --
...
| `Zlsecp256r1c` | Point arithmetic and signature generation and
verification algorithms for the NIST curve secp256r1.  | <<ACE-ECC>>                                    | v0.0.0          | --
| `Zlsecp384r1c` | Same, but for the NIST curve secp384r1.  | <<ACE-ECC>>                               | v0.0.0          | --
| `Zlsecp521r1c` | Same, but for the NIST curve secp521r1.  | <<ACE-ECC>>                               | v0.0.0          | --
| `Zled25519c`   | Same, but for the Edwards curve ed25519. | <<ACE-ECC>>                               | v0.0.0          | --
| `Zled448c`     | Same, but for the Edwards curve ed448.   | <<ACE-ECC>>                               | v0.0.0          | --
| `ZlbpP256r1c`  | Same, but for the Brainpool curve brainpoolP256r1.         | TBD in <<ACE-ECC>>      | v0.0.0          | --
| `ZlbpP256t1c`  | Same, but for the twisted Brainpool curve brainpoolP256t1. | TBD in <<ACE-ECC>>      | v0.0.0          | --
| `ZlbpP320r1c`  | Same, but for the Brainpool curve brainpoolP320r1.         | TBD in <<ACE-ECC>>      | v0.0.0          | --
| `ZlbpP320t1c`  | Same, but for the twisted Brainpool curve brainpoolP320t1. | TBD in <<ACE-ECC>>      | v0.0.0          | --
| `ZlbpP384r1c`  | Same, but for the Brainpool curve brainpoolP384r1.         | TBD in <<ACE-ECC>>      | v0.0.0          | --
| `ZlbpP384t1c`  | Same, but for the twisted Brainpool curve brainpoolP384t1. | TBD in <<ACE-ECC>>      | v0.0.0          | --
| `ZlbpP512r1c`  | Same, but for the Brainpool curve brainpoolP512r1.         | TBD in <<ACE-ECC>>      | v0.0.0          | --
| `ZlbpP512t1c`  | Same, but for the twisted Brainpool curve brainpoolP512t1. | TBD in <<ACE-ECC>>      | v0.0.0          | --
| `ZlSM2c`       | Same, but for Chinese 256-bit SM2 curve.                   | TBD in <<ACE-ECC>>      | v0.0.0          | --
...
The provisioning, export and import operations may only be interrupted at memory read or write operations.
...
The provisioning and import instructions may resume from the point of interruption only if the control flow is returned to them immediately after the interrupt handler has completed: The privileged interrupt handler must ensure that the instruction is restarted only in this situation, and in any other circumstance, including context switching, the instruction must be restarted, in which case if the instruction is `ace.prov` or `ace.import` it must also clear the CR.
Export may resume only of the CR was not used before returning to the interrupted control flow.
...
During SCC export and import, these Locality Secrets are absorbed, in ascending index order, to the input of the AEAD tag computation, before the metadata.
If the _Locality_ field is zero, no Locality Secrets are absorbed.
