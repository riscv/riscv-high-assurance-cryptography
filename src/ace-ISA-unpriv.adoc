...
Progress tracking by `acestart` also applies to `ace.exec` when iteratively processing a multi-block operand, as `ace.exec` may be interrupted between blocks, asd well as to `ace.derive`, whose `ace.exec` endpoints advance the state machines of the source and destination CRs in the same way, and the vector input and output Forms of `ace.mv`.
...
In Forms C and D, `rs2` specifies the destination GPR or vector register.
--
//
