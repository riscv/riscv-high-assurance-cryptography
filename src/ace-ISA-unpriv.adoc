...
[NOTE]
====
Supporting vector lengths smaller than a cipher's block size would not only complicate both the architecture and its implementation, but it would also burden the software using the feature. We could define an buffer in the internal state which is filled by repeated Form B `ace.exec` instructions until `b` bits have been provided, in which case the cryptographic function is performed, and successive Form C `ace.exec` instructions would be used to output the result.  The tracking mechanism could use `acesequence` values from 1 to 2`b/IOLEN` to determine the Form of the next expected operation and when to perform the encryption or decryption operation.

A simpler approach in this case, would be to reconfigure the vector lengths, to use vector register groups, or to use the `ace.input` and `ace.output` instructions.
We do define piece-wise loading of the state for functions with significantly longer inputs, such as hash functions, because these may use the state of the hash function to absorb partial inputs instead of requiring a separate buffer to be used only when full.
====
