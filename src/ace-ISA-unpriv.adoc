...
* `ace.getmd` (<<ACE-instruction-getmd>>) reads the metadata of a CR, and it is used to implement the pseudoinstruction `ace.getst` (<<ACE-instruction-getst>>), which extracts the state of a CR from the MDH of a CR. `ace.getst` is also provides error reporting.
...
.. The CR's _ConfigStatus_ is set to `ace_cfgst_exporting`. This is the value on which
   ...
   (<<ACE-instruction-mv>>) are gated, so no export can proceed until it is set.
