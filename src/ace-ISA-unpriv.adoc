...
| `Zlesha2h`     | SHA-224, SHA-256, SHA-384, SHA-512, SHA-512/224, and SHA-512/256.
{empty}                                                        | <<ACE-hash-functions>>                         | v0.0.0          | --
| `Zlesha3h`     | SHA3-224, SHA3-256, SHA3-384, SHA3-512, SHAKE128, and SHAKE256.
{empty}                                                        | <<ACE-SHA-3>>                                  | v0.0.0          | --
| `Zlsm3h`       | ShangMi 3 (SM3) hash function.              | <<ACE-hash-functions>>                         | v0.0.0          | --
| `Zlhmacm`      | HMAC function with hidden key (with SHA2 when `Zlesha2h` is supported,
and with SHA3 when `Zlesha3h` is supported).                   | TBD                                            | v0.0.0          | --
| `Zlkmacm`      | KMAC function with hidden key. Depends on `Zlesha3h`, since KMAC is only defined with SHA3.  | TBD | v0.0.0      | --
| `Zlascon`      | Ascon-AEAD128 (base, with set Nonce, or with Nonce Masking),
Ascon-Hash256, Ascon-XOF128 and Ascon-CXOF128.                 | <<ACE-Ascon-AEAD128>> to <<ACE-Ascon-CXOF128>> | v0.0.0          | --
...
{empty}
(((Cryptographic Context)))
ACE defines up to 32 Context Registers (CRs), numbered 0 to 31 and denoted `K0`–`K31`.
...
A lazy-loading mechanism for CRs, besides optimizing the performance of context switching, may be used to load CRs on demand; see <<ACE-lazy-loading>>.
====
...
The preferred mechanism for supplying inputs to ACE operations and collecting their outputs is the vector register file of the V extension.
ACE also defines an optional architectural, per-hart input/output buffer (ACEIOBUF) that may be used in place of vector registers, as described in <<ACE-usage-input-output>>.
The ACEIOBUF is not part of any CR.
It may reside within CRF memory or in its own memory region internal to the ACE state.
...
In all cases, the _StateNumber_ field shall be set to _initial_ (0), _invalid_ (14), or _completed_ (15) when the CC is in the corresponding state, and to a different value otherwise. These _initial_, _invalid_, or _completed_ shall be unambiguously identifiable solely from the _StateNumber_ field, without inspecting the Content; in other words, the values 0, 14, and 15 in _StateNumber_ cannot be used together with additional information in the Content to encode additional states.
...
ACE maintains a table of 128-bit Locality Secrets (LS), the Locality Secrets Table (LST). Some entries are fixed or configurable only via an implementation-specific authenticated hardware procedure; these are global and shared across all ACE units in the same SoC. Other entries are per-hart architectural state programmed by privileged modes.

<<ACE-locality-indexes>> lists the architected Localities.
Eleven LST entries are defined: the first six constitute the _HW Binding Group_, binding SCCs to the hardware,
entries 6–7 form the _Boot Session Group_, and entries 8–10 form the _SW Filter Group_.
Seven of the eight _Locality_ field bits are defined; bit 7 is reserved for future use or custom Localities [.red]#({warn} To be defined.)#.
For each Locality, the following information is provided: index in the LST; domain (SoC-, device-, VM-wide, or per-process); configurability; bit-field encoding and value.
...
Implementations are not required to populate every entry in the HW Binding Group, except for ChipScrt and DevScrt.
If an entry is unconfigured, it is transparently replaced by the next defined entry in its chain:
SiPScrt {rightarrow} ChipFamScrt {rightarrow} ChipScrt, and OEMScrt {rightarrow} ProdScrt {rightarrow} DevScrt.
...
one from the HW Binding Group, one of either PhysBootScrt or VirtBootScrt, and one from the SW Filter Group.
...
The Additional Data segment of the content consists of two parts: an architecture-independent part, whose length is determined by the specified algorithm, and an architecture-dependent part.  One, none, or both of these parts may be empty.
The first eight bytes of the architecture-dependent of the additional data segment hold a copy of the `acemarchid` CSR (<<ACE-CSR-acemarchid>>) of the originating machine, and are counted within the length recorded by the _AdditionalData_ field. The importing implementation uses this value to determine whether the architecture-dependent additional data can be used.

_AdditionalData_ records the length of the _entire_ Additional Data.
It is the responsibility of the algorithm to determine the length of the architecture-independent part and process it.
Then, if _AdditionalData_ is larger than then architecture-independent part, the _implementation_ will check the value of `acemarchid` to decide whether the architecture-dependent part can be used.
...
Table <<ACE-error-codes>> lists the error codes defined by ACE.
The non-zero error codes are also listed in decreasing priority order, i.e., error 1 has the highest priority, and error 10 has the lowest priority.
