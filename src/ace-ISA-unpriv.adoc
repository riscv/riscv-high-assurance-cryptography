...
Each hart's ACE state must be exported, then fully zeroized on power down; on resume, the ACE unit must reset and be reconfigured by software again.
...
. Use `ace.mgmt` to prepare a CR to accept an SCC, store MDH[63:0], or MDH[127:0] in it, check the validity of MDH[63:0].
...
. Use `ace.getmdl`, resp., `ace.getmdv` (<<ACE-instruction-getmd>>) to get MDH[63:0], resp, the entire MDH, of the CR to be exported to be able to determine its size and allocate resources.
...
All CR management instructions are split in three phases: An initial `ace.mgmt`, load/store instructions, and a final `ace.mgmt`.
The two `ace.mgmt` instructions perform no memory accesses.

The dependencies of the `ace.mgmt` instruction's timing shall be limited to only very few factors:
...
. Attempts to import SCCs whose total length exceeds what the target machine supports for the given algorithm.
  In this case, the implementation-dependent part is necessarily unsupported. The implementation-dependent part is skipped or only partially loaded, to be discarded without authenticating it; and only the architecture-dependent part is authenticated and decrypted (if the resulting _ConfigStatus_ is `ace_cfgst_complete`) and used.
