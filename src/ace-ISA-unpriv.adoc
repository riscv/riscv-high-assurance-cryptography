...
Therefore, it is an XLEN-bit RW CSR.
...
Only these instructions honor a nonzero `acestart`; all other instructions ignore it and are restarted, WITH acestart[XLEN-9:0] also being cleared.
Provisioning, import, and export are resumed only under certain conditions, as explained in <<ACE-provisioning-export-import>>.

The bottom XLEN-8 bits of `acestart` record the progress of an interrupted operation.
Since ACE does not have an _intrinsic_ concept of element length, `acestart`[XLEN-9:0] counts the number of bytes processed so far, not the number of elements.
The top 8 bits of `acestart` are used to encode additional information and are ignored if not used.

`acestart`[XLEN-1:XLEN-8] is non-zero only if the interrupted instruction is `ace.prov`, `ace.import`, or `ace.export`, in which case it is equal to
`acestart`[XLEN-1:XLEN-8] = `3'b100 @ bin(n,5)` wheren `n` is the number of the CR being provisioned, imported, or exported at the time of interruption.
...
When an ACE instruction is interrupted, the CSR follows `acestart` semanticsn described here.
...
{empty}
(((CSR, `aceiobuflen`)))
`aceiobuflen` is an XLEN-bit URW CSR that programs the ACEIOBUF length for use by `ace.input` and `ace.output`.
...
When `aceiobuflen` is zero, the ACEIOBUF is _unconfigured_ (analogous to an unconfigured CR in state Initial).
Attempting to execute with an unconfigured ACEIOBUF `ace.input` and `ace.output` raise ACE error ace_err_unconf_buf.

Writing a value larger than the implementation maximum raises ACE error ace_err_memory.

To discover the maximum buffer size supported by an implementation, or by the current residual capacity of the internal memory of the ACE unit, write all-ones to `aceiobuflen`: ACE error ace_err_memory is raised, but a subsequent read of `aceiobuflen` returns the actual maximum length; the buffer is simultaneously configured to that length, `aceiobuftop` is set accordingly, and `acestart` is set to zero.
...
{empty}
(((CSR, `aceiobuftop`)))
`aceiobuftop` is an XLEN-bit URW CSR that marks the upper bound of the buffer range used by `ace.input` and `ace.output`. Together WITH acestart[XLEN-9:0][XLEN-9:0], it defines the active transfer window.

The constraints `acestart[XLEN-9:0]` {le} `aceiobuftop` {le} `aceiobuflen` must hold; violating either bound raises ace_err_invalid_value. Modifying `aceiobuftop` does not alter or invalidate buffer contents.

Using ACEIOBUF when `aceiobuftop` − `acestart[XLEN-9:0]` is zero (with `aceiobuflen` nonzero) or an invalid transfer size raises ACE error ace_err_invalid_value.

NOTE: `aceiobuftop` enables per-operation length adjustments without reconfiguring the ACEIOBUF, which can be expensive due to the fact that writing to `aceiobuflen` clears the buffer and may lead to internal garbage collection.
This is useful to process an incomplete “last block” or to supply a short IV or nonce.
...
If bit __i__ of this CSR is set, then ACE error __i__+1 triggers a horizontal trap, as long as `acehtrapvector` is not NULL.
If bit __i__ of this CSR is clear, then ACE error __i__+1 shall not trigger a horizontal trap, but it may still trigger a vertical trap.
...
If this pointer is NULL, horizontal traps on ACE exceptions are disabled, regardless of the value of `acehtrapmask`.
...
ACE provides a set of unprivileged instructions, some of which are implemented as CSRRW operations.
...
+
ACE error ace_err_unimpl is raised if the combination of _Algorithm_, _AlgorithmPolicy_, and _SCProtection_ is not supported.
+
ACE error ace_err_invalid_value is raised if the Metadata is invalid (other than ace_err_unimpl), such as, for instance:
a nonzero _StateNumber_ or reserved field; or an unsupported _Locality_ value.
+
...
If the instruction fails, the target CR is unconfigured, with the same semantics as if `ace.clear` were invoked on the same CR (<<ACE-instruction-clear>>).
+
ACE error ace_err_unimpl is raised if the combination of _AlgorithmPolicy_, _AlgorithmPolicy_, and _SCProtection_ is not supported.
+
ACE error ace_err_invalid_value is raised if the Metadata is invalid (other than ace_err_unimpl), such as, for instance:
nonzero bits in reserved fields; unsupported _Locality_ value; or an undefined value of _StateNumber_ for the _Algorithm_ and _AlgorithmPolicy_.
+
...
+
`ace.import`
appears to other harts as a sequence of individual loads.
+
...
Such system generated keys are system-specific and the “recipe” is in general not portable.
After internal key generation, the actual key value—not the recipe—is stored in the CR and SCC.
For all keys provisioned using `ace.import.S`, the _KeyType_ field in the Metadata is 0, denoting a key represented by value.
...
+
If the V extension register file is not implemented, only Form D `a`ace.exec` is supported and `ace.input`, `ace.output` are used to provide inputs and extract outputs from `ace.exec`, as described in <<ACE-usage-input-output>>.
...
_Description_::
The instruction is used to transition the state machine of the cryptographic algorithm in CR `Kd` or `K(Xd)` from one stage to another-for example, advancing from initialization to data absorption in a hash function, then to finalization and output of the digest.
The `#immed` operand is a constant from a set of admissible values defined for each algorithm.
Optionally, `Xs` or `Vs` contain an auxiliary input.
...
Only the fields _AlgorithmPolicy_, _SCProtection_, _Locality_, _UsagePolicy_, and _ExpirationDate_ of `Xs` may be defined.
All other fields must be 0, else ACE error ace_err_invalid_value is raised.

* `Xs`._AlgorithmPolicy_ is used to set the new `mh`._AlgorithmPolicy_ of the CR.
...
* `Xs`._SCProtection_ is used to turn on side-channel protection or raise its level.
...
Then, when the CR is exported as a CR, it will use the new Locality values to tweak the export and import algorithms.
Generalizing the Locality is not permitted. Attempting to do so raises ACE error ace_err_invalid_value
...
* A non-zero value in `Xs`._ExpirationDate_ will be copied to `mh`._ExpirationDate_, but only if that policy was not already programmed, otherwise error ACE error ace_err_invalid_value is raised.
--
+
On RV32:
...
`ace.input` reads `Xl` bytes from memory starting at address `%offset(Xs)` and copies them to the input buffer, starting at offset `acestart`[XLEN-9:0].
+
...
context switching code. Also, these operations start at the offset specified in `acestart`[XLEN-9:0], so the state of the buffer can easily be saved and restored with the same approach used for vectors.
