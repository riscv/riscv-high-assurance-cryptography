...
|   [31:29] |   3  | Reserved          | Reserved for future use.
...
|   [47:46] |   2  | _KeyType_         | This field determines whether the key is given by value, is a system-defined key, or randomly generated. See <<ACE-KeyType-field>> for more details.
...
// Value is 0 if an input is in the ACE architected format, 1 if in a system-defined format.
...
// If this bit is set, the format of the other 63 bits of the low half of the MDH, xsas well as of the part of the input that would otherwise contain MDH[127:64], is entirely system specific, as is the rest of the input.
...
| 55, 54 | (Reserved)                      | {empty}           | Reserved for future use.
|===


When a CR is being provisioned, exported, or imported, it is in one of the following states:
...
|   57   | `ace_cfgst_exporting`    | The CR has been placed in exporting mode, i.e., its contents have been encrypted and authenticated for export.
|   58   | `ace_cfgst_importing`    | The CR is partially configured as a result of an interrupted import operation.
...
If a CR in in a State with number from 1 to 55, we say it is _Complete_.

If a CR in in a State with number from 56 to 63, we say it is _Incomplete_.

State 1 is the _Ready_ state. All Algorithms must support State _Ready_ as their initial State.

All states in the range 1-47 are called _Valid_.
This includes both _Success_ and _Failure_ states, which must be used for normal completion of an Algorithm in the cases of its successful completion or failure.
...
States 48-55 are termed _Error States_, and are used to report certain specific ACE errors.
...
SGR2:: Any transition listed in an Algorithm's specification may be triggered by performing an explicit `ace.setst` operation (<<ACE-instruction-setst>>), or may be induced by certain conditions specified in the Algorithm's specification. Transitions to and from _Incomplete_ states use the `ace.mgmt` instruction.
...
SGR15:: Usage and cloning of an _Incomplete_ CR is not allowed:
...
`ace.mgmt` is a synonim for `ace.setst`.
+
--
[wavedrom, ,svg]
....
{reg:[
    { bits:  7, name: 0x5b, attr: ['custom-2'] },
    { bits:  5, name: 'Kd|K{Xd}' },
    { bits:  3, name: 'ace.setst', attr: ['0    0    1','0    1    0','0    1    1','1    1    0'] },
    { bits:  5, name: 'rs1', attr: ["no aux input (0)",'Xs','Vs','Xs'] },
    { bits:  1, name: 'r' },
    { bits:  4, name: 0x0 },
    { bits:  7, name: 'immed7' },
]}
...
Values of `#immed7` {ge} 64, depending on the Algorithm, may be used to change _StateExtension_ instead, so that it can be used as state information that is somewhat orthogonal to the _State_ field.
...
When the `#immed7` value is onw of 56, 57, or 58, , the instruction is used to initiate provisioning or export/import operations.
...
|===
| Value | Mnemonic                  |  Description
|  56   | `ace_cfgst_provisioning`  |  Prepare a CR for provisioning keys and metadata into it.
|  57   | `ace_cfgst_exporting`     |  Prepare a CR for exporting it as an SCC (if not yet in the correct format).
|  58   | `ace_cfgst_importing`     |  Prepare a CR for importing an SCC into it.
|  63   | `ace_management_end`      |  Complete current management operation. Semantics depend on _State_.
|===

These values of `#immed7` can be used in all Forms, whereby
[upperalpha]
. Only sets the _State_ field of the MDH according to the value of `#immed7` and the current _State_ of the CR.
. The lower 64 bits of the MDH are in the 64-bit GPR operand at `Xs` (`GR2`).
...
. After `ace.mgmt`, with #immed7 = `ace_cfgst_importing`, the CR's _State_ becomes `ace_cfgst_pp_importing` if the data to import correpond to a partially provisioned CR, and `ace_cfgst_es_importing` if the data being imported corresponds to an CR that was in an Error State.
...
(a) `ace_cfgst_provisioning`,
(b) `ace_cfgst_importing`, `ace_cfgst_pp_importing`, or `ace_cfgst_es_importing` resp.
(c) `ace_cfgst_exporting`, `ace_cfgst_pp_exporting`, or `ace_cfgst_es_exporting`,
the instruction will proceed to complete a
...
.. If `ml`._State_ is a Complete or Error State:
+
...
substituting any other metadata into a CR.
====
...
+
--
If the CR has _State_ = _Unconfigured_, these instructions return zero (which includes the value of the field _State_ = _Unconfigured_).
