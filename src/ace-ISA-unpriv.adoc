...
Use of the zero register `X0` as a CR indirect index raises an illegal-instruction exception, unless explicitly indicated otherwise in the instruction semantics to encode special cases.
...
The entire architectural state of the ACE unit associated with the hart is reset, and ACE exception `ace_guru_meditation` is raised (cf. <<ACE-exceptions>>).
...
|  25    | `ace_state_invalidated`     | _Invalidated_     | The CR is invalidated.
...
(((ACE, algorithm, invalidated state)))
(((Context Register, invalidation)))
We say that a CR has been _invalidated_ when it has been transitioned to Error State `ace_state_invalidated`.
...
For instance, invalidating the CR after a failed side-channel protection upgrade request
(cf. <<ACE-instruction-restrict>>) prevents the CC from being used at a weaker level than
requested if the caller neglects error checking.

Software that needs to preserve the CC should export it as an SCC before requesting the
upgrade, and re-import it if the upgrade fails.
====
...
\begin{tikzpicture}[scale=1.1]
  ...
  \node[error state, text width=1.8cm] at (2,-2)    (invalidated)    {Invalidated\vphantom{g}};
...
either — then the Metadata is invalid, and the CR is invalidated.
...
Management operations, such as exporting (`ace.store`), clearing (`ace.clear`), cloning, or obtaining the size of the exported CR (`ace.size`), do not perform this check. Specifically, the check is skipped when _ConfigStatus_ is not `ace_cfgst_complete`. Consequently, an ACE exception `ace_exc_expired` is raised only when an expired CR is actively used or resumed, while still permitting expired contexts to be cleanly evicted, saved, or unconfigured.
...
+
--
Any instruction with an ACE opcode may raise an illegal-instruction exception for the purpose of emulation.
However, also instructions with ACE opcodes may raise an illegal-instruction exception if certain fields are have invalid encodings in sub-fields (such as `v` = `h` = 1 in `ace.restrict`).
Also, `ace.load` and `ace.store` may be emulated by this mechanism.

In other cases, an illegal-instruction exception may be raised by other conditions, for instance:
 ...
 . Use of a vector-version of any ACE instruction when the vector extension (or RVV-mini) is not enabled.
...
+
--
Certain error conditions make the CR invalidated, which means that it cannot be used further except by unconfiguring it, re-provisioning or, if the algorithms supports it, by transitioning it to State _Ready_.
...
. Algorithm-specific errors may invalidate the CR, or transition it to some other State (usually an Error State), according to the behaviour specified by the algorithm.
...
// If the ACE Privileged Architecture is not implemented, ACE exceptions are replaced by transitions of the CR's _State_ field to an Error State, per the mapping in <<ACE-exception-error-correspondence>>. Conversely, exception handlers must set a CR's _State_ to the corresponding Error State when the following three conditions are met: they cannot resolve the exception; cannot forward it to an in-process handler; and do not suppress the process that caused it.
//
// [[ACE-exception-error-correspondence]]
// .Correspondence between ACE Exception and Error States
// [%autowidth,float="center",align="center",options="header"]
// |===
// | `ace_guru_meditation`                   | Implementation and platform defined behavior
// | `ace_exc_CR_off`                        | Does not apply without privileged architecture
// | `ace_exc_out_of_memory`                    | `ace_state_out_of_memory`
// | `ace_exc_privilege_violation`           | `ace_state_priv_viol`
// | `ace_exc_unconfigured_buffer`                    | `ace_state_buf_unconf`
// | `ace_exc_invalid`                       | {empty}
// |===
// The conditions that cause `ace_state_expired`, `ace_state_invalidated`, and `ace_state_import_auth` do not raise ACE exceptions.
...
Writing a value that is invalid or unsupported for a given Algorithm may still cause a CR to be invalidated
...
Also, writing a value that is invalid or unsupported for a given Algorithm may still cause a CR to be invalidated
when an instruction is later issued or restarted.
...
Using an `#immed7` that is not supported by the architecture or by the current algorithm will invalidate the CR.
...
.. If the combination of _Algorithm_, _AlgorithmPolicy_, and _SCProtection_ in `ml` is not supported,
   ...
   This is distinct from ACE exception `ace_exc_invalid` in order to allow an exception handler to add trap-and-emulate support for non-supported combinations.
...
* If the algorithm configured in the CR does not make use of the field _AlgorithmPolicy_,
  ...
  else the CR is invalidated.

* No operation disabled by _AlgorithmPolicy_ may be (re)enabled.
   An attempt to do so invalidates the CR.
   In some cases, a zero value is not permitted, such as disabling both encryption and decryption. In this case, attempting to do so invalidates the CR.
...
* If the CR's current state is _only_ allowed while performing an operation currently
  ...
  disable that operation invalidates the CR.
...
** The request may only strengthen side-channel protection.
   Any request for a weaker level than the current one invalidates the CR.
...
* If `Xs`._Locality_ is non-zero, it is used to update `MDH`._Locality_, subject
  ...
  . Generalizing (i.e. weakening) Localities is not permitted: Attempting to do so invalidates the CR.
...
* A non-zero value in `Xs`._ExpirationDate_ will be copied to
  ...
  Otherwise the CR is invalidated.
...
Disabling a currently permitted operation, such as encryption or signature generation, while in a State corresponding to the disabled operation will also cause the CR to be invalidated.
...
. Any substitution is allowed only if the ACEIOBUF is enabled and `aceiobuflen` is non-zero.
  Otherwise, ACE exception `ace_exc_unconfigured_buffer` is raised.
+
If `aceiobuflen` is non-zero but `aceiobuftop - acestart` is invalid for the algorithm, the CR is invalidated if the ACEIOBUF is used as an input, whereas if it is used only as an output, then the result is undefined.
