...
* When the 64-bit index system key index in the PI read nby `ace.prov` is an all all-ones value (i.e., `ones(64)`), it denotes that the key material must be generated randomly and, similarly to the behaviour of `ace.import.S`, the _KeyType_ field of the metadata is internally set to 0, just as for any other explicitly provisioned key, and exported as such, i.e., by value. This applies to all key material in all algorithms, unless otherwise specified.
...
This means that interrupts other than memory exceptions must wait until the next memory access.

Provisioning and import instructions may resume from the point of interruption only if control returns directly after the interrupt handler completes. The privileged interrupt handler must guarantee restart only in this case. In any other circumstance, including a context switch, the instruction must be restarted, and if it is `ace.prov` or `ace.import`, the target CR must also be cleared before restarting.

An export may resume only if the CR has not been modified before returning to the original control flow.

The implementation will restart any of these operations if the CR is not in a valid interrupted state.

The initial SIV computation of `ace.export` may be implemented as either uninterruptible or interruptible. If uninterruptible, interrupt handlers may need to wait for completion before accessing the CR, potentially creating performance bottlenecks or timing issues. To mitigate these issues, this computation can be made interruptible. as described following the `POLYVAL` function specification in <<ACE-SCC-POLYVAL>>.
...
AES-GCM-SIV cite:[DBLP-journals-iacr-GueronLL17,RFC8452] is used to export and import CRs.
...
// NOTE: Due to the computational cost of the derivation functions, implementations may cache recently used derived keys.
...
During SCC export and import, the Locality Secrets associated with the Localities encoded in the _Locality_ field of the Metadata are prepended, in ascending index order, to the input of the AEAD tag computation, before the metadata.
If the _Locality_ field is zero, no Locality Secrets are prepended.

The export operation maintains a per-CR flag indicating whether an export is in progress. This flag prevents the resumption of an export after an instruction that modifies the CR's internal state is performed. If this scenario were allowed, resuming the export could compute an incorrect SIV for the remaining SCC data, causing subsequent import authentication failures. Consequently, any operation that modifies the CR's internal state clears this flag. If an attempt is made to resume an export operation when the flag is cleared, the export restarts from the beginning.
...
Interrupting `ace.export` after the key derivation may be possible only immediately before the first block of to the input of `POLYVAL` is processed, resulting in a `acestart` value of 1. `POLYVAL` itself cannot be interrupted between the two arithmetic operations inside the loop. If `POLYVAL` is interrupted after the `montmul` operation, the value of
`acestart` will be 16{interpunct}(`i`+1)+1, the number of bytes absorbed so far, plus 1.
 ...
 ** If strictly fewer than four bytes are read before the instruction is interrupted, ACE leaves `acestart` at 0, and the instruction will be restarted upon resumption.
 ** If four bytes or more have been read, the value of `mh.StateNumber` in the SCC is known. In this case, the instruction may be restarted WITH acestart[XLEN-9:0] {ge} 4 recording the amount of data read.
* Af the beginning of the import operation, the hardware saves the `StateNumber` value from the original metadata and sets
`mh.StateNumber <- ace_state_invalid`. At the end of the import operation, `mh.StateNumber` is set to the saved value.
...
[NOTE]
====
The top 8 bits of `acestart` indicate whether a provisioning, import, or export operation is in progress, eliminating the need for separate `provisioning`, `importing`, or `exporting` bits per CR. This design also means that only one such operation can be interrupted at a time.

An architecture could theoretically support tracking multiple interrupted import/export operations simultaneously. This would require storing copies of the `SIV` and `tmp` values (256 bits) along with other small quantities for each interrupted operation. Since interrupted import/export operations are uncommon and would only be significant for very large states—of which few would be in flight simultaneously—this capability is perceived as not necessary. It is the responsibility of the exception handlers to fully erase `acestart` in these cases to prevent unpredictable behaviour because of the restart of interrupted privisioning or import, or of the export of a CR which has been modified in the meantime.
====
