...
States 56–63, i.e., the Configuration States, are exactly the States that are neither _Unconfigured_ nor Complete.
...
GR8:: _Reserved encodings._
All encoding points under the ACE opcodes that this chapter does not assign are reserved, and raise an
...
.. If `ml`._State_ is _Unconfigured_, the instruction performs no operation and raises an illegal-instruction exception.

.. If `ml`._State_ is an Error State, the CR is left unchanged and no management operation is opened.
...
.. If `ml`._State_ is a Valid State:
+
...
+
The architecturally observable behavior is described in <<ACE-CC-management>>.

... CR's _State_ is set to `ace_cfg_exporting`.

.. If `ml`._State_ is in a Partial State (we are exporting a not fully configured CR or we are initiating a nested CR export):
+
