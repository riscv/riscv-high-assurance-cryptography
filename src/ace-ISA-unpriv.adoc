...
An implementation realizes this in one of three ways: by completing the instruction before taking an asynchronous interrupt; where Rule IRR3 permits, by letting it complete in the background, subject to Rule <<ACE-IRR-conflicting-access-rule>> below; or by discarding its partial work before taking the trap and re-executing it upon return.
...
[[ACE-IRR-long-running-no-data,IRR3]] IRR3:: Long-running ACE operations that access no external memory—including `ace.setst`, `ace.mgmt`, and `ace.exec` or `ace.derive` forms that do not access GPRs, vector registers, or the ACEIOBUF—may continue while the hart is preempted. If an interrupted operation is still running when a handler accesses its CR, the hart waits for completion before resuming. Should another interrupt occur while instruction continues to execute, the second handler must wait as well.
+
An Algorithm may define such operations; examples include scalar multiplication, key generation, and signature generation or verification. Their progress is recorded in the _AlgorithmUse_ field, in an implementation-defined format; `acestart` remains reserved for byte counts (<<ACE-CSR-acestart>>). If progress is recorded, the implementation exports the data necessary to support resumption in the ADS. Any importing implementation may discard this Section and restart the operation, and must restart it on an Implementation Qualifier mismatch. Hence, no internal algorithm-dependent state may be committed until the operation completes, and intermediate results must be discarded. Each resumption either advances the recorded progress or restarts the operation; an implementation capable of resuming also supports restart.
...
[horizontal]
[[ACE-IRR-resumability-options,IRR4]] IRR4:: In general, the following behaviors are allowed when an interrupt or exception handler accesses an input or output of a preempted operation, the associated CR, or any ACE state that may be affected by the operation:
