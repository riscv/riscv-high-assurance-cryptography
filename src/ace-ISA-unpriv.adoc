...
`{fournbsp}{fournbsp}` `varlen` <- 8 + (`HasPublicKey` != 0) {interpunct} ``ub``/8 + (`HasSignature` != 0) {interpunct} 2``b``/8 + +
 ...
 ** From any of _initial_, _set_generator_, _set_scalar_, _set_hash_, _set_publickey_ ->
  ...
  *** _sign_generate_ only if `HasPublicKey`, and `HasHash`;
  *** _sign_verify_ only if `HasScalar`, `HasPublicKey`, `HasHash`, and `HasSignature`.
 ...
 ** Upon returning to State _initial_:
 ...
 This is determined by Bits 0, 1, 2, and 3 of `Xs` as follows:
   **** If Bit 0, 1, 2, resp., 3 is set, then `Generator` is reset to the default value, `SecondPt`, `Scalar`, resp., `PublicKey` is erased, and the corresponding Boolean flags `HasScalar`, resp., `HasPublicKey` is unset as well.
