...
serialized `Content[]` of the CR.  `acestart` tracks the number of bytes loaded.
...
`acestart` tracks the number of bytes stored.
...
_Synopsis_::
Return the size of the `Content[]` of a PI, CR, or SCC in bytes, or 1 in case of error (unsupported Algorithm or invalid MDH).
...
The instruction returns `1` on error: for Form A when the CR is unconfigured, and for Forms B–D
...
A return of `0` is *not* an error indication: it is the size of the SCC of a CC in an Error State
(<<ACE-length-rule>>), and the caller can tell the two apart by inspecting the _State_ field of the
MDH it supplied.
--
