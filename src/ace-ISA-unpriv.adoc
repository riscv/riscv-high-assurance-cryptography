...
Both segments derive their keys from the same CSK and all-zero nonce. Without `sep`,
...
The cost is one bit in each case: the tag input carries 126 bits of the `POLYVAL`
result rather than 127, and the keystream counter block carries 94 bits of the `SIV` rather than
95. Both reductions leave the corresponding collision terms of the same order as the
almost-XOR-universal bound of `POLYVAL` itself, and so they do not affect the security claims.
====
...
The Locality Secrets are fed to the AEAD as _associated data_ rather than mixed into the
Context Sealing Key via a KDF. Both bind the SCC to the Locality set, but the
associated-data path is cheaper: POLYVAL, which is an almost-XOR-universal (AXU) hash,
costs one `Montmul` per 128-bit block. This is significantly faster than most KDFs, and
avoids re-deriving `enc_key` and `auth_key` when the Locality set changes.
A distinct Locality set yields a distinct output except with collision probability at most
_l_/2^128^ for inputs of _l_ blocks.
