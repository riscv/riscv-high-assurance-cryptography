...
|   61     | `ace_cfg_exporting_pp` | A partially provisioned CR has been placed in exporting mode.
|   62     | `ace_cfg_importing_pp` | A partially provisioned CR which has been exported is being reimported.
...
Requesting the SCC's size (`ace.size` <<ACE-instruction-size>>) will return zero (bytes).
...
Error State, `ace.clear`, and `ace.clear.all`, which are normally considered usage instructions, are also allowed
...
Let `size` = the exported SCC's length, as determined by the CR's MDH, which is a multiple of 16 bytes.
`ace.load` transfers offsets `[acestart, size)` where
...
.. `ace_cfg_importing_pp` if the data to import correspond to a partially provisioned CR.
. After `ace.mgmt`, with #immed7 = `ace_cfg_exporting`, the CR's _State_ becomes
.. `ace_cfg_exporting` if the CR is in a Valid State,
.. `ace_cfg_exporting_pp` if the CR being exported was a partially provisioned one.

If a CR is in an Error State, it is exported by simply exporting the copying the MDH to memory.
...
The `ace.mgmt` operation used to start a provisioning, resp., import process should be Form C or D in order to provide `ml`.
Form A with `ace_cfg_provisioning` or `ace_cfg_importing` is equivalent to passing a 128-bit zero string as the auxiliary input.
...
... If `ml`._State_ = `ace_cfg_exporting_pp`, then CR._State_ is set to `ace_cfg_importing_pp`;
... else CR._State_ is set to `ace_cfg_importing`.
--
+
...
... if `ml`._State_ is an Error State, the CR is invalidated. Export of a CR in an Error State is performed by simply extracting its MDH.

... if `ml`._State_ is in a Valid State, CR's _State_ is set to `ace_cfg_exporting`.
...
Form A supplies a zero auxiliary input.
...
+
--
`ace.clear` unconfigures the state of the CR `Kd` or `K{Xd}`, clears its contents, marks it as _Unconfigured_ in its metadata, and releases the resources in the CRF.
...
--
 ...
 . `saved_MDH` <- `M`, the *initial* MDH, that is the image carried in the SCC and passed to `#ace_cfg_importing`. This is the same value that the exporting side used under that name.
