...
==== States for the Symmetric Algorithms in this Specification
...
// Algorithms that differ substantially from those described in <<ACE-algorithms>>, and which are not meant to be handled by the same code as one of those algorithms, may reuse the state values in [1..12] with their own mnemonics and names for their states.
