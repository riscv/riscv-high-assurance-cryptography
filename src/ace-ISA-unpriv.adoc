...
The `_pi` and `_scc` flavours of the four nested States record whether the image the CR holds is
PI-shaped or SCC-shaped. They determine its length (<<ACE-instruction-size>>), which could not
otherwise be recovered once the base _State_ has been overwritten by the nesting.
...
SGR11:: Exporting a CR in an Error State exports only the MDH (all fields and values).
Requesting the SCC's size (<<ACE-instruction-size>>) returns 16 bytes for `ace.size` and zero for `ace.csize`.
...
This instruction also requires that the CR be ready to import information and that the MDH has already been loaded by passing it as a parameter in a GPR pair (quartet for RV32), resp., vector register, to the `ace.mgmt` instruction used to open the CR for provisioning or import.
...
Let `size` be the size of `Content[]` in a PI or of `Content[] = Content1[] || Content2[]` in an SCC.
`size` is zero for a CR in an Error State.
...
Let `size` be the length in bytes of the serialized `Content[]` of the CR, as determined by its MDH,
...
*Bound.* Let `size` be the length in bytes of the `Content[]`, determined from the MDH.

* If `acestart` {ge} `size`, the instruction performs no operation — no CR byte, GPR, or vector
  register is written and `acestart` is unchanged.
* If `acestart` + `n` > `size`, where `n` is the number of bytes the variant would otherwise
  transfer, only the bytes at offsets [`acestart`, `size`) are transferred, and `acestart` is
  set to `size`.
...
Writes, resp., reads past the serialized length of `Content[]` are ignored, resp., read as zero.
...
If a CR is in an Error State, it is exported by simply copying the MDH to memory.
...
.. If `ml`._State_ is an Error State, then the CR is immediately fully configured in the Error State, with
    ...
    No `ace.mgmt` with `#immed7` = `ace_cfg_management_end` is needed, and in fact it will be an illegal instruction.
...
.. If `ml`._State_ is an Error State, the CR is left unchanged: the export of a CR in an Error State
consists solely of reading its MDH with `ace.getmd*` (<<ACE-instruction-getmd>>) and storing it to memory.
...
--
+
. *If `ace_cfg_management_end` is issued when the target CR's _State_ is a Complete State* (i.e., no management operation open on the target CR), the instruction does not perform any state change and raises an illegal-instruction exception.
...
A CR must not be cleared while being exported, unless the handler has first exported it completely (<<ACE-nested-CR-management>>) and restores it before returning control, since until the export completes the ciphertext it holds is the only copy of the CC.
...
These are different quantities: `pi_content_size` {le} `content1_size`, and the two need not be equal.
...
* A read or write of a GPR or vector register that the operation writes as its destination, or reads as an operand.
...
A handler must not clear a CR being exported (<<ACE-instruction-setst>>), unless it has first exported it completely and restores it before returning control.
Should it nevertheless do so, by mistake or on an unrecoverable error, the caller detects it by the same check after `ace.store`; the CC is then lost, and the caller must treat the error as unrecoverable. This allows the error to propagate to the outermost caller.
A further `ace.store` on the cleared CR raises an illegal-instruction exception (<<ACE-instruction-store>>).
...
// +
// `ace.derive` is excluded from this rule because interruton could happen at both the source and destination endpoints, which would make restart of, essentially, asynchronous flows of information, in general untreatable. Therefore, any implicit `ace.exec` operation performed by `ace.derive` between two Algorithms must be non-interruptible.
...
of a PI and of an SCC, as well as the CRF capacity occupied by a CR, are all
...
No other MDH field may affect any of these three quantities.

If the CR is in an Error State, the three rules above do not apply: the SCC consists
of the MDH alone and its length is always 16 bytes, irrespective of every other field,
...
When exporting a CR in an Error State, Sections 2–5 are empty; that is, the SCC consists only of the `MDH`.
