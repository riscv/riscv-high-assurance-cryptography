...
More precisely: For each _j_ from `acestart` to `end` − 1, the byte at memory address ``Xs1 + %offset + ``_j_ is loaded into the _j_-th byte of the
...
Let `size` be defined as for `ace.load` (<<ACE-instruction-load>>).
`ace.store` transfers offsets `[acestart, size)`.

More precisely: For each _j_ from `acestart` to `size` − 1, the _j_-th byte of the serialized `Content[]` of the CR is stored  at memory address ``Xs1 + %offset + ``_j_.
...
For all variants, if the CR's _State_ is not one of the values in the fourth column of its row — which excludes every Complete State and the _Unconfigured_ State — an illegal-instruction exception is raised without state change. A vector variant for which `vl * (SEW/8)` is not a multiple of 16 raises an illegal-instruction exception without state change; the V-enabled check is performed first. As for `ace.load` (<<ACE-instruction-load>>), an `acestart` that is not a multiple of 16 also raises an illegal-instruction exception without state change.
...
as specified in <<ACE-CC-management>>:

... CR's _State_ is set to `ace_cfg_exporting`.
