...
When `ace.exec` operates iteratively on an inputs who length is an integer multiple of the basic processing unit, each block is processed uninterruptibly, but the instruction may be interrupted between blocks; the hardware sets `acestart` to the interruption point and possibly additional information.

The interruptibility of provisioning, import, and export operations is defined in <<ACE-provisioning-export-import>>.
