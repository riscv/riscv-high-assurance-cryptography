...
together with the ACE memory operations `ace.load` (<<ACE-instruction-load>>) and `ace.store` (<<ACE-instruction-store>>), enables the following operations:
...
* `ace.getst` (<<ACE-instruction-getst>>) reads the state (from the MDH) of a CR, which is also used for error reporting.
