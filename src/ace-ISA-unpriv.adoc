...
. After `ace.mgmt`, with #immed7 = `ace_cfgst_importing`, the CR's _State_ becomes `ace_cfgst_importing_pp` if the data to import correspond to a partially provisioned CR, and `ace_cfgst_importing_es` if the data being imported corresponds to an CR that was in an Error State.
. After `ace.mgmt`, with #immed7 = `ace_cfgst_exporting`, the CR's _State_ becomes `ace_cfgst_exporting_pp` if the CR being exported was a partially provisioned one, and `ace_cfgst_exporting_es` if CR begin exported was in an Error State.
