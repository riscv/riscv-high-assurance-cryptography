...
[NOTE]
====
Another difference with respect to AES-GCM-SIV is that we add a further domain separation bit
to generate the encryption keystream. Since we are encrypting two segments (the fixed architecture-defined content and the optional implementation-dependent content) we replace bit 126 of SIV, resp. SIV2, with 0 and 1 in the input to keysteam generation to distinguish them.
The two content segments are keyed identically. No attack follows from this, since a SIV=SIV2 collision
happens with negligible probability; but the separation is then a property of the values rather than of the
construction. Forcing these bits to be different
makes the separation unconditional, which is what a security proof requires.
====
...
function SCC_Decrypt(AD[] : array of bits(128), N : bits(96), SIV : bits(128),
    sep: bits(1), C[] : array of bits(128), K : bits(256),
...
--
 ...
 . `len_PC` <- length of Section 3.
 . `correct`, `K(i).Content1[]` <- ``SCC_Decrypt(AD, zeros(96), SIV, 0, K(i).Content1[], CSK, len_AD, len_PC)``.
...
.. `len_AD2` <- 2, ``AD2[0]`` <- ``K(i)``.``IMPQUAL``, ``AD2[1]`` <- `SIV`.
.. `len_PC2` <- length of Section 6.
.. `correct`, `K(i).Content2[]` <- ``SCC_Decrypt(AD2, zeros(96), SIV2, 1, K(i).Content2[], CSK, len_AD2, len_PC2)``.
