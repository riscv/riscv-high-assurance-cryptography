...
|  52    | `ace_state_privilege_viol`      | _Privilege Violation_       | Violation of Usage Control policies.
...
|  59,60   | (Reserved)               | Reserved.
|   61     | `ace_cfg_exporting_partial` | The CR, which was in a Partial State, has been placed in exporting mode.
|   62     | `ace_cfg_importing_partial` | A CR exported while in a Partial State is being reimported.
...
SGR17:: Instructions `ace.size`, `ace.getmd*`, `ace.getst`, `ace.setst` with `#immed7` equal to `ace_state_unconfigured` or an
...
are allowed in States _ace_cfg_provisioning_, _ace_cfg_importing_, and _ace_cfg_importing_partial_.

SGR19:: `ace.store` and `ace.mv` with a CR as source
are allowed in States _ace_cfg_exporting_, and _ace_cfg_exporting_partial_.
...
| `ace_exc_privilege_violation`      | Transition to Error State `ace_state_privilege_viol`.
...
. If `acemanagedcr` {ne} 32 and the CR it names is importing or exporting a CR, i.e., its _State_ is one of `ace_cfg_importing`, `ace_cfg_exporting`, `ace_cfg_importing_partial`, or `ace_cfg_exporting_partial`, then also `acesivl`, `acesivh`, `acesiv2l`, `acesiv2h`, `aceiql`, `aceiqh` must be saved and restored.
The `_partial` States are included because they correspond to an outermost import or export operation.
...
`ace.mv` legitimately keeps the two independent, since its `acestart` is a CR offset that accumulates across instructions while `vstart` tracks the vector register.
+
NOTE: The uninterruptible vector-form instructions (`ace.getmdv`, `ace.restrictv`, Form C of `ace.size`, `ace.setst`, and `ace.mgmt`) do not produce `vstart` values and therefore do not consume one. Similarly to RVV 1.0 {sect}3.7, which lets an implementation either raise an illegal-instruction exception on a `vstart` value the instruction can never produce or ignore it, ACE leaves the same options to the implementation regarding `acestart`.
...
| `ace.setst`                  | Yes             | No  | Yes, except as noted | Not usage-controlled when `#immed7` is `ace_state_unconfigured` — that is `ace.clear` and `ace.clear.all` (<<ACE-instruction-clear>>) — or an Error State.
...
If the CR's _State_ is not one of the States admitted by Rule SGR18 (<<ACE-State-field>>), or `acestart` is not a multiple of 16, an illegal-instruction exception is raised without state change.
...
[[ACE-mv-variants]]
.Variants of `ace.mv`
[float="center",align="center",width="100%",cols="^10%,^16%,<44%,<30%",options="header"]
|===
| `Form` | Sub-opcode | Transfer | Admissible _State_

| `0b01` | `rd` = `0b00001`
| The 128-bit GPR operand at `rs2` is copied into the CR at offset `acestart`.
| `ace_cfg_provisioning`, `ace_cfg_importing`, `ace_cfg_importing_partial`
...
| `ace_cfg_exporting`, `ace_cfg_exporting_partial`
...
.. `ace_cfg_importing_partial` if the data to import corresponds to a CR in a Partial State.
. After `ace.mgmt`, with #immed7 = `ace_cfg_exporting`, the CR's _State_ becomes
.. `ace_cfg_exporting` if the CR is in a Valid State,
.. `ace_cfg_exporting_partial` if the CR being exported is in a Partial State.
...
. It may affect `acemanagedcr` as follows:
.. if it opened a management operation, it writes the target CR's number to `acemanagedcr`;
.. if it completed a management operation resulting in a CR in a Complete State, it writes 32 to `acemanagedcr`;
.. if it completed a management operation resulting in a CR in Partial State, `acemanagedcr` is left unchanged;
.. if it transitions the target CR to an Error State, it writes 32 to `acemanagedcr`; and
.. if it raises an ACE exception without transitioning the target CR to an Error State, `acemanagedcr` is left unchanged (<<ACE-CSR-acemanagedcr>>).
...
... If `ml`._State_ is a Partial State, then CR._State_ is set to `ace_cfg_importing_partial`;
... If `ml`._State_ is _Unconfigured_, the Metadata is invalid and the CR is transitioned to Error State _Invalid_.
.. In every case above in which an ACE exception is raised or the CR transitions to Error State _Unsupported_ or _Invalid_, all fields of the MDH other than _State_ remain zero and no CRF capacity is allocated; after `ace_exc_out_of_memory` the CR remains _Unconfigured_.
--
+
...
... The CR's _State_ is set to `ace_cfg_exporting_partial`.
...
* `ace_cfg_importing`, `ace_cfg_importing_partial`, resp.
* `ace_cfg_exporting`, `ace_cfg_exporting_partial`,
...
if it is `ace_cfg_importing_partial` or `ace_cfg_exporting_partial`, `ml`._State_ must be one of
`ace_cfg_provisioning`, `ace_cfg_importing`, `ace_cfg_exporting`, `ace_cfg_importing_partial`, or `ace_cfg_exporting_partial`.
...
* If source and destination are the same CR the instruction performs no operation, even if the CR is _Unconfigured_.
...
A CR in a Configuration State raises ACE exception `ace_exc_privilege_violation` (Rule SGR16 in <<ACE-State-field>>) before any _Invalid_ transition is evaluated, with no state change to either CR.
...
Let `content1_size` be the length in bytes of the `Content1[]` of an SCC — equivalently, of the `Content[]` of a PI — for the _Algorithm_, _AlgorithmPolicy_, _KeyType_ and _StateExtension_ of the MDH (<<ACE-length-rule>>),
and `content2_size` = 16 * (_AuxDataLen_ - 2) if _AuxDataLen_ {ge} 2, and 0 otherwise.
...
. If the MDH._State_ is a Partial State, then:
  .. If the MDH._State_ is `ace_cfg_provisioning`, then: 16 * (1 - `q`) + `content1_size`.
  .. If the MDH._State_ is `ace_cfg_{importing|exporting}_partial`, then: 16 * (1 - `q`) + `content1_size` + `content2_size`,
...
An implementation realizes this in one of three ways: by completing the instruction before taking an asynchronous interrupt; by letting it complete in the background, subject to Rule IRR5 below; or by discarding its partial work before taking the trap and re-executing it upon return.
...
It is available only for `ace.load`, `ace.store`, `ace.input`, and `ace.output`, whose transfers are idempotent; `ace.exec`, `ace.derive`, and `ace.mv` use option 3 or 4 (Rule IRR7).
...
The MDH fields that determine the length of a PI and of an SCC, as well as the
CRF capacity occupied by a CR, are all contained in the lower 64 bits of the MDH:
...
If the CR is in an Error State, the SCC consists only of the MDH, and its length
is 16 bytes irrespective of every other field.
...
* If a partially provisioned SCC is exported, the data is in the clear. This is not a security issue, as the plaintext was already available to software and thus to its more privileged modes.

* If a partially imported SCC is exported, the data is already encrypted.

* If the partial export of a CR into a SCC is preempted and the CR must be exported again, the data is already encrypted.
...
| 1       | MDH, MDH   | 128 bits
| 2.a     | `Content1`   | Cf. Section 5.a in <<ACE-SCC>>.
| 2.b     | `Content2`   | Cf. Section 5.b in <<ACE-SCC>>.
|===
...
B -> N1: Preemption.
  N1 -> N1: Save acestart.
  N1 -> N1: Start export: Save current <i>State;</i> <i>State</i> → ""ace_cfg_exporting_partial"".
  N1 -> N2: Preemption.
    N2 -> N2: Save acestart.
    N2 -> N2: Start export: Save current <i>State;</i> <i>State</i> → ""ace_cfg_exporting_partial"".
    N2 -> N2: Complete export: Restore saved <i>State</i> (""ace_cfg_exporting_partial"").
    ...
    N2 -> N2: Start import: Previous <i>State</i> same as in Step 8,\nhence, <i>State</i> → ""ace_cfg_importing_partial"".
    N2 -> N2: Complete import: <i>State</i> restored to ""ace_cfg_exporting_partial"".
  ...
  N1 -> N1: Start import: Current state same as <i>State</i> saved in Step 4;\n Save this <i>State;</i> then, <i>State</i> → ""ace_cfg_importing_partial"".
  N1 -> N2: Preemption.
    N2 -> N2: Save acestart.
    N2 -> N2: Start export: Save current <i>State;</i> <i>State</i> → ""ace_cfg_exporting_partial"".
    N2 -> N2: Complete export: <i>State</i> restored to value saved in Step 23.
    N2 -> N2: Start import: Save current <i>State;</i> <i>State</i> → ""ace_cfg_importing_partial"".
    N2 -> N2: Context save, including ""SIV"", ""SIV2"", ""IMPQUAL"".
    N2 -> N2: Yield control to other processes.
    N2 -> N2: Context restore, including ""SIV"", ""SIV2"", ""IMPQUAL"".
