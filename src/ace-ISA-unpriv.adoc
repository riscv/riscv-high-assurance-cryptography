...
From the point of view of the architecture, the trap would be precise, i.e., all instructions before the one that causes the trap are completed, and the address of the instruction should be set in a the User EPC (`uepc`) CSR. `ucause` would not be used in this case, with `aceerror` providing the instruction.
