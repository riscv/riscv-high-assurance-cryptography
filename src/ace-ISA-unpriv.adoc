...
Hence, as much as possible of the logic implementing CR management instructions is implemented by software, and
...
| `ace.setst` (and the pseudo-instructions `ace.clear`, `ace.clear.all`, `ace.clear.ads`), `ace.mgmt`
| `Zkl` common
| Forms A–C
...
| `Zkl` common
2+<| Always present. On RV32, their `*h` shadows as well.
|===
...
. An implementation-defined amount of memory that can be dynamically allocated to the various registers.
  The region of this memory allocated to a CR contains `Content[]` while provisioning and
   `Content[] := Content1[] || Content2[]` during other management operations.
...
This choice prevents concurrent management operations in the same code: starting a second operation may overwrite `SIV`, `IMPQUAL`, and `SIV2`, destroying in-flight values of the first operation. Hence, the CSRs holding these values must be saved and restored at context switch.
Interleaved management operations within a single process should therefore be avoided, unless a lightweight threads library is used providing support for ACE context switching. Otherwise, management operations must be serialized using a mutex.
====
...
States 56–63, i.e., the states used in management operations, are called the _Partial States_, and are exactly the States that are neither _Unconfigured_ nor Complete.
...
Any usage instruction in State _Unconfigured_ raises an illegal-instruction exception.
...
Error State, `ace.clear`, and `ace.clear.all`, which normally are treated as usage instructions, are also allowed
