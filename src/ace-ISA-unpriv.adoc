...
| `ace_exc_csk_incomplete`       | Does not apply: the condition can only be observed from a mode below M, and the flags are reset on every entry to M-mode.
...
Any of the following conditions raises an illegal-instruction exception.
 ...
 . Using any ACE instruction, or accessing an ACE CSR, if a CSK has not been configured
   (<<ACE-CSK-requirements>>). Two groups are exempt: the `macecsk` group (if present), and the
   read-only identification CSRs `acemvendorid`, `acemarchid`, `acemimpid` and `acemaxiobuflen`,
   which remain readable so that firmware can identify the ACE unit before establishing a CSK.
...
In some Algorithm-specific cases, `ace.exec` and `ace.setst` may be interrupted with a different semantic that does not necessarily correspond to a byte-granular progress, and which is given explicitly in the specifications of the corresponding Algorithm.
...
Without a configured CSK, every ACE instruction raises an illegal-instruction exception, as does
...
written, and use of it in that condition raises ACE exception `ace_exc_csk_incomplete`.
...
representation of the CR, that is the (_j_+16)-th byte of the serialized payload.
Both the memory address and the CR offset carry the same _j_, so the two are always in step.
...
Precisely, for each _j_ from `acestart` to `size` − 1, the _j_-th byte after the MDH of the
serialized representation of the CR, that is the (_j_+16)-th byte of the serialized payload, is
stored to memory address `Xs1 + %offset + `_j_.
Both the memory address and the CR offset carry the same _j_, so the two are always in step.
