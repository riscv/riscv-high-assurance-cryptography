...
The `pi` and `scc` flavours of the nested States record whether the image the CR holds originates
...
Currently, the only such operation is encoded with the value 64 (`ace_cfg_clear_ads`) which clears any ADS so that the CR can be exported without it
...
SGR15:: Any _usage_ instruction that would violate the Usage Control policies will not perform its operation, and raises ACE exception `ace_exc_privilege_violation`
...
SGR17:: Instructions `ace.size`, `ace.getmd*`, `ace.getst`, `ace.setst` with the 7-bit immediate equal to `ace_state_unconfigured` or an
...
[[ACE-SGR-ace-load-mv-allowed,SGR18]] SGR18:: `ace.load` and `ace.mv` with a CR as destination
...
[[ACE-SGR-ace-store-mv-allowed,SGR19]] SGR19:: `ace.store` and `ace.mv` with a CR as source
...
The _ExpirationDate_ of a CR that is not in a Complete State is not evaluated.
...
. The secure clock is converted to the format of the _ExpirationDate_ field.
. This value is then compared to the _ExpirationDate_ itself.
. If the value derived from the secure clock is larger than the _ExpirationDate_, the hardware transitions the CR's _State_ field to Error State `ace_state_expired`.
...
. A reserved encoding point (Rule <<ACE-GR-reserved-encodings>>), including a reserved `#immed7` of `ace.setst` with `r` = 1 and CR register `X0` (<<ACE-instruction-setst>>), and an unsupported ACEV configuration (<<ACE-ACEV>>).
...
. A misaligned GPR pair on RV32 (Rule <<ACE-GR-multiple-GPRs>>).
...
. An ACE instruction issued while `ACES` is Off (see _Fatal_ below) and, when the Privileged Architecture is not implemented, an access to a CSR that <<ACE-CSRs-unpriv>> declares unavailable.
...
`acemanagedcr` is restored after the CRs, or held at 32 for the duration of the restore: `ace.mgmt`
requires it to hold either its target CR's number or 32 (<<ACE-instruction-setst>>), so a restore
that writes it first cannot then use `ace.mgmt` on any other CR.
...
the in-flight `SIV`, `SIV2` and `IMPQUAL` values.  These values can be saved and restored.
CR management operations require particular attention, as described in <<ACE-CC-management>>.
...
Opening a CR for import, as well as executing `ace.clear`, is always allowed in any State, since the
...
GR8:: Instructions that access the ACEIOBUF, such as `ace.input` and `ace.output`, may be used only if the
...
Let `max_admissible` = the implementation's maximum SCC payload length for the Algorithm, which is a multiple of 16 bytes.
...
* A block at an offset at or beyond `end` is not: no CR byte, GPR, or vector register is written
  for it, and `acestart` does not advance past `end`.
...
When `r` = 1 and the CR register is `X0`: if `#immed7` = 0, the instruction is `ace.clear.all` (<<ACE-instruction-clear>>); all other values of `#immed7` are reserved and raise an illegal-instruction exception.
...
* An Error State code (see <<ACE-states-error>>) is always permitted for `#immed7` in any Complete State.
...
.. If `#immed7` = `ace_cfg_importing` and `ml`._State_ is an Error State, then the CR is immediately
    fully configured in the Error State, with the entire MDH given by the auxiliary input.
    ...
    No CRF capacity is allocated, since the CR holds only its MDH (<<ACE-cryptographic-registers>>),
...
if it is one of the four nested States, `ml`._State_ must be a Configuration State of the same flavour,
...
* If the source is not in a Complete State (Valid or Error), the instruction performs no operation and raises ACE exception `ace_exc_privilege_violation`.
...
. If _Algorithm_, _AlgorithmPolicy_ or _SCProtection_ encoded in the supplied MDH (Forms B and C) is unsupported, then: 0.
...
the CR holds, which the flavour of its Configuration State identifies: the `Content[]` of a PI for a
...
The maximum lengths of AD and plaintext/ciphertext are implementation-specific.
The bound of `0x0010_0000` blocks above shall be replaced with the implementation-specific bound
sufficient to export and re-import a CR that holds any Algorithm
...
.. `saved_MDH` <- the *initial* MDH of ``K(i)``, that is the image with _State_ being a Complete State that software saved before issuing `#ace_cfg_exporting`, ``len_AD`` <- 1
.. ``AD[0]`` <- `saved_MDH`
