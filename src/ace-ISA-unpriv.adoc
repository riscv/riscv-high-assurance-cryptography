...
[horizontal]
[[ACE-SGR-clear-cr-content-error-state,SGR8]]
SGR8:: Upon transitioning to an Error State, the Content of a CR beyond the MDH is cleared:
...
[horizontal]
[[ACE-SGR-transition-to-from-unconfigured,SGR9]]
SGR9:: Transitions from _Unconfigured_ to an Error State result may only result from a configuration instructions.
...
[horizontal]
[[ACE-SGR-usage-cr-error-state,SGR13]]
...
[horizontal]
[[ACE-SGR-usage-cr-config-state,SGR15]]
SGR15:: Usage of a CR in a Configuration State, and cloning from it, are not allowed:
...
[horizontal]
[[ACE-SGR-ace-load-mv-allowed,SGR17]]
...
[horizontal]
[[ACE-SGR-ace-store-mv-allowed,SGR18]]
...
[horizontal]
[[ACE-GR-GPR-indirect, GR1]]
GR1:: _Use of a GPR as indirect CR index:_
...
[horizontal]
[[ACE-GR-multiple-GPRs,GR2]]
GR2:: _Use of multiple GPRs for longer inputs:_ A 64-bit operand is one GPR on RV64 and encoded in a pair `(X[s+1],X[s])` as `X[s+1] @ X[s]` on RV32,
...
[horizontal]
[[ACE-GR-opening-cr-when,GR6]]
GR6::
_When a CR can be opened for import or export:_
...
[horizontal]
[[ACE-IRR-uninterruptible-instructions,IRR1]]
IRR1:: Instructions `ace.mgmt`, `ace.getmd*`, `ace.restrict*`, `ace.clone`, non-vector `ace.mv`, and `ace.size` are _uninterruptible_: each either completes or has no architecturally visible effect, writing no destination vector register, GPR, CR byte, ACEIOBUF byte, or memory location.
...
[horizontal]
[[ACE-IRR-resumability-options,IRR4]]
IRR4:: In general, the following behaviors are allowed when an interrupt or exception handler accesses an input or output of a preempted operation, the associated CR, or any ACE state that may be affected by the operation:
...
[horizontal]
[[ACE-IRR-conflicting-access-rule,IRR5]]
IRR5:: All the approaches in Rule <<ACE-IRR-resumability-options>> are subject to one rule: Before the hart performs a _conflicting access_, the in-flight operation must either have completed or have been precisely halted with `acestart` set per <<ACE-Memory-Model>>. A conflicting access is any of the following:
+
--
...
[horizontal]
[[ACE-IRR-which-resumable-instructions,IRR6]]
...
[horizontal]
[[ACE-IRR-algorithms-specific,IRR9]]
IRR9:: Algorithm-specific resumable long-running operations that transfer no data
...
//
| 5.b
| `Content2[]`: Implementation-Specific _Auxiliary Data Section_ (ADS)
| A multiple of 128 bits, but otherwise implementation-Specific. Given by _AuxDataLen_.
