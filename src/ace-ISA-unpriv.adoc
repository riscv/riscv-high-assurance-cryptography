...
ACE instructions with vector operands that use `acestart` for resumption will automatically update `vstart` accordingly in sync with `acestart`.
...
[[ACE-unpredictable]]
Wherever this specification declares a result UNPREDICTABLE, the values written to
software-visible destinations are UNSPECIFIED, but they must be a function only of architectural
state that the executing context is otherwise permitted to observe.
In particular, the Content of a CC, the CSK, and the Locality Secrets must not be disclosed, in
whole or in part, beyond what a permitted instruction sequence could already produce.
The CR is left either in a state reachable by some legal instruction sequence, or in Error State
_Invalid_.
...
when `acestart` {ge} `aceiobuftop` (and `aceiobuflen` nonzero) will result in no operation being
performed, and `acestart` is left unchanged: the operand window [`acestart`, `aceiobuftop`) is
empty. `acestart` is never clamped to `aceiobuftop` (<<ACE-CSR-acestart>>).
...
With instructions that take their input from the ACEIOBUF or write their outputs to it, `acestart` is *not* clamped when the instruction is issued.
If `acestart` {ge} `aceiobuftop`, the operand window [`acestart`, `aceiobuftop`) is empty: the instruction performs no operation, writes nothing, causes no state transition, and leaves `acestart` unchanged.
This is the same rule that `ace.input` and `ace.output` state for their own transfers (<<ACE-instruction-input>>, <<ACE-instruction-output>>).
...
The outcome is therefore deterministic rather than UNPREDICTABLE.
...
{empty}
(((CSR, `managedcr`)))
`managedcr` is an XLEN-bit RW CSR, but currently only 6 bits are used.
...
The value of `managedcr` out of ACE unit reset is 32.
...
[IMPORTANT]
====
The `custom-0`, `custom-1`, and `custom-2` opcodes used throughout this section are placeholders
...
reserved value raises illegal-instruction exception. This can be used to emulate
future functionality. Currently the handler will simply transition
the target CR to transition to Error State _Invalid_.
+
...
[IMPORTANT]
====
Completing a management _process_ under preemption is a responsibility of the software stack, not of
the architecture: the architecture guarantees forward progress for each individual instruction, but
a management process spans several of them and any of its constituent CRs may legally be cleared
between them.
A handler that unconditionally clears partially configured or exported CRs at every context switch
will therefore prevent a long management process from ever completing.
An SCC can be several kilobytes — an ML-DSA-87 Serialized Context is 12496 bytes — plus up to
256 KiB of implementation-specific data, so this is reachable under ordinary preemption rates.
...
. If `Zklio` is available,:`aceiobuflen` = 0 (i.e., the ACEIOBUF unconfigured), `aceiobuftop` = 0, `acestart` = 0;
. The value of `managedcr` is 32.
...
For multi-block `ace.exec`, resumption from `acestart` is mandatory once
per-block state effects have been committed. The interruption points are defined by block boundaries
...
result is UNPREDICTABLE, in the bounded sense of <<ACE-unpredictable>>, when the input and output
overlap (i.e., they are actually the same vector register, vector register group, or the ACEIOBUF),
since the restarted operation would re-read data it has already overwritten.
...
A context-switch handler that itself issues management operations — to save or restore CRs — is such
an interleaving: the per-hart `SIV`, `SIV2` and `IMPQUAL` storage belonging to an interrupted
management process does not survive the next `ace.mgmt` with `#ace_CR_provision_start`,
`#ace_CR_import_start` or `#ace_CR_export_start` on that hart.
A handler that has to run a management operation of its own must therefore either complete or
abandon the interrupted process first, rather than expect to resume it afterwards.
...
. A domain separation bit, `sep`, has been added to distinguish the two segments of the content that are sealed in a SCC: the fixed architecture-defined content (`sep` = 0) and the optional implementation-dependent content (`sep` = 1). This reduces SIV strength by 1 bit with respect to RFC 8452.
...
The lengths of AD and plaintext/ciphertext must be replaced with implementation-specific values.
