...
The Content[] is further partitioned into two parts: Content1[] contains the architecturally defined content of the CR, and Content2[] contains the any implementation-defined content. The structure of Content1[] is given in <<ACE-Algorithms>> for each Algorithm defined in this specification.
...
. An array of 32 128-bit blocks, contains the MDH of each CR.
. An implementation-defined amount of memory that can be dynamically allocated to the various registers, corresponding to Content[] while provisioning and
...
. An integer register (CSR `acemanagedcr`, cf. <<ACE-CSR-acemanagedcr>>), which holds the index of the CR being currently managed, or 32 if no CR is currently managed.
. Three 128-bit registers (CSR pairs `acesivl` and `acesivh`,
`acesiv2l` and `acesiv2h`, and `aceiql` and `aceiqh`), which hold authentication data (the SIV, IMPQUAL, and SIV2) of the CR being currently managed.
...
This prevents concurrent management operations in the same code: starting a second operation can overwrite SIV, IMPQUAL, and SIV2 storage, destroying in-flight values. Therefore, the CSRs holding these values must also be saved upon context save and restored upon context restore.
...
[NOTE]
====
To reduce payload size, the AES-GCM-SIV-derived construction used for SCC import and export omits the nonce. It relies instead on AES-GCM-SIV's resistance to nonce reuse and misuse. The construction remains secure for up to 2^64^ 128-bit blocks observed by an adversary.
