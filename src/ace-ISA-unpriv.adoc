...
| [45:32] |  14   | _AdditionalData_   | If non-zero, indicates an additional data segment appended to Content. Length is _AdditionalData_ × 8 bytes (up to 128 KiB).
...
If an algorithm has 16 or fewer states (including the three fixed ones), the current state may be stored in the _StateNumber_ field. For algorithms with more than 16 states, additional algorithm-specific state information is stored in Content; in this case, _StateNumber_ shall still be set to _initial_ (0), _invalid_ (14), or _completed_ (15) if in the corresponding state, and to some other value (e.g., 1) otherwise.
...
Currently, up to three Localities can be active at once: one from the HW Binding Group, one of either PhysBootSecret or VirtBootSecret, and one from the SW Filter Group. During SCC export and import, the corresponding Locality Secrets are prepended, in ascending index order, to the input of the AEAD tag computation. If the _Locality_ field is zero, no Locality Secrets are prepended.
...
| 0xXXX   | URW       | `acehtrapmask`   | [.red]#Discuss:# Mask that determines which ACE exceptions horizontally trap
| 0xXXX   | URW       | `acehtrapvector` | [.red]#Discuss:# Address of the horizontal ACE trap handler
|===
...
* Invalid Metadata in `ace.prov` or `ace.import` (other than `ace_err_unimplemented`), such as, for instance: Nonzero bits in reserved fields; Unsupported _Locality_ value; and Nonzero _StateNumber_ in a PI.
...
{empty}
(((CSR, `acestart`)))
`acestart` is an XLEN-bit RW CSR that tracks the progress of interruptible ACE instructions: `ace.prov`, `ace.import`, `ace.export`, `ace.input`, and `ace.output`.
`acestart` also applies to `ace.exec` when processing a multi-block vector operand (`IOLEN` is a multiple of the algorithm's basic processing unit `b`): `ace.exec` operates iteratively and may be interrupted between blocks.
Only these instructions honor a nonzero `acestart`; all other ACE instructions ignore it and are restarted, with `acestart` also being cleared.

Unlike `vstart`, which tracks element indices, `acestart` records a byte offset, reflecting the number of bytes processed so far. Hardware typically writes this register on interruption. Software may write zero to force a full restart and may save and restore the register across context switches.
...
The semantics of `acestart` for these operations are described in <<ACE-provisioning-export-import>>.
...
are determined solely by the _Algorithm_, _KeyType_, and _DirectionPolicy_ fields in the Metadata.
...
function RFC8452_KeyDeriv(key : bits(256)) = {
  struct {
    AESE256(key, (zeros(125) @ 0b101))[63:0] @
    AESE256(key, (zeros(125) @ 0b100))[63:0] @
    AESE256(key, (zeros(125) @ 0b011))[63:0] @
    AESE256(key, (zeros(125) @ 0b010))[63:0],
    AESE256(key, (zeros(125) @ 0b001))[63:0] @
    AESE256(key, (zeros(125) @ 0b000))[63:0]
  }
}
----
...
DirectionPolicy enables signature generation (bit 12) and verification (bit 13), where all four combinations are admissible.
...
.3+|   i   .3+| 2           | Bit 0: `HasGenerator`  | Bit 1: `HasScalar`
                           <| Bit 2: `HasPublicKey` <| Bit 3: `HasHash`
                           <| Bit 4: `HasSignature` <| Bit 5: `HasRandomNumber`
|   ii  | 2               2+| `Progress`/`input_base`
|  iii  | 2               2+| `block_base`
|  iv   | 2               2+| Padding
|  v    | 2``b``/8        2+| `Generator`, X coordinate
|  vi   | 2``b``/8        2+| `Generator`, Y coordinate
|  vii  | `b`/8           2+| `Scalar`: `b`-bit string. Can be the private key.
|===
...
|===
|  Pos. | Size in Bytes                             | Field
| viii  | If `mh`._AdditionalData_ != 0: 8, else: 0 |  `acemarchid`
...
If _AdditionalData_ > `varlen`, Position xv contains the microarchitectural dependent part.
--
+
...
* The format of the additional data is implementation defined. The data itself is saved at Position xvi in the content, and its length is included in the _AdditionalData_ field of the Metadata Header.
 ...
 *** _point_mul_ only if `HasGenerator` amd `HasScalar`;
 ...
 *** Otherwise:
  ...
  . `block_base` <- 0
  . A value for `Generator` is loaded, using one or more Form B `ace.exec` operations, each one increasing `block_base` by the amount of data provided by `ace.exec`.
  . Once `block_base` has reached the length of the generator point (excess data in the last `ace.exec` is ignored):
 ...
 ** To transition to any of the States _set_scalar_, _set_hash_, _set_publickey_, or _set_signature_ a Form A `ace.state` is used.
 ...
 . `block_base` <- 0, and a value for Field _X_ is loaded, using one o more Form B `ace.exec` operations (similarly to `Generator`).
 . Once the values `block_base` has reached the length of field _X_  (excess data in the last `ace.exec` is ignored):
 ...
 ** Upon returning to State _initial_:
 ...
 This is determined by Bits 0, 1, and 2 of `Xs` as follows: If Bit 0, 1, resp., 2 is set, then `Generator`, `Scalar`, resp., `PublicKey` is erased, and the corresponding Boolean flags `HasGenerator`, `HasScalar`, resp., `HasPublicKey` is unset as well.
