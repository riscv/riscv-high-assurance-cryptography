...
[[ACE-size-functions]]
==== Size Functions (Notation)

_This section describes the size functions used throughout the specification. It provides definitions for other normative sections._
...
Provisioning, export, and import operations are interruptible only before any memory read or write they need to perform.
...
An export instruction may resume only if the CR has not been modified before returning to the interrupted control flow.

The implementation will also restart any of these operations if the internal state of the CR is not in a valid interrupted state.

The initial cryptographic phase of `ace.export` can be interruptible or uninterruptible.
In the second case, even if this phase is executed concurrently with other operations, an interrupt handler may need to wait for it to complete before proceeding with any access to the CR. This could become a performance bottleneck or cause timing issues. To address this, we could allow the SIV computation to be interrupted and use odd values in `acestart` to denote this. This is described after the description of the `POLYVAL` function <<ACE-SCC-POLYVAL>>.
...
The SIV is the encryption of the computation of this AXU hash on the concatenation of the associated data–namely, the Localities and the Metadata zero-padded to 128 bits–and the Content of the CR.
...
function POLYVAL(&tmp : bits(128), auth_key : bits(128), blocks[0 .. n-1] : bits(128), last_block_fractional : bool) = {
  tmp ← zeros(128);
  local i : int;
...
During SCC export and import, the Locality Secrets
associated with the Localities encoded in the _Locality_ field of the Metadata
are prepended, in ascending index order, to the input of the AEAD tag computation, before the metadata.
...
If an attempt is made to resume an export operation while this flag is cleared, it is restarted instead.
...
function ace.export(&CR, M[]) = {
  ...
  CR.exporting : bool;         // Architecturally "hidden" per-CR ACE unit state
  // TBD: how to reset "exporting" automatically? What operations reset it?
  ...
  // NOTE: {enc_key, auth_key} ← RFC8452_KeyDeriv(CSK);

  // per hart at any given time, which means that there cannot be two interrupted
  // export or import operations.

  // exporting is always false except during export.
  CR.exporting ← true;

  // CR.content[] includes the optional extra content at the end
  ...
  POLYVAL(&tmp, auth_key, L[num_localities*128-1:0] @ zeros(64) @ mh @ CR.content[], last_block_fractional);
...
* Af the beginning of the import operation, the hardware saves the `StateNumber` value in the metadata and sets
`mh.StateNumber <- ace_state_invalid`. At the end of the import operation, `mh.StateNumber` is set to the correct value.
