...
|===
|  24    | `ace_state_unsupported`         | _Not_Impl_        | Not implemented algorithm.
...
| `ace_exc_expired`                       | Transition to Error State `ace_state_expired`.
...
. Any instruction with an ACE opcode may raise an illegal-instruction exception for the purpose of emulation, or if certain fields are have invalid encodings in sub-fields (such as `v` = `h` = 1 in `ace.restrict`), or if the instruction belongs to an unimplemented (sub)extension.
 ...
 . An instruction Form that the Algorithm bound to the CR does not accept in its current state, see <<ACE-usage-input-output>> and <<ACE-algorithms>>.
--
...
The row's CR-state effects apply unchanged.
...
// Unlike ordinary RISC-V exceptions, an ACE exception ACE outcome _ commit the CR-state effects
//   listed in its row before the trap is taken; re-execution of the same
//   instruction therefore observes the updated CR state. This deviation is
//   deliberate and implementations must not suppress these effects.
