...
Upon entering an Error State, the CR's contents are cleared as per Rule *SGR8* in <<ACE-algorithm-state-rules>>.
...
==== ace.size.ex/ace.size.im/ace.avail

WARNING: complete description.

_Synopsis_::
Return the total size of a memory buffer that would be needed to store the data of a PI, CR, or SCC in bytes.
`ace.size.ex` returns 0 in case of error (such as unsupported Algorithm or invalid MDH).
...
[upperalpha]
. `ace.size.ex/ace.size.im Xd, Ks|K{Xs}` - Size from a context.
. `ace.size.ex/ace.size.im Xd, Xs` - Size from MDH[63:0] in one (RV64) or two (RV32) GPRs.
. `ace.size.ex/ace.size.im Xd, Vs` - Size from MDH in one vector register.
--
...
For `ace.size.ex`, `q` is `0`, and for `ace.size.im`, `q` is `1`.
Bit `q` is `0` if we are evaluating the size for export (i.e, total memory size), or `1` for import (only the `Content[]` size).
--
...
The instruction also returns `0`: for Form A when the CR is unconfigured,
and for Forms B–D when the _Algorithm_, _AlgorithmPolicy_ or _SCProtection_ encoded in the supplied MDH is
