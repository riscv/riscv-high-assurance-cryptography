...
For each block transferred by `ace.mv`, `acestart` is advanced by 16 bytes.
...
Any write, resp., read, past the serialized length of `Content[]` is ignored, resp., reads as zero.
