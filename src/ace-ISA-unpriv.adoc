...
All ACE state inaccessible in Debug unless an implementation-defined authenticated-debug signal is asserted.

On unauthenticated Debug entry:
all CRs must be zeroised and unconfigured;
the fields of all `*lcrstatus` shall be set to _Clean_;
all LST entries–and all CSRs setting the SW Localities–shall read as zero; and
the CSK is zeroised.

Therefore, in order to allow resumption of execution after unauthenticated Debug entry, Debug entry shall occur from a M-mode trap handler which also saves the entire ACE state before actually entering Debug mode.
