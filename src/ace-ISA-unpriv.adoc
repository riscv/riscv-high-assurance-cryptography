...
function ace.export(&CR, M[]) = {
  ...
  exporting : bool;            // Architecturally "hidden" per-CR ACE unit state
  // TBD: how to reset "exporting" automatically? What operations reset it?

  local SIV, tmp : bits(128);  // These variables are kept by the microarchitecture
  local enc_key : bits(256);   // internally. We need only one instance per hart,
  local auth_key : bits(128);  // and we assume only one export/import is performed
...
function ace.import(&CR, M[]) = {
  ...
  importing : bool;              // Architecturally "hidden" per-CR ACE unit state
