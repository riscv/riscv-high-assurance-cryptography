...
The trap is precise, i.e., all instructions before the one that causes the trap are completed, and the address of the instruction should be set in the User EPC (`uepc`) CSR.
[.red]#{warn} _If_# `[.red]#uepc#` [.red]#_is not defined, can we define it, however for this type of applications, a local variable can be used to keep track of the place where the error occurred_.# Since `aceerror` provides the cause, a CSR like `ucause` is not necessary.
