...
|   61     | `ace_cfg_exporting_pp` | The CR, which was in an Incomplete State, has been placed in exporting mode.
|   62     | `ace_cfg_importing_pp` | A CR exported while in an Incomplete State is being reimported.
...
are allowed in States _ace_cfg_provisioning_, _ace_cfg_importing_, and _ace_cfg_importing_pp_.

SGR19:: `ace.store` and `ace.mv` with a CR as source
are allowed in States _ace_cfg_exporting_, and _ace_cfg_exporting_pp_.
...
| `ace_cfg_provisioning`, `ace_cfg_importing`, `ace_cfg_importing_pp`
...
| `ace_cfg_exporting`, `ace_cfg_exporting_pp`
...
.. `ace_cfg_importing_pp` if the data to import corresponds to a CR in an Incomplete State.
. After `ace.mgmt`, with #immed7 = `ace_cfg_exporting`, the CR's _State_ becomes
.. `ace_cfg_exporting` if the CR is in a Valid State,
.. `ace_cfg_exporting_pp` if the CR being exported is in an Incomplete State.
...
... If `ml`._State_ is an Incomplete State, then CR._State_ is set to `ace_cfg_importing_pp`;
...
... The CR's _State_ is set to `ace_cfg_exporting_pp`.
...
* `ace_cfg_importing`, `ace_cfg_importing_pp`, resp.
* `ace_cfg_exporting`, `ace_cfg_exporting_pp`,
...
//   .. If the MDH._State_ is `ace_cfg_{importing|exporting}_pp`, then: 32 + size of a PI's `Content[]`.
...
// . If the MDH._State_ is an Incomplete State, then:
//   .. If the MDH._State_ is `ace_cfg_{importing|exporting}_pp`, then: size of a PI's `Content[]`.
//   .. If the MDH._State_ is `ace_cfg_{importing|exporting}`, then:
...
. If the MDH._State_ is an Incomplete State, then:
  ...
  .. If the MDH._State_ is `ace_cfg_{importing|exporting}_pp`, then: 32 * (1 - `q`) + size of a PI's `Content[]`.
  .. If the MDH._State_ is `ace_cfg_{importing|exporting}`, then:
...
B -> N1: Preemption.
  N1 -> N1: Save acestart.
  N1 -> N1: Start export: Save current <i>State;</i> <i>State</i> → ""ace_cfg_exporting_pp"".
  N1 -> N2: Preemption.
    N2 -> N2: Save acestart.
    N2 -> N2: Start export: Save current <i>State;</i> <i>State</i> → ""ace_cfg_exporting_pp"".
    N2 -> N2: Complete export: Restore saved <i>State</i> (""ace_cfg_exporting_pp"").
    ...
    N2 -> N2: Start import: Previous <i>State</i> same as in Step 8,\nhence, <i>State</i> → ""ace_cfg_importing_pp"".
    N2 -> N2: Complete import: <i>State</i> restored to ""ace_cfg_exporting_pp"".
