...
IRR7:: For block-iterated `ace.exec`, `ace.mv`, and `ace.derive`, fully processed blocks remain written, and `acestart` records their total byte count. For transfer instructions, bytes below `acestart` remain transferred at both endpoints. A partially executed instruction commits exactly the state that a shorter instruction performing only the completed operations would have committed. This includes the CR, `acestart`, `acemanagedcr`, `acesivl`/`acesivh`, `acesiv2l`/`acesiv2h`, and `aceiql`/`aceiqh`, `*lcrstatus`, and effective `ACES` fields, ensuring consistent context save, restore, and resumption. Implementations must honor `acestart` for `ace.exec` and `ace.derive`. Handlers must not reset it to zero to force a restart, as this could reapply state changes or corrupt data.
+
NOTE: `ace.derive` is included because it may modify the source CR's state, for example when squeezing a sponge, and may destructively modify the destination. Restarting could process state twice and overwrite data already consumed. Its interruption points are block boundaries and Algorithm-specific points, including long-running computations in <<ACE-ECC>> and <<ACE-PQC>>. If input and output overlap, the result may be unpredictable. However, it remains a function only of architectural state that the executing context is otherwise permitted to observe.
+
...
All implementations capable of resumption of these operation must also support their restart.
// +
