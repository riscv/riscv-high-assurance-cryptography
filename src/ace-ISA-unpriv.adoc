...
`Content[]` is further partitioned into two parts: `Content1[]` contains the architecturally defined content of the CR, and `Content2[]` contains the any implementation-defined content. The structure of `Content1[]` is given in <<ACE-Algorithms>> for each Algorithm defined in this specification.
...
To avoid unpredictable behavior, the KSK's own rules may automatically assign an appropriate _Locality_ (see <<ACE-Localities>>) during CR provisioning to bind the key to the system.
...
[[ACE-SGR-usage-policy,SGR15]] SGR15:: Any _usage_ instruction that would violate the Usage Control policies will not perform its operation, and raises ACE exception `ace_exc_privilege_violation`
...
. This value is then compared to the _ExpirationDate_ itself.
. If the value derived from the secure clock is not smaller than the _ExpirationDate_, the hardware transitions the CR's _State_ field to Error State `ace_state_expired`.
...
An `ace.restrict*` whose request would set a field to an unsupported or invalid value transitions the CR to Error State _Invalid_; fields it does not change are not re-examined.
`ace.size` returns 0 for unsupported or invalid Metadata.
...
The operand window is empty when `acestart` {ge} `aceiobuftop` for an ACEIOBUF operand (with `aceiobuflen` nonzero), or `acestart` {ge} `VL` {times} `SEW` / 8 for a vector operand.
...
+
NOTE: The hardware does not verify that `acestart` and `vstart` are consistent at the point of resumption. If software restores values that denote different points of progress for an instruction that requires them to match, the behavior is unpredictable: which bytes are processed is unspecified, though the result remains a function only of architectural state that the executing context may observe.
`ace.mv` keeps the two independent, since its `acestart` is a CR offset that accumulates across instructions while `vstart` tracks the vector register.
+
...
Form C or D in order to provide it in full; a Form A takes it from the ACEIOBUF (<<ACE-usage-input-output>>).
...
* If the source is _Unconfigured_, an illegal-instruction exception is raised. If the source is in a Configuration State, the instruction performs no operation and raises ACE exception `ace_exc_privilege_violation`; the CR that transitions under <<ACE-exception-error-correspondence>>, or that a handler sets to Error State _Privilege Violation_ (<<ACE-error-architecture>>), is the source.
...
An _exportable field_ is a field that the Algorithm computes during operation and designates as such; a field loaded from a PI or from the System Key Store, or written by an `ace.derive`, is never exportable.
...
. Validate `i`, `j` against the Algorithms' field tables, `length` against the
  block-size rule, and a secret-only source against the conditions above; on violation,
...
An _ACE unit reset_ of a hart resets that hart's ACE architectural state; in an ACE unit shared by several harts it affects only that hart's state, the shared Locality Secrets excepted.
...
.. restore the saved six authentication CSRs and
.. the CR indexed by the saved `acemanagedcr`, with `acemanagedcr` set at the saved value before the import.
