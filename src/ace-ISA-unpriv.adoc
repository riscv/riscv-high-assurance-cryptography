...
in the _AlgorithmUse_ field instead, as rule <<ACE-AGR-long-running-no-data>> of <<ACE-algorithms-other-rules>>
prescribes.
...
IRR2:: `ace.setst` is uninterruptible under every architectural condition defined by this specification. This rule may have implementation-specific overrides (see Rule <<ACE-IRR-algorithms-specific>> below).
