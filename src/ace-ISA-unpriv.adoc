...
| Implementation-Specific. Given in _AuxDataLen_.
...
| 1       | `MDH`, MDH   | 128 bits
| 2       | `Content1`   | See <<ACE-length-rule>>.
| 3       | `Content2`   | Implementation-Specific. Given in _AuxDataLen_.
|===
...
* If the management operation at the base level is an import, `ace.load` and `ace.mv` load SIV at `acestart` = 16 and, if _AuxDataLen_ {ne} 0, then load SIV2, resp., IMPQUAL at `acestart` = 32, resp. 48. Then, they load `Context1[]` and `Context2[]` block-wise.
The cryptographic verification is only performed at the management completion operation.

* If the management operation is an export, then the `ace.mgmt` to start the operation prepares the CR by encrypting it and generating SIV and, if _AuxDataLen_, also SIV2 and IMPQUAL.  Then,
`ace.load` and `ace.mv` store the SIV to memory if `acestart` = 16 and, if _AuxDataLen_ {ne} 0, they store SIV2, resp., IMPQUAL to memory when `acestart` = 32, resp. 48. After this, they store `Context1[]` and `Context2[]` block-wise.

The software, by saving metadata headers in its own state, maintains an implicit stack of the _State_ values, with the top value corresponding to the previous nesting level. The `ace.mgmt` completion operation restores the previous nesting level from the stack. This way, cryptographic operations can be performed only at the outermost level.  Concluding a base-level import or an export early will not authenticate, and therefore they cannot be used to leak plaintext at an outer nesting level. The only way plaintext is allowed to be exported is if the outermost operation itself was a provisioning operation, in which case the plaintext was already available to software.  In order to properly track `ace.load`, `ace.store`, and `ace.mv`, the CSR `acestart` must be saved and restored at interruption and before control return. This holds also for SIV, SIV2 and IMPQUAL. Since software may load and set up these values before issuing `ace.mgmt`, these CSRs must be saved and restored unconditionally.
