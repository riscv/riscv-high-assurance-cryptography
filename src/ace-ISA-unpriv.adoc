...
//  * `ace.mgmt` (<<ACE-instruction-mgmt>>) transitions a CR between states of its management lifecycle.
...
performs (<<ACE-instruction-mgmt>>) detects an accidentally interleaved management operation but
...
. Use `ace.mgmt` (<<ACE-instruction-mgmt>>) to prepare a CR to be provisioned, check the validity of MDH[63:0], and load the entire MDH in the CR.
...
// See the non-normative note at the end of <<ACE-instruction-mgmt>>,
...
NOTE: Although `ace.mgmt` is uninterruptible, the management processes bracketed by its instructions remain interruptible between constituent instructions (<<ACE-instruction-mgmt>>).
