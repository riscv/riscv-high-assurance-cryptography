...
SGR4:: In States _Success_ and _Failure_, `ace.setst` is only permitted to transition to _Ready_, to _Unconfigured_, or to an explicitly given Error State.
...
SGR5:: States _Success_ and _Failure_ cannot be set directly using `ace.setst`.

SGR6:: A transition from any valid state to _Ready_ using instruction `ace.setst` is always permitted, unless the Algorithm explicitly forbids it.
// +
...
SGR7:: The CR state after `ace.setst` completes does not always match the instruction's immediate operand.
...
[[ACE-SGR-clear-cr-content-error-state,SGR8]] SGR8:: Upon transitioning to an Error State, the Content of a CR beyond the MDH is cleared:
...
[[ACE-SGR-transition-to-from-unconfigured,SGR9]] SGR9:: Transitions from _Unconfigured_ to an Error State result may only result from a configuration instructions.
A usage instruction in State _Unconfigured_ raises an illegal-instruction exception.
Transitions from an Error State to _Unconfigured_ result from explicitly clearing the CR.

SGR10:: The MDH of a CR in the _Unconfigured_ State can be read using `ace.getmd*` (<<ACE-instruction-getmd>>), returning all zero values.

SGR11:: Exporting a CR in an Error State exports only the MDH (all fields and values), and requires no
`ace.mgmt`:
...
SGR12:: If a CR is in any Error State, it can still be reconfigured, cloned, or cleared;
besides for clearing, `ace.setst` can only be used to change the Error State.

[[ACE-SGR-usage-cr-error-state,SGR13]] SGR13:: Attempts to _use_ a CR in any Error State will not perform any operation, and in particular they leave the _State_ field unchanged.
...
[[ACE-SGR-usage-cr-config-state,SGR15]] SGR15:: Usage of a CR in a Configuration State, and cloning from it, are not allowed:
...
SGR16:: Instructions `ace.size`, `ace.getmd*`, `ace.getst`, `ace.setst` with the 7-bit immediate equal to `ace_state_unconfigured` or an
...
[[ACE-SGR-ace-load-mv-allowed,SGR17]] SGR17:: `ace.load` and `ace.mv` with a CR as destination
...
[[ACE-SGR-ace-store-mv-allowed,SGR18]] SGR18:: `ace.store` and `ace.mv` with a CR as source
