...
Let `size` be the size of `Content[]` in a PI or of `Content1[] @ Content2[]` in an SCC.
...
the first 8 bytes of the MDH.
Let `imglen` be the serialized CR image length in bytes excluding the MDH, and let `end` =
