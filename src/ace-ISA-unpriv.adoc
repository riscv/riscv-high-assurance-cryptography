...
This allows several small cores to share the same ACE unit, each with its own per-hard state, while minimizing the hardware footprint.
...
|   [95:80] |  16  | _AlgorithmUse_    | May be used by Algorithms, and when not by their and implementations.
...
SGR16:: Usage and cloning of a CR in a Configuration State is not allowed:
...
+
An instruction using one CR carries a selector bit `r`, at the position shown in its encoding diagram. Namely, `r` = 0 when the CR index is the 5-bit immediate held in that field, and `r` = 1 when it is the value held in the GPR that field names. The only exceptions to this encoding rule are `ace.load` and `ace.store`, which use a different value in `func3` for this purpose, since all other bits are used by immediates and register encodings.
...
The behavior of `ace.mv` with `ace.load` or `ace.store` in the same code snippet is not defined.
It may result in corrupted data inside a CR that may not properly authenticate.
...
+
--
`ace.getmd` is an R-type instruction and its encoding shares opcode and `func3` with `ace.exec`, but `funct2` is set to `0b11`.
...
If there is no sufficient CRF capacity to create a clone of the source CE, the instruction performs no operation and raises ACE exception `ace_exc_out_of_memory`.
...
. If Algorithm_, _AlgorithmPolicy_ or _SCProtection_ encoded in the supplied MDH (Forms B-D) is unsupported, then : 0.
...
IRR2:: `ace.setst` is uninterruptible under every architectural condition defined by this specification. This rule may have implementation-specific overrides to this rule (see Rule IRR9 below).
...
* An access to a CR used by the operation, including the the issue of another ACE instruction involving a CR used by the operation.
...
Implementations capable resuming these operation also support their restart.
// +
...
the corresponding `SIV`, and if _AuxDataLen_ {ge}, also `SIV2` and `IMPQUAL`, are configured.

During export of a CR to a SCC, the initial `ace.mgmt` operation encrypts the contents and generates `SIV` and, if _AuxDataLen_, also `SIV2` and `IMPQUAL`. The contents remain encrypted in the CR until the completion operation either restores the clear contents from an internal copy or re-decrypts and authenticates them. The requirements for the latter are the same as for completing an import.
...
`ace.mgmt Kd|K{Xd}, #ace_cfg_management_end` decrypts and authenticates the SCC by the following algorithm.
