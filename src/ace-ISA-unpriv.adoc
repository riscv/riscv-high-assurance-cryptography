...
|   61     | `ace_cfgst_exporting_pp` | A partially provisioned CR has been placed in exporting mode.
|   62     | `ace_cfgst_importing_pp` | A partially provisioned CR which has been exported is being reimported.
...
SGR10:: Exporting a CR in an Error State exports only the MDH (all fields and values).
...
are allowed in States _ace_cfgst_provisioning_, _ace_cfgst_importing_, and _ace_cfgst_importing_pp_.

SGR18:: `ace.store` and `ace.mv` with a CR as source
are allowed in States _ace_cfgst_exporting_, and _ace_cfgst_exporting_pp_.
...
Management operations, such as exporting, importing, clearing, cloning CRs, and getting their size, do not perform this check: none of them is one of the evaluation points listed above. Consequently, the transition to Error State `ace_state_expired`is only triggered when an expired CR is being actively used, while still permitting expired contexts to be cleanly evicted, saved, or unconfigured.
...
| `ace_cfgst_provisioning`, `ace_cfgst_importing`, `ace_cfgst_importing_pp`
...
| `ace_cfgst_exporting`, `ace_cfgst_exporting_pp`
...
. After `ace.mgmt`, with #immed7 = `ace_cfgst_importing`, the CR's _State_ becomes
.. `ace_cfgst_importing` if the data to import corresponds to a CR in a Valid State,
.. `ace_cfgst_importing_pp` if the data to import correspond to a partially provisioned CR.
. After `ace.mgmt`, with #immed7 = `ace_cfgst_exporting`, the CR's _State_ becomes
.. `ace_cfgst_exporting` if the CR is is a Valid State,
.. `ace_cfgst_exporting_pp` if the CR being exported was a partially provisioned one.
...
.. If `ml`._State_ is an Error State, then the CR is immediately fully configured in the Error State, with
    ...
    This works with both `#immed7` = `ace_cfgst_provisioning` and `#immed7` = `ace_cfgst_importing`.
    No `ace.mgmt` with `#immed7` = `ace_management_end` is needed, and in fact it will be an illegal instruction.
.. If provisioning: CR._State_ is set to `ace_cfgst_provisioning`;
.. If importing: CR._State_ depending on `ml`._State_:
... If `ml`._State_ is a Valid State, then CR._State_ is set to `ace_cfgst_importing`;
... If `ml`._State_ = `ace_cfgst_exporting_pp`, then CR._State_ is set to `ace_cfgst_importing_pp`;
...
... The CR's _State_ is set to `ace_cfgst_exporting_pp`.
...
+
--
`ace.mgmt` with immediate `ace_CR_management_end` reads first the MDH of the target CR.
Depending on whether MDH._State_ is

* `ace_cfgst_provisioning`,
* `ace_cfgst_importing`, `ace_cfgst_importing_pp`, resp.
* `ace_cfgst_exporting`, `ace_cfgst_exporting_pp`,
...
If either the source or destination CR are not in a Complete State (Valid or Error), the instruction performs no operation and raises ACE exception `ace_exc_privilege_violation`.
--
...
//   .. If the MDH._State_ is `ace_cfgst_{importing|exporting}_pp`, then: 32 + size of a PI's `Content[]`.
//   .. If the MDH._State_ is `ace_cfgst_{importing|exporting}`, then:
...
// . If the MDH._State_ is an Incomplete State, then:
//   .. If the MDH._State_ is `ace_cfgst_{importing|exporting}_pp`, then: size of a PI's `Content[]`.
//   .. If the MDH._State_ is `ace_cfgst_{importing|exporting}`, then:
...
. If the MDH._State_ is an Incomplete State, then:
  .. If the MDH._State_ is `ace_cfgst_{importing|exporting}_pp`, then: 32 * (1 - `q`) + size of a PI's `Content[]`.
  .. If the MDH._State_ is `ace_cfgst_{importing|exporting}`, then:
...
IRR2:: `ace.setst` is uninterruptible under every architectural condition defined by this specification. There may be implementation-specific overrides to this rule (see Rule IRR9 below).
...
B -> B: Start management: <i>State</i> changes to one of ""ace_cfgst_X""\nwith X = ""provisioning"", ""importing"", or ""exporting"".
B -> N1: Preemption.
  N1 -> N1: Save acestart.
  N1 -> N1: Start export: Save current <i>State;</i> <i>State</i> → ""ace_cfgst_exporting_pp"".
  N1 -> N2: Preemption.
    N2 -> N2: Save acestart.
    N2 -> N2: Start export: Save current <i>State;</i> <i>State</i> → ""ace_cfgst_exporting_pp"".
    N2 -> N2: Complete export: Restore saved <i>State</i> (""ace_cfgst_exporting_pp"").
    ...
    N2 -> N2: Start import: Previous <i>State</i> same as in Step 8,\nhence, <i>State</i> → ""ace_cfgst_importing_pp"".
    N2 -> N2: Complete import: <i>State</i> restored to ""ace_cfgst_exporting_pp"".
