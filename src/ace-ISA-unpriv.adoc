...
[float="center",align="center",cols="^7%,<30%,<63%",options="header"]
|===
...
|   59     | `ace_cfg_exporting_partial_pi` | The CR, whose base operation is a provisioning, has been placed in exporting mode; its image is PI-shaped.
|   60     | `ace_cfg_importing_partial_pi` | Such an image is being reimported.
|   61     | `ace_cfg_exporting_partial_scc` | The CR, whose base operation is the import or the export of an SCC, has been placed in exporting mode; its image is SCC-shaped.
|   62     | `ace_cfg_importing_partial_scc` | Such an image is being reimported.
|   63     | (Reserved)               | Reserved.
|===

(((ACE, Algorithm, partial states)))
States 56–63, i.e., the Configuration States, are also called _Partial_:
...
The `_pi` and `_scc` flavours of the nested States record whether the image the CR holds is
PI-shaped or SCC-shaped, which follows from the base operation. They determine its length
(<<ACE-instruction-size>>), which could not otherwise be recovered once the nesting has overwritten
the _State_ that the base operation set.
...
Currently, the only such operation is encoded with the value 64, meaning `ace_cfg_clear_ads`, which clears any ADS so that the CR can be exported without it.
...
.. `ace_cfg_importing_partial_pi` or `ace_cfg_importing_partial_scc` if the data to import corresponds to a CR in a Partial State, according to the shape of the image.
. After `ace.mgmt`, with #immed7 = `ace_cfg_exporting`, the CR's _State_ becomes
.. `ace_cfg_exporting` if the CR is in a Valid State,
.. `ace_cfg_exporting_partial_pi` or `ace_cfg_exporting_partial_scc` if the CR being exported is in a Partial State, according to the shape of its image.
...
The first usage instruction that needs to access the ADS will then need to regenerate it.
--
...
It also does not depend on _StateExtension_, whose value is ignored in the
MDH of a PI and is replaced with zero by `ace.mgmt` upon provisioning,
nor on _SCProtection_.
...
`ace.mgmt` to complete the provisioning, which may include the computation of certain
...
A CR in an Error State produces no SCC: its exported image is the `MDH` alone, and Sections 2 to 5 do not exist for it.
...
[width="100%",cols="^8%,25%,67%",options="header"]
|===
