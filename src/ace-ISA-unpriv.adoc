...
A vector register (group) is seen as a bit string of length `VL` * `SEW` with `VL` = min(`AVL`,`VLMAX`).
...
If the SKID is the all-ones value, the key material is randomly generated when provisioning completes. It is then exported by value with _KeyType_ set to 0.
...
If _Zklexpire_ is not supported, the implementation rejects PIs and SCCs with a non-zero _ExpirationDate_.
...
It it not used to import metadata or authentication information, such as the MDH, `SIV`, `SIV2`, and `IMPQUAL`.
...
. If the MDH._State_ is a Valid State, then:
  ..  If _AuxDataLen_ < 2, then: 32 * (1 - `q`) + size of a PI's `Content[]`.
  ..  If _AuxDataLen_ {ge} 2, then: 32 * (1 - `q`) + size of a PI's `Content[]` + 16 * (_AuxDataLen_ - 2 * `q`).
. If the MDH._State_ is a Partial State, then:
  .. If the MDH._State_ is `ace_cfg_provisioning`, then: 16 * (1 - `q`) + size of a PI's `Content[]`.
  .. If the MDH._State_ is `ace_cfg_{importing|exporting}_partial`, then:
    ...  If _AuxDataLen_ < 2, then: 16 * (1 - `q`) + size of a PI's `Content[]`.
    ...  If _AuxDataLen_ {ge} 2, then: 16 * (1 - `q`) + size of a PI's `Content[]` + 16 * (_AuxDataLen_ - 2), since `IMPQUAL` and `SIV2` are held in the CSRs and not in the PCCC (<<ACE-PCCC>>).
  .. If the MDH._State_ is `ace_cfg_{importing|exporting}`, then:
    ...  If _AuxDataLen_ < 2, then: 32 * (1 - `q`) + size of a SCC's `Content1[]`.
    ...  If _AuxDataLen_ {ge} 2, then: 32 * (1 - `q`) + size of a SCC's `Content1[]` + 16 * (_AuxDataLen_ - 2 * `q`).
--
...
MDH of a PI and it replaced with zero by `ace.mgmt` upon provisioning,
...
`ace.mgmt` to complete the import which may include the computation of certain
