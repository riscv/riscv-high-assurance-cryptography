...
{empty}
(((CSR, `aceiobuftop`)))
`aceiobuftop` is an XLEN-bit URW CSR that marks the upper bound of the buffer range used by `ace.input` and `ace.output`. Together with `acestart`, it defines the active transfer window.
...
+
`ace.import`
//follows the interruptibility rules defined in <<ACE-CSR-acestart>> and <<ACE-provisioning-export-import>>.
...
function ace.prov(&CR, M[]) = {
  ...
  provisioning[CR] : bool;                 // "hidden" per-CR ACE unit state

  // PREEMPTIBLE WITH `acestart` IN [0 .. 7]
  mh ← M[0 .. 7]
  z ← PI_content_size(Metadata[31:0]);

  provisioning[CR] ← true;
...
[WARNING]
====
Do we need a mechanism to clear the `CR.provisioning/exporting/importing` flag upon context switch, to force a restart in this case?
Or would simply clearing the CR be sufficient, since it does?
====
...
function ace.export(&CR, M[]) = {
  mh : bits(64) = & CR.Metadata;           // By reference, from the CR
  external exporting[CR] : bool;           // "hidden" per-CR ACE unit state
  external CSK : bits(256);
  external enc_key : bits(256);            // RFC8452_KeyDeriv(CSK)
  external auth_key : bits(128);

  exporting[CR] ← true;                    // always false unless during export.
  ...
  M[0 .. 7]  ← mh;  // THIS MEMORY WRITE IS PREEMPTIBLE, WITH `acestart` FROM 0 TO 7
  M[8 .. 23] ← SIV; // THIS MEMORY WRITE IS PREEMPTIBLE, WITH `acestart` FROM 8 TO 23

  foreach(i from 0 to num_blocks - 1) {
    if ((i == num_blocks - 1) and last_block_fractional)
      // FOLLOWING MEMORY WRITE IS PREEMPTIBLE, WITH `acestart` FROM 24+i*16 TO 31+i*16
      M[24+i*16 .. 31+i*16] ← CR.content[i*16 .. 7+i*16]
               xor AESE(enc_key, 1 @ SIV[126:32] @ (SIV[31:0]+bin(i,32) % 2**32)[63:0];
    else
      // FOLLOWING MEMORY WRITE IS PREEMPTIBLE, WITH `acestart` FROM 24+i*16 TO 39+i*16
 ...
 ** If four bytes or more have been read, the value of `mh.StateNumber` in the SCC is known. In this case, the instruction may be restarted with `acestart` {ge} 4 recording the amount of data read.
...
function ace.import(&CR, M[]) = {
  ...
  external importing[CR] : bool;           // "hidden" per-CR ACE unit state
  ...
  // importing is always false except during import.
  importing[CR] ← true;

  // FOLLOWING MEMORY READ IS PREEMPTIBLE, WITH `acestart` FROM 0 TO 7
  ...
  // FOLLOWING MEMORY READ IS PREEMPTIBLE, WITH `acestart` FROM 8 TO 23
  SIV ← M[8 .. 23];
  ...
  foreach(i from 0 to num_blocks - 1) {
    if ((i == num_blocks - 1) and last_block_fractional) {
      // FOLLOWING MEMORY READ IS PREEMPTIBLE, WITH `acestart` FROM 24+i*16 TO 31+i*16
      content[i*16 .. 7+i*16] ← M[24+i*16 .. 31+i*16]
          xor AESE(enc_key, 1 @ SIV[126:32] @ (SIV[31:0]+bin(i,32) % 2**32)[63:0];
      tmp[63:0] ← tmp[63:0] xor content[i*16 .. 7+i*16];
    } else {
      // FOLLOWING MEMORY READ IS PREEMPTIBLE, WITH `acestart` FROM 24+i*16 TO 39+i*16
...
[NOTE]
====
`provisioning[CR]`, `importing[CR]` and `exporting[CR]` need not be distinct bits.
So, in theory, 32 bits would be sufficient.
Also, since only one of these bits is ever set at a time,
we actually only need a single bit and a 5-bit field for the CR number.

When privileged code sets `acestart` to the saved value (or leaves it
untouched, as when addressing a memory exception), the hardware needs
to check whether the `provisioning/importing/exporting[CR]` bit is set and the CR
number also matches. If not, the instruction is restarted anyway.

Microarchitectures may in theory support keeping track of several
interrupted import/export operations at the same time. This
would require keeping also copies of the `SIV` and `tmp` values,
i.e., 256 bits worth of information, and a few other small quantities,
per interrupted operation. Since we believe that interrupted import/export
operations are not very common, and would be significant only for very
large states, of which there would be very few in flight at any given
moment, we are not mandating this.  However, a single instance of
interrupted import or export must be architected to correctly address memory
exceptions.
====
