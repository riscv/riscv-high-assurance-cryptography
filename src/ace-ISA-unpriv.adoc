...
MMR3:: ACE memory instructions are supported only for idempotent memory.
If any byte of the transfer maps to a non-idempotent or side-effecting PMA region,
the instruction raises a load/store access-fault exception before any component access occurs.
