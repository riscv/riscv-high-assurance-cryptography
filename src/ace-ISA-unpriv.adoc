...
| `ace.getmdl`, `ace.getmdh` (and the pseudo-instructions `ace.getst`, `ace.getstx`)
| `Zkl` common
2+<| Available
...
+
--
The software must use Form B, resp. Form C, of `ace.mgmt` to pass along the value of `ml` saved when the import process was started, according to whether it is a 64-bit or a 128-bit value.
Only the _ConfigStatus_ subfield of `ml` is consumed; its remaining bits are
ignored. The MDH image authenticated as ``AD[0]`` is the one carried in the
SCC and already loaded into the CR, not the software-supplied `ml`.
...
+
--
Uses Form B with an auxiliary input in a GPR (or a GPR pair) carrying bits [63:0] of the MDH, or
Form C with an auxiliary input in a vector register carrying bits [127:0] of the MDH,
to restore the _ConfigStatus_ value saved by software.
...
[NOTE]
====
The rules governing `ace.mgmt` exist because any management procedure may be interrupted and resumed. System software is responsible for saving and restoring _ConfigStatus_, which keeps the CPU logic simple and supports arbitrarily many levels of nested virtualization.
...
`ace.mgmt` is uninterruptible as a single instruction; the management _processes_ it delimits are interruptible between their constituent instructions (<<ACE-instruction-manage>>). To bound the latency of the final `ace.mgmt` of an import, whose decryption and authentication work grows with the size of the SCC, implementations may interleave that work with the preceding load phase, as noted in <<ACE-management-code-snippets>>.
...
The external format of a PI is:

{fournbsp}{fournbsp} `MDH` {vvert} `Content[]`.

While it is being loaded into a CR, the loaded information is the same:

{fournbsp}{fournbsp} `MDH` {vvert} `Content[]`.

The external SCC format is:

{fournbsp}{fournbsp} `MDH` {vvert} `SIV` {vvert} `Content1~CT~[]` { {vvert} `IMPQUAL` {vvert} `SIV2` {vvert} `Content2~CT~[]` }.

where `CT` stands for `Ciphertext`.

While it is being loaded into a CR, the loaded information is shorter.

{fournbsp}{fournbsp} `MDH` {vvert} `Content1~CT~[]` { {vvert} `Content2~CT~[]` }

and `SIV`, `SIV2`, `IMPQUAL` are stored separately.

The architecturally observable behavior of the conversion between the encrypted format and

{fournbsp}{fournbsp} `MDH` {vvert} `Content1~PT~[]` { {vvert} `Content2~PT~[]` }

where `PT` stands for `Plaintext` is the following:

. If `acestart = 0`, it means that nothing has been converted.
. Then `acestart` counts the number of blocks converted. So, assuming `Content1[]` has `n` blocks and `Content2[]` has `m` block
`acestart` will be `n` after the conversion of `Content1[]` is complete,
`n + 1` after the conversion of the first block of `Content2[]` is complete,
and `n + m` after the conversion of `Content2[]` is complete.

This described the semantics of `acestart` when `ace.mgmt` it is used initiate the export of a SCC (i.e., the CR's _ConfigStatus_ is _ace_cfgst_complete_) after a conversion from internal formats to the external format (minus `SIV1`, `SIV2`, and `IMPQUAL`), and when `ace.mgmt` is used to initiate the import of a SCC.

In order to avoid leakage unencrypted data, when an `ace.mgmt` operation is preempted, following happens:

. if the exception or interrupt handler does not perform any ACE operation or access any ACE state, then the instruction will be resumed from the point of interruption.
. else, the pre-`ace.mgmt` content of the CR is restored from an internal backup and the instruction will be restarted.
...
NOTE: The algorithms of this section and of <<ACE-SCC-import>> are
independent of how the MDH crosses between software and the CR. The scalar
and vector variants of the management instructions differ only in transport:
with the scalar variant, MDH[127:64] travels as the first eight octets of the
`ace.load`/`ace.store` (or `ace.mv`) stream, with the vector variant it
travels in the auxiliary vector register — which is exactly what the
`acestart` values of 8, resp., 16 set at the start of the operation encode
(<<ACE-instruction-manage>>). In the import algorithm of <<ACE-SCC-import>>,
steps 1 to 6 summarize the effects of the import-start and load phases;
`#ace_CR_management_end` performs the remaining steps.
