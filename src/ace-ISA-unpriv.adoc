...
* The double circled states represent: the _Unconfigured_ State and the outermost Partial States during management operations; and the Partial States of management operations.
...
\begin{tikzpicture}[scale=1.1]
  ...
  \node[off state,text width=2cm,inner sep=2pt] at (-1.75,0)  (off)  {\small Unconfigured (and Partially Confi\smash{gured)}};
...
SGR4:: In States _Success_ and _Failure_, only the following ACE instructions are permitted: `ace.mgmt`, `ace.clear`, `ace.size`, `ace.setst`, `ace.getmd*`, `ace.getst`, `ace.clone`, `ace.restrict*`, and `ace.derive` where the source CR is in State _Success_ or _Failure_ and the source endpoint is one of its fields that can be exported, as described in the specification of the Algorithm.
...
SGR5:: In States _Success_ and _Failure_, `ace.setst` is only permitted to transition to _Ready_, to _Unconfigured_, or to an explicitly given Error State.
It is also permitted with `#immed7` = `ace_cfg_clear_ads` (<<ACE-instruction-clearads>>), which clears any ADS and leaves the _State_ unchanged.
...
besides for clearing, `ace.setst` can only be used to change the Error State.
...
Error State, `ace.clear`, and `ace.clear.all`, which are not usage-controlled, are also allowed
...
GR7::  Where a Form of an ACE instruction carries the whole 128-bit MDH in a vector register or group, that
...
GR8:: Instruction that access the ACEIOBUF, such as `ace.input` and `ace.output`, may be used only if the
ACEIOBUF is enabled and `aceiobuflen` is nonzero; otherwise ACE exception
`ace_exc_unconfigured_buffer` is raised.

GR9:: All encoding points under the ACE opcodes that this chapter does not assign are reserved, and raise an
...
Issuing `ace.state` or `ace.mgmt` with certain state values may require passing an auxiliary input to the operation, which may be a 64-bit or 128-bit operand passed in one or more GPRs or a vector (group) `Vs`. The vector (group) may be replaced by ACEIOBUF if `Zklio` is implemented.
...
A context-switch handler can always force a restart by clearing partially provisioned or imported CRs at context save and restore time.
...
_Synopsis_::
Return the total size of a memory buffer that would be needed to store the data of a PI, CR, or SCC in bytes.
...
An implementation realizes this in one of three ways: by completing the instruction before taking an asynchronous interrupt; where Rule <<ACE-IRR-long-running-no-data>> permits, by letting it complete in the background, subject to Rule <<ACE-IRR-conflicting-access-rule>> below; or by discarding its partial work before taking the trap and re-executing it upon return.
