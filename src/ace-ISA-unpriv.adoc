...
| `ace.setst` (and the pseudo-instructions `ace.clear`, `ace.clearall`, `ace.clearads`)
| `Zkl` common
| Forms A–C
...
| `ace.setst`                 | Yes             | No  | Yes, except as noted | Not usage-controlled when `#immed7` is `ace_state_unconfigured` — that is `ace.clear` and `ace.clearall` (<<ACE-instruction-clear>>) — or an Error State.
...
+
--
`ace.clearads` deletes any ADS from the target CR.
