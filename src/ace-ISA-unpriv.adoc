...
+
If the V extension register file is not implemented, only Form D is supported and `ace.input`, `ace.output` must be used to provide inputs and extract outputs from `ace.exec`, as described in <<ACE-usage-input-output>>.
...
|===
|Extension | Minimum version | Lifecycle state
|`Zlv` (Forms A-C)   |v1.0.0           |
|`Zlio` (Forms A-D)   |v1.0.0           |
|===
