...
vector register (group) of at least 128 bits, that is one of `Zvl128b`, `Zvl256b`, `Zvl512b` or
...
. In States _Success_ and _Failure_, only the following ACE instructions are permitted: `ace.mgmt`, `ace.load`, `ace.store`, `ace.clear`, `ace.size`, `ace.setst`, `ace.getmd*`, `ace.getst`, and `ace.derive` where the CR in State _Success_ or _Failure_ is the *source* of the transfer (described in <<ACE-instructions-detailed>>).
...
| `ace_exc_no_csk`               | The exception is raised as described; on an M-mode-only hart M-mode is both the only mode that can observe it and the only one that can establish a CSK.
...
| `ace_exc_unconfigured_buffer`  | When an ACE instruction is issued that uses the CR with the ACEIOBUF, and the latter is unconfigured or absent, the instruction performs no operation: nothing is read, nothing is written, and the CR undergoes no state transition. This case is exempt from the `ACELEN` granularity rule of <<ACE-generic-rules>>, which would otherwise invalidate the CR.
|===
...
+
--
Using any ACE instruction, or accessing an ACE CSR, while no CSK is active raises ACE exception
`ace_exc_no_csk` (<<ACE-CSK-requirements>>). This covers both a CSK that has never been
established and a `macecsk` group that is only partially written. Two groups are exempt: the
`macecsk` group (if present), and the read-only identification CSRs `acemvendorid`, `acemarchid`,
`acemimpid` and `acemaxiobuflen`, which remain readable so that firmware can identify the ACE unit
before establishing a CSK.
--
...
In any outcome other than the complete execution of the instruction, the latter writes no vector
register, GPR, CR byte, ACEIOBUF byte, or memory location. The exceptions are the instructions
that honor `acestart` (<<ACE-CSR-acestart>>) — `ace.exec`, `ace.derive`, `ace.load`, `ace.store`,
`ace.input` and `ace.output` — when they are precisely halted, or encounter an error, after making
partial progress. In that case the results of the completed prefix remain written, nothing of the
failing unit or of any later one is written, and `acestart` records the prefix-complete point:

* for a block-iterated `ace.exec` or `ace.derive`, the outputs of the `j` {ge} 1 fully processed
  blocks remain written and `acestart` holds their byte count;
* for `ace.load`, `ace.store`, `ace.input` and `ace.output`, the bytes at offsets below the new
  `acestart` remain transferred on both sides.

The error condition's effects on the CR state are unchanged.
...
The general rule, covering preemption as well as exceptions, is this: when an ACE instruction
performs only part of the operations it was asked for, the state committed is exactly the state
that would have been committed by a shorter instruction — one asking only for the operations
actually performed. This applies to every piece of architectural state the instruction touches:
the CR, `acestart`, `acemanagedcr`, `acesivl`/`acesivh`, `acesiv2l`/`acesiv2h`, `aceiql`/`aceiqh`,
the `*lcrstatus` fields and the `ACES` fields in effect are all left as that shorter instruction
would have left them. It is this rule that makes context save and restore, and resumption from
`acestart`, well defined.
...
`acestart` is always a byte count. Where an Algorithm defines a long-running operation whose
progress is not byte-structured — the point multiplications and signature operations of
<<ACE-ECC>> and <<ACE-PQC>>, for instance — that progress is recorded in the _AlgorithmUse_ field
in an implementation-defined way, not in `acestart`.
...
The value of `acemanagedcr` out of ACE unit reset is 32.
...
On RV32,
CSR `acesivlh` shadows bits [63:32] of `acesivl`, and
CSR `acesivhh` shadows bits [63:32] of `acesivh`,
to allow access to those bits.
...
On RV32,
CSR `acesiv2lh` shadows bits [63:32] of `acesiv2l`, and
CSR `acesiv2hh` shadows bits [63:32] of `acesiv2h`,
to allow access to those bits.
...
On RV32,
CSR `aceiqlh` shadows bits [63:32] of `aceiql`, and
CSR `aceiqhh` shadows bits [63:32] of `aceiqh`,
to allow access to those bits.
...
// for development and prototyping only. The RISC-V unprivileged ISA reserves the `custom-`__n__
// opcodes permanently for non-standard extensions, so they cannot carry a ratified extension. Final
// encodings will be allocated from standard opcode space by RVI during ratification; implementers
// must not treat the encodings in this document as a stable interface.
====
...
resumes; the correspondence between addresses and transfer offsets is given by each instruction.
...
.. Check whether `acemanagedcr` is equal to the CR's number or is equal to 32.
...
.. Write the CR's number to `acemanagedcr`.
.. `acestart` is cleared.
--
...
* `ace.getmd` copies bits [63:0] of the MDH of CR `Ks` or `K{Xs}` into GPR `Xd` and bits [127:64] of the MDH of the CR into GPR `X[d+1]`.
...
On RV32, if the destination register is a GPR, `d` must be even for `ace.getmdl`, which writes a
register pair, and a multiple of four for `ace.getmd`, which writes a quartet. Specifically:

* `ace.getmdl` copies bits [63:32] of the MDH of CR `Ks` or `K{Xs}` into GPR `X[d+1]` and bits [31:0] of the MDH of the CR into GPR `Xd`, atomically.
If `d` is not even, an illegal-instruction exception is raised.

* `ace.getmd` copies bits [127:96] of the MDH of CR `Ks` or `K{Xs}` into GPR `X[d+3]`, bits [95:64] of the MDH of the CR into GPR `X[d+2]`,
bits [63:32] of the MDH of the CR into GPR `X[d+1]`, and bits [31:0] of the MDH of the CR into GPR `Xd`, atomically.
If `d` is not a multiple of four, an illegal-instruction exception is raised.
...
The instruction is _not_ usage-controlled.
Every change it can make narrows the CC: policies are only removed, side-channel protection only strengthened, Localities only tightened, and an expiration date only brought forward.
...
| `00`   | secret field `i`          | secret field `j`
| `01`   | output of an `ace.exec`   | secret field `j` (`i` is zero)
| `10`   | secret field `i`          | input of an `ace.exec` (`j` is zero)
...
* On RV32, `s2` must be even, `X[s2]` = `i << 16 {vvert} j` and `X[s2+1]` = `length`;
  if `s2` is odd, an illegal-instruction exception is raised.

`length` is a number of *bytes*. The indices `i` and `j` select a configurable secret field of the
source, resp. destination, CC, numbered in the order the fields appear in the SCC of the Algorithm
in that CR. Fields configured by a System Key Identifier cannot be written, but they are still
counted for the purpose of these indices. An index that names no field of the corresponding
Algorithm causes that CR to transition to Error State _Invalid_.
...
`ace.derive` (<<ACE-instruction-derive>>) exists so that the output of one Algorithm can become the
input, or the key, of another without ever being visible to software. The four `Form` values of
<<ACE-derive-forms>> cover the four combinations of endpoint; the examples below show what each one
replaces.
...
_Ready_; the `ace.derive` then overwrites its secret field `j`. This is the shape used by
<<ACE-PQC-ML-KEM>>.

*Form `00` — secret into secret.* A secret field of one CC is copied into a secret field of
another, for instance to give two CCs with different _UsagePolicy_ or _Locality_ the same
underlying key. Both indices are significant here.
...
_Encoding_::
`ace.avail` is an alias for Form B of `ace.size`, as the latter returns 0 in case of error. On RV32 it therefore inherits Form B's register-pair operand: `s` must be even and MDH[63:0] is passed in `X[s+1] @ Xs`.
...
the Algorithm's current state. The latter invalidates the CR instead (<<ACE-generic-rules>>).
...
//
// . The granularity rule of <<ACE-generic-rules>> applies to `ACELEN` as computed above, whatever
...
Instructions `ace.mgmt`, `ace.getmd*`, `ace.restrict*`, `ace.clone`, `ace.mv`, and `ace.size` are
uninterruptible.

`ace.setst` is uninterruptible under every condition this specification defines: there is no
generic ground on which it may be interrupted. Where an Algorithm makes it perform a long-running
operation, an implementation may interrupt and resume it, and how it records progress is then
implementation-defined — the _AlgorithmUse_ field, as in <<ACE-ECC>> and <<ACE-PQC>>, rather than
`acestart`. Such an interruption must still satisfy the completion guarantee below: each execution
attempt either completes the operation or advances its recorded progress.
...
For multi-block `ace.exec`, and for `ace.derive`, resumption from `acestart` is mandatory once
per-block state effects have been committed. Every implementation honours `acestart` for these
two instructions; the freedom to restart instead is confined to the transfer instructions
`ace.load`, `ace.store`, `ace.input` and `ace.output`. `ace.derive` is included because its
effect on the destination may be destructive, so re-running it from the beginning would advance a
state machine twice or overwrite material already consumed. The interruption points are defined by block boundaries
as well as by Algorithm-specific interrupt points  (for instance
the long-running computations of <<ACE-ECC>> and <<ACE-PQC>>).
While handlers can set `acestart` to zero to force a restart of an `ace.exec` operation, the
result may be unpredictable (but still a function only of architectural
state that the executing context is otherwise permitted to observe)
when the input and output overlap,
since the restarted operation may re-process data it has already read or overwritten.
...
All six fields named above lie in bits [63:0] of the MDH, which is what allows a PI or
