...
* An instruction, `ace.mgmt`, to make this information available to the ACE unit and to convert the internal representation of CRs to an SCC (cf. <<ACE-instruction-mgmt>>).
...
Any attempt to use a CR whose _ConfigStatus_ is not `ace_cfgst_complete` will raise ACE exception `ace_exc_privilege_violation`, where certain instructions in this case are not considered _usage_ and thus can be issued, namely: `ace.mgmt`; `ace.load`,
  ...
  State (hence `ace.clear` and `ace.clearall`).
See <<ACE-error-architecture>> for the behavior when the Privileged Architecture is not implemented.
...
* At the dispatch and initiation of every usage instruction (every `ace.exec` and `ace.derive` instruction); and
...
If _AddDataLen_ > 0 (hence, for import only), `ace.load` will _not_ load data beyond the maximal admissible size allowed for the Algorithm encoded in the MDH. In case an ADS is provided, an implementation may decide to stop the transfer once, upon transferring the IMPQUAL block. In these two cases, SIV2 and, if _AddDataLen_ {ge} 2, the IMPQUAL are loaded in the hart-local ACE state.  This is necessary to allow the `ace.mgmt` instruction to properly process and finalize these conditions.
...
`ace.getmd{nbsp}Xd, Ks|K{Xs}`(((ACE, instruction, ace.getmd)))
`ace.getmdv{nbsp}{nbsp}Vd, Ks|K{Xs}`(((ACE, instruction, ace.getmdv)))
--
...
+
--
If the CR has _ConfigStatus_ = _Unconfigured_, these instructions return zero (which includes the value of the field _ConfigStatus_ = _Unconfigured_).
...
[NOTE]
====
A CR in an Error State has _AddDataLen_ {eq} 0 and empty `Content1[]`, so the procedure only exports 32 bytes.
====
