...
[NOTE]
====
The rules governing `ace.mgmt` are necessary because any management operation may be interrupted adn resumed. Saving and restoring _ConfigStatus_ is the responsibility of the system software. This keeps the CPU logic simple and supports arbitrarily many levels of nested virtualization.

These rules also allow an interrupted import to resume on a different hart, even a hart attached to a different ACE Unit implementation. The architecture ensures interoperability as long as the importing implementation supports the algorithm encoded in the SCC.

Software must ensure that an interrupted configuration or export can resume, while also supporting binary compatibility between implementations that support resumption and those that do not. Resumption is not required: Even where supported, an implementation may ignore `acestart` and restart the interrupted operation for any reason. A context switch handler can force a restart by clearing partially configured or exported CRs at context save and again at context restore. Software must support this possibility.
