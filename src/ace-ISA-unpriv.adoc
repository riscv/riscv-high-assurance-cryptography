...
[NOTE]
====
If we close an export operation prematurely, then we get an authentication error upon import (see next section).
If we close an import operation prematurely, then we get an authentication error.

If we forget to close any of the two operations, we also get errors because we cannot use the CR unless _ConfigStatus_ is `ace_cfgst_complete`.
====
