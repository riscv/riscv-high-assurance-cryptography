...
(cf. <<ACE-CSR-impl-ids>>). These 16 bytes are counted within the length recorded by the _AddDataLen_ field.
The importing implementation uses this value to determine whether the ADS can be used.
...
After an SCC is imported into a CR, this binding no longer applies; if a Locality Secret later changes, a newly exported SCC will be bound to the new value.
Software is responsible for not leaking CRs directly to unauthorized parties.
...
Currently, the only encoding is `ace.reset`.
+
An indirect index outside [0..31] also raises an illegal-instruction exception.
+
An instruction that names one CR carries a selector bit `r`, at the position shown in its encoding diagram. Namely, `r` = 0 when the CR index is the 5-bit immediate held in that field, and `r` = 1 when it is the value held in the GPR that field names.
...
GR4:: The _UsagePolicy_ of the MDH determines whether certain instruction can be issued, and sometimes when, in a given privilege mode.
...
Uninterruptible Instructions::: {empty}
+
--
[horizontal]
IRR1:: Instructions `ace.mgmt`, `ace.getmd*`, `ace.restrict*`, `ace.clone`, `ace.mv`, and `ace.size` are uninterruptible. If one does not complete, it writes no destination vector register, GPR, CR byte, ACEIOBUF byte, or memory location.
