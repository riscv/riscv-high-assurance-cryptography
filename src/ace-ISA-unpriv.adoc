...
For a CR in a Configuration State used as the destination of an `ace.clone`, see <<ACE-instruction-clone>>.
...
State. Explicitly opening a CR in an Error State for exporting is not allowed: exporting such a CR is
performed by simply saving the MDH.
+
...
.. If `ml`._State_ is an Error State, then the CR is immediately fully configured in the Error State, with
    the entire MDH given by the auxiliary input.
    This works with both `#immed7` = `ace_cfg_provisioning` and `#immed7` = `ace_cfg_importing`.
...
encrypt and nothing to authenticate. No SCC and no PCCC is produced, and neither direction opens a
