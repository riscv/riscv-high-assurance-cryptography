...
If the CR it names ceases to be in a Configuration State otherwise than through `ace.mgmt` — because it is
cleared, or because it is the destination of an `ace.clone` — `acemanagedcr` is set to 32.
...
.. if it completed a management operation resulting in a CR in a Complete State, it writes 32 to `acemanagedcr`; and
...
.... The ACE unit performs any necessary readjustment of the CR's content to turn it into the format internally consumed by the ACE Unit. This makes the CR available to software
...
... The implementation must restore the state of the CR to the state it was in when the export process was initiated:
  This can be done by decrypting and authenticating its contents or, alternatively, restoring them from an internal backup.
...
+
NOTE: `ace.load` and `ace.store` may always (be caused to) fail, leaving the CR in State _Unconfigured_.
The configuration operation must then be restarted. Software must prepare the CR again.
...
// The following three rules apply to all Algorithms implemented by an ACE unit.
...
fields _Algorithm_, _AlgorithmPolicy_, _KeyType_, _State_, _StateExtension_, and
_AuxDataLen_ (see <<ACE-extra-content>>).
It does *not* depend on _SCProtection_ (see <<ACE-rules-masked-implementations>>).
...
// All six fields
// _Algorithm_, _AlgorithmPolicy_, _KeyType_,
// _State_, _StateExtension_, and _AuxDataLen_,
// lie in bits [63:0] of the MDH, which is what allows a PI or
// SCC length to be computed from these 64 bits, as required by `ace.load`
// (<<ACE-instruction-load>>), `ace.store` (<<ACE-instruction-store>>) and Form B of
// `ace.size` (<<ACE-instruction-size>>).
// The same applies to _SCProtection_, allowing the ACE unit to allocate
// the requires internal resources upon issuing `ace.mgmt` using only
// the first eight bytes of the MDH.
...
During export of a CR to a SCC, the initial `ace.mgmt` operation encrypts the contents and generates `SIV` and, if _AuxDataLen_ {ge} 2, also `SIV2` and `IMPQUAL`. The contents remain encrypted in the CR until the completion operation either restores the clear contents from an internal copy or re-decrypts and authenticates them. The requirements for the latter are the same as for completing an import.
...
It suffices to save, resp., restore them at context save, resp., restore, and only when `acemanagedcr` is in the range [0..31].
...
The same requirement applies to `SIV`, `SIV2`, and `IMPQUAL`.
These CSRs must be saved and restored unconditionally, because software may set them before issuing `ace.mgmt`.
