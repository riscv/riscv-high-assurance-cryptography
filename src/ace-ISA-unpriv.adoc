 ...
 * secp256r1 (NIST P-256) cite:[NIST-SP-800-186,nist-fips-186-5]: `b` = 256, `h` = 256, `j` = 256, `u` = 2, `v` = 2.
 * secp384r1 (NIST P-384) cite:[NIST-SP-800-186,nist-fips-186-5]: `b` = 384, `h` = 384, `j` = 384, `u` = 2, `v` = 2.
 * secp521r1 (NIST P-521) cite:[NIST-SP-800-186,nist-fips-186-5]: `b` = 576 (the 55 most significant bits are zero-padding), `h` = 512, `j` = 512, `u` = 2, `v` = 2.
 * ed25519 cite:[RFC7748]: `b` = 256; `h` = 256; `j` = 256; `u` = 1; `v` = 1.
 * ed448 cite:[RFC7748]: `b` = 448; `h` = 448; `j` = 448; `u` = 1; `v` = 1.
 * brainpoolP256r1: `b` = 256; `h` = 256; `j` = 256; `u` = 2; `v` = 2.
 * brainpoolP384r1: `b` = 384; `h` = 384; `j` = 384; `u` = 2; `v` = 2.
 * brainpoolP512r1: `b` = 512; `h` = 512; `j` = 512; `u` = 2; `v` = 2.
 * SM2 cite:[GMT-0003-1-2012,GMT-0003-2-2012,GMT-0003-3-2012,GMT-0003-4-2012,ISOIEC-14888-3-2018]: `b` = 256; `h` = 256; `j` = 256; `u` = 2; `v` = 2.
...
`{fournbsp}{fournbsp}` `varlen` <- 8 + (`HasSecondPt` != 0) {interpunct} ``ub``/8 + (`HasSignature` != 0) {interpunct} 2``b``/8 + +
`{fournbsp}{fournbsp}{fournbsp}{fournbsp}{fournbsp}{fournbsp}{fournbsp}` + (`HasHash` != 0) {interpunct} ``h``/8 + (`HasRndNum` != 0) {interpunct} ``h``/8.
 ...
 ** Upon returning to State _initial_:
 ...
 This is determined by Bits 0, 1, 2, and 3 of `Xs` as follows:
   **** If Bit 0, 1, 2, resp., 3 is set, then `Generator` is reset to the default value, `SecondPt`, `Scalar`, resp., `PublicKey` is erased, and the corresponding Boolean flags `HasScalar`, resp., `HasSecondPt` is unset as well.
  ...
  ** If `IsGenInfty` is not true, `ace.derive` can be called, depending on the type of the curve, once or twice, to create an encryption or decryption CR and a CR for the computation of a MAC.

  For encryption, the `PublicKey` field must be set with the recipient's public key.

  For decryption, the `PublicKey` field must be set with the sender's public key and the `Scalar` field must be set with the recipient's private key.

  Before invoking `ace.derive`, it is allowed to transition to
  State _absorb_S1_ where the bit length of the optional S1 must be given as a parameter to the `ace.state` transition call through a GPR, and this value is set in `Progress`.
  Then, one or more Form B `ace.exec` calls can be made to absorb values into the state, and the amount of absorbed data so far is subtracted from `Progress` until it is zero. Excess data is ignored. S2 must be handled by the used using the MAC function (for NIST curves, an HMAC, with S2 appended to the ciphertext).
