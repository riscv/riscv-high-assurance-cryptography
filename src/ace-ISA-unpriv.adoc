...
function POLYVAL(&tmp : bits(128), auth_key : bits(128), blocks[0 .. n-1] : bits(128),
  ...
  foreach(i from 0 to n-1) {
    if (i == n-1 && last_block_fractional) {
      tmp[63:0] ← tmp[63:0] xor blocks[i];
      tmp ← Montmul(h, auth_key);
    } else {
...
[WARNING]
====
Do we need a mechanism to clear the `CR.exporting/importing` flag upon context switch, to force a restart in this case?
Or would simply clearing the CR be sufficient?
====
...
function ace.export(&CR, M[]) = {
  & mh : bits(64) = CR.Metadata_header;    // By reference, from the CR
  CR.exporting : bool;                     // "hidden" per-CR ACE unit state
  local SIV, tmp : bits(128);

  external CSK : bits(256);
  external enc_key : bits(256);   // NOTE: {enc_key, auth_key} ← RFC8452_KeyDeriv(CSK);
  external auth_key : bits(128);

  CR.exporting ← true;                     // always false unless during export.
...
function ace.import(&CR, M[]) = {
  ...
  CR.importing : bool;           // Architecturally "hidden" per-CR ACE unit state
  local saved_SN : bits(4);      // this is a "hidden" state
  local SIV, tmp : bits(128);
...
[NOTE]
====
`CR.importing` and `CR.exporting` need not be distinct bits.
...
to check whether the `CR.importing/exporting` bit is set and the CR
number also matches. If not, the instruction is restarted anyway.

[.red]#{warn} We need a mechanism to clear# `[.red]#CR.importing/exporting#` [.red]# when a
