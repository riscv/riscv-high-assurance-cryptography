...
|   [45:32] |  14  | _AddDataLen_      | If non-zero, indicates that an additional, variable-length, implementation-dependent data section is appended to the Content. The length of this additional data section is _AddDataLen_ * 128 bits.
...
The _AddDataLen_ MDH field records the length of the _Additional Data Section_ (ADS) of the content, a variable-size, implementation-dependent part, as a number of 128-bit blocks. The maximum value is 2^14^-1, corresponding to 262128 bytes.

If _AddDataLen_ is zero, there is no ADS.

The value one is used to signal the state machine in the CR that ADS was present and then discarded.

If _AddDataLen_ is not zero or one, the first 128-bit block of this additional section contains
a copy of the originating implementation qualifier.

``{fournbsp}{fournbsp}IMPQUAL := zeros(32) @ acemimpid @ acemarchid @ acemvendorid``

(cf. <<ACE-CSR-impl-ids>>) from the originating machine. These 16 bytes are counted within the length recorded by the _AddDataLen_ field.
The importing implementation uses this value to determine whether the information in the ADS can be used.

Since an ADS contains at least an authentication tag (16 bytes) and the originating implementation qualifier (also 16 bytes), the minumum length is 32 bytes, corresponding to an _AddDataLen_ value of 2.
...
+
In particular, it does *not* depend on _AddDataLen_: a PI carries no
implementation-specific data, and _AddDataLen_ must be zero in the MDH of a PI. Nor does
...
. The length of a *Sealed Cryptographic Context* is determined by, and only by, the
  ...
  _AddDataLen_ (see <<ACE-extra-content>>).
...
The MDH's _AddDataLen_ field specifies the total size of the Implementation-Specific ADS (`SIV2`, `IMPQUAL`, and `Content2[]`).
...
. Second, if _AddDataLen_ {ne} 0, then the implementation-dependent data is processed:
...
. Finally, output `saved_MDH`, then `SIV` and `Content1~CT~[]`, and, if AddDataLen {ne} 0, `IMPQUAL`, `SIV2` and `Content2~CT~[]`.
--
...
If any of the two operations is not completed, various types of error conditions will occur because _ConfigStatus_ is `ace_cfgst_complete`.
====
...
--
 ...
 . `actual_impdatalen` <- `M`._AddDataLen_.
 . If the length exceeds the implementation's maximum for the given Algorithm, the SCC contains an Implementation=Specific ADS that the importing implementation does not support. The SCC length is therefore adjusted by excluding the Implementation=Specific ADS, and `actual_impdatalen` <- 1.
...
... `correct` <- `false`.
 ...
 . ``K(i)``.``MDH``._AddDataLen_ <- `actual_impdatalen`. Where this is zero or one, the import completes using only the architecture-dependent content. The value 1 signals the state machine of the imported CR that the ADS was dropped. The state machine then establishes an internal consistent state and sets _AddDataLen_ to zero, and then to a suitable value upon export, if necessary.
--
