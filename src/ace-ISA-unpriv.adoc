...
+
--
An illegal-instruction exception is raised, without any change to ACE state, on the grounds below.
The grounds of the first group are decided from the instruction encoding, the GPRs and the vector configuration alone, without accessing the state of the ACE unit; they therefore take precedence over `ace_exc_fatal` (<<ACE-exception-codes>>).
The grounds of the second group require the state of the ACE unit and are raised only once no fatal condition is pending.
...
. When the Privileged Architecture is not implemented, an instruction issued without a CSK (<<ACE-exception-error-correspondence>>).
--
