...
| `Zlkmacm`      | KMAC function with hidden key. Depends on `Zlesha3h`, since we define KMAC only with SHA3. | TBD | v0.0.0      | --
...
For each Locality we provide: index in the LST; domain (SoC-, device-, VM-wide, or per-process); configurability; bit-field encoding and value.
...
Point 3. is very difficult to implement in full generality, since there are several options, with multiple choice of KDF, hash function, and input formats.
In most cases, covering all options would be impractical.
Only the Chinese SM2 standards fully specify an integrated encryption scheme cite:[GMT-0003-4-2012], and therefore we define how to implement it.
For the other curve families, point multiplication is the basic mechanism by which KEM and PKE schemes are implemented and this implies that the key values derived from these must be live in the memory of the environment deriving them, albeit shortly if these are then used in CR self-provisioning.
This may be hardened by delegating the actual key derivation to a more secure environment and then returning the derived key to the less secure environment as SCCs.
