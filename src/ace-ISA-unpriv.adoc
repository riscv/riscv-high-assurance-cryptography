...
The most important instructions are briefly described below, ahead of their full definitions.

* `ace.mgmt` (<<ACE-instruction-mgmt>>) transitions a CR between states of its management lifecycle, and,
together with `ace.load` (<<ACE-instruction-load>>) and `ace.store` (<<ACE-instruction-store>>), enables the following operations:
