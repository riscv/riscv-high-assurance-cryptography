...
//that is encoded in the metadata of a CR,
...
ACE may be implemented within a CPU core or as a hardware unit shared by multiple cores, with separate architectural state for each client hart. This specification defines behavior, not implementation.

ACE need not be implemented entirely in hardware. It may be implemented wholly or partly using trap-and-emulate. For example, an implementation may perform symmetric cipher operations (cf. <<ACE-states-symmetric>>) in hardware while emulating asymmetric operations, such as Elliptic Curve Algorithms (cf. <<ACE-ECC>>). This approach is particularly useful for Post-Quantum Cryptographic Algorithms (cf. <<ACE-PQC>>), whose states may be very large. Emulating these operations in M-mode firmware avoids storing those states in the ACE unit, provided that the relevant CRs reside in secure SRAM or other secure memory.
...
Only M-mode is allowed to access the ACE state attached to the hart it is running on.
...
|   [45:32] |  14  | _ImpDataLen_      | If non-zero, indicates that an additional, variable-length, implementation-dependent data section is appended to the Content. The length of this additional data section is _ImpDataLen_ * 128 bits.
...
| [126:116] |  12  | Reserved          | Reserved for future use.
|===
...
For asymmetric primitives supporting signatures, the lower bit permits signature generation and the upper bit permits verification. In this case, both bits mau be zero.
...
The _ImpDataLen_ MDH field records the length of the _Additional Data Section_ (ADS) of the content, a variable-size, implementation-dependent part, as a number of 128-bit blocks. The maximum value is 2^14^-2, corresponding to 262112 bytes.

If _ImpDataLen_ is zero, there is no ADS.

The all ones value is used to signal the state machine in the CR that ADS was present and then discarded.

If _ImpDataLen_ is nonzero and _not_ the all ones value, the first 128-bit block of this additional section contains
a copy of the implementation qualifier.

``{fournbsp}{fournbsp}IMPQUAL := zeros(32) @ acemimpid @ acemarchid @ acemvendorid``

(cf. <<ACE-CSR-impl-ids>>) from the originating machine. These 16 bytes are counted within the length recorded by the _ImpDataLen_ field.
The importing implementation uses this value to determine whether the information in the ADS can be used.
...
--
 ...
 . If the length exceeds the implementation's maximum for the given Algorithm, the SCC contains an Implementation=Specific ADS that the importing implementation does not support. The SCC length is therefore adjusted by excluding the Implementation=Specific ADS, and `actual_impdatalen` <- ones(14).
...
.. else:
... `correct` <- `false`.
 . If `correct` {ne} `true`, then `Content2[]` is dropped and `actual_impdatalen` <- ones(14).
 . ``K(i)``.``MDH``._ImpDataLen_ <- `actual_impdatalen`. Where this is zero or `ones(14)`,the import completes using only the architecture-dependent content.   `ones(14)` signals the state machine of the imported CR that the ADS was dropped. The state machine then establishes an internal consistent state and sets _ImpDataLen_ to zero, and then to a suitable value upon export, if necessary.
--
