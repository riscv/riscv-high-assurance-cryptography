 ...
 ** `Progress`: integer.
 ** `block_base`: integer.
 ** `Generator` (X coordinate{, Y coordinate}): one or two `b`-bit strings representing the X {and Y} coordinate{s}.
 ** `SecondPoint` (X coordinate{, Y coordinate}): one or two `b`-bit strings representing the X {and Y} coordinate{s}.
 ** `Scalar`: `b`-bit string.
 ** If signature generation or verification is allowed:
  *** `PublicKey` (X coordinate{, Y coordinate}): one or two `b`-bit strings representing the X {and Y} coordinate{s}}.
