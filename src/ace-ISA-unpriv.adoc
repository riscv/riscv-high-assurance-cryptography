...
[.red]#{warn}#_If `uepc` is not defined, can we define it, however for this type of applications, a local variable can be used to keep track of the place where the error occurred._# Since `aceerror` provides the cause, a CSR like `ucause` is not necessary.
