...
[]
----
// The second input to POLYVAL is an integral number of 16-byte blocks.  This means that
// the last block may have to be padded with zeros to reach the next 16-byte boundary.

function POLYVAL(&tmp : bits(128), auth_key : bits(128), blocks[0 .. n-1] : bits(128)) = {
  // tmp is from the parent environment
  local i : int;
...
During SCC export and import, these Locality Secrets are prepended, in ascending index order, to the input of the AEAD tag computation, before the metadata.
...
function ace.export(&CR, M[]) = {
  ...
  local L : list ← {} if (mh.Locality == 0) else get_localities(mh.Locality);

  // Include Locality "prefix" (zero or more 128-bit blocks) in the POLYVAL
  tmp ← zeros(128);
  POLYVAL(&tmp, auth_key, L @ zeros(64) @ mh @ CR.content[]);
...
interrupted import/export operations at the same time, but this
