...
We do not make any assumption on the structure of `Content2[]`.
...
restricted resource: “CRF capacity” below refers to it alone. A CR that holds only its MDH — one in an Error State,
by Rule SGR8 in <<ACE-State-field>> — therefore consumes no CRF capacity and can always be configured.
...
<<ACE-length-rule>> is the sole authoritative source for determining which MDH fields specify the sizes of provisioning inputs and Sealed Cryptographic Contexts.
...
Upon entering an Error State, the CR's contents are cleared as per Rule *SGR8* in <<ACE-State-field>>.
...
SGR5:: States _Success_ and _Failure_ cannot be set directly by the caller.
...
SGR9:: The transitions from _Unconfigured_ to any Error State result from configuration instructions, or
...
`ace.mgmt`: software reads the MDH and stores it (<<ACE-instruction-setst>>).
...
SGR12:: If a CR is in any Error State, it can still be reconfigured, cloned, or cleared. `ace.setst` can only be used to change the Error State (for instance by an exception handler) or transition the CR to _Unconfigured_.

SGR13:: The MDH of a CR in an Error State can be read using `ace.getmd*` (<<ACE-instruction-getmd>>).

SGR14:: Attempts to _use_ a CR in any Error State will not perform any operation, and in particular they leave the _State_ field unchanged.
...
SGR16:: Usage of a CR in a Configuration State, and cloning from it, are not allowed:
...
SGR18:: `ace.load` and `ace.mv` with a CR as destination
...
SGR19:: `ace.store` and `ace.mv` with a CR as source
...
A CR that is not in a Complete State is not evaluated, so expiration never aborts a management
operation: an instruction naming a CR in a Configuration State is rejected by Rule SGR16
(<<ACE-State-field>>) first, and one naming an _Unconfigured_ CR by the rule of the instruction
concerned.
This section is the authoritative statement of when the _ExpirationDate_ field is evaluated;
no rule elsewhere in this specification extends it.

This evaluation is performed as follows:
...
. Otherwise, the hardware does nothing.

Management operations, such as exporting, importing, clearing, cloning CRs, and getting their size, do not check the _ExpirationDate_: none of them is one of the evaluation points listed above. Consequently, the transition to Error State `ace_state_expired`is only triggered when an expired CR is being actively used, while still permitting expired contexts to be cleanly evicted, saved, or unconfigured.
...
A system may support multiple system-specific formats; ACE does not specify how to distinguish between them.

The value _Version_ = 2 is reserved.

A PI or SCC whose `_Version_` value the implementation does not support — for implementations of this version, any value other than 0, and 3 where system-specific formats are unsupported — is invalid Metadata: the CR transitions to Error State `ace_state_invalid`.
...
Data Access to a CR Not Open for Management:::
+
--
Issuing `ace.load`, `ace.store`, or `ace.mv` on a CR whose _State_ is not one admitted by
Rule SGR18, resp. SGR19, of <<ACE-State-field>> raises an illegal-instruction exception
without state change. This covers every Complete State and the _Unconfigured_ State, as well as
a Configuration State opened for the opposite direction of transfer.
--
...
Currently, the only encoding is `ace.clear.all`.
+
An indirect index outside [0..31] also raises an illegal-instruction exception.
+
An instruction using one CR carries a selector bit `r`, at the position shown in its encoding diagram. Namely, `r` = 0 when the CR index is the 5-bit immediate held in that field, and `r` = 1 when it is the value held in the GPR that field names. The only exceptions to this encoding rule are `ace.load` and `ace.store`, which use a different value in `funct3` for this purpose, since all other bits are used by immediates and register encodings.
...
+
NOTE: The hardware does not check that `acestart` and `vstart` are consistent when an instruction is resumed. If software restores values that do not denote the same point of progress for an instruction that requires them to, the behavior is unpredictable (i.e., which bytes are processed is not specified, but the result remains a function only of architectural state that the executing context is permitted to observe).
...
| `ace.clear`, `ace.clear.all` | Yes             | No  | No                   | Encodings of `ace.setst`; see its row above.
...
GR6:: Opening a CR for import, as well as executing `ace.clear`, is always allowed in any State, since the
...
If the CR's _State_ is not one of the States admitted by Rule SGR18 (<<ACE-State-field>>) — which excludes every Complete State and the _Unconfigured_ State — or `acestart` is not a multiple of 16, an illegal-instruction exception is raised without state change.
...
Analogously to `ace.load` (<<ACE-instruction-load>>), a _State_ not admitted by Rule SGR19 (<<ACE-State-field>>), or an `acestart` that is not a multiple of 16, raises an illegal-instruction exception without state change.
--
...
Further usage of the same CR—for instance, a later `ace.exec` or `ace.setst` to a valid state—performs no operation and raises no exception, leaving the _State_ field unchanged (Rule SGR14 in <<ACE-State-field>>) until software detects the Error State.
...
.. If `#immed7` = `ace_cfg_importing` and `ml`._State_ is an Error State, then the CR is immediately
    ...
    The whole configuration is performed by this one instruction, which is uninterruptible (Rule IRR1):
...
//
// For longer-running `ace.mgmt` operations—such as starting or completing an export, or completing an import—an implementation uses one of the strategies of Rule IRR1 (<<ACE-resumability>>); discarding partial work requires internal buffering, so the choice may depend on available CRF capacity.
The above semantics of `ace.mgmt` allow an interrupted import to resume on another hart or on a different ACE unit implementation, provided the target unit supports the Algorithm encoded in the SCC.
...
`ace.restrictl/ace.restricth/ace.restrictv` may be used in any Complete State; in the _Unconfigured_ State it performs no operation, and in a Configuration State it raises ACE exception `ace_exc_privilege_violation` (Rule SGR16 in <<ACE-State-field>>). Care is required:
...
* If source and destination are the same CR the instruction performs no operation, even if the CR is _Unconfigured_ or in a Configuration State, which is excepted from Rule SGR16 (<<ACE-State-field>>).
...
(Rule SGR16 in <<ACE-State-field>>), with no state change to either CR.
...
before any component access of the current execution attempt, with the completed prefix committed and `acestart` recording it as in Rule IRR6.
...
An implementation realizes this in one of three ways: by completing the instruction before taking an asynchronous interrupt; where Rule IRR3 permits, by letting it complete in the background, subject to Rule IRR5 below; or by discarding its partial work before taking the trap and re-executing it upon return.
Software must not rely on whether an uninterruptible instruction has retired when a handler runs, and interacts with it only as Rule IRR5 prescribes.

IRR2:: `ace.setst` is uninterruptible under every architectural condition defined by this specification. This rule may have implementation-specific overrides (see Rule IRR9 below).
...
It is available only for `ace.load`, `ace.store`, `ace.input`, and `ace.output`, whose transfers are idempotent; `ace.exec`, `ace.derive`, and vector `ace.mv` use option 3 or 4 (Rule IRR7).

IRR5:: All the approaches in Rule IRR4 are subject to one rule: Before the hart performs a _conflicting access_, the in-flight operation must either have completed or have been precisely halted with `acestart` set per <<ACE-Memory-Model>>. A conflicting access is any of the following:
+
--
...
IRR4 and IRR5 make asynchronous execution by the attached ACE unit invisible to same-hart program order, as <<ACE-Memory-Model>> requires. Beyond this rule, the architecture specifies only the allowed interruption points for its instructions and Algorithms.
--
...
These rules bind every Algorithm, custom ones included.

A CR in an Error State has no PI, no SCC and no PCCC: it is an MDH and nothing else.
Its exported image is therefore always the 16 bytes of that MDH, irrespective of every other field,
and the CR occupies only the capacity of its MDH (Rule SGR8 in <<ACE-State-field>>).
A PI cannot be in an Error State; an MDH whose _State_ is an Error State configures a CR directly
(<<ACE-instruction-setst>>) and carries no `Content[]`.
...
NOTE: This rule assume that reconstructing or re-randomizing a key is inexpensive.
For example, the ACE unit may XOR the values with data from a PRNG (according to <<ACE-RBG>>).
This approach keeps SCCs compact and ensures interoperability across architectures with different implementations.
To accelerate multiple-share reconstruction of a key, some shares or masks can be included in the SCC's ADS.
...
If shares or masks are included in the ADS as in the remark of <<ACE-rules-masked-implementations>>, it must not be possible to recover the key's value from the included values.
...
A CR in an Error State holds only its MDH (Rule SGR8 in <<ACE-State-field>>), so there is nothing to
...
. Issue `ace.mgmt` with `#immed7` = `ace_cfg_importing` and the 16-byte MDH
  ...
  (Rule IRR1 in <<ACE-resumability>>), so the CR is never observed part-configured.
