...
It contains one or more keys, but it is significantly more than just a key container.
A CC is an indivisible structure consisting of a *Metadata Header* (MDH) and *Content*; all cryptographic operations performed by ACE execute within the constraints defined by a CC.
The MDH has a fixed format and specifies the algorithm that can be used with the key(s), and various optional policies associated with the CC.
...
Algorithm:::
(((Algorithms)))
An *Algorithm* (written with an upper-case "A"), as referred to in the metadata of a CC,
is a state machine that performs cryptographic operations through an interface formalized in this specification.
...
in the _AlgorithmUse_ field instead, as rule AGR8 of <<ACE-general-rules-for-algorithms>>
prescribes.
