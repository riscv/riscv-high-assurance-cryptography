 ...
 ** Upon returning to State _initial_:
  ...
  ** If `IsGenInfty` is not true, `ace.derive` can be called, depending on the type of the curve, once or twice, to create an encryption or decryption CR and a CR for the computation of a MAC. For encryption, the `PublicKey` field must be set with the recipient's public key.
