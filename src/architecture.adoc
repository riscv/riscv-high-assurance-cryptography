...
for i = 0 to num_blocks(P)-1
     M[i+2] {leftarrow} (block # i of P) {oplus} AESE(Kenc, SIV[95:0] || (binary(i+1)[31:0]))
...
(Recall that by `x` {vvert} `y` we indicate the concatenation of `x` and `y`, where the bits of `x` become the most significant part of the concatenated value, and the bits of `y` its least significant part. For instance, `0xAB` {vvert} `0xCD` = `0xABCD`.)
