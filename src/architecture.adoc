...
| [29:28] |  Cloning_Policy | 00 cannot be cloned, else the CC can be cloned: 01 can be re-initialised, 10 must be re-initialised, 11 cannot be re-initialised. | Public| [31:30] |  Reserved | Zero | Public
...
|   [23:23] |  Sealed | Whether the CC is complete (00) or interrupted while being configured (01), imported (10) or exported (11). Necessary to proper restart or resume operations. | Public
|   [27:24] |  Error | Last error value | Public
|   [29:28] |  Cloning_Policy | 00 cannot be cloned, else the CC can be cloned: 01 can be re-initialised, 10 must be re-initialised, 11 cannot be re-initialised. | Public
...
Otherwise, this value is used as an index _i_ in the Lifecycle Secrets Table, and the value Derivation(CTK, LST[_i_]) is used for import/export in place of the CTK.
