...
This instruction signals that the current process domain has stopped using the ACE and all CCs are therefore invalidated.
...
`Cs`{nbsp}{nbsp}{nbsp}is the source CC to be exported (as a 5-bit immed). +
`Xs`{nbsp}{nbsp}{nbsp}alternatively, is a GPR containing a number from 0 to 31 to dynamically address the context to be exported. +
`Xn`{nbsp}{nbsp}{nbsp}is an GPR containing the base address of the buffer where to store the SCC.

Exports the contents of CC `Cs` to memory, starting at address `Xn`.
...
`Cd`{nbsp}{nbsp}{nbsp}   is the destination CHR to be imported, as an immed (5-bit).
`Xd`{nbsp}{nbsp}{nbsp}   alternatively, is a GPR containing a number from 0 to 31 to dynamically address the context into which import the SCC.
`n` {nbsp}{nbsp}{nbsp}    is the name of an integer register containing the base address of the buffer with the SCC.

S = 0,1  true (1) if the key is in a system defined format and not the format defined by the architecture.

This means that system HW dynamically generated keys can be imported but not exported in the same form — if exported they must be in the architectural format. If the value cannot be exported to other devices or to other Locale stages of the same device, the internal logic must bind them to a specific Locale derived CTK).
...
Is the algorithm defined by GPR Xs (this is the value of the algorithm field of the metadata) supported by the µarch?
`Xd` = true/false
