...
A CC contains two fields: the `metadata` and the `state`. Key material and possibly other algorithm-specific data are contained in the state.
...
[width="100%",cols="7%,13%,74%,6%",options="header",]
|===
...
|    [42] |  AC_Policy_2 | Restrict USE to specific User? (Import/export/overwrite always allowed.) | Private
| [58:43] |  AC_Policy_3 | IDs to filter User OR Supervisor (filtering both or even world, or higher resolution, is delegated to future versions, with metadata larger than 64 bits). | Private
| [63:59] |  Reserved | Zero | Private
|===
