...
|   [27:24] |  Error | Last error value | Public
|   [31:28] |  Reserved | Zero | Public
|   [63:32] |  `partialsccis` | ID of the µarchitectural format for CCs with interrupted operations. | Public
|   [69:64] |  Stage/Direction | Which internal stage the algorithm is, as defined by ace_stage or implicitly. Must be ZERO on configuration. This is algorithm defined | Private
|   [73:70] |  AC Policy 1 | Can a User/Supervisor/Hypervisor/Machine use this context? | Private
|   [75:74] |  AC Policy 2 | Restrict USE to specific User (bit 42)/Supervisor (bit 43)? Both bits may be set in this version of the metadata. | Private
|   [91:76] |  AC Policy 3 | IDs to filter by `UKSID` (*to be double-checked: reuse `ASID`*) | Private
|  [105:92] |  AC Policy 4 | IDs to filter by `SKSID` (*to be double-checked: reuse `VMID`*) | Private
| [127:106] |  `marchpriv` | Implementation private metadata | Private
|===


*Remark {counter:remark}*:: We note that in the 128-bit version of the metadata, the first 64-bit word (i.e., at the lowest address) contains bits [63:0] and the second 64-bit word (i.e., at the byte address of the first word, plus 8) contains bits [127:64].+
The long MD format contains 64 bits of public MD and 64 bits of private MD.
The first 64-bit word contains the public metadata, and the second 64-bit word contains the private metadata.
The initial fields of the private metadata have the same offsets as the corresponding fields in the 64-bit metadata format, they are simply offset by 32 bits.

*Remark {counter:remark}*:: `partialsccis` is defined with a process similar to the `marchid` register. It can be used to determine whether the exported format from a suspended configure, import or export operation is compatible with the current µarchitecture, in which case the suspended operation can be resumed, otherwise the operation must be restarted anyway. All implementations must support restart, resuming is optional. +
...
|===
>| `derived_key` {nbsp} = | AES-256(CTK, _b_ {oplus} (0^126^ {vvert} 11))[63:0] {vvert} AES-256(CTK, _b_ {oplus} (0^126^ {vvert} 10))[63:0]
>|                        | {nbsp}{nbsp}{nbsp}{nbsp} {vvert} AES-256(CTK, _b_ {oplus} (0^127^ {vvert} 1))[63:0] {vvert} AES-256(CTK, _b_)[63:0]
|===

Notation: by `x` {vvert} `y` we indicate the concatenation of `x` and `y`, where the bits of `x` become the most significant part of the concatenated value, and the bits of `y` its least significant part. For instance, `0xAB` {vvert} `0xCD` = `0xABCD`.
...
[source,sail,subs="+normal"]
----
P {leftarrow} key/keys { {vvert} additional_state }
Pick random 96-bit nonce N, Derive Kenc and Kauth from effective CTK as per RFC 8452.
SIV[95:0] {leftarrow} Poly1305(Kauth, N, metadata {vvert} P)[95:0]
M[0][127:0] {leftarrow} N[63:0]
                {vvert} \((metadata[63:32] {oplus} AESE(Kenc, SIV {vvert} 0x0000 0000)[63:32])
                  {vvert} metadata[31:0]
M[1][127:0] {leftarrow} SIV[95:0] {vvert} N[95:64]
for i = 0 to num_blocks(P)-1
     M[i+2][127:0] {leftarrow} (block # i of P) {oplus} AESE(Kenc, SIV {vvert} (binary(i+1)[31:0]))
----

//Notation: {oplus}, {or}, {and}, and {lshift} denote XOR, OR, AND, and left shift, respectively.
(Recall that by `x` {vvert} `y` we indicate the concatenation of `x` and `y`, where the bits of `x` become the most significant part of the concatenated value, and the bits of `y` its least significant part.)
