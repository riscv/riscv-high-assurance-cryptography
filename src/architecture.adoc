...
[width="100%",cols="25%,67%,^8%",options="header",]
|===
| Name | Description |Value
| No lifecycle binding | | 0
| SiP Unique Secret | Identifies the manufacturer of the SoC. Permanent. | 1
| Chip Model Unique Secret | Identifies specific chip model/family. Permanent. | 2
| Device Unique Secret | Unique device identifier. Shareable across multiple SoCs. Sharing mechanism impdef, i.e., not architected.
  Can be permanent in a device — re-generations can be optionally triggered. | 3
| Device Type Unique Secret | Identifies device type/model. Not necessarily permanent. | 4
| OEM Secret  | Provisioned by OEM to differentiate their products from the competition. Product dependent. | 5
| System Secret | Identifies System (Stack). Not permanent, must be configured during boot. | 6
| Boot Session Unique Secret | Regenerated at each boot. | 7
| Reserved for Revocation | Regenerated at boot or if there is one revocation event | 8
|===
