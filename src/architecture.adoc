...
| [29:28] |  Cloning_Policy | Value 00 means the CC cannot be cloned, else the CC can be cloned, and +
             01 means clone can be re-initialised, +
             10 means clone must be re-initialised, +
             11 means clone cannot be re-initialised. | Public| [37:32] |  Stage/Direction | Which internal stage the algorithm is, as defined by ace_stage or implicitly. Must be ZERO on configuration. This is algorithm defined | Private
...
|   [29:28] |  Cloning_Policy | Value 00 means the CC cannot be cloned, else the CC can be cloned, and +
               01 means clone can be re-initialised, +
               10 means clone must be re-initialised, +
               11 means clone cannot be re-initialised. | Public
