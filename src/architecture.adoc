...
| [22:18] |  Environment Binding | Field which is either -1 or an index into the table of Environment Binding Tokens. | Public
...
Internally, ACE has a table of 128-bit Environment Binding Tokens (EBT), the Environment Binding Tokens Table (EBTT).
...
If the Environment Binding Field of a context (CC/SCC) is -1 (all ones), then the CTK is used.
Otherwise, this value is used as an index _i_ in the Environment Binding Tokens Table, and a derived key `derived_key` computed from CTK and _b_:=EBTT[_i_] is used for import/export in place of the CTK.
...
Since the derivation function is expensive, the ACE implementation is allowed to cache recently used Derived Keys.
...
| Chip_Manufacturer         | (I.e., Silicon Provider) Identifies the manufacturer of the SoC. Permanent. | N | 0
