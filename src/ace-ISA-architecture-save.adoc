...
(((Cryptographic Register)))
Cryptographic Contexts are realized using *Cryptographic Registers* (CRs), architectural containers provided by ACE to safeguard the CCs.
Metadata and Content are bound to each other to form a CC by writing them into a CR, i.e., by *provisioning* the CR.
A CC can be used for cryptographic operations only when stored in a CR.
Only the ACE unit may access or modify a CC, and only when the CC is in a CR, in accordance with the rules of the architecture and of the algorithm the CC is bound to.
CR Content cannot be extracted in a manner that compromises its confidentiality.

//Cryptographic Contexts are realized as *Cryptographic Registers* (CRs), which are architectural containers that safeguard CCs. A CR is *provisioned* by writing Metadata and Content into it; this operation binds the two sections to form a CC. A CC can be used for cryptographic operations only while stored in a CR. Only the ACE unit may access or modify a CC, and only when the CC resides in a CR, in accordance with architectural rules and the algorithm bound to the CC. The confidentiality of a CR’s Content cannot be violated.
...
This subset can be defined at a later moment and the ACE specification amended to allow the use of RVV-mini in place of V.
...
Its total size and the range thereof used in cryptographic instructions are determined by the `aceiobuflen` (<<ACE-CSR-aceiobuflen>>) and `aceiobuftop` (<<ACE-CSR-aceiobuftop>>) CSRs.
...
ACE defines two external formats for CCs:

. The _Provisioning Input_ (PI) format, which is loaded into a CR to create a CC; and
. The _Sealed Cryptographic Context_ (SCC), an encrypted, authenticated format used when exporting a CR to memory for later re-import.

{empty}
(((Cryptographic Context,Metadata)))
(((Cryptographic Context,Metadata,Header)))
The first 64 bits of both PI and SCC consist of the Metadata Header, whose format is given below.
...
| [16:16] |   1   | _KeyType_          | A value of 0 indicates that the key is expressed as an explicit value in the Content. A value of 1 indicates a system key that the key is given by an index into an internal, system-specific System Keys Table (SKT).
...
| [45:32] |  14   | _ExtraContent_     | If non-zero, there is an additional segment of data at the end of the Content. The length of additional segment is _ExtraContent_ 64-bit blocks. (14 bits allow up to 128 kB of additional data.)
...
ACES is defined analogously to the floating-point context status field, FS, and to the vector context status field, VS.
...
Writes to any ACES field do not modify any other ACE state.
In particular, setting ACES to Off, Initial, or Clean does not clear the CRs or the ACEIOBUF:
Any state reset must be done explicitly before setting ACES to one of these values.
...
* Any ACE instruction raises an illegal instruction exception when either field is set to Off.
...
NOTE: By “trappability” we refer solely to the CPU-side implementation, for instance to allow the implementation of certain ACE instructions using custom instructions that perform smaller units of work.
This is a distinct concept from the custom implementations of primitives within the ACE unit.
The latter can be achieved, for instance, by including a secure CPU with updatable firmware in the ACE unit, but it is beyond the scope of the ACE architecture.
