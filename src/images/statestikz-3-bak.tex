...
\tikzset{
   ...
   partial state/.style={trapezium, double, draw=black, semithick, minimum height=1.66em, text centered, rounded corners=3pt},
...
\begin{tikzpicture}[scale=1.1]
  ...
  \node[off state,text width=2cm,minimum height=1.5cm,inner sep=2pt] at (-5.00,0)  (off)  {\small Unconfigured};

  \node[partial state,inner sep=6pt] at (-2.00,-0.25) (partial)  {\small Partial};
  ...
  \node[error state] at (0,3.5)                     (authfail)       {\footnotesize\begin{tabular}{@{}c@{}}Authentication\\Failure\end{tabular}};
  \node[error state, text width=1.3cm] at (0,-3.5)  (expired)        {Expired};

  \node[circle,thick,draw=black] at (2  .75,2.75)     (configuration hub) {};
  \node[coordinate] at ($(expired.east)+(0.5,0)$)   (before expired)    {};

  \node[state, thick, text width=1.1cm] at (3.5,0)    (ready)           {Ready\vphantom{g}};
  \node[state, thick, text width=2.9cm] at (6.5,-1.5) (algorithm defined) {Machine-Defined};
  \node[state, thick, text width=2.9cm] at (6.5,1.5)  (success failure)   {Success / Failure};
  ...
  \draw[<-,cross line] (off) to[out=10,in=170,looseness=0.8] (ready);
