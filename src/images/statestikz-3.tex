...
\begin{tikzpicture}[scale=1.1]
  ...
  \draw[-,densely dashed]   (before expired) to[out=3,in=230,looseness=0.6] ($(success failure.south)+(-0.667,0)$);
  ...
  \draw[->,cross line]     (algorithm defined) to[out=188,in=0,  looseness=1]  (invalidated);

  \draw[<->] ($(success failure.south)+(-1,0)$) to[out=225,in=10,looseness=0.8] ($(ready.east)!0.5!(ready.north east)$);
  ...
  \draw[->,cross line] ($(algorithm defined.north)+(-0.667,0)$) to[out=130,in=0,looseness=1.2] ($(various errors.east)+(0.7,-0.2)$)
     -- ($(various errors.east)+(0,-0.2)$);
  ...
  %\draw[->,cross line] (success failure) -- (invalidated);
  %\draw[->,cross line] (success failure) -- (various errors);
  \draw[->,cross line] (success failure) to[out=172,in=0, looseness=1] (various errors);
