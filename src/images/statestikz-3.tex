...
\begin{tikzpicture}[scale=1.1]
  ...
  \draw[->,cross line]     (algorithm defined) to[out=188,in=0,  looseness=0.6]  (invalidated);

  \draw[->]  ($(success failure.south)+(-1,0)$) to[out=225,in=10,looseness=0.8]
    node[pos=0.34, above, sloped, yshift=-1pt] () {\tiny Conditional} ($(ready.east)!0.5!(ready.north east)$);
  \draw[<-]  ($(success failure.south)+(-0.8,0)$) to[out=225,in=5,looseness=0.8] (ready.east);
