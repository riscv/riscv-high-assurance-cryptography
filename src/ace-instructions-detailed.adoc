...
==== Appendix: Operations to implement trappable versions of the instructions

Certain ACE operations must handle arbitrarily long inputs and outputs.
To reduce hardware complexity, these operations can be made trappable and implemented in software as *Invisible Traps*.

Instructions `ace.init`, `ace.clear`, `ace.export`, `ace.import`, `ace.input`, and `ace.output` fall into this category.
When a trap occurs, the handler identifies the trapped instruction and can use `ace.size` to determine the size of the data to transfer, and the hardware will also be able to use `ccstart` to follow the resumption point for resumed instructions.
