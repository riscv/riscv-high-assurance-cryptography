...
This instruction is trappable [.red]#(to be determined whether it is security-wise ok)# and may also be resumed or restarted.
...
The instruction admits four variants, namely

[upperalpha]
. `ace.exec  Vd, Cn|Xn, Vs`,{nbsp}{nbsp}which takes an input and produces an output.
. `ace.exec  Cn|Xn, Vs`,{nbsp}{nbsp}which takes an input and has no output.
. `ace.exec  Vd, Cn|Xn`,{nbsp}{nbsp}which has no input and produces an output.
. `ace.exec  Cn|Xn`,{nbsp}{nbsp}which has no input and has no output, i.e., the instruction only performs internal processing in the CC.
    This variant is also used to encode `ace.stage` and other instructions.
...
For the four variants A-D above, the encodings are:
...
may also modify an internal state machine of the CC.
// It has four variants: has
// input and output, has I but not O, has no I but produces an O, has no I and O.
The `ace.exec` instruction may modify the internal state machine of the CC.
