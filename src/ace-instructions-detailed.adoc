...
`ace.import Cd|[Xd], %offset(Xn)` +
`ace.import.S Cd|[Xd], %offset(Xn)`
...
[upperalpha]
. `ace.exec  Vd, Cn|[Xn], Vs` {nbsp}{nbsp} takes an input vector and writes to an output vector.
. `ace.exec  {nbsp}{nbsp}{nbsp}{nbsp}Cn|[Xn], Vs` {nbsp}{nbsp} takes an input vector but does not write to an output vector.
. `ace.exec  Vd, Cn|[Xn]{nbsp}{nbsp}{nbsp}{nbsp}` {nbsp}{nbsp} does not take an input but writes to an output vector.
. `ace.exec  {nbsp}{nbsp}{nbsp}{nbsp}Cn|[Xn]{nbsp}{nbsp}{nbsp}{nbsp}` {nbsp}{nbsp} does not take an input vector and does not write to an output vector.
...
{reg:[
    { bits:  7, name: 0x5b, attr: ['custom-2'] },
    { bits:  5, name: 'Cd|[Xd]' },
    { bits:  3, name: 0x4, attr: ['ace.stage'] },
    { bits:  1, name: 'r' },
    { bits:  4, name: 0x0 },
    { bits: 12, name: 'imm' },
]}
...
`Cn` is the CC in which to set the stage of the internal state. +
