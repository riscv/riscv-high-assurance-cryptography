...
Isolated software components (e.g., enclaves, realms, TrustZone applets) are better suited to handle these protocols, their data structures, and their updates, and are designed to integrate with the local stacks' memory management.
. Revocation typically targets root keys. Since user software generally operates on derived keys, for instance session keys, it is only necessary that revocation be handled by the trusted firmware or hardware managing the root keys.
...
--
The present specification does not include a key revocation mechanism.
Although future versions could support revocation via key identifiers and system-wide broadcasts, such functionality is intentionally excluded from ACE because it would introduce unnecessary architectural and implementation complexity.
The rationale is as follows:
. ACE is designed to prioritize performance and flexibility (for example, algorithmic abstraction). Comprehensive revocation mechanisms, such as usage counters or revocation lists, would significantly increase complexity and could adversely affect performance.
. Multiple key revocation protocols exist, and each trusted TEE or hardware IP component is best suited to implement the protocol required by its own key provider. ACE permits the coexistence of multiple trusted key-providing components; enforcing a single, universal revocation mechanism would therefore be impractical. Moreover, the associated data structures could be large and complex, raising challenges for secure handling within the ACE unit, especially if external memory is required for those structures. For instance, the ACE unit would have to be agnostic to the external software stack's memory management while supporting a mechanism for securely obtaining protected memory from it.
In contrast, separate software components protected by reverse sandboxing (enclaves, realms, trustzone applets, etc.) do not usually face such constraints and can be more easily updated and extended.
. In most systems, revoked keys are master keys at the root of hierarchical key structures. TEE applets and hardware IP key management blocks typically provide derived keys (for example, session keys) to user software. In these cases, only trusted software or firmware continues to use master keys directly and should manage their revocation internally.
. Scenarios in which master keys are used directly by software, rather than solely for key derivation, are generally confined to higher-privileged components.
--
