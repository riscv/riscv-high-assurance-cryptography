...
* No `Smstateen`/`Ssstateen` integration for the new less-privileged state.
We just need a clarification: we have an _Off_ state, so we operate like the F and V extensions.
(*TBD: possibly adapt some of the F and V extension text.*)
...
* PRIV: *`scrstatus` write semantics are unspecified.*  The register is SRW, but nothing says what happens when software writes values inconsistent with actual register state (Off or Clean for a configured register, Clean for an unconfigured one) — whether writes are WARL-adjusted, ignored, or take effect and break the invariants that the exception semantics depend on. *Specify per-field WARL legalization.*

* PRIV: *`mcrstatus`'s normative status is self-contradictory, and lazy loading depends on it.* There is a warning saying it "may be defined", while the extension table and CSR table list it as defined by `Smacestatus`; the lazy-SCC ownership search ("search privilege modes from M-mode downward") needs it. The proposed remedy — making `sstatus.ACES` "unshadowed and independent" — would violate the rule that `sstatus` is a restricted view of `mstatus`. *Decide it normatively and keep `sstatus.ACES` a view of `mstatus.ACES`.*

* PRIV: *"Propagation" of `*crstatus` is asserted but never defined, and the V=1 rules omit the HS-level view.* There is no propagation concept for FS/VS in the base architecture to be "analogous to", and the V=1 rules set only `vscrstatus`(i) Dirty, leaving unstated whether `scrstatus`(i) is also updated — which breaks hypervisor-level lazy tracking. The ACES rules do set both, so the asymmetry looks unintentional. *Give explicit rules.*
**HOW**

* UNPRIV: *The CSR chapter lacks reset values and WARL/WLRL discipline, and value-dependent write traps break save/restore.*  No ACE CSR has a defined reset value or WARL/WLRL classification. Trapping on a *value* written Ωto a CSR is unusual for RISC-V (what about invalid `vstart` values), and here `acestart`'s legal set is dynamic, so context-restore or migration code writing back a saved `acestart` on an implementation that does not support resumption traps unpredictably.  Raising `ace_exc_out_of_mem` from a CSR write is unprecedented. *Assign reset values, classify the fields, and replace value-dependent traps with WARL behavior.*  Support for resumption should be mandatory?
...
* UNPRIV: *`Zlkn` depends on three extensions that do not exist.* Unpriv spec names `Zlaes128`, `Zlaes256` and `Zlesha2`; the defined names are `Zlaes128p`, `Zlaes256p` and `Zlesha2h` (`:94`, `:96`, `:105`). The malformed table row also leaves `Zlkn` with no "Defined in" target, and it appears nowhere in Books 2–4 despite being mandated by the `Zlv`/`Zlio` footnotes. *Correct the names and the row.*
