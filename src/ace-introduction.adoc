...
ACE defines three distinct representations for cryptographic data:

* *Initial Configuration Format:* The plaintext key and metadata in memory used to initialize a CC.
* *Cryptographic Context (CC):* The internal representation of the key, metadata, and operational state.
...
1. *IP Interoperability:* It allows hardware blocks from different vendors within a single system to share SCCs.
...
NOTE: While a unified architected format is necessary, mechanisms for persisting Context Transport Keys (CTKs) across reboots or transferring them between devices are outside the scope of the ACE architecture. For cross-device migration, the use of a public-key key-encapsulation mechanism (KEM) is recommended.
...
Certain ACE operations need to deal with arbitrarily long inputs and outputs.
To reduce hardware complexity, these can be made trappable, and implemented in software as _Invisible Traps_.
Upon trapping the handler will know which instruction has been trapped and can ask ACE to determine the size of the data to be transferred.
We need then three types of operations:

* A move instruction, that loads a certain number of bytes into the ACE unit;
* A corresponding move instruction to export a certain number of bytes from the ACE unit; and
