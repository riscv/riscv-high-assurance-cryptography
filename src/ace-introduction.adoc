...
* ACE protects secret information such as keys from exposure to software by performing cryptographic operations atomically—for example, a complete AES block encryption/decryption or a full hash round sequence between message injections. This is in contrast to common round-based ISAs that expose the keys that must be added between cipher rounds or MAC algorithm stages.
...
[WARNING]
*Discussion point:* In some algorithms we can have `VL*SEW` an integer multiple of the basic processing block size `b`.
Currently `ace.exec` at algorithm states are uninterruptible.
Can we allow them to be interruptible/resumable when they perform an operation multiple times on a vector because `b` > `VL*SEW`, and use `vstart` (same if they use an input/output buffer that is a integer multiple of `b` larger than `b`).
