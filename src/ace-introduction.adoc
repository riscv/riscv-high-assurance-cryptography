...
ACE is architected as an _attached_ unit, that is, it is viewed like a coprocessor with a dedicated, low-latency interface to the CPU core.
In particular, interrupts for the purpose of context switching need not interrupt cryptographic operations being performed by the ACE implementations, and the CPU pipeline may be stalled only when the parent process is saving the ACE state.
Page faults or other memory-subsystem related interrupts however may require restart or resumption of operations.

This model is analogous to streaming accelerators in other architectures.
An ACE implementation may be shared by multiple harts, each maintaining its own architectural state, and can operate concurrently with CPU cores, including in-order cores.
Implementations are not required to be attached: they may also be integrated directly into a CPU core.
...
* No current standardized or architected solution offers fast execution, efficient key management, and trust.
...
The term _opaque_ applies also to a key in a SCC. Furthermore, when we talk of an _opaque key_ we always implicitly include its complete metadata, including both the metadata that is included in encrypted form in a SCC, i.e., the _public_ metadata, and the metadata that is encrypted i.e., the _private_ metadata.
...
Regarding the access control policies in a CC's metadata, we must distinguish how they apply to (i) initial configuration, (ii) usage, and (iii) import/export of opaque keys.
...
*Remark {counter:remark}*:: [.red]#_Alternatives can be discussed:
...
See also <<ace-remark-2>>.

[#ace-remark-2]
Remark {counter:remark}:: _Other Remark._
...
The instructions `ace.init`, `ace.export`, and `ace.import` perform direct memory accesses. This does not violate the Risc-V load/store architecture principles, as ACE is architected as an _attached_ unit that works together with the CPU core rather than being an integral part of the latter. Consequently, it may maintain its own architectural state and have its own load-store unit. Also, these instructions can be considered specialized load/store operations that internally enforce the confidentiality and integrity of key material.
