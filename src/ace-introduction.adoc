...
* Cryptographic operations are performed _atomically_ to avoid leaking data through architectural side-channels.
  Instructions can be interrupted only if they perform multiple cryptographic operations and according to rules defined by the Algorithms they apply to.
...
. We will “`kl`” in this document instead of “`ace.`”: `kl` means ACE is a subset of crypto extensions, the “l” is also the initial of “Locker”, and Cryptographic Locker will replace Cryptographic Register.
...
. Assignment of opcodes: The encodings presented in <<ACE-instructions-detailed>> are preliminary. To a minimum, we need three opcodes in place of `custom-0`, `custom-1`, and `custom-2`.

* **ADD A COMMENT** Any additional inputs for (random) key generation?
An implementor could define a custom consume-anything/output unlimited KDF and use ace.derive to move data from it to keys in a CR.
Maybe define KDFs and architect mechnisms to provide an input with the KDF and `ace.derive` in place of an `ace.exec` in states that expect it as an input.
