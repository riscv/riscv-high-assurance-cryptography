...
Its primary features are the concealment of key values and other secrets, usage restrictions, and support for various cryptographic algorithms including implementations hardened against Side-Channel Attacks.

ACE addresses use cases such as
content protection,
local LLM decryption,
...
* **Variable-length ciphertext and plaintext inputs.**
...
Furthermore, in certain modes, such as in counter-mode encryption, the same method enables both encryption and decryption, potentially enabling misuse by malicious processes. This type risk cannot be mitigated by ACE.
