...
ACE is architected as an attached unit, that is, it is viewed like a coprocessor with a dedicated, low-latency interface to the CPU core.
...
. When software writes a key into a CC, the key becomes _opaque_: its cleartext value cannot be retrieved from the CC.
  If all cleartext copies of the key are erased, then that key value is effectively lost.
...
. ACE enables trusted software or hardware entities to provision opaque keys to less-trusted environments, enabling the latter to perform cryptographic operations without exposing the key value.  Once provisioned, the opaque key can be reused without further interaction with the trusted entity.
...
Algorithm numbers 0-127 are architecture-defined.
