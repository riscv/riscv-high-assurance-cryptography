...
For this purpose we need four types of operation:

* An operation to "open" a CC either for initializing, importing, or exporting;
* A load instruction, that loads a certain number of bytes into the ACE unit;
* A corresponding store instruction;
* A "close" operation to finalize the instruction.

While, in theory, these instructions need only respect the value of `ccstart` (cf. <<ACE-ccstart-definition>>) and update it in accordance with architected behavior, we provide a non-normative definition for such a set of instructions in a later draft of the `ACE` specification.
