...
ACE defines an interface to high-performance cryptographic operations without giving software visibility into the keys.
...
* To enable CC reuse and support OS features like context switching, ACE provides a secure mechanism to store a CC to memory via authenticated encryption, producing a _Sealed Cryptographic Context_ (SCC). An SCC can be retrieved and re-imported into a CR. If an SCC is passed to a process, the key is never exposed in that process’s memory.
...
** In addition to GCM-SIV, UTS, and OCB, ACE defines modes such as CTR, GCM, and CMAC.
If the mode applies, we also define versions with a pre-set IV or nonce, and versions that are encrypt-only or decrypt-only, to reduce abuse.
...
** _Not all these algorithms and primitives must be supported by an ACE implementation:
 A minimal subset and several optional extensions are defined._
...
. Implementing a universal revocation mechanism is impractical due to the diversity of protocols across key providers. Revocation data may become unbounded, and managing it within the ACE unit would require some form of dynamic secure memory access. Such a function is better handled by isolated software components—such as enclaves, realms, or TEE applets, which are designed to integrate with the local software stack while providing strong security guarantees.
 ...
 * [.red]#The encodings presented here are just examples, for the moment RV64-only, and may not be final.
We currently use#
`[.red]#custom-0#` [.red]#for load-type instructions,#
`[.red]#custom-1#` [.red]#for store-type instructions, and#
`[.red]#custom-2#` [.red]#for operational instructions
 ...
 * [.red]#We could have used# `[.red]#L#` [.red]#in this document instead of# `[.red]#ace#` [.red]#but we haven't changed it yet since a different# `[.red]#misa#` [.red]#bit and a different letter may ultimately be allocated to the extension.#
 ...
 * [.red]#In case we allow to define fewer CRs, such as 8 or 16: Indirectly accessing a CR that is in the range [0..31] but is not implemented by the hardware  raises a invalid instruction error or have# `[.red]#aceerror#` [.red]#take one of the values ace_err_invalid ace_err_unconfigured? I favor the first option since that code should not run at all. If code knows the number of CRs-it is discoverable-, it should check whether the CR number is within the valid range before accessing it.#
