...
* Operations are performed _atomically_ to avoid leaking data through architectural side-channels.
 ...
 ** Not all the algorithms defined in the specification must be supported by an ACE implementation: Following Risc-V practice, we defined several sub-extensions and delegate the choices to RV profiles, which would be defined by a separate process.
