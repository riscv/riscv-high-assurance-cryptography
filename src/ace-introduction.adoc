...
. ACE provides a key-derivation mechanism that computes a _Locale CTK_ from the current CTK and a secret salt tied to a specific _Locale_ (as defined above).
Changing this _Locale Specific Secret_ (LSS) also changes the corresponding Locale CTK, automatically invalidating any Sealed Cryptographic Contexts (SCCs) that were exported with the previous LSS value.
...
1. *IP Interoperability:* It allows hardware blocks from different vendors within a single SoC to share SCCs.
