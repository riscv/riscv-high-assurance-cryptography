...
* *TBD: Several changed needed.* Use *WARL* rules for all our CSRs. Trapping should otherwise occur at the first instruction that depends on the values.
**FIXED FOR THE UNPRIV CSRS**

** UNPRIV/PRIV: *The CSRs lack WARL/WLRL discipline, and value-dependent write traps break save/restore. `*crstatus` write semantics are unspecified.*
No ACE CSR WARL/WLRL classification. Trapping on a *value* written onto a CSR is unusual for RISC-V (no, what about invalid `vstart` values), and here `acestart`'s legal set is dynamic, so context-restore or migration code writing back a saved `acestart` on an implementation that does not support resumption traps unpredictably.  *Assign reset values, classify the fields, and replace value-dependent traps with WARL behavior.*  Support for resumption should be mandatory?
