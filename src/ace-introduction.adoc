...
. ACE can bind an opaque key to a _Locale_, defined as a device, a class of devices, a specific hardware or software configuration, a single boot cycle (or, possibly, a specific time period).
...
*Remark {counter:remark}*:: [.red]#_Alternatives can be discussed: By using a configuration but, we can instead use pairs of 64-bit GPRs or quartets of 32-bit GPRs. We also have `ace.input` and `ace.output` instructions that load an input from memory, resp., store output to memory.
