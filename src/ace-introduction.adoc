...
The _Atomic Cryptography Extension_ (ACE) is a ISE that provides an interface to high-performance, high-assurance cryptographic operations.
...
. ACE allows software to perform cryptographic operations without getting visibility into the keys.
. ACE supports various cryptographic primitives and modes, including Side-Channel Attack (SCA) resistant versions.
. The keys can be provisioned to the software using them by other trusted SW or HW entities.
//. Once provisioned, the keys are reusable without further interaction with these trusted environments.
. ACE can restrict the usage of keys according to processes, supervisors, security states, etc.
. ACE can restrict the use of keys to _Localities_ to prevent replay across different machines, product lines, or execution contexts (see <<ACE-concepts>> and <<ACE-Localities>>).
...
* In order to be used, a Cryptographic Context must be stored in a special kind of one-way architectural state called a _Locker_.
Lockers are architectural and each hart has 32 lockers. ACE instructions use Lockers in place of the keys.
...
ACE minimizes environment switches, supports efficient software-managed context switching, and allows software to reuse CCs without having to create them again or, even to have to request anew their delivery from trusted environments with potentially significant delays.
