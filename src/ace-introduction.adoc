...
While a unified architected format is necessary, mechanisms for persisting Context Transport Keys (CTKs) across reboots or transferring them between devices are outside the scope of the ACE architecture. For cross-device migration, the use of a public-key key-encapsulation mechanism (KEM) is recommended.
