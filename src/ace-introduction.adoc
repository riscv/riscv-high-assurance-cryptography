...
* ACE performs cryptographic operations atomically, such as a full AES block encryption/decryption or a complete hash round sequence between message injections.
  This atomicity enables the use of keys held in _Cryptographic Registers_ (CRs), architectural containers that conceal their contents, without exposing the keys to software.
  This design contrasts with typical round-based ISAs, which require keys to be explicitly supplied between cipher rounds or MAC stages.
...
** Supporting only cryptographic primitives is insufficient for modes such as AES-GCM-SIV, XTS, and OCB, because leakage of derived keys or intermediate values can compromise their security. Hence, ACE also defines modes of operation and secure key derivation.
** Beside XTS, OCB and GCM-SIV, ACE also defines modes CTR, GCM, CMAC, which not only can be instantiated not only with AES-128/256, but also with other block ciphers like, for instance, SM4.
...
** Not all these algorithms and primitives must be supported by an ACE implementation:
 A minimal subset and several optional extensions are also defined.
* To prevent misuse, ACE associates the secret information with metadata that restricts the usafe of the former to a specific algorithm and can optionally constrain usage by process/VM, security state, and other policies. ACE then treats the metadata and the secret information as a single entity, called a Cryptographic Context (CC). A CC, not just the key material, is the information contained in a CR.
