...
* **Key revocation:** Current specifications do not include a revocation mechanism.
  While it can be envisioned that future versions may support revocation via key identifiers and system-wide revocation broadcasts,
  we feel that this feature is out of place in an architecture such as ACE and it would add unnecessary burden to the system.
  The reasons are:
  . ACE has been designed for performance, and flexibility (for instance algorithmic abstraction), and complex key revocation mechanisms (such as usage counters, revocation lists, etc) would easily add too much complexity and potentially affect performance.
  . The keys that are revoked are usually the master keys of hierarchical systems: TEE applets and HW IP key management blocks will then provide derived keys (such as session keys) to user software for most applications. In these cases it is only the SW/FW in trusted TEE applets that still uses the master keys, and they should handle key revocation of such keys themselves.
  . The cases where the master keys are used directly by software, and not for derivation, usually are implemented in higher-privileged components.
