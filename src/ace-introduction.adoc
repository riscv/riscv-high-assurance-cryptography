...
* To prevent misuse, ACE associates the secret information with metadata that restricts the usafe of the former to a specific algorithm and can optionally constrain usage by process/VM, security state, and other policies. ACE then treats the metadata and the secret information as a single entity, called a Cryptographic Context (CC). A CC, not only the key material, is the information contained in a CR.
...
[WARNING]
*Discussion point:* The question of whether resumption of import operations can be supported is still open (cf. <<ACE-incomplete-contexts>> and <<ACE-comparison-of-interruption>>).
