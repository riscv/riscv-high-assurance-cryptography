...
The instructions `ace.init`, `ace.export`, and `ace.import` perform direct memory accesses. This does not violate the Risc-V load/store architecture principles, as ACE is architected as an _attached_ unit that works together with the CPU core rather than being an integral part of the latter.  Consequently, it _may_ maintain its own architectural state and have its own load-store unit.  Also, these instructions can be considered specialized load/store operations that internally enforce the confidentiality and integrity of key material.
