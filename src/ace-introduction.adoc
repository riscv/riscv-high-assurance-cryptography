...
* To mitigate key misuse, ACE binds cryptographic secrets (such as keys) and
  ...
  algorithm and, optionally, to enforce usage constraints.

* ACE introduces _Context Registers_ (CRs)—architectural containers that
  conceal their contents from software. A CC is created by _provisioning_
  (i.e., writing) secrets and metadata into a CR. ACE instructions operate
  only on CRs. As a result, software can use keys without holding them
  explicitly in the process' memory, while also benefiting from higher
  performance and side-channel-protected implementations.

* Operations are performed _atomically_ to avoid leaking data through
 ...
 ** ACE supports symmetric and asymmetric algorithm, as well as hash functions.
 ** ACE defines modes of operation and secure key derivation for block-ciphers.
    Leakage of derived keys or intermediate values can compromise security.
 ** The interface supports SCA-resistant implementations of the algorithms.
 ** Opt-in support for different algorithms and security policies. Each
    capability is defined as a sub-extension.
