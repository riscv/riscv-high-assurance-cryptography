...
**Adversaries** are assumed capable of compromising processes or gaining unauthorized access to memory contents, either through software exploitation or by abusing hardware interfaces (including SoC/memory interposition). The model does not assume an adversary capable of sophisticated hardware attacks to extract information from internal registers.
