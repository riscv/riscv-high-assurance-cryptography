...
. A "secret" Cryptographic Context Transport Key (CTK) must be used for import and export.
...
. ACE provides a key derivation mechanism to compute "derived CTKs" from the (currently programmed) CTK and a secret salt associated to a specific lifecycle. When this _Lifecycle Specific Secret_ (LSS) is changed, the corresponding CTK is also changed and all the SCCs exported with the older value of the LSS become automatically invalid.
...
* ACE, in its current version, does not provide a built-in mechanism for key revocation.  Key revocation is a complex feature that requires additional infrastructure and coordination between entities, while at the same time being a rare event.  However, keys that may be revoked can be tied to a "current boot cycle only" or "refreshed whenever a revocation message is received" lifecycle -- there may be one or more of the latter type -- and can be revoked in a provisioning applet or IP block.  When the derived CTK for the key is invalidated, then any process using that key will need to re-obtain it from the provisioning entity, and if the key is revoked, then this process will fail.  However, keys that are still in CCs can still be exported -- a mechanism to revoke keys _currently_ stored in CCs is not yet provided. (A possible implementation would consist in assigning an optional 64- or 128-bit identifier to such a key and all the ACE units in the system would obtain a broadcast message to erase all CCs currently matching that field.)
...
ace.set_stage C0, #ace_const_encrypt
ace.error X1, C0
