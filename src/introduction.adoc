...
.. Besides SW, additional optional sources for keys are: (i) A System Specific Key Table (SSK Table);  (ii) External blocks for dynamic key generation.
...
.. Supported algorithms must be discoverable. There is a mandatory list of algorithms, optional architected ones, and an encoding space (in the metadata) for custom algorithms.
...
//
// into multiple instructions, and in particular the CC load/stores into sequences of shorter load/stores explicitly would make SW very complicated // and raise all sorts of limitations on how internally the ACE implementation can work. It adds complexity while adding no advantage.
