...
ACE is designed to address the needs of key use cases such as:
...
. Keys are made opaque by making them unreadable once written into an architectural state called "Cryptographic Context" (CC), together with some metadata that restricts the usage of the key.
...
==== Additional Features Implied by the Basic Feature Set

. In order to be able to save/set keys securely without having to call into some special HW/SW environment each time, ACE allows software to securely export keys to memory by encrypting and authenticating them, as a "Sealed Cryptographic Context" (SCC).
. A SCC can then be re-imported into a CC.
...
.Since we have public and private metadata, and keys (obviously private), we need to use an AEAD mode of operation to export/import (AES-GCM-SIV as per RFC 8452).
. A "secret" Cryptographic Context Transport Key (CTK) must be used for import and export.
. Since an AEAD mode of operation is needed, ACE can also expose it to user.

=== High Level View of the Feature
...
The inputs/outputs of the ACE Execution Unit (XU) are:
