...
*ADD* load/store of long additional input/results in clear (such as for public key schemes).

*ADD* mechanism for a CC to configure another CC, such as one CC getting a wrapped key, and then deliver it to a second CC or to memory.
Could this allow SW to run without V?
