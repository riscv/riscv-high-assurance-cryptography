[separator=::,top=25%]
= White Paper: Cryptographic Lockers Extensions (KLEE) — Hardware-Enforced Stateful Cryptography
...
To resolve this fundamental tension, we propose the *Cryptographic Lockers Extensions (KLEE)*, an ISA-level architectural framework designed around the concept of *Cryptographic Contexts (CCs)*. A Cryptographic Context is an opaque, hardware-isolated data structure that encapsulates cryptographic keys, algorithm state, and usage policies. KLEE provides hardware-enforced protection of these contexts within dedicated *Cryptographic Lockers (CLs)*, and secure export to memory via *Sealed Cryptographic Contexts (SCCs)*, and later import of SCCs into CLs, using authenticated encryption.

By executing cryptographic operations atomically at the primitive level (e.g., full block encryptions or hash rounds) and referencing opaque Cryptographic Lockers instead of raw keys, KLEE significantly reduces key exposure in memory and prevents state-manipulation attacks. KLEE provides a uniform, algorithm-agnostic ISA interface that scales across heterogeneous cores, preventing instruction set bloat while delivering HSM-grade security at CPU-instruction speeds.
...
Historically, high cryptographic throughput has been achieved through the use of accelerators,
for instance the IBM 4758/4764/4765 Cryptographic Modules cite:[SMITH1999831,IBM4758copr.955100] or the Sun Cryptographic Accelerator PCI boards cite:[SUNCA1000].
Versions with high security of such accelerators exist as well,
such as hardened modules cite:[HSM-Overview] and secure processors for embedded applications, which can provide strong protection for cryptographic keys,

// high-assurance environments have relied on Hardware Security Modules (HSMs) cite:[HSM-Overview] or secure coprocessors
// along versions optimized for high performance.

While highly secure, and often fast, these systems suffer from significant invocation latency and setup overhead.
To reduce this latency, modern SoCs can rely on Trusted Execution Environments (TEEs) or secure enclaves, e.g., ARM TrustZone cite:[alves2004trustzone], Intel SGX cite:[intel-sgx], and perform cryptographic operations directly in the secure environment.
...
Complex modes of operation, such as XTS cite:[nist-SP-800-38E], GCM-SIV cite:[RFC8452], and OCB cite:[DBLP-journals-joc-KrovetzR21], to make a few examples, rely on the secrecy of derived keys, initialization vectors (IVs), and intermediate states (such as authentication tags). Exposing these intermediate values allows attackers to bypass confidentiality or forge messages without recovering the master key. Key-hiding-only ISAs fail to protect this runtime algorithmic state.
...
Instead of treating cryptography as stateless operations on raw keys, KLEE models cryptographic execution as stateful operations on *Cryptographic Contexts (CCs)*.
...
. *Machine State*: Intermediate values, IVs, and state machine stage.

Once provisioned, a CC's internal structure is completely opaque to software. It resides in dedicated, non-addressable *Cryptographic Lockers* and can only be manipulated via well-defined architectural instructions.
Only the metadata and certain algorithm state information can be made visible to software, while the rest, including key material and other internal derived state are kept hidden within the hardware, or exported in a secure way–by means of authenticated encryption.

=== -- {nbsp}{nbsp}KLEE Instructions and the KLEE Unit

KLEE decouples the ISA from specific algorithms by defining a minimal, constant set of instructions. The two primary instructions are:
...
The KLEE hardware unit can be implemented as a closely coupled core pipeline or as an attached accelerator shared among multiple heterogeneous cores, maintaining independent architectural state per hardware thread.
...
By design, KLEE eliminates traditional cryptographic attack surfaces while maximizing performance:
...
* *Side-Channel Mitigation*: Because algorithm execution is encapsulated within the KLEE unit, hardware-level masking and countermeasures can be implemented transparently. Software requires no modification to benefit from these protections.
...
* *Machine Agility & Future-Proofing*: The ISA remains constant. New algorithms (including Post-Quantum Cryptography) can be supported by updating the KLEE unit (either the hardware, or its microcode) without adding new CPU instructions.
...
1. *Unified Formats*: Standardize the binary representations for Cryptographic Locker initialization and Sealed Cryptographic Contexts (SCCs) to allow secure migration across heterogeneous systems.
...
The Cryptographic Lockers Extensions redefine hardware-accelerated cryptography by shifting from stateless key-hiding to stateful, policy-enforced execution. By encapsulating keys, state, and metadata into hardware-isolated Cryptographic Contexts, KLEE delivers HSM-grade security at the speed of native CPU instructions. Standardizing these interfaces across the industry will pave the way for secure, high-performance, and agile cryptographic infrastructure across heterogeneous cloud and client systems.
