...
* Internal state: {metadata, key}.
 ** metadata: 64 bits
 ** key: k bits (8|k, in fact it is almost always 128|k)
* Block Size
 ** b = block size in bytes, (8|b)
 ** The configured vector size should be equal to the block length
* Generic Functions
 ** `metadata_deserialize` as above.
 ** `metadata_serialize` as above.
 ** `initial_content_deserialize`. input[k/9-1:0] = key, where k/8 = key length in bits.
 ...
 ** Behavior
  ...
  *** In the encrypt state `f` = `encrypt_block`.
  *** In the decrypt state `f` = `decrypt_block`.
...
* Instructions
 ** The configured vector size should be equal to the block length.
 ** If `ace.exec  Vd, Cn|C[Xn], Vs` is issued, then `Vd = f(Vs)`.
 ** If any other form of `ace.exec` is issued, then `ace.error` will return ace_error_invalid.
...
In the "non-X" modes the lengths of IV and counter/LSFR add up to the block size of the underlying keyed PRP or PRF:
IV and counter/LFSR are concatenated to form the input to the keyed PRP or PRF.

* Internal state: {metadata, key, IV, {counter}}.
 ** metadata: 64 bits
 ** key: k bits (8|k)
 ** IV: n bits (8|n)
 ** counter: j bits (8|j)
* Block Size
 ** b = block size in bytes, (8|b), b = n+j if not an "X" version, OR b=n=j if an "X" version
 ** The configured vector size should be equal to the block length.
* Generic Functions
 ** `metadata_deserialize` as above.
 ** `metadata_serialize` as above.
 ** `initial_content_deserialize`. input[k/8-1:0] = key.
 ** `content_serialize`. output[k/8-1:0] = key, where k = key length in bits, output[(k+n)/8-1,k] = IV.
 ** `content_deserialize` converts the output of `content_serialize` back to the original input format.
* Algorithm-Specific Functions
