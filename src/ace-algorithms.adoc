 ...
 ** `initial_content_deserialize`. input[n-1:0] = key, where n = key length in bytes.
 ** `content_serialize`. output[n-1:32] = key, where n = key length in bytes.
 ...
 * State Machine
 ** States: initial, encrypt, and decrypt
  *** In the initial state, `f` is undefined and any execution of `ace.exec` will result in `ace.error` returning ace_error_invalid.
